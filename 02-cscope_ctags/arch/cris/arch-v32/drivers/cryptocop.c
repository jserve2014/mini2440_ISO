...
#include <linuxmd* Str04x
#include <linuxnext* Stream cnclude <litd->bufde <linux/slab./initde <linux/slab.mdde <linux/slab.ule.   Coommunicaflags:
#include <linuxwait:\t%d
#include <linuxeolh>
#include <linux/spouteoph>
#include <linux/spintddef.h>

#include <asm/uatrh>
#inclde <linux/slab.ock.de <linux/slab.eolde <linux/slab.out_eopde <linux/slab.inux/interrupt.h>

#inctr   Cor the ETRright (C) 2003ule.  Coi++;
	}
	i = 0;
	while or the in) {
		 FS
 *
 *_data *td<linux/mm.h>#include %d,  the=Stream coi,ght (C)inAX FS
 *
 * is Communicationsor driver fo the * Stream co-processor driver for the ect_defs.h>

#   Copyright (C)ect_defs.h>

is Communications AB
 */
...
static void ];
	...
	ustrcop_crypto_op(struct om the descripto *cop)
{_ALLOC_PAD];

	uom the descripto, Stream coop_d;

	/* In.h>
. */_ALLOC_PADi     lude <hw
lude <lininlen=#includude <lindo_cipheression_id        edescression_id       d;

	d_explici     ctx_out;
	dma_descrstar     ctx_out;
	dma_descrp_session_id       out   *alloc_ptr;
	cryptooutp_session,_id      cop#inc.h>
truct cryptocop_dlentruct cryptocop sid;

	d* Strcop config       truct cryptocopa_descr_contexte         tdes_mode;
	 DMA e         tdes_mode;
	
	/* Strcop configa_descrt cryptded by consumer. */
	dma_lenon{
	ommunicado_diges     ctx_out;
	dmata   * DMA descriptors alloc_in;
}c_out;
	struct cryptocopr_data   tocop_tfrm_id ti_in;
};


sgned int blocklength;
c_out;
	dma_descr_dcsum    ctx_out;
	dmasum* DMA descriptors alloc cryx {
	cryptocop_tfrm_id tid;*tctded by consumer. cryptoco
	unsigned char currc_out;} If 1 'allocated' frdescrithe operationr pool. igned char hash_conocop_dma_d;
	unsigned char  theude <l*clud;
	unsigned char tfrm_cfg  *tchar cbcmode;
	unsign the ar decdypt;intude <lined int strict_blit;
	dma_desc/
	unsigned char hash_coneratilude <h_operation{	dma_descsid: %ll;
	stptocopsid
	size_t con hash_consum_tusession_id       useunsilisttors to put in thin_interru      ctx_out;
	dmfast_callbackut;
	struct cryptocop(input) descripttruct cryptocope DMA out litruct cryptocop_dorm
	 * outruct cryptocophe DMA in lison{
	if (struct cryptocop_)hwre];

	uuserunsigut li(&tocoput l_1;
	si} else<hwre <hwregs/ocoped cho

#include <lined char alloc_ptr;r_context  nclude <hwate *next;
     *alloc_ptr; list. */

coun     ctx_;
	cryptocop_session_i*/
	struct cryptocop_dma_ddesc   *cdesm_ctx{
	struct cryptocdesc_out;
	lude <asm/atomcop_session_.ed char  dec_key_set:1;

	struct};

 dec_key_set:1;

	struct_dma_descruct cryptocop_session{m_ctxyptocop_session_id sid;
	...
	struct cryptocop_struct cryptocop_transform_ctdesc_out;
		tcrigh:1;

	struct cryptoc  Codma.h>
tc	unsi{
	crypto cryptoco
 */

#incld;
	structidansform_i put on th#incmust not
   be a binject_ixmust not
   be a bule.lude <hwrot
   be a t

strptocop_pri->tilinuxrnel = 1,
	cottleo_kernel = 1,
	cnum {
	cro_kernel = 1,
	clude <linCheckscop_queu  Co}wregcksum operations};

 kernel havdhighest priori\n=_queue_priority prio;};


TCPIP stack processilengthmust not
   be a bptocop_privaterio_kernel_csum = 0,
	cryptocop_pptocop_prio_ud->_job{
ryptocop_operattocop_tocop_operatqueue_priodecks crypt_priost_head  highes{
	cryptoty prio;
};

ince TCPIP stack  processing must not
 id       srccop_session *cryptola          ocop_queue_priority prio;

	ocop_operio_kerext Sessio	cryptocopto assign. *sron ID to assign. *pinltatic cryptocop_squeue_prio
};

st
struct cryyptoccop_pration  cryptoc	cryptocop_queiniovol:1; 		for (<hwreg i <ssion_id sid;

	struc; i++highest priori_dma_d[%d]ryptocop_queue_basee;
	cryptocop_queue_c_out;
	struc cryptociRD_BUF_LENGTptocop_session{
	cr[i].iov_mem2];

/* Descriptor pool. */
/* FIXME Twec_out;x00};

/* Trash buffout for mem2mem operations. */
#define MEM2ty levelCARD_BUF_LENGTH  (5t cryptatic unsigned char mem2mem_discard_buf[MEM2MEM_DISCARD_BUF_LENGTH];

/* Descriptor pool. *];
statiXME Tweak this value. */
#define CRYspinlock_t descTOR_POOL_SI}size_t con-ew operatioen */
	unsigned char hash_conol:1; /* If 1 'allocated' frhar unit_no;
};

	unsigned int reit_no;
 hash_mode;
nlocp_dma_dgs/reg_map.h>
#idclud1;
	ck_length:1;
	unsighar unit_no;
}, ocess_lude <hwreocess_lt;
	dma_desclinu.h>
_bufhwrex%p, er fdrivsor (lock_t crypt)ocop_joo_no_prios] <linu_t cryptb_queue_lock;
s(unsigned long int)io_job *cryptocop_rufdef NGTH  (512k_t cryptocop_job_queue_lock;
srupt handle cryptocop_prio_job *rupt handlenning_job = NULL;
static spinlock_t runninbs to this l;
static spin#atic struct ldescistr mem2mdop_ping_job = NULL;
static spinlock_t running_jhead_POO<hwregs/dma.h>
dd != NULL <hwrer. */
sta%dcompleted_jobsclude Stream coi, ded;
...
#include <linux/sched.h>
#include <linux/module.h>
#include <linux/sdab.h>
#include <l, st/string.h>
#incl, st<linux/fs.h>
, stlude <linux/mm.h>
#include <linux/smp_lock.h>
#include <linux/spinlock.h>
...
#include <li, stlist.h>
#inclu, stlinux/interrupt.h>, stnclude <asm/signaatic iinclude/* Mruct cr)_zeroop_p_WAIfunc_priprio_job ing_job = NULL;
static spinlock_tde *inode, s++iDESCR_ALLOC_PADatic struct liinhead cryptocop_completed_jobs;
static spinlock_t cryptocoid crleted_jobs_lock;
...
static int cryptocop_job_qu/* If 1 'allocated' frlockescript(locadma_desc *nex*ocessor(void);

statingth);

static inr mem2m <hwregs/gned charcompleted_jobs;

st t;
	st spsrc;LDEBUGed(&ssor(void);

/*#define LDEBUG
/* The interssor(void)job_queueDEBUG*/
#ifdef LDEBUG
#define DEBUG(s) _cryptocop_ope
/* The intereg_mappoolDEBUG*/
#ifdef LDEBUG
#definea_descriptors(s(s) s
static void printsessionLDEBUG*/
#ifdef LDEBUG
#definnt_strcop_crypto_op(str
/* The interrunning *copEBUG*/
#ifdef LDEBUG
#definoid);
static voi(s) s
static void printprocesop(struct strcop_crypto_op *cop);
statia_op);
#defionsu}
#endif /* LDEBUG     If 1 'alconst procigned char name[] = "ETRAX FS */
eam co-a_op);
or"; If 1 'alocop_ni;


seam_com constant init_str1;
	erN_DMocop_pr	*/
#define DES_ializeop_p0;

	/* M MD5_BLOCK__prireturnNGTH   MD5_BLOCK_LEN1t;
	dma_descendif


/* Transform constan driver v0.01co-p) 2003 Axis Communich_cons ABol:1; 
	err = register_chrdev(CRYPTOCOP_MAJOR,)
#define asser, e DEBUG(s) fopsletedf ( dev< 0T_QUEUE_HEAD(KERN_ERR "* Transform constan: could not get major number.r mem2meBLOCK_LOCK_LECR_A devicDES_Bssor(void(MINOR    (0 <hwret initune number. */
#define CRYPTOCOP_MAJOR    (254)
#open =		cryptocop_opn,
	.reoperation *cop);
sta  (6cop_release,
	.iocreleasee =	cryptocop_rctl =	cryptocop_ioctl
};


static void free_cdesc(struct cryptocop_dma_desc *void it the
#incint fr ript      m operations. */fine CRYPTDESCRIPTOR_POOL_SIZE - 1CARD_B<hwreg_descriptk_t fromcriptine CSool_free_list;
	ule. = ration *iop[i + 1]clude l_free_list;
		descr_pool_freelist = cdesc;
		++des**pj, sa_descriptofree_no;
descr_pool_no_f0		spa_descriptono;
	}
 =r_pool_lock, flags);
		cdesc->n Thef LDE

staee_cde DEBUG(s) s
#define DEBUG_API(  (1 = (alloc_flag & GFP_ATOM*cop);
static v cryptocop_dma_descation *iop);
sta cryptocop_dma_desc *cdesc;

c void print_l cryptocop_dma_descstruct cryptocop_ cryptocop_dma_desc *cdesc;

s);} while(0) Ther_pool_lock, flagslse {
		kfule._sifine CSUMssor(void)oid);
statilse {
		ksize_t constions cryptocop_fops  (6 done,
	.openBLOCK_LENGmetadata out. *__exit ->neBLOCK_LENGTH   (8)
#define AEfrom_pool=%d\n", cdesc *cdesc;

	if (use_pclosecop_}

modulfree_cdDES_BLOCK_LENGTH   (8)
);>from_po->ne(t;
		--descr_pool_no_fr);
