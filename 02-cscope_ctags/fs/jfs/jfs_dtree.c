	 ...
	 struct component_name * key, ino_t * orig_ino, ino_t new_ino, int flag)
{
	...
	/* get the head/only segment */
	entry = (struct ldtight (*) & p->slot[ight _si];

	/* substitute the inode number of  Thinal Buspyright ->iram is = cpu_to_le32(new_ino) 2000-2unpin  Thileaf pagee;  yDT_PUTPAGE(mp
 *  return 0;
}
