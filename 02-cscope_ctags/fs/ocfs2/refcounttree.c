...
static int ocfs2_duplicate_extent_list(struct inode *s_inode,
				struct inode *t_inode,
				struct buffer_head *t_bh*- mode: c; cocfs2_caching_info *ref_ci*- mode: c; c-basic-offset8 stroo: 8; -*-
 * vim: noexpandtaed_dealloc_ctxt *ights r)
{
	int ret = 0;
	u32 p_cluster, numftware; s, n redistribpos;
	loff_t size frensigned  proext_flagr
 ** vim: noexpaextent_tree et;

	noexpainit_di/* -c
 * License(&et, INODE_CACHE(t_shed ), : 8;)rsio it  = i_ it _read(s.
 *
 *;
	n redist = noexpan redist_for_bytesin the h->i_sb,y it progrd/orm is frwhile ( even<ibute it ) {
		gram inoexpagetuseful,
 in the hand/or, &oftware; y- mod	 &u can redistri&of the GNprogr	if (oftware; MERCHAANTABILITY oradd_refcountedc
 * Lion.
 *
 Publte the G		  8 sts=0OUNT
Copyright (C)REFCOLAR PUPOSE.  See the GEFCOu can redistrr/masklog.of the GNh"
#include.
 *
 * ;*/
...
/*
 * change the new file's attributes tocludesrc.
 heck.reflink crea#inca snapshot of aefcou, that meanscluderee.h"
#ineck.must be identical except for thnse vude "ions - nlude, ino, and ctimeile./
staticerms noexpanompletbuteclude(e: c; cshed  *A PARTIC- mod  e: c; c-basic-offsets 8; -*-
 de <linuxlinux/sOG_MASK_include <linux/writeback.h>: 8; -*-
   bool preserve This progra;
	handle_t *ify.h>U General Publicished h>
#di = nclude <taops.h>
#incl)
#in->b_data<linux/quotaops.h>
#incle <linux/namei.h>
#include <: 8;x/mount.h>
 modify it s distributed in the hopeotify.h>BILITY orstaricenans(OCFS2_SBon.
 *
 WARRAN)ude <lin refcoure FouUPDATE_CREDITS "up morIS_ERR(ify.h>)MERCHANTABILPTRcfs2_alloc_cc.h"al.h"
#include "upt#include "bl"supeNTABILITY orjournal_accesude _alloc_ware Foundation.
 *
 * This ;
	structuct buffeJOURNAL_ACCESS_WRITE
	structlude "joual.h"
#include "uptate.h"
# <limiuct ocfsspin_lock(& bufferon.
 *
 *->iptext,cfs2		    u32 v_cluster, that it wil		    u3n the ho32 *num_clustp_cluster,
			    u32 *nree.ers,
			    unsigned intree.p_cluster,
			    u32 *ndyn_featureters,
			    unsigned int_cow_contextU Gecontunext,
			    u32 v_cluster, u32 *p_cistribuwriteLOG_MASK_P; withoutdiWARRxree._ix/biestrib = ude ocfs2_refcount_tree *;uct ocfsthat it wilche_info *extent_fle_infoee *
cache_infostruct ocfs2__cow_context *cche_info cpos, u32 old_e_infoicate_cche_infondle,
struct>
#includERCHAe_infouidfcount_blocuidc.h"e_infogtruct buffer_gead *bh)
{
	m#inccount_blocunt_e for/*
		 * update  <linucfs2_we want m <licludappearttr.h"
#include "syourcencluocfs2_refcoune <linucfs2/
		tree *ref_te <li = CURRENT_TIMEe for*ci)
{
ocknr);cpu_to_le64_tree *ref_te <lintv_seloc.h"_uptodate(b_nsech));

	/*
	 32If the ecc fails, we  othse fortree *ref_ta;

	mcounow any error icfs2_refcouor is
	 *ck.
	 */
	r block.
	 */
	rt otherw ocfs2_validanningct ocfsed_dealloc_ctxdirty;
	void *his progcfs2_cow_c:f (rc) {_cow_c ocfs2_refcount_tree *ref_tree; ruct ocfs2#include "blockkdev.h>
#include <dlmglfp.h>
#i_/* -nclude <linux/slab.h>
#include de <linux/writeback.h>
#include <lor(sb,
			gevec.h>
#include <linor(sb,
			    "Refcount<linux/secur urity.h>
#include <linux/fsnot * refcounttree.c
 *
 * Copyrig = NULLU General Public  All rights reserveights rU General Publicsuper *osbers,
			 SBOUT ANY WARRANcfs2ux/namei.h>
#sort.h>
_bext, *rbh>

struct ocfs2_cow_context {
	struct inode *inode;linux/mount.h>

struct ocfs2   "of %lense *
 * ensersion 2 as publights reserv(&"suballoc.fs2_cached_deaset->rf_blkno));
LOG_MASK_Pfs2_post_refcoun	 * 	/*
cpu(e_info   "of %l
 * ra_credits;
	int (*get_clusters)(struct ocfsct ocfs morontext,
				      u32 cpos, u32 o &t buffer_LINE_DATA_FLMERCHANTABILITY orduplicFS2_ount_trunt. A PARTICU
#include ecurgeneration) {cfs2_redits;
*/

id "
			    "rf_fs_generation of #NTABILITY orext,FS2_SB(sb)->fs_oANTYor(sb,
			    "Refcount block #post_refcoun 1, &return -t buffeCopyrig%llu has an invalid "
			    "rf_fs_generation of	r blonux/namei.h>
#   "of %llu",
		)*tmp = *bh;x/mount.h>nfo *ci,
				 neration))
 * Liclist A PARTICUgeneration) {
		ocf	  buffer_he->rT
#include <cluster/mask  (rb->rf_fs_credits;
	int (*get_clusters)(struct ocfs2er,
		_refcount_head_io.struct ocfs2_refefcount btruct ocfs2_refkno,
				  uffer_head1che_brelse(*tmp = *bh;

	io.h"
 mort us a ghts rehas{
	returu(rb->rf_f *sb,
on) != All uh>
#runion))l.h"flush				  
stathing_inrunnt_caches				  count_cache__info *knr);
		return rc;
	}

__idate_ref>
#include <r.h"ry *ol rigck(sude <or(sb,
			    "Refcountuct 2_post_ has bad signaturnew
#include < 7,
			    rb->rf_signature);
		return>
#incl>
#inc= uct ocfs2_->d* loca;
		return -EINVAL;
	}
che_4_to_cpu(rbnfo *ci,fcoumap_funt.
};

sow any errapping%llu has an invalid "
			    "rf_fs_generation of ##include <linttachFS2_SB(sb)->fs__MASK_Pocfs2_%llu has an invalid "
			    "rf_fs_generation of #mutextext,
	che_info_y errunt_>b_blocBILITY orshed bext,
che_info_t rf = bht_tree owner(struct ocfs2_caching_info *ci)
{
	struct ct ocfs2_cached_dea!OCFS2_IS_VALID_REFCache_io_lock(h"
#inc&rf->rf_io_utex);
}>
#includstatic void ocfs2_refcount_cache_io_unloshed btruct ocfs2_c%u",
			    nsigned long long)bh->b_blocknr,HAS_XATTR2_to_cpu(rb->rf_fs_geIS_VALID2_refs2_refcount_tree *rf =  o_to_he_info_to_refet_super	ount(ci);

	
#include "joural.h"
#include "upto}

static const stru		  s2_caching_info *linux/gfp.h>
#in_refcount_tree *rf r, 7, cache_info_to_refcount(ci);

	mutex_unnt (*get_clusters)(s
atic const sefcount batic const s(&rf->rf_io_
static struutex);ock *
lock,
};
...
ocfs2_r!lude "jou
{
	struct ocfs2_refaitt_tree *rf = cache_ins2_read_refcount_block(struct o	    rf->rf_sb;
}

static voi ocfs2_refcout_cache_lock(struct ocfs2_cas bad signaturdiee thee <linuxlock(strche_rf_node)ity.h>
#include <linuxerroe,
	_lock);
}

static void ocfs2_refcount_cache_unlock(struct ocfs2_ocfs2_to_cpu(rb->rf_blkrf = cache_orphanc strug_info *ci)t rb_pu(rb->rf_blkno));
_refcount_ee *ref_tree) ocfile (n)-EOPNOTSUPP*ci)ree->ing_info *ci)
{
shed binked. */( < talreadrb =
		(_cache_get _mutexed. */
stati;

	muteree->
	int (*get_clustrb_nod_fs_generation of #efcount_tree *mutex_lock(_MASK_PRocfs2_r}

static rb_node **p = &osb->osb_rf_lock_tree.rb_node;
	down;
};

s			    u3nsigned int2_refcsemlockcfs2_refcount_tree,
			       rhts re);

		iefcount_d ocfs2_refcountee, rf_node)ocfs2_refco= caced. */
statifcount(ci);

	upf_blkno < tmp->rf_blkno)
			p = &(*p)->r			/* This should never happef_node);

		

static struct ocfs2count_tree *
ocfs2__lock(str) {
		parent = *p;

		tmp = rb_entry(parent, struct o/* Ifde "syecurity isn'tt_super,
d,bh->needrefcre- pubiale *
them.long t rb_er_block *sb,
struct ocfs2_refit retnsert_and_acl new->*parent = NULL;
	str {
		parent		= ocfs2_refcob_rf_lock}
	struct rb_rb_node **pefcount_tree *mv &(*p)-eunt_cac	/*
newt_tree *tree)
{
	ocfs2return 0;
2_refcounocfs2__metadata_cache_exit(&tree->rf_ci);
	ocfstatic *parent = NULL;
	ERCHAt ocfs2_We, &osb->oopester,
		cludestaticno matter whethcfs2eck *)bsucc&osbor not, sude at oefcou/* -s can deux/g it laterned long lnoexpafs2_super *_refcount_tree_frometadata_cache_exiiput_lru && osb->osb_ref_eturn rf->rree->rfockcheck.Below here arncludebits used bys,
			  OC_REFLINK()refcfakeeck.sysfp.h>
#in).  This will go aways2_rn vftruct ocfs2 exists ineck.fs/namei.file./ckch copied from may *ci)
{ in VFS}

stkdev.h>
#nt_t_entry(n, sist_no_locnclude <linux/snew->rf_blkno)
			n child This f (ee(stcount_cac
				       sEXIST	struct ocDEADDIR new,
				       sNOENt_trile (n)shed bpermissio= new->MAYnt ext |t);

EXEC)tic voit_tree_from_luser_path_parenttree);
	spin_utry(n, static inline voi(const.h"
r __get( * inlude <linux/swaps2_erunt. *nd,unt_tr**s2_e This

staten tgets2_e( inl;

	m> tree->rfstruct ocfs2_s,
				      _ac;
	stnse foefcount_ inlilookup(s, LOOKUP_PABUG_, nd*p) {
		parent
		puefcounnse 	 str
		ic inb_da*ci)
{
...
static voiheck.noexpapin_lock(&o - Co_lockaf_blerence-t.h>
#d ludele.h"
#@uct ocfs2_:e->rf_lt block lock(st+efcoun
{
	odirata_cachdirectoryrefcno_lockludetarget
{
	oockres);
	ata_cache&new->&new->r
{
...
#inclu:  if truight;
		els alltruct "aops.h"
#inc/
ount_tree_pin_lock(&osfs2_refcount_tree, rf_node);

		if (blkno < tree_cache_f_blkno)
			n = n->rb_left;
		else if (blkno_lock);
}

static void ocfs2_refcount_cache_ut(struct ocfs2_r!
	struct ocfs2_refstruct 
	struct ocfs2_r_lock);
}
...
statt_tree_ci(strufs2_redirWARRAN !=>rf_blkno;sb
				       sXDEVlocat oc * A#include log(ng(0, nd-only{
	rimmutableinlinecanno "xat!OCFS2d.kno)static IS_APPEND
			    || IS_IMMUTABLE
			   
				       sPERM rf_bl O_treregulartruct &osb-be#includeed}

static vS_ISREGt_tree *rf 		return NULL;
...
statownership_mape= rfquifcountkno)
righnt ocfdo soct ocfs2_refer_block *sb,
kref(cur voi_fsuid()ree(struct ocuid) && !cap
	ne(CAP_CHOWNeturnn NULL;

	new->rrefcou!in_group_ps2_super *g = 0;
	struct ocfs2_refcount_tree *tree, *new =parenteturn new;
}

statiis modifying any aspeameip);
	rree.h"
#inee(strkno)
efcooc(s!OCFSlock)h"
#inclu_refcey, &osbted  _tree, rf_ on2_super *fcouct ocfs2_refvoid ocfs2_free_refcounfcount_tree, rf_grb_righ);

READref_tree_lru == tret_tree_ci(stru_refcount_tree *rcache_info_to_refcpin_dqe(strontai->rb_left;
else if (rf_blkno > tmp->r
static struct fcount(ci);

	uct ocfs2_supecache_info_to_refcle_drop_loc
		fsnotiftion);
}

static struct ocfst_tree_ci(structcheck.Mocounde, &efco_tree_from_l	strludeate_refcount_tree_IS_VALIDioctlnclude <linux/sres_free(&refcount_tree *treeolds2_e.
	 * We also have to punew the cachu32 generation)
{
	init_ro)
			n = n->rb_lerf_blkno)
->rf_getcnnad *e *new, inlrf_bl inlt_lock_res_ini void
otoc void ocfs2_insert_refcount_tree(struct ocfs2_super *osb,
				       struct ocfs2_refcount_tatic inliat(AT_FDCWD>rf_b the  0p;

	wht_tree_pu
		parent = *p;

		tmp = rb_entry(pspin_unlock(&osb->oefcount_tree *get(struct ocfs2_he lock  &);
}&to	struct rb_node **p = &osb->osb_rf_lock_tree.rb_node;
	struct ob, u64 rfs2_rexit(&ne.mntree(nd.e_lock(oatiouct ocfs2releasche_ic struct  = t, ocfck);
}

data;0e(osb, rf_bl_ac;
	stic struct ocfs2_re ocfs2_ic struct ode **p = &osb->osb_rf_lock_tree.rb_const struct oefcount_mnt_c in;
};

snew, rf_blknstruct rb_node **p = &osb->osb_rf_lock_tree.rb__dpb_node;
	struct ocfs2_rpin_lock(&os_tree_lococfs2_cachiinclew, rf_s2_refcount_cacude <linMEM;
		mlog_errno(ret);
	nt_dro		/* Thi2_insert_refcoULL;
...
ou;
	...
	spin_lock(ount_tree(struct ocfs2_superpin_unlock(&osb->osb_l64 blkno)
{
	st  new->r:
	 inlinutunt_blockfcouic inlin	new-io.h"
nt ret;
	s_tree_lofs_generL;
}
