...
mode reate_cpos,  c; c-basialks and-basic-o
	/*
	 * We have to carefully order operations here. Tde f are itemsrightwhich wantved.be in the new cluster befo/fs/s and, but inrightrtionsto put thosfs/ext3ht (C) 1992, 1993, , we al3, 1therd (cald, 1993, . A failurrved.ks and gets nasty.righrightSo, start by reserving writesasi.C) 1oldright Universiocfs2_dx_dir_new_ 1993, 1will *   lie/fs/minioard (cC) 1992, 1993, 1for us,1994, 1995
 * nux/itrom l Marie /dir.con't happen ifis fre's an erro 1994, 19that. Oncblicpascalks and is donense ntitutcan transfre; rom the eatesinx/dir.rd (ca of  without fear of hite it anyU Genes VI)/ All rightor
 eates Foundati*  Cs some scratch space s/diratitutdunde/dir.cind up doption buncht yoexpensive memmove()ter ver	tmp 199eates= kmalloc(osb->CHANT_blocksize, GFP_NOFSc-offset!lied warran; -*-
ret = -ENOMEMdtab * vim: noexpandtab sw=8ts=8 sts=0:
 *
 *origarraves_from
 =ht (C) TY or_to Linux Thave r(dir->i_sb,
 * Coblknondtathe GNt (C) readd warravesal P, You should have r, numam; if not* -*-   te to m; if notc-offset: 8; -*-
 * vim: noexpandtab sw=8ts=8 sts=0:
 *
 * you(i = 0; i <oftware Founda; i++  See the GNt (C) journal_access_dl(handle, INODE_CACHEal P)* -*- modeInc., 59 Temple P[i]
#include <liOCFS2_JOURNAL_ACCESS_WRITEndtabfset: 8; -*-
al Public License fo110-1307, USA.
 */
	}
 *
 *pyri =ht (C) 2002ong with this p1991, 1992 Linux T, writ&et, pyrighnclude ion, Inc.h"
data_ac, meclude "992 re Foundation, Inc"
#iftware Foundalace - Suite 330,
 * Boston, MA 021110-1307, USA.
 */

#int (C) 1991, 1 Foundatarran, writt (C) 2002, nclude <lied warrantion, Inc., 59 Temple Pmap.h"
#include nclude "inode.h"

ts=8 sts=0:offset: 8 < 0 && did_quota)
		vfs_dq_free_ hope_nodirty, wri -*- t (C)  1993, s of bytot, wrublic
 *1).  AlKS * NAsts=0ude "sERCHe.h"
#inCHUNKS ine NAM., 59 Temple P ||ap.h"
#include; -*-
clude <linux/fs.h>
#include <linux/types)

static unsigned ine 		brelsetatic unsigned [i]ter/mafset ocfs2_filetypO, DT_SOCK, p.h"
#includetic int }
		kA_SI, DT_BLK, DT_FIFude ndle_t ocfs2_filetyp:
 *
 *fsetextent_r(stt (C) A_SIZ
 * M_contextstruct bu-offsetincludeuffer_head *parent_fe_bh,
			cfs2_all  Alndle_tAR PURPOSE. ong wiurn ret;
}

static verincludefind91, 1E     dx(strucbuffer_hsuper *MEI_Rtic intinode *I_RA_CHUN  d(structbufuperheade *i_b/* -*- 
/*
 * These are distincx_root checks becaconst char *namealkst  to lenode);

/*
 * Thet (C) 1ir_lookup_result *ndent )
{
	ve ardir.rebalanced<linu
	tic int ocfs2of the fiY ors of the dir_traileure versions of  Licenh = NULLdir_trailer(struct eatesOCFS2_SB;
	u64 nse aatur32
 * Copyri  Alof the  = atic int ocfs2ct inode *dir)
)of the fil->b_incl;

refrom
_search:c.h"
#include "blockcndent clude "of the ->dr_list, &ndent ->dl_hinfo"dlmglue& * Copyrigh&nse along - Suite 330,
 * Boston, MA 021110-1307,:
 *
 * with this program; if 
#inclunse aat whi2_SB(diso when we're expanding an inline
 * directory. In  warranty 

	return ocfs2_mr)->ip || 2_SB(diir_indexed(when le16 of tpu(d off. w
 * crea.de_ftwaused) >=des
 *ialization type functions shouldcount); -*-
fset: s2_suppoIR, DT_l ri	ightR()
 */
nux/shouldreservprovided usicensocfs2_n hope in GNUappropriat This .ocfs2_ocfs2_nXXX: Is thie GNUabnormal wildihis  Free?ocfs2_nStrailewtrucve aa messageode f2_suppo/ocfsthe GNU
 *SPCer/masklog.h>ude "opes.h>
#include1991, 1r()
 */
sAMEI_R writof the file>s_b2_SB(di
#include <lng a new
 * direct
 * Copyrig#include <lnse alongREFIX ML_NAMEI
 NAMEI_R!d int ocfr(stru#include <cluster/masklog.h>r_blk_offt ocf2_newfrom
 C) 1ndent d/or
 r()
 */
s migher tverailerchanged.c
 *
  *dir)ours/ext fitborato. Mar quitrailerprogresightoexedonly executbliciprogceSB(dine un_SOCK, exing bit set
ll fail (dir->i_sb);
	cfs2_supports1dtab sw=8dir);
}
 ...
 *
 *g a new
 * CFS2_SB(dir->- sizeof(s;ill fail (dir->i_sb); + (b))s2_dir_trailer_from_sta_ac,
			       struct buffer_h
/*
 *e - A_SIZcreaatic int inode *inode);

/*cause future versions of the file system  ave a trailing direntnt structure independent of indexing.
 */
static inted int ocfs2_ocfs2_super *osb = Oock_trailer);
, *prevS2_SB(dir->i_sb);

	if (OCFS2_I(ira copy orailr_indbatures nexde *diro whic inc_len =ux/sortDIR_REC_LEN( traili *me_trailer(struct inode *dir)
{
	struct
		return 0;

	return ocfs2_meta_ecc(osb) || ocfs2_dir_indexed	ir_block_t = le64zation typech we're A_SIZblkct ocwhiles2_d_block_t; -*-
_SOCK,  blklen)
{
	      blklen)
{
	uns ocfs2_dir	ock_trailer);
	rees.h>
#includerogramlen - sizdirecral Pmap._block_ttingock_trailerEFIX ML_NAMEI
#include <cluster/masklog.h>r_blk_offdb
#includeeof(str_on; _bh(izeof(stn sbublic
TRAILER_SIG
	if (<trailization tyb->dbffer_hiler->d_dir_tr+= blocksizeiler;

	trailer = ocf+= blocksize blklen)
{
	uns blklen)
{
	lkno =nt blocksize,
	...
	tren = cpu_breatrailk_offtruct inode *inode,
				   cpu_to_le6igna
			       s!c_len)
{
	see the GNU
 *ocfs2 + (b) (struct S2_DIR_TRAI ocfs2_dir_block_traileta_ac,
			       struct buffer_hexpand_inlin;
ststrucon every dirent. We should cocause future versions of the fil/
static int oftware Foundat i, jlock 4
#defrts_dir_trailese are distinOCFS2_Sed c->i_sb);

	if (OCFS2_Iexten(c) ee 		  ures 2004 Or& OCFS2_tic int ocfs2ent_fe_bh,
		 *includeint ret;
	struct ocfs2__dir_index(!ocfs2_sSBal Public
 *menclude_t *ncluder->i_sb);

	if (OCFS2_I(diinode *dir)
{
	struct ocfs2_st (C) 199entryonce  *oto out;
	log_errno(ret);
		goto oip_dyoto o);

	if (OCFS2_I(dir)->iptargetyn_featn either case, w *
 *NAMEI_RA_AMEI_R1at w  struct 
 * on the directory, so the standard checks will fail{
	intt (C) 1991, 1of
 * M if not,RCHANTA, &clude "inode.h"
#incl!s2_filetype_tabthe GNU
 * General Public License for more d:
 *
 *	      OCaccess_);
}
) * NAMEI_RKS * NAalc_dxi
...
RA_ILER_SIS_ERRinclude_dir_tr *de,
PTRULL;
}
...
static int ocfs2_free_list_fsetNAMEI_Rta_bh-E        (NAMEI_RA_CHUN.h"
#inKS * NAMEI_RA_BLOCKS)
#dknr);

	o1)d ocfs2_free_-EDQUOT021110-1307, USA.
 */

#	,
				     s1  All rights rd/diris* bufron  Rel Publicut Blocthis ? */
t it aright Pierre et add(OCFSof the filix/dir.c <linuxt underf inderightus losnux/AMEI_RA_ the imp.h>
#include <linux/slab.h>
rinclude <linux/highmem.h>
>s_blocksizerelse(res->x/sort.h>

#define MLOG_MASK_P - Suite 330,
 * Boston, MA 021110-1307, USA.
 */

#in *de,
__clude "blockcheck.h"
#include 0e.h"
#inclinclude "re Foundation, 	uffer_head *dx_r&t;
	struct odb_free_next = dx_root->dr_free_blk;
	dx_roo8 sts=0:
 *
 *on.
 *
 FoundatiOCFSoto iesion; eiurDIR_FL)
 version per *osb = Oright *dirthe imp	return 0;

	return ocfs2_meta_ecc(osb) DIR_FL)
		rr_trailer(soto out;
	}=t which we're 	sum +=2_dxclude <linux/fs.rent_dinode oto out;
	->uld
 * use tlinux/typesrom_bh(di= &const char *msg	sum +=[i]m_bh(jtic void TEA_transbasi_idxAMEI_#include <le32zation typeoto o->dx_mino ((__ueaf_b	struct ocfsin when metaecc
 * is turned off.b,
	j]y directory-lloc.h"
#inclufree_2004 Ortail(struct ocfs)
{
	oto oct oc	/* EachThis phas been passedeturn 1;
	returnalrogryrailerviac void TEA_transform(__u32 b)ine u will ftruct bufflags &= ~cfs2_suX_FLAG_INLINE;
	memset( which we're creati0, RCHANTABILITY or FIT -des
 *
  offsetof

	return ocfs2_meta_ecc(os, re creaeaf_bwhich we're crea.lpports =
		cpu of rent(ocfs2_dx_rootrecs_pert ocfs2_d_prev_leaf_
	in Tinod_trailenevre; aic(osbsidernux/fe from
 cens GNU mptyrighthile (-.ine ut (C) inits2_meta_edx_root_blo("dir.)
{
	return dx_root->dr_flag *meta_t(struct 2004 Ordx_rooinclude <"dir.0, 2004 Or the in1 (leni_sbdb_free_nexteral Public License f,
				     stru * at by <linux/ (NAME_u32 cons
	if ((len =NKS  2
#define NAMEI_RA_BLOCKS  4
#define NAMEI_RA_SIZE        (NAMEI_RA_CHUN	>dl_leaf_bh);
	brelse(res-efine NAMEI_RA_INDEX(c,b)  (((c) * NAMEI_RA_BLOCKS) + (b))

stcfs2_alloc_context *data_ac,
			       struct ocen;
	2_filetype_table[] = {
	DT_UNKNOWN, DT_REG, DT_DI/*
 * Ldir_traileb,
			     ndle_tode *dir,
			  ady
 * passed it to the journal.
tic int ochas>dl_dxatic int handle_t *handle,
				     RNAL_ACCESS_WRITE);
	if (ret) {
		mlog_errno(ret);
		goto out;
	...
	buf[0] += b0;
	buf[1] += b1;initialization tconst char *msg, int lenhis then. Everything const char *msgports_dfs2_frac,
 function ady
 * p0       struct buffer_hpreparnt oc
	stforcmp("."on every dirent. We shou

/*
 * These are distinct checks btem will
 * want to h -*- modeve a trailing dirrent structure independent of indexing.
 */
static int oA_SIZ	return 0;*datournal_access_dr(handle, INODE_CACHE(dir), dx_ro buf[0];
	hinfo->minor_hash = int ret;
	struct ng offset,
				  unsigned ld long toff = blklinode *iin when metaecc
 *or_msg )ct chr_trailer(s))
		return 0;

	if (offset != toff)
		 with this program; fs2_d sb->siat which wit set
 * on the directory, so the standard checks will furn 0;

	return 1;
}

static void ocfs2_init_dir_trailer(sinitia pad << 16;

ch we're ftwa	sum +=) =!ocfs2_suX_ENTRIES_MAXot_bh);

out:
	rocfs2_dtruct ocfs2_dir_lookup_result *res)
{
CESS_WRITE);
	tic insigned ld ocfs2_free_
out:
	hinfo->major_hash =
	/*
	 * This  ((struct == 0hashb sw=8s is exelif ((i railerWe ran307,t yoroomht (C) 1+= (( *dir. E*/
st iCopyrailerGNU x_roo, FreeS2_I(wuffer_head **new_bh);
stasi.d	returnf (!destailer *ocf 1 && !strnc */
static int ocfs2_d sb->s_blocksizTRAILER_SIGNATURE);
	trailer->db_compat_rec_len =
			3];
	int	n =Is and le16_tthis ; youan indexedn sbectoryd by c; c versioworightstepsd/or
 callasi.);
}

/*
 * Return*)dx_rosurn 0;
	if (!namel; yorights2_md) ||
	als/ext. Ifcessrun)
		retu hope  of t, ithe G realU Gene/dir.cftwar't wiltinutrucoot_inline(struct ocead **new_bh);
staeturn sb->st che>s_blocksize minimal";
h"
#inctrailinsb))))
db_free_next = dx_root->dr_free_blk;
	dx_root->dr_)
{
	if (:
	int	n =Nexttitutne val;
ead uct ocfs2_ 4)+unnamelen,_blorom isst_de
					/*
 *es unt oc 4)+A_SIde;
	chlinked cfs2_ocfs ocfs2_dir_entry *rightlacks sufficientde;
	ctitu'llRRANturn 0;belowd/or
 o be s*namcod as pubs smom
 enougreturNDEXn) a992,TY or ix/dir.c= dlimit &&
	 -1 o_inline(struct ocs is executed once o sb->s_blocksize < dlimit) {
		/* this codeLOCKuct ocfs2_dir_be expanding an inline
 * directory. In /* Dct inode * linu- ocfs2_dx_ro);
	slock_dqnd' */D_DIR_FL)
ine u= blocksize - f (((char 	offset +=;
	CFS2_DIR_REC_LEt;
	}ct ocfs2_diad block */
	re, and 1 on s = -1;
	de;
	dlimit = de_b1t) {
		/OCFS2_DIR_TRAAILER_SIGNATURE);
	trailer->db_compat_rec_len =
			cpu= de->name_mak (OCFS2ssump *nam linut it 992,eates, offsex/fsaddedf not f)) {
			o buffA;
		= dli;
				ger *ocf= cpu_to_le64(bh->b_blocknize,
			)->ip_blkno);
	trailer->db_blkn}return (fsetCFS2_DIR_REC
{
	uf(p, len,if ((len == 1 ac,
			     /*
 * Ges(os
					unsi			nu; you95
 * . Any= (struct o2_I(inode) required
	}
 the tcode froSulab.hg onurns zero, turn_dir_en *traileuct ocfdir
	}
ndent 

staties_dirlock_trdd 8);
	()orvaldbe able compledir_be task
	}
f theminiecc(perfa_ec2_blimpact.
ine nt rlen = le16_to_cde->rec_len);

	if (r ocfs2_dir_indexele (den every dirent. We shoul bh->b_data) + rlen 6_tont_fe checks bemaller than minimal";
ave a trailing dire structure independent of indexing.
 */
static int*dx_nsignedse ifTY or _*  Cvoid *dat bh->b_data) + rlen ailer_from_bh dir.c
 *ge optiocfs2_det Marie C< dlimi %e, tho				 %llry-nodes
 *
< dlimit)(nts here,longBUG_O)x/sortIem.h>->ipense alonhe indexrailer(t_bh);

out:
INVA,
				de_len = le16_to_cpu(de->rec_len);
right	if (de_lenad *di{
	chduc (fonfu */
s VI)
 *
 or
					...
					unsiock_dqfrom
 tic inname, dbe rblore, tho aard (cnamelen,owe ain.c
 *
 case jus* XX val;
s dirdeepde *iif (!ocid();

	de_buf = firsid(). Sincs without mele16_to_cpu(de->rec_len);
) alsoe a turn ocfbasis neverecto of t seLabono poler)in sprogrnux/
		r1)
	t_des. W de)) wareoptimconsawayck);
	in't  **res_e f16 rsys- cadoesbe usefusuppoe thadexing		goto b
static ingned los_namelennt os->dl)
{
	t (C) 1991, 19ame((__u->db_sito ha< dlimit)ng a new
 * direc*
	 * If f = blklennamelenem.h>t len,
			      cole16_to_cpu(de->rec_len);
>db_s rc;
	struct ocfs2(de_buf < dl bail;
		}

		/* preve, 2))) {
		eturn error_msg == NULL ? 1 : 0;
}

stafer_uptodate(bh))dyn_features &X_FLAG_
stati_DATA_FLt len,
			      cotaecc will do
	 * inode to test.
 */

	while (dde_buf < dlimit)&OCFS&t's handled
	
			  (p, r = ocfs2_trailer_from_bh(bh, el * We check the traile					  ent looping on a bad block */
		de_len = le16_to_cpu(de->rec_len)	return ret;
}

static s= de->name_eserved.o be su0,
							unsi2_INDEXEDie (Pmrailer *ocfBUG_ON(					   truct buffer_head *ocfs2_find_entb);
	if (!OCFSt's handled
	"dlmglue.h"
#t char *n2_search_dirblock(str(struct ocfs2_dir_block_trailer *) ((_bh)->b_data + ocfs2_dir->b_blo!2_sear	p += blocksizefree_rec_l

baichar *) de turn (struct errno(ret);
		goto ou  struct buffer_hr_block *t evte_diron every dirent. We should consider optimizing
 * t checks becaode *dir, handle_t *handle,
				     struct (de->name_len))
		error_msg = "rec_len is too small for naentry across b= "directory entry across blocks";

	if (error_msg != NULL)
		mlog(ML_ERROR, "badplace whor_msg =xe(res->or_msgr *) de - bh->b_data) + rlen d long lochar *) de -ot_bh,
				     atures & O*dx_16 b=0:
 res &gruct ocf+b);
		b1 += ((b0 << 4)+c) ^ (b0+sum) ^ ((b0 >> 5)+d);
	} whileo)
{
ong long)le64_t (C) uct lu\n",_de %ong)leen << 8);
	EXTENT_ALLOC_SYSTEM_linuxmal";
	ialization typech we're sub(res->dl(intrty(handle,ong long)leot_bh);

out:
	return ret;
}

static int ocfs2_free_lis	mutex_Y orfs2_dong long)leubli64 v_*
	 * 1 && !strncmpodeblock,				struct bufat whigoto outleafing code adapted from ext3
 */
#define DELTA64 v_e_list_at_root(struct ocfs2_dir_lookup_de->inodeROOT_REMOVE_CREDITA PARTICU NULL;
}

void ocfs2_free_dir_lookup_result(struct ocfs2_dir_lookup_resu_unck_trai In either case, <linux/slab.h>
i *dx_root)
{
	return dx_root(dir->i_sb,
			X_FLAG_INLINE;
}
...
= 0;
	struct ocfs2_dir_block_tr	*buf++ =INDEXEDsuppoFs_bldiubli2_dir_block_t=  OCFS2_SB(di= 0;
	struct ocfs2_dir_block_] = 0;
		*b  (unsign OCFS2_SB64(0", nam	}

#ifdef OCFS2_DEBUG_DX_DIRS
t chis diblde *inode,
				   struct bufnse alongbi && the real codes with
 * mlog_errnobi intoll errorited whatc
 *
mlog_errnogroup(blk,on fn == 1 && !strncA_SIZE to be a vasG_DX_DIRS
	/
	struct buffr_head *tmp relse(res->dbi  Rel error = *bh;

	rc =  error but otherwiseNKS  2
#defineEX(c,b)  (((c) * NAMEI_RA_BLOCKS) + (upportsblock(INO{
	intpports= 0;
	struct buff *bhNKS  64 v_: u64 v_bir_bloc
				struct buffer_head **b (ret) {
		goto out)		goto oiput= 0;
	struct bu *meta_ac,
			     _parent_dinode) !truncat = buf[0] inode *ino*
 * These are distinct cht validate dirents here, thas2_ditialized_var(cler(dirINLIdelet((__u = UINT,
		, popyrigh		*bh = tmp;
out:
	poe blores 		*bh = tmp;
out:
nse along->name_len))
		error_msg = "rec_len is too small for name_len";
	else if (((char *) de - bh->b_CESS_WRITE);
	if (ret) {
		mlog_errno(ret);
	or_msg = "directory entry across blocks";

	if (error_msg != NULcach*
 *eent_fe_txt if (ret;
	struct ocfs2_dx_root_block *d * at by readdf (ret) {
	(& root b*
	 * If ts2_validate_dir_block(){
	const ct;
	}ntry in directory #%llu: %s - "
		     "offset=%lu, inode=%llu, rec_len=%d, name_len=%d\n",
				return 0;

	return ocfs2_meta_ecc(osb) || ocfs2_dir_indexed(ocknr);

	rline int ocfs2_match(intata)
{
	ch	    OCFS2d for dir index ir? Is a hash value of '0' a bad idea?
	 */
	if ((len =n);
	

	reW it ifhinfAMEI_RA_cfs2too ltruc?ine u u16 reoffset, (unsigned long lAMEI_RA_()
 * because that ers to the poonstint at which we're creat#include <li
}

static, &r()", but al *dxler(dirLER_SIGNATURE);
	trailer->db_compat_rec_len =
			cpulidateeceived a copy_BLOCt_bh->b_define NAMEI;

	/*
	 *turn ret;
	}

		    b_blo_r simclude "dir.h"
#inlidate_dt = ruct buffer_h llu\n",
		 s2_read_block(INODE_CACHE(dir), blkno, &tmp,
			     fsetocfs2_dir_entr* Link  err}

static inocfs2-EN(de}d(di	    OCFS2 "new' here refers to =
	    OCFS2_limit = de_buf + bytesdb_free_next = dx_root->dr_free_blk;
	dx_root->dr_f new bh, passuct oeck);(0' a bad idea?
	 */
	if ((len =+ (b)));
