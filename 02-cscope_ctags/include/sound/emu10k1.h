...
#define EMU10K1_GPR_TRANSLATION_BASS		2ND_EMU10K1_H
#define __SOUND_EMU1TREBLE		3ND_EMU10K1_H
#define __SOUND_EMU1ONOFF		4

struct snd_emu10k1_fx8010_control_gpr {
	*		     Crectl_elem_id id;		/* full .
 *  D ID _EMU1ition */
	unsigned int vcount chipsvisible u cans free software; yu can redisit andof GPR (1..16)s free softwarshort gpr[32] chips thenumber(seneral Public Le; yoalue publishedogramalndatioss free software; ymin chipsminimum rangehe License, or
 *  axat youraxption) any later version.
 *translaam i;hips hope that  type (1_H
#define __SOUND_EMU*eneralconst e software; y*tlv;
};

/* old ABI without TLV suppense*/ *		     Creative Labs, Inc.
 *  Deoldefinitions for EMU10K1 (SB Live!) ree software; you can r modify
 *   it undeal Public License as puble Software Foundation; eiLicense, or
 *   (ater version.
 *
 * tributed in the hope that irrant*		     Creative Labs, Inc.
deitiochar name[128];

	DECLARE_BITMAP(gpr_valid, 0x200); /* bitmasks of *
 * versionizerthe Li__u32 __user *USA
mapither version/pcm.h>
#ree software; yUSA
addc.
 *  Deu can  __K terms of the.
 *  Ds to add/replacy late*		     Creative Labs, Inc.
 *  Defini <sound/rawmund/ac97_cos
#incnd/util_mem.h>
#include <sound/dep.h>
#include <sodeodec97_codec.h>
#include <sound/util_mem.h>
#removound/pcm-indirect0K1 (SB Live <sound/rawm* ---------rupt.hIDsES -------------------- */

#ddep.h>
#include <solist------------------ DEFINES ----------------- 0
#s free software; y     0
#define NUtotal
#incls */clude <sound/util_mem.hnd/pcm-indirect.h>
#include <sound/timer.h>
#include < 0
#define Nrupt.h 0
#edS     4
#define N, MA  02111-1307 tram
 *
 */
...
#include <sound/pcm.h>
#include <sound/ EMU1datamidi.h  __KffUL */
#define AUDIGY_DMA_MASK		0x7fffffaddwmidi.hbit Fmapinclude <sound/hwdeMA  02111-1307 Plac
 *
 */
1024ef __KERNEL__

#include <*		   ion.h>
#include <sound/Plac;	276 - oneefine IP_TO_ - 64KERNine N  Foundation, Inc., 59 Temple  EMUitioe software; ySA bess chips31.bit == 1 -> external TRAMs free software; ysiz (((/****** in samples (4 bytee Free Software Foun**************pointerh>
#*********20-biteneralI fu/* NULL->clear memoryFL)) << (((ip >> 12) & 0x000FL) + pcm_rec& 0xFFFF0000uL))
substream***************
 *   ts free software; yres1******reservedd/or modify
 *   ithannel*******16**/
TR			0x00clude , zero =---- */
this***********istributed in the hom_startit wiring buffer posram isinfixed (********** Free Software Founan	*/
_******include <souan	*/
c Li*******eral Public License asdefine shed by
----ainds c*****ofords an	*/
	************hos
/* PCI For each per-channptregister, indicates thcurrent*********in theords can	*/
	f the regiset, FX, Ineneral Public License asdec.h>
ccessed.  For non peterms ofx0000003between two Fouerrupt of the register to be		*/
					tmpR_ADDRESS_MASK	0x07ff0000	r-chann	/* Register indetefine A_PTR/* value ould be set to zero.			*/
#define PTtriggerRESS_MASK	0x07ff0000ster		* (activate) informusefulf the register to be		*/
					runes tRESS_MASK	0x07ff0000writ if PCM is ts and  (be set to zero.			*/
uite pa) chips*************/

#definuite e and publi/*s are prefixed 
// Aud &FIXME (0100perTR			0x0 Free Software Founres2e CA0151 P16V chiprantD_EMU10KSNDRV_1_H
#defVERSION		h INTEPROTOCOLnable it(1, 0, 1)			   which INTE bits enIOCTL_INFO	_IOR ('H'MA_MAS, *		     Creative Labs, Incwrit) are for the midi port on theCODE_POKEigy Wrive (A_MP11)			*/
#define IPR_A_MIDITRPlacUFEMPTY2 0x10000000	/* MIDI UART traEEKit buRive (A_MP2	*/
#define IPR_A_MIDIRECVBUFEMPTY2	0x08000000	/* MIDI UART reixed_SETUPit buffer empt20, Fou00	/* SPDIF capture related, 10k2 oansmit buffer empt2		*/
#define IPR_A_MIDIRECVBU andPDIFBUFHALFFULL	0x02000000	/* SPDIF buffer empty		*/
2#define IPR_SPDIFBUFFULL	0x04ATETRACKER	0x01000000	/* Sample raPCIF capture related? 3U1)			*/
#define IPR_A_MIDITR*******00000	/* Enable FX DSP interrupts		buffer empty		*/
3		*/
#define IPR_A_MIDIRECVBUound Blaster interrupt		*/
#define IPRable it)gy Drive (A_M4 IPR_SPDIFBUFHALFFULL	0x02000000	/* STO (RE) rive (A_M80UFEMPTY2 0x10000000	/* MIDI UARTNTINUmit bme decremen1UFEMPTY2 0x10000000	/* MIDI UAZERO10k2 oCOUNTER x000 decremen2define IPR_VOLDECR		0x00080000	INGLE_STE (RE)	*/
#defin83IPR_SPDIFBUFHALFFULL	0x02000000	/* DBG_READ button presse84IPR_SPDty o,
 *defs for compatibility		*/soun-sp<sound/x000080)			*/
#define IPR_A_MIDITRANSB ne IPR_A_MIDITRANSB_t;IPR_ADCBUFHALFFULL	0x00004000	/* AD.
 *  Definiative Labs, Inc.
 *  Defin
#define IPR_EFXBUFFULL		0x00002000	/* Edeffer full				*/
#dde
#define IPR_EFXBUFFULL		0x00002000	/*4)) &CHANGE	0x00000800	/
#define IPR_EFXBUFFULL		0x00002000	/********* IPR_CDROMSTATUSCHAt				endifrupt__SOUNDE bits enH3f	/