...
typedef struct _IOCB {	/* IOCB structure */
	...
	union {
		struct {
			...
			uint16_t ulpIoTag;/***Low **********************#else/*** __LITTLE_ENDIAN_BITFIELD******************************************
 * This file***************Context*******************************#endif
		} t1;
		struct {
#ifdef __BIGulex Linux Devicers.                                *
 * Copyright (C)********************1:2* Fibre Channel Host Bus Adapters.            *****0:14******************
 * This file is part of the Emulex Linux Device Driver for         *
        *
 *                                    *
 * www.emulex.com                                                          *
 * Copyright (C) 2004-2009 E2;
	} un1;
#definehe GNU Genee Fr.t1.e GNU Genee Software F*****  on.    *
 ******rogram is distrib0ted in 2he hope t0
ights reserved.           *
 * ****32******Timeout:8;RESENTATIONS AXS:muleSENTATIONS AFCP2RcvyS, INCLUDING ANY PU:by tSENTATIONS AIrS, INCLUDING ANY Class:3SS FOR A PARTICUommand     *
 * WARRANTStatus:4BILITY,  *
 * FBdeCountTABILITY,  *
 * FLeS, INCLUDING ANY OwneTNES***********************7file is part of the Emulex Linux Device DriRS ARE HELD *
 * TO BE LEGALLY INVALID.  See the THAT SUCH DISCLAIMERS ARE HELD O THE EXTENT THAT SUCH DI * DISCLAIMED, EXCEPT TNFRINGEMENT, ARE      *
ULAR PURPOSE, OR NON-IITNESS FOR A PARTICANTABILITY,  *
 * FIMPLIED WARRANTY OF MERCHIES, INCLUDING ANY ND          2004-2
	union {lex.  All rcv_sli3    0xff; /******s 8 - 15****
	/*** Fabric-31 used for que_xri_cx iocb******.  All ID     64cx_ext_fieldsMask_DID        0 Fabrulex.  All fcp_irw    f00U
    
******ATION0xffWFabr[24]efine96 extra bytesast_DSLI-3     0xfffffeU
#de15ast_DBlockGuardffU
#define M0xff_bg0xffffffD	2

#dy the FU
#de
e Software Fot_hfffaU
2000	/* Defaultlfine FDMI_DI 2000	/* DIOCB_FCP	   1/***     isne Bcast_DFCP ELS cmds-ulpRsvByte (C) TOV         IP	15	/2 Default AL_TIME (15mI */
#definTOV         PARM_UNUSED * Defau0/***PU xffff (U
#d 4) notne Bca_ARBTOV         REL_OFF* Defau* Def_TOV (1900ms) */

= R. O.LPFC_BUF_RING0     AD_CHECK* Deault f buffers to post toData Transfer LengthLPFC_BUF_RING0   NPIV_DID15	/3         CLASS1* Defaulefault ARBULAR  1LPFC_BUF_RIN 0	/*2ring 0 for FC* Deftiator2commands */
#defi3ring 0 for FCault tiator0U

#ands */
#defi    _INTERMIX 7/***s) *imum->Cls 1, all is pLS com   1	           STAT_SUCCESSng 0 for 0xt ARB           - HBA Softwa LPFC_BUF_RINRING       RSP_ERRORe SLI1SLI-2 FCP commanREMOTE_STOPine SLI2SLI-2 FCP commanLOCAL_REJECTne SLI          RING   NPORT_RJing ee SLI4SLI-2 FCP commandABRICOCB_CMD_R10x5ies */
#define SLI2_IOBSYefine SLI6RIES      4	/* SLI-2 exe SLI2_IO0x7ies */
#define S     2EDing _R1_EN8134	/* SLI-2 FCP SOCB_CMD_R1_ extr9ies */
#define SBAA_ENTRIES 36	/* Adefine SLI2_IOCB_SVD ring 0 for 0xB#define SLI2_IOCB_RSne LPFC_EXTR0xC#define SLI2_IOCB_RS other proto0xD#define SLI2_IOCB_RS4_IOCB_CMD_R2_Eies */
#define SLEED_BUFFEes *_R2_Fies */
#define SDRIVERonse ring 0x10  fineB_CMD_R0_ENTRDriver    172	/* SLI-2 FCP commanDEFAULNTRIES 36	0x4	/* /* Same as rsvdcalID	now/* SLI-2 FCP commanCNNTRIES 36	/ ELS r1

}      t;
_FCP_NEXT_SLI1_SLIM_SIZE   (4 * 1024)

/* Up to 498faults will fit into 16k
 * 256 (MAILBOX_t) + 140 (Pdefi
#de( 32 (
#defi) *IOCB_t to< 16384
LPFC_BUF_RINSLI2_R3XTRA_ENTRIE6S 32

#define Maximum_CMD_SIthatIZE	32
#defi


/*F_DEmLPFC_BUF_RINMAX_R3X2_ault   IOCB_NL_VID_TYPE_PCIM| PCIRA_ENT(

/* vendor ID - \ibut_VEN(sizeofSP_SIZE	32
#deons */
SLI3_I)efine HBQ entries are 4fffeU
#each = 4kLPFC_BUF_RINLPFC_TOTAL_HBQ Transpons */
.  All lpfc_hbq_s iny8
#dnd definit24;
	}sli} bitcE EX(Stru InId:24;
	}sli2trucmSCR_P_SIZE	32 mbx;
	SLI3_ pcb;
	
#defi_CMD_S[X)

/* Common Tran];
};fine2_IOThis functne Schecks PCI deviceI2_Iallow special handlingast_DLCRIESs.
 ze:16Parameters:2_IO;
};

#: .  All pci_dev 's
struct xffff4_FEATUreturn 1 => TRUE2_IO for FCPndiaFALSorma/
static inltwarint
4;
	}is_LC_HBA(unsig72	/short
struct)
{
	if ((;
};
...
finitions;
	uint8_t Rsrvd1;
	uPion CtCommandResponse CommandResponse;LP101 CtCommandResponse CommandResponse;BMID8_t Explanation;
	uint8_t VendorUniqSMB CtCommandResponse CommandResponse;Zue;

	union {
		uint32_t PortID;
		strZct gid {
			uint8_t PortType;	/* for GSAT_ue;

	union {
		uint32_t PortID;
		strTypect gid {
			uint8_t PortType;	/* for GRion )
		in Big E;
	is prequests *0;
} Size:16DRGETmId;
	f anfault failed because of a link event or firmw EndresetC4_F/ponsRevisionId;
	uint8_t error_lost_:7;
SIZE	12 *ffffpnt8_tin Big (* Typ->B_CMD_R0_E==LI-2 FCP response rin&&
		nt32_t rsn*
 *U
#d[4]else	/ERR_R3X_ABORTEDCtCom	xffff*/
			uint32_t rsvd0:16;
	LINK_DOWNfcpReg:1;	/* Type 8 */
			uint32_t rs		ui;
		))IAN_BNL_VID_TYENLO_TRANSI2_IOTYPE 0xfe/
			uint32_t rCONTEXT 0/
			uint32_t rPU           32_t rsIMEOUT 3ft;
		struSETVAR_MLOMNTLS re310a responseequests */RS		uint8007*/
			uintBPL_ALIGN_SZ 82_IO8ID_MA alignmuintst_Dbpl and mbufEF_AR