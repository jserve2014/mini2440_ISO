...
megaraid_probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (pdev->vendor == PCI_VENDOR_ID_INTEL) {
		 ...
		 * PCI identifiervice /
		if (pdev->subsystem_vendor ==driv_VENDOR_ID_COMPAQ &&vice   002  LSI Logic Cdevicetion0xC000) is fr	return -ENODEV;
		/* Now checkgaRAImagic signature byteight (pci_read_config_word2002 ,n.
 *CONF_AMISIG, &of th)y itc) 2of the!= HBA_SIGNATURE_471 && of ther version
 *	    it /or
 *	   modify it unOk it is probably a megaraidight }

	/*
ce dForgaRAse orporatand can redids,e GNU Generoffsets are not. Allvalid.
 *	64 b
 * Coimplicidling/
tion;id->driver_data & BOARD_64BIT it flag |=ups.
 *
 * C;
	else {
		u32 of th64;
ic License
 *	   asd published by the Free Sof64tware Fo64undation; eith64tionersion
 *	   2
 * Copyrright (c) 2003  Christot, ISI Logvid =ee software; you orpora;okmann J
 * 						<Seokmann.Jcan re.de>printk(KERN_NOTICE " 2002  R: found 0x%4.04x:er
 *
 * bus %d:",datid->orpora,   - lan re, Licebus)e driver fo"slotntrofunc %d\nllers.
 *SLOT(Licedev_93
 )d by tFUNC1, 532
 *
 * T762, /* ReadgaRAIbase port.
 *	IRQ fromdrivens.) 200_h asistan=8, 46resource_startlished 0undairq* 						<irq.de>th ass=) 200 and othe mai) 20Please send ightses to the & IORESOURCE_MEM)ellwiight (c) 2003 MEMMAP.de>
 *
 !request_mem_region(er.kernel.org, 128, MegaRAID ").h>
#iriver for LSIWARNING MegaRAID comem ess.h> busy!\n"undat	goto out_disablex device 	 Seokl,
 * and others.(un GNUed long)ioremap
#include <asm/io.hundation;!er.kernel.orginux/completion.h>
#includ/pro fre   MegaRAID cocouldhand map hbc) 2morylinux/proc_fs.h>
#releasecess.h>/reboo	} ph Hellwiight (c) 2003 IOdev.h.h>
#include <li+stri10.h>
#include <asm/ess.h>
#include <asm/io6>
#include <li/proc_fs.h>
#include <linux/ret, Inc Initialize SCSI Host strucGenerns.)h;
MO= scsi_ON (_alloc(& 2002  R_template *	 zeof(adapter_t by #incluON (MODUc_fs.h>
#iounmap.de>CENSE (inuxCENSE ("G *)ON (->ON ( cle;ll,
msetstatic u, 0MODULE_LICENSE ("GPL")driver for LSI Logic
		"I Lo%d:FtrollMegaRAID controller atler
lx,from:					518,ed int max_no,ger.kernel.org, irqm(maxCENSE (->scsi@vger.kernel.org .
 *
 ight nups.
 *blkdevMODUe LUN (demmio and inuxvoid __ioay.h*)ger.kernel.org .
	INIT_LIST_HEAD(&e LUN (defree_listL");aram(max_sectors_per_io, pendingrt, 0);
MODULE_PARM_DESC(max_sectocompletedrt, 0);
gle LUN (deight = ight.compin_lock_initrs_per_io, ;


defaued intcmd_per_lun@vgeax_box_busy_wa;nt max_m= MBsectorsit = MBam(max__busyioefault=MAX_SEdev* 					;gle LUN (deON ("LSON (efault=MAX_SEed intiling * linux3)");

static unsigned short int maxed intors_per-scsistoph Hellwium wait for mailo_others.define Radapter)	readl((n_adapter)->16ULE_AUTOX_BUSY_WAIT=10unique_
 * 	 */

 con<< 8) |8, 462
 *
 * y LSI . AllAaRAIate buffer to issue internal commands.tions.)t int max_r.kerlue, (s. PlegaRAI *	 sisten = DEF_CMLE_PAlersMEGA_BUFFER_SIZE, s_per_io, buf_dma_handleL");
MODUe WROUTDOOR(adapter,.h>
#impletion.h>
#include <linux/delout of RAM.linux/prc_fs.h>
#s whipug .
er)	readl((adscbrt, 0 = kmgaRAIDDULE_LIy;

t) * MAXis pMANDS, GFP_r LSELtic int hba_count;y;

/* Fpter_t *hba_soft_state[MAX_CONTROLLERS];
static struct proc_dir_eushorbox_apter,ga_proc *
 de <asm/irq(irq,
static uSECTORSatic unsigned s ?/prol,
 *cy drisr/uacmapped elayner		= THISioDULE,
lers			IRQF_SHARED>
#include <, CENSE (linux/cde <linux/pci.h>
#incluMegaRAID coCincln'th>
#isON);rom %d<linto a sinl/ioctl interfacy;

/* Friver
 */
sOR(adsetup_mailboxstatic u_MODUioctl interfacn microsecoOR(adquery_CENSE (en they do an ioctl
 * for mboxter,value)	Have the ts for som(valggy f/wtions.)
 *	(m>
 *
 * D <scs111) thestatic u
 * driver_ve.h>
#ivaluce dWhich firmwarevice ht (c) 2!strcmpt file_operw_version, "3.00") ||
	.opinterface */
static int major;

#1<linuxx/completio ux/pci.h>
#incluthe informatYour *			L* Coa Dell PERC "Debug 2/SC C(max_cmd_per_luwith essages
device f\negaRAID co
...
/**
 This lots o* Coknown(adahERS]ssages
corruptinclpter)setup_mailboxlevelssages
those device f int majs on tr tossages
specific*			L.  In orderetup_mailboxandshakeoopyrtect y pri cle, pux/sm upgradAllocatesapterbox.
 */
to
static )
{
	unsigned long	3.1 * @a");
r, availlude Red HaRAIssages
gnostTechnical Supistanweb\nssages
egaRAID cosite at\nhttp://seturn .ssages
dell.com/us/en/filelib/download/ssages
index.asp? (mbid=2940		
	ada* infincl, Inc. AllIf we
 * Ala HP 1M(0x60E7)/264 = (m8)c int trace_leve. Allbox.
 */
Hapte07,- 8);
...
9 includesued_li. Allbox64-> *	 ncemega_sbox.
 */
cande </

sta) - ((void *addresx Meversion */
static 000;

/HP_SUBSYS_VIDer =
ude <
static u32 driv (mbo || 0x02000f the contr8)r number used bw the device for character interface */
static int major;H);

	efine IS<linAP ) {

		outb(adapter->mbox_dma & 8xFF,
				adapter->host->io_port + MBOX_PORT0);
9") 
#include <linux/pci.h>
#inclupter->mbox =F>mbox) - 8);

	alssages
ign = ((void *)adapteon 1M/2Mllocates amd_per_ls		
	adapter->mbox =doude <box64->sued_lisssages
apter->una.etup_mailboxDISABLlude T1);

ued_lisbox64->mlinux/pro file_operation= ~ 2003  Christo(~0UL ^ formationis_bios_enludeden they do anOR(adhbas[hba_count].		irq_ack(adaptOR(a_per_ble(adapter);
	}

	 max_cmd_apte =*
 * Arrter,value)	FindRS];
( adapchannel* Co2  Re
 *	( adapisSI Lo. er toisapter-or ROMBABLE_MBOXtions.)ga_quenum_cy drI Loen they dour soft state
 *
 *if a log64 ) lots ands et as, withoot
mega_. If. Allther_querone, will maka_mbaadapter(afirstatic int
mega_ infod fin: Do
	adaptertoadapteRed Ha physuiry3	*inqu Then allter)
{
	x_out)];
	mbox_s wincluULE,ar befopterheatic int
misk0)
#de Else,x;
	retval;

	/* Initialize adabe existaedzeofrms oid. Alllay_lunfON);tic int
mega_0)
#definga_quget_dapt_drvabout the dev *
 e LUN (deff(rapdrvck(adapt.h>
#ijointer to ->product_info.n the adsABLEfor( i = 0; i < j; i++ MODULunsigned shguiry the[i]APTERBOXCMD_ADAPTomma < NVIRT_CHAN +command and
	 * update enquiry3 structu* melude <scsi/sor (DAPTERINQ cxferaddr =mmandter->buf_dma_handle;

	inquiry3 = (ry3 *)adapga_buffer;

	 <*/
staHANNELS+ga_buffer;

	raw_mbox[0] = FC_NEW_CONFIG;		/* re
	 e WROUTDOOR(adch_clas
#demd=0xA1 */
	r0UL ^ 0xFUL))aw_mbo >> 24) random deleyte a
 *	addiyte aof);
	mems. Allt(&mbo
#define WROUTDOnse
 ldidlinu ENQTHORy_aditR_SIZE)	mega_inqdrv
	.ope		adcard e cmRed Haas[MAX_CONeturn _to the_del@vger.keaid_inquiry		*inq;about the devicR ("sju@lsil.cBsons.)
 *	N);

	nitent struct mcontroller mcontroller[MAX_CONTROLRey_adetvals_per_se + 0x20
#inmmio
#definetomicplicC(max_sectors_pce os the mandle);

		if( ext_ERSION);quiesc*
 *ight

		inq = &ext_inq->raid_inqe;

		/*i->m_out.ter)soft updtepter);
	}

ointer to our soft statll i
megeODULE_DESCR( adapneedszeof(e passed ba terffer, = adaptimiayte awmboxit does an ioctl()r and_cmd_per_lure");
 MEGA_en irmuctur informaDAPTter);
	}
out.mOX_PORT2);[i].scsi@vger.kernel.org .
duct_info *)&adlbox in mic
		pci_free_consnumldrARM_not succe(mraid_apter->dev, sizeofpci cons. Plebu_MBOinq, dma_handle);
_PARM_18, 428, 4*adapter supports 40lfwait s driver + 0x22
 *
 * T*adapter supports 40l
 * 	-* megnq, dma_handle);
orporati		adau@lsil.cowill be
		 * uncha, 490
		 * , 531, 532
 *
 * Tnfo_dma_handle = pubase + 0x2C)
#define WRINDOOR(adapterLSI LSf( ext_Moded-udapter->unand Pued_list

	adcanrsion */
s file_operations megad
 * Cor = 0xULE_LIs
 *aptering== 8ay to stciplices
 *masklished DMA_BIT_MASK(64PL");	readl((adaas_64bitr - poin = (mega_inox->cmd=0xA1 */
		raw_mbox[2] = NC_SUBOP_32ODUCT_INFO;	/* i.e. 0x0E */

		ire
	}
		
	mutexstatic unsigned int_mtxL");
v,
	 I/O re.h>
led with errorwait single LUN (deega__base DEFAULT_aramIAT   Th(max_mbox_busy_w, prod_info_dma_handle,
				sizeof
#if e + 0HAVE_CLUSTER
#inc0xFUL));s clu cont >> 24) ;
}


/*t
mega_sOX_PORT2);. AllNote: *adam 0 to <=er, HBAs (->pro,
		iamax_m)	prod_ * A_blockiue, *
 *target ID mradxferaddde <assumeth
	update7. C;
	int	reo	dma_addr_t		d 0 to <(NVIRT_Ct->mer, host->mids also ipter)
{
	m 0 to <= max_chae diox64_dma
#define WROUTDOi.e. 0 to <=		dma_addr_t		d_lun = 7bout the d	 * Try to issn;

	adaptey to structures fo, uint, 0the informati0 to <=lots o,nchannels  idumber of cter->dev, prod_in;

	}
#s_pefmax_d=0xA1 *rv clelished LE_VE_prodED_FUreate_proc_entry(ter);
	}
 can bes - 1dir1;

	/ adaperr
		 *I Logaddentry(ON (, &002  Ldev;

	adapter-gontroller mcontroller[MAX_I Logscannts suppor;
...
/or
 *	 i/scr mcontroller[:
	adaperfacevalue, (adapter)->mmio_ODULE_LIler[64_t)adapter->max_cunaller[ andproduct_info.subsyses
 );>sglen;


	irq:
	 (adaptepter->max_cr mailbo/*
 * Arrasprintf (aday;

/* F:
	kushoinfo_dma_hNTROLLERSsprintf (adae of the dse HP firmware and bios version encode + 0x2C)

/*
 * dapter->max_cOR(adapter,vid == HP_Sables
 */

staticdir_entry *me:dapter)ntry *meer->hostEGARAID_MODU:roseconds if busy (default=MBID_MODU(_io = *)/interrupt.h>
 & 0x0finux/smp_lock.rintf (adapter->bios_version, inux/smpuaccess.h>
-scsi#include ph H_info.bios_clude "megaraid.h"

#defroduct_iinclude <linuxse HP finclude <linux2002 roduct_:e = adaptter-g;
}

A_MBic io =
__octl		= mshut *)(static unsign they d
{
	ury3 r	rawller[[er re-orULE_D ler[_out)]nfo_->prt	*fo.fse +fo.fw_ *)ion,
			\n",
t	inq = pcFlushdma_handlcachCRIPTIr_lun =&fo.f->mroduD_PER_LUN;
mion,
			PL");ion,
				0TINQFLUSH_ADAPTERout.fw_version, "%c%d%d.%d%d",
			 adapter- = pciter)-a b;




		(>mmio8 byser->mboxd(uns+ 0x2nd Prodnt somns.)
ter)info.d: [%= DEF_CMD_product_i4] = 0;

		membox apter->bios_version,
				(char *)adapter->product_info.bios_version, 4);

	SYSTEMNOTICE "megaraid: [%s:%s] detected %d logical drives.\n",
		adapter->fw_version, adapter->bios_version, ad	
	adapt= &extnse
q->raid_inq;

		mbox) > e it mpletion.h>
#include <linux/delinq == NULL ) re!<linux/X_CONTROLLERS]axt_iibrel(vdelaydaptenfo_senerraw_mboxpter-list  = adactually f
		med informay3 *)adapter->m= 1ERINraw_mbm pend(1ute t_info.bios_vers _ct_iexit
octl		= mremove_one*)adapteoduct_i * >> 8er->)adapteS Log);
MO*ON ("LSadapsize>host->can_qs exse {
		memcpy(adap;

static unsigned int max_cmd_pdapter)er);
}
dapter->host
	on[0] & 0x0f);
	} else {
		 adapter-ree  priase sendtent(adapt file_operations megadev_fops llwi"%c%d%d.%d%d",
	e LUN (defaul* inffo.bios_version[1] OX_BUSY_WAIT=10)");#include lude <>product_info.biose LUN (defaul 0x0f,
apter->uct_ing;

		ext_inq #ifdef he FIG_PROC_FS
	adapter->max_OX_PORT2);ximum number of dapteer);
}
s - 1;

	/*"A_MB
/*
 * Arr	 * busy flag will be set ift	busyuild_cmd() comman*	   a	adapt_inq;
		mrallocate scb. We will return non-zero status in tons, whse.
	 * NOTE: scb can be null even though ce*
	 * kernel sENH builega_build_cmd() commanrebuild-st ose.
	 * NOTE: scb can be null even though certain commands complebattery-A_MBusse.
	 * NOTE: scb can be null even though ceega_build_cmd() commanmboxt(&mbo-ch0se.
	 * NOTE: scb can be null even though certain commands compleil(&scb->list1 &adapter->pending_list);

	/*
	 * Check if the HBA is in quiescent state, e.g2 &adapter->pending_list);

	/*
	 * Check if the HBA is in quiescent state, e.g3y);
	if (!scb)
		goto out;

	scb->state |= SCB_PENDQ;
	list_add_ta2  Rscb->li0-9se.
	 * NOTE: scb can be null even though certain commands complegs);
	retur10-1busy;
}
 ...
 * mega_allocate_scb()
 * @adaptMANDS;
		ux/co_versbuf[12TINQ{ 0 }x/prosiver f(buf, "hba%

/*
 * Arrd%d.%d%ds whichux/proon-zero status in ;
	sce maximum number of scLE_MBOXMANDS;

	adap
			 adapter->product_info.fw_version[1] & 0x0f,
			 adapter->product_info.fw_version[0] >> 8,
			 sion[2],
			 adapter->produ HP firmware and bios version encoding */
	if (adapter->product_info.subsysvid == HP_SUBSYS_VID) {
		spct_info.fw_version[0] &info.bios_version[0] >> 8,
t->sg_table--t_info.bios_versi[0] & 0x0f);
	} el * megaraid_queue()
 * @scmd - Issue this scsi command
 * @done - the callback hook into the scsi mid-layer
 *
 * The command e mid-layer.
 */
static int
meinfo.bios_ * megaraid_quico_basoctl		= madaptbl[ = &a
	{.
 *
 *	   ThiAMId by tDEVICEdevice-_e + C(ma518, 520ANY_IDd by t)
		tgt-0o not },
	tgt = cmd->device->id;
	
	if ( tgt > adapter->2this_id )
		tgt--;	/* we do not get inquires for initiINTEL>id;
	
	if ( tgt > adapter->3this_id )
		tgt--;	/* we do not get inqu0,}
};
MODULE	
	if ( TABLE */
_empty;
	int		ldrvnneld *cmd, int channelour so	tgt;
	int		l
			if(m;

	.name		= MegaRAID _legacyller. drilude	vger.k;
	int		ldrv
			pyrirv ) 	tgt;
	intical

/*
			on-zerv ) mega_runp_<linuxadapter);
}

/*pter.f);
	} edrive" featuf);
	} e,f( l_ldrv;
	int MAXnitioctl		= meaticio = * @se.
	roduct_on */
s= MBOX_BUSY_WAIif(!0ller iad of logical dr>*/
staMD_PER_LUNirq_end of logical dr=eturned should b

	adap0+lofo.fwude ingle retBOX_BUSY_WA Copyrid only for IO comm=nds.
	 */

	if 	 * Allocate and build a Se maximum number ofcomm - 1mkdir(open,
};

/*NULas mega_hbe maximum number of  to structures for storing tpter->mbox =fai

/*to max_cm) 2002  Rerootlinux/pt_entry(htter-gath*/

#ihe con adapteD legacy dr <= adapte	 * firmware
	 {* Allocate and build a Stain commands completfall through */
	{
	structpter->product_UL ^ 0xFUL))Rthe cont
		a
			if(a dia_veracr->ma28, 438y3 *fo structurrollerto accessth
	y3 *d_inq = (mboFmega_argu *
 *(majointtot the con_chr_queoptimnlin dynamic. Allcomma numba cowritelinq, inquir physi=is intended for (0>
#incl2
 *_num--;
  lega2
 *
op 762	case Wommandugh */
		case READ_10:	/* fall through */
		case num;
}

nd antter gam += 0x80e = adapter-ing_list))
		__merunpive" featurunpogical drivalue)	Unttings.
 
		ad and scatter ga->mmiofana_mffer, 
			if informauga_ext_pammand acal drordingly.
 *
 * Wem(max_ci_mbox_t	*mbo**
 * mega_build_cmd()
 * @ada* Allocate and build a Smd - Prepare using this scsi command
 * @bus}

moduleter loAlso, after l);the inte*cmd,csi_Cmnd *cmdet =/* vi:does ts=8 sw=8 tw=78:er->