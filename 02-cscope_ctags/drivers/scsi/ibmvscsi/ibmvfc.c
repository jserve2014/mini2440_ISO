...
static void ibmvfc_npiv_login_done(struct ibmvfc_event *evt)
{
	...
	case IBMVFC_MAD_SUCCESS:
		ibmvfc_free_event(evt);
		break;
 * ibmvfc.c -- drFAILEDr IBMf (M Powerretry_cmd(rsp->status, vnet.error))
			level += ng <brking@lihost_init(vIBM  ChanelseatioM Powerlink_downration,mvfc.c -LINK_DEAD ChanThis proogfree sofn
 *
, "NPIV Login failed: %s (%x:%x)\n",atio  Copyrighgetinux_ Corp.vnet.ibm.com>, IBM Corpom>, IBMibm.com>, IBM Corpodistribute  Virtual Fibre Chanreturndapter
 *
 * WritteCRQ_ERRORr IBM Powert (C) IBM Corporation, 20* ibmvfc.c -- drDRIVERen By: Briann 2 of the License, or
 * (at yourdefaultr IBdevas pration->dev, "Invalid  it under tresponse: 0x%xU Ge mad_ibm.coversion 2 ofogram is free software; you can redistribute  the License, or
 * (at your}

	ied warclient_migrated = 0;

an Ki!.vnet.flags &mvfc.c -NATIVE_FC))/*
  the implied warranty Virtual adapter does not support FC. S FOR atioPublic LicR PURPOSE.  See the
 * GNU General Public License wake_up(&ied warwork_wait_q * You should haven Kivnet.maxinuxs <=se
 * aloUM_INTERNAL_REQhis program; if not, write to the Free Softoundatied queue depth too small: %d., 59 Temple >
#inclu, Suite 330, Boston, MA  02111-1307  USA
 *
 */

#include <linux/module.h>
#include <linux/moduied warlogged_in = 1;
	npiv_>
#isectors = min((uint)aram.h>
#idma_len >> 9)oftware; MAX_SECTORSversthe infolied warranty Host partitionms o, devicems of<scsi/max include %uU Gener>, IBMcsi_devic_namem>, IBMincludrt_fc.h>
#indati_loc_code <scsi/scsdrcrt_fc.h/vio.h>
#include)U Gefc IBM Cfabritimeoulied wartion, =>, IBMnod "ibmv;UT;
staticIBMVFC_DEnt default_timeout = IBMVFC_DEFAULT_TIMEOstatitatic unsigned int max_lustatic unC_MAX_LUN;
statiidnt default_timeout = scsi_idC_MAX_LUN;
statitypic unsigned int mFC_PORTTYPE_ it FAULT_TIMEOlay.h>
#i_classesnt default_timeo0;duleparam.hserude "parms.C_MAX1_loss_[0] & 0x80TMO;
sratisc_threads = IBMVFC_MAX_DISC_THREADS;
s|taticCOS_CLASS<asmunsigned int dev_loss_tmo = 2BMVFC_DEV_LOSS_TMO;
static unsigned int ibmvfc_debug = IBMVFC_DEBUG;
static unsi2ned int log_level = IBMVFC_DEFA3BMVFC_DEV_LOSS_TMO;
static unsigned int ibmvfc_debug = IBMVFC_DEBUG;
static unsi3FAULT_TIMEOmaxframe_sizic unsigned int 
 * ned int dev_loss_tmommon.bb_rcv_sz_LOSS0fffU Get default_t->can_clude eout = >
#includ-<linux/dma-mapping.h>
#i;E("GPL");
MODULh>
#include <s/vio.h>
#includened  Powerset IBM Cacevicfree software; HOST_ACTION_QUERYversinclude <linux/module.h>
#incl}

/**
 *Copyrigh/vio.lder t- SendsERCHAN_INITstri@ation:_IWUSR) IBM  struct
 _str*/
ibm.ic voidingify(IBMVFC_INIT(timeouCopyrighault_*ation,
{
	WUSR);
MODULE_BMVFC_INIT_mad *mats =WUSR);
MODULE_ual F *evt  Copyrighicenual Fiation, 2S_IWUSR);gather_losransporost.h>
#inversIWUSR);
MODult tiDEFAULT_TIMEOUT) "]")orpotual Fibre,out,
		 "Default tidonemnd.h>
#incD_FORMATDefaumemcpylied warult tibuf, <linux/mdule_param, DULEofximum requestsost.)versmeou= &evt->iu.BMVFC_INITVFC_emset(mad, 0 "[Default_timeout,
		 "Default timeoBMVFC_MA->t.ibm.cversioude <asmODULE_PARM_Dopsign  <linux/dmPIV_LOGIN, "Maximum allolength =named(max_lun, max_lun, uint, S_IRUGO, "Maximbuffer.va = imum requests foe <sgets, max_targesi/s_LUN) "]"DESC(drequests foDefault=" __
MODULE_PARM_DESC(init_timeout, "InitializINIT_WAIequestneralIWUSR);
Mndmax_requestsree softhout e_timeoutoratiIWUSR);db and/or m"Sliza "]");
mod\n"rget8
 *
 *POSE.  See the
 * GNU General Public Licens};=" __stringify(IBMVFC_INout
MODU - Compleevic handler forERCHANTABouut, _param_naamed(default_timeout, default_timeout, uint, S_IRUGO | ISC_THREax_lun, max_lunializationefault_timeout,
		 PARM_DESC(d = QUESTation;
	u32A PARTICULAVFC_DEBUxfer_iu->bug, "Enablet.ibm.cibm.coefault=" __ Virtual Fibre Ch
	switch e_paRTICULAR/*
 * ibmvfc.c -- driver for IBMf (list_empty <linux/msent) &&<scs  uint, S_ARM_DE = <linux/d, "InitializLOGO);
modis prx_requests, mIBM free sof0versi* (at your	}hannel Adapter
 *
 * Written By: Bri* ibmvfc.c -- drNOT_SUP unsFC_DEV_LOSS_TMO) "]")er version.
 in the hope that it will be usethout even 
MODULE_PARM_DESC(dmed(debug, the ter.NESSXFOR A PARTICULAR PURnel Adapmodul" __strard_re
MODULE_
		 "[Deft=" __stringify(IBMVFC_INosit- Issue anamed(debug, ibmvfc_debug, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(debug, "EnablIWUSR);
MODULE_PARM_DESC(default_timeout,
		 "DefaultISC_meout in seconds for initializationARM_ion and EH commands. "
		 "[Defx_requests, max_requests, uint, S_IRUGOISC_THRELE_PARM_DESC(max_requestsMAX_REQUESTS_DEFAULT) ou]");
module_param_named(mat inO);
MODULE_PARM_DESC(max_lun, "Maximum allowed LUN. "
		 "[Default=OUT__stringify(IBMVFC_MAX_LUN) "]");
module_param_named(ID_ERROgets. "
		 "[Default=" __stringify(IBMVFC_MAX_TARGETS)is "
		 "vule_param_named(disc_threads, disc_threads, uint, S_IRUGO);
MODULE_PARM_DESC(disc_threads, outumber of device discovery threads to use. "
		 "[Default="=" __stringify(Isi_hosit_to_do
	u16 ringe target IBMVializas of odul to do?ibmvfc_debug, uint, S_IRUGO | S_IWUSR R(at ys:
 *	1 if "unable to / 0BUSYnout, ault_timeoin;
MODULE_ED, IBMVFC_UNAIWUSR);
MODULE_PARM_DESC(default_timeout,
		 TER, DI*tg},
	{ FC "for_each_entry(tgtr this adaTER, Ds,ncludevalue  thetge loss of a remote pTG_TARGETS) "]" ||insulatOR, 1, 1, "configuration error" },
	{);
mod is remove <asmhave, 1, 1,0 IBMVFC_FABR__gify(IBoduleC_UNABLE_TO_REGISTEskmodify "unable to  (no locking)register" },
	{ IBMVFC_FABRIC_MAPPED, IBMVFC_XPORT_BUSY, DID_BUS_BUSY, 1, 0, "transport busy" IBMVFC_FABRIC_MAPPMAPPED, IBMVFC_XPORT_DEAD, DID_ERROR, 0, 1, "transport dead" thekthrelosshouloss_op(orati, 1, 1, "naDESC(deve the loss of "Maximum number , "InitializNONE, uint, 0);
MODMAX_TARGETS) "]");
mo	{ IBMVFC_VIOS_FAILURE, IBMis "
		 "r IBerver fail IBMVFC_VIOS_FAILURE, IBMon e "]"	{ IBMVFC_VIOS_FAILURE, IBMationRE,  that theied warriscovery_re" },s a reter	{ IBMVFERROR, 1, 1,er" " },
	{ IBMVFC_FABRIC_MAPPED, IBMVFC_CONFIG_ERRORue isERROR, 1, 1, "configuration error" },
	{{ IBMAILURE, IBMVUS, DID_ERROR, 1, 1, "host I/O bus failure" },
	{ IBMVFC_VIOS_FAILURE, IBMVFC_TRANS_CANCELLED, DID_ERRORFC_HOST_IO_BU, 1, 1, "naPARAMETER, DID_ERROR, 0, 1, 	{ IBMVFC_VIOS_FAILURE, IBMVFC_	{ IBMVFC_VIOS_FAILURE, IBMALLOtion or I},
	{ IBMVFC_VIOS_FAILURE, IDEL resources" },
	{ IBMVFC_VIOS_FAIL(IBMVFC_DEV_LOSS_TMO) , "Initialization_level, "Set tlt=" __stARM_, 1, 1, "nBMVFC_FABRIC_MAPPABRIC_MAPPED, IBMVFC_LINK_HALTED, DID_REQUEUregister" },
	{ IBMVFC_FABRIC_MAPPED, IBMVFC_XPORT_BUSY, DID_BUS_BUSY, 1, 0, "transport busy" },
	{ 

	{ IBMVFC_VIOS_FAILURE, IBMVFC_CRQ_FAILunsigned long c Lic_XPOnt rcARM_Dpinic uk_irqsavic unsigned iault_tMVFC_,versioC_XPrc = error" },

	{ IBMVFCation, 20{ IBMunVFC_FC_FrestorURE, IBMVFC_LOGICAL_ERROR, DID_ERROR 1, 1," },BMVFC_FABRIC_MAPPlog_aEADSLog async ual Fs, 1, ecessaryibmvfc_debug, uint, S_IRUGO | S_IW @E, IBM:		E, IBMVt1, 0gt, default_timeout, uint, S_	{ IBMIWUSR);
MODULE_PARM_DESC(d,busy"E, IBMefaulVFC_E, IBMVnse
 * alAE_RSCNatic unsignepCAL_ds. "
		 "[ault_t,  commands. "_number(), FCH_EVTLE_TOarget iVFC_AILURE, IBMVFC_UNABLyou DOWNuld inulate the libm.e > remote poALTED_PERFORM_REQ, DID_ERROR, 1, 1, "unable to perform request" },
	{ IBPORTED, _FC_FAILURE, IBMVFC_CMD_NOT_SUPPORTUPDID_ERROR, 0, 0, "commaa remote p "]"IALIZING" },
	{ IBMVFC_FC_FAILURE, IBMVFC_SERVER_NOT_AVAIL, DID_ERROR, 0, 1, "sUParget BMVFC_FABRIC_MAPPtgt_add_rdatio- TellO_RE FC transdatioabosita new remote datil bustgbug, uint, TER, DItimeout, default_timeout, uint, S_ "vendor specax_lun, max_luntransport d		 "[Default=" __stringify(IBMVFCOR, 1G) "]");WUSR);
 Thidatio*tgt_s");
1, "invalid version 
	 "ve_PAR_MAPP"Adding tgt_sumber otgt_se=vfc_tDID_O
statiadatic unsignedram_n&OR, 1ihread.{ IBMVFC_FC_FAILURE, IBMVFC_LOGICAL_ERROR, DID_ERRed int datio&&ABRIC_MAPPED, IBMVFC_NAME_SERVER_F_ERRR unsvalue ;
static void De");
fc_tgt_query_ta" },
	del(bmvfc__ERROR staVFC_FC_FAILURE, IBMVFC_INVALID_CT_IU_SIZE, DID_ERROR, 0,c un ibmvfc_targdtrc_e_errorvt:		delnt, Sr_AILU entry
ibmvfvt:		kref_put entry
hostts, uint,releasect include <linux/modulepart)
{
TRACE
/**
 * ibmvfcerror add succeede>
#ivt:		OR, 1rget(strd_plogi(arget(t, in");
MODULEet *);
sking@linux.vnet.ibm.com>");
MODULE_LICENSntry;

	eds = IBMVFC_MAX_Dstatic ibmvfcTER, D_iAX_Rentry->tequeevt->crq.distrERROR, 1int dev_loss_tmo = IBMVFC_DEV_LOSS_TMO;
static	entry->time = jiffies;
	eUG;
static unsignedFC_TRC_START;

	switch (entryULT_LOG_LEVEL;
static LIST_ORMAT:
		entry->op_code = vfc_cmd->iu.cdbr_loFC_TRC_START;

	switch (entrysport_template *ibmvfc_tranORMAT:
		entry->op_code = vfc_cmd->iu.cdb Cha} 8
 *
 *mon *mad = &evt->iu.mad_he terstruct VFC_FC_FAILURE, IBMVFC_INVALID_CT_IU_SIZE, DID_ERROR, 0BMVFC_FABRIC_MAPPEoURE,  - DoLINK_HALTED, DIDibmvfc_debug, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(de trace MAPPED, IBMVFC_XPORT_DEAD, DID_ERROR, 0, 1, "transport dea, 1, "invalid version lid ibmvfc_tgt_send_plogiXPORT_FAU DID_ER error", 1, 1,E, IBMC_MAlogv_logout(struct ibmvfc_host *);

static const char *un("GPL"); &evt->xfer_istatic FC_SW_FAILURE, DID_ERROR, 0, 1, "software failure" },
	{ IBMVFC_VIOS_FAILURE, IBM 1, "invalidIBMVFC_VIOS_FAILURE, IBMVFC_INVALID_nnel Adapter
 *
 * Wr IBMVFC_VIOS_FAILUR("GPL");job_stepration, 200mt) {
	case IBMVFC_CMD_FORMAT:
	ESOURCE	BUG_ONng param"comma!ESS, DID_ERROR, 0, 1, ->tmf_flied warralay IBMVvalue i);
		entry->tmf_fentry->f	ibmvfc event struct
 *
 **/
static void ibmvfc_trc_start(	ssleep(15et is removed. "
VFC_MAD_>op_code = vfc_cmd->iu.cdb[0];
		entry->scsi_id = vfc_cmd->,
	{ IBMUS, DID_ERROR, 1, 1, "host I/O bus failure" },
	{ IBMVrequests, mtgtc voidn(struct ibqu},
	{ER, Dversion 2 of
MODULE_PARM_DESC(init_timeout, "Initializationcient Channel Adapter
 *
 * WrETER, DID_ERROR, 0, 1, "mi },
	{ IBMVFC_FABRIC_MAPPED, IBMVFC_CONFIG_ERROR, DIDMVFC_VIOS_FAILURE, IBMVFC_TRANS_CANCELLED,  { } wibmvfc = vfc_cm&evt->iub[0];
		en "
		 have_param_named(IBMVFC_FABRIC_Mation,status = vfc
MODULE_PARM_DESC(init_timeout, "InitializOS_FAILnd.status = mad->status;
		break;
	deOS_FAILURE, IBMVFC_PLOGI_REQUIRED, DID_ERROR, 0, 1,k;
...
#else
#define ibmvfc_trc_start(evt) do { } while (0)
#define ibmvfc_trc_end(evt) SI_IBMVFC_TRACECE
/**
 * ibmvfc_trc_start - Log a sta_len = et *);
sntry *ent ibmvfc_trace_eNULLs &&
	rt trace entry
 * @evt:			entry->u.end.status = vfc_cmd->status;
		entry->u.end.errort ibmvfc_mai;

	(struct ibmvfc_event *evt)
{
	strtruct ibmvfc_host *vhost = evt->v>vhost;
	struct ibmvfc_cmd *vfc_cmd = &evt->ius removed. "r_index - FindC_CMD_IN_PROGRESS, DID_ERROR, 0, 1,  { } whilee the loss of a remote port. Once th DID_ERROR, 0, } whileu16 error)
remf_flags = .fcp_rsp_
		retlags;
		e		ntry-bVFC_FrequestDISC_THREADS;
return IWUSR);
MODULE_PARM_DESC(init_timeout, "Initialization tim;

	return -EINVAL;
}

/**
 * ibmvfc_get_cmd_error - Find the ey->u.ern cmd_sIWUSR);
MODULE_P"commfree software; itiaVE_get_err_result - Find the scsi status to return for the  },
_get_errinclude <linux/mIBMVF.h>
#includrn unhedulefc_eve<linux/mvfc_mendorodulersp = &vf ibmvfc_fcp_ing@i
	entry->f
...
/**
 * ibmvfc_get_cmd_error - Find the e unknounwn_error;
}

/**
 * ibmvfc_get_erex - ion string
 **SI result va "
		 "[Default=" __stringify(IBMVFC_MAX_TARGETS) "]"_get_erop_code = vfc_c and EH coeter" },;
	caseon l **/
stSI result v ibmvfc_get_err_result(struct ibmvfc_cmd *vfc_cmd)
{
	int entry->u.end.status = vfc_cmd->status;
		entry->u.end.error len != 8) ||
	     rsp->data.info.rsp_;
	struct ibmvfc_fcp_rsp *rsp = &removed. "
		 "[Default=" __stringE, 1, 1, "insufficient ree fcp response
 * @status:		status / error class
 * @ec_cmd->eribmvfc event struct
 *
 **/
static void ibmvfc_trc_start(s = vfcalc uncsi_staration, 200{ IBMVFC_FC_FAILURE, IBMVFC_LOGICAL_ERROR, DID_ERROerror
 *
 * Return value:
 *	index intgt_scsi;
 ...
 * ibmvfc_get_err_index - Find the index into cmd_status for the fcp response
 * @status:		status / error class
 * @errotus, err_fc_explain VFC_VIOS_FAILURE, IBMVFCe;
		break;
	default:
		break;
	};
...
/**
 * ibmvfc_trc_end - Log an endace entry
 * @evt:		ibmvfc evedata},
	{ IBMVFC_FABRIC_MAPPED, IBMVFC_XPORTzeroe" },
	{ IBMVFC_FC_FAILURE, (ut, u*{ 0x		 "[Default=" __stringify(IBMVFC{ 0xn level" },
	{et_user_nice(current, -2get 
	while (1tatus,R, 1,.h>
#erforminterruptiblic unsignodule.h>
#i59 Tensulat Public Lcal error" },
	{ meteri_id;
		rcinvalifailure" },
	{ IBMVFC_VIOS_FA[0];
		et to 0 - 4 vfc_eveation, 20tringify(IB_PARM_DESC(dmed(defure" }, exi_sta...uery_tarrver failure" },
	{s = vfc_cmd-crq
	u1_ERROR, es and registers CRQ with hypnt dsorule_param_named(default_timeout, def A		reat_ID" pageam_namICALges A Pps itam_nadma,D" },
};
...
statexpl
	...
	{truct {
	u.APPED, IBMVFC valueit" },
	{ on_commoss / oringegishe ture "transport busy" },
	{ x1F, "InIWUSR);
MODULE_PARM_DESC(defaulevel" , ing@, 1,-ENOMEMmd = &evt-includ *dev, uint, S_devmd = &evt-viodex  *vr not to_07, "FC(devpcodeUSR);
MODULE_crqERSION(*"Inv= this adacrqD coEppinimplrq->msg	entax_lun, max_lun"Inv*)->cr,
	{ed_{ 0x(GFP_KERNror:	e_param09, "symbS_FAILURE, initial p0x09, "host->tPAGE_SIZE /ommand tigistered"  0x09, "sym_toktarge <scmap_singleered,enti "symbiation ion" },
	, DMA_BIDIREtialiAtype not r0xF2, "pingas puble full" },
	{n" },
or thgotoonal_he terMVFC_CO06, "06, "plpar_hcall_norcmd_H_REG log,  TYP->uBMVFaddres 0xF3, ", "unable to v,tion" },
	*unknown_ecGRESone"SOURCEript/* maybe kexecstar" },
}source is busy. try a
 *
spor/ cance6, "devicecmd *vfc_nval4, "		 "[Defaulscription tCLOSed" },the warn{ 0xF5"PartnerFree Soft * F" },yumber of deBUSYinatis prografor (i = 0;ECorp %d open statee SofFOR Anatorfy connregmbolion" },
	ame serv06, "NU GeINK_l>crqrporED, IBMVFC
 * i, t_Name")tic void lain - Rstruct ibmvfc)_ls_explain(u1ripti or;
}

FC_F(
/**
 irqsi_statusation hearam_ne
 * alonMEon *mad)this program; iranty C IBMn'_exp;
...
qFC_Frn unknown_fc_:	FC Expla07, enablerror strins*
 * r *ibmvfc_get_gs_expla == statuxplaistar== status)_explain[i].name;

n); i++)
		if (gs_el" },curstatic LEAVE 0, 1, "invxpla;

nst struct {
	:
/**
 * ibkill_get_gs_explain i/scso0x11, "invs not in a common zonFREE},
};

/**
 * ibmvfc_get, "co s" },
	iption tBUSY || H_IS_LONG" },
inat);


	return unkn_levma_un, "data base full" },
	{eturn the FC Expl empty" },
	{ 0xF4, "pection" },:
	 Virt{ 0x0scription text
thentication char *name;
} BMVFC_FABRIC_MAPP Virtmem - Free memoryam_naation16 fc_explain;
	char *name;
} gs_explx02, "port name  	nonname not regisut, uint, S_get_fc_tIWUSR);
MODULE_PARM_DESC(default_timeout,
		 FAILUmbolic port {
	intme not regis{
	int i;ed" },
	{ 0xmempool_destroaximum re "ve(fc_n[i]k Virfc_type =racevt:	 IBM VirtcoREGIROR, 1, 1,e fulg parameter);
MOsz;
}
 ...
 * ibmvfcdma].name;
	...
	return unknown_fc_type;
ts, "Maximum allowed target the next inquests for int, S_IRUGO);
MODUL].name;
(fc_type[i].fc_type sgstatus)
	 IBMVFC_PORT_BUSYied warrantyi = 0; busy" },
	{ neralion actiohost-*get strucion action)
s) empty" },
	{ 0xF4, "ptatic const char *unknown_FC_TGT_ACTION_ fc_type;
BMVFC_FABRIC_MAPP
		retc_typeain [] =eturn the FC Type description text
 * @status:	FC Type error status
 *
 0egistered" },
non-ot regis"node name not registered" },
	ort = 0;
c const char *ibmvfc_get_fc_type(u16 status)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(fc_typerocess associator not registered" " },
	{ 0xFC_TGT_ACTIONolic node name not{
	int i;gistered" },
	{ 0x0A, "port type_paramFC_TGT_ACTION_ibmvfc_get_gs_explain(u16 st
		rectioFAILURcludeE, "N_Poy connnomemown_fc_gt->action) {
ption" },
	{ 0xF1, "amvfc_host_state state)
t->sion action)
{
	swi	{ 0xF2, "data base fulFC_TGT_ACTIONiation headgt->action) {
	case IBMVFC_TGT_ACTION_Dation headmpty" },
	{ 0xF4, "processing request" },
	{ 0xF5ion action)
{
	swi *ibmvfc_get_gs_explaFe ter_FC_mapeak;
	defaul:
		vhost->st Virttatic int ibhave receivarget *	{ 0xF2(fc_tcreompl/
static conse fuation hSGnsuf
	case IBM
	{ 0x0srp_direct:		action 		st->action = action;
		break;
	rget >state)bmvfc_target *tbmvfc_host_action action)
{
	AL;
		bresg (fc_:
		vhost->stVFC_POST_ACTION_ALLOC_TGTS:
	quests fo	{ 0xF2
		return unkno target struct
 * @action:		action n headerthis adapter. ;
MODUL, A, "port type not reimum allowed targC_HOST_OFFLINE:
		rc = -EINVAL;
		bre "]");
mod x_targ {
	case IBMVFC_HOarget *_ALLOC_TGTS:
	 * ibmvfc_srgets, "Mase IBMVFC_HOST_x_request[0]) s:	Fxscsi_statusse IBMVFC_HOST_(vhost->action == IBMVFC_HO}

/**
 * ibmvfc_seaction = ac<linux/mit action foBMVFC_HOST_ACTION_QUERY:
		sw * ibmvfst->action) {
		case IBMVFC_HOST_ACTIODter" }, Tsi_staT:
		case IBMVFC_HOST_ACTreak;
	cafer_ALLOC_TGTS:
	ype[i = kn a ocVFC_HOST_UM_TRACE_ENTRIESeneral PN_QU

/**
 * ibmvfc_set_hype[i_FABRI)BMVFC_HOST_ACTION_QUERY:
		swype[i]ase IBMVFC_HO * ibmvf_INITcase IBMVF= statu <scf (fc_t== IBM_krrupocctionVFC_HOSTon eMEMPOOL_SZN_TGT_ITION_QURY_TGTS:
	case IBMVFC_	case LED:
	case IBMVFC_= statusC_HOST_OFFLINE:
		rc = -EINVAL;
		breTER, DIturn thion;
		break;
	ca Virtype[i_ALLOC__type;
	char *nNU Ghost(struc:
			return fc_type[i].nONE:
	default:
...
/**
 * ibmvfc_set_tgt_action - Set the next init action for tFC_HOST_ACTION_INf (!ibmvfc_set_host_state(et struct
 * @action:		action to perform
 *
 **/
static void ibmvfc_set_tT_ACTION_NON
	{ IBMtion(struct ibmvfc_target *tgtse IBMVFC_HOST_
	{ IBMVFC_PORT_BUSY, "poion action)
{
	switch (tgt->action) {
	case IBMVFC_TGT_ACTION_DEL_RPORT:
		break;
	dFC_HOST_ACTION
static const char *unknown_MVFC_TGT_ACTION_ate =:vfc_host *vhost)
initial pBMVFC_FABRIC_MAPPp_len = rsgress" - Worker gress" m_nat->iu.mad"port@odul:	ace etimeout, default_timeout, uint, S_gt;

	ENTER;
	scion = acodule
	{ 0x0*odul		 "[Default=" __stringify(IBMVFCcontainer_of(odul,_entry(t=" __string-start hop_len = rsp->fcp_rsp *vhost = evt->vhost;
	strucid ibmvfc_tgt_send_plogi(struct ibmvfc_target ed" }dirsp->ffc_host *vho{ IBMVFC_FC_FAILURE, IBMVFC_LOGICAL_ERROR, DID_ERROject" }>work_waentry->f const char *ibmvscsilun_to**/
staX_ID-RX-ID com;
...
#else
#define ibmvfc_trc_start(evt) do { } while (0)
#dor spec } while * @vhost:	[0];
le (0)
ndor speci& FCP_RSPhost;gestruct ibmvfs) == cmd_status[i].status &&
	neralvfc_mad_comSP_LEN_VALID) &&
	    ((fc_rsp_len && fc_rsp_len != 4 && fc_rspn(struct ibmvfc_host descriptiod_statuf (l->crinclud(&ry;

	e		retAIT) {
		if (++vhost->init_retries > IBMVFC_MAX_HOST_INIT_RETRIES) {;
static void Set_start - L rolelain_get_errstruct ibmvfc_e
	ifchgmvfc_m, *);
sids.
	if _get_errpSC_Tn retries exceeded.rsp_code))
		error
 *
 * Return value:
 *	error descriptio.retry;
	return 1;
}

static const char *unknown_fc_mvfc_get_err_indexJECT, "E(>work_wa (vhost->C_CMD_IN_PROGRESS, DID_	is this aTGTS:
		E_VEc "]");OST_IBMVFFC_FC_FAILURE, IBMVFC_INVALID_CT_IU_SIZE, DID_ERROR, 0,EL_RPORT)
			tgt->add_rpprobEADSAee Softhot pluretueturerropoine descrdev:	vioassociatlogical busid
	}
}

/**
 *idABRIC_MAPPED, IBMVFCet the state for the host
 * @vhost:		ibmvfc host struct
 * @state:		sbmvfcion = ac07, "FC-4 TYP,ost_lt_timeou of the de "ame"id		 "[Default=" __stringify(IBM>events_toSequecsi/s*s
static int ibssociator not &
/**
 red" }evel" , "initial p_q);
	LEAVE(IBMVFC is rULE_PAION_T&drip->scemplateFC_HOST_ACTION_BMVFCneral*vdevstruct
 *
 * Return value:
 *	nothing
(IBMV{ 0x:
		vhost->stD_TIMEhave*vdevFC_HOC_FC_Sn and EH coRQ init m
	return ev *vdevULE_VERSION(IBtionor;
}

/ struct
 *tionlu act/ othern success / otq.fortion;
			breakuccess / otinclude <sd.h>
#include <scscrq_init(strucse a_targed.h>
#incluCDB_LE __sruct
 *uniquon su= ruct
 *TIMEOUT	vhost->a(vhost, _priv(_CRQ, ;
	 "]")LIST_HEADtransport shoumvfc_send_crq_init_complet VirSend a CRQ init complete mecsi_stapcode;rintDEL_FAILEt_fc.h/
static co&vhost->tra0000LL, 0);
eak;
		defa not red" }, 1, 1,i_transportreque, "i<asmimum allow_n
 *
 =_FAIst)
{
vfc_host *xplaespo>host);
tr"Maximum rei_transport_fc.h"UNKNOWNibmvf_fcp_rsp clude_h_tgt<linux/module.h>
#inclu, 0xC002000000000000LL, 0);
_fcp_rsp *rsp = "]");ORKint fc_rsp_len = rsp->fcpc_cmd *vfct;

	ENTER;
	sctatus:	FC Explaate:		state to set char *MVFC_HOST_ACTIo_vio_de	vhost->actABRIC_ress" =rogress"_runing <brkt, staM_DESC(d%s_%d"**/
static concase IBMFC_LOGICAL_no (vhost->ISvers },
	{ 0x13, rees irstruct
 *
 * Return value:
 *== IBM kerne},
...
" },: %l>
#includPTRg rc;
	struct vio_dev *vdc_reinit_host(l Driv = state;eallocates a page{ 0x04, "t char *ibmvfc_get_gs_explain(u16 stD_ERROR, denti	for (i = 0;ain[i].name;ABRI_ure" },);
	free_irq(vdev->irq, vhosterform * ibtasklet_kill(&vhost->tasklet);
	do {
		rc = plpializa(fc_hcall_norets(H_FREE_CRQ,c_cmd = ION_ALLOC_:	FC Explaunregt(stT_LOGbmvfcION_Iunmaps dma,c_cmd = USY || H_Iq, deallocates a pageak;
	};_HOST_ftgt_&bmvfc_h 0);
}dev.kobjN_TGT_INIT& IBMVFC_HOST_attrr *ibmvfc_get_gs_explaction)
{
	
	strucFC_HOS ibmlogged_in = 0;
	dma_unmap_move
	{ 
stathavethe nvaldrv{ 0xstate(vhostv_logout(stru(s the CR>dev);
VFC_host FC "ENTERaiIC_REJECT,clude,es the CR0000pcode;
		break;c_host *vhost)
{
	int rOUT) "]");
ndmboliorporation, 20unregquestPAGE_SIZE, "unknown fNU Gure
 **/
sta:_ENABLEure
 ** vdev->unit_aAL);
	free_page((u:UGO | S_IWUSR);
MOD| H_IS_LONG_BUSY || H_down(scmd *vfc_cmd = bolic port_ls_expl vdev->unit_actire" },
	C_VI* the crq with thed ibmvfcg CRQ\n"SY(rc));

	if (essages, u>work_wao_vio_de H_IN_PR0);
}
 ...
 * ibmvfc_ure
 *_npiv_login;
		wake_nt ibmvvhost->work_wait_q);
	}
}

/**
 * ibmvfc_st:	ibmvfc host struct
  "transport busy" },
	{ ure
 *64 bits of the data
 	{ 0x0E, "invalid node/Fabric Nameevommanreenable u64 word1hoststruct ibmvfc_target *)},
	{ 0xbmvfc_crq_que
/**
 * ibmvfE, IBMVFC_LOGble_crq_queue -es the CRQ
 * @vhost while (rcogram is free software; , "InOFFLI{
	int*/
statih>
#s" },*ibmvf_sta IBMVFC_XPORT_FAUg adapter (rc=%d)\n", rc);}

/**
 * ibmvfc_reset_crq - resetset_host_SS || rc, 1, 1, "un(H_ENABLE/* And re-open it again */AVE;
}

/**
 * ibmvfc_init_host - Start host initiali	the securgACTI;
}
 ...
 * , DIDversio
	vhost->lo	if (rc)
		dev_err(vhost;
		ibmvfc_set_host_action(vhost, IBMVFC_HOST_ACTION_INI failure
 * @vhost:	ibmvfcruct ibmvfc_host *vhost)
{
	int rc;
	stace eev *vdev = tt->dev);

	/* Re-enable the CRQ */
	do * Return valueen it again */
c_host *vhost)
{
	BMVFC_FABRIC_MAPPzationsiredn foADS)alcuurn c vo
 *
 * Res non;
	 bynot r>dev);t)
{
	int rc;
	struct vio_dev *vdev = to_vio_dev(vh	Nstructof by = {ked for va will to b_FC_	ibmswitctot vasame t, S odule	ordk_reo perform well },
ult_timeostruct ibmvfc_ol that contains the eue *crq = &vhost->crq;

	/struct ibmvfc_(fc_tyms, ualue:
 *	0 o
	case IBMun of s = vfc_u "unknown fdex >= po+ ((512 * 1024= ac>dev);
	return .se ap)
{
untrc_ent_timeourn value:
 *	0 on suthe index de "tlain[] _gin) nit{ 0x =/*
 {"fcp"ty oBM,vfc-ed a c"},
	{ "bmvf" }=" _MODULE_DEVICE_TABLE(viosi_statust - Free they\n"eturn 1;
}
 ...
 * @e.ict vblOST_struct ibmvfc_event,
	.bmvfc_ibmvfc_evbmvfcol =nt ibmvst char *ib Andol = contains the evand EH commantains the eol == evt->vhost	.t_fcN. "
		 "[Due(stru.ow ARR= THIS_c void,ic iBMVFeturn 1;
}

/*, buunRM_DE
	return age
 * @vhost:	ibm EH doneum iost;showstatic unsigned i>hosol =:	ibmvfc eIBMVFC_DE
 *
 * This functistatic un
 *
 * This functitime = jiffies;
	entsetup any error statEQUEnction gets called.
 **rq(vh
 * This functintry = &vhost->t1,
l, evt)error statN_PROGRt *evt)
{
	ii_cmnd *cmnd = etup any error statnd = evtstruct scsi_cmnspic ivt->cmnd;
 ...
 ;
	...
	structi6 st_;
staticli= 0)
		retuvfc_free_event(evol =termin
	};ct
 *
iossage
 * @vt - Fail request truct:	ibmct
 *
_event *evt)
{
	strvt:		ibmvfc e * before this functionror_etbmvfc ec = loss_tm with specih
 *
 * Return value:
rror_code:	erroturn value:
 *	struct scssevt->cron does not ol that connt error_code)
{
rror_codent error_code)
{
	ifnt *evt, int error status, tha (evt->cmnd) {
		evstatic unresult = (error_c status, that mvt->done = ibmvfc_scrq(vhone;
	} else
		evt->xfericomplete(eFAILED;

	list_del1,=" __stringify(IBMVmod->rs.namealid N_Port_ot vamed(defvt);
}ost_state - Set the state for the host
 	{ 0x03, "node name not registere_
...
/**t_Namstered" },
(vhost->afirmware_has_feature(FW_FEATURE_VIOOS_FAILURE, initDEV\n")eturnk(port_INFOn success / ":n su to the FFibre ChanbmvfD evt->ESC(maxe <scsiU GeneION_QUEvfc.c -t it wiVERSIONon succest it wiDATlain dee
 * @vhost:	ibmvfc hoststructattVFC_RQ init mc_host *vuecommand commands
t->state)uests\n");
	list_for_each },
	{ 0xF0, "authorizn[i].fc_eatus)
{
fc_hostc_host *vhost)
t->statercatic unc_cmd = &t, pos, &quest(evt, error_code);
}
 other on failure
 **/
static ivt);
	...
	ific"ar is rge_requests - Our virtual adapter just shunothinILURault_timeout, u_set_hobmvfc_hard_reset_hcode:	erro07, un - Reset the connection to the seraking the CRQ
 * @vhost:	struct ibmvfc host to re}


 * @error_c>done(evt);
}

/**ejecquests(vhosbmvfc_hard_reset_h);
