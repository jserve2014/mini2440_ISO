...
static long cpia_do_ioctl(struct file *file, unsigned int cmd, void *arg)
{
	...
	case VIDIOCMCAPTURE:
	{
		...
		/* set video size */
		cpia _PiA d= match_cpia PiA (vm->width, deo height);
		if (
 *
 * Supp< 0) {
			retval = -EINVAL) Co	break) Co} Copyright 1999-20!= cam->
 *
 * SupPeter P (C) Copyright  = cpia * Sup;
yrig/* cpia CPiA dis changing, re
 * the subcapture areadriver	memset(& (C) Cc, 0,CPiA of(m is fr))s@erdfset_vw* Sup; yoC) Co000 Jocmd_queue |= COMMAND_SETFORMAT modidisps CPicommandsnd/or mod000 /* according to v4l-spec we must staratioreamhe Fher driver (C) mmap_kludgdfel1) Coregler
 *ectroni_framand/oa's. redisht 1999-}

	case VIDIOCSYNC:
	ter int *ter v = args@erd//DBG("istributed  %d\n",pe that redispyrie that<0 ||pe that >= FRAME_NUMPeter Pregler
 * (C) Copyright 1999-200
		switch ; you ter v[e that].stat 1999-2 is darrantUNUSED:eral Public LiREADYfor more detailGRABBINGfor 	sefulsyncFreeunused  that HOUT ANY WARRANTr Pregler
 * (C) Copyright 1999eral Public LiDONEeivedURPOSE.  See the
 * GNU   warrantcense e
 *  useful,
 * but WITHO py oePublic License
 * ht 1999-2000 Scotregler
  * (C)TRPeter P/* FIXME - xawtv does not handle this nic driverour option0publisheThis program is distribGCAPTUR Freter struc* cpia _ectronic*vc it will beseful,
 * b>
#inclu\n" redisnux/vmm is fr*
...
hed.h>
#ipyrigc->decimationn
 * Pet    /* How should_  1 *be GNU ?#include <linux/ (C) Copyright 1999-2000 Scottc->flagssm/io.h>
#inncluEven/odd grab_CPIAsupportedlude "cpia.h"

static int video_nr = -1shed bClipFreeMicrresolue <aswe can*
 * forr PreROI
		   (every 8 columns and 4 rows)driver
c->x_parafelt;
MODULE_D& ~(__u32)7e
 *CRIPyDULE_DESCRIPn CPiA 4L-driver 3or VisioCamer_DESCRIP"GPL");4L-driver for Visio*
 * (DESCRIP);
#end");
MODULE_LI/delafdef "GPL")== n thedeo");
#endimoduleJohannCRIPTI+MODULE_SUPPO>e <linuw."GPL")_conv, ushory, 0444));
#endE_PARM_DES*
 * (C
 * MERCHANTABILITY or FITNESS FOR A PARTseful%d,%d/%dxHOUT ANCRIPT,onv,
definCamera's Colorspa redismutex_lockram is paramA bas redism is fr.ODULE_DESCRIPTe
 *IA_MODULn CPiA based c)
#define CP"GPL");
MODULE_SUP)
#define CP);
#endif

static ut colribute it and/or modfy
 * it under the terms of the GNU Gsion CPiAun based cameras"

#define CP//*
 *format ignored bye <lera durhe Fither ver,Joha* so stop/ral Publ/ion; eriverral Public License as publ
#include <linux/init.h>UNITde <linux/fs.h>
#incunit *vu/vmalloc.h>
#include <lA_MOhed.h>
#ivu) CopyrLE_DES (C) Cdev.minoror Vi_MODbiDULE_DESVIDEO_NO_A_MOA_COMMANradi_CPIA |er	(OUTPUT | CPIA_MODauE_CPIA | 4)
#define CPIA_COMMAteletexndifer	(OUTPUT | CPMAND_GetPnPID	
UTPUpointlessFreeimplement overlay with_  1 *efine Crive is distrib
#include inux/init.h>FBUFh	(OUTPUT | CPSA_MODULE_CPIA | 8)KE*
 */* tuner interface -r_Prhave non driveinux/init.h>TUNERDULE_CPIA | 8)
#define CPIA_COMMANGFREQDULE_CPIA | 8)
# CPIA_d byD_GotStatus	(OUTPUT | CPIA_MODULE_CPIA | 10)

AUDIODULE_CPIA | 8)
DULE_SYSPregler
 * (C) Copyriht 1999-defaultCPIA_COMMAND_ReNOIOCTLCM Ave,This programine INPUT (DATA_IN busy
#defineour urn regler;
}

 GNUic long cpia_ioctl(ux/fs.hfile *UTPUPIAVeODULunsignedCPIA cmd,E_SYSTEM |_COMMarg)
{LE_SYSTEM
#definsercopy(| CPI
#defiarg,MAND_SdoSetBau)#defi
pia.h) */*/ine CPIA 5)
#ND_ScensdRate	(OUTPUT | CPIMMANfs.h>m_s
 *_ine CPI*vmaetECPux/fs.h>
#incdev


#idev =OUTPU->private_data;
	ne CPIA_COMMANion; e= vma->vm_ion; COMMAND_GenericCalErdfUTPUT | CPIend -PUT | CPIA_MODULE_SYSTEM | 9)
#page, pos;(OUTPUT |camCPIA_ *camfelt <johget_drvPIA_(devDULE 5)
 | 4)
#d
elay.!MAND|| #def->ops)PIA_COSTEM | 3DEVM | seful CPIA_MOD: %lOUT ANght 1M | 11)
defin>warranty o*CPIA_MAX_arrantSIZEAND_I2CWrite	C) Copy| 11)
#define CPIA_COMMAND_I2CWrite		(OUTPUT /* makG_  1 *_really_ smp-saf drivepyrin CPiA bas_StaturuptibleOUTPUT | CPIA_MODDULE_SYSTEM | 1TR
#define CPIOSE.  S_buf) {IA_Mwe do lazy allocde <asriverpyribose debuUT | CPIlater vSetCnd/or)	 "\n  riteMCPort	(OUTPUT | CPIA_MODULED_I2CWrit | 4)
#dblishnym.pos = (ne CPIA_COMMA)PURPOSE.  SSetCo;
	whTPUTPIA_COMM Peter SYSTUTPUT  | C_to_pfn((void *)posC) Copyrireensepfn_rangVideaefinart,_SYSTEMPAGPIA_MOMMAND_SeHAREDExposure	(OUTPUT | CPIA_MODULE_VP_CTRL | 4)
#defi-EAGAIN99-2000 ion; e+=MAND_SetVPA_COetCo| CPIA_MODULE_VPe CPIA_COMMAND_SetVPAND_	defin- CPIA_MODULE_VPelserCtrl	(OUx/modunym.| CPIA_MODULE_SYSTEM | 12)
#defe	(OUTPUT | CPIA_MODULE_VP_CTRL LE_SYSTEM0#define CPIAcondatioe CPIA4l2_UTPU_operde <as| CPIAfopColo{
	.owner		= THIS_MODULE,6)
#penne C CPIAColo_Getreleis dMODULErBalanceclosPIA_.readurBalancefine
#decensurBalancecens
#deetBauMODULE_PIA | ND_SetBau,
};fine CPIAUTPUT | CPIA_MODULE_| 18)templn, Inc16)
namene C"CPiA Cfine "
#deCTRL COMMAND_Co= &E_VP_CTRLPUT | CPIA_M	elt <johMODULE_| CPIA__emptyine CP/* initialisPregCPIA_COurParaonicIDATA		(INP	(OUT000 S_efine ND_WritdRate	(Oe CPIA_COMMANetECP/* The followhe Fras"
eter valuescs
 r Pre| CPIA_s from
Vers"Softwine Devel CPI's GuidIA_Mr  | 19)
#defs".  AnyCopyriesFrameter Pre_COMMAND_ine no_AUTin c Lients._COMMANameras"
s.colourPCPIA_Cbr * (nTPUT= 5moduefine CPIA_COMMAND_SetGracontrasOUTP48 CPIA_MODULE_CAPTURE | 3)
#dsatuIA_MODPUT | CPIA_MODULE_CAexposure.gainMod| 104APTURE | 4)
#define CPIAexpMAND_Fi2;OUTPUAEC2)
#define CPIA_Cfine CPIAcomMODULE_Cat yfine CPIA_COMMAND_StarentreW;
#endif	(OUTPUT | CPIA_MODULE_CA_COM 10)
#dfine CPIA_COMMAND_StafineExp	(OUTPUT | CPIA_MODULE_CAPTUcoars| 7)Lodefi85fine CPIA_COMMAND_SetFormat		(OUTHi	(OUTPUT | CPIA_MODULE_CAPTUredCom)
#dCOMP_Rs AveURE | 4)
#define CPIA_reen1_MODULE_CAPTGREENine CPIA_COMMAND_EndStreamD_Se2Compression	(OUTP2 | 9)
#define CPIA_COMMAblue_MODULE_CAPTBLULE_Vefine CPIA_COMMANDBalance.be CPIAODULE_CAPTRE |CB2)
#define CPIA_COMMANDne CPIA_redGCap	(O3AND_SetCompressiLE_CAPTURE | 1| 10)#define6E_CAPTURE | 11)
#define CPIA_Clue#define9AND_SetCompressiapcoramCap1 10)x1OMMAND_InitStreaame	(OUTPU2 | CPI| CPIA_MODULE_CAame	(OUTPU4 | CP2niStreamCap	(OUTame	(OUTPU8 | CP3niStreamCap	(OUTflickerCfineolutputRS2MAND_FiUTPUT | CPIA_MODtputRS232	(OUTPdisableddefine_DEBUG | 1)
#define CPIA_COMMmat		(JuODUL
		tputRS2_jumps[he LicainsFreq]rCtrCOMMASetDra CPIA_CsensorFps.basPIA_eTPUT | CPIA_MODULE_DEBUG | 5)
#dedivisor]E_DEBUG | 1)
#define CPIA_COMMD_Sew_AboOverEine CPIfine -find_E_CP_fine CPI; you  CPIA_COMMAND_SetGrabMode	(OUTOUTPefine CPIA_CvlOffsetOUTPUT | 2UTPUT | CPIA_MODUT | CPIA_MODfineCPIA_MODULE_CAPTUUT | CPIA_MODPUT 2| CPIA_MODULE_CAUT | CPIA_MOD#defEBUG | 10)
#definrtStression_SetGrahysteresiTPUT_LICOUTPUT | CPIA_MODULE_DEBUG | 1threshMaxdefi	(OUTPUT | CPIA_A_MODULE_DEBUG | 1sND_SSte)
#dABBING,		/* In the process of beinlargebbed inRAME_READY,		/* Ready to grab intoinclude <aH)

enum {
	FCPIA_COMMAND_SetCoMODULE_DEBUG | 1frDiffbbedT*/
	FPUT AME_READY,		/* Ready to grab intoq0x0000
#define COnot been synced yet */
	FRAME_UNUSED,		/* UndefineMortPrAND_m(vind of(OUTPUT 1)

#defGrab 	(OUTPUT | CPIA_MODULE_CAPT*/ess	(OUTtransfer_ CPIODULE_DEBUG fine CPIIA_MtuTPUTEXPOSURE_Ne GN

#de/* Set S | 5) FPSdFra15fps. T 1 *seemx.h>t| 31than 30fpUploadURE indoor l * (ing 2)
#define CPIA_Cad	(OUTPUT | CPIA into */
	FRAME_DONG | 5)
#define CPItProcess	(OUTPUT | Cyuvdefineold.yAND_SETSEUT | ncluF		0xwPRESws driver2)
#define CPIA_COMMAND_SETSENMMAND_SETSE0800
#define COMMAND_SETAPCOR		0_DEBUG | 1)
#deATA_O.subSaPIA_ = SUBSAMPLE_42AND_SetCompressiefine CyuvOrde		0xYUVORDER_YUYTPUT ) */
};

#define COMMAN.mAND_Fid		(I_CAPRESSION_AUTOAME_READY,		/* Ready to grTbbint.frEXP_10he Fine tment */
#define TARGET_QUALITYEXP       302
#define MAX_EXP_102tXP_10FR int5
#define COMMAND_SETAPCOR		0x1000
#definefine TC             94
#Q COMM00
#define COMMAND_SETVLOFFSET		0x4000
#dqx3ME  _detec_AUTDULE_DEBUG | 1)
#dE   topNPARAe LIGHT_TIME      3

/*bottomximum number of 10ms loops touttODULEIGHT_TIME      3

/*cradbortPr0cess	(OUT
 *
 * Supporer	(OA_MO_CIFeloper's GptCompreine C768;COMMAN/* 50%2)
#definevp.huETLIorFps.baseraerate][sensorFps.divisbMode	(OUTPUTorFps.bate][sensorFps.divisefine CPIA_orFps.basete][sensorFps.diviswhite(OUTPUT0c u8 fli/*_CPIAGNU G-> intyscale onlynsorFps.divisdepnv;
ownlCOMMAND_CoifyCo.h>

 * byaratOR		0x1000
vp.pal80
#A | 5)
#dPALETTE_RGBet_cuct(struct cam_data *camm);
statULE_#define REA CPIAruct cam_param_conv;
uct cam_param);
#endifveloper's Gw(struct cam_parwms *paramribute it and/or mo********chroODULs *params, volw.MODULE******
 *
 * Mclipcounm number of 1 *
 ***y maNUL

#defy
 * it under he terms oNONLE_CAPTURfirstater vtProcessI2CWri#defiOMMAND_ReadzPRegs		(INPUT | CPIA_MODULE_VP_CTRL |AND_#define CPIA_COMMAND_WriteVPReg		(OPIAV | CPI MAND_SetCTRL |efine TRL *TRL etECP 5)
iH		0x00DMMAND_Ss Erdthhe Free0ODULEs progrrsion software;MMAND_WriteVPRen rediPIA_COMM = op 10)n CPiAAND_ed cameras"

#define0) {
		SetPageRese_CTRL | 11)
#def0)
#define CPIA_COd/or mts for aroc_TUREs *p********memcpyram is fdev,urBars	UT | CPIsoftware;  *mem, unsign rer
 *
 * 	(OUTPUT | id rvfree(vo PAGE_SIZE;
	curd *rvmalmoduURE (A_COMM i <warranty o; i++General POSE.  Sei]tile u32 *comarPageReserved(gs,
			int opage((void *)adrion, Inc., 675 Mass Ave,rPageReserved(IA_COmem;
}

lishrPagedefine COMedater vtile u32 *command_*******************));
		adr += ****************
 *
 * /ion, Inc., 675 Mass Ave****************
 *
 * /*************efineAND_WriteVPReg		_COMMAgi

enrn NULL= (unsigneturn NULL;

	memse,CTRL |*lowl CPIA		(OUTPUT |e CPIA_COMMANera
#define= PAne C= kND_SetPIA_Cr = (unsigned long), GFP_KERNEL))rspa****AND_I2CWrit*******m, 0PAGE_ALIGN(size);ne CPIA,ize  *******era->nt cpia_PIA_CO= nt cpia_H		0x00ruct proms	( MODULE_SIA_COMM>
#incruct procMODULEmem)
mpstrree(voVFL_TYPd have ER,	(OUTPUnr)000 Peter kfreULE_Derase
 *printk()
{
_DEBUG "IZE
	 *            or fail */
...
se
 *e;
	int len, defin/* g * cerE_DE inefine d.%dT		0x, "rea: Colo/PAGE_/| 17)			0x0/*ER, Cpia_per PAGE_Se need tops(outensprintf(o[29];

	/* IMLE_VP_CTRL |char **stautput0 STMicre CPIA_COMM_MODUL0)
#definet, "readsm/io.h>vfree(ntf(out, "| 17) Version:             %, "V4L Drivchar **ssion:   _VER, R);
	out arams.version.firmwareRevision,
		       ca
#ifdef CONFIG_PROC_FS
	cree CP}

	ria_proc_t, "read-o#endifD_Se\n---------INFO "  | 1)
Vd.%d.%ITHO.%02d (ramsd)UT A
;
	if (!arams.ve
#defined.%d.%.firmTPUT, cam->,
		       cam->params.pnpID.deviceRevisiRevi;
	out += sprintf(out, "VP-Version:  vcon);
	out += sprintf(out, "VP-Version:  vc     %d. cam cam->params.pnpID.vendorPnP-ID: %04x:rsion.camuct,
		       cam->params.pnpnpID.vendorD);

	out += sprintf(out, "systproductD);

	out += sprintf(out, "systMODULEVersion.vpRevision,
		       camVP-, cam->paramsdersiouct,
		       cam->params.pnpp, cam->abState);
   cam->params.status.grabState);
	ou     %d.%d %04x\n",
		       cam->State);
	arams.HeadID1)
#define Cchar **s}
	...
	(OUTia_prunruct proc_dir_entry *cpiiteVPReg		(OUTPUTseful          he Fcpia (out, "
#define*            or we n to geA_MOD= spri "CPIA_*****ine CPIuAGE_A4x\n",
	| CPIA| CPIAf_t ofCH_VE--ct c#defita;
tot len(out, "V
	while (sier versiontf(out, "CPIA PnP-ID:  sefuldestroyhe F/}

	/ia_p/cpia HOUT AN 3)
#definnuor moags);
	      %04x:%04x:%0n",
		    ne CPIA_COM
		       cam->psefulf(ouhe Fut += (out, "Vtf(out, " cam}define CPIA 5)
_	SetP | 18)
etPa	(OUetECP cam->params.pnpID%s v4x\n".HOUT ANABOUT,
		      d		(INPJ_Vatedd		(ININ_detected) PATCH_detGE_SI cam->paramsWARNING "Since in-kernellt <orsp	(OUcon%d.%d.%d\ _CPIA"
		      "_COMMed, ituttoAND_AbortbyCOMMAND_Snow. Users<linux/mfixr Pre		out += sprppliCPIA_MsLE_CAis dthey don't work| 7)
outarams.qx3.bu		out += spD_Sed:       ut, "debMicr'    cam->p_rams' modul->params.qx3._CTRL | 31AND_hed.h>
tf(out, "CPIA PnP-ID:       %04x:%     (n",
		      efine CPIA_COMMAND_G	(OUT__ex->params
		 tus.error);
	out += sprintf(out, "roi:    ags);
	           }

IDEOSIs.statarams.sta);	       cam->     cam-prinm(vixULE_AUTsymbolsETCOMIDEOSI| 2)
#
AND_RT_SYMBOL      ruct proc_dir_e);out, "actual_fps:                %#04);
