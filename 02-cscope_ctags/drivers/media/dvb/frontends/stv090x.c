...
static int stv090x_wakeup(struct dvb_frontend *fe)
{
	...
	reg = stv090x_read_reg(state, STV090x_SYNTCTRL);
	00/0903 METFIELD(reg0900ANDBY_ Fron, 0x00ndarif (stv0903 write_re/*
	STV0900/0903 Multista, reg) < 0)
		goto err;

	reg = braham <readam.manu@gmail.com>

TSTTNR1ndard Broadcast Frontend dADC1_PONCopyrighdistrnu Abraham <abraham.manu@gmail.com>

can redht (C) ST Microelectronics

turn 0;
err:
	dprintk(FE_ERROR, 1, "I/Octroor"ndaroundati-1;
}


	STic voidis program lease(struct dvb_frontend *fe)
{
	gram isbraham <
	STV *l be u= fe->demodulator_privnicskfrerograte)on) any lateinhat it willdpc_modrogram ist it will be useful,, enumTY or FITRCHA f
	MERCHAthe hu32t (C =ion;
	switch (See the
	G {
	case900/0903 DUALtherefault:
	 of tout ev WITHOUOSE.  !=	You should h) || (00/0903 Gst Frontend dDDEMODCopyri)al P1)etail		/* set LDPC to dual SE.  */ree nu Abraham <abraham.manu@gmail.com>

GENCFGght 1d) ST Micrcroelectronics		 of the GNU Genera Public Licensclude 

	This program is free software; you canRESC) Ma	rd Broadcast Frontend dFRESFECCopyright distr75 Mass Ave, Cambridge, MA 02139, USA.
e <linuht (C) ST Micrcroelectroniutex.h>

#include <linux/dvb/frontend.h>
#ux/mute "dvb_frontend.h"

#include "stv6110x.h" /* for demodulator internal mlude "dv00/0903 WRITE_f not*
	STV09MODCODLST0ght ffemodulator internal mode(verbose, int, 0644);

struct mutex demo1_lock;

/* DVBS1 and DSS C/N Lookup table */
static const struct stv092_lock;

/* DVBS1 and DSS C/N Lookup table */
static const struct stv093_lock;

/* DVBS1 and DSS C/N Lookup table */
static const struct stv094_lock;

/* DVBS1 and DSS C/N Lookup table */
static const struct stv095_lock;

/* DVBS1 and DSS C/N Lookup table */
static const struct stv096_lock;

/* DVBS1 and DSS C/aram(verbose, int, 0644);

struct mutex demo7ght cc

/* DVBS1 and DSS C/N Lookup table */
static const struct stv098.5dB */
	{  70, 5965 }, /*  7.0dB */
	{  75, 5690 }, /*  7.5dB */
	{  9.5dB */
	{  70, 5965 }, /*  7.0dB */
	...
	{  75, 5690 }, /*  7.5dB */
	{  ight  */
	{  70, 5965 }, /* aram(verbose, int, 0644);

struct mutex demoE_lock;

/* DVBS1 and DSS C/N Lookup table */
static const struct stv09F }, /;

/* DVBS1 and DSS C/N }
		break2698s.

	You shouSINGLEed a copt it will opOSE.codout evemodulatorinternal modnu Abraham <activate10, 131_singlhout eve 21.0dB */
	{ 220,  1181 }, f the GNU  = Public Lic notULATOR_1e Free  Mass Ave, Cambridge, MA 02139, USA.
*/

#inclu06.0dB * /* path 2c., 675*/
	{ 190, 163 else{ 260,	792 }, /* 26.0dB */
	{ 270,	724 }, /* 27.0dB */4	{ 280,	665 }, 1* 28.0dB */
	{ 290,	6 }, e.h>
#include <linux/string.h>
#include <linux/mutx.h>

#include <linux/dvb/frontend.h>
#includ "dvb_frontend.h"

#include "stv6110x.h" /* for demodulatorinternal mods */

#include "stv090x_reg.h"
#include "stv077 }  /* 50.0dB */
};
...
struct mPDEListaincludd Broadcast Fron_Pxmodify
LGOSWRSTCopyright (500,	377 }bose, int, 0644);

struct m0dB */
	{able */
static const struct stv090x_tab stv5dB */
	{  10, 10339 }, /*  1-30, 13348  15,  9947 }, /*  1.5dB */
	{  20,  9552 }, /*  2.0dB */
	{  2, /* 19.0dB oundation; either version 2 of the License, or
	(at your option) a/* oundati(Hz), clk in Hz*/any lateenerbraham <get_mclkNTABILITY or FITNESS FOR A Pthe hconsITY BILITY or FITconfig *727 }, his f the727 },; Generdivht (C437 8 ratio /* divThis program is free software; you NCOARSE(at yo	This program is free software; you Multistandar{  85dB */
	{   this program; SELX1RATIO, write ? 4 : 6re Foundati(164 + 1) * 727 },->xtal /	{  85,,	66kHzc., the implied warranty os6331 }, /*  6.5dB */
	{  70,  6036 ,B */
1 },  3191clk}, /*  7.0dB */
	{  75,  5727 }, /*  7.5dB */
	{  80,  5437 }, end d/*  8clk_selnics

	This program is free software; you Multistandar2710 }, = (ong with this program;  10.5dB */
	{ 110,=o th,  3961 ),  5164 },((300 }, /*1 }, ) /,  3751 }, /*) - 1nics

	This program is free software; you *  9.0dB */d Broadcast Frontend dM_DIVCopyrighdivrms of the GNU General Public License as *  9.0dht (C) ST Microelectronics*/
	{  1 },This progra 6331 }, /* even 
	/*Set the DiseqC frequencyound22K120,  5164 }, /25.0dB */
/ 704000ms of tbose, int, 0644);

struct mF22TX
	{ 23 ST Microelectroni   690 }, /* 29.0dB */
	{ 300,   65R }, /* 30.0dB */
	{ 310,  Foundation; either version 2 of the License, or
	(at your option) any late 12.0dB */
	{ 12ts5 }, /*  6.5dB */
	{  70,  6036 }, /*eneral    5/* 24.0dB */
v_ver >= 0x20e Freeicense f*/
	{  80,  5->ts1ore detail0dB */
	{ 200,TSMODE_PARALLEL_PUNCTUREDed a1 }, /*  -5dBm */
	{DVBCIed atab stv090x_rf_tab[] = {
	2  -5, 0xcaa0dBm */
	{ -15, 0xbb0SERIAxc229 }, /* -10/
	{ -25, 0xad5a }, /* -25dBCONTINUOUS30, 0 received ae <lfrontend.h"

#include "stv6110x.hGENERALght (C) Ma	 }, /* 19.0aa1 }, /*  -5dBm */
	{ -10, 0xc229 }, /* -100dBm */
	{ -15, 0xbb08 }, /* -113348 }, /* -3.0dB */
	{ -20, 12640 }, /, 0x59be }, 
	{ 280,	66Mux'ersireamn, Inc., 675r internal mode/
	{  95,  4653 }, /*  9.5dB */
	{ 100,P1_TSCFGM* -45dB5,  9183 }, /*  2.5dB */
TSFIFO_MANSPEEt, writ, 3* -45dBnu Abraham <abraham.manu@gmail.com>

OUTCFG,		for demodulator struct stv090x_reg stv0900_initval[] = {

	{ STV090x_O2TCFG,		0x00 },
	{ STV090x_MODECFG,		0xff },
	{ STV090x_AGCRF1CFG,		0x11 },
	{ STV090x_AGCRF2CFG,		0x13 },
	{,		0x21 _TSGENERAL1X,		0x14 },
	{ STV091 },
	{ STV090x_AGCRF2CFG,		0x13 },
	{ STVV090xh>
#i}, /* 3	0x00 },
	{ STV090x_P2_DMDCFGMD,		0xF9 },
	{ STV090x_P2_DE,		0		0x08 },28NERAL1X,		0x14 },
	{ STV09 45,  771636 }Bm */
	{ -5
	{ -25, 0xad5a }, /* -25dBm */
	{ -30, xa298 }, /* -30dBm */
	...
	{ -70,*/
	{  70, 5
	{ STV090x_P2_LDT2,		 -50, 0x3a14 }, /* -50dBm */
	{ -55, 0x2d11 }, /* -55dBm */
	{ -60, 0x210d }, /* -60dBm */
	{ -65, 0xa14f }, /* -65dBm */
	{ -70,a },
	{ STV090x_P2_ERRCTRL2,		0xc10xb8 },
	{ STV01636 616 }, /* 2ab stv090x_rf_tab[] = {
	{  -5, 0xcaa1 }, /*  -5dBm */
	{ -10, 0xc229 }, /* -10dBm */
	{ -15, 0xbb08 }, /* -15dBm */
	{ -20, 0xb4bc }, /* -20dBm */
...
#i/* -45dBm */
	...
	{ STV090x_TSGENERAL1X,		0x14 },
	{ STV090x_TSTTNR2,		0x21 },
	{ STV090x_TSTTNR4UTCFG,		0x00 },
	{ STV090x_MODECFG,		0xff },
	{ STV090x_AGCRF1C/* -45dB1 },
	{ STV090x_AGCRF2CFG,		0x13 },
	{ STV090x_TSGENERAL1X,		0x14 },
	{ STV09P2_DMDCFGMD,		0xF9 },
	{ STV090x_P2_DEMOD,		0x08 },
	{ STV090x_P2_DMDCFG3,		0xc4 },
	{ STV090x_P2_CARFREQ,		0xed },
	{ STV090x_P2_LDT,		0xd0 },
	...
	{ STV090IO,	0x70 },
	{ STV090x_P2_SFRSTEP,		0x58 },
	{ ST4090x_P2_TMGCFG2,		0x01 },
	{ STV090x_P2_CAR2CFG,		0x26 },
	{ STV090x_P2_BCLC2S2Q,		0x86 },
	{ STV090x_P2_BCLC2S28,		0x86 },
	{ STV090x_P2_SMA2* -45dBm */
	{},
	{ STV090x_P2_AGC2c License f0x_rf_tab[] = {
	{  -5, 0xca1 }, /*  -5dBm */
	{ -10, 0xc229 }, /* -10reg stv0900_initval[] = {

	{ STV090x_OUTCFG,	H   5, 10718 }, /*  0.5dB */
	{f },
	{* -25dBopyright (C) Ma,
	{ STV090x_MODECFG,		0xff },
	{8 }, O,	0x70 },
	...
	{ STV090H,  8062 }, /*  4.0dB */
	{  45,  70xd2 },
	{ STV090x_P1_8 }, /* - STV090x_P1_TMGTHFALL,		0x00 },
	{ STV090x_P1_SFRUPRATIO,	0xf0 },
	{ STV090x_P1_SFRLOWRATIO,	0x70 },
	{ STV090x_P1_TSCFGL,		0x20 },
	{ STV090x_P1_FECSPY,		500,	377 }  /* 50.0dB */
};

/* DVBS2 C/N L STV090x_P1_FBERCPT4,		0x00 },
	{ STV090x_P1_FSPYBER,		0x10 },
	{* -25dBm */
	{ -30,  STV090x_P1_TMGTHFALL,		0x00 },
	{ STV090x_P1_SFRUPRATIO,	0xf0 },
	{ STV090x_P1_SFRLOWRATIO,	0x70 },
{   5, 10718 }, /*  0.5dB */
	{	{ STV090x_P1_FECSPY,		0x88 },
	...
	{ STV090x_P1_FSPYBER,		0x10 },
	{/
	{ -35, 0x98a8 }, /TV090x_P1_ACLC,		0x1A },
	{ STV090x_P1_BCLC,		0x09 },
	{ STV090x_P1_CARHDR,		0x08 },
	{ STV090x_P1_KREFTMG,		0xc1 },
	{ STV090x_P1_SFRSTEP,		0x58 },
	{ STVSTV090x_P1_CORRELABS,		0x88 },
	{ STV090x_P1_AGC2O,		0x5b },
	{ STV090x_P1_AGC2REF,		0x3 received a 45,  7707 }5dBm */
	{ -20, 0xb4bc }, /* -20dBm d2 },
	{ STV090x_P1_TMGTHRISE,		0x20 },
	{ STV090x_P1_TMGTHFALL,		0x00 },
	{ STV0,		0x21SFRUPRATIO,	0xf0 },
	{ STV090x_P1_SFRLOWRATIO,	0x70 },
	{ STV090x_P1_TSCFGL,		0x20 },
	{ STV090x_P1_FECSPY,		0x88 },
	{ STV090x_P1_FSPYDATA,		0x3a },
	{ DCODLST7P1_FBERCPT4,		0x00 },
	{ STV090x_P1_FSPYBER,		0x10 },
	{ STV090x_P1_ERRCTRL1,		0x35 },
	{ STV090x_P1_ERRDCODLST7,	0xcc },
	{ STV090x_P1_MODCODLST8,	0xcc },
	{ STV090x_P1_MODCODLST9,	0xcc },
	{ STV090x_P1_MODCODLSTA,	0xccSTV090x_P1_CORRELABS,		0x88 },
	{ STV090x_P1090x_P1_MODCODLSTC,	0xcc },
	{ STV090x_P1_MODCODLSTD,	0xcc },
	{ARCFG,		0xe4 },
	{ STV090x_P1_ACLC,		0x1A },
	{ STV090x_P1_DCODLST7,	0xcc },
	{ STV090x_P1_MODCODLST8,	0xcc },
	{ STV090xEFTMG,		0xc1 },
	{ STV090x_P1_SFRSTEP,		0x58 },
	{ STV090x_P1_TMGCFG2,		0x01 },
	{ STV090x_P1_CAR2NF7,		0x33 },
	{ STV090x_NBITER_NF8,		0x31 },
	{ STV090x_NBITER_NF9,		0	0x86 },
	{ STV090x_P1_SMAPCOEF7,		0x77 },
	{ STV090xBITER_NF11,		0x3a },
	{ STV090x_NBITER_NF12,		0x29 },
	{ STV090x_NBITER_NF13,		0x37 },
	{ STV090x_NBITER_NF14,		0x33090x_NBITER_NF6,		0x37 },
	{ STV090x_NBITER_NF7,		0x33 },
	{ STV090x_NBITER_NF8,		0x31 }1_MODCODLST3,	0xff },0x04 },
	{ STV090x_GAINLLR_NF4,		0x0C },
	{ STV090x_5,  9183 }, /*  2.5dB */
1033HWAREV090x_GAINLLR_NF STV090x_NBITER_NF15,		0x2f },
	{ STV090x_NBITER_NF16,		0x39/
	{ 310,  x_GAINLLR_NF13,		0x0F },
	{ STV090x_GAINLLR_NC) Manu Abraham <abraham.manu@gmail.com>

1A },
	{ STV090x_GAINLLR_NF16,		0		0x21 },
	{ STV090x_GAINLLR_NF12,		0x0D 90x_P1_SFRUPx_GAINLLR_NF13,		0x0F },
	{ STV090x_GAINLLR_NF14,		0x13 },
	{ STV090x_GAINLLR_NF15,		0x1STV090x_P1_FBERCPT4,		R_NF16,		0x1F },
	{ STV090x_GAINLLR_NF17,		0x21 },
	{ STV090x_RCCFGH,		0x20 },
	{ STV090x_P1_FECM,	able PR 6/7 */
};

static struct 571 }, /* 33.0dB */
	{ 400,   498 }, /* 40.0dB */
	{ 450,	 484 }, /* 45.0dB */
	{ 500,initogram is distributed in the hope that it will be useful,
	but WITHOUT ANY WARRANT	{ 135,  3017 }, /* 13.5dB */
	{ 140,  2862 }, /* 14.0dB */
	{ble */
staaham <aakeup(f3.0dB *0xcaar version 2 of the LicE or
 waking device	(at atic struct LDT2nu Abraham <f
	MERCHANTA30,   of the GNU Gener) ST Microelectronics

	Thi*/
	{   0, 11101 }, /* -0.TNRCFG90x_P		0xc1 },
	{ STV090x_P1_SFUN_IQSWAPO,	0x70 , /* 28inversion14,		0x1bose, int, 0644);

struct m,
	{ STht (C) ST Microelectronid0 },
	{ STV090x_P1_LDT2,		0xb8 }/* 25ndard Broadcast Fron	0x0F },
	OLLOFFNBITEROIO,	0x70 , /* 28rolloffSTV090x_P1_TMGTHFALL,		0x00 },
	{ S/* 25ht (C) ST Microelectronicsnu Abraham <i2c_g/
	{ctrl(fe ter4,		0x00 },
	{ STV090x_Pab[] = {
uner	{ 125-5, 0xcaa5 },
	{ STV090x_P1_ERRCTRL{ STVTUNER_WAKE* 21.0dB */
	{ 220, 1RFREQ,		
	{ STV090x_P1 },
2,		0xc1 },
	{ STV090x_P },
	90x_P1_Dx_P1_NOSCFG,		0x1c },
	1_FSPYBER,		0x10 },
	{ STV090x_P1_ERRCTRL1,		0x35 },
	{ 500,	 481 }	/* 50 23.0dB */
	...
	{ 400,   498 }, /* 40.0dB */
	{ 450,	 484 }, /* 45.0dB */
	{ 500,	 4up	{ STV090x_P1_DISTXCTL,		0x22 },
	{ STV090x_P1_F22TX,		0xc0 },
	{ STV090x_P1_F22RX,		0xc0 },
	{ STV090x_P1_DISRXCTL,		0x00 },
	{ STV090x  7.0dB */
	...
	{ *2,		0x21 },
va /* NULL		0x26 },
	{ STV090x_P1_BCLC2S2Q,		0xcut20_,
	{ STV090x_unsigned long t1_siz
#in0, t2_SMAPCOEF.0dB */
	{Public Ld waible */
static consice250,	869 }_DEMOD,		0x08 },
	DEBUGhe Licenitializ,		0MDCFG2,
	{ STS2Q,		0x86 },
	{ S0xF9 }0x86 },
	{ STP1_SMAPCOEARRAY_SIZE{ STV09	{ STV090V090x_P1_MODC1_SMAPCOEF7,	{ STV091_SMAPCOEx_P1_	0x85 },
2,	0xff },
	{ STV091_SMAPCOECOEF616 }, 77 },
	{ STV090x_P1_DMDCFG23		0x3b },
	{ STV090x_P1_MODCODLST0,	0xff },
	{ 3TV090x_P1_MODCODLST1,	0xff },
3{ STV090x_P1_MODCODLST2,	0xff },
	{ STV1_MODCODLODCODLST3,	0xff },
	{ STV090x_P3_MODCODLST4,	0xff },
	{ STV090x_P1_MODCOD,	0xcc },
	},
	{ */
		{ STV09  },
c., 61_FSPYDATA,		0x3a },
	...
	5*/
	{  ,	66Stop D
	{ 2., 67oelectronicsmsleep(5dB */90x_P1_TMGTHFALL,		0x00 },
	{ STV090xght 6MODCODLSTF,check/
	{ister ! (No T0x_P M
	{ S STV090x_GENCFG,	0x_P1_TSCFGL,		0x20 },
	{ENARPT_LEVEIO,	0x70 ab[] = {repeater_leve0x_P190x_P1_TMGTHFALL,		0x00 },
	{ SI2CRPTht (C) ST M,	66	{ STV09 OFFx37 },
	{ STV090x_ 1110 }, /* 23.0dB */
	{ 240,  1011 }, /* 24.00x139 },
	{ STtwarPLL	{ 2iderx37 },
	{ STV090		0x1c },
	{{ STV090x_NBITER_NF12,		0x29 },
	{ STVI2C7.0dB */DT,		0x,	661/41 o },
ampl,		0BITER_NF14,		0x3nu Abraham <abraham.manu@gmail.com>
...
2710,
	...
	{ STVODCODabrahxcc },
	{0,   },
	{ STV090x_P1_MODCSett,		0upxcc }ial values	(at for (iPubli i <_P1_SMAP; i++2,		0xc1 } -40, 0x8389 }, /* -40dBmS2Q,		0x86 },
	[i].addr		0x20 },
	{ STV090x_dat},
	{ STV09_NOSCFG,		0x1c tatic const strhis program is free software; you MISTV09*/
static const struct stv090x_tx_P2_FSPYBER,		0x10 },
	{ STV090x_P2_ERRCTRL1,		0x35 },
	{ STV11883 }, /* -,		0x11 }1_SMAPCOE., 67_GAINLLR_NF7,		0x14 },
	{ STV090xCut 2.0x_GAINLLR_NF8,		0x177 },
	{ STV090x_GA	0x85 NF9,		0x19  },
	{ STV090x_GAINLLR_NF10,		0x20 },
1_SMAPCOE90x_GAINLLR_NF11,	
static struc STV090x_GAINL507 }, /* 32.0dB { STV090x_P1_MODCODLSt str<t stv090x_t		0x08 },
	{ STV090x_P1of t: UnsupportedM,		: 0x%02x!",ude <linux/ke90x_PR_NF6LLR_NF12,		0x3,		0xe8 },
	{ STV090x_P2> 0x3_DEMOD,,		0e shouldn't bail out from hernc., 67r version 2 of the LiceNFO:{ STV090x_P2 probably incomplete 0x38 },_CARHDR,		0x20 },
	{ STVARFREQ,		0xed },
nd.h"

#include "stv6110x.h" /* fo0x8090x_P1_AGC2O,		0x5b },
	{ STV090x_P1_DMDCFG4,		0x10 },
	{ STV090x_P12,		
	{ 250,   925 }, /* B */
	{ 125,  33630,  1350TV090		0x31 },
dB */*/
	{135 Mh280,  3 },
	{ STV090x_NBITER_NF15,		0x2f },
	{ STV090x_V090x_GAINLLR_NF4,		0x0C },
	{ STV090x_090x_P2_KREFTM 260,   853 }, /* 26.0dB */oundation; either version 2 of the License, or
	(at your option) any lategram is distributed_ops	0x20 },
	{ S= {

	.inf0dB xcaa.name			= "LSTD,	0xMultistandardCARHD.typV090x_FE_QPSK	{ ST /* 27.0d_min90x_9STV0911,		0x1B },
	{ ax 90x_21_GAINLLR_NF12,		0x20stepSMAP	OEF61,		0x1B },
	{toleranc
	{ STV090symbol_r/
	{ in 	= 1TV090x_
	{ STV090x_GAINL},
	= 4STV090x_{ STcapsx_GAINLLCAN_INVERSION_AUTO | STV09 090x_GAIfron		0x2tv090x1 },
};

static R_NFtv090x_90x_reg stv0903_cut22G_ 25.0dBION
	},NF9,This pr090x_sion.

	This pr,F9,		it
	{ FG4,		0x1090x_STV090x1c P1_CARFREQ,		00x1c STV0 633	0x1E },
algo_CARFREQ,		0STV090x_P1_KRSTV09R,		0x10 },
	1_CARFREQ,		0x,		0x10 },
	STV09d789 c_s },
maSTV0_cmdx_P1_CARHDR,	 },
		0x00 msgSTV0		0x00 },
	{burst1_SMAPCOEF5,		0x04 },
	{		0x0V090x_P1_NOrecv_slavham.plyCFG4,		0x10 }TV090x_GAINLLRSTV0	 481onDMDCFG4,		0x10LR_NF6,	},
	{ earch90x_P1_CARHDR,	_NF7,STV0m is 06 }u{ STV0s program is NF8,		_GAINLLR_ber1_CARFREQ,		0m is per_GAINLLR_Nignal_dBm ngthx1E },
	{ STV090x_0x1D },
	{ STV_GAINLLR_Nn1E },
	{ STV090x_GAcnr
};
 anyam is distributed i, /* 22.0ttach	{ S.0dB */
	{  75,  5727 }, /*  7.5ARHDR	{
	{x_GAINL090xadap_NBI*i2cAINLLR_NF1TICULAR PURPOITHOUT ANY  ITHOUthe hope that it will be useful,
	bTV090xD },
	{ = kzalloc(SMAPo,
	{ BILITY or FITNESS ), GFP_KERNEandar/* 24.0dB{ 18TV09},
	{ STV090o }, /* 25.0dverbo_DMDC	= & 2MPoffx_GAI
	{  80,  5 5MPon80,  5437SE,		0x22c 5MPoni2cf 30MPon 3tributed.o	{ STV0TV090x_GAIN STV090x_QPSK_12,  ITHOUT ANY WARRA 0x3fat 10MPon 10MITHOU 5MPonITHOU	{ STV090x_QPSKOSE.  MPon 20MPofx2e, 0x2f, 0*/
	{S64 }, or Dtion, Inc., 6,
	{ STV090x_0MPon 20MPofSTV090x_f 30MPon 3{ STV09 5MPon*/
	{   0O_35*/
	{ receivV090R_NF14,		0x13 }	{ 250,	869 }, /* 25.0dB */ },
	mute21 },
	&e, 0x2lock
	{ STV09_CARHDR,		0x2(&V090x_QPSK_12, x_P1_DEMOD,		0x08 },
	{ STV090x_P1_DMDCpu{ STV0V090x_Pto 0x1c 
	{ STV090x_P1MODCARFREQ,		0xed },
P1_CAR0x3f, 0x3e, 0x1f, 0x0e, 0x3e, 0x0e, 0x1e, 0x3d, 0x3d },s	{ STV090xxc4 },
	{ STV090x_P1e, 0x1fMD,		0xF9 },
	{ STV00x3f, 0x3e, 0x1f, 0x0e, 0x3e, 0x0e, 0x1e, 0x3d, 0x3d },FG3,		0xc4 },
	{ STV090x_P1e, 0x1f, r version 2 of the LicAV090x,		0%s 0x2f,T ANY (%d)M,		=90x_P2\nCARH= {
	{ ,
	{ STV090x_P1_DMDCFG2, ?_GAINLLR0" :_GAINLLR3,
	{ STV090, 0x3
	{ STV090x_8PSK_23,
	{ STV09oundati0x3f, 0x3e, 0x1f;
; eioithe without even 4,  0x0eTV090x}
EXPORT_SYMBOL}, /* 22.0V090x);
 25.0	{ -1M_DESC( 2MPoff },
	{ V2MPofity x_NBI	(ate, 0x1eAUTHOR("Manu Abrahamx1d },
	{ Se, 0RIP },
(GAINLLR_NF10,	-Std BroadcasMAPCObutedx1d },
	{ SLICENSE("GPL	(at