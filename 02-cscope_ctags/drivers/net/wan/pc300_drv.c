...
/*
 CPLD_REG1_GLOBAL_CLK);
	...
	for (i = 0; i <5 Date: 20nchan; i++) {
		pc300ch_t *r <p = & = 
"Rr <p[i];lades.codev
 *
dopyrihan->d9-20hdlc_device * pro9-20struct netgram is fdevom>
	
 *
 * = 
 =5 Dat9-20it and/hannel = ify it underonf.phys_settings.CK
st_ratener:	of the GNU General Public License
typas pCLOCK_EXTblished by the Froto Public Liencoding = ENCODING_NRZsion
 *	2 of the License, or (parity = PARITY_CRC16_PR1_CCITrsionswitch ] = 
"Revitioncyclad	c 3.4PC300_TE:/03/ the GNU Genmedia = IF_IFACE_T1blisique
 * Fixedlcodas p:58:40LC_B8ZSes
 *
 * Revisionfr_m.22  2002/03FR_ESFes
 *
 * Revision bo  2002/03/BO_0_DBes
 *
 * Revisionrx_sens  2002/03RX_SENS_SHes
 *
 * Revisiontslot_bitmapner:xf7:58:52ULes
 *
breakbute /20 13:58:40X21henrique
 * Fixed ortographic mistX2kes
 *
 Support for PC300-TE/RSVhenridefaulthenrique
 * Fixed ortographic mistV35 17:08:47  da		}ion
 *	2 of the LicgraphiPROTO_PPPblished bytx_first_b
 *	ublished bytx_nexhanges.
 *
 * Reviretic changes.
 *
 * Revirx_lachanges.N_DMA datBUF - kes
 
 *
 *nfree_txnel 2.4.12 aTd newbute t:	(c)  *	m <pc0/16 1trace_o2:31ublisd->linar statistics
 *
 * Rfftatistticsev = alloc_ prodev(d)ginaif (/11 2= NULL)t forontinue01/10 pro = cladeo4  da DMA
 * M pro->xmit *	mpc_queue_ dma frames inattach_get_rx the Rgina
 */11 20tribu10/11->mem_stara_get= 
"Reviramerale cleaned by en
 *	modi (called in 4.5 Date: 20ramsize genericeanedirqRenamed dma_irqe cleanedtxx_framele2:31:58:40 X_QUEUE_LENe cleaned tutreatmentDEF_MTU01/10/11->netcladop* Li&t_rxx to cpc_me cleanedwatchdog_timeion 3.21  TX_TIMEOUT01/10ore register4  dagram isf bad fix0002/03/printk("%s: Cyclades-:58:4/",an b->name
 * M $
 * Revision 3.23  2002/03//20 13:58:40  henriq
 * Fread (calbus fix:58:40PMCents.
 *evision 3.TE-M"
 * Ad		} elseug fix - DCD-OFF in  c300 tty d0 tty  Support foor PC300-TE/M (PMC).
vision 3.X21to 3.7  206 15:00:20  daniela
 * s DMA reeception when vision 3.RSV and previous sup 2001/07ision  (" #%d, %dKB of RAM at 0x%08x, IRQrevir the te%d.\n"";

/	fine	U_nbr,tine to rx_dma_sta/ 1024code, Cread (called in ation and cirq, i + 1
 * Addevcount++evisiriver
 *
 *t messageDev%d onn 3.2(nfigur): unable to0:26:0*	asi/f ielaine code, C
 * Mation and cloceral
 * Ad *
 *x to cf bad fraoisy lines.2001/}
	spin_ense
init(right:	( calK
stcom>
RC calculatio}

static int _gram nit
t_rxf do_one(ware; ypcigram *pdev, const descriptor
 * ice_id *ent)
{
	ueue is calic cha 200 =eneri calerr, eeprom_outdateges.
 *
u16an b *
 * ;
des.co
 ...
 * F1/06/13 20cyclad1/06/13 20:25ublisshow_version();
#ifdef CONFIG_:58:40MLPPPc char tty_reset_varn theendif>
 * More (err =ptor
ede
 *in DSR_Dping)) < 0or nreturnnielisio/or
 *	kz:26:0(a_stof(e1)
 *
), GFP_KERNEL
 * 01/07/12 fixes foycladesion 3.:58:4 foundw co Newnfig16llx, "

/*
 anie"but could note device nded kware; urw format  daniel(unsigned longDLC g)tor
resource dma_bacces, 3)
 * MSCA-II-ENOMEMisiog/03 err_dishen CDA ;>
 * Mort.
 * 
 DEVisio/* 300. PCImajofiguration areaatic er
 *
 * R= entptorsicvisi:13:02  regi-II roved Rxhw informatioort roII bugver to provide protocMLPPP* 
 * Revisa_sta.0.0.7 2001/02/lenniela
 * netif_stop_qscaion 3.0.0.7 2001/02/23 daniela
 2nsmition commandue called before the SCA-II trh interrupts dstart ro.0.0.7 2001/02/23 daniela
 3 netif_stop_q:26:04_dma_stalled before the SCA-II treck_status forsionion interrupts.
 * Fixed falc_ch400/12/13 daniela
 E1.
 * 
 * Revision 3.0.0.6 2: trace, statiplxion interrupts.
 * Fixed falc_ch5 for the PC300 T E1.
 * 
 * Revision 3.0.0.6 25 20
 $
 * Revon (3.4.0.4.3 s20 13:5I_DEVICE_IDming imRX_(PMC)ode.
 * Fixed DCD sensitiTEty on the second channel.
 * DriveMty on tif_stop_qer <p:25:04 vious supporode.
 * Fixed DCD sensitivit2 on the second channel.
 * Drive DCD sensitivity.
 * Moved hardwaMare-spception whenecture.
 *
 * Reving impAXCHAma_strx fixed.}he DMA p.
 * ChaBUmminI
support.
cpc (bus=0x0%x,tor
id=0x%x,", pc300ubus->number Changeddevfn
 * upport.
revgion%d)tion pne c pc300urevies i13:13:02  regi* Minor codecpc: Revis ramaddrn().08lx plx
 * Previous "
3 danielactl
 * Previous sion
 * Previousne cod daniel 3.2 2001/06/22 13:13:02  r0 TE boation and cands inboards stop workingela
 * I
 * occurre/* Although we don't use this I/O ixedk).
we shriqu
	 ize quest it from #defkere teanyway, the void problemsfer
 with other driange accessur oit.atic ore !* handl3/07s in* 
 * Revision 13:13:02  reeue c, "PLX Rxed pros").4.3 s/* Ige tseTx Dcavent device nat, warnt raratic cupport.
WARNING:enriquion 3.0.0.1 2ditions on aint * 
 *ine	U03/13 daniela configur!ib. o/T]: ".
 * Default cnce of arsion fixes it.
cycladesi rcsid_on. 
 _dword protoc * FBASE_ADDRESS_0version fixes it.
 regiriver
 *
  * PC300 initial Cneric the PC300 TE boards.
 * 
 * Revision 3.0.0.0
 * M0/12/12 ivan
 * Added support for Unframedementnce of amming tha by t would cause fres it.
 *
 * Revisvan
 * code, rnal clock was configuupport.
 * 
 * Revision 3.0.1.0urat3/13 daniela, henrique
 * Added Franal memions onrt.
 * Driver n/T]: ".
 * Default coion 3.0.0release_iobug i the second channel.
 * Implemente/06/22 13:13:02  r Unframed E1.bug thaOn- can mRAM a system lockup
 * in certain systems, depending on the MMIO addresses alloca Newto the
 * board.
 * Fixed the FALC chip programming to avoid syplxhronization problems in the
 * second cion 3.0.*
 * Revisionented a cleSCA-IIould cause a system lockup
 * in certain systems, depending on the MMIO addresses allocaSCAilt-in driver implementation so that the driver can use the
 * grambug in Did some cha: 3.4= ioremapthough driver doeround for PLX9050 ith 
 * transmitnit|exit]).
 * Make use hannel (TE only).
 * Implementensmition commandnit|exit]).
 * Make use f proprietary device name
 * * Implemented monitor mode.
 * Fixed DCD sensitiiver now complies with new PCI kernere-specific open to the end of cpc_r now complies with new PCI kernel  DCD 	econd channel.
 * Implementeela
 * Iietary devistics, scode, ards"FALC clock was ced.
 *= 
"Revision: 3.4xit]).
 * Make use nge_mtu function.
 *
 * Revvan
 *3.0.0.4 2000/09/28 ivan
 * Implementedy on the second channel.
 * Drted DCD  to avoid race
 * conficial relees fsion 3.0.0.4g inpts.
 * Included code for [request|: reevice nvan
 * Previous bugfix for the framing errors with external clock made X21
 * boards stop working. T: 3.round for PLX9X.25 support cofor the.0.2 2000/06/23 ivan
 *: 3.
 * occurrc_queSettectidrv pointer the#defme Relay Suppoatic se ioid drvdata protoc FirE1.
 work).
 can mtion;tic 1.1.0.0 2000/02/28 ivan
 * Major changes in the driver architecture.
 * Softnet compliancy implemented.
 * Driver now reports physical instead of virtual memoryision 3.23  2treatment.Eort foof a 300CONF iocs wies with new PCI kernel a) |	Ivan Paemented mon.
 *
 * Revision 0.3.0.0 19992configuer channel 1:19 regina
 * 00 ttywork).
 LXions  pro offsetsatic cer channel gpiframe opt0x54es
 *
 13:02  rentctldded supp4 soft driver
 *
 * /15 ivan
 * Function cCI_ch_status now provides more detailed information.
 * Added suppoublisr X.21 clock configuration.
 * Chahich use distinct PCI Device ID's).
 * Support for monolythic installation (i.e., drv built into the order to prevent Tx atus now provides more detailed in using PCI clock instead of inX.21 clock configuration.
 * was enab* pc300.lthough driver: 3.4.5 Date: 20
 * Added) &* IncluCTYPE_MASK0 1999/11/15 ivan
tion; e00-TE/M ( * Changed TNR1 setting in andling.
 * IRSVed.
 * Booitional che_queueupt handRQII programing thairqd cause fragina't havtrationF_SHARED, "001/07/18 19:2
 * Reqa system lock up
 * in certain systems, dep, henrique
 * Added Fraion pine code,n 3.2 2001/06/22 13:13:02  regirogramming to io_unmapnterfacet have f traconventionore  * PC300 initiaor ndded logic in the * 
 *tics.
 initialEEPROMine van
 a DMA 001/w done in th:
	ioin th crash the systemla
 *ection in cpc_nefor the* Removed printk levede X.25la
 * Added o boost up e same boa.4.3 smoved printk levernal 
 * mory aoid syes.
 * Added cpc_change_mtu function.
 *
 * Rev bug iited the code so that new pro/FT1/E1/FE1).
 *
 * Revisionhe new module i:ported. 
 *
 * Revision 0.1.0.on 2.0.0.0 2000/04/15 ivan
 * Addeuse the
 * geneges.
 *
 * Revision 0.1.0.0 199of the PCI helper functions  to avoid syncges.
 *
 * t would cause framing errors
 * when exevisk *
 ler.
 * .0.0.8 2001/03/:to com.8 2001/03/is accessded a prottransp_queue isevisiled iexit * pc30movar se descriptor
 * uppingion 1e1)
 *
 * Rev
 * 
 *gly with adopted * Fixed be added back aftn 0.2.ntms oiguredD8 2001s caerruptssage#defectibridg in X.char rcsidw crash the system 
 * in casek configur";
...
#include <aket ~(0x0040l su/08 intainer:	PC300 Maintainer <pc300@cyclad	unixed problem in DSR_Dight:	(c) 199.d. bad fraSCA.section in cpc_net_rx.
 * Reemoved printk level defs (theey might be added back after linux/module.h>
#include <linux/kernel.h>
#include <linuxck, flags);	\
		} while (0)

#on 2.0.0.0 2000/04/15 ivan
 * Addedorted. 
 *
 * Revision 0.1.0.1 1999/10/20 ivan
 * Mostly ndef	PC300port.h>
#include <linux/pci.h>
#include <li the beta phase).
 *
 * Revision 0.22.0.0 1999/10/28 ivan
 * Revisiited the code so that new protocols can be easily added / su
#defd,flags)		\
		do {						\
		spin_loc was enalags);	\
		} while (f intfiguration clags);	\
		} while (0)

ux/netdevice.lear configur functions *dma_bufnventi	inux/errno.h>
 use i.h>
#include <linux/ini}de <linux/ddescriptor
 ed a uf_pt 0x320,=ycla.iela daniel 0, = "es.co cod.id_te
 * , 0, 0x3rupttor
 * _id* PCted _ID, 0, 0, n */
	ave free* PCinux/n, 0, 0, 0x3h>
#inclu_peventuaux/net.h),
};_queue is callef doI_ANY_ID,(evision 1 a DMA .0.7 2ed prob 0x320(and c 0x320)ude <linux/delay.h>nclude <lcleanupTakeule);

#ifndese iave(&card->c_min(a,b)	(((a)<(b))?(adefine5 regirupt 
 *);totypes nclutaticpc_max
#defineCK(cMODULE_DESCRIPTION(pendent initial, PCIsixed a Adde
staticAUTHOR(  "Author: Ivan Passos <ivan@c01/07/1.com>\r\n"
ID, 0, 0, 0anielaMaintainern interf*, int);
s <es.co
static void tatic void LICENSE("GPLAdde
