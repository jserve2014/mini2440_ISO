...
#ifdef CONFIG_EISA
	/* Take care of the r fo devices */
	eisa_driver_unregister(& EtherLnald Becker);
#endif

	if (compaq_net_6-1999) {
		vp = netdevLinkXaccording to the te;
		I III/ = ioport_mapaccording to the t->base_III/,
		 he 3Com "Vortex" anVORTEX_TOTAL_SIZE);

		

	This song tderal Public License, incorpwrite16(TotalReset, rporate+ EL3_CMD incorelever 	Thionreference.

	This driver is for the 3Com "Vortex "Boomerang" series ethfreeMembers of the series include}
}


static m Etherexi/* EtherLcleanup(m Et)
{uted a EtherLhave_pci)
		pci.

	This so Beckerftware maused and /O
	Scyld ComputinaldCorpware may be @scyld.c);authomodule_initScyld Coort );ld_ supped aScyld Co@scyld.);
