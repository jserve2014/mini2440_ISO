...
static void ql_reset_work(struct work_struct *work)
{
	...
	if (test_bit((QL_RESET_PER_SCSI | QL_RESET_START),&qdev->flags)) {
		...
		clear_bit(QL_RESET_ACTIVE,&qdev->flags);
		clear_bit(QL_RESET_PER_SCSI,/*
 * QLogic QLA3xxx NIC HBA Driver
STARTight (c)  2003-200ql_cycle_adapter(*
 *,QL_NODriver QLA}
}

static void ql_tx_timeout_work(struct l.h>_#includ*l.h>)
{
	#includql3 copyrig **
 * =LA3xontainer_of(l.h>, ude <linux/types.h, linux/kernel.h>..h>
#;

a3xxx for copyright and QL_Densing dets.
 */

#include <get_board_info
#includnux/types.h>
#inc#include <linuxxxx_port_registers __iomem *de <linus = *
 * Qmem_maplinux/memp;
	u32 value.h>
threanux/l_read_page0linu_ldma-map&clude <li->de <Statuc QL
	*
 * Qchi
#inv_id = ((
#incl& PORion
 TUS_REV_ID_MASK) >> 12 QLAif include <linux/ip.h>
64)qla3
 * Qpci_width = 64;
	elseinclude <linux/if_eth32h>
#include <linux/if_arp.hX#include <linux = 1.h>
#include <linux/ <li0;lude <lilinusloth>
#u8) PCI_SLOTht an->ptlinkdevfnclude <linux/sched.hincluux/kr(unsigned long ptr#include <linux/types.h>
#inclu ux/slab.h>
#include <)ptrinuxueue_delayedel.h>
lude <l.h>	"qla, /*
 * Qlinlinuateel.h>, 0clude <linux/constinclude net_device_opsh>
#inclnetdev#defi= {
	.ndo_open		ude inclu con,
staticstart_xmitst char ql3senddriver_namopst char ql3closedriver_naet_multicast_lisinclNULL, /* not allowed on NIC side */
staticchange_mtust cethCRIPTION("Qdriver_nvalidworkaddrogic ISer " DRV_VERSsion[] = DRV_ac_VERSess_NAME;
stati);
MODULE_VERdriver_nlinux/kernst char ql3linux/kern,
};.
 */
...
#includlinuincl*.h>
,
				 V_VERSION	"v2.ETIF_MS5"
#id *linuentry#include <li03.00-k5"
 *nde "ql_AUT;de <linux/mm.h>

#include "qlram(debugETIF_MSG_cards_foun.h>
linuMSG_un | Nialized_var(linuusing_dac), errncluerr =

staenabla3xxc in(.h>
.h>
#incerr)"

s	printk(KERN_ERR PFX "%s canQLogODULE_<lin ARM_DE\n" | N  ql3xxlinunamESC(msi QLA3gotoam(m_outetail, int, 0);
Mreques<linuxonsSC(ms, DRV_NAMEi, "Turn on Message Signaled Interrupts.");

staobnux/ruct resourcesvice_id ql3xxx_pci_tbl[] __devinitdata = {
	{_disULE_P.h>
PCI_DEVff.h>RSION/memSC(msi, >
#inc!_tbl);

dma
/*
k QL3022_MA_BITde <l(64)) Messagsi;
module_p<linux/ int, 0);
M);

_VERx/menPHY's which are used
 */
typedef eetai 
#in  the n on0,
   PHY_VY's which are used
 */
typed32) enum {
   PHY_TYPE_UNKNlinu  = 0,
   PHY_VITESSE_VSC8211,
   PHY_AGERE_ET1011C,
 VICEPCI_DEVTurn on Message Signaled Interrupts."no uE(pciusedV_VEfigurationuired last entry */
	{0,}
};

MODULE_DEVICE_TAfree_QLOGIC,PCI_DEVdule_paic Cc_etherdev(sizeofux/slab.h>
#include;

st the dule Message Signaled Interrupts.")ould"QLogic Ccic IVITESvice_id ql3xxx_pci_tbl[] __devini} PHY_-ENOMEMNOWN"},
	 {PHY_VITESSE_VSC8211, 0x00ver
NETDEV_DEV(dulencluh>
#incle areDEVICE_etrvdata QL3022  0x0nclude < =s abV_NAprivu32 s

stlude <inde <lievel (0=nonmap_regisdule_paduleinux/skbuf*port_ ql3xxxlude <tic int de 0);
MODtrymask | sinux/skbufe <lXXX Networ = LS_DOWNT1011C,msi#include <msi<linux((sem_mamsgMODULE_rt_regif_s.se | N(debug, default	if e are themsi;
module_pa
	003f1->features |= NETIF_F_HIGHDMAT1011C,sem_mask | sem_bi= QL3032sk, ICE_IDturn 0;
		ssleep(1);
	} while(IP_CSUM |	} while(SG>CommonRegsck.h>
#include <bits),
ioreh>
#bnt m2 sem1ET1011C, /spinlock.h>
#include < Message Signaled Interrupts.:");

stamap pci_de inux/mempust take hw_lock.
 */
static int ql_sem_sIONOWN"},
	 {PHY_VITESSEonds = _DEVspin_lock((valu/*
 * Qcopyrig, u32

stqdev, u32 sem_mask, u3hwits)
{
	
	/* Set driver 		    pointsODULE 0;
			DRV_NAME " "&ne PFX		DRV_NAME ;  u32 ETHTOOL_OPSu32 sem_har ql3ethtool#def

st 0;
		watchdognux/ke = 5 * HZiomeg);
		napi_VERort_regs-istersapi,de <poll, 64_iome 0;
		irq		write>> 16iomem *make sure the EEPROM is goodODULE;
	reh>
#innvram_paramsreturenum {
 e Signaled ALERTerru_id ql3xxx"TIF_MSG_LINK: Aypes.h>#%d, Iner "  NVRAM t_forememp.vice_id ql3xxx_registers;;
}

static int ql_sem_lock(struiounmapapter *qlVERSION);<linurvr_liomem *Vr " DRV and setdev,
				 Q
 */
stationRegs << 1QL_RMessa 0;
		mtunux/spinlql_wairt_r.macCfgude <1.CalleMtu;
MO .qla3xxERSION);

stu32 sems: Timed out waitfung forfn2tingALE_VERAX_PHY_DEV_(KERN_ERR PFX "%s: Timed out waiting for "
	0		       "driver lock...\n",
				       qdev->ndev->name);
				retu0n 0;
			}
		} el
	memcpyu32 s->permE_LICEcondsmask e(struct ql3_VERS_lede <_mask >>tx_	"qla3len_DESCM_REQ_Q_ENTRIESiomem *Recordruct bus <linrmN,   .ODULE.h>
#include <linu{
			if (i 
	  *portler Maximum Memory Read Byte Countkthrea. We do this to handle< 16)jumbo f,
		s.< 16;
			} else {ncludnum {
   PwriteVITEfigel.hd= qdev-(int)0x4e, (u16) 0x0036

static} PHY_inux/mem		DRV_N->mem_mapTurn on Message Signaled Interrupts.emaphoreRinux/mems ab pci_device_id ql3xxx_pci_tbl[] __devinitdata = {
	{->mac_index)
		/* we're goingreadreset, so assume we have[] =XXX  for nowspCo->Commoncarrimodulf->mem_mapg);
		ar q3xxx_p->mem_ma((sem_maRING 	"QL
	u3reworkmodulethlinuxRING 	"QLegister_tbl

stINIT_DELAYED_WORKmask, u3qrestk Drivee <li0_reg_l(eg);
}

static u32 ql_read_palinux/kernel.h>struclinux/kernel.h>eg);
}

static u32 ql_read_paXXX Network DriveqlnRegs.semap;
MOhinrk Drie are nitlude <lask, u32 sem_biude <_map_regisster_page(qde);
		,   lude #include <,0);
	value = readl(regexpip(1)= jiffies + HZ * 2;g);
two second xxx"
;

	wr
	value = readl(regrt_rncludnux/delay.h>)reg( are t(!evel (0=nonock(struct ql3_adapter *qdepts.vice2_DEVSTRING QLA3ge != 0)
		ql_set_regisDregs =_tbl: %s, VersionwritL_DRVR_ ql3ct q_DEVICE_age(qdVERSIONvoid qlqlABLEplayPARM< 1)) ct ql3_adevel (0=non++;
	returnhyId
s);
	value = re:
	>mac_in else {
s;
	writel(sem_ma;k_irqsavect ql3_ad:
	ct ql3qdev,
			     {PHY_VITESSE_VSC821:uct qlreleasSE_VSC821SC(msi, EVICE_TABLE(pci, ql3w_flagsBLE(pci,ARM_DESC(msi, "t ql3xxx_port_register_AUTc void ql_:;

	spin_m(msis.
 */

#includPROBEex NETIF_MSGremovK
    | NETIF_MSG_IFUP	/* defaults above */
module_pa   P
#in_port_regist{
	st<linux/mm.h>

#include "qlregs = qdev->mem_ma
	un_adapter *qdev,
			     e, reg);
	readl(reg);
	udela *reg,ULE_Pinterrupt_drvr_l are theapter *qdev,
		tk(KERcancel3xxx"
#define read_page0_reg_l( QLA3xe)
{
	if (qdev->current_paned long hw_flags;
	destroyeg)
{
	retu__iomem *reg, u3.qla3pter *qdev,
			  ram(debatic lock, hw_flags);
	writel(value, rflags);
	return;
}

static v
				u32 __iomem *reg, u32 valspin_unlock_irqrestos.
 */
...
staam		  apter *qd
	.id_tphoreRe
#includci_tblwrit_LINK, reg);
	reLINKwritatic v =g);
	retur_p(;
}

static v)_DRV | NETIF_MSG_PR | NETIF_MSG= 0)
module(nclu#incl
	spin_lags);dapter age !=(s->Commoage !=clude <linux/sched__eturn;
}

steturtruct ql3_   PH(1);
	retu     u32 __iomem *reg, u32 _reg(s((valu_write_page2_reg(s);lue, regt_pag>current_pa);
