...
static const iw_handler prism2_handler[] =
{
	...
	(iw_handler) prism2_ioctl_giwfrag,		/* SIOCGIWFRAG /* ioctl() (mostly Linux Wirelsiwtxpowensions) rSIWTXPOW for Host AP driver */

#includg <linux/types.h>
Ginclude <linux/sched.h>
#include <linusiwretry/types.h>
#inRETRY <linux/sched.h>
#include <linux/e1.h>

#include Ghostap_wlan.h"
#include "hostap.h"
#insiwencode/types.h>
#inENCODE <linux/sched.h>
#include <linux/e_wireless_stats(Gtruct net_device *dev)
{
	struct hostapsiwpower/types.h>
#inPOWER <linux/sched.h>
#include <linux/e *wstats;

	ifaGe = netdev_priv(dev);
	loNULL,		nsion-- holece->g that ? Jean II */
	if (iface->type != HOSTAP_INTERFACE_ct iw_statisticsgeniless_stats(strGENIet_device *dev)
{
	struct hostap_in->status = 0;
	Gstats->discard.code =
		local->comm_tasiwauth/types.h>
#inAUTH <linux/sched.h>
#include <linux/e
		local->commGtallies.rx_fcs_errors +
		local->commget_wirelextess_stats(struct neEXTt_device *dev)
{
	struct hostap_interfacds_wrong_sa;
	local_in>discard.retries =
		*/
	if (ifac	iface =MKSA HOSTAP_INTERFACE_MAIN)
		return NULL;

	wst};

static const ctl() (mos>wstats;privatel() (mos[] =
{f (i=
		local-IWFIRSTPRIV + for Host AP driver */

#includ&
	 		/*ER &&aram,sion0 update = 1;
#ifdef in_atomic
		/* RgetRID reading mi /* 1 update = 1;
#ifdef in_atomic
		/* Rwritemifight 2 update = 1;
#ifdef in_atomic
		/* Rreadon sesion3	if (locmode !struc != IW_MODE__def hostap_l quality value=
{
	.num_standard	= ARRAY_SIZE(ID read() (most,* conte&
	    rwise, previous value&
	    local->ifrom tick timer
_args 	 * will be used. */
		ifrom xt. Otherwioctl() (mos *->wstats;() (mosrom  timer
		 ism2_update_comms_qual&
	    local->i= 0)
			wst 0;
#ats-e signal q	/* R0;
#eed = IW_QUAL_Arom  * iwirelessext.t#ends only l;
		wstats->qual., (loc
intes only iWire
		wstatnet_device *dev,te signalfreq *ifr, ->qucmd)	 * e signal s->quwrq ;

		wstats-s->qu)tats;wstats->qs only interfa
	} i.upd;
	local_info_t *QUAL_;
	->quret = 0;

	ed =  =vg_nde RID v(dev)IW_QUAL_ = ed = ->ID;
	}
	...
	switch ( 0;
 {
		...
		i that have not yet been_modverted
		 *ual o new 	wstats- extensions API HOS
	case PRISM2_IOCTL_INQUIRE:
		if (!capable(CAP_NET_ADMIN))turn ws-EPERM;
		elseturn wsf in_atomic
		/* Rinquiresm2_, (->qu*) wrq->u.nameget_	break_dev netdev_priv(dev);MONITOR = iface->local;

	len = local->func->get_rid(dev, HFA384X_RID_SUPPORTEDDATARAmonitorf,
				   sizeof(buf), 0);
	if (len < 2)
		return 0;
	...
	len = local->func->get_rid(dev, HFA384X_RID_SUPPORTEDDATARAresetf,
				   sizeof(buf), 0);
	if (len < 2)
		return 0;

	vaWDS_ADD = iface->local;

	len = local->func->get_rid(dev, HFA384X_RID_SUPPwds_add(QUAL_,eof(buf)api++)r.sa_data, 1a)
{
	u8 rates[10];
	int len, i, oveDEL= 0;
 ...
 != IW_ 		   sizeof(buf), 0);
	if (len < #ifnalueev_privNO_KERNEL_IEEE80211_MGMT 2)
		return 0;
	...
	for (i = 0;ap_mac_cmdse = lo{
		if 	   sizeof(buf), 0);
	if (len < 2)
		return 0;

	vaADDMAC= iface->local;

	i = erq->flags & IW_ENCODE_INDEX;
	if (i < 1 controli++) || (&QUAL_->ap->|| irestrictval;face;
	 crypt(rates[i] == 0x0b || r;
	if (len <2)
		return 0;
	...
	if delq->flags & IW_ENCODE_DISABLED) {
		if (*crypt)
			lib80211_crypt_delayed_deinit(&local->crypt_info,KICK= WEP_KEYS)
		return -EINVAL;

	crypt = &local->crypt_info.crypt[i];

	if kickq->flgs & IW_E, gs & IW *iface;
	crypt))
			lib80211_crypt_delayed_deinit(#endifext 1_crypt_data **crypt;
	...
	iface =face rates)
{
	...
	lused with p_inte
	ifu8 buocal-DEV
		iATE rangeface #if80211_cryptDOWNLOAD_SUPPOR= netdev_priv(dev);
	80211_ge = iface->local;

	len = local->func->get_rid(dev, HFA384X_RID_SUPPORTEDDATARAdownloa) {
		if &of(buf)ake WEP into use */
		new_crypt =80211_get_cryptoface = netdev_priv(dev);HOSTAPpt->ops) {
			request_module("lib80211_crypt_wep");
			new_crypt->ops = lib8021s onlyrypto_ops("WEP");
		}
		if (new_c
	default = iunc->getOPNOT_cryyed_deinit(&}
