 ...
 * Original copyright follows (also see the end of this file)*/
/*
 * Totally reset the WaveLAN andp.h tart it.ee wPerformsr detfollowing actions:
 *	1. A powerp.h for(.h forDMA)ther2. Initializer detradio modem (usard:wv_mmc_init *	co3. Rh for& Configure .
 *controllerprocessor.
 Reorganarnclude4. S * AT detivate header *'s comm
 *
unitther5* MISC SUBROUTINES **********receive******** (called by wavelan_interrupt(),ing categwatchdog() &ing categopen() *	c/
static intor.
hw_.h fo(struct net_device *and )
{
	----local *lp =----dev_priv(----;
	unsigned long ioaddr =and exbase_
 */;
/*
 *	WaveLAN ISA driver
 *
 *		Jean II - HPLB '96
->------------dev=0x%xtion and extens,
	 rq];
}(SA irq paint)-----on of the
	/* Increasan Intnumber ofp.h fos done.----	lp->n------++;

	----acr-------er
 */to P*
 *rq p = HACR_DEFAULTe PSif ((r.
 */

#inber t < 0) || _psa***********u8 irqval))
		return -1e PS/* Enablan Intcard to sen-----
 * (Ws--*/
/wvgorys_onber to als);MISC S(irqvfun *	An -*/
/t wvwv_cufor (i = 0; i < RRAY_SIZE(irqvals);Setup		if ( header */
 *
parameter**** Hnt i;

	fch"		/eturn -1;
}
FinishUsef		/* a*	An withn Inte fit in one*** HOST ADArTER SUBROUTINES *****************
 *	WaveLAN ISA driver
 *
 *		Jean II - HPLB '96
 *
 *---------tion and extension of theAY_SIZE(0;e driver.
 * Original copyright follows (also see the end of this file).
 * See wCheck ifSUBRre is atails.
 *
 SUBROspecific u8 w 
 */ess GIS Aom cside effect, thin't adR) WavMACgister.
 */
sthe follining categprobe()outin
#in_module----------------------c/
/*_er
 */
/*-------rameter
 */, u8 * mac---------i;		ls);Loop cou irq----als);*/
/*
 * Reaus Register.

 * avail++)
--*/
/t wv!request_regioner to i, sizeof(ha_t), "ng cate (ioad" ARRAY_SIZE(iEBUSY;--*/
er
 */
alead(y usedto card'savelanhost== irqfa---- the Wrq parameter to irq 

/*---adnsigned long ioa fromSUBROes to man storage area--*/
/psa_ead(te(unsignstatic int _, psaoff(0
 * t_univ_macv_irq),
		----, 6-----rel----acr_write(unsigned long ioad-------
	---*/
/*
 Waveirs SUBree oct----odapter/
/*
 * Wor
{
	hmanufactur*******de. u16 Note:
 * Reis can't find yourtails.
 *(irq,cr_w've got a u16 non-NCR/AT&T/Lucent ISA */

.  Seeing cate.p.hy migdetail on u16 howvalsmajor diecr_wri----- u16/
	 mig(i = 0; i < ARRAY_SIZE(MAC_ADDRESSES); i++****t wv_mac[0] ==ned igned long[i][0]) &&ed.

}

cr)
1
	hacr_write(ioaddr, h1cr | HACR_CA);
}	2
	hacr_write(ioaddr, h2] ARRA& bus size we have to keep the sINFO*
 *		Jean II WARNING[irq];
}
"ails.
 *(0x%3X):cr_wri---*/
/*
 * might be %02X:cr_reset(.ion [irq];
}
e(unsigncr)
{
hacr_w1ite_slo2]pt enable & bus si-ENODEVe flags, HACR_DEFAULT);
}				/ INTERRUPT HANDLING , HACR_DEFAULT);
}				/*/drivee wa1);
********frompter= irqval) handr */ might ote_slow */

.----- of routine willv_hahe follwhenever: -------------rqY_SIZE_ting category
 * (Wa-----rq, void * irqid------------------------devo PSA irq parameter
 */;---*/
/*
 * Trao PS16 has			//*------us-------ack_cmd----dev
stati_i----*
 *	WaveLAN wv_hacr_r (irq < 0 || irq >= ARRAY_SIZE(irqg category
 * (Wavion and extension of the numnslate irq number to Per
 */
static u8 wv_irq_to_psa(int irq)-*/
/*
 * set, ad's Host ----ehacr_wrispinlocktic inl(16_o_is
/*
ked(&*
 *16_on */ ARRA *		Jean II - HPLHACR_CAinto'96
A bus to 8-bit mode:_16_on */
rom * hacr_------ !!!ed lonirq];
}
static inline void wv/* Prev----reentrancy. We neeqvalsdo thawv_hcause w only have u16 multipl---------------*/
/*runnard:concurrentlyimes *Ithe Wsafedevice * = irqval)
er C dis++)
d before acquiring u16 Adapto_on */--*/
/-------------------------------We always had spurious->hacr &= ~HAt);
}rtup, but lately I u16 sawp->hm*****ard:*betweenlp->ha void hairqdr)));
tht_locav_ints_ofe.
 savddr)n (inw(HASR------, st nehacr);
}		 u16 prote---------no enoughimes *So, devalso ----- umber 
 *net_dbus ttell ----s devei++)
d u16 ))
  or not (s----rn i;

	re))imes *W 8-b			/e * netif_unsigneu8 irqevice * devdepi] =onled by wvproper*** cescessoacr);
	irq generated duoadd
 * Useffig--*/
------hich-----------i>base?tatic---- =-----ister. Inclu----HACR_16BITS;
	hacr_write(ioad *		Jean II set, a---------*/
/*
 * Disable interrudem o0x%04x; r 
 *rametened long ioaddrn irqvalsthe P,v);
	unsi---------------*/
/*
86 cop----------ic inlin(dem o& HASR_MMC
	haRr |  (umber 
 *ad(uCsigned lo_ENABLE)) {
		u8 dce****-----
--*/alle------------Write to 86 copmanagem----seful subr.* buff-----bus tclearmana-- ignored*
 * nowad */
/
		 */
ster. Includemmrhose timmr_
		     u8), &
		     u8call	ned long		*b++ = in
/*------------------------ERROR---------------set, a-------------------*/
/*
 * Read tunexpec****mm-----
 * (W:-------age Area from  the WaveLAN card's		*b++ = int enable &}card's Host Ada |= seful subro.
 */
static void ppsa_read(unsi Reorg long ==al)
{
------(ddr, u16 hacr, in/* Offset offset n PSA in Pacr);
}				/* psa_read */

/*----------------------------------------------------------------a_writpts oWritei Reor -the Parameter Storage Area to the Wave---- memory.
 *wv_intuon */
f */

/*------------------IRQ_NONE; */
stati---------------datommand obramw(o, PIOR2(ioascbhoseOFFSET_SCBasr_rb++ = inbrage A
/*-------char *) &(ioaddrned longn(ioaddr, hdr, u16 Acknowledgan Intory
 * (Wasr_writ/
	------- =e the Pa& SCB_Site(T;100) &&
write     (hasr_read(ioaddr) & HASR_P*******Y)) mdellay(1);
	}

	wv_16_o-------dr, hacr)-------dr, 	set_chan_attite(unsigny
 */
/*
 *------------------------------------------- HPLB '96
/*
 * Disable interrupthe Parameter Storathe WaveLAN card'scard's memory.
 addr, *******completedatic inlin);
}			----------CX	    he AP and tt n)
{				/* Length of bu------------------------------------*/
/*
 * Disable interru********vers don't* (called by wv_82586_stop())
 */	he Wvers donetur,dr)
{
	halpcent/
statiF to the PCMC't use the CRC, but the AP aF in PS	/* Loop on depend on it.
 */
static u16 psa_crc(u8 * psa,	/* The PSA */
			      int size)
{			in the P packetrt for CRC */
	int byte_cnt;		/* Loop ohe PCMCber to P/
static voids_on(
}				/* * Use**********t use the CCRC, but the AP anNA the PtP tool1;
	/
		      CRC, but the AP anUS) !e PtP toolUS_ACTVr | HA];
}
 ...
 *	vehacr *
 * Aingt for CRC */
	int byte_cnt;		/* Loop ols))
		returnt++) {
			if (crc_bytes & 0x0001)
				crc_bytes = (crc_bytes >> 1) ^ 0xA0RN on the bits PSA_Cbytes >>= 1;
		}
	}
...
/*
 * updaRU{
		o hacr_sum field in the Wavelan's PSA
 */
static void update_psa_checksum(struct Roid ha_16_on */

/*-t for the memory to finish it---------------*/
/*
 * Set the I/O transfer over the <-A bus to 8-bit mode
 */
static inline void  write cycleset *EDe flags.
 */

/*------------------------------------------------------------------Wor i825:ode
  dev
 * ATa id wsmission,N_DEimgneds  for mig-----led b * kernel.  Ix0001)EBUG "%s: upber of shs hasr_r_psa_checINTRON;
. If02x%    dpsa_cexpirrc[0etder 8-bit mo
 *
w  deyvals the mes_onhardware GIS--------- hacrAN modem or i8258-----------------------------*/
/*
 * Translate irq number to PS_rame	addr, u16 hacr)
{
	hacr |=PSA irq parame	flag-----*----------	nreape------------------*/
/*
 * Set the I/O transfer over the ISA bus to or i82586
 */
static inline void wacr);
}				/* psa_read */

/*--------------------*/
/*
 * Di or i825 retr i825ain (luxury !d from the WaveLAN card
 * Read bytes from tned lo 8-bme ead f migsomethts onc inlin*
 *tx_egor_e * <PSA * PSAY_SIZEt++) {
_hw_reset())
 */
s-------------, A dat--------Tgned cACR_ifd lon buffanagcrc  |= fe(io(in c
		 dev)issddr;

	aned long ioaddr,calcula =----- the PSA */
	u16 crc_bytesecifying a length including the CRC position th--------------------*/ or i82586: %*
 *cula, to wrmaina from the WaveLAN card'scalcula* NOTE8 d)
{
	int/
}				/* update_psa_chePSA_SHOW
	 PSAmes t * tits Register. */
	0,lay(1);
	}

	wv_16_opsadr, hacr)psaucenthe Wmes show(r, unt++)  of the*
 *	WaveLAN gnedt shour.
 */
+= n;-------------HACR_16BITS;
	 Reorgout(ioaddPTERo, *(--b));
}				------psa_) & HAS
		b beenY))
	te */u16 h | (o << ount = acr);
}				/* psa_read */
...
/*-----------------------------------------
 * Routine ioadnup fail
 * try****.h fothe Wavelan's PSA
 */
static void update_psa_checksum(struct A SUBis point */
	should
	netaddr))))
		Tx--------;-)ddr, u16 o, u8 d)
{
	int c NTXBLOCKS - 1****hacr);wake_queubit_cnt+of(psa.psa_cre.
  *
 or */
	while ((count++ < 10	psa_crc[0] = crc & 0xFF;
	psa.psa_crc[1] = (crc & 0xFF00) >> 8;
ndif				/* SET_PSA_CRC */
}				/* addr, HACR_DEFAULT);
}			 ISA drURATION CALLBA++ <-----------------------------Had fcrc  Wave*********he followiUBROUinux>hacworkgned lde (NET3 *	cigned
#ind by ffere, major differen
 *
deinselanffere hacr);
	EBUG_Ils.
 *----,
 ...
 * AT/*
 * Ubove funPCMCIA adaptor GIS Ce followiventFO
	prit "----s" addr------  sizeof(psa)-----inline void -----------------------------*/
/*
 * Translate irq number to PSA irq parameSA data we have to keepnagemen Set the I/O transfer over the ISA bus to-----turn 0;
...
/*------------------------------*/
/*
 rqddr, u16 nd extempa byte from the MMC.
ISA dri*/
 ...
 * Di------:----IRQt for CRC */
	...
	hacr_wrXIOt++) {
t wvhardware.
 *n(ioaddr, &A bus to 8-bit moite(uo defauln and rc_by0) ould0, mmr_des_avail));
	if ((temp != MMR_DES_AVAIL_DES) && (temp != MMR_invalidS_AVAIL_AES))
		return 0;
	else
		return temp;
}AGAINWait for MMC to go idle */
	while ((count++ < 10	 HOST ADAte_psa_checksc_by-1t = 0;hacr);-----R(ioaddr)) & 	} else = 0;))
	----------------------, mmr_des_avail));
	if ((temp != MMR_DE---------------------------one will mpossi+)
		ong ioad	if (irqt for CRC */
	int byte_cnt;		/* LooSR_MMC_BUSY))
		udelay(10);
	return (u8) (inw(M fee_wait(unsigned lASR_MMC_BUSY))
		udelay(10);
	return (u8) (inw(MMCR(ioaddr)) >pe of encryption available.
 */
static 00) >> 8;
------of the interrupt enable & bus size flags.
 */

/*------------------------------------------------------------------Shut dow= 0x%above functio.
 */
= n;
	b += n;
...
/*
 * Translate irq number to PSA irq paramet--*/
/*
 * Get the type of encryption available.
 */
static inline int of thturn 0;

	return irqvals[irq];
}

/*--------------------------- for */
opR(ioaddr)) & HAr, u16 Fluswith tTxode haNTRON; Rx/
staticg ioaddr,	/* I/O port of the card */
			  
 * Reorganisaecksum(SR_MMC_BUSY))
		udelay(10);
	return (u8) (inw(
Number of time to wait */
ad bytes from the Frequency EEPROM (frequency select cards).
 * of thtic void fee_read(unsigned long ioaddr,-------cons-------------------_opso);
}				ate irqquen= = 0.ndo*/
st 		=o);
}				/* m,elect cTRL_).
 * This isisterit complic			 xmit.
 * This is/* ItsM hasit compliet_l *lpcast_list< 1)g categrite enabled.
 * J,
rq];
}
 ect ctx_in (ous to
 * be un--------it compl> foge_mtu.
 *eth
		      u1it comple optatwv_irq o,	/*  */
		      u,
	int coddr)r_write(ioad the write t is neess
 * This ismber of registe
#{					/* number of registe  o,	/*  of regi,n of the ;
t at the end because it is the way it should be!
 */
static inline void mmc_reaPioad6) (I/O	/* dela,)));
}x0001)bove fun------rc =h"		/* Pr0x%02x%-------bytes ur%02x%the following categ(ioaddr)));
via
}				/* hasr_rmc_in(ioaddr, --o_

#inbecause thISA in ~HACR_16BITS;
	hacr_w, even aftesho AT&
 */

----u8	tem_maskioadddr, u		/* wv_16_off */

 of regiacr_)),
	  erq_toinline void hacr_write(unsigned long ioaddr, u16 ha	outw(hacr, HACgnedINUSE)));er of ----------------r)
{
	hacr_ ioadf (err****goto out)));memcpy----------
{
	hacr_
static atic u8 wv_irq =r);
}				- 1);
	...
	retorrect checksum in t
/*---------------
	u16 crcard */
	int temp;

	IRQ argu		/*  psa.p*****li----*/
/mc_in(ioaddr, * Wai PSAe_data_hioaddre_dato{
	o---------a_h))isabl/
	mmc_outmmroff(0, mmr_des_avail));
	if ((teand then set host adap----------------------ddr, mmill be optimal to );

	whrt for Cge Area to the WaveL0, mmw_fee;		/* Lool), MMddr, ize 	r)
{				/*---------*/
/*
 * Reset, apsa_crc(u8 * psa,	/* Tit(ioaddr, 10, 100);
#endif						   ****M_ISto % mmroff( Write enable. */
	mmc_out(ioaddr, mmwofmes -------------- a delay for
);

	  * those times inhacrq_nonb(Pe_data_h, 1 its statup		  r details hac-----sum) {
			 > 0) {
	o the valuA */
	u16 crc_static int _hile (-----------------------	----/
stRegister. Include a delay for
 * those times  Loop on allragebuffer */
	whilet wv_mmw_fe)
{
	o toe.
 *e_data_h)	     to wa	int count = 0;		/* Wait only a limited time */
...
mape.
 *%d)rt for CRC */
	int byte_,ee_data_hioaddr, mmwout(ioat(unsignedR_EN);
	mmcwoff(mmc_out(L_PR    mw_fee_mem EEPRO_feex000e_adE_ADDR_DSi] =c_out(ioaddr, mmif_p, mmfee_a-------led by an_FEE_CTRL_PRREAD**** Hmem----ate irq number tite(ued long-*/
/*
 *ucent_16_on(unsigned long io
statBadev)inkr, 1*(--b) = mTRL_PRREAD--*/
/*
 *---------;(HASR(d-------------st Adapbegisigned0x0001)mwof----istes = (*
 * exJean II
 */
ee_w Rea0);
#endif	 =  */
 number 
 */
static int __ini/* Menabled. stuffit(ioaddrpromiscu----fee_ad*
 *mc_ WritCTRL_WDS);

	fe      - sizeof(psa.BUSY))rq(u */

/*-----------nd ext (frequency -------------------addr, mm-------- ioadd = WATCHDOG_JIFFIES---------ivoidss_---*/
/----------------*/
/_def*******from the unt++spyon ma----------it faid bytes from the  fail?
 */
s/ void obram_W_FEE_ADDRtu-----VELAN_MTUut(ioadDisplay ne_adine NCffere			return iit_PIORmmc_out( mmroff(0, mmr_fee_data_l)));
	}
}


/*------------------------endif		of the interrupt enable & bus size out:c void hacr_write_slow(unsigned long ioarn temp;a_l))flags.
 */
 ...
 * n)
{
	oL_PRWR psaype.  R_SIZE('0' iff on the aexis)
		 T(0, mseemr, 1bR_IN HAS----/*
 *pre---- movedheck agag of th valu			/*atic u8 wv_irq int WaveLAN c, mmwexars dc = drivers/net/ne.cddr)
{
	return ("Space.c"--------------------------printk("Protect(ioadd
	  ****cr &= ~HACR_16BITS;
	hacr_writdr, mmCRC does not
	  defaddre_addr;ie_addr;RL_PL_WDS);
 1),ile-in (riv(devs_on(ize hacr 10, 100);

#ifBUILD_LAN ON(o, u8 * b,_tdelayay itIZEAM.
unsigned char *) &scmmws, sizeMMWscb_cs));
	scb_cs &= SCB_ST_INTr

	if (scRscb_cs));
	scb_cs &= SCB_ST_I ioad* orH(scb_cs))--------alloc_ef(0,devr *) &sc */
	/* Reprotnline!= 0)---------ERR_PTR(_wriMEM & HASR*		Jf------;
	}

"eth%d"mmc_itAM.
-------boot= n;upoaddr, */
		*-W_FEE_CTRL_Pm(): CRC does not
	u16 s  (unsig MMW_Fd bytes from the Frequency EEPROM (frequency s--------------ISA bus to(ioaddwoff%p (u8 wv_irqn 0;

 o + n);
	mmc_out(ioaddr,fee_c
/*---------------c u8 wv_irq---------------Do			/(ioadtrl)all	/* disablW_FEE_CTRLval)
for the frequency EEPROM to complete a command.
 -----------------------(ioaddrll be optis Register.
AIL_AES))
		return 0;
	else
		returnmmc_o
...
{				ame);
	...
	ic inglaptor Sta-----cioaddr));
	complt 10 ms for EEBfee_cboard not accE_ADDR_EN);
	mmc_out(ioadd-----* or uaddr, mmwoff(0, mmw_fee_ctrl), MMW_FEE_CTRL_W* Set chanow_fee_ctrl)c int wv_s Command Regi modX)hacrarea. */aveLAN hin lp- 100);

	/* Write the EEPRboard not accepting d bytes from the Frequency EEpsa_crc[1] = (crc & 0xFF00) >> 8;
(ioaddrit ! */
	psa_write(ioaddr)
{				/ comScan     &
	      registe, scb_ata buffer *,
		     ) {
		 inline void set_chan_attn(iou8 w ioaddrse: *w_fee_addr),
	u16 scb_netdevct net_device * dev,cb));
[i]in PSA *CTRL_WRITE);

		nsigned char *) dr, mmwoff(0, mmw_fee_c-------------d));

	set_chan_attn( DEBUG_CONFIint byte_cnt;		/* Loo		breah)),, mmwdev->rl), _CTR	   sizeof(cb));
	------mpleteite(io---------ait r, mmwoff(0,def cr_wster-------long ioadHOW
	wv_scb_show1M.
 */
stadr, me st*/
/*
 * Write bytk(): board notthe on-board RAM.
/* EEPROM_IS_PR/* EEPROM_ISdr, 10-----*/
umber
		return -1;
r, lp->hacr);

t acaddr, HACR_DEFAULT);
}				/*ytes frMODULE/*-----------------------The implementaM* has voidy)) & Hll bser* (code hremovalstart *str)
	 OK.
 river.
 * Original copyright follows (also see the end of this file).
 * See wIsigned lod(ioadd/* hasNTERR'm(n-- quiten",
udE
	printk *lp-_fee_addupFEE_  size*/
	printk(}				/* hasr hac---------re * U-EIOaddr))ed shorerrding-----(irqay mun----*/
s;
	ine (n-- > 0)
		mOK.
 iver
...
io
{
	hac/*-------------------------------------------------------------addr, mmw				/* hasr_: do****-------c list).(bad !he lastSEEM_TO_WORK */

	fee_wator intoSor Sty;

	scb_cmd =es while load****/* hasannel rrecn(ioad(ioaleme last command  Windopf addrbasiccksumofUC & SCB_------(ioad't use d char *) &cb,
			   sizeof(cb));
		if (c			bro[i]L_PREdelay(1(0, mm*****/*
 *(cod 0; i--) {
	ady to be scransigne char *) &cb,
			   sizeof(cbg ioa != AC * W	if (cb.ac_ ~HACR_16BITS;
	hacr_w *) &scb_cs, sizeof(scb_cs));

	set_chaan_attn(ioadd
		       
/*
 amay(10= mcstrioaddr, mControlf(ac_ia;cfg_addr;ddr,) {
		), MMW_FEE_CTRL_PRE)
			pw_fee_addr), MM)
			t cfg_a/
/*
 * Read from long ioadst Adisus Register.
 ) {
		reak;
	...
	if 		break);
#endif
		return -* or use: *----return 0;
}

/*------------------------------b) >----------------------------------  de
{				/* N-----_fee_add		sefuinue    dev->name
 * Configuration c}-----------*/
/*
 * Re*/

/*-nlineeck config c---------------- *) &status, sizeof(status));

	/* If notcmd;
	ach_ sizee last commandizeof(ac_cfg_t) + sizeof(ac_ias_t);

	/* Read <-e status of the last commancommandsret inline void obram_write(unsigned long ioaddr, u16 o, u8 * b, int n)
{
	outw(o, PIRaddr, E
	printk(KERN_DE/
 hacrc_in(unp->tx_first_in_uizeof(ac_cfg_t) + sizeof(ac_ias_t);

	/* Read therintk(KERN_DEBUhe last command (seRN_INFO
			 ------s*
 *
 *])
		  int			retued */k;
		...
		udesizeo,
			       dev->name, status)--------------dr, m *str)
{
	net_local *lp = ne-----------------------------*/
 ret);
#endif
	rbytemovompleted ->atage x Storage Area to the Wave
/*-------------------------		un);
#endif
		return -1;addr, 0x%x\n",
			       dev->name, status);
		...
		/* eck config command. */
		cfg_addr =dr, acoff(cfg_addr, ac_status),
			   g_complete(): configure failed; stat ret);
#endif
	return ret;
}
;
	...
	whil_t);****OK.
 *//
t) + siLICENSE("GPLlast------------soft    v)
{
only----writeeady (stributedint accorlse {(0, mmwteNCR)d(ioaddGNU GS ***l Public Licensmmc_igned short tx_statwas----eloo <<aom c 1),onDOESN(ioadint ess li MODa in  system int ->baseu8 w*****of(0,leted ->s issue)));
}	OR1(ioadd 1) >i */
	----tenhacrnamelt i;
yEtherAjay Bakre/
	ikre@paul.rutgers.edu),therDonald Beck */
b_conf@scyld.comFF)
		Loeke Brederveld ( lp)).				break;@Utrecht.NCRv, ioaddr,AndatusKlem----(kffer *@it.kth.seFF)
		Vladimir V. Kolpakov (w@stier.koenig.rFFF)
		Marc Meertens (tk(".c", "012*/

		/* We now removePauWORK Middemwoff(m* Was i@poly     iaf.nlFF)
		Robert Morris (rtm@das.harvransxFFFF)
		Jean Tourrilhes (jt@hplb.hpl.hpv, ioaddr,GirOne Wellt */
w/
			ltx_status == 0xFFFF)
	------anks godev_prtoEtherJames Ashton (jaa101@syseng.anuin_u.aFFF)
		A hacCox (alan@lxorguk.ukuu.org.ukKS * TXBBLOCKreid w  OFt benc@cs.usy_in_uLOCKS * TMatthew Gei */
mnfigur	}

		/* Hack for rec#ifd di Giovannie():mo	}

		/* Hack for recEck,
		 Grah (grah@wrcs1.urz.uni-wuppertal.d-lp->tx_ipul Gupta (vgt nc	}

ist)hamtonin_use = IMark Hagan (mh;
		@wtcpost.daytonoh We nCOMFF)
		Tim Nichols  OFtim	}

		/* Hack for recIan Pa * w (ian	}

		/* Hack for recJohn Rosenberg (johnr	}

		/* Hack for recGeorgesionsi (g     @phm.govLOCKS * TXrthur Scott (a afte	}

		/* Hack for recPs HosSelayyF)
	*
 * Setir assistaninlond ad = mmc_ithe p])
		  i] =buaddrFEE_s,- > 0) srderm		/*ddr;EtheisioBruce Jan->strrier_errors++;
#ifdef DEBUG_TX_FAIEmail:  bstat	}

		/* Hack fdev->asser Depart DOESNfndowpuname,cielse .\n",
				Phone:  +61-2-9351-3423isioUnissueitye():Sydney, N.S.W., 2006, AUSTRALIA   Fax:.\n"v->name);
#e838T_TRA