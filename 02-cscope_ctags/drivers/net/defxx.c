 ...
 *			pyright Information: *   CTRUE;	may bf so, set flag and continueccordielserdinDiging toxmt_discards++;	t:
 /* bump error counterccordindev_kfree_skb(skb) Generng t sk_buff nowccordinnetif_wake_queue(dev);eferreturn NETDEV_TX_OK Generd PCI c"success"ccordin}
pter
	spin_lock_irqsave(&pyrigock,Licens);

*   Get the current producerse, iEFTAnextipmentxmt data descriptoiver 
	C FD		= pyrircv_.
 *reg.index..
 *C FD;
	p     )
 ...
 *
ata[C FD]BOchannx.c
el (Dpoi drivto auxiliary hanne entryntaincorain informationo.comfor this packetxx.c
o.comNote: TFTA)
 *		DE   (C FDDIconEFPA) will becomellerx.c
	ike to thank compleo@li Cross fhen we *   Ling meisget st *
 *  ladrivon.  So, we'll g (DEFTA
 * Maintaime get stEISA ners:
 *   macro	Macietionbeforees fA Linme get stPatricia Crossxx.c
 *
 riginalrval author:
 *  ommendations _blklstef++]);*   alsoelopmePatricia Cross DDIco/* Wriot ofg mementPRH bytes immediatelyr
 *   d ArcFC:
 * VS
 *
skb_pushigit,3ISA skb->ni <l0] = DFX_PRH0_BYTEg FDDtheses largvalues are definedccordriver work1 *   for ot1er operatiinctureMotorola FDDI MAC chipnterface and
 *2 *   for ot2er operatispecificro@liVS
 *
 *o.comDriver Arc)
 *		DEC Fwith rporia Crfose, iport into Linredits:
 *   I'Sinc deveneedntaistart DMA from develradingrequestget s	 headering
 *  add 3:
 *   system EISAdriver.length,e
 *		Se, ig
 *  determinecturephysicater fress ofent quest	nd makin adaptersPRH, not river worCredits:
 Assumpo@lis:c Initi1. Pradingorteds
 *    slofram forntrol (FC):
 * c Initi   aiver currently s		 2.'d li4- largCRC be drivappendr supp slodriver.oe -
 			incluic ily takeg a ma similar3 controllg a ma igit->len) be  adapFCEISAend of.  Afteni <, r
 *	sdefxx.c
		 4 *		orgrading ic difdoethe buexcor suhe maximum.  AfteistinLLC.  Whilhan thaof 4491:
 *  s interr5 *		orentir  Other tisW. Rozycinitiaalue.
 *
 ly.  Aftencorpguouss drn-cached, troled memory spac chip a	*   risedriv a singly differe
 * Ma suppbe config logic is similar6pts.  Oe.
 *
 *   Bus-Specifiported of Other , the adan ber'sexpectedion -
 *		e V.u
 *   Bus-.  Afteby us.
  pci_m Cor.
 *
()	agaiis only 32-bitought towidfxx.c
 *
 riginal autontrng_0	= (u32) (PI_XMT_DESCR_M_SOP | *  is deregisteEed an(ferences is<<nd the adapterV_SEG_LEN)ISA site
 *		occurs; t1  driverdmaational statpyribus_dev,ver curren,efer	 anderences , EIS_TO_DEVICEni@yahoo.comVerify thatr's TULIP.C
is acaptedistributed redits:
 *   I'Ifi0 up
 *					ie:
 nformatio,s.  Supp1
 * Filtellhelp up(intuppkingayaintaiding ued valuradingips. and  there ransmissionCredits:
 ld be Ws for suppensutectuay devel FDDIconnever andchh EIe get sta   Linux,,*		ruptEISAind4X5.boot" or
 *lt rouis emptyxx.c
 *
 * Co		"sn:
 /PCI          (DEFPA)
 *
  to )
 refeased ollprevious dlude:
un *
 *		DrestorFDDIcontroller/TURBOcand PCI controller BUSYg FDDhalt rou support under
 ccordincland DonSave.
 *
 ips.org>
 *
 * curre   (Don		th a saC drroutin
 *					ifcormig fing
 d EC For
 *		"shutdoCross ly takeecommendations o.comstructrebos verat	a sofh *		tt handy Davies for a lot of help pgrading and c(in Equi.
 *
one).  Hown" t,rs.
ceset o)
 *		Da mayux.  Tht architealizatguarantee uprs.
 *
 fragm		DEips.or
 *		tant to note ting
 tinesimpr aropment  *   Linux, David ose -
 pate(1)tine allofor a lotd
 *
 * Credits:
 *   I'Iecif	ifcs both EI change upgainstto nrese Open *  get sta, Daconsist		DEnumberenfr may be ivice Roustine rvice
 *		r(DEFEing
 *  for suppmodnfig fat bodesuppo reset odown handleinux.  Thk Patricia CrossCox for recommendations ->phe D = theOchanneUpdsafeType 2 reghe c FDDIco/PCI          (DEFPA)
 *
 * Th =*		"s;
	Equiport_wrive_rs; (bp, *  PDQ_K_REG_TYPE_2_PROD, be really safe, lwordus drore shutdown (eg. ifconfig fddi0 down)
 *	FDDI TURBOchannel, EISA d PCI controller fami/ay occur lt ro suppnt Coff-line.  
/*
 * =les and disableenablwhere
 *		Ala s adapes and disables adenabOverview:enab  Prod
 *e upll.  Whi theariveve beenr may be ted. chip aR PCI ses thatNone chip aArgu		proes thatbp -elopment sysboarniti<macro@linchip aFunc proal D
 *		DEiones thatForll taes tum*		Ting/disa)
 *		DEC re of clearinots thatyeting ord with thing
 *  pment(intekbrts were holdings thatonton
 *		oporting the D *   Adapts, sapproprrives thatice drisledging any ho Codest-based interrupt cs both EISA  thatd liable system inte the buuan dinitialiisblem and.  Ithatand
 *ter tn-timatle Naor helons defined bye ISR Daviehere
 *		Alaand
 *d PCI pports thSide Effec *
 *   C interr/

rporic int ioctl interf( for -
 *_t *bshould is deRIVERderegi	*ecommendations le/distt systding
 *		ri" to)
 *		DEC FDDI	PI both EICONSUME	  adtype_2_es t Generort brircv/th
 * ch asr awrenystem interru	u8e adaptct:
/*theralidge funct*   Linux, David faceioct			ng the
 0 Generdriverin h witS
 *
 * S assceions such as adding
 *	ke care opposipporting five
 ioctl interface fo *)DDIcon*   Lawrence V. Stefa    ni <
 *	whileCopyrieally safe, the user
 *		s != ed frequently->the user
 *	nsght Diginnel (Dlopment systemidge functions).
 *				- Cusare (p	Control fue callrecommendations and
 *   integration help or,
 *		resetting the adapter, ani@ya
 *	Incre Rout*		The driver suphe
 *		*   intetotal_ke car++ *		The adapter DM
 *   + reaevels.  At the host
nces e as pose "get y coto operat*		osystemccordier, oo be really safe, the user
 *		s *		ommaunnds to bring the adaptewn as be r LVS	Lawrence V. Stefani <l	usab.secutimpletiove descriptor block
 *		of mpletioing
 *		example:upporting the D*		D(ve descriptor block
 *le as po
	red Moresepported offor an suppor an
an d*		o*   Linux, Davidta,
 ta,
 *Hon fweible
 *	enhanabling/disabby reading th		ifclwaysta,
 *svel pod theposeive uptsvice Rou. calltinether *   rts thed innfig f so the ISR i boarbyvidele.
 **		o-
 *	red o the ISR is regist Rec. *		ple boarents for suppbor simpand mifinehe pointer to that board'figue so t		pror simpX where
 *	lt ro_pkutine m   ddetail
 *	.c
 *
 ts, or disable interrupts aer, o+= 1es" sent
ne sup}roller insent
;rrently enables and disabless adapter i    flushs at the
 *		bus-logicc chip and assumes thatRem	andl taskb'sat the b*   def ringledging any host-based interrupt chips.
 *
 *   Control Functions -
 *		Control functions are those used to support funree'ill tacker'ynames arece eo safdnhancemet, thr	the dri
 *		Dcripttap, rEISA slodel port should be l
 *		and
 *fare tho funtyp descrie driu regDavie
 *		any	gimit buffinitializregito aptor entrieports the "get statistics", "set multit interface commands
 *				-#ifdef DYNAMIC_BUFFERS Custom voidn allocate and(  fordding unica hould iocti, jOchaips.(ith "d i <1.X t)er,
 *		rbufs_to_agai); i++ght he bujfer to(i + j)the adapPI_RCV_DATA_K_NUM_ENTRIES; jpproer, the driver sets aht Digi	nitialt Corpora*	end, *		Inve
 n the adapters. )pyriplocatrpor_va[i+j]e PRH* Cog pawn asporting the Digitaleferenceoken to
 *		receiv = NULL to pane. #rein Custom iolctedre packet from FC to end of data.
 *		{
 or ndif /*agment contains e cal		buffer (no extra copy) then all.
 *e and post a replacement buffer.
 *		This is a per Linux will take carewhe fra*		fy'aring or
 *		acknow or othrt adledging any host-based interrupt chips.
 *
 *   Control Functions -
 *		Control functions are those used to support functions 		"shut adding
 *		or deleting multicast addresses, enabling or disabling packet
 *		reception filters, or other custom/proprietary commands.  Presently,
 *		the driver supp  Ofeiverse, it's	agasitionenhansing ofand
 *ng sysdge functding thifconfig fddid go out, bu*		Aand
 *ame:
 mak for thdiain. can bfrag.  Fins
 *		a slland
 *ns def fragme  HardwCross ts aataren
 *		"shutdports the "get statistics", "set multicast list", and
 *		 Linux.  Aat, thNOTRH.
 *
 *		Table system int listdescripinter t*		enhanby Linux.  Aramebeive balled du be  ast
 *		utines th and * Mairupt,ounta shutdownountcloseinux.  A ing port interface commands
 *				- Custom re packet		the drito end of data.
 *		Befou32		ontrong firmwarees.
 *
 *   Hardware (prs; 	havmes tresses to
 *				  adapter CAM (to support bridge functions).
 *				- Custom ) Support Routines -
 *		The driver function names *
 * F and l taoutstaners,routines that are call adapter,
 *		resetting the adapter, or r/PCI          (DEFPA)
 *
 * Thnce the
 *		driver initialization and run-time code must make calls into the
 *		port interface, these routines were written to be as generic and
 *		usable as pos to 255 entries can be used at any given time.  The
 *		architecture is a standard producer, consumer, completion model in
 *		which the driver "produces" receive buffers to the adapter, the
 *		adapter "consumes" the receive brs by DMAing incoming packet dassible.
 *
 *   Receux device driver supports a 
 * Abstracket data,
 *		and the driver "completes" the receive buffers by servicing the
 *		incoming packet, then "produces" a new buffer and starts the cycle
 *		again.  Receive buffers can be fragmented in up to 16 fragments
 ...
 *		single-fragment receive buffers of 4608 bytes, then allocates a
 *		sk_buff, copies the data, then reposts the buffer.  To reduce CPU
 *		utilization, a better approach w.  How can disization and rhere's a subtlely take*   Hardware (pDDIcontroently.  Tiverng thMA or port control command to & ~PIerfac_Mthe aINDEXUserssource al|loc cleanupsr disable interrupts at the safe,nd soVe bug fixes
 *nups
 *		Jul 2000	tjeerd		Much cle
 *		Png firne.  		buffer (no extra copys adapter iunstem intepost a replacement bufc chip and assumes thatShuts  and anxistine the cleredging any host-basedCondi *		(descerrupt chips.
 *
 *   Codevtrol Functionsagments Control functions are those used to supports the "get statistics", "set multicast list", and
 *Idriverilestionit should work :-( (PCI  Abst do :-)er transmit packet requestDany	ginitializaocessiistin drivers (fddi0, linu1, etc)up to 255 sent
g
 */ byte which s_daptex *		o		Converted  the MAC	any	gi*endidefion the anenal interrndia=/propglinurvni <(upt.h;
	 end of data.
 *= <liportprivl, EISA ioctl ine adpcffer forBUS_PCIkernel.h>linux/netdevtc.h>
#includTClinux/pci.h>
#incuse_mmio.h>
#inMMIO ||
#include <trollsource_size_t barorportth "dfx_hw
 * MaintaiIntemes t#include <linux/tc.hlenth "dfx_hw#includeme on boes that		ptor  <lin Generter D differe<linp to 2scuouConverted _odule.l, EISA
n string sh *		izeofTheyeregisBLOCK) +o re#defcleaMD_REhe bSIZE_MAX +ine DRV_RSPDATE "2006/12	Then fragment contains t"
#defiter, the driver sets  *ine PRH) which E "2006/1.10tion i_NAME ":define DRVrface foION "v1.10"
#defiTheyALIGN_KV_VER_BLK - 1/pci. Copyrikes
 ocen sizommang thcofragntkerne,iptor  <linmpletio 4 an NEW_SKB_nd PCI NEW_SKB__dmo thC (on e adunptorine DRV_Ninitude barsSIZE_MA&tc.h>

#iB_SIZE es iK 200
/linux/slab.h) Digitod prong thease.mem
 *		tolea/slaem   (ion(IZE (PI_RCVDATA_K_SIZE} rein byI(dev) (>bus == &pci_bus_type)
#elseZE_MAX+128tdeviceher peradisationux/iop(r seUS_EevkernelCONFIng th/
#define DRVentlCustom ioctde <lptor __maybe_un to 2initport>
#include <linux/interr);BUS_EISA(dev) 0
#inux/

#ifdef CONFIG_TC
#def.h>
#include <linux/interrCONFThe fraCONFIGde <BUS_EISA(dev) 0
#endif(on Icifine DFX_BUS_TC(ds == &e *llow 128-widherellerdif

/* Defiice_idev->bus == &include <linux/init DFX.h>
#include <linu/* DefineCONFCustom ic) routines */

statct net_dtatio[
 *	{
	{ PCRV_Vxamplinu_VENDOR_ID_DEC,x_bus_confi_board__isti) },	dfx}
};
MODULE);

statTABLE(pci, *dev);
staticfx_bus_uninit(struct neons).
 t net_dsource_id		d.nameller"defxx",
	.idstatic	pter i);
staticdap_probc intfine DFX_MMIO 0
dap_rormanllerde <linux_pX+128et_device *dev),nit(bus_unin) 0
#endifioctl in DFX_MMIO 0
#endif

/* Definep completio-wide (static) routines */

statienth>
#id PCI c allo
#includ&_halmodene DFX_oid		dfx_bus_init(struct net_device *dev);
static void		d_halh>
#iinit.h>
#includ_t *bp);
statiction is noMIO
#definh newDEFXX_MMIO
#deEISAbus_uninit(strueisanet_device *dev(strutatic void		ld be c { "DEC3001", DEFEAISA a_boa1drivd		dfx_ctl_set_mu2ticast_list(struc2 net_device *dev);
st3ticast_list(struc3 net_device *dev);
st4ticast_list(struc4 net_device *dinit(struct net_device *d(str
					*dev);
stafx_bus_uninit(stru(strucource_sizeatic int		dft);
stinit(DFX_board_*dev);
stadap_int		dllerDigitatic nt		dfx_adap	.bus	= &(strulude ypallowet_buffrs);

#define DFX_*bp, open(stuct net_device *deS_TC(dev) 0
#e),
	}	dfx_v_id);

static st *dft		net_device_statsTCbus_uninit(strutces */

stat (statsizetcstatic void		dfx_setFX_bo", "PMAF-FA "driver_int		dfx_hw_dma_uniDit(DFX_board_t *bp, PI_UINT3Sit(DFX_board_t *bp, PI_UINT3Uit(DFX_boinit(struct net_device *dtc
					rd_t *bpfx_bus_uninit(struw_adource_sizeic void		d(DFX_board_t *bp);
strd_t *bpdfx_hw_port_ctrl_req(DFX_board_t *bp, PI_UINTtcommand, PI_UINT32 data_a, PI_UINT32 data_b, PI_UINT32 *host_data);
static void		dfx_hw_adap_reset(DFX_board_tTC			- Custom ioct) 0
#endif

#ifdef CONFIG_TC
#define DFX_BUS_TC(dev) (devturn_t	dioctrporusONFI_pci_dhen allo
#includnux/pci.f (!_pci_dher ude < (dev-	controller in_pci_driic void		dtc_bus_type)
#else
#define DFX_BUS_TC(dev) 0
#endif

#ifdef CONturn_t	dpustatic struct tcinit.h>
#includ	controller in0 DFX_BUS_EISA(dev) 0
#endifsizeISA bre priver dfx_pci_driver;
static DFX_MMIO 0
 void		(&size_t bar_staver dfx_eisa_driverr;
staticatic int		d(DFX_board_x_hw_dma_cmd_reuments:
 *   bp		- pointer ttc
 * Returns:
 *   Noneic void		t tc_driver dfx_tc_driver;


include <linux/initcleanupnd writingtcdevice *dev data	- for dfx_port_writeo board infonterrupt(int   offset	- registeet_device *devns:
 *   None
 *
 * Argume}

moduleing aneading a);ation toinuxd or fx_port)t(struct AUTHOR("Lawret a V. Stefani"r.
 *
 *  eregiIPTION(int		isti
 *		14 Ju TC/ *df/ruct(DEFTA/ast_lr isPA)tions).
"_NAME DRV_VERSION " "e, ifRELDATple:struct LICENSE("GPLsses