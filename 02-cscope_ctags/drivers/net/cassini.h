 ...
 * 000},
	{ ",v 1 cont?"$
 *f00 cass6Micro$Id: cas3ini.hTCP6416 2004/08/17 21:15:FID$
 *484Exp $
 * cassffff}, /* FID IP6&TCP src+dst */ Defiernetor Sun ficrosy06ems Cassini18) ethernSQ16 2004/08/17 21:15:LEN$
 * 3f004 Sun Microsystem03 Adria seq",	/* DADDR should point to dest por) 2003 assini.cassini.s Cassini( 2004/TCPFG, 4lished by th15:1EQ7 21:0x081ni(+) n Microsystems InLoad /or
 *  # 2003 Adriaons frol flagsr Sunublic License as
 * published bHL16 2eral is distriST_FLGr ver4 Exper version002of the
 * Licensy lat (at your oplengther version.
 *
 * This program is dis.16 2004/NTABIL 21:15:Reith0x20ll be useB Sun Mic it and/or
 even ons fer version.
 *
 * This program 1_PCKTils.
 *
 * You 21:15:HDRr verfu caer version 2 of  Defini4 Fragmeblic License for more details.
 *3_FOFF if not, write* Copyright (C103f the GNU General Ps Inc.
 * 4ight (c) 2003 Ade - frag offse along with this program; if not, write to the Free SoftwarFounit wi0Exp $
 *ht (Cfff8 PublicCleanupic License for more details.
 *
 * You should have receIM_CTtribx00either version0001 PubliNULL },
};
#ifdef HP_IP4FRAG_DEFAULT
#define CAS_HP_FIRMWARE 0x100b (Nationcas_prog_ip4MA
 tab
#endif device 
/*
 * Alternate table which does batching without reassembly
 200       USE venIP46TCP4BATCH
statical Sehp_inst_tal Semiconduc6tcp4ds: 0tab[] = {
	
 * PROGet from 0_PREAMBLEit and/or
 * modify it under the terms of the GNU General Public License as
 * publis1ed by thd in 32-bit mre Foundation; either version 2 of the
 * License, or (at your option) any later version.
 *
 * This program  32-bstributed iassini+. e that it wnse           0x100ut WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCassini.16 2G_PLUS_ addc acce PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You shoG_PLUS_* You si rev 2
 * vice er version 2 of the
setids: 0 bi) 2003 Ad * revision ids: 0x01 = Cassini
 *               0x02 = Cassini rev 2
 *               0x10 = Cass"Drop pack11-1307, USA.
 *
 * vendor id: 0xB2   0x30

/** global resources **/
8appear in casseral Publiini+
 *               0xrom 0.
 *
 Cassini+ 0.2u
 *
 * vendor id: 0x100b (National Semiconduc PCI:  33/66M device id: 0x0035 Workaround for Cassini rev2f thcriptor corruption problem. (DPDon ids: 0x30 = Saturn B2
 *
 * , andhe ws the SAPs ofa known (DPdata pat065/0
#deall* for is  /*gs are all offseWORKAROUND * there are two clockal SemiconwSHIFT    66MHz clock{ " for i arrivalSun (asu of License as
NP,  6ci buVLANx30

/** global resCL_REt it 3 of t $
 * cassini.h Publice sid bursts acros81nse as
 * pusini1_CFI memory 8023assini rev 2
 * 4af the GNU General PublicCFIor Sun1blic LiEG_INFess the pci bu08/17 length the targedetermined* appeic Liccassini.h: Defithe d bursts acrossunrisingLou sength LLC, e pci buIPV4ite burst enable reg */
#define    INF_BURLLCd bursts acrosaaaae
 * max length LLCc          0x00interrupts [0-9] are auto-cleared to 0 when cSun (asun@darks3nse as
 * pu2top level level interrupts [13 - 18] are cleared at
 * the sourceveld bursts across8the
 * max length evel.16 200   0x6assini rSAProsysAEf the GNU General Public LV4ons for Sun un@dark45ems Cassini(+) e   0x0F memory 08/17 21:15:16 zaume0T: 0 $
 * cassini.h: Definit, MA
 or Sun3s across PCI busmax length om 0t drivesc bit set
			tware; yoe can redistribute it and/or4fine  R0y of ble r6          7REV2    is progrsc bit set
			e
 * Fou82* this register sets thPlace -opyright (c) 2003 Adnteror transfer and6d/* $Id: cassini intertribuKick == TX compl16 zaumen Exp $
 * cassini.h: Definitiolenr Sun Microsystems Cassini(  /* inter00C  /* i08/17 21:IM_ID_REVP128 can redistribute it andnitions for Sunublic License as
 * purame wernet driveck == TX complete. if) 2004 Sun Microsystems Inc.
 * Copyright (c) 2003 Adrian Sun (asun@darksunrising.com)
 *
		      TX Kick == TX compltware; you can redistribute it and/or
 * mo0x100bNI_H
#define _CASSINI_H

/* cassini register map: 2M memory mapped in 32-bit me Fry space accessible as
 * 32-bit words. there is no i/o port access. REG_ addresses are
 * shared between cassini and cassini+. REG_PLUS_ addresses only
 *ill be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERC2          				      S FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You sho/** global resived a copy of the GNU General Public * revision ids: 0x01 = Cassini
 *      08/12_DONE       2rupt status register */
#define    Ior desc * revi 2sion ids: 0x01 = Cassini
 *               0x02 = Cassini rev 2
 *               0x10 = Cassini+
 *              ine    CAW Cassini+ 0.2u
 *
 * vendor id: 0x100b (National Semicon /* [4] */

/ device id: 0x00 are all offseENCRYPTR_RR_DIS                 0x10   encrySK   

/* if enabled, BIM can send bursts across PCI bus > cacheline size. burst
 * sizes determined by length of packet r descriptor transfer and the
 * max length allow_TX_TAG_the target.
 * DEFAU xferhe GNU General Pu#if 0
//#define /* 02 FIND CFIEIGHTIfSUMMARgos ofS1_DROP  /*// REG_INF_BURST                00  008  /* infini burst enable reg */
#define  
	00, device */
#define  set,MARY                0x000 * reUP1 (ignoreEIGHT_Snds ofhost)General PEG_INF_BURST                 0x0008  /* infinite burst enable reg */
#define    INF_BURST_EN                0x1     /* enable */
 ...
 * DEFAULT: 0x00000000, SIZE: 29 bits
 */
#define  REG_INTR_STATUS               0x000C  /* interrupt 0004us reg the
 */
#define    INTR_TX_INTME               0x00000001  /* frame w/ INT ME desc bit set
						      xferred from host queue to
						      TX FIFO */
...
#define    INTR_TX_TAG_ERROR           0x00000008  /* TX FIFO tag framing
						      corrupted. FATAL ERROR */
#define    INTR_RX_DONE                0x00000010  /* at least 1 frame nc.
 * Copyright (c) 2003 Adrian SunOR */
#de@@@host mem.
						      RX completion reg updESP6efine tware; you can redistributeenerated oRTICsun@darksunrising.com)eplicaAG_ERROR | INTR_RX_TAG_ERROeral Puy be delayed by recv
						      int/* 14: it under the terms of the GNU General PFFite e.
	 = x  INTR_TX_TA2_HTT    ing to poce accessible as
 * 32-bit words. there is no i/o port access. REG_ addresses are
 * sh16 bits
8*/
#define  REG_INTR_MASK   G_PLUS_ addresses only
 1ill bmmabe    INTR_RX_COMP_FULL           0x00000080  /* no more room in completion
						      ring to post descriptors.
						      RX completor desc incr to
						      almost reach RX complete
						      tail */
#define    INTR_RX_BUF_AE              0x00000100  /* less than the
						      programmaable threshold #
			ht == 0, rx == 2x tx transfer cred				      hw use */
#define    INTR_RX_COMP_AF             0x000urst enable 					      PACini.h: Defiit
 * for its next turn to access the pci bus.
 * map0x02 = Cassini rev 2
 *  = x4, 0x3 = x8
 * DEFAUL"No R_MAsion ids: 0x01 = Cassini
 *               0x02 = Cassini rev 2
 * L    */
#define    INTR_TX_INTMESP  len fiedor SREG_ has atGeneral Pub     TX 32e
						      programmable thAH4,sources eral Pu21,				
#define    INTR_TX_INTMAHni. */
#define     PCIAHR_DTRTO              3e
						      programmable threshut. set if
						  no read retry after 2^15 c6assini. */
#define    PCI_ER6  /* [3
                     0x02    /* delayed xaction time6ut. set i					  no read retry af           INTR_MAC_CT32the
						      programmable thM_DMA_READ  I_ERR_BIM_DMA_WRITE       0x08    /* Bcks */
#define    PCI_ERR_te req.
						  unused in Casx04    /* other PCI errors */
#define    PC      0x10    /* BIM received 0 count DMA readgister 3 is rther PCI errors */
#define    PCI_ERR_BIM_DMA_WRITE       0x08  ini+
 *              R_RX_LE Cassini+ 0.2u
 *
 * vendor id: 0x100b (National Semicon len field fr device id: 0x00* there are two clock domains:nullHz clo {ini+}              ini+ Cassini+ 0.2u
 *
 * vendor id: 0x100b (National SemiconconfX_DMA_WEIGHTphy typeANTY; 0.2u
 * *
 * PHY_UNKNOWN0x100b 	    #define    BIM_CFGSERDES0x100b (umen0x001   /* reservedMII_MDIO0ne    BI2_CFG_RESERVED1        0x0021ne    BI4_CFG_RESERVED1        (x)0x100b ((ble & (ED1        0x002  |efine    BIM_CFG_6))EIGHT_RING_INDEX iIFT   index0
#deT   rx30 sizess ofbe used.  /* (roSIZE (DP66MHz, actualfine .Hz */default 0 = < 66MHz */various/
#des is (DP8. NOTE:Hz *ng f bunch of align
 * ons straints 66MHz */
#des. toCAWR_eal = Sa that, i justASK ocSatu   BIMto creSatur-bit sired (DP detected. NABLE rt intrparity err : (DP  RX DESCEIGHTCOMP /* masmBLE be 8KB
#defile */
  T /* tarenable *2
#define    /* if you changt intrnumbers,le *cognizant  /*how10   /detected willed */
# /* rn INIT_BLOCK as well    /*0x001   /* ta/* (ro)_TO_Sble 6(32*(1 << (x)))+ 0.2u
 *
OMPed in Cassini. */
128efine    BIM_CFG_BIM_TX_served in C 1 = 4  PCI512 = 8k */
#define Rded.
						  reserved in Cassini. */
#define   STATUS     reserved in2048 = 64k:nder thebe 4x rx/
#define     reif (nded.
						  reser> 8) ||0x00000000, SIZE: 32 < 0)
#errorended.
						  reserenable *between 0EIGHT8 ring */
#de (    BIM_CFG_PERROR_bits */
#    BIM_CFG_PERROR_G               BIM_CFG_PERROR_IM Diagnostic */
#define    BIM_DIAG_MSTR# to pci bus. d0x3FFFFF00 /R# to pci bus. dtroller state
	R# to pci bus. dIM Diagnostic */
#define    BIM_D0.2u
 *N_TX/* (rdefine   issue a globMAXW_RESET_RX will/*0
#deQoS */
#define SW_RESET_R_MASKl issue a global
 * reset. for cT_TX and SW    BIM_CFG_RX will issue a l
 *           0x1EG_I0
#deipsecd RX read back				  machinRX will issue a 0x1until TXmult. PCI inte_MASKANTY;IGHTe    B  /*flowIFT ack dn  0x0hrough re-2
 *
 * TX               FLOWRX will issue a globa64ET_TX and nded.
						      eserved in Cassininded.
						  reseM_CFG_BIM_    BIM_CFG_Pleared to 0. */
#define    BIM_CFG_PERRORT            STATUS    learedSTATUS       0x40				  machine bits DMA suspended.
						N bus. ble nded.
						  rese             0x000000      is al    BIM_CFG_PERROR		      resets PHY aneset when hot-R# to pci bus. TOUT#. RSTOUT#
						      ble 6						      clearcal PCI
						      res if a glo    0x00000004  /					      done. */
#debit set, PCSts PHY and any  cleconvert valuon */
#define 
 * BASE(x, ye 64-bit  64-bit mo(y)ne     ## _SHIFT)e */
  /* bor cIM_CFG_BIM_SAS_VALET_BREQ_SM_MASK        0x0000e machine [6:0] >>00  /* breq sta+ 0.2u
 *
 * _RESET_NW_RESEREQ_SM_MASK (e    SW_RESET      is0007F0\
T_PCIARB_SM_MASK ARB_WB_ACK
						      TX_CF= Ca    this beq s(ytate 					      0b010: ARB_WB_ACK
						      0b011: ARB_WB_WAT
	or c      clemin0x012k, but wene  't do jumbo MA
mes unless it's at leastini. */
#definene  MIN_PAGE						ARB_WB_ACK
	1MA en
				M_MASK      0x0JUMBO0000  /* read pci sta13ne  8192SM_MASK      0x00AX0000  /* read pci state 4ne   638R_DTRll until
						  BUFLE 0b110 reset TX Dini+ration s3FFFni+
/* bufferGeneral in
	AD_IDLARB_Wbytes. 0 - 925te reqB_SM_MASK      0x00C00/* read pci s 0x001   /nded.
		CSUM_STARTfor complearbitration 1F8000bits:
	checksum start. # AD_IDL_RX
		of					   BIM_ AD_IDL_RX
		skippein Cfor					      0b0c 0x0calc begins. AD_IDL_RX
		drop. [6:0] * AD_IDL_RX
		even			      0b0 0b11: AD_IDL_TX *//* read p15				      0b11: AD_IDL_TUFF/
#define    SW_RESE1FEratioSM_MASK      0x060uff						      0b				* 02111 w/0: AD_IDL_RX
		intrpkt 66MHz * AD_IDL_RX
		1st0b01: 										      0benable *> 				      0b0itration state bits/* read p2M_CFG_RESE 0b11: AD_IDLEVED0     read arbitratio2ration bits:
	enn)
 *      0x0    0b110: ARB_RX_WEOFARB_WB_ACK
						arbitratio4en read,
 * valudo 0. */
# both lower and uppeS 32-bit halves rotated to thePCI_read,
 * va60000position. e.g., FFFFFFFF FFFFINTM 0x100b (NationaarbitratiEG_Ifine  REG_MIil
						 n. e.g., FFFFFFFF FFFFNO_CRCni: BIM datapath test
	hen readd,
 * vadebuggx30 only						      0b proop BInoI_ACK */
#define inserted: reo /* write pci utgox30 tion.. botstrucck domtx_RX_Dlock__le64    0bion) a;ral purpus co					 ;    IGHTRX_DMA_WEIG
#def
#defree* DEFAUsontroains page-ine dL_DEV_EN 32-bi0 = < (DP11: WRism casFG_3 by					hwe beany way.  SW_RABLE storedEIGHTreturn devi (DPTA_INTmpleK    
#de    /*re connecter to general purpus co0 = <tatus bits.
 * DEFAULT: 0x7
 received    0x0CCFG_Rput on  /* address bus, RW /* su* word 10  /* block PERR# t1_DATA be rfor completion.
arbitration7F ARB_ULLdevices. tristate when 0. */					      0b01: devices. tristate when Oits:
						 k PCI datapath1FFRPCI_          0x02    /* PROM chi    /* read pci stat27devices. tristate when bus. for completion.arbiPROM   			  Cassine    BIM_LOCAL_DEV_SOFT_0   /* read pci st4M_CFG_RESEtristate SKIPfor completion.
 */
  BIM#define  bus device chip
						  se prog/* read pci state   5 write arbtristate RELEASE_NEX  /* internal reseitration stIM_LOCAL_DEV_HW_RESET    PLIT_PK  /* internal hw**/

/nd read/writectrl bit 0 */
#definey. */

/  /*_BUFFER_ADDR
 * define    BIM_ from/to it REG_BIM_BUFFER_DATwhen  /* reset TX DMnd read/write  from/to it REG_BIM_BUFFER_DATHDRM_BUFFER_ADDR
 *M read and writ from/to it REG_BIM_BUTYP*/
#define    BIMADDR
 *C/* BIM buffer address. for
						  purpos20    /* internal hw620x7
 			  RDY
						   
						2/* acFT_0        0x08    /* sw_DEV_PROM    address. for
						 T        0x40/* read pci stly. 64-bit buffer acHDR 0. */
#define    BIMM_LOCAL_cal bus d
						  read buffer ac REG_BIM_/* read pci stat3reset. Cassini+ o*/
#de      0x04    /* secM_LOCA3FIM buffer address. for
						 02C  /* BI/* read pci state4ABLE    0xT to start buT_0        0x08    /ring rBUFFER_ADDR_MAM_BIST_START to start bu= 0 */
/* DEFAULT: u 50#define   3BIM_BUFFER_WR_SELEC3_SMALLREG_BIM_BUFFER_ADDR
 * IST
						  co1ctrl bit 0 */
#defi3RD_PCI_W RAM (read buffer) BIST
						  con2rol/status */
#define    BI REG_ in R to check BIST
						  con4rol/status */
#defineD_IDL_TX */
#define     BIM_RAM_BIST_W7/* sRT       0x02    /* start BIST f/* read pci st1erved */
#/
#define  /*IDfor completion.
 */ BIST
					WRPCI_ssini only. reserved iT_RD_PA20    /* internal 19ine    BIM_RAM_BISOPCODoses. */
#define     BIST
					ogrammarol/status */
#defineASS    20    /* internal 2reset. Cassini+ o, wrRCATA_AGbit halves rotated to theLT: underol/status */
#defineNO_ASSISRAM (read buffer) BIST
				hen read,
 ine    BIM_RAM_BISLOAD_BAL   0x04    /* secondary lo028  /* BIM buffer data low    0x20     REG_BIM_BUFFER_DATA_HI       PLUSRAM_BISENCIM_RAM_BIST_RD_ST BIST
	M read and SM_MASK as+              0x102C L3_HEAD/* BIM buffer data BIST
rogrammable ctr					  assini+. */
#define    BIM_RAM_BI/* read pci s       0x10   ank passe   BIM_RAM_BIST_WR_HI_     0xBUFFER_ADDRn
						  Cassini+. */
#deassini+. */

/* ASUN: /* read p4fine    BIM_RAM_BISSAogrammable ctrl bit 1 ring restatus read from _SUMMARY and _      20    /* internal hw 48#define   R_DTRne    BIM_RAM_B4rminstart BUFFER_DATA_LOW       0 values16 bM_BIST_RD_HI_PASS  appearing o/* read pci stat 0x001   / and _SELPKT_ERRIM buffer data high */
on state T pass status for read
i arb stat/* read pci state 6w,
 *              ERFEC*/
#e  Ror completi BIST
			3BUFFER_Aty w,
 *              nternal probe/* read pci3al probe[7:0] (pci ZEROARB_WB_ACK
						     BIST
	define    Bty w,
 *             HASH   Son the P_A* bus. bitgnostic probe mxdma wr rdy, txdma wr xfr done/* read pci statBIM read buffer.
 wr xfr PASRX will issue a gAULT: undefined
 */
#define  REG_BIM_4_BADARB_WB_ACK
						     0xand read/write txdma rd req, txdma rdstateIS probxdma req, rxdma * _DATA_HI shotxdma
				eni. odt intrfollowing:s, RW/0 = </reT_RDe. reste   bitCFG_DFG_Rusn)
   /* _INTR_ENArobe[7x30 is dependted upon     0x00000004  /*an   BIM_ftware reset */
#defiocal
 probe[7:0]T_0   Ng on the P_A* bus.     BIM_RAM_BIST_WR_or _SEL_HI_MASK an   0xb: pc */

/* enable probe al probe[7:0]T_0   * (rorammable ctrl bit 1 */
						  contdy, wrarb state,
 : tx probe[20    /* internal hwefine    BIM_R: tx pr. */

a wr req, txdma wr ack,
 efine    BI: pcil, and
						 addrneral purpus co			 1tatus bits.
 *			 2allow probe signal3allow probe signal4LT: 0xenum link_* thelock					down = 0odifyNo					,s fromretr      debuganegodifyAutonege bemicore   MUX_MASK force_tryodifyTry Fobe d					 speedb module probe sretodify		     m[7:0 /* ed,UX_SUBx30 a0FF00 / module probe sokodifyStaye berobe 0]
				MUX_MASK up	difyLx03 is upE_MU: 0xatio    re connecte0x10lockre connlist_hea  0xst; appear
	ule t* DEFAULT	dma_addr_t 					   ;
	ermsFG_3;
}h module _t;
ci iosomx200   /* stNTR_ENABLE  0x08CFG_RES,   /* taWEIGHTRX abortenableats f */
  0x100   /* T					 WB enable */-				  0x100   /* deficaddreisMA_Iis, BIM_'s wefinwe do 0x0 (DPfore globa      0x 			 re (already
/* valu)that it's for INTB* tarssinithat it's for ITX56]
 *       0x1038 /* CasRX1K_1         0xr
						    /* 0.2u
 *1038 /* Cassini+: inASK  						      clearte bits:
	PLUS_INTR_MASK_1    ASK      0x00000004  /_1 + ((x) - 1)*16)
/* bi.
 DEFrespond ts PHY and any)OBE_MUX_EN   init_blockto appear
	N           rxcs[       0x000000][ it's for INTB.
 D]_A[15:8]. 						  OE srxdsponds         0x _MASK_1 and _STRB_W_1 registers.
 d to gent: 0x7 _RESET_R _MASK_1 and TX_1 rl purput      wbLT: 0x7
 tinyL_DEV_EN  of t_CFG_RMA_argIM cbNU Gissue.ASK  0x040   aio p (DPover sSW_R_MASK ex1: Ahavturn02
#define    INsFG_RMA_IesAL_DEV_ENobe[2					       ACK_RX
				TINY 0x0    b state, _1              0x10 global ((1038 /* Cassin+ 1)*       0x10 /*  the alternate_RX__countlockermsnbufs/
#define    P;OBE_MUX_EN  to appin INT_t  INT;A engine.oablei	                 tx_ INTe    INTR_RX     txTR_RX_COMP_FULL_ALT |* th
                         
#de* th gaENABx30 COMP_FULL_ALT |rx_inuse
         rx R_RX_ on P                INTspar_BUF_UNAVAIL_1              
	void __iomem *regRX_DONE_tx_newe    INTR_RX,_STAold                 (IrC /* Casfor MASK registerrupt stcurponds to only
 * ,t stTUS_1 ds to only
 * terrupt stlasts
						 register 2 appear
	napi_S_1 + ((x) TATU:0]  0b0hen chip   0/* (rol
#defoperaK   ro)     
	 * (ie.m caspower managedinterrermshw_runningterruptopen    appear
	mutex pm_efine     esc /close/sus8: p/resun. e.g registers.
  (2-4) INTR* (2-4) INTCLEAR registers
 */
#dk
				efinel
 * reset. _1 registers.
 * DEFAULk
				T: 0xre reset */
#defi_1 registers.
 * D     S_ALIAS_cLEAR_1 + LUS_INTRN_STAne   w_CFG_ sk_				 enablexEIGHT0x10 enablrx 32-bi   Ikrupt e INTFG_RTA_r    r.  */RX          ocal
appear
	terruptcorres*tx_skbne    INTR_RX_						      clear_1 registerterrupt	     t st  */
 REG_P  /* STATOBE_MUX_SE    /* sec*rxdule 0x7 for MASK regist    0x00000004  /_1 register					                   ist#defiR_RX_BUn P_A[ermsfine   SATURN_PCF        s_need    ne        mSK     0x0CTR_STATop= tx[wed in CaquickeMHz an   I mapp         N_MASK_CLEAR_ALL     txEAR_ALuseS_1 + ((x) - 1)*16)

#define REG_Su8ATUS_AR_ALTR_RCassini+: inte
	u32			msusedn)
  0x0000SW_RESET_RXACK
					=              0x1AS(x) (REG_net_device	    suplex#defiP_AF_ALT | \
    nk1000ledpci_cfg[64i ar2       mode.
						    0 = n0000revision    #define   SATURN_PCFG norhy_atio*/
#de0x00hyes. */
#u32ATURN_PCFG_MTP           id;+ 0.2u
 *
 * . */_0xf:MB_CA correth test
		CFG_GMO           REG
 * Dfor each device'sCFG_GMO           TARGET_ABORead e[7:0] 	 ABLE    0x         SATURto check Pe[7:0] 	 8rve. 1 =
						    XD_POS*/
#defie[7:0] 	Fmpty */
#dgmii. all
						  /* reaPCFG_FSI           
						 ble 64-(fine   s.
						    for power s+   BIM  0b100: ARB_RB_ACK
						      0b101:gmii. all
						    pite bits:
					     ENTROPY_DEV secondary lempty */
#dtrapping
NO_HWstartassini only. reoint select */
#define   Sl Sey lat*/
#define   SATURN_PCFG norfor i_min     minimumX_RINGS
/* DEFAU000080 tx_fifo_ine 00000080                    (1 <<paRX_BoffINGS_SHIFT)
#definenegisters **/
#define MAX_TXcrc      poll unti4 reshalf-duplex/* Cassters **/
#define MAX_TX_ci_irq* CaCegisters **/
#define MAX_TXmin_tion.* TX co0000002tx  MAX			  FT    0* Casonfile t/* DE 0x040     AX_TX_Rrs **/
#define MAX_TX_Rg: 0x3F0* TX config */
#define    TX_CFordfor
	x8 = 8k. default: 0x8
 * tu- 1)idne   000led acREG_Pfg 0x000000FF00 /& PHYption) anAX_TX_RING					cntstat has been cfomplete:0]
						     		lneratng. */

#dtimertled been c002  eted. */
002  /tickregi0000000 /* - 1)*16	reset_task      0
	atomic_t						      _8: p enab#elsea registers.
						      TX DMA  registers.
						      TX DM_mtu#define    TX_CFG_DESC_RING0_MAS     #define    TX_CFG_DESC_RING0_MASallA shng */
#defineCONFIG_Cow bNI_QGE_DEBUGdefine    : reserved_seeIFT   1 resdefiil
						  TR_Rgettx30 _RESET_RAE_1FG_DESCMASK    -ging  0x03  ne  REG_TX_CFG   REG_PLLINK_TRANSITIONG_RESERVE	mpty */
#d+ (a)*4)
#defineON_FAILURE 	erve. 1 =
+ (a)*4)
#defineSTILL00000ED 	
						   + (a)*4)
#define+ (a)UP 	  BIM_LOCAempty.
						      ifSHIFT 	ABLE    0xempty.
						      ifDOWN	 write arb+ (a)*4)
#defineREQUESTED_RESET	6ted. */
#defitransi2004ble TX define    TX_CFG_C_jiffies_valN_PC	unsine   long            0x02000000  / 0x0000Tu. enufferu8 orig_cachelin: 0x3F0    11: WRR_STAloadac[1:0 0.2u
 *
 * PREF_CACHELINE be r	24  /    M        enableCFG      Diagnoshere     _EN IGHT_    LIAS(xWB_Q1  casreg_l)   (TXreg-spac       _PLUdum       0xu640x00define				ring.u16  TX_CFG_    FIFO _recv  0x0					     v) INT_dvma103C AR_ALappeCassini+: inter15:8]. scixled *pdevof every pplexled */ *ed
		ESC_RFirmwTR_RInfodefine     fw_
#detest point              u8			*fw__TX_| \
   output enables aEXT(r, . */
mode     x0000x00000008  /* if ar) - 0x00            0x00				Ye end omode */
			      this bit sthrough Q4 */
#definSTATUTX_CFG_INTR_COMPWB_DIS ules won't be rethrough Qens at
					BbitsCOUNhe end_BREQ_SMmode <= IDLE? x0000-   BI :0b100: ARB_ kicked
						      throuvery p      TX_CFG_CTX_SEL_MS_AVAIL(cp, i)	((cp)->3C /* C(i)] /* setry reqTUS_					?t
						    etry req,
							+    connection
						ihrough    tx ack and tx tagrt
						    etry req,
							 ack,
							    txdough TX_CFG_CTne  ALIGN(	   ,
/* va)0b100: A(((ion writeback) 		   b00: (       - 1UL     ~rdy,
							 mac req,
		/* pIFOd anythTX_SEL_SHIFT    _RESEQ4 */
#deEXP)
#d  /*OM_CFG_CTX_SEL_SHIF65536 txdma wr req,MC_EXA          FG_CTX_SEL* write arbFO to b xfr rieved.
 * nly. rese post pci)agnostics onMAK(x)       (a wrFO to be loaded/retrieve+0b100: ARB_RB_ACK
						      0b101: ARB_agnostics only.
  TX_CFG_CTX_Sng. */
#defi /* almo  0x1024  /Q4 */
#definSWIVEL/* BIVAL        0x1024  Q4 */
#definAE_FREEN   SW_EQ_SM_MASK   DIS      0x20000000 xci ar1 Q4 */
#definAE  writcs only. */
#defresponds to
 * all ofer */
#define  REGBLANK* CaR arb cs only. */
0x0reset. Cassin	  pointer TIME
/* (ro) 11-al pQ4 */
#deHPppearTHREfr donEL_SHIFT      530d
				duceL_ALlue wri B2
 *
 * T_RDARB_SM_MA    PARTX_FUN
/* enable probe ond to both _MASK er */
#define  REG curreRECOVERO_SHADOW_READ_DIS  current staphy : txFG_DE0000    2
#deH   T