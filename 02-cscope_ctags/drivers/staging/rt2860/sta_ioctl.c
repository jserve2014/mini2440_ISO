...
AutoSwitch;*****else * Ralink Tech Inc.OPEN, No.36, Taiyuan St., Jhubopen,
 * Hsinchu County 302,
 * Taiwan, R.O.C.
 *
 * (c) Copyright 2OpenRalink Technology, Inc.
 *
 *SHARED, No.36, Taiyuan St., Jhubshared,
 * Hsinchu County 302,
 * Taiwan, R.O.C.
 *
 * (c) Copyright 2Sware Ralink Technology, Inc.
 *
 *WPAPSK, No.36, Taiyuan St., Jhubepapsk,
 * Hsinchu County 302,
 * Taiwan, R.O.C.
 *
 * (c) Copyright 2      on) any later version.          NONE                      *
 *   none                                                                 *
Noneon) any later version.          2                         *
 *   2                                                                    *
NESSon) any later version.                                *
 *                                                                    *
     *
 * MERCHANTABILITY or FITN FOR A PARTICULAR PURPOSE.  Sehe         *
 * GNU General Public License for more details.        Ralink Tecchu Countreturn FALSE;
*****y 302,
 * Taiwan,PortSecured = WPA_c) CoX_PORT_NOT_SECURED       DBGPRINT(RT_DEBUG_TRACE, ("Set_ R.O.C.
 ****::( R.O.C.
=%d)\n",ty 302,
 * Taiwan, R.O.C.
))              TRUE;
...
/******=**********************************************************
    ...
    Module NaMA  02escription:chu CountSet Encryioctl Typn, IncR     .c

    Ab *** * Rall parameters are OK,       otherwion, Inc**********************************************************

    Module Name*/
INTbstr_ct:
  TL r *************************************************************************
 * Ralink Tech Inc.he hope that it will be useful     *
 * but WITH****
 **
 * Ray 302,
 * Taiwan, R.O.C.
 >                     *
nchu Count         *
 ****efin// do nothing                           WepStatusfine *
 * (c) CopWEPDisabltion) an                      airCipher 4

extern UCHAR    CipherWpa	                     GroupCipherWpaextern UCHAR    CipherWpa2Tem}n) any later version.        EP, No.36, Taiyuan St., Jhubei  RTDebugLevel;
#endif
...
extern UCHAR   EnpherWpa2Template[];
extern UCHAR    CipherWpaPskTkip[];
exterINFO, *PR CipherWpaPskTkipLen;

typedef struct PACKED _RT_VEINFO, *PRT_VE  UCHAR       DriverVersionW;TKI   UCHAR       DriverVersiotki;
    UCHAR       DriverVersionY;
    UCHAR       Drive< WEP_SMALL_KEY_LEN 			(40/8)
#define WEP_LARGE_KEY_LEN 			(104/8)

#define GROUP_KEY_NO                4

extern UCHARct:
    IO2INFO, *PRT_VERSION_INFO;

struct iw_priv_args privtab[] = {
IW_PRIV_SIZE_MASK, " CipherWpaPskTkipLen;

typedef struct PACKED _RTIW_PRIV_SIZE_MASK, "conn  UCHAR       DriverVersionW;AES, No.36, Taiyuan St., JhubaesV_IOCTL_SHOW, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
  ""},
/* --- sub-ioctls definitions --- */
    { SHOW_CONN_STATUS,
	  0, IW_PRIV_TYPE_CHAR | IW_PRIV_SI3E_MASK, "connStatus" },
	{ SHOW_DRVIER_VERION,
	  0, IW_PRIV_TYPE_CHARradio_on" _SIZE_MASK, "driverVer" },
    { SHOW_BA_INFO,
	  0, IW_PRradio_on" },
	  UCHAR   , Inc.,                                      Orig          = GROUP_KEY_NO              , MA  02111-1307, USA.             *
 Rory Chen   01-::(Rory Chen                                                  *
 *************************************************************************

    Module Name:
    sta_ioctl.c

    AbstraDefault Key IDelated subroutines
...
INTKeyID *************************************************************************
 ULONG0/8)
#define Set_DefaultKeyID_P);

dx       oc(
   = simple_strtolch Inc0, 10)Ralinkif((RTMP_AD>= 1 ) &&       ar<= 4inchu County 302,
 * Taiwan,    arg);

empl(*****)Set_Key1_-;

IFoundation, Inc.,               _LENInvalid****ument MA  02111-1307, USA.             *
     arg);
 ...
 *************************************************************************
    ...
    AbstraWEP KEY1elated subroutines

    Revision History:
    Who         When          What
    --------    ----------    ----------------------------------------------
    Key1 *************************************************************************
 *ntNT Set_DefaultKeyID_PrultKeyID_Proc(Le GNU GeMP_ADAPTER   pAd,
    IN	PVOID			pBuiRalink*************** PUCHAR          arg)CipherAlg=CIPHER_WEP64       define NR_WEP_KEYS 				4
#define WEP_SMALL_KEY_LEN 			(40/8)
#de WEP_LARGE_KEY_LEN 			(104/8)

#deff);

IDAPTtrlench I        s007, Set_KLenLevel;
#endif

#case 5: //wep 40 Ascii tL relatehu County 302,
 * Tware Key[BSS0][0].DAPTER	pAf);

INT SetNT (*setmemcpyine NR_WEP_Karg);

static struct ,****,AR *nam);

INT   arg);

INT Set = TGnWifiTest_Pr PUCHAR     2111-1307, USA.             *
 _ADAPTER	::    1=%s andAPTER=%s      info" }P_AD"     PUCHAR     breakn_Proc},
	mit_P10oc(
	IN	PRTHexAPTER	pAdapter,
	Ifor(i=0; i <AR *name i++(40/8)
#define
#endif

#P_ADAPTERf( !isxdigit(*ch I+i)) (40/8)
#defineKey2_Proc(
    IN  PRTMP_NotRegiovalu      *amble_Pr  UCHApter,
	IN	PUCHAR			arg);

static struct {
	CHAR *nam / 2 n_Proc},
	{"CoAtoHch IncRTMP_ADAPTER pAdapter, PUCHAR arg)					Set_Con_Proc},
	{"CoC, RTMP_PRIVATE_SUPPORT_PROC[] = {
	{"DriverVersion",				Set_DriverVersion_Proc},
	{"CountryRegion",				Set_CounHexgion_Proc},
	{"CountryRegionABand",			3oc(
	IN	104TMP_ADAPTER	pAdapter,
	IN	PUCHAR			arg);

static struct {
	CHAR *name;
	INT (*set_proc)(PRTMP_ADAPTER pAdapter, PUCHAR arg);
} *PRTMP_PRIVATE_SET_PROC, RTMP_PRIVATE_SUPPOR128PROC[] = {
	{"DriverVersion",				Set_DriverVersion_Proc},
	{"CountryRegion",				Set_CountryRegion_Proc},
	{"CountryRegionABand",		26
	{"HtGi",		egionABand_Proc},
	{"SSID",						Set_SSID_Proc},
	{"WirelessMode",				Set_WirelessMode_Proc},
	{"TxBurst",					Set_TxBurst_Proc},
	{"TxPreamble",				Set_TxPreamble_Proc},
	...
	{y_Proc},
	{"HtBaWinSize",		        	Set_HtBaWinSize_Proc},
	{"HtRdg",		        		Set_HtRdg_"HtMcs",		                Set_HtMcsdf

INToc(
ADAPTER   pAdapteOC[] = {
	{"DriverVersion",				Set_DriverVersion_Proc}ADAPTER   pAdapt (n",				Set_Cion_Proc},
	{"Co              _Proc}                arg);

static struC, RTMP_PRIVA
INT Set       //bstrakeys (into ASICnchu Cdefine NR_WEP_KEYS 				4
#define WEP_SMALL_KEY_LEN 			(40/8)
#deKEY_//	(10 supporType_P Tech      OldTER  stuffevel;
#endif

#AsicAdd_Key3_ProEntr(PRTMP_ADA******WpaSupport",              0   {"WpaSupport",                  Set_Wpa_Support},
	{"FixedTxMo,						Set_Key3_Proc},
	{"Key4",						e",                 Set_FixedTxMode_Proc},
    {"TGnWifiTest" argst",					Set_TxBurst_Retry",	  NULLorceGF_Proc},
	{"LongRetry",	       on_Proc{"Key3"     *
 ************************************************************************

    Module NameAPSK_Proc(
    IN  PRTMP_ADAPTER   pA2apter,
    IN  PUCHAR          arg);
...
static s1ruct {
	CHAR *name;
	INT (*set_proc)(PRTMP_ADAPTER pAdapter, PUC000);
g);
} *PRTMP_PRIVATE_SET_PROC, RTMP_PRIVATE_SUPPORT_PROC[] = {
	{"DriverVersion",				Set_DriverVersiLONG		c},
	{2CountryRegion",				Set_CountryRegion_Proc},
	{"CountryRegionABand",			Set_CountryRegionABand_Proc},
	{"SSID",						Set_SSID_Proc},
	{"WirelessMode",				Set_WirelessMode_Proc},
	{"TxBurst",					Set_TxBurst_Proc},
	...
	{"TxPower",					Set_TxPower_Proc},
000);
		pAd->bPCIcet_Channel_Proc},
	{"BGProtection",				Set_BGProtectionMode >= 	{"RTSThreshold",				Set_RTSThreshold_Proc},
	{"FragThreshold",				Set_FragThreshold_Proc},
	{"Ht Ndis802_11AuthModeWPANone)
            "HtMcs",		                Set_HtMcs_Proc},
	{"HtGi",		                Set_HtGi_Proc},
	{"HtOpMode",		  000);
		pAd->bPCIclkOff = FALSE;
	...
	if (pAd->StaCfg.AuthMode >= Ndis802_11AuthModeWPA)
	{
		if (pKey->KeyIndex &y_Proc},
	{"HtBaWinSize",		        	Set_HtBaWinSize Ndis802_11AuthModeWPANone)
            {
                NdisZeroMemory(pAd->StaC_Proc},
	{"HtAutoBa",		        	Set_HtAutoBa_Proc},
	{"HtBaDecline",					Set_BADecline_Proc},
	{"HtProtect",		        	Set_HtProtect_Proc},
	{"HtMimoPs",		        	Set_HtMimoPs_Proc},
	...
	{"PktAggrEY));
            pAd->SharedKey[BSS0][0].KeyLen = LEN_TKIP_EK;
            NdisMoveMemory(pAd->SharedKey[BSS0][0].Key, pKey->KeP_EK + LEN_TKIP_TXMICK, LEN_TKIP_RXMICK);
            }
            else
            {
    Mode",					Set_AuthMode_Proc},
	{"EncrypType",					Set_EncrypType_Proc},
	{"DefaultKeyID",				Set_DefaultKeyI Ndis802
	{"Key1",						Set_Key1_Proc},
	{"Key2",						Set_Key2_Proc},
	{"K);
            }
...
#endif
    {"WpaSupport",                  Set_Wpa_Support},
	{"FixedTxMo1e",                 Set_FixedTxMode_Proc},
    {"TGnWifiTe  	pEntry = Key.TxMic, pAd->SharedKey[BSS0][0].TxMic, LEN_TKIP_TXMICK);
 t_ForceGF_Proc},
	{"LongRetry",	        		Set_LongRetryLimit_Proc},
	{"ShortRetry",	        		Set_ShortRetryLimit_Proc},
#ifdef RT2870
	{"efuseFreeNumber",				set_eFuseGetFreeBlockCount_PSK_Proc(
    IN  PRTMP_ADAPTER   pA3apter,
    IN  PUCHAR          arg);
...
static s2ruct {
	CHAR *name;
	INT (*set_proc)(PRTMP_ADAPTER pAdapter, PUC  }
		g);
} *PRTMP_PRIVATE_SET_PROC, RTMP_PRIVATE_SUPPORT_PROC[] = {
	{"DriverVersion",				Set_DriverVersiable ac},
	{3CountryRegiontryReey1_Proc},
	{"Key2",					untryRegionABand",			Set_CountryRegionABand_Proc},
	{"SSID",						Set_SSID_Proc},
	{"WirelessMode",				Set_WirelessMode_Proc},
	{"TxBurst",					Set_TxBurst_Proc},
	{"TxPreamble",				Set_TxPreamble_Proc},
	{"TxPower",					Set_TxPower_Proc},
        ...
        elet_Channel_Proc},
	{"BGProtection",				Set_BGProtection    pAd-	{"RTSThreshold",				Set_RTSThreshold_Proc},
	{"FragThreshold",				Set_FragThreshold_Proc},
	{"HtAd->StaCfg.DefaultKeyId], Hexf(CIPHER_KEY));
            pAd->SharedKey[BSS,
	{"HtGi",		                Set_HtGi_Proc},
	{"HtOpMode",		    }
		}
        else
        {
            ...
            pAd->StaCfg.DefaultKeyId = (pKey->KeyIndex & 0xFF);
y_Proc},
	{"HtBaWinSize",		        	Set_HtBaWinSizeAd->StaCfg.DefaultKeyId], sizeof(CIPHER_KEY));
            pAd->SharedKey[BS_Proc},
	{"HtAutoBa",		        	Set_HtAutoBa_Proc},
	{"HtBaDecline",					Set_BADecline_Proc},
	{"HtProtect",		        	Set_HtProtect_Proc},
	{"HtMimoPs",		        	Set_HtMimoPs_Proc},
#ifdef AGGREGATION_SUPPORT
	{"PktAggrcryption2Enabled)
            {
                NdisMoveMemory(pAd->SharedKey[BSS0][pAd->StaCfg.DefaultKeyId].RxMic, pKey->KeyMaeyId].TxMic, pKey->KeyMaterial + LEN_TKIP_EK, LEN_TKIP_TXMICK);
             SharedKey[BSS0][pAd->StaCfg.DefaultKeyId].TncrypType",					Set_EncrypType_Proc},
	{"DefaultKeyID",				Set_DefaultKeyIAd->StaCherAlg = CIPHER_NONE;

            // Update these related information to MAC_TABLE_ENTRY
      2 	pEntry = &pAd->MacTab.Content[BSSID_WCID];
            NdisMoveMemory(pEntry->PairwiseKey.Key, pAd->SharedKey[BSS0][0].Key, LEN_TKIP_EK);
        	NdisMoveMemory(pEntry->PairwiseKey.RxMic, pAd->SharedKey[BSS0][0].RxMic, LEN_TKIP_RXMICK);
        	NdisMoveMemory(pEntry->Pairwis2e",                 Set_FixedTxMode_Proc},
    {"TGnWifiTeS0,
        aredKey[BSS0][pAd->StaCfg.DefaultKeyId].RxMic);
...
static s3ruct {
	CHAR *name;
	INT (*set_proc)(PRTMP_ADAPTER pAdapter, PUC	pEntrg);
} *PRTMP_PRIVATE_SET_PROC, RTMP_PRIVATE_SUPPORT_PROC[] = {
	{"DriverVersion",				Set_DriverVersic WEP c},
	{4CountryRegion",				Set_CountryRegion_Proc},
	{"CountryRegionABand",			Set_CountryRegionABand_Proc},
	{"SSID",						Set_SSID_Proc},
	{"WirelessMode",				Set_WirelessMode_Proc},
	{"TxBurst",					Set_TxBurst_Proc},
	{"TxPreamble",				Set_TxPreamble_Proc},
	{"TxPower",					Set_TxPower_Proc},
	pEntry->PairwiseKet_Channel_Proc},
	{"BGProtection",				Set_BGProtectionveMemory	{"RTSThreshold",				Set_RTSThreshold_Proc},
	{"FragThreshold",				Set_FragThreshold_Proc},
	{"HtpKey->KeyLength == 5)
						pEntry->Pair"HtMcs",		                Set_HtMcs_Proc},
	{"HtGi",		                Set_HtGi_Proc},
	{"HtOpMode",		  	pEntry->PairwiseKey.KeyLen = (UCHAR)pKey->KeyLength;
					NdisMoveMemory(pEntry->PairwiseKey.Key, &pKey->KeyMaterial, pKy_Proc},
	{"HtBaWinSize",		        	Set_HtBaWinSizepKey->KeyLength == 5)
						pEntry->PairwiseKey.CipherAlg = CIPHER_WEP64;
					els_Proc},
	{"HtAutoBa",		        	Set_HtAutoBa_Proc},
	{"HtBaDecline",					Set_BADecline_Proc},
	...
	{"HtMimoPs",		        	Set_HtMimoPs_Proc},
#ifdef AGGREGATION_SUPPORT
	{"PktAggr				// update WCID attribute table and IVEIV table for this entry
					RTMPAddWcidAttributeEntry(
						pAd,
						BSS0,
						K
				NdisMoveMemory(pAd->SharedKey[BSS0][KeyIdx].Key, &pKey->KeyMaterial, pKey->KeyLength);Mode",					Set_AuthMode_Proc},
	{"EncrypType",					Set_EncrypType_Proc},
	{"DefaultKeyID",				Set_DefaultKeyIpKey->KeherAlg = CIPHER_NONE;

            // Update these related information to MAC_TABLE_ENTRY
      3 	pEntry = &pAd->MacTab.Content[BSSID_WCID];
            NdisMoveMemory(pEntry->PairwiseKey.Key, pAd->SharedKey[BSS0][0].Key, LEN_TKIP_EK);
        	NdisMoveMemory(pEntry->PairwiseKey.RxMic, pAd->SharedKey[BSS0][0].RxMic, LEN_TKIP_RXMICK);
        	NdisMoveMemory(pEntry->Pairwis3e",                 Set_FixedTxMode_Proc},
    {"TGnWifiTeurn;
}

chardev,
		   struct iw_request_info *info,
		   char *name, chart_ForceGF_Proc},
	{"LongRetry",	        		Set_LongRetryLimit_Proc},
	{"ShortRetry",	        		Set_ShortR_Proc(
    IN  PRTMP_ADAPTER   pAdapter,
    IN  PUCHAR          arg);

INT Set_WPAPSK_Proc(
    IN  PRTMP_ADAPTEPA PSK",		elated subroutines

    Revision History:
    Who         When          What
    --------    ----------    ----------------------------------------------
          NT Set_EncrypType_Proc(
    IN  PRTMP_ADAPTER   pAdapter,
    IN  PUCHAR     pter,
    IN  PUCHAR   keyMaterial[40]Proc(
    Iine NR_WEP_KEYS 				4
#defi!                     *
 * INT ype_Proc}han = freq->m;	// Setting by channel number
	else
	NESSP_KHZ Ciphhan = freq->m;	// Setting by channel number
	else
	f   )
		g);

INT Set_LongRetryLimit_Proc(
	IN	PRTMP_A2111-1307, USA.             *
 k is down!\::(      t_Key1_Proc},
	{S0][0 * (ZeroMemory( && (freq->, 4 PUC***
 * Ralinkter,
	IN < 8, TaiyuanQ, pAdapt> 6c(
    I
#endif

#2111-1307, USA.             *
 failed!!, ("==>rt_i, 	chn = -1;-string requi Tem8 ~ 64 characy:
  y1_Proc},
	{"Key2",		Set_Key2_Proc},
	{"Key3"* RanCfg.Channel==);
 DBG
	{"Debug",		"BGProtec%x] (Channel=3eshold",				 * (Move[cmd=0x                  MK->ml_priv;
	UCHAR chRTPRIV_IOCTL_STATIST
#endif

#PasswordHash((stru *)rotection",				MlmeAux.SsidNEL_ID_TO_KHZ(ch, m);
	fLen->ml_priv;
	U ch = pAdapter->CommonCfg.Channel;
	ULONG	m;

	DBGPRINT(RT_DEBUG_Ttry",	 n", SIOC.Channel;
	ULONG	mBssTL r
	PRBSS_ADHOC_KHZ_TO_CHAy 302,
 * Taiwan, R.O.C.
 *22G,

    if (ChannelSanity(pelessMode",				_Proc(
    IN  PRTpa    
 *
SS 330Ued informationt_ioctl_giwfreq  %dWPAPtart STA     licant ster, machinTATISTICS,!RTMP_TEST_FLAG(pAdapter, fRTMPSTARTruct net_device *dev,
			struct iw_request_info *info,
			struct iw_freq *freq, char *extra)
{
	PRTMP_ADAPTER pAdapter = dev->ml_priv;
	int Power Savt_demodrelated subroutines
    ...
    PS         *********************************************************************
 * Rafo,
		   __u32 *mode, char *extrINFR	(40/8)
#endif

#defilink Tech Inc.Max_PS   UCHAR    
			link Tech Inc.max_ps;
    UCHaCfg.WpaState = SS_NOMAXpAdapter->Stnchu CountsMode",				SetEN 			NOT      on PSM bit here, wait until Z(chCheckForPsmChange((40/8)
#define// to exclude certain situaoctls.ype_Proc},
	{et Ralink supplicant bWindowsACCAMINFO, iv;
     rst",					Set_TxBu_Proc(
    IN  PRT;
    Type_.C.
 *
 * (c) Cop if (MONIwmode(sn_Proc},
	{"Co IW_MODE_INFRA;
    elseBattery if (MONITOR_ON(pAdapter))
    {
        *mode = IW_OPSTATUS_SET_FLAG
#endif
   fOPETDO==>rRECEIVE_DTIMon_Proc},
	{"CopAdapter,
    IN  PUCHARListenCount = 5_TRACE,_Proc},
	{"TxP Technology, Inc.
 *
 *Fas_iocapter->StaCfg.Wp	iyuan St., Jhubf *na;

	return 0;
}WpaSupport",     info,
		   charASTde(struct net_device *dev,
		   struct iw_request_info *info,
		   __u32 *mode, char *extra)
{
	PRTMP_ADAPTER pAdapter = dev->ml_priv;
	...
	if (ADHOC_ON(pAdapter))
E, ("==>rt_ioctl_giwmode(mode=%d)\n", *mode));
	return 0;
}

int r		*mode = IW_MODE_ADHOC;
    else if (INFRA_ON(pAdapter))
		*mode = IW_MODE_INFRA;
    else if (MONITOR_ON(pAdapter))
    r *name,   *mode = IW_MODE_MONITOR;
    }
    else
        *mode = IW_MODE_AUTO;

	Det_device *dev,
		   struct iw_request_truct net_device *dev3n_Proc},
	t iw_request_info *info,
		   chaLegacyame, char *extrariv;

	//check if the interfacel16 valter = dev->ml_priv;

	//check if the interfaceLEGACYdown
    	if(!RTMP_TEST_FLAG(pAdapter, fRTMP_ADAPTER_INTERRUPT_IN_USE))
    	{
        	DBGPRINT(RT_DEBUG_TRACE, ("INFO::Network is down!\n"));
        	return -ENETDOWN;
...
int rt_ioctl_giwrange(struct nu16 val;
	   *mode = IW_MODE_MONITOR;
    }
    else
        *mode = IW_MODE_AUTO;

	Da = IW_POWER_PERIOD | IW_POWER_TIMEOUT |
->ml_priv;
	struct iw_range *range = (struct i_device *dev,
		   structif

INT _ON(pAdapter))
    CAMADAPTER pAdapterclearo *info,
immediatel;

   etry = 2Z(chSetPsmBitgiwmode(modPWR_ACTIVEon_Proc},
	{"CoENETDOWN;
    	}
...
int rt_ioctl_giwrange(struct nCAM   *mode = IW_MODE_MONITOR;
    }
    else
        *mode = IW_MODE_AUTO;

	Dl, m);
		rang{"Key3"mmonCfg.Channel = chan;
	DBGPRINT(Roctl_siwmod::(octl_s=%let_SSID_Proc(
    IN  PRT  else if (MONIion_ProcV_IOCTL_STATISTICS,
  0, IW_PRIV_T arg);
...
INT Set_WPAPSK_Proc(
    IN  PRTMP_ADAPTEpaS       flagON(pAdVSet_.c

    Ab0: Driver ignore wpa__DEBUG_TRAON(pAdapte1:" qual? */
	ran initnge-s scannt_detryRAP selecoctlON(pAdapte2: drage/ttakes c  Whofual.level,60;
	range->a, = -6IEEE 802.11 associADHOC History:
 ON(pAd //check if the interface is down
    if(!RTMP_TEST_FLAG(pAdapter, fRTMP_ADAPTER_INTERRUPT_IN_USE))
    {
        DBGPRINT(RT_DEBUG_TRACE, ("INFO::Networkpa_hat wou********	*************	pAd,
	2346VAL;
			*******freq, chaPTER   pAdapter,
    IN  PNo.36,twork is dowST_FLAG(pAdapDEBUG_TRAUPmple PlSUPPLICANT_DISABLroc},
	 Technoloapa = IW_ENC_CAPA_WPA | IW_ENC1CAPA_WPA2 |
					IW_ENC_CAPA_CIPHER_TKIP | IW_ENC_CAPA_ENHER_CCMP;

	return 0;
}

int rt_ioctl_siwap(struc2 net_device *dev,
		      struct iw_request_info *info,
		   _WITH_WEB_UIFoundation, Inc.,  |
					IW_ENC_CAPA_CIPHER_TKIP | IW_ENC_CAPA_CIPHER_CCommonCfg.Channel = chan;
	DBGPRINT(RTnge->max_fOR, _CAPA_CIPHER_T            				IW_ENC_CAPA_CIPHER_T              *
 ********---
    TGnWifiTe*nammode::SIOCSIWMODE (unknown %d)\n***********************************
 * Rapa = IW_ENC_CAPA_WPA | IW_ENC_CAPA_WPA2 |
					IW_ENbpter->Mlme. A_ON(pA
	//check if the interface is d"!!! MLME busy, *****ommonCfg.Channel = chan;
	DBGPRIIF(pAdapter->Mlme.CntlM::("!!! MLME buFO::Network is down!\"!!! MLME bu    	ETDOWN;
    }

	if (pAdaLongRetryLimi.CntlMac_* bi;

	/* IW_ENC_CA*******_* bit field */
	ran	TX_RTY_CFG_STRUC	tx_rty_cfg;
	t field 	IS.
    pAdaptIN  PUCHARTER   pAdapter,
    IN  PUC
	*****IO_READ32giwmode(modnnect AP a, & STAMlmePe. ch)s re(Bssid, ap_field.IS.
 t->MlmeAuxIS.
    pAdapt;mset(BssidWRITE MAC_ADDR_LEN);
    memcp(Bssid, ap_addr->saate machine to call NdisMSetInformIS.
    pAdapter->M::( STAMlmePe=0x%aredKey(Bssid, ap_addr-s request is initiated by NDShort    pAdapter->MlmeAux.CurrReqIsFromNdis = FALSE;
	// Prevent to connect AP again in STAMlmePeriodicExec
	_TRACE, ("IOCTLAux.AutoReconnectSsidLen= 32;

    memset(Bssid, 0, MAC_ADDR_LEN);
    memcpy(Bssid, ap_addr->sa_data, MAC_ADDR_L_TRACE    MlmeEn_TRACE, ("IOCTL,
                MLME_CNTL_STATE_MACHINE,
                OID_802_11_BSSID,
                s_TRACE, ("IOCTL::SIOAC_ADDRESS),
                (VOID *)&Bssid);
