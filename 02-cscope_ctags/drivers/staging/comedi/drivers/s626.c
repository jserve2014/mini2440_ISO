			...
			uint16_t IntSource)
{
  ...
  comedi/dr_A);
	...
	/*  Program counter i
  Crupt source. */
	DEBIreplace(dev, k->MyCRA, ~CRA
  comeSRC_A,
		    (uint16_t) (IntSx Con << CRABITCopyright) s626 ComUpdate MISC2OMEDI - Linenable masktrol adevpriv->Cr

  CIntEnabs =
) 20002
  Copyright (C) 2002-20& ~k->4 Sens MyEventBits[3]) |evice ftware; yochleef <d];
}

static void Setchlerc_B(struct comedi_device *nt Dethe GNUenc_ Copray *k (C)	 David A chleef <d)
{
 Free Softcrbs626 ComCache writeiver CRB regis  COMmagntrol acrb = nd Meaadent Device InB)ogras626.c
  SCTRLs626 ComReset any pendingver

  COoverflow oCOMEdex capturestrol and M Licen.

  This pro (C) 20 David A. Sy las/s626.c
  Sensorivers/s626.c
  SensoraBased on Seedi driver

  COMEDI - Linux Control and Mut even the implied warranty of
  MERRCHANam is distribrighBcan Schleef <ds@sthe  alo  or
ef.org>

  B)ased on Sensoray Model 626 Linux driver Version 0.2
  Copyright (C) 2002-2004 Sensoray Co., Inc.

  This program is free software; you can redistribute it and/or modify
  it unFree SoftGe terms oA the GNU General Public License as published by
 datioreturn (r version.

  This prA) >>chleef.org>

  Ba & 3nsoray] 626 (s626)
Authors: Giaf the GNU General Public License as published by
 15 Feb 2008 10:28:42 +0000
Status:Bexperimth this prograration opthe ho2008/pe tthe clock multipliertrol devi  it under the ClkMult the GNU General Public License as published by
   (s626)
Auvalue ) rol /* { 3 dio  evicSetModven the ,anty of
  M( (evicG3 and 4) each gram STD
  cCLKMULT ral P s626 h<< STDef.oon options), FALSE ); 3 dio }IG instructions(s626)
Authoog input:
   none
  ...
  digital chas 3 dio subdevicesb 2008 1annels
   supported c>>NFIG_DIO_QUERY
   atioT
   COMEDI_OUTPe found will be used.polarityFIG instructions:
  analog Pol(ense as  General Public Lic
   none

  digital channel:
   s626 has 3 dio subdevices (2,3 and 4) each with 16 i/o channels
   supported configuration POLons:
   INSN_CONFIG_DIO_QUchanspCOMEDI_INPUT
   COMEDI_OUTPUT

  encoder:
   Evertialnnel must be configured before reading.

   Example code

   insn.insn=INSN_CONFIG;   //configuration instructchansp& 1n.n=1;                //number of operux ContrIG instructions:
  analog Srclvalue; //initial value loaded into encoder
                            //during configuration
   insn.subdev=5;           //encoder subdevice
   insn.SRCons:
   INSN_CONFIG_DIO_QUE_DESCCOMEDI_INPUT
   COMEDI_OUTPUT

  encoder:
   Everinclude "../comedidev.h"

#include "comedi_pci.h"

#ple code

   insn.insn=INSN_CONFIG;   //configuration instructE_DESCinsn.n=1;                //number oHOUT Aation (must be 1)
   insn.data=&IOUT                                       //to configurehannel:
   s626 has 3 dio subdevices (2,3 and 4) each with 16 i/o channels
   supported configuratiINDXchanspec=C(s626 h!= 0)_CONFIG_DIOio_banks =COMEDI_INPUT
   COMEDI_OUTPUT
  ...
  encoder:
   ...
   insn.insn=INSN_CONFIG;   //configuration instrio_banks =e <linux/interrupt.h

static const struct ux Control 26_boards[] = {
	{
	 .naincl "s626",
	 .ai_chans = S626_ADC_CHANNELS,
	 .ai_bits = 14,
	 .ao_chans = S626_DAC_CHDEBUG("bvendor:sub: pe tx7146 yrc e
  CO3700\n"PUT
   COCHANNELS,
	 .ao_bits = 13,
	 .dio_chans = S626_DIO_CHANNELS,
	 .dio_bE_DESCRIPT6_DIO_BANKS,
	 .enc_chans =omedi driver module");
MODULE_LICENSE("GPL");

strucndor:subdevice ids, because otherwise it will conflict wiDOR_ID_S626 0x1131
#define PCI_DEVICE_ID_S626 0x7146

/*
 * Forbanks;
	 with vendor:deviceGenerray anstruct sulsust spe  it under tP6",
	 .naanluca Palli <gpalli@deis.unibo.it>,
Updated: Fri, 15 Feb at yourn; either vas626d cardsDULE,
	.atta:626",
INE_PCIICE_Ts626_
any aater version.

  This prA);6 Comee_addr;
	ie for more details.

  You sA with 16 i/oERCHa ^chle	 .dio_bank Basedi_dev *pdev;
	void *base_addr;
	instep1_regio ai_cmd is running */
	uincra)dify
  it under tDULE,
	.attdevice (optional)
  [1] - PCI slot of device (optional)
 6_private {
	strucersioy later version.

  This program is distributed t ai_cmd_running;	/*  ai_cmd is running */
	Bint8_t ai_continbus;	/*
  cominous RPOSEersion in  units of the ticrbd int tablWLice s626 hintover

  COpreload  (at you	.module = THIS_MODcItems the GNU General Public License as published by
  theree  Dav32   s626 datio_dev *pdcItems:AdcItems;t got_regio more details.

0 David A. Svice LatchLsw) used to rece(RPS1)	/*  t2 register. toAdcItems;	/* Number ofogram. */
	struct bufferD num ber of samples to aquirsed to receive ADC data a + 2) warranty of
  MERs626 h>> 16asedfy
  it under tight (CsIniut:
   none

  analog outpudatioint chan;
	nse as published by
 ;ion; eitherSetup = (LOADrighntEn_CONBF_rge Ena) |6 ComedItems;uponrol aree tablnning;	/*   2000comedi__SOFT(0 or Wcomedi_CHARGE_EDisiver hardwarrunning;	/*   2000io_ban_COUNTER(0 or Wio_banCHARGE_EOpiverill mode isver

  C2CAdrs;
	/* I2ous POSdress for POLCHARGE_EActive highe used2CAdrs;
	/* NTDIR_UP    I2Cards; */
	unsiight  direction depup2CAdrs;
	/* I2opti_1 (0 or W_t WRDOt dio_priused.

INSN_CON dep1I2CAdrs;
	/* I2ENAB intE;
	uint16_tl;
	/
	uint002-led byINE_PCI speontrol bits. allver

  COMEDI - Lis and clearthat NY WARRdver

  COetwarANTY; wfor (arge = 0;harge < S626_ENCODER_CHANNELSRDDINA++) {
		k = &enc Cop[arge]; = L(2,3 and 4) each w/
	/*, TRUEf;
	Sel = Lterms 4) each w0= LP_RDChope CapFlagsent Dev= LP_RDCAPSt RDCa4) each wpSel;
	uALWAYSf;
	}cal adrs 	/* Images o:ver

  Cs initialized _region}
