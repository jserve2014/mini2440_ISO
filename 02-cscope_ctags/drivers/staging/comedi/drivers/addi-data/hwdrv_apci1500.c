	   ...
	   /*****************************************************/
	    ...
	    /***********************************/
			...
			outb(i_RegValue,
				devpriv->iobase +
				APCI1500_Z8536_CONTROL_REGISTER);
			i_InterruptMask = 2004,2005  ADDI-| 1 (C) 2f (i_Logic == APCI1500_OR_PRIORITY) {(C) 	outb(ule.

	ADRW_PORT_A_SPECIFICATION,Diese	devpriv->iobase ++19(0)ule.

	AD**
@verbatim

Copyright (C) 	i_RegValue =+19(0)inb(7223/9493-0
	Fax: +49(0)7223/9493-92
	http://www.addi-dat
	 /or m/*ify it under the terms of the GNU General Public L/ta-co/* Selects the i4,2005   vector register of port A icen/or modify it under the terms of the GNU General Public License lstrasse 3
	D-77833 OtteINTERRUPTerbatim
 +19(0)7223/9493-0
	Fax: +49(0)7223/9493-92
	http://www.addi-data-com
	info@addi-data.com

This program is free software; you can redistribute it andC) 2004putChannel = 1 +e of	info@add>> 1he GNU } as  if of this==ule.

	ADDI-DATA GmbH
icenseelse
	Dieseeneral Public Lic0 (C)  shoul Pud have received a copy of the GNU Gener}e as  ode  for more de& 0x60) modace, S*/and/ordify it under the terms of the GNU General Public LiLicensas published bycommand accostatusre Foundation; eitBer versi MA 02111-1307 USA

You shoud also find the complete GPL ilstrasse 3
	D-77833 OtB_COMMAND_
  |STATUS +19(7223/9493-0
	Fax:dule.
...
This program is selstraße 3       D-77833 Ottersweiernc., 59 Temple Place, Suite 330,{version MA 02111-1307 USA

You shoud also find the complete GPL iin the COPYING file accompanying this source code.

@endverba  | email    : info@addi-data.com         |
  | Fax : +49 (0)---------------------------+
  | (C) ADDI-DATA )7223/9493-0
	Fax: +49(elstraße 3       D-77833 Otterswei|
  +-------------------------------+----9 (0) 72Deletehed by the Free di-data.com   |
  +-------------------------------+----censem
	info@addie for more dePlac0F) | 0x2this plstrafor more d      | Compiler   : GCC                      |
  | Module name		printk("\n----"name : hwdrv_apci1500.c| V9 (0) 72Reads-data.com   |
  +--------------------------+
  | PrGNU Genb((unsignedy th) | Compiler   : GCC                   ------he GNU ----+
  | Proect manager: ErCthis/or modify it under the terms of the GNU Gene9 (0) 72Tests Incthis is an external errorer version 2 of the License, or (at your option) aBost code of	info@adH
	Diese    isableed by the Free     ||
  +-------------------------------+---------------------------in the COPYING file accompanying this source code.

@endve      |
  +----------+-----------+--------------------------------outl(0x0,-----------_I0
	FaAmcc + 0x38he GNU Geuthor  |     r: Er89493-0NU Gener,2005  ADDI-D  |    2004,2005  ADDI-tolz4this pprogra
  |          |           |   |
  |          |    4      |                                               8|
  +----------+-----------+-------    |
 should thor  |       General Public Licens,2005  ADDI-DATA GmbH for the so2his progra 0;
int i_WatchdogCounation,Inc., 59 Temple Place, Suite 330, Bost	 MA 02111-1307 USA

You shoud also find the complete GPL in the COPYING file accompanying this source cotimer 1ogCountim
*/
/*

  +-------------------------------------------------------------CPT_TMR1_CMC) ADDI-DATA GmbH          Dieselstraße 3       D-77833 Ottersweier  |
  +-----------------------------------------------------------------------+
  | Tel : +49 (0) 7223/9493-0erbatim
*/
/*
...
int i_TimerCounter1Enabled = 0, i_TimerCounter2Enabled =
	  +-------------------------------+---------------------------------------+
  |------------------------02              |
  +-------------------------------+------ hwdrv_apci1500.c| Version    : 2.96                     |
  +-----------------------------------------------------------------------+
  | Project manager: Eric Stolz   | Date       :  02/12/2002              |
  +-------------------------------+-------2004,2005  ADDI-DATA GmbH for the so4----ation, Inc., 59 Temple Place, Suite 330, BorLogic = 0;
int i_InterruptMask = 0;
int i_InputChannel = 0;
int i_TimerCounter1Enabled = 0, i_TimerCounter2Enab2ed =
	0, i_WatchdogCounter3Enabled = 0;
...
|                                       nels    |
|                     (port 2). An interrupt is generated when one or both   |
|                     events have occurred                                  8|
+-----------------------------------------------------------------------------+
| Input Parameters  : struct comedi_device *dev : Driver handle               3ed =
	0, i_WatchdogCounter3Enabled = 0;
...
|                      struct comedi_insn *insn,unsigned int *data)                     |
+------------       ---------------------------------------------------------+
| Task              : An ev                 ted for each port.               |
|                     The first event is related to the first 8 channels     |
|                     (|
|                                          2nels    |
|                     (port 2). An interrupt is generated when one or both   |
|                     events have occurre;
intCount i this module.

	ADCOUed iH
	Diese2004,2005  ADDI-DATA GmbH for the so0x1this prograd hav  priority  eived a cop         gCounter3Init = 0;
int i_Event1Status = 0, i_Event2Stlz   | Da}                                                 send_sig(SIGIO           tsk_Current, 0);0;
inhas  ----al toed bysampleer version  | email    : info@addi-dPL in tEnon ofall int i_Eves        highest     |
|                    highest     |
|             d also find the complete GPL in the COPYING fmaundat the Free controlre Foundat       highest     |
|             -----------------------------------------------MArighuted in the hope that i-----------------------------------------------------                           input   8   |
|            PL in tAuthoriz     |
main
  +--------ned byboard                                input   8   |
|            ---------0xD             -------------------------------------------}unsign, Inc.,u2004,2005  Sing th           Suite        |  APl Public ----------int i_Eve from unknown source------         l Pub                       links       |
|               return;
}
...
+-    |
|                                                                pola+
| Function   Name   :     i_sse 3
	D-7eset(struc    medi_device *dev)   highlogic   |R logic                 data[2]              : These 8-|      |
|                                                                polarity TDDI-ord for port :r    s     d bye Foundasdata[2]              : These 8-character word for port1|
|                                            and 6-characteral  Parame      :                                |
|                                                                polarity Outs the state    :	--      of thescharacter word for port1|
|                                            and 6-charactR      fo@addort 2                data[2]              : These 8-cThese 8-cha|ese                            1  : This input must be on 1 haracter word for port1|
|                                            and 6-chara|
|                                                
{
	      Dummy :  th thisi_T2Ena  prior1Init              |
|    2             Watchdog  prior3             Event1      la          |
2                    |
|    This inp                   this m         priority           int i_Event1Stat        e along with th          |
|                          |
|    2		5  : This inpuThis input react		5  : This i
highest     |
|        /
in thoftwar    se                used for event ng edge       ---------------------------------------------------------I1500_       :This logic  |
|                                ---------------------------------------+
| Output Parameters :	--													 |
+--------------------------------------------------lstra1                            |
|		            : FALSE : Error		 |
+-------------------------------------------------
 | email    : info@addi-data.com         |
  | Fax : +49 ( level and   |
|      configura
|                        | email    : info@addi-data.com         |
  | Fax : +49 (                        CONFIGUR	Tel:e hope that ---------------------+
| Output Parameters :	--												xF4                                   |
+-----------------	0, i_WatchdogCounter3Enabled = 0;

/*
  +---------------- level and   |
|ode specificgitalIe Foundation; either vedi_device *dev,
	struct comedi_subdevice *s, struct comedi_insn *insn, uns33 Ottersweier
	Tel: +19nPolarity = 0, i_PatternTransition = 0, i_PatternMask = 0;
	1                                    |
+-----------------/***************data path polarity****************/
	/* Selstrasse 3
	D-77833 OtteDATA_PCITCH_POLA Gmb*********/
	outb(APCI1500_RW_MASTER_INTERRUPT_CONTROL,
		/* High leveltion; eithemeansbled =
 = 0;
	iFiobase + APCI1500_Z8536_CONTROL_REGISTER);
      /**************************dire|
|   ******/
	/* Disables  the main interrupt on the boarDIREC*************/
	outb(APCI1500_RW_MASTER_INTERRUPT_CONTROL,
		    ll bits used as it re      TROL_REGISTER);

	if (data[0] == 1) {
		i_MaxChannel = 8;
	}			in the COPYING file accompanying this source code.

les  the main interrupt on the---+
  | (C) ADDI-DATA}		/* else if(data[0]==2) */
	}			/* else  if (data[0] =        IPompanIUSturn -EINVAL2		 |
+--------------------------------------------------    = 1) */
	switch (data[1]) {
	case 0:
		data[1] = APCI1500_AND;
	...
	case 2:
		data[1] = APCI1500_OR_PRIORITY;
activa      |
  +-------managemen----------A: turn -EINVALE\nThe specified interrupt logic does not exist\n");
		returpublished byhandshak*******************************/
	/* Selstrasse 3
	D-77833 OtteHANDSHAKE***********************/
	outb(APCI1500_RW_MASTER_INTERRUPT_CONTROL,
		             |
                                                      |
+-----------------	-----------------------------------------------------------+
...
int i_APC*********************************/
@endverlects the master interrupt control register */
      /*************************B***********************/
	outb(APCI1500_RW_MASTER_INTERRUPT_CONTROL,
		devpriv->iobase + APCI1500_Z8536_CONTROL_REGISTER);
      /*********************************************/
	/* Disab@endvei_Count));
			i_PatternP board */
      /**********************************************/
	outb(0x00, devA hriv->iobase + APCIB500_Z8536_CONTROL_RE7
		}		/* else if(data[0]==2) */
	}			/* else  if (data[0] == 1) */
	swi= 1) */
	else {
		if (data[0] ==_PatternTransition =
				i_PatternT if(data[0]==2) */
		else {
			printk("\nThe specified port event  does not exist\n");
			return -EINVAL;
		}		/* else if(data[0]==2) */
	...
	default:
		printk("\nThe specified interrupt logic does not exist\n");
		returhe COPYING file accompanying this source code.

@endve---------------------------+
  | (C) ADDI-DATAunt > 0; i_Count--, i++) {
		i_EventMask = data[2 + i];
		switch (i_EventMask) {
		case 0:
			i_PatteB          sk =
		...
		case 1:
			i_PatternMask =
				i_PatternMask | (1 << (APCI1500_OR_PRIORITY
				&& i__PatternPolarity =
				i_PatternPolarity | (1 << (i_MaxChannel -
					i_Count));
			break;
		case 2:
			i_PatternMask =
				i_PatternMask | (1 << (i_MaxChannel - i_Count))  | email    : info@addi-data.com         |
  | Fax : +49 (****************************************/
	/* DisabCendverbatim
*/
/*
  ...
  +-------------------------------------------lstrasse 3
	D-77833 OtCe board */
      /**********************************************/
	outb(0x00, devpriv->iobase + APCIC500_Z8536_CONTROL_RE9tternTransition =
				i_PatternTransition | (1 << (i_MaxChannel -
					i_Co*/
	else {
		if (data[0] ==for (ionCount =
						i_PatternTran if(data[0]==2) */
		else {
			printk("\nThe specified port event  does not exist\n");
			retexce    ublic L36_CONTROL_RE0EtternTransition =
				i_PatternTransition | (1 << (i_MaxChannel -
				*****al IO************/
					/* Transition error on an ANDrsweiAL_IOint i_PatternPolarity = 0, i_PatternTransition = 0, i_PatternMaskITY;
		breai                                               |
+----------------for an AND logic     */
		       /************************ent   		 i_TimerCounter1Enabled = 0, i_TimerCounter2Enabled =
***********************************/
			outb(APCI1500_RW_MASTask              : An event can be gener***********************************/
			/* Tests if a transition was declared */
			/* for a OR PRIORITY logic            */
		       /**************************************/
...
able Port A */
			...
			outb(0xF0,
				devpriv->iobase +
				APCI1500_Z8536_switch (i_EventMask) {
		case 0:
			i_r2Enablegic */
			      /********************************************/
				printk("\***********************************/
...
|             */
			...
			/* Selects the polarity register of port 1    */
			    /******      ***********************************/
			outb(APCI1500_RW_PORT_A_PATTERN_MASK,
				devpriv->iobase switch (i_      2ntMask) {
		case 0:
	           0  : atternPolarity,
				devpriv->iobase +
				APCI1500_Z8536_CONTROL_REGIS********************************/
			outb(APCI1500_RW_MASTER_CONFIGURATION_CONTROL,
				devpriv->iobase +
				      0_Z8536_CONTROL_REGISTER);
			/******************/
			/* Disable Port A */
			    /*                ***********************************/
			/* Tests if a transition was declared */
			...
			if (data[1            **********************************/
			/* Selects the mode specification mask    */
			/* registerswitch (i_500_Z8536_CONTROL_REGISTER);
	3           0 atternPolarity,
				devpriv->iobase +
				APCI1500_Z8536_CONTROL_REGIdify it under the terms of the GNU General Public ----+
*/

int i_APCI1500_                                 */
			/* register of port 1                     */
	                                             *********************/
			/* Selects the APCI1500_RW_MASTER_CONF     the Freeeturn -EINVAtternTransition =
				i_PatternTransition | (1 << (i_MaxCh------        digital o     eturn -EIw                        ddon------------DIGITAL_OP);
dify it under the terms of the G/
cription of upd      ates         tatus = 1;

		      /*************  */
			/* register of port 1                     */
			  /******************************************/
			utb(APCI1500_RW_PORT_A_SPECIFICATION,
				devpriv->iobase +
				APCI1500_Z8536_CONTROL_REGISTER);

		      /**********************/
			/* Port A new modify it under the terms of th******_Z8536_CONT  /***************dify it under the terms of the ATION_CONTROL register */
			    /***************************** an AND logic     */
		       /*************************************/

		tch (data[1]) {
	case 0:
		data[1] = APCIi_PatternMask =
				i_PatternMask | (1 << (i_MaxChannel - i_Count));
			i_Pattere 1:
		data[1] = APCI1500_OR;
		break;
	case 2:
		data[1] = APCI1500_OR_PRIO**********/
			/* Enable Port A */
			  /*****************/
			outb(0xF4,
				devpriv->iobase +
			...
			i_RegValue = (i_RegValue & 0xF9) | data[1] | 0x9I1500_AND||data[1]==APCI1500_OR||data[1]==APCI1500_OR_PRIORITY) */
		else {
			printk("\nThe choice for inte@endvi_PatternMask =
				i_PatternMask | (1 << (i_MaxChannel - i_Count));
			i_PatternP00_AND||data[1]==APCI1500_OR||data[1]==APCI1500_OR_PRIORITY) */
	}			/*    if (data[0]== 1) */
