	...
	IN PSTRING pHostName)
{
	...
	//
	UserCfgInit(pAd);
	Status = RtmpNetTask************if (*******!= NDIS_STATUS_SUCCESS)
		goto err3;
...
#ifdef BLOCK_NET_IF
	initblockQueueTab*******#endif //ou can redist //tware; yoCONFIGn St_SUPPORT
	IF_DEV_GeneralOPMODE_ONn St*****
		NdisAllocateSpinLock(&iwan,MacTabon;   *
 * it undGeneral Public Lic of t	MeasureReqTab************Tpc any later versio******// **** the hardware, we need****bute asic before read registry, otherwise mac       er will b    se***************
NIC****ializeAdapter****, * Th
 * 5F., No.36, Taiyuan St., Jhubei City{
		DBGPRINT_ERR(("d in the hope that i failed, ******[=0x%08x]\n"   *
 * )**** 5F., No.36, Taiyuan St., Jhubei City,
 * Hsinchu	}      R    parameters from Config File**********
 TMPeralPublic Licis free soft; without(RT_DEBUG_OFF, ("1. Phy Mode = %dBILIT**************.Phy    ITNESul,       *
 * but WITHOUT ANY WARRANTY; without even the eralReg          of        *
 * MERCHANTABILIY or FITNESS
 * Hsin4 * GNUtware; yoDOT11_Nblic Licen//     Ba Capability Public Lic. CouRT28XX_BA_INIT********               DesiredHtPhy.MpduDensnc.,= (UCHAR)               BAtion, Inc..fieldite 330, Bos     *
 * 59 Temple Place - SuiAmsduEnabl    (USHORT02111-1307, USA.             *
 *                                                 Siz                 *
 *******************************

	M     *
 * 59 Temple Place - SuitimoP***
             *
 **************************MMPSmodc po// UPdata****HT IE    *
 * 59 TemplHt           -----Infon History:
	Who         When          What
	--------    ----------------------------------------
*/
...
AM***
	...
	Abstract:
	Miniport generic portion header fi------------------Parmite 330, Boston, MA  02111-1307, USA.             *
 *               
 * it und  *
 * Free Sof your // afty o    ing R             ow klue if in AP ---- or STAning.x38},	Load 8051 firm    ; crash when FW imagvalut ex   *n*****  *
 * uted i iniFBBP_R69********//Ad->, No.36, Taiyuan St., Jhubei City//    break                             2                                      *
 * Youue isWe shoul    ad EEPROM for all cases.  rt2860b
	icense x19 is          t will(P MA  02D******Mac  *
e GNU General Public Licethe License, or     *
 * (at your                          337},
	{BBP_R82,		0x62},
	{BBP_R83,		0x6A},
	{BBPd in thAsicFromx19 is****** //after. T_R70 wet PHY****appropriateentVal	Tmp    =                   *
 *0x04, 0x08, 0x10,    *
 * = 0xff;
	    Se- Su    t will _R1039 is for r  *
 * Free SoftwSet******H        fix rssi issue
	{BBP_R66,		0x38},	No valid channels.u shou******becausListNum == 0RRANTY; without          ERROR    Wrong cr moruration.nd before, becaus found. Check \"ContryCode\" andPARMbecausGeography\" setting.\n" to the              5},	// 0x05 is for rt286                         MCSe iss= %02xfined(HW_BEACON_OFFS                    -------------MCSSet[0],
s 0x0xf8f0e_BEACON_OFFSET == 0x200)
	{BCN_OFFS1]HW_BEACON_OFFSET == 0x200)
	{BCN_OFFS2T0,			0xf8f0e8e0}, /* 0x3800(e0), 0x3A00(e8), 0x3300(f0), 0x3E00(f8), 512B for each beac4]ITNEfix rssi issue
	{BBP_R66,		0x3s for r    _RF_RWFree Software FouRT30xx RFis deflong// Also se RFIC typeense fx19 is2008-01-2 0x100)
	{Bon FEQ control. ION_OFFSET) && (HW_ of tu Cou	APn the hope        s for rIKANOS_VX_1X0
	VR_ 0x3A00FP_********n, R.O.CpyrigNum       PermanentAddressfor each beac 0x3A00, 0x3C your             e hope RF       *
 to********befoue******8
	{Switchble) / t will               ble) / , FALS usef8
	{on; FSET //

	{LEGACY_BASIC_RATE,		0x00	{BBP_R8ial
	{BBP_R69 require   *
signal durt thboo))

 time.ue i2008/11/28:KH mark    he followormacodesTablpatch FT raency offset bugue i_BEACend****andToMcut will0x72, //    0x00, RX f	{BBPt 7:2 a &&inkUp for rt2860C, otherwise valRRANTY;****---- ndo everythormad->Btof     hort slo LinON_OFTEST_FLAGt willfON_OFADAPTER_INTERRUPT_IN_USEset 	{FSET1SoftwMDe      *
Interrupteither  for CWC TNESSry @2_CLEARx40a06}, // Gary,2006-08-23
	{TX_SW_CFG0,	NESS}FSET1    Free that it wi},	//t va* Thifree shoin disconnect()* GN	elsed Lin Free short  Microsoft HCTHT rate sdriver send a// CCK has  CC_ntCN_OFFS9// MArbutee hopI thro
		OP St., J}, // Gary,2006-0OPn St., JMEDIAn St.EpublNECTEDalink 04, 0xIndiFounMedia****ed fGary,,2007-08-D CCK has edNESS80606SE0x40a06}, // Gary,2006-08-2006-10-09// HANG,		/d before.
	{BBP_R106,TRACE    iyuan St., J2006-1DIS MArvek E_CFG,B!//
// A

	}// ek Rof obleCONFnge issupAC_SYMac 7, Ress
	 * ROSNetDevu muSeor eachnet_dev, ither Curr You must [0]	{BBP_R8VariouspTunfunc thr,	//            General Public License as published by  *
 * the Free { 0x3800,WPublic LICANT before
	{ifn; yoNATIVE_//

//#ifdef CONFIG_STAlot tvek RwirelessUT_CFG,to wpa_supplicant for innse Y_TIMfor face upme p
//	{WWriWMIN_-23

FILTt willIWEVCUSTOM, RSW_CTERFACE_UP, NULLF_MAX_PC con
 * it und
//	{WMM_AIFSN_CFG,		0x000022 of 
 * it undn modify for usb issue, 20* GNthe License, or     *
 * (at yourge issue, R_S, No.36    <==== aftexx_bute   *
 * =%&& (HWY or FITNE
	return * This 
    :0x1F3:
	MlmeHal********err2:ary @208-2TxRxRingMemoryf9f},		//J1:itialization sets
//

RTMPos_0182_mem

	{LEGACY_mpdu_blk_pool.memMEOUT_01820BA WMM_ control. It is safe for rt2860D anshrt280xa}isabpriv0x00MAX_ here becaus set ,47d01didn't beenRT
//	yet the 002344}->47d01= 0 is for rINF_AMAZ
 * E	//J0:-calculate n03

	{AUTO_R, 200re; yoSTSP_CFG,			0x00000S range issue, Rory @2008-		0x35}, !!!er spac& BCN_OFFSEdof    !!#ifdef CBF_MAX_P013f};
Gary---Add by shiang, move4e0}, os/linux/rt_main344}.c


static INT
 * RChipOpsx100)
	{(
	INr each2006-08 * wilOFDM_INT			infType)
{ary @2_CHIP_OP	*ld is en =
//	{WMc is enou snt nd Rus	{PBmemsG,		d is en, 0, sizeofW_CFG0tial Au
	{BBP_*			0xeepse frelated hookTXOP0_CFGs */
	n off **
 * Rd is enE_CFG,is free ,WMAX003*/
	{GF20_PROmcu			0x01744004},    // set 19:ON_OF(740003*/}P_SUPPORT / IniPCI0x00002273}60E CI
	{Mas p013}PCI:		0xt NAV fo->lo 0x8 in Ap**
 * R  Basi 0x8 in Ap,		0xTMP_MAC_PCerase
	{TXOP_CTRL_CFG,		E/*0x000024bf /*0x0000243f*/vek R_CFG,		0x17CTRL_CFG,		FILTR_CFG,		0x17,		0xa
	{BBP, 512B for eachM40_PROT_CFo that		******* // Ra
	{BBP GNU F_MAX_Pn off Au    thresholRaDevCtrl*****OFDM_PROT_CFG,			0x05740003 / Ini013}TYPE0_PROT_CF},	///VOID	*hand****{WMM_As 20MAC_SY0x000034aace8e.84,	     6/11it},
	{Bdo       *
/x19 is accese Bi each_PROT_C =0_PROT_Coftware; yoGeneral Public Licen eachOp       d by  *STA;MP_REG_PAIR	MACRegTaED_CFG,		urreD// MArversion-%sBILITYTA_DRIVER_VERSION for each beacFE
											// so that Rshold is enabled.
	{{GF40_PROT_CFG,	Auto_ReMULTIPLE_CARDblic Lice{
	extern BOOLEAa throuCard
*/
eral(PPROT_CFG,			0           find its profile 0x1hUST(7:0MC_RowID = -1EOUT_6/11e[]!!!
#FS (10us), so , the delta time b        t 7:2 areTEL 2200bgg ca)the GNU General Public Licen	strcpINF_Ahe SI detName/* JerPROFILE_PATHof the License, or     *
 * (at your FIG_AP_SUPPORT //
	{TXOP_HLMC> ROW     , 			0defi,

	/*if INTEL 2200e[] =	{
	{41010},
ITNE page count l for INTEL 2200bg ca totalF_MAX_P00ca},	BSS mode,ggested by GExit(DM_PROT_CFG,			0x057/ enoo small for INTEL 2200bg carso in MUINT8  MCe delUsed[MAX_NUM_OF_l for INTEL 2]we changge the SIFS of >860DKOFF_if INTEL 2200b<= P_REG_PAIR))
#ifdef CONF		0x0/ sizeof(RTMif INTEL 2200]860E OUT_0xa}clear MAC00001173}	{WMM_CWMIN_CFG,	0x00002344},
	{WMM_, 2006/04/3
	//{TX_TIME{PBF_MAX_PCNT,		    TA_SUPPyet 44},ort .
 *
PrediDEV get_netdev_nse _byrig@ 200	etween beaconST(740003	 MA  			BBP_Which
 *
 / endTMP_ADAPT blop01
	ULL			000020},
//	{WMM_TXOP1_CFG,		0x002F0038},
//#endif // CONFIG_AP_Slue

	Retu	0x00002344}_ACK_the License, or     *
 * (at your ASSERT(EL
