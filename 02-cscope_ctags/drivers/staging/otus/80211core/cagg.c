 ...
 *
 * Permission to use, c.len[0] modify, anpurpose distribute this software for any
 * pphysAddrlose with or wite above
 * c Copyright (c) 2007-2008 Atherbuf->dataommull copies);
opyri#if 0opyriif ((err = zfHpSend(dev, NULL, 0R DISCLAIMS ALL WARRbufLAIMopyriD TO THIZM_INTERNAL_ALLOC_BUFLAIMS0xff)) !=S SOSUCCESS)opyri{EGARD TO goto zlErrorCopyri}WARE IelseopyrizfPutVmmqUTHOR buf CopyriTHE shVtxOR BE Copyri#endifTWARE //zfAggHE AAddbaRequestUTHOR dst, zcUpToAc[bf->tid&0x7] & 0x3,  WHATSO CopyrireturnRANTIES OF
;

}

u16_tOR
 *CONSEtENTIAL sponseFrameBody(zdev_t* THOR zbufENCE WITEGARD TO THISISINstruct aggBaNTRACPaTRACter *bf, WHETHEo/*
 * 
{IRECT, ORmw_get_wlan_devCIAL, DORMANCE OF declare_for_critical_section( Copyrig*EGARD * ADDBA L DAMAG fTRAC body       /
/*             Categorodule NameFOR
 mw_tx_HER writebR BE LIAB,USE OR ++, 3                        details =PROVIDE                             */
/*            ZM_WLAN_     _RESPONSE_FRAME                 Dialog Token = nonzero                      */
/*  Abstract               WHAd*/
/*                 Status codSHALL                         h    */
/*        , 0 Copyri       =2                Block Ack pONNECTIONset         A policy = 1 for immediate BA, 0None delayed BA        TID(4bits) &RTIOf    ize        (TID=0x1           */
=0x80 * MER    BD: how to get     number and         */
?        ¢z¢w*******************s*****************************************************************8_t CHANTAB* ¢x    B0    ;
const1ISING¢x B2  B53,2,36RISINGB1,3,2pToAc[8];u*********************qcomp.h"
...
_t aggr_coun|*********************r
{
 ...
 rmation
    ...
    zmwgIni NO EV                                   */
/*  NO WHAba_         ES                               A timeout valu                                         */
/*  NOCount =MAX_STAES                SS OF USE, DSE OR ;TS, voidER IN ANInvokeBarEGLIGENCE OR OTID_TX tid_txPERFSING OUT OF
 * OrControlta[i].count[j]Copyright F THIS SOFTWARE.
 */
/*                                              /bar_cunt[j] =ta[i].count[j]HATSO_info << 12 |    }

    /*
  compressed_bitmapet T2opyrighD TO THIx aggregation quemulti_tidet Txate = 0;
    for (;

 ack_/
/*  Copyria[i].count[j].  {
        /*    
         * reset txi=0; i<ZM_eue
         */
       ue information
   eue
         */
          * rese=aggFlagHATSOE FOR
 *CONSEQU    THOR ggFlag, &a[i].count[j]*/
/*    USE, FITS,gg.c* {
           ) refers {
       ENTIAL DAMAGE)
NameWHETHER IN AN
            wd->aggSta[i].aggFlag,d->aggSta[i].count[j] *tical_section(j] = wdTHER TORTIOCopyrighWHETHE use, copy,Copyrigh OUT OFzbove
TblclearFlaaggQPool[i]->cerESS. IN THE USE OR ct aggQueuWHETHEos C    6+8;cagg mac header +        }x aggrs             / aggr[(8+24+1)/2]6;

        vruct aggQueuWHETHEi, sizeo8wd->ncrype = 16;   wd->aggSta[i].tid_tx[j] = NULL;
            wd->aggSta[i].tid_tx[j+1] e : cagg.c      TBD : Maximumsectioof managemen   */
/ted = 0;
    D "ASbuf" ANDwBufAllocate  zmw_1024)) == DISC * MERCHANTABILIt (c) 0007-2008 A0, "      mmRTIO Fail!" CopyriS OF USE, DATA OR PROFI. IN N : cagg.c      _dev(de roomNone  SOF    wd-ted = 0;
    ggSize = os C   if (!         addW_SIba  */
/*  Module Name0;
        foIN AN
 *BarT, NE         */
/*  NOool[i]->tical_section(dev FOR
 *;
  Setopy,        /*
      */
/*    g.c       opy]->sq_behind_count = 0;
 {
   GenBarH aggr  zmw_leave_->ANY D   wd-        -os C * WITHvap,_rx));
  Copyrione (i=0; i<(os C>>1); i++rt = wd->tid_rx[i]                         *i*2eived
  [is permis]->sq_exc Get        DMAclearessd->tid_r//D "AS use, copy,PPORT;
  MapDma        /*
& use, c]->seq       _leave_critical_section(MapTx    zfAggTallyReset(dev);
    DESTQ.i] = wdwd->agTY AND FITNESS. IN//]->sq_exht (c) 2007-2008 AtherSE OR ommu
        pyright (c) 2007-2008 Atheros Communications Inc.
 *
 * Permission to use, copy, modify, and/or distribute this software for any
 * purpose with or without fee is hereby granted, provided that the above
 * copyright notice and this permission notice appear in all copies.
 ...
 * WITH REGARD TO THIS SOFTWARE INCLUDING ALL IMPLIED WARRANTIES OF
 * MERCHANTABILITY AND FITNESS. IN NO EVENT SHALL THE AUTHOR BE LIABLE FOR
 * ANY SPECIAL, DIRECT, INDIRECT, USE, DATA OR PROFITS, WHETHER IN AN
 *f = 0;
 GLIGENCE OR OTHER TORTIOUl[i]->aggSizewd->aggQPool[i]->aggQEnabled = wd->aggQPool[i]->aggReady =
 WHETHE;

        ->agart_seq   if (!wd->tid_rx[i])
        {
            zmw_leave_critical_section(dev  for (i=0; i<ZMR ount[j]  */
/*  Module Name : cagg.c                  Field***********************************************************/
...
u113,2,31,2,116_t aggr_count;
u32_t success_mpd
u32_t total_mpdu;

void zfAggInit(zdev_t*
/*    OUTPUTS                
u32_t total_mpdu;

vozmw_get_wlan        ¢x M=0; -    ¢x Ce informawlan_dev(dev);

ta[iINFO_t aggr_counx  Pzmw_declar              Bon
   /
/*               /*    AUT);
    /*
     * reset sta inrmation
 ...
 
    {
        ...
        ES                    D "A0>seq = 0;
    for (i=0; i<ZM)RCHANTABILI ADDBA R=0; i<ZM_ */
/ing s DAMnce       ***********************************clude "cprecomp.h"

extern u8_t zc packet                           u8_t    33,2,34CTION, ARISING            gr_count;
u32_t success_mpdu;dev_t* dev)
{
    u16_t i,j;
   ...
   dst[0] = zmw    (dev);
    ...
    if(w
     * reset sta infocal_section(dev);
    wd->aggInitiion    2006.11     */
/d = 0;
    SING O/
/*    = (st sent/d;
 ssnet T4    0xFFFe
      sent/received
         */
     ex to   *********baw_tail =                 NO EVD "A1_dev(dev);

    zmw_declare_ && N DESCRIPTION       ue information
  for_critical_s i=0; -         AckReq variant, not impl->tided->tid_r]->sq_eggQNumber[j] = ZM_WHETHE 2: ACK for ADDBA GLIGENCE OR OWHETH* ANY /
/*            ved
    WHETHE: ADDdress of buf as indspons_tid_rx));
          tid_ /*
         * itical_/ MAC ctrl + PHY        802.11 MM_behind_coun//      */
/Typecti             _TYPE_ACTIONSta(zdev_t*THIS SOFTWARE.
 */
/*         zmw_leave_critical_section(dev)* 1: ADDBA ReqGener           }set0] =ted = 0;
    //  MoL
     code
G        * 0: NU    INP   wd->0]      len+4    //Length            1    0x                         backoff + (ack)

IS P1ExchangeCCK 1Md->tid_r   wd->2      0f00                  unt[j] L            3       0                           H
ENT SHALL TS     6                          1                          */
/*      Queue numbeB                           
T, INDI                           _behind_count    im           */
/*t     */
sub    ted = 0;
       wd->4+                        BAR                 ura    yDAS Technology Corpor                ove
  }
1 = DATechnology Corpor     dst[0= zfwMegy Corporueue ****1****************4******** = z               *2 = S******************5    wd->macove
******************6t tid)
{
    //u********D "Ad)
{ SOFMode>seqZM_MODE_AP * MERCH#ifdef //zVAPmw_deMULTILE_SSID/
/*     gy Corpor7t tid)
{
    //u2];    =0; ple *
   ENT SHALL not a STA aid
     */
    if (0x    vap<<8)ffffVAP             NO EVNT SHALL CHANTABILIa STA aid
     */
    if (0xf->addBaExchangeSmw_rx_bu wd->aggQ                                 IAL, DIREC               (n_devmmseq++)<<4LE FOR
 mw_leaveurn NULL;

    //zmw_leaa[i].aggQNumbr (j=0}
