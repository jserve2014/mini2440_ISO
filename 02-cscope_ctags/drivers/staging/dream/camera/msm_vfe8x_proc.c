...
#include <ux/slab.h
		if (-2009 QUALCOMM Incorporaenc
*/
#inclu&&
		08-2009 enc
#in.multiFrag)h>
#-2009 QUAImask#include <PingpongIrqnux/spTRUck.h>
#include <linux/io.h>
#include 
#include <lit.h>
#include <linux/delay.h>
...
#ince.h>
#inclu"msm_vfe8x_proc.h"
#include <media/msm_ted.
*/
#inclut.h>
#includeted.inux/delay.h>
#include <linux/platform_ted.
ce.h>
#includeboth y and cbcr frame is ready. */

	/* bitera.h>

struct msm_vfe8x_c1:
	 * generate IRQ when both y and cbcr frame nerate IRQ when both y a	} /* VFE_AXI_OUTPUT_MODE_CAMIFToAXIViaOutput2 */
		breakk.h>caseifConfigLocal;
	strucpt_maskAndt vfe_interrupt_mas1: {008-2009 QUACamifMM Incorporacata 2BusEnable
	struct vffe_bus_cfg_data     vfeBusConfigLopt_mas;
	stru vfe_cmd_bus_pm_staLCOMM IncorporarawWrite
#inSelect  =h>
#fConRAWLocal;
	VIEWt (C)_PATH 2008-2009  <linux/p#incl	strdude <lina_lut_sel   vfe_cmd_bus_pm_stax/platform_devnclude mma_lut_sel  vfeGammaLutSel;

	boorqCompositeMean vfeStartAckCom vfename     vCO/*
* Copyright (C) 2008-2009 QUALCOMM Incorporade <linux/lis_sel  vfeGammaLutSel;

	boude <media/msm_camera.h>

strucl  vfeGammaLd_bus_pm_s:
	 * ge;
	struct vfe_gamma_lut_sel  vfeGammaLutSel;

	boolean vfeStvfeSAckPendingFlag;
	boolean vfeStopAckPending;
	boolean vfeResetAolean  ing;
	boolean vfeUpdateAckocal;ONLY 2008-2009 QUALCOMM Incorporated.
*/
#include <linux/slab.h>
#include <linux/interrupt.h>
#include <linux/spsm_vfe8x_proc.h"
#include <media/msm_cam<linux/list.h>
#include <linux/delay.h>
#include <linux/platform_device.h>
#include "msm_vfe8x_proc.h"
#include <media/msm_camera.h>
...
struct msm_vfe8x_ctrl {
	...
	struct vfe_module_enable vfeModuleEnableLocal;
	struct vfe_camif_cfg_data   vfeCamifConfigLocal;
	strucdata   vfeStatsCmdLocal;
	stru   vfeImaskLocal;
	struct vfe_stats_cmd_data  1vfeStatsCmdLocal;
	str2ct vfe_bus_cfg_data     vfeBusConfigLocal;
	struct vfe_cmd_bus_pm_start vfeBusPmConfigLocal;
	struct vfe_bus_cmd_data     vfeBusCmdLocal;
	enum vfe_interrupt_name     vfeInterruptENCLocal;
	uint32_t vfeLaBankSel;
	struct vfe_gamma_lut_sel  v vfeGammaLutSel;

	boolean vfeStartAckPendingFlag;
	boololean vfeStopAckPending;
	boolean vfeResetAckPending;
	bo	struct vfe_cmd_frame_skip_config vfeFrameSkip;
	uint32_t   axiOutputMode;
	einux/slab.h>
#include <linux/interrupOperationMode;

	uin	struct vfe_i      vfeSnapShotCount;
	uint32_t             vfeRequestedSnapShotCount;
	boolean             vfeStatprog_hw(uint8_t *hwag;
	uint32_t            vfeFrameId;

	struct vfe_cmd_frapyright (C) 2008-2009 QUALCOMM Incorporated.
*/
#include <lin_bus_cmd_data     vfeBusCmdLocal;
	t.h>
#include <linux/iod;
	...
	uint32_t vfeImaskPacked;
	uint32_t vfeImaskCompositePacked;
	enum VFE_RAW_PIXEL_DATA_SIZEde "msm_vfe8x_proc.h"
#include <media/msm_camera.h>

struct msm_vfe8x_ctrl {
	/* bit 1:0 ENC_IRQ_MASK = 0x11:
	 * generate IRQ when 
	struct vfe_irq_composite_mask_config vfeIrqC 1:0 VIEW_IRQ_MASK= 0x11:
	 * generate IRQ when both y and cbcr frame is ready. */
	s
	struct vfe_irq_composite_mask_config vfeIrqCompositeMaskLocal;
	struct vfe_module_enable vfeModuleEnableLocal;
	struct vfe_camif_cfg_reg);
	for ;

	struct msm_vfe_callback *reuct list_head tasklet_q;   vfeImaskLocal;
	struct vfLASTLocal;
	strucENUM:vfeImaskLo)
{
	/switch   v
	/* Save the addresses for each ;
	s.   vf/* ot_mask ;
	s   vffcnt = t vfeLaBankSelfragCount    pcircfe_but vfeLaBankSelyinux/urther Buffer;
	pdeso_lock, flags); */nextFrameAddrBuf(uintsrc = &(in->ock_irq.ock_irYctrl.h>
ments[0][0]);
	esti(i = 0; i < ->io; i++#inc*t32_t *++ = *_unl++spin_unlock_irqrestore(&ctrl->io_lock, flags);1*/
...
static void
vfe_set_*((spo_addr(struct vf32_t *)(ctrl->vfebase +ceratFE_IRQ_MASK);

	rc in_unlock_irqrestore(&ctrl->i
#in_lock, flags); */
...
static void
vfe_set_bus_pipo_addr(struct vfe_output_path_combo *vpathAddress = (uint8_tutput_path_combo *epath)
{
	vpath->yPath.hwRegPingAddress = (uint8_t *)
		(ctrl->vfebase Address = (uint8_tY_WR_PING_ADDR);
	vpath->yPath.hwRegPongAddress = (uint8_t *vfe_set_bus_pipo_urth(&      vfeSnapS, 	(ctrl- <linux)Local vfeLaBankSelackPendingnux/slab.h>>vfebase + VFE_urren*)temp = pinggPingAddress = (uwhichpt_mas_VIE = 1gPingAddress = (u VFE_I
	teIndex = 2gPingAddress = (uiBUS_VIEWhwRegPongAddress = (uint8_t *) VFE_IhwCnt8_t *laPathrl->vfebase + VFE_BUS
		(ctrl-ADDR);
}

static void pin_lock_ir1save(&ctrl)
		(ctrl->vfebavfeSnapSh VFE_IRQ_MASK);

	rc = *((struct vfevfeSnapShle *)temp);
	/* spl->io_lock, flvfeSnapSh
	temp = (uintunlock_irqrestore(1ctrl->io_lock, flags); */

	return rc;
}

static void
vfe_set_bus_pipo_addr(struct vfe_output_path_comb

	/* force it to burstutput_path_combo *epath)
{
	vpath->yPath.hwRegPingAddress = (uint8_t *)
		(ctrl->

	/* force it to burstY_WR_PING_ADDR);
	vpath->yPath.hwRegPongAddress = (uint8_t *)
		(ctrl->vfebavfeSnapSh_BUS_VIEW_Y_WR_PONG_ADDR);
	vpath->cbcrPath.h

	/* foAddress = (uint8_t *)
		(ctrl->vfebase + VFE_BUS_VIEW_CBCR_WR_PING_ADDR);
	vpath->cbcrPath.hwRegPonput2.outputY.imageHeight;
E_BUS_ENC_Y_WR_PING_ADDR  */
	cmd.out2YPingAddr = out2->yPath.addressBuffer[0];

	/* VFE__t *)
		(ctrl->vfebase + VFE_BUS_ENC_Y_WR_PING_ADDR);
	epath->yPath.hwRegPongAE_BUS_ENC_Y_WR_ath->cbcrPath.hwRegPingAddrENC_Y_WR_Int8_t *)
		(ctrl->vfebase +vfeSnapSh_ENC_CBCR_WR_PING_0wIncrement + (out rPath.hwRegPongAddress = (uiENC_Y_WR_IMAGE_SIZt = temp;

	/* AXI Output 2 Cbc_PONG_ADDR);
}

static void vfe_axiENC_Y_WR_IMAGE_SIZe_cmd_axi_output_config *incall to programeed fregisters/* spiddreaxi_ock_ir(in + VFE_BU>vfebase + VFE_BUS_ENC_Y, axiock_w);
}

void ddrenfigL_cM Inc(struct_Cbcr_mdr_WR_IMAGE_SI *in)
{
	E */
	cmd.ouata cfg cmd;
	memset(&cmd, 0, sizeof(cmd)_WR_PCDBG("putCburatme pixelsPerLin	(ct%d\n", rqreidth . (out - 1)) /	rettCbcr.imageWidth +line - 1*)
		(ctout;
	cmd.out2Cbc_BUS_ENC_Cbcr64bit = temp;

	window first (out_WR_BUFFER_CFGtLengt.;
	cmd.outrstLength = burstLengthlacmd.out2CbcrNumRoows = in->oump = (in-rstLength = burstLength;
	cm_BUS>output2.outputCbcr.ouut;
	cmd.utCbcr.outRowCount;
	temp =cmd.out2CbcrRowutputCbcr.outRow = temppin_ldetermmd.oif epoch interrupt needsS_ENbe eruct v.    vf#incirqreout1Y1.addres =od;

	)_IRQ_MA
	/* VFE_BUS_BUSiPongA<ame  R_CFG */
	cmd.out2CbcrBur#incSel;

	boolean vfeStputCbEFE_BUhen bo1_PIN0];

	/* VFE_B2S_VIEW_Y_WR_PONG_ADDR */
	cmd.out2YPongAddr = out1->yPath.addressBuffer[1];

t vfe_bus_cfg_VIEW_Y_WR_IMAGE_SIZE 2/
	cmd.ou	}PING_A s, need fcontentS_ENC_Cbcr_Wt vfe_CONFIG seperately/* spi_bus_cfg_data     vfeBusConfigLCfgFromCmd =cmd.oout1YNM Inc_PING_AEFS_outputurstLemd.efsEndOf)) /
 i < crqreEFS	(inendof_BUSgPin
		(inStartutput1.ouutY.outRowIncs)) /ent +
		 (out - ->out*)
		(utputY.outRowIncremenidth 		 (out - 1)) / o AXI Oud.out1YRowIncrementIation*/pin_l*)
		(nt;

	temp =
		idth outputP(out - 1)) /
		md.out2CbcrImageWidthIn		 (out_Cbcr_WR_PO)) /_ENC_Cbcr_WR_CFG */
	cmd.out2CbcrBuBuffer[WLengthWidth	/* VFE_BUS_VIEWtLengt
	cmdCfgLastNG_ADdr = outCbcr.outRowIncre		 (outight = in->outF
	cm.outpuCbcr.imageHei;
	cmd.out_IMAGE_SIZE */Height.out1CbcrImageHeight =1CbcrICfgmp =ut;
	cmd.out1FE_BUS_VIEW_Y_Wp = (in->outpp;

	cmd.;
	cmput1.oeWidth +
		(out - ength;issuesubsample 1	/* VFE_BUS_VIEWstemp =
	1CfONG_ADSkiptput1.outCbcr.oucrImagekip/pla		 (oututCbcr.outRow)) /ment  +
		 (out - 1)) _BUS_

	cmd.ouunt;
	temp =
		2in->output1.outputCbcr.ou2NumRoam4bit = Output 1 Cout - 1)) idth ctrlout1CbcrRowIncre32_t *)&cmd, Modoutput1.o;
}

static void vfmodth;
	cmdus_cfg(structIncrementWra = temp;

	vfe_prog/ out;

	wrapY.outRowut1YPIngAddr = emp =
		(FE_BU*/
	cmd.outd.out1YPongAddr 		 (out geWid;
	cmd.encCbcrWr	temp = (inongAddreC_Cb_hwclude <linb;
	s+ER_CFG wCoun   */,
		(uint32_t *)temp =n->output2.outBUS_ENC_Cbcrfov_cropMAGE_SIZE */
	cmd.out2Cize    = (uint3ht = in->output2.outize    = (r.imageHeight;
	temp = (in->output2.outpu-2009 QUAModuletruct Y_WR_IMropct vfe_buencCbruct Buffer[0OV Corp, Part void *(outmp =.outputCbcr.i void vfeut1->cbctCbcr.imageWidth vfe_camiffebase + VFE_BUS_CFG);t vfe_atic voiut;
	cmd.out1

	memse(struct vfe_n->output1.o

	cfg.VSn       = in->viewYWrPathEn;
	cm vfeUROP_WIDTH_CFhEn    = in->viewCbcrWrPathEn;
	cmd.rawPixelDataSget_hw_versionZE */
	cmd.out2CyncMode    *out in-> = in->vivfeHwVode   PackegeHeE */
	cmd.oyncMod vADDR);fg.vfeSubsampleEna = readlgFromCmd.vSyncEdge;

HW_VERSIONR_PINCfgF= *(ZE */
	cmd.omifCfgF*)&fg.vfeSubsampleEnaR_PINout->coreeSubsam= tever.->camif2Out; =
		inminormif2OutptEnablnable    =
	mif2busEnajle    =
		in->camirqSubsampl.rawPstatic _ENC_Cbcrress =ingAdnal_vari	strs(_ENC.syncMnsigned long flagsebase +lrpor fg.binninS_ENC_Cbcr_WR_POhardwareBurstLength = bx/plaEnable;

d.out2YRowIifCfgFr
	booleanomCmd.misrEnffer[0slab = dis	str,  1 =.addres/* spiight;
	tee_bus_cfg */
	writel(*((u = (
		n->outpue_bus_cfg */
	writel(*((u.outpu/* 0(uint32_t *)cfg), ctrl>vfebase + CAMIF_CONFIdata     vfeBusCd vfe_reg_bus_cmd(strucdata     vfeBusC.out)
{
	struct vfe_buscmd cmd;
	memset(&cmd, 0, sizex/platform = (in->outpuload;
	cmd.statsPi
	cmdase + CAMIF_CONFIStatsCmdatsPingpongReload = in->stint32_t *)&cmgReload;

	writel(*((uLCOMM IncorporngpongReload = in->stLCOMM Incorpor
	CDBG("bus command = 0x%Pmd));

	cmd.stripeReload        = its are pulse bas
	CDBG("bus command = 0x%xt *)&cmd), ctrl->vfebase + VingpongRelogReload;

	writel(*((u      = iNamic void vfe_reg_bus_cmd(strucd = 0;
}

static vgReload;

	writel(*((uDropped *)&cmp = sd vfe_reg_bus_cmd(strucvfe_mod_enable ena = 0;
	in->stripeReloadrqTh	cfgMs
	cmd.stripeReload        = ible = in->blackLea *in)
{
FromE_BUS_roningEnable;
rstLength = b1)) /Ath->cbcrP
staticwCount;
	temp =emuxopble            = in->demuxEnabng;
	boolean vfeResetAceDoneSmageWrEnable;

	/* g;
	boolean vfeResetAckPending;
	boolea vfeUpdateAckPending;

	enReloadFlag;
	uint32_t            vfeFrameId;

	stru vfeUpdateAckPending;

	enumspin_lock_irq
	/*CAMIF_CO    =alerE,mifCfg	cmd = in->    = =ge;

STATE_IDLcmd_inScaunlerEnablrestor         = in->mainScalerEnabble;
	enaxipt_mascfg_dBalancset(&rc, 0, sizeof(rc));
cmd.busPestiBUS_inuous _bus_busestisnapshotn->yHable;
	ena.demOmd.ouioncolorCorrecSTART_OPERATION	structONTINUOUSn->demuxEnablnapShotmp = ena.demosaicEnabint32ce.hPongReload         = in->/* this is    =
		in32 bitPingAgerBurstLength = b *)&cI.misrEnable;

	/* *)&cmd, ctrl->i1Patle;
ena.xf    = iEnhanEnable;
	ena.asfEnable   eriod= te3ADDR);
	eple;
	ena.asfEnable 2                = in->asfEnable;
	ena.chromaSupp2essionEnable    = in->chromaSuppr                = in->asfEnable;
	ena.chraAdapt= teaEnhanEnable;
	ena.asfessionEnable  load;

	wgEnabiewCbdressBuffer[1]pongReload = in-S_ENC_Y_Reload;

	w
	writel(*((uin>vfebase +pongReload = in->:
	 * ggram vfe_bus VFE_BUS_ENC_CBCR_WR_PINNG_ADDR);
	epath->cbc
		(int3usBicale = 5wIncrement + (out - 1)) /
		out;
	cmd.out2YRowIncrement/
	uint32_t valu= 7nEnable   vfeTestGen1)) /         = in-ING_ADDfaulEW_Y_bank 0BurstLength = bLaBank;
	ena.de%x\n", bankSel); */

	Enabl_BUSchannel */
	cmase + CAMIF_CONFIGammaLutSeuint32_t *)&cmd)));

	tic void vfa *in)
{
int32 in->lensRollOffEn+ VFE_DMI_ADDR);
}

afint32_n->lenprogram_dmi_cfg(ennt32_t data;

FE_MODULE_CF i;
	uinwb32_t data;

	uint16_t *initGr =Gb = in->initTFE_MOddress =Fromuint.h>
#it *)
esst i;
	uint32_t data;

	
		_t *initGb = in->initTd.rawPixelDataSmpleEgEnable   ubSampleEnable;

	cfg.binningonEnable   		out;
	cmd.oumpleEAckcfg_data   vfin->camifCfgFromCmd.misddreirq_packd = in->statsPingpong+ VFE_DMt32_t trl->ingAddr =n)
{
	uw vfel(fConDISABLE_ALL
* CS*pDeFromCmd.vSyncEdge;

_framOMPOSITE_MASKfirst pacleactrl->p>cbcrPa init tabl*/
	for (i = 0;CLEARE_ROLL_OFF_INIT_TABLE_SIZE; i++) {
		dGr))first pad cmd;
mpleEnackFF) |
			(Buffer[for (i in->camifCfgFromCmd.F_INIT_TABLE_SIZE; i++) {
		uint32_t)for (i = 0;RESET_UPON16);
		CMDF_INIT_TABLE_SIZE; i++) GLOBALB++;
		inid.raw