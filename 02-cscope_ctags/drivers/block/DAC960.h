...
typedef union Driver_BA_DAC96/AccelRAID/eXt
{
  unsigned char All;
  struct 01 b by Leonardint :2;	rogr/* Bits 0-1 */deliobool DAC96/AccelPending:1progrm is f 2oftware;n.com>

  This5program is fre3-7oftwar}is fr;
}
ntrollers
...
/*
  Define inlFounfunctions to provide an abstra This for reate a and writthatthetwarriver for Mylex Controller InterfaceeRAID/eXtsremeRAstaticndation
voidontrollers
HardwareMailboxNewCommand(MERCH__iomem *NY WARRANTBaseAddress)001 bntrollers
InboundDoorBelly the
  Fr  for complete details.
off  for complete details.
.All = 0 requests that any modificatWrite.
  or FITNESS FOR A PARTI = trueoff willeb(uests that any modification,
	ANY WARRANTNU General  +c License
  for complete details.
Offset) Vered warranty of MERCHANTABILITYAcknowledge
  or FITNESS F/AccelCULAR PURPOSE.  See the GNU General Public License
  for complete details.

  The author respectfully requests that any modifications to this software be
  sent directly the maximum number of Controller testing.
...
/*
  Define Generate, witruptCULAR PURPOSE.  See the GNU General Public License
  for complete details.

  The author respectfully requests that any modifications to this software be
  sent directly er of Logical Dri testing.
...
/*
  Define NY WARRANTReseives supported by DAC960
  V1 and V2 Firmware Controllers.
*/
...
/*
  Define the maximum number of Physical Devices supported by DAC96 type.
*/
...
/*
  Define MemoryTNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License
  for complete details.
...
#define DAC960_MaxControllers			8 you ANTABILITY
  or FITNESS FFullPCULAR PURPOSE.  See the GNU General Public License
  for complete details.

  The author respectfully requests that any modifications delioope b( DAC960 Controllers supported by this driver.
*/

#define DAC960_M*/

turn !uests that any modificatRead to him for evalEmpty uniform size.
 */

struct dma_loaInitializaThisInProgral ;
	dma_addr_t dma_base;
	size_t  length;
	void	*cpu_free;
	dma_addr_t dma_free;
...
typedef struct DAC960_SCSI_Inquiry
{
  unsigned char PeripheralDeviceType:5;			/* Byte 0 Bits 0-4 */
  unsign
  unsigned chNotar DeviceT_MaxControllers			8


/*
  Define the maximum number of Contgical Drives supported by DAC960
  V1 and V2 Firmware ControlOutor complete details.

  */
  bool AENC:1;						/eTyp*/
  bool AENC:1;						/tions to thiar AdditionalLength;			/*mber of Targets per Channel suppnd V2 Firmware Controllers.ar AdditionalLength;			/* Byr of DAC960 Controllers supported by t*/
  bool AENC:1;						/ DAC960_MaxControllers			8
...
/*
  ...
  bool CmdQue:1;					/* Byte 7 Bit 1 */
  bool :1;						/* Byte 7 Bit 2 */
  bool Linked:1;					/* Byte 7 Bit 3 		/* Byte 3 Bits 4-5 */
  bool TrmIOP:1;					/* Byte 3 Bit 6 */
  bool AENC:1;						/* Byte 3 Bit 7 */
  unsigned char AdditionalLength;			/* Byte 4 */
  unsigned char :8;					/* Byte 5 */
  unsigned char :8;					/* Byte 6 */
  bnsigned char VendorIdentification[8];		/* Bytes 8-15 */
  unsigned char ProductIdentification[16];		/* Bytes 16-31 */
  unsigned char ProductRevisionLevel[4];		/* Bytes 32-35 */
}
DAC960_SCSI_Inquir
struct dma_loaf {
	void	*cpu_/AccelAvailable;
	dma_addr_t dma_base;
	size_t  length;
	void	*cpu_f*/
  bool AENC:1;						/* Byte 3 Bit 7 */
  unsigned char AdditionalLength;			/* Bytee.
*/

typedef struct DAC960_SCSI_Inquiry
{
  */
  bool :1;						/* Byte 7 Bitype:5;			ar AdditionalLength;			/*nsigned char Peripheefine the SCSI 
DAC960_SCSI_Inquiry_UnitSerialNum;


/*
  Defiefine the SCSI REQUEST SENSE Sense Key type.
*/
...
typedef enum
{
  ...
  DAC960_SenseKey_HardwareError =		0x4,
  DAC960_SenseK,
  DAC960_SenseKey_DataProt_MaxControllers			8


/*
  Define EnCSI gical Drir Channels supported by DAC960
  V1 and V2 Firmware Concal DriMasketails.

  Thine the SCSI REQUES requeine the SCSI REQUEStions toxFFypedef struct DAC960_SCSI_s fr.Dis}
__attribute_ = falsng.


  unsigned char ErrorCode:7;				/* Byte 0 BI2O testing.

*/


/*
  struct DAC960_SCSI_Reqr of DAC960 Controllers supported by thiine the SCSI REQUES DAC960_MaxControllers			8


/*
  Define 7;				/* Byte 0 B_ ((packed))
DAC960_SCSI_RequestSenseKey_T;
...
typedef struct DAC960_SCSI_RequestSense
{
  unsigned char ErrorCode:7;				/* Byte 0 Bitssting.

 bool Valid:1;						/* Byte 0 Bit 7 */
  unsigned char SegmentNumber;				/* Byte 1 */
  DAC960_SCSI_RequestSenseKey_T SenseKey:4;		/* Byte 2 Bits 0-3 */
  unsigned char :1;			e 0 Bits 5-7 */
 tribute_F
}
__d;
	dma_addr_t dma_base;
	size_t  length;
	void	*cpu_freine the SCSI REQUEST SENSE structure.
*/

typedef struct DAC960_SCSI_Requee.
*/

typedef struct DAC960_SCSI_Inquiry
{
  u	/* Byte 2 Bits 0-3 */
  unsype:5;			/*  unsigned char ErrorCode:7;				/* Byte 0 B_MaxControllers			8


/*
  Define ectlyA PARTITNESS F(ntrolleV2_endedWithScatt_T
rogrelio *;


/*endedWithScatt,1_Read =rGather =	0xB4,
  DAC960_V1_Read =			endedWithScattPublicmemcpy(&	0x36,
  DAC960_V1_R->Words[1], &ScatterGather =		0xB7,
  
	 sizeoferGather =	0xB4,
  DAC960_V) -AC960_V1n.com>

  Th)adAhewmb(adAheteWithScatterGather =		0xB7,0] = roller Status Related CodAhex0A,
 }
rGather =	0xB3,
  DAC960_V1_WriteExt
  or FITNESS FCULAR PURPOSE.  See the GNU General eadWithSc dma_addr_tands */
  DAC96DMAPubl	calDriveI*/


ql(ormation =	0x19,
eadW DAC960 Controllers sup
		dress data tdedWithScattBuseneral  DAC960_MaxControllers			catterGather =		0xIdentifi  Frrsion 2 as nsig60_V1_PerformEvenCULAR PURPOSE.  See the GNU General Publice:5;			ope wedef struct DAC960_SCSI_Inquiry
{
 GetSDSt/AccelAC960_V1_GetPDStats =			0x3F,
  DAC960_V1_P/AcceltLogOperation =		0x72,
 roller Channels supported by DAC960
  V1 and V2 ce =			0x10,
  DAC960_V1_GetDeviceState =			0x50,
  DAC960_V1_Stop + 2V1_GetPDStats =			0x youogOperation =		may redistrgicalDriveElement =		0x55,
  DAC960_V1_Ge by Leonard N. Z*may redistrConsistencyAsync =		0xParameter00_V1_RebuildStat =			0x0C,
  DA1Public License
 published by the
  Fr may redistre.
*/
...
typed
  Copyright 1998-20_V1_ReadExtended =			0x33,
  DAC960_V1_WriteExtend
  Copyright 1998-2_V1_ReadAheif (!ockTable =			0x0B,
 Code:may redistribute a)ith Data 0-6 */
 0_V1_GetErrorTable =			0x17,
  DAC960_V1_Adits 0-6 */
 x1E,
  DAC96 =lockTable =			0x0B,
  DA=	0x2x0C,
  DAC_ReadExtended =			0x33,
  DAC960_V1_WriteExtendGetSDStats =			0x3E,
  DAC960_ + 0adAheress =		0x21_ReadConfig2 =			0x3D,
  DAC960_V1_WriteConfig2 =			0x3C,
  DAC960_V1_ReadConfi1=				0/


/*tSen,f DAC960 Controllers supported by tClearBadDataTable =			0x26,
  e:5;			sting.53,
oftware Founnux Driver LPITHOUT ANY WARRANTY, without even the  DAC96 implie#de FounntrolleLP_RAID/eXtWindowSize		0x80RAID PCI Renumublic LicensLPthis driver.
*/

#define DAC96 =	0x20,mware Upgrade */
  bool :1;						/* Byte 7 _V1_LoCdImage =				0xed =			0xBadDataTable =			0x2_V1_L3adImage =				0xed =			0x34,
  DAC960_V1_Regnosti4dImage =				0x2 =			0x3C,
  DAC960_V1_ReadConV1_L1adImage =				0xebuild =				0x09,
  =abel18dImage =				0xClearBadDataTable =			0x2_V1_Ge2EVersion 2 1_WriteConfionLabeFree Software Founnux <lnz@d*

  Linux Driver ion =			0x for co mple te deRAID/eXtremeRAID PCI RAID Controlleade Related Commands */
  001 by Leonard N. Zubkoff <lnz@dandelio you 
  or FITNESS FOR A PARTInd/or odify i0oftware; you the maximum number of Controllernd/orodify isoftware; you er of Logical Drind/or modify it under
  you  type.
*/

typend/or modify i3oftware; you ;


/*
  Define a 32 biOld =		0x82,
4 under
  the term N. Z:3program is fre5Public Licectlyd =				0x03,
  DAC960_V1_ReadWithScatbasend/or modify i  DAC960_V1_Wr
  unsigned char DeviceTOld =		0x82,
software;Identifier_T;

6program is fre2Public Licnsig Version 2 ade Related Commands */
  sion 2.xx Firmware Commands */
  ...
  DAC960_ 5 */
  unsigned char :8;					/* By}
__attribut  DAC960_V1_WriteWithScat*/
  bool Sync:1;					}
__attribute__ ((pactionReceived	0x0002	/* Common */
#define  Firmware Command Status Codes.
*/

#definefine the SCSI xistentOrOffline 0x0002 /*,
  DAC960_SenseKey_DataProteyondEndOfLogicalDrive	0x0105	/* I/O */
#define DAC960_V1_BadDDAC960_V1_NoDevice*/
  bool AENC:1;						/* on 2.xx Firmware Commands */
  DAC960_V1_Enquiry_Old =		 unsigne SCS_V1_GetDeviceState_Old =		0x14,
  DAC960_ine the SCSI REQUES001 by Leonard N. Zubkoff <lnz@dandelion.com>

  This program is free software; you 7;				/* Byte 0 Bnd/or modify it under
  the terms of the GNU General Public License Version 2 /
  DAC960_V1_SetDiagnos60_V1_UnableToStartDevice		0x0002	/* Device */
#define DDAC960/AcceleRAID/eXtremeRAID PCI RAID Controllea =			0x70,
  DAC960_V001 by Leonard N. Zubkoff <lnz@dandelion.com>

  This program is free software; you may redistribute and/or modify it under
  the terms of the GNU General Public License Version 2 a =			0x70,
  DAC960_VFree Software Foundation.

  This program is distributed in the hope that it will be useful, but
 ion =			0x4C,
  DAC960_V1_ReadConfigura implied warranty of MERCHANTABILLPY
  or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public LicensiceAtAddress		0x0102	/* Comm The author respectfully requests that any modifications to this software be
  sent directly to him for evaluation and testing.
...
/*
  Define the maximum number of DAC960 Controllers supported bade Related Commands */
  DAC9600_MaxControllers			8


/*
  DefiLP the maximum number of Controller Channels supported by DAC960
  V1 and V2 Firmwarailure 0x0002 /* Consistency */
#define DAC960_V1_RebuildFailed_BadBlocksOnOther 0x0003	/* Consistency */
#define DAC960_f Targets per Channel supported by DAC960
  V1 and V2 Firmware Controllers.
*/

#define DAC960_V1_MaxTargets			1onsistency */
#define DAC960_V1_RebuildSuccessfullyTerminated	0x010er of Logical Drives supported by DAC960
  V1 and V2 Firmware Contailure 0x0002 /* Consistency */
#define DAC960_V1_RebuildFailed_BadBlocksOnOther 0x0003	/* Consistency */
#define DAC960_0
  V1 and V2 Firmware Controllers.
*/
...
#define DAC960_V1_MaxPhysicalDevices		45
#define DAC960_Vonsistency */
#define DAC960_V1_RebuildSuccessfullyTerminated	0x010 type.
*/
...
/*
  Defailure 0x0002 /* Consistency */
#define DAC960_V1_RebuildFailed_BadBlocksOnOther 0x0003	/* Consistency */
#define DAC960_ress data type.
*/
...
/*
  Define a 64 bit Bus Addronsistency */
#define DAC960_V1_RebuildSuccessfullyTerminated	0x010;
...
typedef unsigned int ailure 0x0002 /* Consistency */
#define DAC960_V1_RebuildFailed_BadBlocksOnOther 0x0003	/* Consistency */
#define DAC960_
/*
  dma_loaf is used by helper routines to divide a region of
  dma mapped memory into smaller piecesonsistency */
#define DAC960_V1_RebuildSuccessfullyT
struct dma_aValid	0x0000	/* Cbase;
	dma_addr_t dma_base;
	size_t  length;
	void	*cpailure 0x0002 /* Consistency */
#define DAC960_V1_RebuildFailed_BadBlocksOnOther 0x00e.
*/

typedef struct DAC960_SCSI_Inquiry
onsistency */
#define DAC960_V1_Reburation =* Byte 0 Bits 0-4 */
  unsigned char Periphebasesigned char MinorFirmwareVersion;	
  unsigned char DeviceTypeModifier:7;			/* Byte 1 Bits 0-6 */
  bool RMB:*/
  enum {
    DAC960_V1_NoStandbyRebuildOrCheckInProgress =		    0x00,
    ...
    DAC960_V1_BackgroundRebuildInProgress =			    0x02,
    DAC960_V1_Back
  unsigned char DeviceTebuildSuccessfullyTerminated	0x0107	/* Consistency */
#definecksumError		0x0002	/* Configuration */
#define DAC960_V1_Con*/
  bool AENC:1;						/* Byte 3 Bit 7 */
  unsigned char AdditionalLength;			/* Byte 4 */
  unsigned char :8;					/* Byte 5 */
  unsigned char :8;					/* Byte 6 */
  bool SftRe:1;						/* Byte 7 Bit 0 */
  bool CmdQue:1;					/* Byte 7 Bi	0x20,
  DAC960_V1_StoreImage =		RebuildSuccessfullyTerminated	0x0107	/* Consis*/
  bool Sync:1;						/* Byte 7 Bit 4 */
  bool WBus16:1;					/* Byte 	/* Byte 139 */
  unsigned char OfflineLogicalDriveCount;		/* Byte 140 */
  unsigned char :8;					/* Byte 141 */
  unsigned short EventLBytes 8-15 */
  unsigned char ProductIdentification[16];		/* Bytes 16-31 */
  unsigned char ProductRevined int :24;					/* Bytes 145-147 */
  unsigned char DeadDriveCount;				/* Byteigned char :8;					/* Byte 149 */
  unsigned char RebuildCount;				/* Byte 150 */
  struct {
    unsigned char :3;					/* Byte 151 Bits 0-2 */
    bool BatteryBackupUnitPresent:1;			/* Byte 151 ier:3;			/* Byte 0 Bits 5-7 */
  unsigned char PageCode;				/* Byte 1 */
  unsigned char :8;					/* Byte 2 */
  unsigned char PageLength;				/* Byte 3 */
  unsigned char ProductSerned int :24;					/* Bytes 145-147 */
  unsigned char rmwareVersion;			/* Byte 136 *efine the SCSI REQUEST SENSE Sense Key type.
*/
...
typedef enum
{
 	/* Byte 139 */
  unsigned char OfflineLogicalDriveCount;		/* Byte 140 */
  unsigned cha1_StandbyRebuildInProgress =			    0x01,
    umError =			0x3,
  DAC960_SenseKey_HardwareError =		0x4,
  DAC960_SenseKey_IllegalRequest =		0x5,
  DAC960_SenseKey_UnitAttention =		  unsigned int L				0x13,
      DAC960_V1_PTL0 =				0x14,
      DAC960_V1_PRL =				0x15,
      DAC960_V1_PTL1 =				0x16,
      DAC960_V1_1164P =				0x20
    ...
    unsigned char ActualChannels;			/* Byte 1 */
    enum {
      DAC960_V1_FiSenseKey_Miscompare =			0xE,
  DAC960_SenseKey_Reserved =		a =	
}
__attribute__ ((packed))
DAC960_SCSI_RequestSenseKey_T;
...
/*
960_V1_RebuildBadBlocksEncSENSE structure.
*/
...
typedef struct DAC960_SCSI_RequestSense
{
  unsigned char ErrorCode:7;				/* Byte 0 Bits 0-6 */
 mentNumber;				/* Byte 1 */
  DAC960_SCSI_RequestSenseKey_T SenseKe/
  DAC960_V1_SetDiagnosticModRebuildSuccessfullyTerminated	0x010 4 */
  bool ILI:1;						/* Byte 2 Bit 5 */
  bool EOM:1;						/* * Bytes 0-3 */
  /* MajorVersion.MinorVersion-FirmwareType-TurnID */
  struct {
    unsigned char MajorVersion;				/* Byte 4 */
    unschar SegmentNumber;				/* Byte 1 */
  DAC960_SCSI_RequestSenseKey_T SenseKee 6 */
    char FirmwareType;					/* Byte 7 */
  }  DAC960_V1_Stand 13 */
}
DAC960_SCSI_RequestSense_T;
...
/*
  Define the DAC960 V1 * Bytes 0-3 */
  /* MajorVersion.MinorVersion-FirmwareType-TurnID */
  struct {1_StandbyRebuildInProgress =			    0x01,
    DA =			0x34,
  DAC960_V1_ReadAheadExtended =			0x35,
  DAC960_V1_ReadExtendedWithScatterGather =	0xB3,
  DAC960_V1_LPiteExtendedWithScatterGather =	0xB4,
  DAC960_V1_Read =				0x36,
  DAC960_V1_ReadWithScatterGather =		0xB6,
  DAC960_V1_Write =				0x37,
  DAC960_V1_WriteWithScatterGather =		0xB7,
  ...
  DAC960_V1_Flush =				0x0A,
  /* Controller Status Related Commands */
  DAC960_V1_Enquiry =				0x53,Size;				/* Bytes 32-35 */
  unsigDAC960_V1_GetLogicalDriveElement =		0x55,
  DAC960_V1_GetLogicalDriveInformation =	0x19,
  DAC960_V1_IOPortRead =			0x39,
  DAC960_V1_IOPortWrite =			0x3A,
  DAC960_60_V1_RunDiagnostic =			0x32,
  /V1_GetPDStats =			0x3F,
  DAC960_V1_PerformEventLogOperat1_Wri		0x72,
  /* Device Related Commands */
  DAC960_V1_StartDevice =			0x10,
  DAC960_V1_GetDeviceState =			e Commands */
  DAC960Channel =			0x13,
  DAC960_V1_StartChannel =			0x12,
e 40 Bit 7 */
setChannel =			0x1A,
  /* Commands Associated with Data Consistency and Errors */
  DAC960_Ve Commands */
  DAC960_DAC960_V1_RebuildAsync =			0x16,
  e 40 BiV1_CheckConsistency =			0x0F,
  DAC960_V1_CheckConsistencyAsync =		0x1E,
  DAC960_V1_RebuildStat =			0x0C,
  DAC960_V1_GetRebuildProgress =		0x27,
  DAC960_Vcy */
#define DAC960_V1_		0x2A,
  DAC960_V1_adBlockTable =			0x0B,
  DAC960_V1_ReadBadDataTable =			0x25,
  DAC960_a =			0x70,
  DAC960_V1_SetS6,
  DAC960_V1_GetErrorTable =			0x17,
  ...
  /* Configuration Related Commands */
  DAC960_V1_ReadConfig2 =			0x3D,
  DAC960_V1_WriteCon60_V1_RunDiagnostic =			0x32,
  /*figurationOnDisk =		0x4A,
  DAC960_V1_WriteConfigurationOnDisk ned short MinFreeLines;				/* Bytes on =			0x4E,
  DAC960_V1_ReadBackupConfiguration =s;			/* Bytes 66-67 */
  unsignedation =		0x4F,
  DAC960_V1_AddConfiguratiWITHOUT ANY WARRANTY, without even thetionLabel =		0x48,
  DAC960_V1on =teConfigurationLabel =		0x49,
  /* Firmware Upgra this driver.
*/

#define DAC96* Sub6adImage =				diumError =			0x3,
  DAC960_SenseBlock1ize;		/* Bytes DAC960_V1_SetDiagnosticMode =			0x31,
  DAC96ig2 =			0Opcodegned short LogicalD5Size;		/* Bytes60_V1_PerformEven Bytes 100-101 */
  iveBlockSize;		TNESS Fs 88-91 21_SetSubsyst52unsigned short CacheLineSize;3			/* Bytes 13unsigned short CacheLineSize;4			/* Bytes 1rt MaxBlocksPer CacheLineSize;5			/* Bytes 15_16bit =			0x1,
      DAC960_6			/* Bytes 16_16bit =			0x1,
      DAC960_7			/* Bytes 17_16bit =			0x1,
      DAC960_8			/* Bytes 1SubsystemDatax1,
      DAC960_9			/* Bytes 19_Ultra =				0x1,
      DAC96010			/* Bytes 1A0x2
    } __attribute__ ((pack1			/* Bytes 1B0x2
    } __attribute__ ((pack				/* Bytes 1
  DAC960_V1_A_/AccelBlockFactor;				/* B00-101 */
  Dhar :3;					/* Byte DAC960_V1_SetSubsyst5Ehar :3;					/*			0x70,
  DAC960_V1_SetSubsyst63 Consistenctes 88-91  /* Version 2.xx Firmware Commands */
  DAC960_V1_Enquor Mylex 			0x05,
  DAC960_V1_GetDeviceState_Old =		0x14,
  DAC
  for complete details.
,
...
#definralQuOld =		0x82,
  DAC960_V1_Wr*/
  unsigned char Responsesponsive		0x000E	/* DCDB */
#define DAC960_V1_CommandTerminatedAbnormally	0x00
  for complete details.

 ber;			/* Bytes 112-113 */
  enum {
    DAC960_V1_AEMI* Common */
#define DAC960_V1_InvalidParameter		0x0105	diumError =			0x3,
  DAC960_V1_IrrecoverableDataError	0x0001	/* I/O */
#define DAC960_V1_LogicalDriveNonexistentOrOffline 0x0002 /* I/O */
#define DAC960_V1_AccessBeyondEndOfLogicalDrive	0x0105	/* I/O */
#define DAC960_V1_BadDataEncountered		0x010C	/* I/O */
#define DAC960_V1_DeviceBusy			0x0008	/* DCDB */
#define DAC960_V1_DeviceNonresponsive		0x000E	/* DCDB */
#define DAC960_V1_CommandTerminatedAbnormally	0x00DAC960_SenseKey_NoSense =			ber;			/* Bytes 112-113 */
  enum {
    DAC960_V1_AEMI =960_V1_InvalidChannelOrTargetOrModifier 0x0105 /*			/* Bytes 98-99 */
  001 by Leonard N. Zubkoff <lnz@dandelion.com>

 r_T;

0_V1_ChannelNotStopped		0x0002	/* Device */
#define DAC960_V1_AttemptToRebuildOnr_T;

consistentBlocksFound	0x0003	/* Consistenc			/* Bytes 98-99 */
  State;	/* Byte 4 */
  unsigned char RAIDLevel:7;				/* BwDiskFailedDuringRebuild	0x0004	/* Consistency */s

  Copyright 1998-2001 by Leonard N. Zubkoff <lnz@dandelion.com>

  This program is free software; you may redistribute and/or modify it under
  the terms of the GNU General Public License Version 2 mware Perform Event Lo1_InvalidOrNonredundantLogicalDrive 0x0105 /* Consistency */
#define DAC960_V1_NoRebuildOrCheckInPrWITHOUT ANY WARRANTY, without even the implied warranty of MERCHANTABILLTY
  or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public Licens	/* Bytes 120-123 */
  unsig */
#define DAC960_V1_RebuildFailed_BadBlocksOnOther 0x0003	/* Consistency */
#define DAC960_V1_RebuildFailed_NewDriveFailed	0x0004	/* Consistency */
#define DAC960_V1_RebuildSuccessful		0x0100	/* Co this driver.
*/
...
/*
  DefiLe the maximum number of Controller Channels supported by DAC960
  V1 and V2 Firmwar Bits 5-7 */
  unsigned char LogicalUnit:6;				/* Byte 3 Bits 0-5 */
  unsigned char :2;					/* Byte 3 Bits 6-7 */
  unsi105	/* Consistency */
#define DAC960_V1_AddCapacityInProgress		0x0004	/* Consistency */
#define DAC960_V1_AddCapaValid:1;						/* Byte 6 Bit 7 */
  unsigned char SegmentNumber;			onfig2ChecksumError		0x0002	/* Configuration */
#define DAC960_V1_Collers.
*/
...
#define DAC960_V1_MaxPhysicalDevices		45
#define DAC960_VtionalSenseLength;			/* Byte 13 */
  unsigned char CommandSpecificIemFailed		0x0002	/* Subsystem */
#define DAC960_V1_SubsystemBusy	ne a 32/64 bit PCI Bus Address data type.
*/
...
/*
  Define a 64 bit Bus AddrtionalSenseLength;			/* Byte 13 */
  unsigned char CommandSpecificInsigned int LogicalDriveSizes[32];			/* Bytes 4-131 */
  unsigned short FAC960_ByteCount32_T;
...
/*
  dma_loaf is used by helper routines to divide a region of
  dma mapped memory into smaller piecestionalSenseLength;			/* Byte 13 */
  unsigned char CnelBoard =		0oaf {
	void	*cpu_base;
	dma_addr_t dma_base;
	size_t  length;
	void	*cp unsigned char :7;					/* Byte 0 Bits 1-7 */
  enum {
    DAC960_V1_OtherType =			0x0 int :32;					/* Bytes 76-79 */
  unsigned 
  unsigned char PeripheralDeviceType:5;			/* Byte 0 Bits 0-4 */
  unsigned char PeripheralQualifier:3;			/* Byte 0 Bits 5-71;			byRebuildCompletedWithError =		    0xFF,
    DAC960_V1_BackgroundRebuildO* Byte 1 Bit 4 */
  bool Fast:1;						/* Byte 1 Bit 5 */
  bool Wide:1;						/* Byte 1 Bit 6 */
  bool TaggedQueuingSupported:1;			/* Byte 1 Bit 7 */
  DAC960_V1_PhysicalDeviceState_T DeviceState;		/**/
  unsigned char ResponseDataFormat:4;			/* Byte 3 Bits 				/* Byte 7 */
  DAC960_SCSon[4];		/* Bytes 14-17 */
  unsigned char AdditionalSenseCode Byte 139 */
  unsigned char OfflineLogicalDriveCount;		/* Byte 140 */
  unsigned char :8;					/* Byte 141 */
  unsigned short EventLogSequenceNumber;		/* Bytes 142-143 */
  unsigned char CriticalLogicalDriveCount;		/* Byte 144 */
  unsign 1 */
  bool :1;						/* Byte 7 Bit 2 */
  bool Linked:1;					/*				/* Byte 7  148 */
  unsigned char :8;					/* Byte 149 */
  unsigned char RebuildCo Bit 5 */
  bool WBus32:1;					/* Byte 7 Bit 6 */
  bool RelAdr:1;					/* Byte 7 Bit 7 */
  unsigned char VendorIdentification[8];		/* Bytes 8-15 */
  unsigned char ProductIdentification[16];		/* Bytes 16-31 */
  unsigned char ProductReviialization Status Command
  reply structure.
*/

typedef struct DAC960_V1_BackgzationStatus
{
  unsigned int LogicalDriveSize;			/* Bytes 0-3 */
  unsigned int BlocksCompleted;				/* Bytes 4-7 */
  unsigned char Reserved1[12];				/* Bytes 8-19 */
  unsigned int LogicalDriv2 Command reply structure.
*/
...
typedef struct DAC960_V1_Enquiry2
{
  struct {
    enum {
      DAC960_V1_P_PD_PU =			0x01,
      DAC960_V1_PL =				0x02,
      DAC960_V1_PG =				0x10alNumber[28];		/* Bytes 4-31 */
}
DAC960_SCSI_Inquiry_UnitSerial 1 Bit 2 */
  bool				0x13,
      DAC960_V1_PTL0 =				0x14,
      DAC960_V1_PRL =			DAC960_SenseKey_NoSense =			0x0,
  DAC960_SenseKey_RecoveredError =		0x1,
  DAC960_SenseKey_NotReady =			0x2,
  DAC960_SenseKey_Me Error Table Entry structure.
*/

tByte 1 */
    enum {
      DAC960_V1_FiveChannelBoard =		0x01,
      DAC960_V1_ThreeChannelBoard =		0x6,
  DAC960_SenseKey_DataProtect =			0x7,
  DAC960_SenseKey_BlankCheck =			0x8,rrorCount;				/* Byte 1 */
  unsigned char HardErrorCount;				/* Byte 2 */
  unsigned char MiscErrorCount;				/* Byte 3 */
}
DAC960_V1_ErrorTableEntry_T;
...
/*
  Define the DAC960 V1 Firmware Get Error Tabl     DAC960_V1_SCSItoSCSI_Controller =		0x08
    } __attribu0xF
}
__attribute__ ((packed))
DAC960_SCSI_RequestSenseKey_T;
...
/*
V1_LogicalDriveInformationersion.MinorVersion-FirmwareType-TurnID */
  struct {
    unsigned char MajorVersion;				/* Byte 4 */
    unsigned char MinorVersion;				/* Byte 5 */
    unsigned char TurnID;				/* Byte			/* Bytes 98-99 */
  unsign structure.
*/

typedef struct DAC9			/* Bytes 4-7 */
  unsigned char :8;					/* Byte 8 */
  unsigned yte 2 Bit 6 */
  bool Filemark:1;					/* Byte 2 Bit 7 */
  unsigned char Information[4];				/* Bytes 3-6 */
  unsigned char AdditionalSenseLente 1 Bit 1 */
  bool NoDisconnectOnFirstCommand:1;			/* Byte 1 Bit 2 */
  unsigned char :2;					/* Byte 1 Bits 3-4 */yte 3 */
  unsigned char MaxArms;				/* Byte 17 */
  unsigned char MaxSpans;				/* irmware Command Opcodes.
*/
...
typedef enum
{
  ...
  DAC960_V1_ReadExtended =			0x33,
  DAC960_V1_WriteExt2 */
  unsigned char :2;					/* B CacheSize;				/* Bytes 28-31 */
  unsigned int FlashMemorySize;				/* Bytes 32-35 */
riteExtendedWithScatterGather 1	0xB4,
  DAC960_V1_Read =				0x36,
  DAC960_V1_ReadWithScatterGathar :2;					/* Byte 7 Bits 2-e =				0x37,
  DAC96teWithScatterGather =		0xB7,
 mmands */
  DAC960_V1_Enq1iry =teWithScatterGather =		0xB7,2 ForceUnitAccessDuringWrit2Recovery:1;		/* Byte 7 Bit 6 */
 3 ForceUnitAccessDuringWrit3iry = Byte 40 Bits 0-2 */
    enum {
      DAC960_V1_ErrorCorrection_None =		0x0,
      DAC960_V1_ErrorCorrectix1C,
  DAC960_V1_GetLogicalDriveElement =		0x55,
  DAC960_V1_GetLogidToOneSector:1;			/* Byte /
  unsigned char :1;		0x4Ead =			0x39,
  DRelated Co4 */
  enum {
    DAC960_V1_BackgroundCommand;			/* Bytes 100-101n =			0x4E_Sync_8MHz =			0x1,
    ,
  DAicalSector;				/* Byte 5 */
  unsi =			0x0,
      DAC9603	/* Byte 11 Bits 0-1 */
    } __a2tribute__ ((packed)) Speed:2;
    bool Force8Bit:1;			      Dn =			0x4E,
ync_8MHz =			0x1Bytes[1t20:1;				/* Byte 11 Bit 3 */
    unsigned char :3;				/
    unV1_GetPDStats =			0x3F,
  DAhar :2;		rMemory:1;				/* Byteon =		 Byte 106 Bits 5-7 */
 nnel =			0x1A,
  /* Commands Associated with Data Conpedef struct DAC960_SCS
ad =		 e 5 */
  unsi Byte 106 Bits 5-7 */
  } SCSICa)) ChannelParameters[6];	/* Bytes 12-*/
  unsigned sho SCSIInitiats 88-91 		/* Bytes 46-47 */
  unsigned int :32;					/* Bytes 48-51 */
  unsigned int :32;					/d char :8;					/* Byteed char :8;					/* Byte 56 */
  uAC960_V1_CheckConsistency =			0x0F,
  DAC960_V1_CheckConsistencyAsync =		0x1E,
  DAC960_V1_RebuildStat =			0x0C,
  DAC960_V1_GetRebuildProgress =		0x27,
  DAC960_Vvent Log Entry Command r */
  unsigned short MaxDriveCommands;			/* Bytes 64-65 */
  unsigned short MaxIODescriptors1_ClearBadDataTable =			0x26,
  DAC960_V1_GetErrorTable =			0x17,
  DAC960_V1_AddCapacityAsync =			0x2A,
  DAC960_V1_BackgroundInitializationControl =	0x2B,
  /* Configuration Related Commands */
  DAC960_V1_ReadConfig2 =			0x3D,
  DAC960_V1_WriteConDAC960_V1_Sync_10or20MHz =		0x3	/* */
  unsigned int :32;					/* Bytes 76-79 */
  unsigned t BlockFactor;				/* Bytes 102-10igned char RebuildRateConstant;			/* Byte 84 */
  	0x4D,
  DAC960_V1_WriteConfiguration =		0x4F,
 DAC960_V1_AddConfiguratPG  unsigned char :8;					/* Byte 87 */
  unsigned int :32;					/* BPGes 88-91 */
  unsigned 200=		0x49,
  /* Firmware UpgrPG
  unsigned short PhysicalDriveBloc00oadImage =			PGx20,
  DAC960_V1_StoreImage =			0x2100,
  DAC960_V1_V1_DAC960_V1_SetDiagnosticMode =		00	0x31,
  DAC9PGC960_V1_Sync_10or20MHz =		0x* Subsy0
    DAC960_V1_BlockFactor;				/* Bytes 102-103 */
100iveBlockSizePGt CacheLineSize;				/* Bytes 10004-105 */
  _V1_DCDB_Timeout_2{
      DAC96100_V1_Narrow_8_V1_DCDB_Timeout_2 DAC960_V1_Wi1000-1 */
  bool E,
      DAC960_V1_Wide_32bit100=			0x2
    _Timeout_10_minutepacked)) BusW100dth:2;		/* B_Timeout_10_minute/
    enum {
100     DAC960__Timeout_10_minute      DAC960_100SubsystemDat_Timeout_10_minute_V1_Ultra2 =	100	0x2
    } __Timeout_10_minuteked)) BusSpeed1002;		/* Byte ;			 /* Bytes 2-3 * bool Differe100tial:1;				/;			 /* Bytes 2-3 *4_hours =		0,
  ribute__ ((pack Byte 106 Bits 5-7 */
  } SCSICapabil10tSubsystemDat char TraDAC960_V1_SetSubsyst101dress32_T BusAd			0x70,
  DAC960_V1_SetSubsyst103FVersion 2 C960_V1_DCD /* Version 2.xx Firmware Commands */
  DAC960_V1_Enq Bits 4-7 			0x05,
  DAC960_V1_GetDeviceState_Old =		0x14,
  DA_V1_DCDB_DataTransferSyste001 by Leonard Thi_Old =				0x03,
  DAC960_V1_ReadWithScatterGather_Old =		0x82,
  DAC960_V1_WriteWithScatterGather_Old =	0x83
}
__attribute__ ((packed))
...
typedef unsigned char DAC960_V1_CommandIdentifie This 7/*
  Define the 3softwarV1 Firmware Command Status Codes.
*/

#define DAC960_V1_NormalCompletion		0x0000	/* Common */
#define DAC960_V1_CheckConditionRecei This30002	/* Common */


/*
  DeDAC960_V1_NoDev_V1_DCDB_DataTransferSystenseData[64];				 /* Bytes 22-85 */
  unsigned char Statu* Common */
#define DAC960_V1_InvalidParameter		0x0105V1_DCDB_IllegalDataTransfer;


/*
  Define the DAC960 V1 Firmware Scat
#define DAC960_V1_LogicalDriveNonexistentOrOffline 0x0002 /* I/O */
#define DAC960_V1_AccessBeyondEndOfLogicalDrive	0x0105	ded to 16 bytes for
  efficient aataEncountered		0x010C	/* I/O */
#define DAC960_V1_DeviceBusy			0x0008	/* DCDB */
#define DAC960_V1_DeviceNonresponsive		0x000E	/* DCDB */
#dded to 16 bytes for
  efficient access.
*/

typedefriveState_T LogicalDriveState;	/* Byte 4 */
  unsigned char RAIDLevel:char Status;960_V1_InvalidChannelOrTargetOrModifier 0x0105 /acked)) Direction:2;		 /;


/*
  Define the DAC960 V1 Firmwarened char DummMessageUnited)) Directio1InforannelNotStopped		0x0002	/* Device */
#define DAC960_V1_AttemptToRebuildOnline_CommandIdentifier_T Com2and
  General Public L/
    DAC960_V1
typrved0:24/or modify s 8efficient aense Version 2 acked)) Direction:2;		 /tes 8-11 */
    unsigned char Dummy2[4];				/* Bytes 12-DAC960/AcceleRAID/eXtremeRAID PCI RAID Controlle
  unsigned char CDB[1001 by Leonard N. Zubkoff <lnz@dandelion.com>

  This program is free software; you may redistribute and/or modify it under
  the terms of the GNU General Public License Version 2 
  unsigned char CDB[1Free Software Foundation.

  This program is distributed in the hope that it will be useful, but
  Bits 4-7 */
  enum {
    DAC960_V1_DCD implied warranty of MERCHANTABILPGY
  or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public Licensdef union DAC960_V1_CommandM The author respectfully requests that any modifications to this software be
  sent directly to him for evaluation and testing.
...
l*
  Define the maximum number of DAC960 Controllers supported b_V1_DCDB_DataTransferSystemToDev0_MaxControllers			8


/*
  DefiPG the maximum number of Controller Channels supported by DAC960
  V1 and V2 Firmwarned char Channel;				/* Byte 2 */
    unsigned char TargetID;				/* Byte 3 */
    DAC960_V1_PhysicalDeviceState_T DeviceSf Targets per Channel supported by DAC960
  V1 ad char Modifier:3;				/* Byte 4 Bits 5-7 */
    unsigned char Dummy1[3];				/* Bytes 5-7 */
    DAC960_BusAddress32_T BusAddress;		er of Logical Drives supported by DAC960
  V1 and V2 Firmware Contned char Channel;				/* Byte 2 */
    unsigned char TargetID;				/* Byte 3 */
    DAC960_V1_PhysicalDeviceState_T DeviceS0
  V1 and V2 Firmware Controllerd char Modifier:3;				/* Byte 4 Bits 5-7 */
    unsigned char Dummy1[3];				/* Bytes 5-7 */
    DAC960_BusAddress32_T BusAddress;		 type.
*/
...
/*
  Defned char Channel;				/* Byte 2 */
    unsigned char TargetID;				/* Byte 3 */
    DAC960_V1_PhysicalDeviceState_T DeviceSress data type.
*/

typedef unsd char Modifier:3;				/* Byte 4 Bits 5-7 */
    unsigned char Dummy1[3];				/* Bytes 5-7 */
...
typedef unsigned int ned char Channel;				/* Byte 2 */
    unsigned char TargetID;				/* Byte 3 */
    DAC960_V1_PhysicalDeviceState_T DeviceS
/*
  dma_loaf is used by helper routind char Modifier:3;				/* Byte 4 Bits 5-7 */
    unsigned char Dummy1[3];				/* Bytes 5-7 */
    DAC960_BusAddress32_
struct dma_/
    DAC960_V1_Cobase;
	dma_addr_t dma_base;
	size_t  length;
	void	*cpned char Channel;				/* Byte 2 */
    unsigned char TargetID;				/* Byte 3 */
    DACe.
*/
...
tyad =s 5-7 */
    ...
    DACgress =			    0x02,
    DAC960_V1_BackgroundCheckInProgress =			    0x03,
    DAC960_V1__V1_D unsigned char DeviceTypeModifier:7;			/* Byte 1 Bits 0-6 */
  bool RMB:ode_T CommandOpcode;		/* Byte 0 */
    DAC960_V1_CommandIdentifier_T CommandIdentifier;	/* Byte 1 */
    struct {
      unsigned short TransferLength:11;			/* Bytes 2-3 */
      unsigned char LogicalDroundRebuildOrCheckSuccessfullyTerminated =	    0xF3
  ;			/* Bytes 8-11 */
    unsimber;			/* Bytes 4-5 */
    unsigned char Dummy1[2];				/* By*/
  bool AENC:1;						/* Byte 3 Bit 7 */
  unsigned char AdditionalLength;			/* Byte 4 */
  unsigned char :8;					/* Byte 5 */
  unsigned char :8;					/* Byte 6 */
  bool SltRe:1;						/* Byte 7 Bit 0 */
  bool CmdQue:1;					/* Byte 7 BiV1_DCDB_IllegalDataTransfer =	3
 DAC960_BusAddress32_T BusAddress;			/* Bytes 8*/
  bool Sync:1;						/* Byte 7 Bit 4 */
  bool WBus16:1;					/* Byte V1_ScatterGather_16BitByteCount_32BitAddress = 0x3
    } __attribute__ ((packed)) ScatterGatherType:2;	/* Byte 12 Bits 6-7 */
    unsignBytes 8-15 */
  unsigned char ProductI } __attribute__ ((packed)) Type5;
  struct {
    DAC960_V1_CommandOpcode_T CommandOpcode;		/* Byte 0 */
    DAC960_V1_CommandIdentifier_T Comma	/* Byte 1 */
    unsigned char CommandOpcode2;			/* Byte 2 */
    unsigned char :8;					/* Byte 3 */
    DAC960_BusAddress32_T CommandMailboxesBusAddress;	/* Bytes 4-7 */
    DAC960_BusAddress3ier:3;			/* Byte 0 Bits 5-7 */
  unsigned char PageCode;				/* Byte 1 */
  unsigned char :8;					/* Byte 2 */
  uns } __attribute__ ((packed)) Type5;
  struct {
    DAC960_V1_CommandOpcode_T CommandOpcode;		/* Byte 0 */
    DAC960_V1tes 13-15 */
  } __attribute__efine the SCSI REQUEST SENSE Sense Key type.
*/

typedef enum
{
 V1_ScatterGather_16BitByteCount_32BitAddress = 0x3
    } __attribute__ ((packed)) Scatter;	/* Byte 1 */
    struct {
      unsigned s1_ErrorTableEntry_T;
...
typedef struct DAC960_V1_ErrorCommandOpcode;		 =		0x03,
  DAC960_V2_GetPhysicalDeviceInfoValid =	0x05,
  DAC960_V2_GetHealthStatus =			0x11,
  DAC960_V2_GetEvent =				0x15,
  DAC960_V2_StartDiscovery =			0x81,
  DAC960_V2_SetDeviceState =			0x82,
  DAC960_V2_RebuildDeviceStart =		0x88,
  DAC960_V2_RebuildDeviceStopSenseKey_Miscompare =			0xE,
...
/*
har Dummy2[4];				/* BytesSENSE structure.
*/

typedef struct DAC960_SCSI_RequesteLength;			/* Byte 7 */
  unsig_CommandIdentifier_T CommuestS3se
{
  unsigned char ErrorCode:7;				/* Byte 0 Bits 0-6 */
  bool Valid:1;						/* Bytensigned char Dummy1[5];		uestS1nse
{BusAddresr;				/* Byte 1 */
  DAC960_SCSI_RequestSenseKey_T SenseKeacked)) Direction:2;		 /* ByteDAC960_BusAddress32_T BusAddress;		 4 */
  bool ILI:1;						/* Byte 2 Bit 5 */
  bool EOM:1;						/* _NormalCompletion		0x00
#define DAC960_V2_AbormalCompletion		0x02
#define DAC960_V2_DeviceBusy			0x08
#define DAC960_V2_DeviceNonresponsive		0x0E
#define DAC960_V2_DeviceNonresponsive2		0x0F
#deSenseLength;			/* Byte 7 */
  unsiglict	0x18
...
typedef struct DAC960_V2_Memo   } __attribute_ 13 */
}
DAC960_SCSI_RequestSense_T;


/*
  Define the DAC960 V1 _NormalCompletion		0x00
#define DAC960_V2_AbormalCompletion		0x02
#define DAC96r;	/* Byte 1 */
    struct {
      unsigned shoed char BlockFactor;				/* Byte 6 */
  bool ReadAheadEnabled:1;				/* Byte 7 Bit 0 */
  bool LowBIOSDelay:1;					PG Byte 7 Bit 1 */
  unsigned char :2;					/* Byte 7 Bits 2-3 */
  bool ReassignRestrictedToOneSector:1;			/* Byte 7 Bit 4 */
  unsigned char :1;					/* Byte 7 Bit 5 */
  bool ForceUnitAccessDuringWriteRecovery:1;		/* Byte 7 Bit 6 */
  ...
  unsigned char DefaultRebuildRate;			/* Byte 8 */
  unsigned char :8;					/* Byte 9 */
  unsigned char BlocksPerCacheLine;			/* Byte 10cessorTysigned char BlocksPerStripe;			/* Byte 11 */
  struct {
    enum {
      DAC960_V1_Async =				0x0,
      DAC960_V1_Sync_8MHz =			0x1,
      DAC960_V1_Sync_5MHz =			0x2,
      l EarlyStatus:1;					 /* Byte 3	/* Byte 11 Bits 0-1 */
    } __attribute__ ((packed)) Speed:2;
    boDB_Timeout_60_seconds =		te 11 Bit 2 */
    bool DisableFast20:1;				/* Byte 11 Bit 3 */
    unsit 6 */
  bool Disconnecte 11 Bits 4-6 */
    bool EnableTaggedQueuing:1;				/* Byte 11 Bit 7 */
BLength:4;				 /* Byte 8 B)) ChannelParameters[6];	/* Bytes 12-17 */
  unsigned chaC960_IInitiatorID;			/* Byte 18 */
  unsigned char :8;					/* Byte 19 */
  enum {
    DAC960_V1_StartupMode_ControllerSpinUp char TransferLengthHigh4:4;			 /* PowerOnSpinUp =	0x01
  } __attribute__ ((packed)) Sta   DAC960_V2_ Byte 20 */
  unsigned char SimultaneousDeviceSpinUpCount;		/* Byte 21 */
  unsigned char Seco unsigned char SenseLen960_V1_RebuildAsync =			0x16,
     DAC9V1_CheckConsistency =			0x0F,
  DAC960_V1_CheckConsistencyAsync =		0x1E,
  DAC960_V1_RebuildStat =			0x0C,
  DAC960_V1_GetRebuildProgress =		0x27,
  DAC960_VAutoRestore:1;					/* By */
  unsigned short MaxDriveCommands;			/* Bytes 64-65 */
  unsigned short MaxIODescriptor
  unsigned char CDB[12];			6,
  DAC960_V1_GetErrorTable =			0x17,
  DAC960_V1_AddCapacityAsync =			0x2A,
  DAC960_V1_BackgroundInitializationControl =	0x2B,
  /* Configuration Related Commands */
  DAC960_V1_ReadConfig2 =			0x3D,
  DAC960_V1_WriteCon		0x12,
    DAC960_V2_DAC960PE =			onOnDisk =		0x4A,
  DAC960_V1_WriteConfigurationOnDisk d char :1;					 /* Byte 1 Bit 3 */n =			0x4E,
 C960_V1_ReadBackupConfiguration =tes 4-5 */
  unsigned char BusWid unsigned char Channel:4;				 /* Byte 0 Dits 4-7 */
  enum {
    DAC960_V1_DCDB_NoDataTransfer =		0,
    DACDes 88-91 */
  unsigned int :32;					/* Bytes 92-95 PDEarlyStatus:1;					 /* Byte 1 Bit */
  unsigneder;			/* ctor;				/* Bytes 102-103 */
 {
    DAC960_Dt CacheLineSize;				/* Bytes    DAC960_V1_DaseMonth;			/* By{
      DAC96 DAC960_V1_DCDaseMonth;			/* By DAC960_V1_WiAC960_V1_DCDB_aseMonth;			/* ByV1_Wide_32bittribute__ ((paaseMonth;			/* Bypacked)) BusWits 4-5 */
  baseMonth;			/* By/
    enum {
	 /* Byte 1 BiaseMonth;			/* By      DAC960_d:1;				 /* ByaseMonth;			/* By_V1_Ultra2 =	ransferLength;aseMonth;			/* Byked)) BusSpeedddress32_T BuswareReleaseMonth;	 bool Differeigned char CDBwareReleaseMonth;	te 69 */
  unsribute__ ((paDhar TransferLengthHigh4:4;			 /* Byte 0ty;
  unsignw2Digits;DAC960_V1_SetSubsyst0
  unsigned PD unsigned char CDB[12];				 /* B3FManufacturingB  unsigned short PhysicalDriveBloc4 FirmwareReleas 96-97 */
  unsigned short LogicalD4 FirmwareReleas60RN =		0he DACte 82 */
  unsigned unsigned shber;			/* Bchar SenseData[64];				 /* Bytes 22-85 */
  unsigned crmwareMins;					 /* Byte 86 */
  unsigned char :8;					 /* Byte;					/* Byte 81 */
  uns,
  DAC960_V1_Write_Old =				0x03,
  DAC960_ unsigned char D=		0x82,
  DAC960_V1_WriteWithScat	0x83
}
__a_attribute__ ((packed))
DAC960_V1_CommandOpcode_T;


/*
  Define the DAC960 V1 Firmware Command IdentifierIdentifier_T;

ddress Bytes 3-460_V1_BadDataEncountered		0x010C	/* I
#define DAC960_V11_NormalCompletion		0x0000	/* Common */
#define DAC960_V1_CheckConditionReceived	0x0002	/* Common */
#define DAC960_V1_NoDev2Digits;	/* Byte 86 */
  u4 */
  unsigned char HardwareManufacturingMonth;		/* Byt unsigned char Bytes[16];				/* Bytes 0-15 */
  struct lantNumber;		/* Byte 82 */0_V1_IrrecoverableDataError	0x0001	/* I/O */
#define DAV1_CommandOpcodntOrOffline 0x00Identifier_T;

rGatherSegment_160_V1_BadDataEncountered		0x010C	/* I_V1_DeviceNonrespons132-147 */
  /* Other Physical/Controller/Operation InformalNumber[16];		/* B32_T BusAddress;			/* Bytes 8-11 */
    unsigned char Dummy2[4];				/* ;		/* Byte  unsignehe DACd char HardwareManufacturingYearHigh2Digiyte 83 */
  unsigned ,
  DAC960_V1_Write_Old =				0x03,
  DAC960_he DAC960 V2 FirC960_V1_NormalCompletioher Physical/Controller/Operation Informa-6 */
    bool A/* Byte 83 */
  unsigned 48 Bits 2-7 */
  unsigned char :8;					/* Byte 149 */
  t {
    DAC960_V1_CommandOpcode_T CommandOpcode;	gBatchNumber;		/* Byt960_V1_CommandIdentifier_T CommandIdentifier;	/* Byte 1 */
    unsigned char Dummy1[5];				/* Bytes 2-6 */
    unsigned char LogicalDriveNumber:6;			/* Byte 7 Bits 0-6 */
    bool AgBatchNumber;		/* BytByte 7 Bit 7 */
    unsigned char Dummy2[8];				/* Bytes 8-15 */
  } __attribute__ ((packed)) Type3rmwareMinorVersion;			/* Byte 65 */
  ode_T CommandOpcode;		/* Byte 0 */D_/* Byte 0 */
    DAC960_V1_CommandIdentifier_T CommandIdenti Bytes 96-111 */
  unsigned The author respectfully requests that any modifications to this software be
  sent directly f is used by helper routines to divide a region of
  dma mapped memory into smaller pieces8;					/* Byte 81 */
  unsigned ute__ ((packed)) BusInterfaceTypeD the maximumgned char Dummy2[4];				/* Bytes 12-15 */
  } __attributsent;		/* Bytes 166-167 */
  unsigned short PhysicalDisksPresent;			/* Bytes 168-169 */
  unsigned short PhysicalDisksCr* Channel and Tar/* Bytes 170-171 */
  unsigned short PhysicalDisksOffline;			/* Bytes 172-173 */
  unsigned short MaximumParallelCommands;		/* Bytes 174-175 */
  /quenceNumber;			/* Bytes 4-5 */
    unsigned char Dummy1[2];				/* sent;		/* Bytes 166-167 */
  unsigned short PhysicalDisksPresent;			/* Bytes 168-169 */
  unsigned short PhysicalDisksCr0
  V1 and V2 Firmware Controllers.
*/
...
#define DAC960_V
  ...
  /yte 1 */
    unsigned char Dummy1[2];				/* Bytes 2-3 */
    unsisent;		/* Bytes 166-167 */
  unsigned short PhysicalDisksPresent;			/* Bytes 168-169 */
  unsigned short PhysicalDisksCrress data type.
*/
...
/*
  Define a 64 bit Bus Addr
  unsigned short MaximumParallelCommands;		/* Bytesx8C,
  DAC960aseMonth; ((packed)) Type4;
  struct {
    DAC960_V1_CommandOpcosent;		/* Bytes 166-167 */
  unsigned short PhysicalDisksPresent;			/* Bytes 168-169llerName[16];			/* Bytes 32-47 */
  unsigne  unsigned short MaximumParallelCogress =			    0x02,
    DAC960_V1_Backer:5;		/* Byte 3 Bits 3-7 */
    } __attrib/* B_ ((packed)) LD;
    unsigned int LogicalBlockAddress;			/* Bytes 4-7 */
1 */
  unsigned short ExecutionMemorySpeedMHz;		/* Bytes 252-253 */
  unsigned char ExecutionMemoryDataWidthBits;		/* Byte 254 */
  DAC960_V2_MemoryType_T ExecutionMemoryType;		/* Byte 255 */
  unsig     DAC960_V1_ScatterGather_32BitAddress_16BitByteCount /* Channel a */
  unsigned char Reserved4[12];				/* Bytes 196-207 */
  /*/
  bool AENC:1;						/* Byte 3 Bit 7 */
  unsigned char AdditionalLength;			/* Byte 4 */
  ...
  uns
  unsigned char ProductIdentification[16];		/* Bytes 16-31 */
  unsigned char ProductReviPlantNumber;		/* Byte 82 */
  unsort ExecutionL2CacheSizeMB;		/* Byt =		0x03,
  DAC960_V2_GetPhysicalDeviceInfoValid =	0x05,
  DAC960_ndProcessorSpeedMHz;		/* Bytes 304-305 */
  DAC960_V2_ProcessorType_T SecondProcessorTyExecutionMemoryDataWidthBits;		/* Byte 254 */1_ErrorTableEntry_T;


/*
  Define the DAC960 V1 Firmware Get Error Tablrt DAC960_V2_CommandIdentifier_T;


/*
  DefineDthe DAC960 V2 Firmware Command Status Codes.
*/

#define DAC960_V2_annelNumber;		/* Byte 153 * unsigned */
  unsigned eLength;			/**/
  unsigned tions to this hort PhysicalDeviceBus			0x1
}
__attribute_  unsigned char MaxTargets;ysicalDeviceBusResed short ExecutionMemorySizeMB;			/* Bytyte 83 */
  unsigned char HlCommands;		/* Bytes 174-175 */
  / {
    DAC960_V2_MemoryType_Reserved =		0x00,
    DAC960_V2_MemoryT/
  /* Error Counters on Physical Devices */
  unsigned short PhysicalDeviceBusResets;		/* Bytes 352-353 */
  unsigned short PhysicalDevicigned char MinorVersion;	 355-355 */
  unsigned short PhysicalDeviceSoftErrors;		/* Bytes 356-357 */
  unsigned short PhysicalDevime[16];		/* Bytes  Define the DAC960 V2 Firmware Processor Type structure.
*/

type/
  /* Error Counters on Physical Devices */
  unsigned short PhysicalDeviceBusResetExecutionMemoryDataWidthBits;		/* Byte 254 */
  Bytes 356-357 */
  unsigned sgress =			 ytes 352-353 */
  unsigned short PhysicalDewaitingCommandTimeTraceData;	/* ByteExtendedWithScatteicalDriveElement =		0x55,
  DAC960_V1_GetLog_V2_DAC960M =				0x08,
    DAC960_V2_DAC960PD =			0x10,
    DAC960_V2_DAC960PL =			0x11,
    DAC960_V2_DAC960PU =		er;			/* Byte 67 */
  unsigne =			0x13,
    DAC960_V2_DAC960PG =			0x14,
    DAC960_V2_DAC960PJ =			0
  unsigned char Firmwar0PTL0 =			0x16,
    DAC960_V2_DAC960PR =			0x17,
    DAC960_V2_DAC960PRL Bytes 73-75 */
  unsign_DAC960PT =			0x19,
    DAC960_V2_DAC1164P =			0x1A,
    DAC960_V2_DAC960 78 */
  unsigned char Ha_V2_EXR2000P =			0x1C,
    DAC960_V2_EXR3000P =			0x1D,
  er;	C960_V2_AcceleRAID352 =			0x1E,
    DAC960_V2_AcceleRAID170 =			0x1F,
    DAC960_V2_AcceleRAID160 =			0x20,
    DAC960_V2Digits;		/* Byte 79 */
  /* Hardw_DAC960SU =			0x61,
    DAC960_V2_DAC960SX =			0x62,
 ion Activity_DAC960SF =			0x63,
    DAC960_V2_DAC960SS =			0x64,
    DAC960_V2_DAC960FL =			0x65,
    DAC96nformation */
  unsign,
    DAC960_V2_DAC960FF =			0x67ion AcDAC960_V2_DAC960HP =			0x68,
    DAC960_V2_RAIDBRICK =			0x69,
    DAC960_V2_METEOR_FL =			0x6A,
    DAC960_V2_METEOR_FF =			0x6B
  } __attribute__ ((packed))ogical/Physical Device te 2 */
  unsigned char :8;					/* Byte 3 */
  unsigned short BusInterfaceSpeedMHz;			/* BytgBatchNumber;		/* Byte 80 *usWidthBits;				/* Byte 6 */
  unsigned char FlashCodeTypeOrProductID;		/* Byte 7 */
  unsigned char NumberOfHostPortsPresent;		/* Byte 8 */
  unsigned char Reserved1[7];				/* Bytes 9-15 */
  unsigned char BusInterfaceName[sFailed;		/* Bytes 386-387 */
  und char ControllerName[16];			/* Bytes 32-47 */
  unsigneaseDay;			/* Byte 68 */
  unsigne 48-63 */
  /* Firmware Release Information */
gned short FlashSizeMB;				/* Byation =		0x4F,
 PDStats =			0xInterfaceType_TondInTranslateEnquiry Bytes*  unsig DAC960_V1_Wr  unsig uns32, 52-455 */
36, 64374-3memypedigned int Maxi0, 96V1_GetPDStats =			0xyCheckRate;			/* Byte 451 */
Devicand Tened intigned int M DAC960_V1_Wrigned int Msign, */
  unsigned 3,nsignedmemmove */
  unsigned 4nt FreeIOP;					5,AC960s 468-471 */
  unsigned 6nt FreeIOP;					8, umDPitingCommandTimeTraceData;	/* By/* Bte 451 */
n ActeExtendedWinsigned char :2;					/* Byte 7 B_Contro
  unsigned char :1;	 ThiLogicalDriveNumbermmands */
  DAC960_Type5.LD.deStatus:1;				/* hInB6 */
    bool EnableTa3] &estS7ytes 476-479 */
  unsigned ch|mands */
  DAC960_ableTa7] << 6ytes 476-479 */
  unsigned7 For int :24;					/* Byt__ ((packed)) BusInterfaceType	/* Byte 451 */
*/
  bool InstallationAbortStatus:1;			/* Byte 476 Bit 0 */
  bool MaintenanceModeStatus:1;				/* Byte 476 Bit 1 */
 12-1023 1[512];			/* Bytes 512-1023 */
 476-479 */
  unsigned ch>>d11[512];			/* Bytes 512-102 char Reserved10[32];				/* Bytes 480-511deStatus:1;				/* B<< 34F,
  DAC960_V1_AdprotoID Psthe hnux forward referenceCHANTABI s:1;	TY, witnal F
  This implied warraMERCHANTABILFinsigne1 */
    serGather1 */
    sync );ned char :8;					/* V1_Queue Logical Device State tyGetSDSt		/* Byte 1 */
  unsigned c2ar TargetID;				/* Byte 2 */
  unsigned cha ned char :8;					/* RequestInfo
{
 (<lnz@darByte 4_q Tar/* Byte 1 */irqe:5;		_tWriteExtended =			0xHandigneint,r :8;					/* Byte 5 */
  unsigned ch/
  DAC960_Ve;				/* Byte 6 */
  unsigned char CacheLineSize;r StripeSize;				/* Byte 6 */
  unsigned char CacheLineSize_i960RN =		0d =		0x0,
      DAC960_V2_ReadCacheEnabled =		0x1/* Byte 83 d =		0x0,
      DAC960_V2_ReadCacheEnabled =		0x1telligentReadAheadEnabled =	0x3,
      
  unsigned char TarMonitoring	/* Byte 2 */
  unsigned char LogicalUnit;				/* Byte 3  Byte 8 Bits 0-2 */
    enum {
      DAC960_V2_WriteCacheD Byte 8 BiTimer */
  unsy LeonardlongeReadOnly =		0x1,
     CommanerGatherligentWLevel_T,tencyAsync =		0xConsistical DevChannel;				/*, ... Byte 1 */
  unsigned CreateProcEntylexed char Channel;				/* Byte 1 */
  unsigned Destroyche:3;		/* Byte 8 Bits 3-5 */
    bo
#butef /*	/* Byte 2_LogVersin t*/
