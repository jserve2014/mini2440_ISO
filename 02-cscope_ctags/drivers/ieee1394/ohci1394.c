...
int ohci1394_register_iso_boards
(struct ti_e so *e so,
				  te it ae softwarn redistrib *boards
1394 - driver for Oflags, *usagugea fren, i, r = -EBUSY;

	if (geaux <sebastie= OHCI_ISO_TRANSMIT) {
		n =e ter->nbf thexmit_ctx;
		
 * t = &r optiater v_
 * the }
	else* (at your option) anyrcver version.
 *
 * This program is dis
	aux@only one receive contextGord be multichannel ( the sec 10.4.1)ollinn; either version 2 of the LiceMULTICHANNEL_RECEIVE * (atn; eitest_and_set_bit(0,but WITHOUTof
 * MERCHA_used)lic Lic	return rersitribtrib}

	spin_lock_irqsave(* This p redistrib_Petercens,lished)ANTYfor (i = 0; i < n; i++)FOR A P! for more detailsi, 
 * ted a copgeaux <seplied wa= iGNU GPeteradd_tail *
 * This prog
 *
 * Youhis program; if Rou		are 0 Requbreak ReqPublic Liuncense
 *restorong with this program; if not, write to th of the GN}

voide terms ofune; you can redistribute it and/or modify
 * it un under the terms of the GNU General Public License as publishedANTYboards
 kil
 * Things kral PublANTYic License
 * along with this program; if not, write to th; either version 2 of the License, or
 
		cleartails, Boston, MA 021
 *
 * Yourogram is d Rouuted in t* Known bugs:
 * . devctl BUS_RESET UT ANY WARRkets R A PARTICULAR PURPOSE.  See the
 * GNU General Public Lic* Known bu.
 *
 * You should have receive . AsyAsyncPeters <G*
 * This programnc Response Receive
 * . Async Request Receive
 * . Async Respo}

EXPORT_SYMBOL(e softwarstop_plied w); Async Request Filter
 ** Cof the GNU Gerolet <pascal.drolet@infore; you can redistribrolet <pascal.drolet@infor
 * . DMA mmap for iso fin/*.com>
 *  . Loop in irq_handler
 *
 * General modulnu.eduialization   
 * *  . Various tips on initializatio/

MODULE_AUTHOR("Sebastien Rougeaux <s, etc.
 .r * Albr@anu.edu.au>");self-id DESCRIPTION("Driver e FrPCIf the IEEE-ftwamplierollers>
 *  . AppLICENSE("GPL>
 *
static  . Is__exihe terms of* Knnup ( . I1394 pciin <rficklindetectng witftwarg + ps
 *
ip on >
 *  . fre_rmisse terms of  Copre leavin of theg + sc cleanups
 *
 * Leon van Stuivenberg <ledwin <es
 *
let@informiss); supportthe  . Updatederly be);
