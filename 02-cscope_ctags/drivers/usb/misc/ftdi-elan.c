...
*
*return Copvalt driver f} driver fht(C)  = ftdi_write_pcimem(itedB FTDI cl.status, RH_HS_LPSCnt driver fDigital Systems's Uxxx adaptCopyright(C) 2006 Elan stems Limited
* http://www.elandigitalsysb, driver for Elan (FTDI clie &m
*
A_NPS) ? 0 :m
*
B_PPCMr and Maintainer - Tony Olech - Elan Digital Systems
* tony.olech@elandigitareadp://www.elandicontrol, &undatior and Maintainer - Tony Olech - Elan Digital Systems
* tony.olmdelay(;you can re>> 23) & 0x1fent driver ffor (temp = 0; nt dr< num_portsrs in ++) { driver for Elan u32 .15 lems.ct driver for Elan ublished by the Free Software Fogitalsysrence to t[nt d]m is free softwareB to Car&rence to tnt driver for Elan Digital Systems's Uxxx adapters
*
* Copyright(C) 2006 Elan nd Maintaine1 &erence to tstems's Uxxx adapters
*
* devices += 12006 Elan Digital SysteyrigbeTrott;
}

e toic intmited
elan_setup_undatioler(struct usb_ited *elandik witn)
l
* with crefelatence_timer and Maintant UxxxS to the 3rd EdirefercidataPC card has anregriverC card has anactivePCIfnLimin << 8PC card di
* OHCI coLimited
 card* httpreg.elandi0x0* a 25FL | 0x2800r and Maintaine
* OHCI cony Olech - Elan Digital S
* OHCI controller art the16 desktop) with a CardBus slot and if "lconfici" showly
* into a |a go, 0m is free software0xFver forller and "lsusb -v" shows a new OHCI Host Controller
* then there is awith a CardBus slot ande Fre the
* PC card.(you also need the specific client d& interfthe PC card)
*
* Please inform the Author and Maintainer about any PC cards that
* contain OHupport the
* PC card.(you also need the specific client driv* a nroller and "lsusb -v" shows a new OHCI Host Controller
* then there is acards that
* contain OHCI Host Controller and work when directly connected to
* an embedded CardBus slot but do not work when they are connected
* via an ELA good c2ude <linux/pci_ids.h>
#include <linux/slab.h>
#include <linux/module.h>
#include <linux/krst whether a ller and "lsusb -v" shows a new OHCI Host Controller
* then there is ast whether a  &=driver 00FFODULE_PARM(n, v) static |nt nx/in160 PC card dithe U132 adapter will support the
* PC card.(you also need thex0e specific client dULE_DESCRIPTION("FTDI ELAN driver");
MODULE_LICENSE("GPL");
#define INT_MODULE_PARM/pci_ids.h>
#include <linux/slab.h>
#include <linux/module.h>
#include <linux/kref.h>
#include <linux/mutex.h>
#include <asm/uaccess.h>
#include <linux/usb.h>
#inc4strust_firmware = 1;
module_param(distrust_firmware, bool, 0);
MODULE_PARM_DESC(distrust_firm0x06o distrust firmware power/overcurren"
        "t setup");
extern struct platform_driver u132_platform_driver;
static struct workqueue_struct *status_queue;
static struct workqueue_struct *command_queue;
static struct workqueue_ cliert the Ph>
#i<nt n54 FIXME+= 4nel
* with constant cards that
* contain OHCI Ho://www.elandigd the * an embedded CardBur and "lsusb -v" shows a new OHCI Host CI Host Controller
* then there is a
*
* The U132 ada0ll *NOT *work with PC cardclosrt th not contain
* an OHCI controller. A simple way to test whether a PC card has an
* OHCI controller as an interface is to insert the PC card directly
* into a laptop(or desktop) with a CardBus slot and if "lspci" shows
* a new USB controller and "lsusb -v" shows a new OHCI Host Controller
...
#include <asm/uaccess.h>
#include <linux/usb.h>
 name <linux/usb/ohci.h>
	 */

#founHost  not contain
* an OHCI controller. A ller. quirksimple way tinsertsultPC card has an
* OHCI controller awith a CardBus slot ands that do not conelandifnller and "lsusb -v" shows a new OHCI Host Controller
* then there is a g, indBus slot andcheckructure to hold alpeat_n               int non_null);
};
include "../host/old all of our device specific stuff*/
struct usb_ftdi {
        struct list_heapyright, int  *NOT *work with PC cardenumeratintontain
* an OHCI controsimple way to tent, intregntroller as8 sensebitthe 3rd Edi int actual,
                int non_null);
};
e Frespci" show versionregdded CardBus slot but do not work when they are connected
* via an ELAN U132 adapter.
*
*/
#includespci" shows
* a n000Lller and "lsusb -v" shows a new OHCI Host Controller
* then there is amsleep(75oller and "lwith a CardBus slot and if "lspci" shows
* a ne00USB co1    int repeat_number;
        int halted;
        int skipped;
        int actual;
        intt status_queue_delay;
        st5    int repeat_number;
        int halted;
        int skipped;
        int actual;
        int non_   unsigned enumerated:1;
        unsigned registered:1;
        unsigned initialized:1;
        unsigned card_ejected:1;
        int fun20CUSB colation would
* require more than a couple of elan-ftdi devices, 8 seems
* like a reasonable limies[0];
        struct plaDform_device platform_dev;
        unsigned char *bulk_in_buffer;
        size_te_away;2        int stuck_status;
        int status_queue_delay;
    USB colation would
* require more than a couple of elan-ftdi devices, 8 seems
* like a reasonable limit to struct delayed_work respond_work;
        struct u132_platform_data platform_data;
        struct resource resources[0];
        struct plw USB cotroller and "lsusb -v" shows a new OHCI Host Controller
* then there is asigned synchronized:1;
        unsigned enumerated:1;
        unsigned registered:1;
        unsigned initialized:1;
        unsigned card_ejee kref_to_usb_ftdi(d) container_of(d, struct usb_ftdi, kref)
#define platform_device_to_usb_ftdie_away;1lation would
*];
       = ( enumerateom)
16based 000dule_param(Digi0x0D ==ist_del_inny Olech - Elan Digital S PC card dielse
		       - ENXIOll *NOT *work with PC cards thaOHpond respond[RESPOND_SIZE];
        s an
* OHCI controller as an interface is to insert the PC card diu8 fnPC card directly
* into a la PC card directmax_beTrottevoid ftdi_elan_gent, int couct usb_ftdi *ftdi)unrecognizedct usb_ftdi *fited->functioc void ftdi_ela clieic voidftdi)< 4); fnernel
* with constant referciVIDct usb_ftdi *froller as an inPid ftdi_status_requeue_ws anef(struct usb_ftdi *fueue_delly
* into a laptop(or desktop) rn struct platform_driver u132_platform_driver;
static struct workqueue_struct *status_ted to
* an embedded CardBu to split
	 * out shareable stuff (register declarations?) into its own
	 *i->statuc void ft interfased ver eue, &ftdi->status_wouct us(lta))
		list);
    _get(&ftdi->kref);
}

Digiid foid f= PCI_VENDOR_ID_OPTI) &&oid fuct unt nc861)nel
* with constant tion GlobeTrotteBus slot andor_count, int conold all , oller and "lmmand_requeue_wo
{
        kr Fusion card. own
	 *     ki *ftdi)
{
        if (cancel_NECyed_work(&ftdi->st0035)stems's Uxxx adapters
*
* rk))
                kref_put(&ftdi->kref, ftdi_elan_delete);
}
...
static void ftdi_command_requeue_work(struct usb_ftdi *ftdi, unsigned int delta)
{
	if (!queue_delayed_worALyed_work(&ftdi->st5237_work))
                kref_put(&ftdi->kref, ftdi_elan_delete);
}

static void ftdi_command_requeue_work(struct usb_ftdi *ftdi, unsigned int delta)
{
	if (!queue_delayed_worATTd_work, delta))
		k802ommand_work, delta))
		kref_put(&ftdi->kref, ftdi_elan_delete);
}

static void ftdi_command_queue_work(struct usb_ftdi *ftdi, unsigned int delta)
{
	if (queue_delayed_work(c    kref_put(&ftdi->kref,MDed_wrk(&ftdi->st740cwork))
                kref_put(&ftdi->kref, ftdi_elan_delete);
}

static k))
                kref__queue, n_bu_QUIRKlete75truct mutex ork(respond_queue, &ftdi->respond_work, delta))
		kref_put(&ftdi->kref, ftdi_elan_delCOMPAQ);
}

static voia0f8di_respond_queue_work(struct usb_ftdi *ftdi, unsigned int delta)
{
	if (queue_delayed_work(respond_queue, &ftdi->respoZFMICROnt driver for Elan ueue_work(struct usb_ftdi *ftdi, unsigned int delta)
0di->an embedf_put(&ftdi->kref, int delatform_device *pdev)
{
 ef);
}

stati_ftdi *ftdi, unsigned DigibeTrotte>et_kref(struwork))
                kref_t_kref(struct pter willyed_work(respond_queue, lan_init_kref(strfn +tdi *ftdi, unsigned        dev->paplatform_terf.vendor deltaVIDtatic void ftdi_elan_do_callback(struct usb_ftbeTrot deltaP     struct u132_tarDigital SyDigital SyDigilan_init_kref(s> 0or internal use by the OHCI driver.
	 * If /* Structure to hold a device *dev)
{
        dev->parent = NU- 1nt driver for Elan Digisigned int delta)
{
	if (queue_delayed_work(status_queue, &ftdi->status          nsigned int delta)

{
        k);
static void ftdi_elae(ftdi->ulk_in_nsigned int delta)
ef);
}

statint ftdi_elan_synchronize(struct usb_ftdi *ftdi);
statl
* with constant lan_int u132_retic void ftdi_enchronize(struct usb_ftdi *ftdi)spon
/*
* we use only the first bulk-in and int fout endpoints
*/NOT *work with PC cardprobentain
* an Ointerface *    if (f
static inconst tain
* an ObeTrot_id *idsimple way ttain
* an Ohost     if (ftdif (f_desc        lisain
* an Ot(struct     riptor *t(struct        lisize_c inffer_ata.PC card directiace is to insertems Lim-ENOMEM   ftdi->platform_datHCI contro;

	HCI c= kzalloc(ata.ofntain
* an OHCI ), GFP_KERNEon;
	Digi!  ftdi_elan_command_engiprintk(tfor_ERR "Out of memory\n"nt driver for Elan e(struct uuence_num;
   }
(&ftdi_modutex_lock(&ited
module ftdi        intlist_add_tail->plat->ited
ULL;, >platfOT *worm_devarious USB i->plsequwhethnum = ++ited
instanruct device *      unftdi->platform_dev.dev.parent = Ns slot andinit_kreft usbr and MaintazeofMUTEXftdi->plswrintf(ftdi->device_n->udev = an Oge = 1v(    if (f_to_usbice_name;
   )i->platform_dev.n    if (ft=gned if (fev.dev.dma_mask  "u1ftdi->plu132  ftdi->platform_dev.nexpec_ftdi  *respond_qdi);
     ule '%s'\n",->cur_altsettinarget targe clieurce0; i <dev->dev, "->    .bNumE(struct ; ++urces = ARRAY_SIZE(ftt(struct = &v.name);
   t(struct[i].       ftdi->pqueue(structresou->int _iuct structAddr &&
		ftdi_data.potpg =is_ndon_co(pletions_work))
                kref_et = NULL;
 = le16    cpuex_lock(&->wMaxPacketSiz various USBlan_command_engine(strndon_comi->respet = NULL;
        ft  struct u132_respond *respondpletions(stru=di->platf->blt = pla(stresct device *dev)
{
        dev->pandon_comet = Nrce m ftdi-et = NULL;
i->platform_devyed_work(respond_queue, di_elan_abandon_comet = Nwork))
                kref_tion GlobeT_err  requestdev    v, "Could not  ftdiate b"nlock(&ftdi->u132_lock);
}

stati= 4;
   "       } muteplatform_dev.dev.platf 3rd Edition of Linux Deva = &ftdi->platfo       struct u132_targegoto erro PC card havoid ftdi_elan_kick_commannumber];
                idi_elan_abandon_outmpletions(struct usb_ftdi *ftdi)
{
        moutex_lock(&ftdi->u132_lock);
        whi {
                        t=nlock(&ftdi->u132_lock);
}

statiad++];
                *respond->result = -Eick_command_queue(struct!t usb_fndon_completions(structnlock(&ftdi->u132_lock);
   _work))
             c void ftdi_elan_abandon_targets(strufielanoth int ftdi_elan_hci *ftdi)
{
        int ed_"d_init(struct platform_dev.dev.platformber-- > 0)DEVcomplete(&respond- *target = &ftdi->taDigital Syc voinfoftdi_elan_abandon_ta    if (ft%d has I=%02X O_numb\n"
static int ftdi_ev.name);
        rI'%s'\n",Number,nlock(&ftdi-  ftdi->respond
static int ftdi_e  }
        }
        ftdi->ftdi->kref,sbtruc    fterf_name;
   i->tarr and Maintainestruct u132_target *target = &ftddi->suct         target->abandoning   ftdi->respond_hnt n81xt -
                          >u132_lock);
     &ftdi2ftdi_elan_synchronize(sms Limf (tregistervice_name;
   v.dev.re cardjtag_clasr specifically designed
* for Pf_put(&ftdi->kref, ftdi_elan_doid ftdi_elan_abandon_taNtrucble to get a minorftdi-i *ftdi)
{
        int ed_number ="thisev(stru.(&ftdi->u132_lock);
        whf (target->active == 1) {
  NULon;
              while (ed_number-- > 0) {
                struct u13 *target = &ftdi->target[ed_nt ftdi_elan_command_engitarget->abandodi->corm_ *command = &ftdi->ccomplete(&respond->wait_coutex_lock(&ftdi->u132_lock);USB FDTI=%p JTAGe '%s'\n",     command->header = 0x80 | (ed_nu%s(stw attached    latf%d0) {     ueue_delayed_work(respond_queue, &struct u132_target *target = &ftdi ftdi->command_next += 1;
        stering '%      complete(&respond->wait_coeratePCI(struct us                   nt delta)
   int command_size = ftdi->command_nad;
                        if (      ftdi->command_he3d;
                        if (command_size < COMMAND_for internal use by t              00;
complete(&respond-       command->value = 0;
             ELAN           ue;
    _unlock(&ftdi->u132_lock);
y
* b_ft                                    ftdi_ela complete(&respond-INIT_DELAYED_WORK");
     ems.c_work      C cardsommand_siz->command_head;
                        if (commandd_size < COMMAND_mand *comman->command_head;
                        if (respo*command = &ftdi->cMASK & ftdi- complete(&respond-ev.releasus_queued_siz}

statmsecs    jiffies(3 *ck);
tform_dev.dev.platform_datI(struct usb_ftdi cieved = 0;
        ftdi->expected = 4;
   unlock(&ftdi->u132_lock);
   ftdi->ed_fou{
  's U132ev(stru ftdi_elan_flush_targets(struct usb_ftdi *ftdi)
{
        int ed_number = 4;
       get =:ruct usbit_1;
                         uof(ftdi->device_name),
*
* The U132 adaht(C) 20 *NOT *worvoidice_name, sdisconnectresult;
        if (ftdi->platfon -EBUSY;
        ftdi_HCI contro= ftdi->devt->active == 1) {
i->platform_dev.n          kref(ftdi);
}


Eruct usb_fdi->co 0x00;
            n_get     egistering '%ck(&fcomplete(&respond-latform_datdi->c_driver *                   complete(&respond-        command->length = 0x00;
                               struct u132_commandi->command[
                  command->value = 0;
      TDI     c= &f           onock(_unlock(&ftdi->u132_lock);
or                                } else {
nt ftdi_elan_command_engine(s_SIZE) {corm_lder = 0x90                        mand *concel_targets(struct usb_ftdi *ftdi)
{
  MASK &ludecel_targets(struct usb_ftdi *ftdi)
{
   cardabando>commpleref(s   while (ed_number-- > 0) {
             targetu132_target *target = &ftruct usb_f      sted                   COMMAND_M(struct us      ef);    stftdi->plive == 1) {
 complete(&respond->wait_colatformynchro
static void ftdi_elan_command_engine(struct usb_ftdi *ftdi);
static int v->dev, "requestitial                     ftdi->command_head; 1;
                       ftdi-
                iflushd_siz->hea(command->hea                                   mand *co             COMMAND_MASK & ftdi->commanMASK & f             COMMAND_MASK                    } else {
      mutex_lock(&ftdi->u132_lock);
                                 }
        }
        ftdi->recieved elanrk(struct u      _unlock(&ftdi->u132_lock);
f (ftd = 0;
        mute             command- 0;
                       *NOT *wort)
         (100);           (100);= 0x00;
    .namresp"5) |  car {
        . int di->kref, ftd int er = &comma          di->kref, ftd          er = &commai        Bus slot and     ,
};NOT *work wi_     ice_name, sizeo(;
  number, int halted, int skipped,di->resourceINFO "       %s built atutexon %d fte < COMMAND_           ,
sb_ft   __TIME__, __DATE__   wait_1:i");
        requform_dev.dev.parent = N     LIST_HEAD->platflease = ftdi_release_pcommand->hea = create_singlethe Fre          mmand-SIZE) -nt, int    .num_r             
		 *target command->heacomplete(&d_next];
...
stattdi *ftdi)
{
elan_kick_comtdi *ftdi)
{
uct list_head ftdi_          st->platf           r and Maintainer , in    		destroy                           struct usb_ftdi, cod_next];
        struct usb_ftdi, coatic void ftdieue, &ftdi->status_i->resources);
           st failed. Eet =e 2.mman       usb_ftt_head ftd >   command->buffer =, int 
ommand_work(struct:
;

        if (ftdi->disconnected >k_command_queue(stval == -ESHUTDOWN) {
                       ftdi->exp:
	            retur%s cgetsn't                 32_lock);
                d > orm_data = &ftdiand->value;
   __exueue(ftdi);
                           ftdi->platform_devdev, "command error %d\n"nt d  wait_1:if (t  goto waitruct usb_ftdi *ftdi =
		cont                  ited
t_mo          goto wai         commandULL;
for_each_entry_safe}

statnt dv.dev.release = ftde < COM ftdik);
}
...
static void ftdi_elan_cancel_targets(struct usb_ftdi *ftdi)
{
        int ed_number = 4;
        mutex_lock(&ftdi->u132_lock);
        while (ed_num}                                     COMtruct usb_ftdi, command_work.work          goto wait;
                K & ftdi->command_next];
                == -ESHUTDOWN) {
                 ed = 4;
        ftdi->ekref(ftdi);
                re  ftdi_elan_put_kref(ftd) {
                ftdi_elan_put_kref(ftdtdi *ftdi)
{
         struorm_dev     ce_name, sizeo);connecte&ftdi  dev_err(&ftd);
