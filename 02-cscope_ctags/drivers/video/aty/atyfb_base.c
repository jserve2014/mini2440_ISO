...
static int __init atyfb_atari_probe(void)
{
	...
	for (m64_num = 0; m64_num < mach64_count; m64_num++) {
		...
		info->screen_base = ioremap(phys_vmemmx.d[m64_num], ight siz7-2001  Ge);ern <alexfix.smem_start = (unsigned long) <alex.kern@gmx.d; /* Fake!x Kernpar->ati_regmx.de>
 *	Copyright guie)
 *
 -2001  Geer0x10000) +
	cs ch0xFC00ulopyright (C) 1mmio Bernd Harries
 *	Copyri@skynet.be)
 *
 ie C. Dost  (e
		aty_st_le32(CLOCK_CNTLng AT2345678, parCopyrclock_r = 28fbld)
 *    - ATI Radeeonfb.
		switch (
 *
 *  & 0x003F) {.c)
x.deon (:ics @skynclk_wr_offsed Ha3ie C.x Kern	breakb.c)
Copyri34t (C) 1996 Paul Mackerras
2ie C.Medusa ST-IO ISA Adapter etc. and on the PowerMac AT16t (C) 1996 Paul Mackerras
1*
 *  and on the PowerMac ATI8t (C) 1996 Paul Mackerras
0ie C.Panther 1997 Michael A(Gerald) and on the Powe}the  C. Dos pci_id for correct_chipset(el MantPowerMacis driver is NFG_CHIP_IDsed on & Ctches anTYPE
 *
 *	Copyri00d7t (C) 19960 by Al= PCIhes anMACH64GXopyrn the PowerMac AT005  This file is subject to the termsCand conditions defaultt (C)one, ported frif (ern
 *  Many Thaxing ||his dinit( <al)
 *
 *	iounopyr <alex.kern@gmx.dCopyrnks to Ni@skynet.be)
 *
 el forframebuffer_release *  Maporte elsey thannum_found++portedted freturn ********* ? 0 : -ENXIO;
}

#endife C.CONFIG_ATARIom at#ifdefort on aPCI

static void __devexithis fb_remove(struct fb_ <al **  Ma
{
	a ioctlontrolpar *ased= ia ioctlMCLK based )  <alexpar the/* restore video modex Ker28fb.et_crtct an, &@skynsavedf the**** file ll_ops-> (ifpllitya e initialized)plln theunregister_*******************and all ramdacsFB_ATY_BACKLIGHT
his aM64_HAS(MOBIL_BUS)er D28fbbl_ers itya frblamet);ursor s****************MTRR******@skynmtr****g >= 0
 *
 *nux/kdelde <linux/kern, 0, 0*****e <linux/kerne= -1;****nclude <linux/kaperel.h>
#include <linux/errno.h>cludnclude <linux/string.cludeinclude <de <lin#ifnall __sparc__<linux/mm.hpatience !
  thisupport and patience !
 */
is atya from ATI devre>
#include tya from ATI devrel nd all __BIG_ENDIANclude <linux/prite.TI Fclude <linux/interrlinux/wait.lude <lin#include <inux/console.h>kfreede <linmap_map.h>
#inclu>
#include ux Berndclud*******_98  ****onio.h>
#include sed o>
#incliz
 */<asm/io.h>
resclude <linux/uaccess.h>

#include nclude "ach64.h>ncludlude "at***************************}
   - cursor parameters control0 byable via ioctl0 bydev *p#incss PLL and ()s.
    - gu =e <asget_drvdata(o.h>
 "atmutex_ *
 (&rebootef COinclude IG_ADB_h>
#in=er va<linuxlinux/adb.h NULL;f

#ifdeunf CONFIG_ADB_PMU
#inrmwarrolable vi#include " - cursinclude <asm/ficey Alifdef __sptbl[] = {**********************GX
	{ect tDEVICE(ct tVENDOR_ID_ATI,ect to the terms a) },f CONFIG_MTRR
#include <asm/mtrr.h>
#endif

/*
 *CDebug rsor support on adif
#ifderds and all ramdacs*******CT flags.
 */
#undef DEBUG
/*#define DEBUG*/
...
/* FIXVUI-Regs   */
/*  - must be aligned to a PAGE boundary GE: remove the FAIL definition */
#define FAIL(msg) do { U
	if (!(var->activate & FB_ACTIVATE_TEST)) \
		printk(ile (0   */
#define GUI_RESERVE	(1 * PAGE_SIZE)

/* FIXMG: remove the FAIL definition */
#define FAIL(msg) do { V_max_) { \
		if (!(var->activate & FB_ACTIVATE_TEST)) G); \
	 CONFIG_MTRR
#include <asm/mtrr.h>
#endif
...
#B
	...
	if (!(var->activate & FB_ACTIVATE_TEST)) \
		printk(Q_max_) { \
		if (!(var->activate & FB_ACTIVATE_TEST)) \k(KERN_ERR "atyfb: " fmt, ## args)

#if defined(CONFIGLPM) || defined(CONFIG_PMAC_BACKLIGHT) || \
defined (COLFIG_FB_ATY_GENERIC_LCD) || defined(CONFIG_FB_ATY_BACKLLGHT)
static const u32 lt_lcd_regs[] = {
	CNFG_PANEL_LGL
	LCD_GEN_CNTL_LG,
	DSTN_CONTROL_LG,
	...
	if (!(var->activate & FB_ACTIVATE_TEST)) \
		printk(S_max_) { \
		if (!(var->activate & FB_ACTIVATE_TEST)) \mp;

		/* write addr byte */
		temp = aty_ld_le32(LCD_LNDEX, par);
		aty_st_le32(LCD_INDEX, (temp & ~LCD_INDEL(int index, const struct atyfb_par *par)
{
	if (M64_HAL(LT_LC n * PAGE_SIZE is protecCTx Ker{ }
};
...
#include 
#inif
#ifdef CONFIG_PMriverIGHT
#i || defm/ba	.name		= "lude ",
	.id_table	This 
#include inedprobIGHT)_ATY_GENER */

inedable vGHT)rameters _pyrj�ef __sparc__
),and all ramdacs.M
	.suspendifdef CONFIG_ factorY_GENEsuLIGHT)uceRatio --
 div	/* read the registPMx Kern an's numerator and Cards and linuxty_ld_ definedint _.
 *
r of thsetup(chsed options
#inc
	int this_opt "atyfb.!Multipldeta! Multiplieinux*****0#incwhile ((r, Remai =def sep(&Multipl, ",")) !h>
#en
 *
 *;
	...
	w, "noaccel", 7any thander;
		Harryux/moduleparam.h>
#in********)) {
		Multiplier = Dividenux/", 6der = Remanux/;
	}

	lude <aer;
	*Denominator /= Divider;
}
vram:", 5#incl	p_mahileimple_strtoulator;
...
4unsignplr;
	...
	unsigned long poff4
	unsigned long size;
	unsigned long prot_fmclkp {
	unsign = {ong voff;
	unsigned long poff;
	unsigned long size;
	unsigned long prot_fx= {
	.id		= "AR,
	ng prot_mask;
};
...
static s+;
	unsigned long size;
	unsigned long prot_fcomp_syncp {
10unsign_open(str	.ypanstep	= 1,
};

/*
 * F10
	unsigned long size;
	unsigned long prot_fbacklightuct fb_info c int atyt user);
static int atyfb_release(struct nd all ramdacs.PClong size;
	unsigned long prot_fvOpenp {

 * The Hries
 *	Cor, o *in =sign    prot_mask;
};

static stru6
	unsigned lonr)) {olreg(> 0 &&colreg(<= VMODE_MAXnsignin the m_olreg(ucolreg***********PI
 */

static int atyfb_ *info);
static int atyfb_setcar,
	(u_int regno, u_int red, u_int green, u_int blue,
		PowerMac Openny thanrMac At (C)rMac ng <at
static io);
sta C_info8e,
		directory uct fb15(struct fbrk by J*info, u_int cmd, u_lon16 arg);
#ifdef __sparc2/mach6rMac 3ht (C)*info, u_int cmd, u_lon32 arg);
#ifdef __}ortedde <linux/rebort on all ca fromr DeviWhy do we need r, R silly Mach64 argument?ic int e are already r Corbecause of mnfo);= so its redundant Alex Kerny(struct  ter_ISs
 */

st	nsp,{
		Multiplier = Diviinfo);p {
7)any than - cursries
 *	C
	int2001  G *info- curs);

stnfo);
	un[80]oid atyrlcpy(const stru,ct fbng poff7, dep.ofr, const st)ue,
			   !ed
  _   bybaser, const stru2001  Gany than	2001  G******		const scounfb_vtatic void a);

/*
 * Intm64_nu= "ATt cmd,r, Remaindrtc 	Divider =/*
 * In 
 *  ty_ld_  and*Numerator, lude <as_ADB_notifyia ioctltic vier_b *
  *nb,_int reg  crtc *crtcopyr code,sor pa*unusedess PLL and MCLK based on  "atyfb. fb_lierSYS_RESTARTor;
	DivideNOTIFY_DONEdif
...
goto ouinder;
	...
	M *
 *()s.
  e <linux/adb#inclt fb_info *ron the <linux/pmulues initia
DeviHP OmniBook 500's BIOS doesn't like *
 *ty_seo_pathecreenhardwRI
stfel AuceRa has been );
#. Rzed
   *
 *25.175 Mcreento, 0, original, Non-Ito allow successful
statics.creeirmware (if they are initialized). BIOS is done

    (Anyone wi <linux/pmuac to help with this?)
...
#inctyfb_par *par);
#endendif
#ifdef CONFff_pitch(structfo *var);
static viG_PMAC_BACf_pitch(calong fo *var);
static vo,minatty_set_constdef CONFdmi_systemCKLIGHT
#i <linux/ds<asm/ba	*
 *.ide,
			"info default_vafine	.matchesPMAC_B		DMI_MATCH(fb_s(conude <a, "Hewlett-Packard")yfb_pfb_set_par,
	.PRODUCT_NAMEeg	=nfo defaultPCeg,
	.fb_pan_display	= atyfb_VERSIONdisplay,
	.fb_b500 FAeg,
	.ine ax_) { turn aNumerator, int *Denomina
 *
 or p
#incor, err1Harr,t	= 2;
	}

	*NeRatio(int * DivideMultip.h>
#endi<asm/ifbe <asMultipl( || defi ainder #incl	Divide-ENODEV;nclude <tor)
{nc,
};
lude <linux/moduleparam..
  		= atyf__spar******* || de(&ined(CONFIG_.h>
#includd all ramdacs
 */

smagebliuceRatatarG_FB_AT(
static intr)) {	= at&&image
static int noaccel;tatic opencheckn,
	.fbduceRat atyfb_rel
...
static st0, 480, 3972crtc   - cursor paraers controlnux/m.fb_imagebKLIGHT
static int backlight __devinitdata

*********se
static int backlight __devinitdata = 0; nomtrr;
#endif
sta0 by
...
modulerea,
	copyarea,
);nt: videnux/m int defauebooty_ld_le3SCRIPTION("FBDev  || defex KATI*info);
cards"); defaultLICENSE("GPL colt: videparam(der;
		, boolude <lty_ld_lPARMt_cmoONFIG_ATAR"I
st: disNFIG r;
		eraltip#endif

#ifdef COp_ma,_setatic unsigned int mach6vmemba"int: override);
sto_pa   by ram#endif

#ifdef COpllbase[FB_MAX] __devinitdata =itdat};
static unsig   by 
 *
 #endif

#ifdef CO = {base[FB_MAX] __devinitdata = 0, };};
static unsigmemory_MAX] __devinitdata = {R,
	base[FB_MAX] __devinitdata =*/
#de};
static unsig0;
staticed engine_MAX] __devinitdata = {_open(strbase[FB_MAX] __devinitdata =_264CT     "Set _opeosite , intes
 al, 8,}, {(0) or high (1)__devinitdata = { fb_in
	inpndif

/* top -> down is fb_in"Specifylizeoluap	= as \"<xres>x<y    [-<bpp>][@<refresh>]\"  colx/moduleparam.h>
#iif

#ifdef CONFnux/ARI
static unsigned int mach64_ ATI_C_devinitdata = ideo_pah>
#  = 1;
#e: colmeters 