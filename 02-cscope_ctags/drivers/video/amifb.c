		...
		if (par->vmode & FB_VMODE_YWRAP) {
			if ((par->vyres-par->yoffset) != 1 || mod2(par->diwstrt_v)) {
				if (par->yoffset > par->vyres-par->yres+1) {
					...
					while (line >= 512) {
	/vide(cops++)->l = CWAIT(h_end1, 510);s/video* lin-x/drit frame}s/videif
 * linuhips && IS_AGA95-2maxfmode + par->clk_shift == 0)s/video/amifb.c -- Amiga builtin ch* liet frameelses/video/amifb.c -- Amiga builtin2his file is bap =en
 *
bplpt0wrapt framepyrimod2(n
 *
diwstrt_v+n
 *
vyres-n
 *
yoffset)th work per dn
 *
next_* lie is based on the p +        with work by A*    } sed   *    Copyright (C) -       with work by*   for (i = 0; i < (short)  Copyrp; i++, b
 *           planevers/vi/amifb.c -- AmiMOVE(highw(p), yrigh[i]et frald
 *              2(low    Greg Harp
 *   *   amif -- AmiEND;
	}
}

static void __exit amifb work(  wi)
{
	unregister_framebuffer(&fb_infoet fby Wildeinit(          video_offGeer}

module_    Gby Wil    );        liam by William);
