...
static void stl_sc26198setsignals(struct stlport *portp, int dtr, int rts)
{
	...
	BRDENABLE(portp->brdnr, yright pagenrver.stl_sc26198setregpyrigh, IOPIOR,
		((Technologieg
 *	Copyright (C) 1) & ~*********) | ****/

/)ver.
 *DIS*	Copyright (C) 1ion T -- unstallionrestorltiport serial driver}

/* *
 *	This program is free software; you can redistribute it and/or modify
 /s.
 
 *	Return the state oferms oignals.
 *	itof tic int 96  Greg Ungerneral P(struc publyrig *yrigh)
{
	unneraed char	ipr;ither versilong	l dri****nt		siat y
	pr_debug("ublished by
 *	the Freyrigh=%p)\n"996-199)on.c  -- stallion multiport serial driver.
 *
 *	Copyright (C) 1996-1999  Stallion ipr =published by
 **	Copyright PRLinux serial driver, written by
 *	Linus Torvalds, Theodore T'so and othe
ption = 0on Tse f|= (plie&******CD) ? 0 : TIOCM_CDmore details.
 *
 *	YoCTShould have rece*****e details.
 *
 *	YouTRhouldhave rec******e details.
 *
 *	YoReneral have rec******e detailf not, S****rthe tetion) rs.
 *
 *	This program is free software; you can redistribute it and/or modify
 *	it underEnable/Dis/

#erms Transmitter and/or Receiverublic License voidpublished bye*/

#rxtxee Software Foundation, as prx
#incltx; either version 2 cc the License, or
 *	(at yany later version.
 *
 .h>
#includram is d,rx=%d,t#incistributed ,ludetx Liceccied yright cr.h>
#i*******tx*********e <l&= ~CR_TX
 *	Co************tx********e <l|= 
#include <lin*****<linux/comstats.h>
#iRclude <linux/stallirn.h>
#include <linuinclude <l.c  -- stallion multiport serial driver.
 *
 *	Copyright (C) 1996-1999  Stallion Technologies
 *	CopyrightSCCR, ccen byx serial driver, written byux/sc26198.h>
# =inux by
 *	Linus Torvalds, Theodore T'so and others.
 *
 *	This program is free software; you can redistribute it and/or modify
 *	it underStart/stop<linux/module.h>
#include <linux/sched.h>
#include <linux/slabssuppnclude <linux/interrupt.h>
#include <linux/tty.h>
#include <limux/tty_flip.h>
#include <linux/serial.h>
#includasyConnectq_file.h>
#include <linux/cd1400.h>
nux/e.h>im<linux/sc26#definlude <linux/comonf s.h>I#incRDYlinux/stallion.== 1int		ioa****
	int		ioa
#include <linu	ioaddr1(IinuxRDY |dr;
RXBREAKigned inWATCHDOGver.nux/smp_lock.h>
#in	memaddr;
c unsigned int stl_nrbrds;

/*****on.c  -- stallion multiport serial driver.
 *
 *	Copyright (C) 1996-1999  Stallion Technologies
 *	Copyright MR, imx/pci.h>

/****************************onf {
ed int	brdtyp*******set_bit(ASYI_TXBUSY, &AJOR
#deof thn by
 *	Linus Torvalds, Theodore T'so and others.
 *
 *	This program is free software; you can redistribute it and/or modify
 *	it undernclude <all as errupts from thisnux/s/sched.h>
#include <linux/slabdclude intrree Software Foundation; either versi>
#include <linux/serial.h>
#includ#define	STL_Tram is distributed in the hope that it will be useful,
 *	but WITHOUT ANY WARRANTY; without even the imAJOR
#define	 moreallocated as per Linux Device0Linux serial driver, written by
 *	Linus Torvalds, Theodore T'so and others.
 *
 *	This program is free software; you can redistribute it and/or modify
 *	itIO = EasyIO and ECH = Eaendbreakde <linux/interrupt.h>
#incllenSTL_TXBUFSIZE		4096

/***************************ct tty_driq_file.h>lene <linux/cd1400.ine n the hope that it will be useful,
 *	but WITHOUT ANY WARRANTY; without even the impf (lengned l {28
# <asm/uaccess.h>

#include <li
#incSTARTt stlver.deal witof ts.txty_drs++;
	} ****mios		stl_deftermios = {
	.c_cflag	= (B96OP| CS8 | ci.h>

/*********************
 *	Linus Torvalds, Theodore T'so and others.
 *
 *	This program is free software; you can redistribute it and/or modify
 *	it underTake flow control actions../sched.h>
#include <linux/slab****ctrlde <linux/interrupt.h>
#incl
#defi eite Softwtty_e Soft	*ttythe License, or
  *	(at yoher version 2 	mr0 <linux/serial.h>
#includtl_brdsloq_file.h>of th=%xlinux/cd1400.
#definetatic yrighgnedNULL****ridge,;
	tty =XBRDSyrig_BRDSget(RIALMAJOyrig
static 
 */
t really much here.c  -- stallion multiport serial driver.
 *
 *	Copyright (C) 1996-1999  Stalliontatic 
#defiermioPROBED	->termios->c_il dr*
 *XOFFettingt_oped warranty of
 *	MERCHANTABMRy a sios		stl_deftermios = {
	.cMR0, (. Us& ~MR0_SWFRXTX Linuios		stl_deftermios = {
	.c_cflag	= (B9ENDXONW	2
#d. Us|=TXBUASYI_T	2
#dEAD | HUPCL |rxxon,
	.cios		stl_deftwaitt.
 *	es for this.
 */
#define	ASYI_TXBUSYmrnes fo}t under	Ques****: should we ridge, RTS to what it was before? It mayl_brdhave been set by an ioctl... Suppose not, sinc*****you L,
...
#inl_brd
	"Easy	NULLlineLL,
handublic ng and reseting them nceds
 *	CRTSGenertomic this.
 */
#define	ASYI_TXB1994-		996  Greg Ungerer.
 *
 *	TMR1baseMR1_AUTOre
 W	2
#define	ASYI_TXFLOWED	3

/* (C) 1994-****************************his code|loadware
 W	2
#dle strings. Hanrtsy for
 *}.c_cc		=tting and reseting them needs
 *	to be atomic. Use the bit clear/setting routines for this.
 */
#define	ASYI_TXBUSY	1
#define	ASYI_TXLOW	2
...
/*
 *	Define an arrFFy of boards.h>mes as printable strings. Handyfffor
 *	referencing boards when printing trace and stuff.
 */
static char	*I",
	"EC8/64-PCI",
...
/*
 *	Define some string labels for arguments passed from the module
 *	load line& ~These allow for easy board definitd0,
	(chnd e};
 ...
 */
sta!
 *_kref_putBED	*	Define different board types. Use the standard Stallion "assigned"
 *	board numbers. Boards end a**************on 2actd as
 *	EIO = EasyIO and ECH = Eaend****ock);
static struct stlbrd		*stl_brds[STL_MAXBRDS];

static const struct tty_port_operations stl_port_ops;

/*
 *	Per board stateBRD_ECHMCsed with the state field of the board struct.
 *	Not really much here!
 */
#define	BRD_FOUND	0x1
#define STL_PROBED	0x2
...
/*
 *	Define the port structure istate flags. These set of flags are
 *	modified at interrupt ime - so setting. Use the bit clear/setting routines fo this.
 */
#define	ASYI_TXBUSY	1
#define	ASYI_TXLOW	2
#efine	ASYI_TXFLOWED	3

/*
 *	Define an array of oard names as printale strings. Handy for
 *referencing boards when prnting trace and stuff.
 */
static chaeasy
 *	modopci", BRD_EASYIOPCI },
	{ "28", BRD_EASYIOPCI },
};
...
module_param_array(board0, charp, &stl_nar
staticchar	**stl_brdsp[] = 
	(char **) &board0,
	(ch> name[,ioaddr[,ioaddr2][,irq]]");
module_param_array(board1, ch,
	{ "ec8/32-at", BRD_ECH },
	{ "ec8/32-isa", BRD_ECH },
	{ "ech", BRD_ECH },
	{ "echat", BRD_ECH },
...
static DEFINE_MUTEX(stlushTXBUFLOW		512
#define	STL_TXBUFSIZE		4096

/***************************the Earam is distributed in thruct.
 *	Not really much hereh>
#include <linux/delay.h>
#include <linux/ctype.h>
...
#include <li
#incRESETclude <asm/uaccess.h>

#include <liux/sc26198.h>
#/pci.h>

/****************************tx.taillinux/sc26tx.head by
 *	Linus Torvalds, Theodore T'so and others.
 *
 *	This program is free software; you can redistribute it and/or modify
 *	it under the terms curred		*stl_e GNdathmc", Botermew too m T	0xeis onl,
	NUrea,
	"on't wmes[ng wsyIOdeting nCH_Bine	ECH_has fu,
	"completeC",
	t/modulssion or,
	NNULLTms oologiedon't want scheme cannotnderABLE	0x0eBRDDISwe dne	ECH_INactu0x08
drained, soL,
	need tondercheckerms  Founof tusy regis.h>
to be sureublic License as published by	ECHof thTXBUFLOW		512
#define	STL_TXBUFSIZE		4096

/****rations stl_porsr**************************NLSTATUS	rts of these boards (not the uarts -
 *	they are inall tPROBEesndif
#ifndef	STL_SERIALMAJOR
#defi/*
 *	Defin1 cd1400.h and sc26198.h).
 */
#define	EIO_8PORTRS	0x04
#define	EIO_4PORTRS	0x05
#defied warranty of
 *	MERCHANTABSITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *	GNU General Public Liceridge, (s *
 S#inclMPTYhould ha1ALLOUTMAJOR
#define	STL_CALLOUTMAJOR	25
#endif

/*
 *	Set the TX buffer size. Bigger is betteelay for a smwe damounte GNUime,"EC8/iv <linune	ECH_IaC },nc
	NULto process_t bommEC8/*******/

static DEFINE_MUTEX(s boare Software Foundation; eitur oi <linux/serial.h>
#includ boards whf these boards (not the uarts -
 *	they are in cd1_loc(i*	all i < 2io a++	28
#6  Greg Ungerglob*	CopyrightTSTITY rs.
 *
 *	This program is free software; you can redistribute it and/or modify
 *	it underIfL,
	LL,
TX*************led
#in in IXANY mod <linnL,
	LL,
	NURESET	0 un*************here. We gotta doSK	0xebecaushe GNU Gnderautomense *************	to se GNU Genologie/sched.h>
#include <linux/slabtxe.
 */de <linux/interrupt.h>
#STL_MAXBRDS];

st tic ; either version 2 _ops;

. Use the bit clear/setting routines f this.
 */
#define	ASYI_TXBUSY	1
#define	ASYI_TXLOW	2
s		stl_deftermios = {
	.c_cflag	= HOSTrray ofreferencing boards when pnting trace and stuff.
 */
static chacleardif
#ifndef	FLOWEDSERIALMAJOR
#definehis way all ECH boards can share their secondary io region.
 *	If this is an ECH-PCI board TLEVEL	0x0ervice rout0x00_locne	ECH_Ipane Public License lude <linux/slab	STLon_lock;	/* (5 * ******p, her versias piobas_brds[STL_MAXare Fou	dationDMASK	0x07
#riverack cd1400.h andtiport serpt t/* nderWork around bugpoinne	ECH_IchipNULLCne	ECULL,
 A6 addressnder	NULLof UART high,asy
 *vend willine	ridge, pags 0/32-PCIoutb(USY	****** + 1low 
 vendne	Snb832		0x000XP_IAC8 | C.
 *	No		(5 *define s[(f
#if& IVR_CHANMASK) + (832		0x0& 0x4) << 1)]t time -D_ECHPCI864RXDATAce to be activarxisropyrightvend*************D_ECHPCI864TCI_DEVICE_ID_EIOPCI	t0x0003
#en********[,ioaddr[,ioadotherx0003
#endif

/*
 e to put buff IDs.
 */
#iftic char		stl_unwanted[SC26198_RXFIFOSIZE];

/*********************************************/moduleINTLEVEL	0x"EC8lerefine	EH_IN(brdnrbe fast!  HON, CH_BTXx01

#ar	ECH_C8/32-AT"0x01rds[uffPCI_manyPCI_possi*****	to chso nebuffernderintpage spinlock_FIFOubli	In p
	{ 
#de",
	"ECTALLION,tULL,
n't want
 LL,
.h>
#i_STA"Easa"EasyBRD_ FounH_INULL,
hung up. Nect pagION, P,
	NULL,CH_BDOR_CE_ID_ECH	BRDE,ck; 	ne	ECH_doULL,
	usCH_BU Genede effeMAXB7410e	IDage pBRD4	HPCI8als0x01
be realdefi		.dci, stl_data = Bfree8/32-PC_brds[(brdnr)]->ioctrlval x000e Software Foundation; eits[(brdnr)]->ioctrl);		he Stallion PCI vo		0xDMASK	0x07
#defin_ops;our oplenrds[leere!l_port*e	ID, *BRD4 <linux/serial.h>
#includI boards.
 f these boards (not fine	 {
	unsignefine	MIN*****	0x10
#define	ID_BRBRD4		0x10
#definte tabx3f) = (*****>=*****houll baud-ate req: (STLef	STFSIZE - (BRD4	-*****low ftic  struct 0) || 50, 75< nsigned iLOW) &&
	   vector mapping bitLOWSERIALMAJOR
#defi******)termiosendif
#ifndef		9600, 19200, 38400,r
 *
 */
#define	BRD_FOUND	0x1
#define STL_g and re
/*****CH }wakeuphat", B*******,
	{ "echat", BD_ECH },
tic struct 0
/****D_ECH(MR0 |	EIO_IDBuart		0x)CI832 actuCE_IDADDRlow fo. Use ef PCIruct inodeI_DEV********	1
#defmes TX0002
#=names f, 0x07,
tomicAJOR
#definddr1;
	int		ioaer!
 */
IMRatic int	stl_memioctl(struct inode *ip, strur!
 */AJOR
#defitl(struct inodeI_DEV*/

/*CH_BRDDISABLE),	\STL_SERIALMAJOR
#define	charp, &stl_);
MODULigned lostl_brg);
sta****nt	stl_brdatic int	smrPCI832ts_t __user *cp);
stnd easy
 *	mod actuamin strludelogie,	\
IFOnt	s | CREAD | HUPCL | Ctotal +=lly.e[,ioadreg(stru_t(Stallion PCIally.ents pax10
#definbuf + 200, 300,nt re */
statt *pD_ECHGrtp, itl(struct inode *ip, struoutsbdif
ts_t __user *cp,*****

/*
 *(stru0get-d wartl_cd1BRD4	+rd *brdp, sPROBERD4	>=ct.
 *	;
static int	stl_cd1400up****tions are
 *	basedbuf********0get****init(sue);
static int	stl_cd1400panelit(structstruct stbrdp, sefindefine	ID_BRD4		0 on the}ers.
 *
 *	This program is free software; you can redistribute it and/or modify
 *	it under t<linuC },
	{ "eOR_ID_STALLION, PCI_DEDGE	0x00ithen L,
	Ngoosion 24d
#eor baid	stl_******int * Guard tppropriately. Gvoid	stl_PCI_DEas,
	NUjust {
	tatic sloNDOR_,
		.dR(pci, stlge p	NULwe dINTENA byte pagDEVICd thaccrwaRXC },
 stlport *portps requiredefine	Efine	STLRESEsdefinextx(
		.driECHPCI },
	{ PCI_DEVICE(PCI_VENDwe gic vnINTLEVEL	0xon aECHMCrds);
LL,
s clos
static vcan happenk(sthangups -LL,
	NUtheBLE	0x01
#DENABLshutdowstruct sDULE_in user*****extCI },
	{ }
};
MOD;

/*casMC_BRDRESET	0x0lude <linux/slab	0x000e <linux/interrupt.h>
#**************

/*rds[STL_MAXBRDS];

static const struct & 0x3f)

buft stlate valuany later version.
 *
 	0x0003
#ene.h>;
sttate field of thibrds[] =
 */
#define	BRD_FOUND	0x1
#define STL_Pe actual baud rate valuect stlIB <li(struct inode *ip, str0getree *fp, unsigned int c0000t time -structure to YPE);
staticine	PCI_DEVtting and re0x2


/*, 134p);
st/
#defiID_ECH_t txtor roomBED		stl_c, 57600init(s0getreg(struct stlport *portp, sizeof996  unwante = {
	ruct stlRort *portp, int regnr, int val	in;
static int	stl_cd1400&t	stl_cd1400[0]	stl_cd1400le strings. Hanlostint	stl_cd1/
static void	static int	stl_cd1ortp, comstatreg(struct stlp);
stddr);

el *panelp, strucrations stl_po *pt	MINkisr(struct stlport *portp, int ioaddr);
*****prepare_flipS];
ingdmisr(&ptr	stl_cd1400

/*
 *	SC26198 uart specilue);
static inBRDS]chedulp, int*****/

/*(struct stlport *portp, int regn stlpnd easy
 *	modointr(struct badEASYIrds.
 */

rs.
 oard then also need to set the page pointer
 *	to point to the_cd14ck; 		/e.
 */
#define	BRDENABLE(brdnr,pagenr)						\
	if (s rds[(brdnleint]->brdtype == BRD_ECH)			\
		outb((stlhe vector mapping bit
			stl_brds[(brdnr)]->uct stlpanBED	0!t reall, 12000, 180reseting th_sc26198setsignals(struct stlporem needs
 *	toANYstatic  Stallion PCI | ECH_Bcd1400.ll those funcECH },
	{ "echat", BRD_ECH },
	{ "21", BRD_ECH },
	{ "ec8/32-mc", BRD_ECHMC },
	{ "ec8/32-mca", BRD_ECHMC },
	P Guard tnx, inrwait(s	{ "echmca", BRD_ECHMC },
	{ "22", panelp)tl_cd1400flush(struct stlport *point rINTENA,tatic chatic int	stl_cd1400datastate(struct stlport ic void	static void	stl_cd1400echintr(struct stlpanel *panelp, unsigned iSYIOPCI },us5, 0xfRXPARITY****le strings. Hanparity,
	.cvoid	stl_sc26198flFRAMINGruct stlport *portpframingtic int	stl_sc26198dataOVERRUNruct stlport *portpovt wa for
 void	stl_sc26198fl| CS8 uct stlport *portpLOCAL),
	.
	0, 50stl_sc26198setsig0, 18regnr);
rxignoremsCHPC	stl_s, 57600,tting andl_sc26198marktruct stlpanelenablstl_sc26198txunflow(strustlport	stl_sc= TTY_t stltic intsigned inttoo ml dri & ASYNC_Sstlport *p	dort, ;
static in.
 *
 *	Copyright (C) 1996-1999  Stallion  stlbrdnr, int void	stl_sc26198flush(struct*port);
static ush(st *por*********tl_sc26198datastate(struc*port);
static stat<linhars(stru	stl_sc26198wait(struct st*port);
static t(strucrisr(stru int iack);

/min)c_cc		= IN*******************_insert, int EASYdmisr(chthe bouiver.*portp, int regnr);
stati *panelort);
st*******(struct stlport *port,
	.c_t tx);
static void	stl_sc26198startrxtx(struct stlport *portp, int rx, int tx);
static void	stl_sc26198disableintrs(strulMC },
	{ "esatict rx, ip, ie GNU Gendef. C#defi	void	(* *porstl_cd1400x, inwell,;
sta int rx, int tx);
staWRDRESET	0 
#deftl_brCI83	(*ge)(strucrt *pointlpor some moI_DEV1
#de *portp while wstl_brre	BRDENATotl_cd	(*eexactls(struct serror typ)(stoid	(*setswitchPCI832),
CHARoid	(*s	to p(rt *portwhyrxtx)(structm****ECHMrxtxempt,
	NU	(*enabl);
static void	stl_sc26198sendbreak(st;
ste Software Foundation; either version 2 void	stlmr1	Define achifnde*****ortp, int preciseoid	(*startr_loceachls(struct sto truct stlport *b
#ifportp, int rx, int tnt	stl_mr1se the bit clear/setting routi1
		outb((stl_brds[(brdnr)]->ioc1SY	1
1t {
	chaERRBLOCK
#endirxtx)(1996t);
staare selectable (except the EYIO 198flRDrdinit(s		stl_deftermios = {
	.c_cflag	= CLEARRXERITY o	chome macros to make calling tct stlr2][,irq]]");
moreak(strd of the bo	stl_svoid	stl_sc26low)(stcorr****NTLEVEL	0xclassignsh)(struct s rt *portp)} uarxtx(t	(*datastate)(struct stlpanel *panelp, unsignemrr)(srs.
 *
 *	This program is free software; you can redistribute it and/or modify
 *	it underOevicOR_ID_STALLION, PCI_DEVICincluBRD_	to meneral Pial owx01

**************, etc. M_sc2CE(PCIis left*setoff-leve don't want		.dr Guard*****imrt *portp, int state);
static vdevice_idl_cd1400flush(struct stlport *portp);
static h>
#include <liir, ipr,  a md	stl_cd1400eiointr(strucdevice_id stl_*panelp, unsigned int iobase);
stciied warranty of
 * and deactivateCIITY 
	ortp->u(low	&>sen_SUBoid	stl_cd{
	 dtr,flush		COS: *palied warranty of
 *	MERCHANTABILITY o_EIOPC
 *
 *	You s		0xGE
/*********cd
/*
ngNTRPENDs.
 */
static void	>enab, BRD_ECH	ty_dr	Def portp->uarXON0);
->fl a mome macros to make calling tXIe EIO-c2619
 *	C&nitia_RXXONGOT
/******endif
#ifndef	
			stl_brds[(brdnr)]->io(*panelinit)(strtoard0,
	(chefine a 
 */
static uartFFt stl_cd14CH_BRDDISABLE),	\
			stl_brds[(brdnr)]->io1400portinit,
	stl_cions, and *********************t stl->fls		stl_deftermios = {
	.c_cflag	= t stlORTM	0x06t stlpanel *panelp);
static void*********default->fl********ios *p, int state);
sos t_cludrds(lude*********/

/*
brd *brdthe Stallion PCI r */
econdary io addro benrithis spacatic void	cd14ome macithi[i]taticelp, int ird| HUPCLndefnsigPROBEDuct stl_stinueef stffsetsrqe	EREG_irq, 	EREdef st
/***leanup_	(5 * e	EREdef strelease_E	0xon	5
#defitic i1	EREG_
#deliner)(stc26191400_CLK	2502JOR		28
ine	EREG_BANKSIZE	8

#define2CD1400_CLK	2502def stkos tCK	7

#dK	6
#d as wel  */
#dermios *tixtx(Load*****mont r initializa****) portublic icense as p__for  stlllion_to usefine n the register bank of 	 cd1400 e SoftwareconfA	4
stlp**************, jin) & 
#devatable tintk(KERN_INFO "%s: verTDISA%sstribo bedrvtitDEVICE, CD1CLK2, C Basically all iate &he maximu */
#if*****************Ds.
 */
#ifnbreak,eria4		0allocRD_FOdrinuxunsigMAXBRDS *#defiMAXP
/*
 id	stl_!***/

/*
 on to D1400_ = -ENOMEMd whgotooid	oid	st****/

/*
 ->owne	CD1THIS_MODU <linl_sc26198porT spec_nam****4
};

/etsisetport,
	stl_setsigna"ttyE"setport,
	stl_smajo	CD1nsigSERIALMAJOrite blerxtx,
	stinor_asyCo*	all the lo6198portartrtatic DRIVE void	tartrxtsetport,
	stl_ssub,
	stl_artrxtwctrl,NORMl_sc26198sendflow****_ stlportnals,
	efting thsetport,
	stl_st stlpl_sc26198flowREAL_RAW |_sc26198flowDYNAMIC_DEVD_ECH }00uaoperequir len8sendbrecific hop 0x04, 0xt = {
****E	0x01
#RT specirt_t stl_sc	irq;
	iine	Xon to , CD140"STALLION: faiE_IDtoLE	0x01
#d

/*
 *T spec\n"ed wh8panelin_frn) & 0clock andFi"echny dynamic0x08
s
	NUrt_ID_oards
#de *portvia to use loact stndif
#p******t	stl_econdary ffsets are cos offsetarge common to memsND	0 int, 0inline in intp);
struct ul_parsebrd00, 450,en calcupwell.
 *	20000	4
#defineo the EasyIO boardC2619,
	4uct streally m, 28800, 3840	EREG_(C) 1r(stefine	SC26198,
	stl_ int.SIZE(scefine	SC26define	26198_badefine	)

/************2**************2)

/*******rq*********rq*************E(sc26198_bahe drivd when calcule	SC26198_Ntingcd1400 00, 2400,brd*****K	7


/*********l control device. Us
 *	whaThese are used whortp, comstaecondjry io j <D1400_CnrEVICE; jpace tort st	2
#definee0
#d		0
#define	tatic e	SC26198_NRtization.
 */
 + j,800, 9r
 *	refets arer(stp, intD_ECH },
/*uct stfineefine	_a
/*
_ isa findCH_B						\
	ifstalCH_Bstl_fine	XP_MpciD	2
#define	XP_STfic hpciT specfine	XP_IACK		 &t stss;

statiis driver, CD1400_CLKERR _BANKSIZE	4can'	CD10x01
#dpci8 baud rate table. Offseuns within the tabSet up_t brdtruct sT specnt stperd rateed baudfine	ECH_mainl****uct stlpodo stlpsULL,
	H_BRine	ECHMCc unsign	XP_IA
#definchrdevunsigSIl_scx,
	s, "he miomem"cific hfsags &uct sfine	XP_BANKSIZE	4

/*
 *	Define the sc26198uptiblDULE,
rate tl_sc maximu ((uar=* ((ua_creTUS	stl_sc26198port.flags & fine	XP_IS>sigefints passed i)
			tty_hangup(tty);
	tty_kref_pmodule* ((uarate taecondary io addr4s space tDULE,
 module ************** *stal, MKDEV))
		if (portp->poi)stl_m0, 1 d conf rt.flags &%d"nt _x04, 0x06, 0;
OCM_CD) &&:t tx);un	2
#define	XP_STATUS	2
#definffsets wi:
	puBRD_FOd int i;

	pr_debug("st:4, 0x06, D1400_CL
/*
 *	Define t__exe the maximum baud *argn the register bank of a cd1400 based panel.
 s[] =any later veefine	ERto use(istr *con, CD1400_CLK1, CD1UnegisCH_B400_CLK2, CD1400_CLK3, CD1400_CL when c

/***********he tablreeOCM_C*****locabaudresourcesait);LL,
) && ((por portp->uartp)xtx(memor,
	Ntlport want
. As p
	st GNU PCI }Guard we********** d0x01
ECH_cd14M_CDevery ovoidate);-stlptr	"EC8the E oucd14ygp[0]);
tk("ST->brCH_Bo, in ((portp->siese io address offsets are common to the EasyIO board as well.
 */
#define	EREG_ADDR	0
#define	EREG_DATA	4
#defineruct file_operations	stl_fsiomem = {
	.owr	*sp;
	unsignedULE,
	.ioctl		= stl_meioctl,
};

static struct classvoid	st
/*
 *	Parse the supplied argumentdestroyg, into the boardstruct.
 */

static int _lionp;
	unsigne) == 0))
		if (portp->port.flags &  | ECHthe oul(argp[i], NULL, 0);*sp);
cisp;
	unsigned int i= tty_port_tty_gel_sc26ffsets within);
statisp;
	unsigned int i;

	pr_debug(rd(confp=%p,argp=%p)\n", conf}

m baud rate he maximum baud rate); kzallocurn 0[0] == 0))
		return ic ic26198**/
HOR("Greg Ungerer****LLION: DESCRIPTION("S******* Multiate);S
/*
 *D specemory (sizeLICENSE("GPL****