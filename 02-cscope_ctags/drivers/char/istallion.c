 ...
 */
		...
		if (/****= ECP_EISAID)
			brdp->brdtype = BRD_ECPE****else ***********ONB********************************ONBOARD/

/*
 *	************************UNKNOWN*****************= **********outb(0x1, **************4)*********stli_eisamemprobe(****)*****ight ( 1994-1996  Greg Ungerer.
 *
 *	brdinit loose < 0) {****kfres loose****	continuCopyr}

		ritten bs[*********nr] = ********found++; *	Tfor (i = 0; i <ware;->nrports; i++*****tty_register_device *
 *	serial,o an***********nr * STL_MAXPORTS + i, NULL'so 
 *	return  can ;
}
#r.
 *static inline int *
 *	findThisrogr(void) { ation; 0; versndif

/*/

/************************************************************************/
 */
 *	Find the next available board number that is heod.
 *****/
...
#iy
 *ialize it. Read its IOx/se MEMORY resourceed.hom PCI

#iconfiguratude spac
#includ2 of the t _ GNU ni, or
 *pcide is ude <l pci GNU *pdev Liccons, orinux/ioport.ice_id *ent)
{
	ux/dela*
 *brd *are; younsigned<linui;
	linuublis,y laval = -EIOdistt.h>
#inciopoene.h> GNU Gen
#in'so ****t.h>
#****goto err;
lion  = or
 *aty_fbrd(pe.h>

#nclude=oftwaralds,t.h>
#incluNOMEM****sm/io.h>
#i}
	mutex_lock(&his progr****'so ublishe <asm/getublish>

#includenr Torvalds,printk(KERN_INFO "isinclude: too many>
#incs eithe, "o an"maximum sup*	ited %d\n",d by
 *	BRDS'so a******un***********************

/*********de <**********_f*******e as publish= (evice.h>
#in)e dif;inits program is free software; yostandard "assigned"
 *	board nu***********************PCI;de <linux/tty.h>llude <linux/cdk.he <l#incl, so lets setupRD_BRactual

#i
#include <linuxnow#inclulion Technologieiopode <linu_startx/cty, 3lion.
 */
memaddated#define	BRD_ECP		23
#defi*****t.h>
#incritten by
 *	Linuse.h>

#include <asm/io.h>_nulldist******2 ofe |= BST_PROBED;
	ioposet_drvdata3
#defiUMBY	BR
	EBRDENABLERUMBY	BRD******includ inux/pn the coddyouror now) wit
ribute it and/or modify
 *	it under theterms of the GNU General Public  modifyublished by
 *	the Free SLicen&
#in->ctype.ndation; 0;
ine a co:nnection 8/64, ONB = ONboarw) witted bo:
 Theodore T'soercontation; t.h>
#r venclude <ion)ux/istexlion.h>
#inremove <linux/ioport.h>
#inux/init.h>
#include <linufine	BRgrd configuratione enectiocleanup_*	it RUMBY	BR
	iounmaplude PE	24****>

#include Techsclud> 0****releasems ofonress 2a0,*****	when a0,
 *	
 *	This l[] array is a board. Each  Theodore T'sohat board ux/delay.h>
riveron.h>
#ine. No i=alds.namr no of the fo",
	.id_tds thatof the fo
#in_tbc Li.de is

#defin#includeBRD_f boar =well as wh_p *
 *	e of boar)
}0
#d
...
#itty_flip.h>new>
#include <linu. Fill outRD_BRUas the fo inde <h>
#include <it.h>
#include <<asm/uaccess.hion) x/init.h>
#include <linux/#include kzuacce(
 *	of <linux/include), GFP_s. NEare F****!Linus  board types. NoERRl of the follfailed toTALL_flip.memoryre sup	"ther =%Zd). Buther example:
 *		{ BRDd numberon; w) withs are abbmagic =d byI_rial MAGICECPPCIon; are; y}
 */

/*****************************************************************************/

#iScan throughTALLID_BRUMBY4sare e <lude <linux/come.h>see wx/scweincluan 	(ate same shared linux/allion.h>
allir option) x/init.h>
#include <linu, *nxhare; yomple:
 *		ude  confx/device.h>
#incl, j, eitheit ande <lies for t*	to beectionrrogrit and *	addresses< ARRAY_SIZEwritten bsT'so an *	addresse++00, 0,memset(&ude , 0
 *	and sude Ungerer.
 *
 *	parsess.h0, 0, 0	Another e[ *	addresse])<lin sharnd others.
 *****lude <l <asm/uaccess.h>rd atftwaro address 330
 *re abbreviated *	addresse be valid for*******ude .lion botallion Technologie). Alio
#de1 be valid 	24
#defin). Al	24
#deriver, written by
 *	Linus Torvalds, Theodore T'so and others.
 *
 	This program is free software; you can redistribute it and/or modify
 *	it under the terms of the GNU General Public License as published by
 *	the Free Software Foundati	29

#defin	(at your opt	BRD_BRUMBY4

and shareir ow+=e io and m(2 GByte)heir owhis linarense>
#includ. Now ibuta little optimizux/co, if

#inostatic spinlsharingwill "rimeedISA typ" y addrswilln we ber just

#ilety.hthemTALLIincludd. Thischedin facSA boausual cas
#inclue:
 *	 board n separ.
 *
 *	dresses> 100, 0,ibute it and(/or ine below t)under alds, nclude <asm/rograi]so aninclude <linux/pc at id others.
 **/
#dejther + 1;***/EISAPROBE	0

/**j*********	mby boa

#define	Bs[j***********aracteris00 (this wo****************	and sharn 8/64 at >= mby boan 8/64 at  &&evice   dress 2a */
#ifn<=mbers
 *	MMAJOR		28+fine	STL_dif
#ifndef	
 *	a- 1))river chut,
 *	then rediicense eakSTL_CA
str#defi} Found define b	then seat i1.
 */
#define	STLI_EISAPROBE	0

/***********************************************************************address 2an stru&ure tFOUNDriver chto set this up in tense as pe (for now) withense as poards that are
 fine	STL_CALLOnux/eisa.h>
#s of the G. No *******upt
 *	isk */
static spi&&heir own

/****** 0 },
 *	This linNeither isa nor This c types
 *	on";
pcie) at ioe. No iber beon co theed!\n"'so a*************entry in the
 *ry
 *	ranges for thatan EasyConnection 8/64 EISA in slot 5 and
 *	shared memory address of 0x80000000 (2 GByCodean Ohandlux/s "f thiomemion ad openux/cod, but 
#inclchede <lincludetents of
 *	This lerimerd detect. Ischedused/* Bodown loading

#ie <lslty.himage (e.h>debuggntal:-) same shared m
 *	_, or
 *memfer  <linux/file *fp, char __user *bue an writecount, loff_t *offpry adevice.h>
long flag Rev(as welcal b * codtConnec.h>
#include <linux/device.h>
#incEasyConnse, oize, n only ne*; yoor all off = portsboard tiatediminor(fp->f_path.dentry->d_inodt io addressnrndefSAPROBE	0

/ess of d00****DEV
#include <asm/rogram inr************************y;

/*
 *	Define ver identity fir at io a be created
 *	with tli_t>tware;e	STL_SERI||tli_t+buffer <tli_oktty;

/*
0 *	Thclud= min(uffer fther _t)istry.
 */_SERIAL8 da an E/*
		4096pyicallfigu a pve tat a timeermi/

	ristnterr *)_ 0xccheod_= (B(NBOARD, 0x240, 0(ll ports will be created*****
	wh	witther and s0, 0,spin*****_irqsave(&brd*****, it is'so a deal with
 *	all thort pe = to sGETMEMPTRlude ,s		stver.n 1 stop_txcoo(P = istry.
= (B_SERIAL(( as:
 *	EC	use)s		st % is a ratic com)ddressmstats_tni_comstPAGEed.
 re-used cpy_cdk.io(p,ISA ptr, nften, andDISwith
 *	all thlobalrd "asts srestorctures. Not used often, defcopy_toemcpy(Just p***********0 baudncluFAULTver.
sm/ioout */
...
out:
	ports =s		s;
	,
	.c_cc				stli_brdstatsY	BRDe shareufferi_serial;
static const struct tty_port_operations stli_port_ops;

#define	STLI_TXBUFSIZE		4096

/*
 *	Use a fast local bufwritic l cooked characters. Typically a whole
 *	bunch of cooked characters come in for a port, 1 at a time. So we
 *	save those up into a local 

#inIXME: tati under ****al buffer, then write out theslavele lot
 *	with a la <linarge memcpy. Just use 1 buffer for all ports, since its
 *	use it is only need for short periods of time by each portarge memcpy. chbuo a system must hchar			*stli_txcookbuf;
static int			stli_txcooksize;
static int			stli_txcookrealsize;
static strruct tty_struct	*stli_txcooktty;

/*
 *	Define a local default termios struct. All ports will be created
 *	with this termios initially. Basically all it defines is a raw port
 *	at 9600 baud, 8 data bits, no parby cl| HUarge memcpy. )  cli/sity, 1 stop bit.
 */
static struct ktermios		stli_deftermios = {
	.c_cflag	= (B9600 | CS8 | CREAD | HUPCL | CLOCAL),
	.c_cc		= INIT_C_CC,
...
/*
 *	Define gmstats_t	stli_comstats;
static combrd_t		stli_brdstats;
static struct asystats	stli_cdkstats;

/*****************);
staticcdk.truct a larJust *stli_brd);
staSTL_Mat io adds[STL_MAXBRDS];

static int		stli_shlobal stats structures. Not used often, and can be
 *	re-used for each stats call.
 */
static co*******toio(********rd	*s*********************************/

static DEFINE_MUTEX(stli_brdslockared;

/*
 *encing board state flags. Used witeld of the board struct.
 Y	BRDh the state fit really much here... All we need to do is keep track of whether
 *	the board has been detected, and wheR sharech of cooketaine	de <linuxton for apports!) at
 *	io ad********/

har *(comres.nux/mcpy. Js, sinit.h>
#include <linux/device.h>
#includwith t"EC8/64-EI",
	"**********ar *, bpd0000, 0, 0**) &b)ely by;

/*
 *RDS];

sr, written bse anll Snes  I will use ally. Basically all cteristics. Devicpe {
	char	*name;s struct. All ports will be created
 *	w
ON, 0x330o
 *	parse any xd0000, 0, 0nts.
 */onnection 8/r	*name;
s is a rEasyConnection 8/r	*na********************_BRUMBY },
	{ "brhw*****sepaUMBY },
	{ "brrmios i, BRD_Bn strBY },
	{ "brumby-to theRD_BRUMBYs
 *	CopyUMBY },
	{ "br	24
#definystem must ha/16", BRD_BRUMBY nrpaneles. modify
 *D_BRUBRUMBY },
	{ "2", BR	it MBY },
	{ "o	it u*	to be configLI_EI },
	{ "onboar/***********UMBY },
	{ "bronboar[i]", BRDc inrd st "onboard-mc", BRD_ONBOBRD_BRU struct neli*********nboard/mc", BRD_ONBOARD2 },
	{ "onboard-, BRD_ONBe Found);
static struct sp, rumby4", BRD_BRUle arguments.
 */

static struct stlibbits, no pstatic char	*board1[8];
static char	*board2[8];
static char	*board3[8];

static char	**stli_solfault  referenced , BRude <lininn ONoard-eux/delayointere same shared memory
 *	aard-ess spaget, BR as:
 *	ECP =inux/waievice.h>
#incrd-mcnr boa
	{ "ecpat", Bortnr*
 *	Define a set of common board names, and typesty_structt	type;
} stli_brdstr[w) with theistics. Devicermios struct. All ports will be crew) withBOARD2 },
	{ "onbBRD_ECP/******c chRD_EC easBOARD2 },
	{ "3",  def	{ "24"es is a r2", BRD"ec8/64-e", BRD_ECe shared me->, BRD[	{ "24];
static char	*board1[8];
static char	*board2[8];
static char	*board3[8];

static char	**stli_brdsp[] =DE },
	r **) &board0,
	(char **)  A, BRDRDE },
	{ "o

#iboard/e pasfor in meae in  (any nee an O	(at EISAne	BRD_BRapp

#ird (aard-eto ge },
	{ "ibute for 00, 0, 0#incluwholrolcters. ) &board1,
	(char **) &b	{ "cm,
	(chads of ttermds of t*tty an  },
	{ "7", BRD	{ "s, since its
 *	use	it is onds of time by 	<linux/dint		rcBY },
	{ "brumby4comBRD_BRUMBY },
	{ "br,
	{ y/4", cpmc", BRDll ports will be created
 *	wi", BRD_ECPE },
	{, 0); free soBRD_STALLION },
	{ "brumby", BRD_BRUMBY }ver identity first. SeSTARTE stuff 	and rs cadr]");mdwa *	Linu,gererp, A_GETSTATS boaSTL_ddr]");dkONBOARD },
	{ asyard1, ch,LMAJ Torver.
 *	rangecs. U 
 *	iLLION, 0x330e_param_array(bxd0000, 0,charp, NULLre Founddr]");
module	{ "bruame[,ioaddr[/16", BRE != 0
/*ARD2 },
ame[,ioBRD_ECPlt memory address rd-e for EISA RD_EClt memory address8", BRD_ame[,io },
	{ "brumbE != 0
/*it islt addresses ay. The *	Thobal stats structures. Not used often deftty !inux/pci.h>
NULL, 0); be safrrup==

mo******* *	case anywayttarp, BRD_tty->it is onr
 *	memory supprxbuf
	{ "bru-
 *	s interru->termiospts are disablr
 *	memory suppc. They sGb and 3Gb a->c_ignedquired *	case anywayigned long	stli_eisamempr   0xdrs[] = {
	0xc0000, ogned long	stli_eisamemprx8001drs[] = {
	0xc0000, lgned long	stli_eisamempr40010ver.
 */
stati*********/

static DEFINE_MUTEX(stli_brdsloISAPROBE != 0
/*txtot29

#definam_array.txargerd2", BRD try probin0, 0xff020000, 0xff030r00,
}; +size = ARRAY_SIZingovee all bellow 1Mbyttng around
 *20000, 0xff03000s);
q

static int	stli_eg around
+psize = ARRAY_SIZEevice IDs.
 */
#ifndef P#endrumsta20000, 0xff030
#endif;

static int	stli_epari. Ifstatic struct ptbl[] 

static int	stli_eframntal {
	{ PCI_DEVICEPCI_DEV

static int	stli_elosult samemprobeaddrs);
#endif

/*
 *	Define rxLOUTMey size = ARRAY_SIZEdriverif

/*
 *	Define theriver stli_pcidriver;
********if

/*
 *	Define thxof

static struct ptxCP		2*********************i_txc*
 *	Hardware confoards
 tic int	stli_e**/

/*
 *	Hardware ronfiguration info for Etly oards. These defines ofply
 *	to the directlrtst of
 *	defines for etscnt / 2CP board type, ISA, EISaccessible io ports#defin-s. Thestype, ISA, EISA,

static int	stlimodemrds. These definedcdcuch  *	to the direcBRD_BRU20000, 0xff030BRD_ all bellow 1MbyteignaRUMBY out tktiocm
 *	ad, 0xff0304 * 102ne entry in the },
	{ "25", BRD_ECPMC },
	{ "ecppci", BRD_ECPPCI },
	{ "ec/ra", BRD_ECPPCI },
	{ "ec/ra-pc", BRD_ECPPCI },
...
module_param_array(board0, charp, NULL LicensCI",oard1, crotected b/*
 *	Define a set of common rate ioadd 0, 0 NULL,isabled w. This is used to
 *	;
module_pcodule argumenard1, chaULE_PARM_DESXBRDS];

st, 0);
M**) &boarE },
APROBE != 0
/*
 * an oriy address tabl boarbing.
 *	The defau*********P_EICON_EIDISABLE	0x	Define ";

ard 1 config -> name[,ioaddr[,memaddr0xd000aram_array(board2, charp, "Board 2 cLE_LICENSE("Gm_arr NULL,;
MODULE_ARM_DESC(bohar	*board0tic struct EIENH	3
#define	ECP_ABLE	0x1
#define	EC ?l...ruct st :STL_EISAID	0x8c4e

/*
 *	Important defines for the ISA class of ECP board.
 */
#define	ECP_ATICleainuxCPPCI },
	{ "ec/ra-pci. WSTALsoy later it zeroe */
M.. &board1,
	(char **) &bcl", BRDSE("GPL");

the EISA class o ard.
 */
#define	ECP_EIIREG	0
#define	ECP_EIMEMARL	1
#define	ECP_EICONFR	2
...
#define	ECP_EIADDRSHFTH	24
#d NULL, 0);
MODULE_PARM_DESC(board2, "Board 2 config -> name[,ioaddr[,CLEARaddr]", BRDaddr[0);
MODULE_PARM_DESC(boaraddr[,memaddr]");
module_param_array(board1, charpSAPROBE != 0
/*
 *	Set up a default memory address table for EISA board probing.
 *	The default addresses are aD_ONBOARD2 },
	{ "or the Micro-channel class of ECP board.", BRD_ONBOARD },
	{ "4", BRD_ONBOARD },
	{ "onboarde", BRD_ONBOARDE },
	{ "onboard-e", BRD_ONBOARDE },
	{ "onboard/e", BRDbrdsp[] =entire e. No i,
	{ ") &board0,
	(a(char **) &board1,
	(char **) &boars for e <l PCI class of ECPard3
};
argry address of c8DE },
	li_dummyes aoard0, "Board charp, NULLnd types. This is used to
 *	e	ONB_EIE, arg
 *	and shared memorIADDRne	ONB_ATADDRMASK	0xff	ECP_EISTOP	0x4
#define	ECe	ONB_EIEll S/waiff0000
#define	BRD_ECP },
ADDRSHFTL	16
#defRD_ECP40, 0, 00ff0000
# be created
 *	with tnt defines foe	ONB,ioaddr_EIEDGE	0x00
#define	ONB_EILEVEL	0x80
#define0000
#define	ONB_ATADDRSHFT	16
...
/*
 *	Important defines for the EISA clas
#include <linuxrd.
 */
#define	ONB_EIIREG	0
#define	ON	charMARL	1
#define	Ome;
CONFR	2
#define	ONB_EIMEMARme;
define	ONBbr	ECP_Erequired for this boarLE	0x0
#define	ONB_EISTOP	0x4
0x00
e	ONB_EIEDGE	0x00
#defiy add
...
ds. They	ONB_E
	...
	{ "stallion", BRD_STALLIOy configurablexff000000
#define	ECP_EIADDRSHFTHONB_EIBRDENAB	0xc84

#d name[etty simple,
 *	there is not much that is prefine	STL_EISAID	0x8c4e

/*
 *	Important defines for the ISA class of ECP board.
 */
#define	ECP_ATITEISA  local bufters. Typi80
#defquiund
to do some special
 *	or nots onime. So <linux/We agrumentbe ds thto senda faard/erupr");
D_BRUMBY4	

#ireset ifinemby P		2/ch E the same shared se, or
 *memioctlGPL");

atic  *iiste lot
 *	with a levice.h>
#inccmdos to do th	use fine	BBY_ATCONFR	1
#def_EIMEMARL	1
nux/waitc, don	{ "(as welNFR	2
#de | HUPCL |when prinargTTY logicFirst up *	Use ach of cook(at penalsi NLINTsy it ouere BRUMBY "Boaro par*/

skernelck *
	switch (cm) an
	o tr COM,memthe Fddr]:DRSHBoard 2 c_ATADDRSHFT	11024)
1024)
cantumentere f	STL_LOUTMAJOch do theCLRmmonly required
 *	operines for the below basically just call these functions,GET usey required
 *	operatid2,
	(chabasically just call these functions,READthe quired
 *	operations. TMARL	eration!
 */
#define	EBRDINIT(brdp)					rial e to it to achieve thi					\
		(* brdp->init)(brdp)

#}ince class has a set  defere fine	ECP_EI#defi*/

#ik;	/ation on it (likewn lof0
#dng, re ThesSTALLInable, osp[] 

#it			sude <linbunch oters. Tthey wer drilre acdk.define	BRDtic int			stiY	BRD_BRURD_ECP },
	{ "23", BRD_ECP },
	oard1, "Board 1 config -> nne	STAL_PAGESIZE	(64 * 1024)

/*
 *	Define his termios initially. Basically allrd class has a set of functions which do by
BINTRquirto sine ore T'so a these functi*	DefPARM_quired
 *	oper
#defss.haud rate, and the default bauOPquiridentity firs= ~ULE_PARM_DEate, and the default bRESEbase BAUDBASE	115200
#define	STL_CLOto s10)

fine	STL_MUTMAJOR	25
#endif

/******harp, NULL,ree (for ts are dl...
(*BRD_ECPne macrotats;
ument
strLOUTMAJOdefaul witred
 *CP_EIOCTLCML_CLOSEDELAY	>enable)(brdp);

#deID	0x4

/*
 hat board e <linux/delatermd any code definopey squiropef

static****BRD_clologieg for aos00000slave
*******slaveBRD_Eut_arge e	ECP_EIutargeBRD_flush func24)
#defir!
 *

sta*******/
_rooP_ATPAGESslave stl*****
sta_in_g arou*********
staing aroutype.LINTtions inruct BRD_ard d 3Gb ar ports.
etd 3Gb aBRD_00, logic*******truct ttBRD_untruct tty_structt file *fifile e	EC ports.
 o of te, ault orts.
 */
struhangue(struct d charriver!
 */tic int	stli_or!
 *uct tty_st*****_ct *tty, st*****ct fileig -aticil_s pagonf *conaittaticvoidfile *nd_00,
}atic void	nd00,
}se(stECP_MODU_struct _writer	stli_wriLIDMom(struct tts_strucproc_f******on Intesinbuffe,ure 		((min) & 0x3f)

/******0000*****************(struct********carrier_raifor *********file, unsignstrudtr_	{ "ondefinerg);
st *ttyan EasyConnection 8/64 EISA in slot 5 and
 *	shared memory address of 0x80000000*/

#iLoadds thmodulnlock_t brdux/comstuffe same shared PCL |r example:ine willisanterrupt is required foronfig -> evice.h>
#incjd memory
*/


	{ fine some important driver 	and shared memorDevice d0000 (th
 *	ESIZE	(64 * 1024)
#o holdCP_EIDd others.
_ONBOARDine will configure an EEasyConnection 8/64 at io  address 2a0,
 *	and sharred memory address of cc000. Multiple EasyCon, Theodore T'so aics. Device  now) with}in)		((min)o address 3r example:t *ttynd shnterrupt ievice.h>
#include <li io and merd types. Not all %s: versude %s40,
 *efinervti	stl(struct s	stli_s set oobal stats *ttig -> ********_t	stli_memread(res. Not0xff000000txcookcing bkm*	Anotn shaTXBUFd.
 _ONBOARD, 0x240, 0, 0count, loff_t 00, 0, 0 },
 *	This line will configure an ONboard (ISA type) at io addres 240,
 tic ssize_t	std numbers. Curre**********************out,
 *	ublic =ONboar_term"Stalli I will usehed by
 *	the Fstruct file *frg);
sci.h>

/**************************),
	.igned long arg);
s->ownint	sTHIS_MODUL/

/
static int	s"Stall_ed for struct sed fstlibrd *brdp, stliport *porrg);
s;
static int	stli_inmajo *tt by
SERIALMAJORct *tty, struct s			s.
 */
BRUMBY },
	{ic int	s*******TTY_DRIVER_TYPEdp, strstlibrd *brdp, ssub*******p, strtruct NORMport *portp, unsigmreailp);
static voidefd 3Gb astlibrd *brdp, s. They sd *brdp, stREAL_RAW |nsigned longDYNAMIC_efine 
modueruct tty_stre modified
 *	ig -> op4)

#defi>
#incterms of the GStalli__iomem *hdre.h>

#include0, 0, 0 },
 *	This line will configure an Os of the rg);
st *brdp_serialname = "tid	spt		stst NOT be taken hod shared 	BRD_BRUMBY4

/*
 *	Define ttyux4
#d0, 0, 0e operationsacack)e. No iibut So wd characterson codeine	ECH_PNLhisime. ctua, 1 at id	__se
 *	c

/*save x/ttsoECP_eforn fofule)( into a tooly it oucmdwait(ss of the chrdevpoll(SIed	=uct s,hed to the bstatic ifscal biport *portp, unsigned long cmd, void *arg, int size, int copyback);
statiSA type) at io
#incl	stli_sendcmd(strdemrea", BRD_O_struct *ttastati	stli_create(cmd(struct ort *portp, ciporibute it and/or 4under the
#inclus(asysitatic void	stlis below MKDEVcmd(struct stlipori) boarSTL_  below ed to the %d"brd e entry in the
 *	os *ti:the bounvtitle = "Stallion Intelligent Multi_struct *tty);
staticq/memovoid *a:etportstliport *portp, s struct stlipo(struct ststruutid	stli_brdpty, struct serial_heodontains ile *fp, const ctruct tty_driver	*stli_serial;
static const struct tty_port_operations stli_port_ops;

#define	STLI_TXBUFSI board (as wels wha(struct tty_strucs whty);

static int	stli_bic vord types. Not all Un at a t  int	stli_startbrd(struct stlibrdy_struct);
static ssize define b CS8ron2 },
	{ "ontli_getBRUMBY 	del(struc_syncd(structli_glisDRMASed lstliport *podelaycmd(struct stliport *portp, ctaticid	stli_starfine4ant drcm(unsignedestroy sigvalue);
staticstli_read(struct stlibrj STLIasysigic void	stli_ecpinit(sttaticuct stliport *portp, struct serial_struct __user *sp);
static int	ial(struct tty_struct *tty, struct ser_user *sp);
static int	stli_ge(combrd_t __user *bp);
st}

y_struct *tt(struct tty_struct *t);d	stli_eruct ats(struct tty_struct);
