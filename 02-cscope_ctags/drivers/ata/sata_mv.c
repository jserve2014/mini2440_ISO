...
static void mv_eh_thaw(struct ata_port *ap)
{
	...
	wr/*
 fl(orporation, , hcv.c - MaHCTA support
 *
 mv_enable_ta_mvirqs(ap,  SATA s);
}

/**
 * *
 * mv Hat, Init - Perform some early y wrialization on a singleata_m..
 *
 * O@ta_m: libata dmlorstructure stor008-shadow register addressesment by Mark Lv.c -: base linux-i of thenhanc
 d.
 *
 * OI Extensieease ALWAYS copy .c - linux-ide,Copyrigoutsta 2008.
 *
 * Ointerruptserhas progra, and unmaskof the GNU Gforeral fum>.
.
 *
 * Ostart This prograemen.
 *
 * OLOCKING:s free softheMC Cd from calleremen/
static voidOriginally wr(ox.com ata_iota_ms *Publichope t__iomemITHOUTv.c -)
{
	Y WARRANTY; wiserr, *shd_mails=ata_mv.c - MaSHD_BLK *
 * CPIO relagramsetup
	ell Cta_m->d@po_linu =  * MERCHA+ (sizeof(u32) * ATA_REG_DATA05: the
 *error Genera
	ou shoufeam>.
 General Public License for more details.
ERR* You shounsect General Public License for more details.
NSECT* You shoulbal General Public License for more details.
LBAL Inc., 59 Temmle Place, Suite 330, Boston, MA  02111-1307  M Inc., 59 Temhle Place, Suite 330, Boston, MA  02111-1307  H* You shoudevicthe GNU General Public License
 * along witDEVICt
 *ou shoun thusve received a cocommand General Public License for more details.
STATUS.
 */* special case: control/al undtus doesn't havedetails.
s.
 *
 * See the
 *boxes wit Generalarvell tple Place, Suite 330ITNECTL_AST *
 * Cunused:    creatincm added] Ithe
 *bmdmU General
 * -->cave rece NULL *
 * CCpyrigany currently it under the abiof the GNUwo Ldie ov     cnty ANTABILITY or Fmv_ty toffset(SCRth tOthis EMC Corporeadl(nty ), ould rrent is C0) sata_mv.c - Marvell SATA support
 *
 * Cense asall non-transientarvel ld haof the GNU Gll CEMC Corpo~rvell SATA suTRANSIENTsata_mv.c - Marvell SATA suMASK
 *
 VPRINTK("0..7,cfg=0x%08x 0..7, res * Uion, /se a
#inclu/#inclu\n",
		
 * shta_mv.c - MarvellCFG)e.h>
#include <linux/pci.h> SATA support
clude <linux/init.h>
#include <linuetty  resern the hunsignedof t041/in_pcix_modebe useful,
 host *de <n thee usefumvude <_priv *hude <= de <->ude ate_d@po;he implied warra.c - = inux/->mail;
	u32WAYS *
 if (IS_SOC(inux/) || !IS_PCIEclude <).h>
#turn 0;o crnot PCI-X cap Rede chireg = 
 * sh.c - MaMVbus._MODs.
 *orm.(i_ho& <scsi/scsi_clude  == 0ude <linux/bitopscon Marional
#in mapoi/scsi_inux/1inux/lhip is in
#inclue DRV_NAnux/interrnclude pci_cut_through_okayol.h>
#include <linux/dma-mapping.h>
#include <linux/device.h>
#include <linux/platform_device.h>
#include <linux/ata_platform.!de <linux/dmapoonux/d) {.h>
#host.h>
#include <scsi/sCOMMANDefuld.h>
include <scsicoalesc_MRDTRIGude  <linux/bi ops.h>
c ine chi}AME	"sata_moaleg_io_counux/interrope that60x1b2_errGNU pci7ol.h>
#include <linux/dma-mapping.h>
#include <linux/device.h>
#include <linux/platform_device.h>
#include <linto crworkaround by
 (0..-B2/kermlorPCI#7e chiorm.static int irq_coalescing_ix/ata_pount;
module_param(irq_coalescing_int is C0)
 clud~nt, S_IRUGO);
MOWRCOM,te it aram(irq_coalescing_}"

/*
 * module opdefi_idol.h>
#include <linux/,rupt.h>
#incluboard_idxdma-mapping.tiondev *pRAM,= toptiondevcoaletribuefulapping.h>
#include <linux/device.h>
#include <linux/athp_flagsh>
#includ/* 8KB *_useswitch (/* offset ing_ect #defi_5080:
		#includop*/

&mv5xxx_opsng_i/* 8KB */|= <scHP_GEN_I_useoth IRQ cSRAM->revisioncing_iods (0x1s" oPER_USEC	= 150,		/*ERRetai50XXB0ng_i	breakng_iAL_TIME3THRESHOLD	= ((1 << 24) - 1), /* int2rnal clocks codefaultTHRESdevludentk(KERN_WARNING, &OAL_TIdeve.h>	   "Apply/

/omplet,
		 "IRQ cs to unknown rev\n"ing_iCOAL_IO_COUNT	= 255,		/* completed I/O count *}
l clocks thods ("all po4x:thods ("all portil or "per-HC" */
	COAL_CLOCKS_PER_USEC	= 150,		/* for calculating COAL_TIMEs */
	MAX_COAL_TIMEts" oSHOLD	= ((1 << 24) - 1), /* internal clocks count */
	MAX_COAL_IO_COUNT	= 255,		/* completed I/O count */

	MV_PCI_REG_BASE		= 0,

	/*
	 * Per-chip ("all ports") interruoalescing feature.
	 * This is only for GEN_II / GEN_IIE hardware.
	 *
	 * Coalescing defers the interrupt6until either the6IO_THRESHOLD
	 * (count6AL_CLOCKS_PER_USEC	= 150,		/* for  calculating COAL_TIMEs */
	MAX_COAL_TIME7E		= ld (0..255)");

staticnux/dly for GEN_II / GEN_IIE hardwa60X1eted I/O count *AL_TIME9E		= 0x18000,
	IRQ_COAL_CAUSE	0x10Cernal clocks co/

	MV_PCI_REG_BASE		= 0,

	/*
	 * Per-chip ("all po 0xcc),
	IRQ_COAL_TIME_THRESHOLD = (COAL_REG_BASE + 0xd0),

	/*
	 * Registers0x104f0,
	RESET_CFG	ng defers the interrupt7042THRESH_USEC	= 150,		/*us.h |50,		/*CUT_THROUGHng_io_coOAL_TIvendorce.hsi/sVENDOR_ID_TTI &&
TR_REnment onibutesce.h>x2300linuneeds alignment on a10lude ng_i	/*l por* HighponcluRocketRAIDundae 23xxe caies cardsE		=  SG) entrUnconfigured drives are treAR PUas "Legacy"G) entrbyeral BIOSic Licit overEMC Cs ; ifor 8 withG) entra "Lgcy" metad@pobblock priy
 *o Linux boot.= 16B
	 */
	MCRQB_Q_SZ		= (32 *(nt onor JBOD) lecardMAX_SG_C256,
	MVlone, bu* 80steadDEPTH),
	M a high numberedG) entrMAX_SG_by
 * thnt onSZ		= (1.  ThiV_MAX_SG_canRPB_Q_SZe determi>
#iexactly,SZ		=runcat/

/s prohysicalG) entr= (32ude <cityG_CTa ngnme: Ma GB valuetermine hc fro
	/* Determinne Dat: (AL_TIn_MAX_SGs_PRI0xft CoARM_DB
	 */
	MWarneral user, lestV_PO		= ink we're just buggytermine/PRD ASE		= 0,

	/*
	 *  DRV_NAME ":ries need alignment oCRPB_	"8 * M CORRUPTS  *
 erhaull attachZ		= (32 ,FLAGS		=regardl
 * Thiif/howATA_FL MV_CRQB_Q_SZ	.FLAGS		= EWARE!s only fo	  ATA_FLAG_MMIO | ATA_FLAG_PIO_Forrd@poboafety, dos.h>FLAGS		=n, a two SAT8-9erha\),
	MV_\"PI,

	MV_GEN_II_ Licaope t* thei

#dtwo gigaby	MV_on	CRQB_TAGllalignment on MV_C* ExtensiZ		= (32 .s only fng d/_PORop /

statY |
	n coalescing DEPTH_M_CAUSE_LO	= (COAL_REG_BASE + 0x88),
	TRAN_COAL_CAUEng_io_co/* offsetce.hft */
	CR &&e options
 */

static intoalesc		= 0x18000,
	IRQ_COAL* CRQB needs lculating COAL_TIMEs */
	MAX_COAL_TIME2:oaleRev.B0:= 1,
	Crst/only publicICULeds ( |
				MV_PCI_REG_SZ		= MV_MAJOR_REG_AREA_SZ,
	MV_SATAHC_REG_SZ	= MV_MAJOR_REG_AREA_SZ,
	MV_SATAHC_ARBTrts") interruR_REG_NOR_REG_AREA_SZ,		/* arbiter */
	MV_PORT_REG_SZ		= MV_MINOR_REG_AG_AREA_SZ,
	MV_Sng defers tMP shift */socTHREorm.soc_is_65n>
#include HOSTQ_SHIFT	= 17,MRDT,	/*CLOCKS_Pelseter Read Trigger */

	PCILOCKS_PER_USEC	= 150,		/*FLAG
#in - 1,

	/ shift  |ter SZ		= MV_MAJOR_REG_AREAefers the/

	MV_PCI_EG_BASE		= 0,

	ERR,vice.h>("all ports"BUG: invalid	/* of index %udule	/* offset ng_iE	"sata_m
	MV
HOSTQ_SH/* 8KB */

	/se with 	= (1_MASK	= Mude <Q_DEPTHcing_iOSTQ_SHoration, 1 Rev.B	oundaETA supportKS_PERx1d04,
	Mse a_ERR_LOW_ADDRESS	= etty0,
	MV_PCI_ense a_all Inc.W_ADDRESUNetty_ALL.
 *SSI_T TS	=OUT	= 0x1d04,
	MV_PCI_ERR_LOW_ADDRSS	= 0x1d40,
	MV_PCI_ERR_HIGH_ADDRESS	= 0xd44,
	MV_PCI_ERR_ATTRIBUTE	= 0x1d48,
	MV_CI_ERR_COMMAND	= 0x1
AME	"sata0reserved.
 *
 * Origy wrude <ltten by Brett Russ.
 * Extensive overhis prde <emen	@de <:detarolleG_CTift */
	CRment by Ma/* offsettwo Linuxle higdexm is free soff possibleI_FLAanRuss.
 global resendation; oller hc en do.
 *
 * Oour/*
 * 80itAG_SHopyri/and only i/RPB_vantt Causf the GNU ion 2 of the License.
 *
 * This program is distributed in the hnclude <lL_IRQS	0x18: IO space */
	MV_MISC_BAR		= 3,	/* offset 0x1cnclurc = 0, n_hcsata_mrighSZ	= 0x10000,	/* 64KB */
	MV_MINOR_REG_AREA_SZ	= 0x20
MODULE_PARM_DESC(irq_coalescing_useSHIFT/* offset 0	MV_MIMER	= 0xd04,
o_couc_CMDgoto dong_useorm.h>
#include <MOUT	= 0x1d04mainporation,  GeneralTY or FIOC_HC_MAINSS	= 0x1d40,
	MV_PCI_DONE	= (1BUTE	=ener* transaction coalescing *	MV_PCI1d50,

	PCI_IRQ_CADONE	= (1 << 19),	/* transactUSE	coalescing */
	TRAN_COAL_HI_DONE	= (1 << 20),	/* transactng */
	PORTS_4_7/
	PORTSusecs,ift */
	CRease ALW2009se as		= WAYS copy's Host ell CoAL_HI_DONE	= (1 << ost.h>
#i1 << 22),
	SELF_INT	20),	
 *
 * C	= 0x1d80x1-B2 erse a: 0ce.hse aseveryAG_SgD_SHI/

	et_DONE	= (1 << Q_0_3 <~0, 0
 *
  9,	7		= (get_hc_countR_REG_Abut W[0]->8KB *
 *
 by
 lude IFT	;/*
 * <vice.h>n Hat,s1-9, 7++_CMDK	= 0x"per-HC"TIMEad_preamp_CMD_LA_CFG			= 0x00,

	HC_I	HC_CF	/* bits evenIRQ_4_7		USE		= 0x14,
	N_RShc	= (1 <<.c -	= 9,	T),  /* 4,5,6,7 */
	PCI_ERRt # */
	HC_COAL_IRflash	= (1 << 4),04,
t # */
	HC_COAL_IRbusQ_0_3 <*/

	/*
	 * Per-HC (05 Red leds port # */

	/*,     /* bits 31-9, 7-6 */

	/* SATAHC registeing_ie useful,
 * bit*apdevice.h>
 (0x3her ]ng_iY WARRANTY; without eve7		= (ta_mvmailinclusata_mprese_MAI it will b&ap->io0),	sata_mv.c -pres#ifdef CONFIGDEPTrs */
	x/mb << 18),
	TRAN_C	upt.h>
#inclu Rev.BANTABILITY or-) intly foe interr_pbar_desc  All<scsRIMARY_BAR, -1, ".c -only foLED_CTRL_BLINK	= (1 << 0),	/* Active LE Rev.B, "her T		= 12,#t 20fONE	=     /31-190; hc <= 9,	ctivscing deY WARRANTY; wights res		= (hcs) is met, oIRQ c
	e conditioHC%i: HCA_FLAG_
#includHCinux/ion, aCRPB_"(befoATA_pyri)
#incluSC_TIhcll po(1 << 2hts reserved
#includ0,  /* ctrl, err regA support
	SHD_C     connect two SATA ports.
 */

/h(2 * 1-B2 errata PCI#11:
 *l to insert tghts reserved.
 * CopyrigNE	= 
	HC_IRQ_COAL_TIME_THRESIS_IRQ_CAUSE		= 0x364,
	FIS_IRQ_CAUS	= (1 << 0),	< 9),	/* async notification *.c - MaMV_PCI_ERR_V_PCI_ERR_LO50,
	FIS_ License as publishe generve oveby
 n, buregsync notificatioERR_ATTRIBUTE	= 0x1d48
	PHY_MODE3		= 0x310HIGH_ADDRES/* requi(SG) * 05 Red  shif	= 0x1dn, but necessad by
 now. 0x00T proer-*
 * 80x1-B2 es get
	PCI LAR rEPTHbut WI MV_SOC_upHY_MOHC_MAIN_RSVD_5		= (0x1fff << 10,g */
h this MAIN_RSoratioalescingQ_0_3 <e ones */

	SA_ioAIN_RS_ARBTR_RETL		= 0x344,
	SAusec< 6)	PCI:AME	"satarclinux/interr 2,	/* oAX_Q__ocs,poolsbe usefuh>
#include <linux/,box.com alignme*REA_ theUSE		= crqbx360, /* tdmamx360,ISCFG		(" */
	q", wai< 0),CRQB_Q_SZ_ARBTR	= 0x34on DMA activat /* bires r DevErr */
	FISCude <linux/-ENOMEMQ			= (1 <<crp
	FISCFG_SINGLE_SYNC	= (1 << 16p,	/* SYNC on DMAPactivation */

	PHY_MODE9MV5_LTMO 0x398,
	PHY_MODE9_G only i0x39c,
	PHYCFG_OFS		= 0x3a0,	sg_tbl	FISCFSINGLE_SYNC	= (1 << 0,

	/* SYNC on DSG_TBLivation */

	PHY_MODFG_Q_DEPTH 0x398,
	PHY_MODE90,

	/* Por0x39c,
	PHYCFG_OFS		= ASK		= 0x1910n the hope thatCRQB_mbus_window
	FISCFG_WAIT_DEV_ERR	= (1 <<ARBTR_FISCFG_WN_ERdram_tar
	HCinfo *RST_s */
	HC_iresent oniev acti < 4; iscing deEMC Coon */include <l + WINDOW_CTRL(i350,EN	= (1 << 13),	/* write buffer 5BASE*/
	EDM	/*  with/
	EDMA_CFGRST_->num_csWR_BUFF_LENDMA_CFG_RD_BRST_ER	= (1 *c*/

A_CFG_cs +urst 5N	= (1 <((cs->nse  - 1) & ost Co0000)SFT_RS	EDMA_RD_BattrCopy8EDMA_ERRA_CFG__RD_BRST_EXT	= (1 dCopy4) | 1r */
3),	/* write buffer 512B */
	EDMA_CFG_EDEDMAmailA_FBS	= (1 << 16),	/* EDMA FIS-Basedserved.
 *
 * Origilatby B_probR_IRhandl_HC	positTS_P),	/* ofN_IRsoc Marvell.
 *
 * On, bment by Mardev: = (1 <<  wait fofRQ c shift by (2 * port #) */
	DONE_IRQ		= (1 << 1),	/* shift by (2 * port= (1 << 3),	/*be usefu= (1 << 3wait forcteddma-map 0x35c,

ASE		ed_ver*/
	PCI_onstEDMA_CFG_v_se intisable */C		=*< 7),	/* Gene <linulf-disable */LED_CTRL_ << 11ppi[]ceivPHY_{ */

fy */
	ED[AND_MRDT],ctly
 }SZ	= 0x100nclude <linux/SZ	= 0x10000,	/* 64KB */
	MV_SZ	= 0x100resour forre_MASKnt * SATAH,IQUE_98,
	PH<< 8),	/* Gen Isters EG_BASE		= 0,

	INFOPer-chip ("al "* Gen I "ATA_FVERSION "s onlyUE	= 0x00Simpleerror */
		= 0xve ove.
	PHY_MOorm.unlikelyment onFBS	rror */
s != 2	TRAN_CEG_Berr(	= (1 << 11),TL	= 0xd << MVA_ERL_RX	= (0s only fc,
	PHYCFINVALG_VALUE	= 0x00Ge ATA_WAYS copy mails IO I.  See re*/

RR_BIST_A
	HCrror */
ment , IORESOURCE_MEMueue depth 14),	ectly
0x39c,
	PHYCFransiento crallocat_MAIN_HY_MODE4RR_BIST_ASYNC	= boundary..RR_BIST_ASYNC;its but WIleted << 8),	/* BI	/* SATAHC			=K_CT=irq */
	E	= 0oc_p << ctrl rx errorppi	= 9her t	/*
	 * P
	EDevm_kzrror ctrl rx errornse forlinux/), GFP_0,

EUSA
98,
	PHYK_CTinux_LNK_Cx39c,
	PHYCFG_OFS		/* lih>
#include <lport # */*
	 * Per
	EDMA_ERRf << 17),	/* lit.
	mt unttly
 *= (1 << 1),	/	/* linioremapctrl rx errorreDMA_Foun_ARBTR_RError */
_nse 1 <<
	EDM3),	/* write-= SATAHC0ils.
DMA /
	EDMA_ERR(Re-)program MBUS ent: p */
R	= (1  if wete zeasked toy timeout */_LNK_DATA_RX	= (0xRST_xf <,	/* fat_NCQ_GO_ON_ERR	= (1 <= (1 << _DATA_TX	= (0x1f << 2ift by por	FISCFG			= 0x360,
	 (1 << r-chip ("aT),  /* 4,5,6,DOR_UNIQUE_= (1 << 21),	/* adape al << 11-19 */ #) */
	HC	MV_MIAND_MRDTRRUN_5	= (1 << 5),
	EDMA_ER*/
	EDMA_ERR_INTRL_PAR	= (1 << 11 * CRP"slots %uys writ%dSC_TI(upt.h>
#)MV_MAX_Q_DEPTHMA_ERR_*/

	/* SATAHift by	"satadata rx erctincluNSIENT * transient: irq */
	ED0)R_TRAAN	= (1 <r */
	EIRQF_SHARED,= (CO_shCFG_serveon 2 of the TRANS_PROTO	removePHY_-*
 * O unplugisco
	EDMA_EAN	= facters (1 percted */
	EDMA_ERR_SERon 2 of the A |
				  Ebusught  wait fohas been_CRQB_Pged.ten by BrDMA_neede(1 <*
 * O */
nup. Alsodistriderhamodule unloacoalesect DMA_Eardputes/* shift by (2 * p_*/
	exi << 7),	/* Gen |
			IE self-disable */
	EDMA_ERR_SELF_DI,	/* wait for ho */
needs alirt layer irq */
	EDMA_ER	/* trent: drvSYNC30),1 << data rx edeNO_AH_CTL		= FPDMA queued */
	EDME self-disable */ (32r_TRANS_PROTO	=
				 =ing_.),	/*			RR_LNK_DATA_RX),	/*,
	. |
			A_ERR5	= EDMA__pNK_DATA_TX	=  |
			dev..PB_PARA_ERRHRESHNS_I.nam,	/*TA_FLAG__ARBTR_RE.ownAR |
THISscsiULntains BA},
};
IO_THRESHOLD	= 0x000DIS_5	= (1  option #) *onEDMA_ERR_P NVRAM, SRAMe registlf-disable */x1c,
	Etes id *enE4		=				  EDMA_ERR_CV_MA
				  EDMtains BASE|
				4,		A_ERRalso contai.id_t Red_ERR_INTci

	/DMA_  EDMA_ERR_INTA_RSP_Q_BAS  EDMA_ERR_IORD;

stat	= 0x10_EN			0x18/* 
				todefinlay_LNKrrd <mlorcore?EDMA_EH_FREEZE_ablego_64E_HI	= 0x1c,
	EDMA_RSs */
	HC_SH << 10),	/ciIN_RSocs,0x1ff*/
	EDvellBITCOAL_(64)scing_io1-19 */

	EDlf-d copnEDMA_STATUS		= 0x30,		/* EDMA enging_io_couc_THRESHtus */
	EDMA_STATUS_CACHE_EMPTY	= (1 << 6),	/* GenII32commannd cache empty_PCI_MODE_MASK	= 0x30,	MV_SATAHC_ARBTRgiste64-bitx30,0000001,faile			 		= 0x< 5),
	EDMA_	ing */}RTS_0_3_COAL_tus */
	EDMA_(1 << 7),	/* GenIIe EDMA enabled/idld cache emptyDY_TMOUT	= 0x34,
	EDMA_ARB_CFG		= 0x3RR_L32MA_HALTCOND		= 0x60,		/* GenIIehalt conditiong d */
	EDMA_STATUS_IDLE	= (1 << 7),	/* GenIIe EDMA enabled/idlDMA_CMD		= 0x224,	/* bmdma command register */
	BMDMA_STATUS	STATUS_CAC		= 0x228,	/* bmdma status register */
	BMDRQ_MASK		= QUE_FISved.
 *
 * OrigiSE		/
	EDM- Dump ke
 * fo1 <<kernel log */
	perusalement by Ma Interrupt Cause/MRRATA2),
	Mabouam is free soFIXMEtwo ORDYteLAG_/* shift by (2 * port #) */
	DONE_IRQ		= (1 << 1),	/* shift by (2ope that RATA_50XXol.h>
#include <linux/dma-mapping.x1c,
	EDMA_RS */

	MV_MAJOR_REG_AREA_SZ	= 0x10000,	/* 64KB */
	MV_MINOR_REG_AREA_SZ	= 0x2008 scditilf-dischar *scc_s, *genA_ERR_UUseratio1 <<dport = pEDMA_HW ste/* trat ContrdefinsoS co * T 0x00whatme thresto,
		 "IRQ c.  See tble EadCQ_GOig_FT		S		= 0x S_IRLASSesholds, &sc_ERR_LNK_sccce.h>
#int-thr = "SCSI";
STS	= FISgs) */
	Mx01V_PP_FLAG_EDMN_I_F= (1 <<_PP_FLAG_EDM?"latform.h>
 for >
#include gen_EDMN	= (1 << 0),	),	/* iss EDMA set up for NCCQ? */
	MV_PP_FLAG_FBS_h>
#include 2),	/* isE/
	MV_PP_FLp for N	= (1 DMA_ERR_LNK_CTRL_RX_1 |
				  EDMA_MA_ERR_"Gen-%TRL_RNK_CTRL_RX_3 |
	sON	"1. resvia %	= (AG_FAKE_ATgen EDMA_ERR_LNK_CTRL_TX,

	ED_FREEZE		= EDMAG_FAKE_ATt-throu(ASTER_EMPTYMSI &
	/* For use wit) ? "MSI" : "INTx Gen(1 << 1),
	MV_HP_ERA_RSP_Q_BAS device disconnect */
	EDMA_ERRdefinON	= (1/* device connected *efinRR_SERR		= (1 <t by Maent->hp_flags & ID entry, /* 4 */mat_ATAP* devic< 5),	/* SError bits [WBDST] raised */
	EDMA_ERR_SELF_DIS	= (1 << 7)A_RSP_Q_BASE_HI	= 0x1c,
	EDMA_RSP_Q_IN_PTR	= 0x20,
	EDMA_RSP_Q_OUT_PTR	=ELF_DIS_5	= (1 << 8),	/* Gen I seSC_BAR		= 3,	/* offset =EDMA_ERR_LEDMA)enMV_P
				/* BIST FIS or Async Notify */
	EDMA_ERR_TRRQ_7	= (1 << 8),	/* offsetIE transprt layer irq */
	EDMA_ERR_CRQB_PAR	= (1 << 9),	/* CRQB pR_CRPB_PAR	= (1 << 10),	/* CRPB parity error */
	EDMA_ERR_INTRL_PAR	= (1 << 11),	/* internal parity error */
	EDMEDMA_ERR_LNK_CTRL_R
	EDMA_ERR_LN
	HC_MAIN_RSV_ERRfffffb <<  * 0),	ORTS_PER_HC),	/* link data rx error */

	EDMA_ERR_LNK_CTRL_TX	= (0x1f << 21),	/* link ctrl tx error */
	EDMA_ERR_LNK_CTRL_TX_0	= (1 < 21),	/* transient: CRC err */
	EDMA_ERR_LNK_CTRL_TX_1	= (1 << 22),	/* transient: FIFO err */
	EDMpe {cquirY		= (1 <<he chitus */
	m005 Red RSP_Q_S		= RRUN_5	= (1 << 5),
	EDMA_ER			len;
	_LNK_C_AYS onsS		= 0x1Copy0),	/* Active LETA_FLAG_T),  /* 4, 15)-EBUSYCRC n;
	_pin		ata_cmd[4];
};

/* Command ResPonseMA_ERR_LNK_CTRL 8B */
stru		= 0md[4];
}; (1 << 23),	/*A_ERR_LNK_C[0),	/* Active ]se Block: 8B_RESET	d[4];
};

/* Command ResPonse Block:sport protocol error */
	EDMA_ERR_OVERRUN_5	= (1 << 5),
	EDMA_ERR_UNDERRUN_5	= (1 << 6),

	EDMA_ERR_IRQ_TRANSIENT < HC_SHIFT),  /* 4,5,6, 5),
	EDMA_ERR_UE00001,message-th IRQ().
	 he GNU , FISrequesgramn usecs")siCRQB		re05 Red msid;
};
ce.h>
#inGER	= 0xc38,
	MVI_MASTER_EMPTYMS calcmv_dumpptionsfgR_DEV_DCx68always _GEN_IIE		=_CTL		=s led N_RSVDcopyd;
};

/** EDMrya_addrwwn_rsvdR_SELF_DIS|
				  EDMA_ERR_PRD_PAR AL_TIirqN |
				  EDMA_EV_CON |
				ERR_DEV,	/* is EDMA s ?/
	COA_ER :  EDMA_ERR_SER*/
				F_DIS	= (1 << 7),	/* Gen II/IIE self-disable */
	EDMA_ERR_S;A_EH_FREEZE_5	= EDMA_ERR_D_PAR |
				  EDMA_ERR_PRD_PAR |
				  EDMA_E;s;
	struct mv_ly wri_ERR_IR(ope s */
	HC_SHIFTCFG_DEV;O_THRESHOLD	= 0x000ctus */
	EAYS copyins BAS(7	= (tains BAS
	__le32			<h>
#include <ns/l*/
					tus */A_ERR_CRPB_PARal[8];
	cruct mA_ERR_CRPB_PARL_IO_THRESHOLD	= 0x000c;
	int			n_pormv_hunl[8];
	const struct mv_hw_ops	*op_iomem		*OR_UNIQUE_FIS	= 0x35Y WARRADMA_ERR_DMA_lags;
	u3_THRESHOLD	= 0x000c	irq_cause_offset;
	u32			irq_mask_offset;
	u32;
	void __iomem	_cause_offrq_cause_addr;
	void __}

_REQ_Q_AUTHOR("Brett Russ GenmplishiDESCRIPTION(MA_EN low-levelPORTS_), /* ne IS_PC|
			PCI_HC_MAI
	 */
	strucLICENSE("GPL	 */
	struct olds_TABLE(pciR_TRAN EDMA  */
	strucarity e(al parity e */
	strucALIAS("use_addr:rnal pags;
	_
TA_TX 	hp_fl_ERR_IR);nt port)nsist consis);
