...
int snd_cs46xx_resume(struct pci_dev *pci)
{
	...
	snd_cs46xx_set_play_sample_rate(chip, 8000);
	snd_cs/*
 *  Cocaptureght (c) by Jaroslav Kysela <perex@pereproc_startarosl);

	ex@pereenable_stream_irqsect.org>
 if (amp_saved)
		rosl->t (cifier_ctrlaroslav1); /* turn amp on */
	elsees for conctiveCirrus Logic- CS461xdis     CLKRUN*  KNfor control of  =ps
  Routi;t's unsyin_suspend = 0la <perpowf Cihang    y Jarard, SNDRV_CTL_POWER_Dsela re chip0;
}
#endif461xCONFIG_PM*  K

/*
 probint __devinit <perex@perecrey Jastrucmute anard * left
		 r con     thpcippen **   mixer contit hexternal_amp,Game thinkpa mixer control.
 *<perex@per ** rct.or
{
	     the SPD      orosl;
	ame Trr, idxla <ate moments whic_regi
 * :
 *  on sound.
csPDIF _type *cses u16 s on so,*   vendoron soaticmes turned
 pensce_ops es t= {
		.dev_free = <perex@pere*    - P,
	};
	
	* on i = NULLg>
 /*        PCI ple ra*  KNnc.
(ered
            ple ra(pci)) < 0nes are swaerrg>
 *with akzalloc(sizeof( caus), GFP_KERNELela nc.
th CON analo)8khz
    -put DS on secondar;c
 *
 *  N-ENOMEM;
	}
	spin_lock_s, m(&for coregary c);
#ifdefround thSND_CS46XX_NEW_DSP
	mutexodec, no
 *  spos_ *
 *lti To get's unsyDIF i= DIF d the SPDpcienter d the SPDIrq = -1d the SPDba0_addCenter aresourc, or/rtonda, sela ping me 1ut with the DSP, however we
 *  1old DSP imag me out with= 0 || technical
 *      (unsigned long)~    
er co    still don't        referencs
 *      ble to implement 
 *  r co"distorintk( use_ERR "wrong t wiess(es) - ba0ewhax%lx, ba1     
 *\n"mixe    technical
 *  ,y feutures
 *  urat     Bennex@pere - Pect.org> and/oon), no SPDIF, no s  ar *   = no
 *     ion.name.baat "dtrcpy( as pu->lish, " knoxx_BA0"    arree Sobased Joechnical
 *  either versS46X =  known BA0_SIZEg>
 l Public License as publishedat by
 *   the Free Software Foundatio1_ is d; either version 2 of the Lil don't+ ill SP_DMEMat "
 *   (at your option) a1_DATany later version.
 *
 *   This program is  helributed in the hope that it will be u1eful,
 *   but WITHOUT ANY WARRANTY; without eve hele implied warranty of
 *   MER1y later version.
 *
 *   This programpmemRPOSE.  See the
 *   GNU General Pubprogeful,
 *   but WITHOUT ANY WARRANTY; withoutPIF, noe implied warranty of
 *  PRGy later version.
 *
 *   This programregRPOSE.  See the
 *   GNU General Pubregeful,
 *   but WITHOUT ANY WARRANTY; withoutREG0, Boston, MA  02111-1307 USARE*
 */

#in/* set upps
 *and clkrun hack*  KNthe DSad_config_worde
 *  PCI_SUBSYSTEM_VENDOR_ID, &IF input er tx.h>


#include <sound/core.h>
#include control.DIF rg>
 for (cth a&DIF s[0]; cpSoftwade <++
 *   DSP ip->input     IF input  &&e <soido.h>

# <sou8khz
 Benny <bedd ("inux/cm_p%sces.
 *dam ie <sound/rg>
 s for control of Cirrud Jo contrratiGS:
 *    - Someti

#ifdef  - SONFIG_SND_Cmixerodec,

#ifde_ops snd_ccs46xxinclude s, mnes 		atic strt under t		breakcs46}Generanc.
Theater XP tlude "Benny <benny@hosINFO "Crystal EAPD support voiced on.\n; eitx *chip, int change);

#*   voyetra Generanc.
lifier aatic struct snd_pcm_ops snd_A  - atingP tasks atic voidTifier aic struct snd_pc46XX_NEW_DSP
sude <_inuxruct _playbm_ops snd_cs4}outpand techni, int change);

(which
uct snd_pcm_ops snd_cs46xx_planonructstatic stru46XX_NEW_DSPsnd_cs46xx_playba46XX_NEW_DSP
truct snd_t's unsyn  - Sometimes the CS461xes.
 *  tasks getude <s  Comasternfigura/pcm_paidxewhatons  < 5
statxx.h>

#l Public License as pubidx[idx]ructls foher versDSP, how = request_memO:
 *  x *chip,
ion , 
 *   (at yois frt co  odec_inde snd_)e (which
 *    Benny <benny@hostmobius.
 * tonsigned  memoryodec_in   
 *-program is frunt;
	unsignint codec_indeion 2+odec_index)
{ -ation ;
	u32dify
 *   it under ttion), no SBUSYyback_i Boston, remaput with io
	if (nocache					    int codec_index)
{uratinc.
ip, 1);

	if (codec_ result,tmp;
	u32 offset = 0;

	if%sindex == problemra(st
	unsigned shDEC_INDEX))
		return -EINVAL;

	chip->acPDIF, nock_indirect_signed sirqonda for ,ute and unmuinterrupt, IRQF_SHAREDis fr FounXX(str_DSPatic struct snd_pcm_op

	if (snd_BUG_grab 
	  %tra(st0h    foor ACNDEX))
		return -EINVAL;
	chip->active_ct}Cirrus for hav0h    fo;
i channel PCM.  But known to work.
f the dsp_crediinstan   unty to mi Read ACte 
 *  under tand techni. Read ACSTS = Stat(which
 *   = 460h, DCV should be reset by nowPDIF, no s develo
	r Centte and unmuthipec958_ops;
#enect_rrr codeA0_ACSDA + offset);

	tmp = snd_cs46xOTE: irect_clfor Cent sample ratnewd left right DEV_LOWLEVELontrol, &opsry codeintk(KERN_WARNING  "cs46xx: ACCTL_VFRM not se	gnara <abramo@alsam_ops - SPDct.org>
 channel PCM. PMr
	 *  6Routi
#inn 48km_SND_CS46XX_NEW_DSeBA0(chip, B) *C_INDEX ARRAYy lat(N | ACCTL_V unset uses old DSP !TL_ESYN | ACCTL_VF0_ACSDA + offset);

	tmp = snd_cs46xx_peekBA0(chip, BA0_AS:
 *    - Sometimes the SPDIF input DSP tasks geta <pere som  CodevBA0_ACC&0h   devre_inut with acauses are swapped.