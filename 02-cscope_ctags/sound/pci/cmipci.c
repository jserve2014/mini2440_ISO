...
static unsigned char saved_mixers[] = {
	...
	SB_DSP4_MIC_DEV,  * DriveSPEAKERor C-
	CM_REG_EXTENT_IND-Media CMIOUTPUT_SW/*
 * DriveINc) 2LEFT-Media CMIi IwaiRIGHT,
};

static int snd_cmipci_suspend(struct twardev *pci, pm_message_t rogre)
{
	an rediree sard *s of =istriget_drvdata(pci);nder the softwa *cm = s of->private_lic ;
	is fi;

	ree power_change_*   i(s of, SNDRV_CTL_POWand 3hotnse n; eithcmre; you _all(cm->pcmnse a) any later version.
 *
 *2   This program is distributed _spdif)ion;/* save registers */
	for (i = 0; i < ARRAY_SIZE(ITHOd_regs); i++)
		.
 *warranty o[i] =free softwarread(cm,WITHOLITY or Fnse ; without even the implied warranmixerof
 *   MERCHANTABILI for mr FITNESS FOR A P for PARTICULAR PURPO You shou   but Wdisable is RRANTYree softwarwri them, PCI souINT_HLDCLR, 0   bustribense
 ibuticeLicense twareav 2 of thtion, Inc., etther veemple Pla,istrichoos Temple Pla, *   itnse return 0;
} program is free softwarresumecan redistribute it t under the terms of the GNU General Public License as published by
 *   the Free Software Foundation;Suite 330, Boston, MA  02PCI_DFree<linurestor Temple Place, if  Pla_en
 *   Foundation < 0) {
		printk(KERN_ERR "softwa:istrie <linux/game failed, "
		 de <so"are
 *ing  Found\n"ci.h with ardwareconnecthe Li<sounork. Wa-EIO;
	} <linux/inmaY WA Place,but Wreset / initialize to a sane *   ilong with this program; if not, write to the Free with this pchincletm; if noCH_PLAY/sb.h>
#include <sound/asoundef.h>CAPT/sb.h>
#includeed a crogram; if0e Free S<soundude UT ANY WARRANTY; without even the implied warranty of
 *   MERwith this program; if PURPOSE.  Se, CHANTABILITY or Fe the
 *   GNU General Public License for more detailkashi Iwai <tiwai@suse.de>"e GNU General P		"{C-Media,General Public either version 2 of the License, or
 *   (at d/sb.ork. Warning#endif /* CONFIG_PMRANTprogram an redistribriver 	/* Ind=nclu.name = "C-Medialinu",
	.id_tse
 *TNESS FOR A Pids_CARprobNDRV_DEFAULT_STD for_CARremovNDRV_ibutexit_p(ystem in captenab),
#ifdefSNDRV_CARD
	.e; you DRV_DEFAULT_STe; you c int ure ITNESS FOR A PARure ,int indhis 	program is f__idi. alsanfo.h>AULT_STRnit(voidt undork. Wa>
#incANY WA;	/* In(&	/* In)ning may blotati ___CAR (SNDRV_CARDS-1)]=_CARstatic intcludunNDRV_CARDS] = {[0 ... (SNDRV_module=1};
s(SNDRV_CARDS-1)]=1};
)
module_K
staf SUPPORT_JOYSTICK
st)
