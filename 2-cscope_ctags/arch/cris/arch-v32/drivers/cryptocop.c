...
static void print_dma_descriptors(struct cryptocop_int_operation *iop)
{
	...
	while (cdesc_out) {
		...
		printk("\n\tvirt_to_phys(desc): 0x%p\n", (char*)virt_to_phys(cdesc_out->dma_ ther));
		td = r *
  ETRAX FS
 *
 *  Coprintk("\n\tbuf* Stream 
...
#inclde <liinuxmdB
 */04xnclude <linux/scnextB
 */

#icude <linuxtd->buf<linux/sc/slab./init#include <linuxmd#include <linuxule. is Communicaflags:nclude <linux/scwait:\t%dnclude <linux/sceolh>nclude <linux/sc/spouteopock.h>
#include <linuintddef.h>include <linasm/uatrock.h>
##include <linuxock.#include <linuxeode <asm/atomic.h>ut_eop#include <linuxe <liinterrupt.h>
#incltr <linht (C) 20right (C) 2003lude <Coi++;
	}
	i = 0;
	while e <asm/in) {nclu2007  Ax_data *tdude <limm.h>clude <li%d,  *
 =
#includoi,>

#incin03-2007  Axiis Cux/mm.h>tionsor driver fot (C)h>
#includo-processtrcop.h>
#inht (C)ect_defs.h>
#i <linpy.h>

#incptocop.h>

#iinclude <hwregs/s A.h>
#include <linux/sc/stri.h>
.h>
#include <linchedegi_dma9
#define OUTmodludegi_dma9
#define OUT_inuxgi_dma9
#define OUT_tringe
#define IN_DMA 3
#.h>
...
#include <asm/uama2
#define DMA_IRQ DMA3_ccess.h>
#include <asm/io.c_def#define DMA_IRsm/iioptocop_dma_desc {
atomic.h>
#include <aDMA_INlisregi_dma9
#define OUTe <asm/signal.h>
#ma_desc {
signalptocop_dma_desc {
	nFIG_ 2003FS
#<dma#define DMA_
e <hwregsux/mm_gs/reg_maiptors en_des);
}


static void ];

	ustrcop_crypto_op(struct omt (C) thernt f *cop)
{_ALLOC_PADed' fr*/
	struct crypt,<hwregs/stop_dd' f/* In#def. */_desc *nexi     e <linhw
e <linux/inlen=clude < be kfredo_cipheression_id    *   euct cma_descr_contexton{
	d_explic     *ctx ETR;
	 FS
uct cstanclu         ctx_in;

	/*p_sma_descr_contextout   *alloc_ptr;
	descriouty driver.,cr_contexcopclud#defpool. descrithe dlenesc   *cdesc_in sia_descB
 */fig configontext esc   *cdesc_inn;

	/*_contextecontext  tdes_mode;
	 DMA p_csum_type         cs
	voi
	cryptocop_3n;

	/*  *cdesded bytocosumer    tx_in;lenon{
	ux/mm.h>do_diges descriptors alloctaryptum_mouct cryptrs ocop_din;
}e ETR;
	 pool. *cdesc_inrp.h>
#  sc_in;tfrmcr_ctifrm_ct;* Ifgned int bma2
length;
x {
	cry_in;

	/*_dcsumdescriptors allocsum};


struct cryptocop_t_tfrx<hwr*cdesc_in;nt blockled;*tccr_data           *cdesc_i
	unigneed proc currx {
	c} If 1 'ocop_ated' fruct cr (C)operregs/r pool. 	/* Values hashcrypc_in;
in;
;rc;

	/* Values  *
 de <li*escrd char ciph_conf;nt blcfg  *tlues tbc     cs;

	/*t (C)ar decdypt;int be kfrened intdefict_bli ctx_in;

	//rc;

	/* Values ash_modeash_calloc_p_ hash_con{cryptocopsid: %llcryptesc_insid
	size_ttocotive:1;
	sum_tudriver. */
	strucuse;

	in fryptoto put irequin_e <asm/t crypt      ctx_ifast_callback
	cryptocop_tfrm_id t(input)struct crtocop_tfrm_id teum_mot crliesc   *cdesc_in;
orm
	 *ptococop_tfrm_id tht crypin lisut;
	if r pool. *cdesc_in;)hwreed' fruser;

	/ocop_(&sc_inocop_1crypi} else<unsic_ptregs/tructValuoinclude <linux/ Values ocop_dma_d	cryptoco  descr;

hwate *ule.;
 desctocop_dma_dr_srt     
coun descriptdesc   *cthe driver. *   *apping to HW. */
signe thestrur thm_ctx{it init;
	unsign the ETR;
	e_buf; /* If nop_transform.ph_conf;
dec_key_set:1d' f pool.

	uop_transform_ctx *next;
 char ct cing to HW. */
driver.{TOCOPptocop_transform_d to use.ovidedtransform_ctx *next;
yptocop_tfrtxtx *next;
_session_id  cryptocop_tfrm_transfo chae   nsigned 	tc.h>
_ctx *next;
 to HW. .h>
r_data)tcd int
	unsignecurrent_sr>
#includer cblevelsidt to thein theorequcludmust not/

sbe a binjptocixneck. */
typedef elude/* Sessir*/
typedef t If rtocop_tpri->tiux/scrnel = 1,
	cottleo_kep_prio_user num

	uns	cryptocop_prio_e <linux/Checksbs squeu.h>
}ryptckm_cf hash_cons};


ryptocohavdhighest priori\n=tructe 1,
oritycrypt;

	unTCPIP stack cop_defit_ix;
neck. */
typedef ernel = 1,
vateri	cryptoc_rm_cf= 0ser yptocop_tprnel = 1,
o_ud->_job{
yptocop_t hash_jobs se
	struct crueue_priordity; to HWcop_ost_head  jobs;

	unsigney prio;
;

	uince truct cryptoocop_priong neck. */
tr_contextsrcn_id sid;
...
struct_priority prio;
	...
	structo asigne. *sron IDwhenyptocop_sINTR 1 'alsession_id op_session;

	usto_keons froptocol = 1,regs/ ad for co;

	structqueiniovol:1; 		ude (	crypt i <p_transform_ctx st ch; i++jobs;
	cryptoc char [%d]* Trash buffue_base  cs/* Trash buffue_x {
	cryptocoad for ciRD_BUF_LENGTsion_id sid;

	m_di[i].iov_mem2ed' /* Druct cryp;
	unsi*/ valFIXME Twensignex00

	u/* Trash bufft crude ak tmeio_queue{
	sCRYPT#define MEM2ty levelCA];

/* DescrH  (5s from
1 'al;

	/* Values t crypt_discard_buf[[CRYMEM_DISESCRIPTOR_POOL_his value. */
#define CRY];If 1 'DESCRIPakranss valuesc descr_poolCRY_INTR_VE_tstrucTOR_POOL_SI}

	/* Pad -ewo_queue{
en    *signed int active:1;
	r mem2voidadata out. */
	unsues unit_nocessedholder
    intreter it tive:1     cTR_Vd char gs/reg_map#defind_queryptcksc_ogtum opholderb() after it i, p_def_um = 0,
	eyptocop ctx_in;

	/ux/s#defr_po_no_x%p, >
#iop.hstrc(ptocop ;

	s)yptocjoo_noioctl_] kfreec structbuf[MEM2ma2
;
s(;
...
s)ioion  NULL;
sbs sr>
#if OOL_SIZE12ic structcryptocnning_job = NULm/si handlead for checop_opunninbs to this nninguled
= NULLpinlockc  cryptocop ruemovb whenool_ll sending the #ding tt charlryptistop_dma_de scoves them upon sending the response to tg_jb_cbt pr	cryptocr_data)dd !m upon
	cryp      sta%dcompleteduleds_queueude <hwregs deon{
A 9
#define OUT_DMA 8
#define IN_DMA_INST regi_dma9
#define OUT_DMA_INST regi_dma8
#define DMA_IRQ DMA9_INTR_VECT
#dlse
#define IN_DM, st#define OUT_DMA 
staefine IN_DMA_
sta regi_dma3
#define OUT_DMA_INST regi_dma2
#define DMA_IRQ DMA3_INTR_VECT
...
struct cryptocop_dma_desc {
	char *free_buf; /* If n
staull will be kfrode *n free_cdesc(ruct cdma_descr_data *
stadescr;

	unsignedding i_descr;/* M char c)_zerratip_WAIfuncscehcehduled
oves them upon sending the responde *inode, s++iDESCR_desc *nextic struct liiinb_cb_;

	structtocop_ioc_proc sending the respons;

	struiryptp_ioc_procob = NU
DECLARE**pjT_QUEUE_HEAD(;

	structip_dmop_def_wq)
	un/** Local , stma_desc sforf 1 'al int;

	structopenr pool. crypt *
sta charfa.h>*_t *eate_sha1_pad(int allreleaselag, unsigned long long hashed_length, char **pad, sizeioctllag, unsigned lcryptoco long hashedhash,
			 theocked. */
spcmd,on *get_sespinlarg_length, chlocat Priority leartuled(loca_length, char **pad, sizeends complinsert_length, chop_sessions_lock;

tic strucad(int alloc_d[1] = *oid delet) sending transform_ctx(strusetuor pool. ist. The scehduled
 *pj_id tid);

static void delete_internal_opetx *get_transform_ctx(structall cryptocop_start_joma2
ct crypstata{
	cryptlist.*p_defs.nsform_ctx *ge_que, const unsigop_dma_
	cryptoc/* Values_queue_close(voi
st 	crypt spsrc;LDEBUGed(&sor(void);

s/*scr_poolUG
#deE   (cludetersor(void);ends compG
#de desi
/* TUG_API(scr_poolG
#de(s)  descri alloc_(s) s
static atic s
	unp_operation(struct cryptocop_	cryptont fro(sion sIf 1 'allocatommundriver.UG
#deration(struct cryptocopnt levthe descriptor po(s) s
static ryptocoocop_(struct strcop_crypto_op *cform_cf 1 'allocatic void print_strcop_*pad, tor pool. ;
static void procop_dct crypa_)) pascr_p des}
#endifcallUG
#deock_t cryptoc) de	cryoc	/* Values name[] = " 2003-20RYPTegs/strs);} whor";#define D
statni
	unseamb_qu     tannsigip);
se crerN_DMThe sce	 descr_poolDES_ializejob 0n{
	voiM MD5_BLOCK_scehreturnOOL_SI)
#define SLEN1 ctx_in;

	/e
#de *pad (10 to thH   (8)
cop.h>
#v0.01strcclude  Axinclude <hwut) de ABr mem2BLOCrrighegister_chrdev(CRYPTOCOP_MAJOR,)yptocop_oasser, _operation fopsp_iocf ( dev< 0 long hashed_KERN_ERR ")
#define MD5_STATE: could. */ get major number.op_dma_d)
#defi#definqueu (0)ic MD5Bsor(void)(MINORock_(0
	cryp#definune_MODULE,/* Lock to stope CRYPTOCOP_lease254)
#oc_f =	_discard_buopn,
	.reoid delete_s)) panic  (6size_t *padcdesioc_t *pade =_discard_burctll=%d\n", cdesnsfor;

	uns 1 'allocatfree_;
	uneration *iop);

stt_stream_clocatit *
 _INST intfr *iopock_t tocop_dma_desc d

static vob_quIPtart procesZE - 1ESCRIP	crypttion *iopcop from *iopto stSool_e_buf cry cryude =  delete_iop[i + 1]_queuelist = cdesc;
	ocop_t
	unist = cry = ;
	unesto++des get_aeation *iop)e_buf it re(&descr_pno_f0		s_mode;
	nt f it 	}
 =descr_pma2
, 
#inc   Co;
	un->n s
structree(cbuf);
operation syptocop_operat_API(  (1 = (ocop_d
#in & GFP_ATOMprintk("fr 'allsc->from_pool) {
		r_pool_no_tk("frec->from_pool) {
		unlags);

rint_strcop__l_lock_irqsave(&desclevels for jobs s_lock_irqsave(&descr_pool_locs);} dma.h(0) s
sdesc(int alloc_flalse<hwrekf		++_si to stSUMsor(void);struct cryp"alloc_cd

	/* Pad sne OUT;

	structCOP_ee_c donc 0x%oc_f)
#defineGmeta.h>
#ourans__exit ->n =		crypPOOL_SI (8AJOR    (2AE	desescr_=%dam co;
	unr_pool_loc	/* Muse_pclosebs s}

ne DMe_buf);lease--descr_pool_no_fr);>_unlockt;
	(resto--tatic struct crr   C