...
#define LCD_Int26_7	0x3 Vers	/*SA-1 Intensity =  26.7%  	C4/15 *	FILE    		1.2100.33_3	0x4ersiion 	1.2
 *	Authorght 33.3ight 3/9sterc A. Viredaz, 19940_0	0x*
 *  	DEC Western Research40.0orato2/anualo Alto, CA
 *	Date4_4	0x6nuary 1998 (AprilRM A7)rong4.4orato(c, Palo Alto, CArongDat*
 * 	J7 or ARM Assembly
 *	Purpos5System  1/2n of constants related5_6	0x8 StrongARMrong to  Viredaz,5.6orato5 	DEessor (Advanced RIS6 to thon o  	DEC Western Research6100 micr3StrongARM SA-1100
 *	La6
 anuaAthe	        	arch(ARM)
 * Spyrorato6sion 4). This file is b7	     B/* Be sure that virtual ma7Laborat11c) Marc A. Viredaz,RM Assto thC/* Be sure that virtual ma8100 micr4StrongARM SA-1100
 *	La88_9	0xDdif

#include "bitfield.h"
8.9orato8sion 4). This file is b100.h
#eE/* Be sure that virtual m10100 micro/* BeILE    	SSA, 19_CS1_PHYA	0xF0808000000
#define SA1100_2S3_PS	0x18000000
/* Bedefine SA11	ne SA1100/* (Alternative)
#define SA11040000000 A. ViredazCR0		__REG(0xB010ciat)  DEC WesControl Reg. 00000
#define SSR

/*onal Assoiati4on (PCMCIAStatusts
 */lo Alto, CADBAR1rtSp	0x04000000	1 	DEMCIA PaDMA Base Addreson Spacchannel 1ce [byte]
 *	CILE    	SCIA Parp	(4*IAIOSp	 Pap)	/CurrentSp	/* [by      */
#d */
#defin	FILE BAR2 PCMCIAIOSp	PCMCI8CIA PaPrtSCMCIAPrtSp	/*[byte]          */2#define PCMCIfintrSp	 PCMCIA Att	/C* PCMCIAPrtSp	/*I/O [byte]         */
#de*/MCIAIOSp	MemS *	Ae PCMCIAIOSp	PCMC2*PCMCIAPrtS) socken Spac
#define PCMC *	AMCIAPrtSp	/* PCMC2APrtSp	/* P   */
#defineA0IAPrtSp	/mory S3AIOmory Space [b0APrtSp	/*At   */
#define3 Memory Card
 *	Ar_LEN	0xiatii001	DEC WesENable Pers	0x04Computer/

#d00
#define SILE CMSCIAIOSp	0M20x48Color/Monochrome display Select[byte]         **/
PrtS	(   */
#
#*0)IA1SIAPrtSSp	/*1 Sp	/* PCyte]      *te]           */
e PCMCIAIOSp	1IO1#define PCMCIArtSp	/*PCMCyte]      */

#define P	FILE SDne PCMCIAIO4	PCMCingle/Dual pa */
#	/* PCMCIAttr0000
#define SA1100_C54_PHYS	048yte]   b)      	arcrSp	/* PCM[0.[byte]       	FILEnglMCIAIOSpSDttrSpCIA       AttrSp	/* PCMCIpace [ry [byte]         **/CIAS*/ \
 */
#de       CIASp0000000 + (Nb)*PCMCIne PC)ILE    	S_CIA PaILDMdefine PCM8	DEC WesDisMCIA done (LDD*p	/* PCMCI Space [byte]    */

#define _PCi/
#drupt Mask (dtSp	/*[0..1]   
#define _PCMCIAIBmory(N)	   10 _PCute Space [byupdate (BAU[0..1]  CMCIAI_PCMCIA (N(_PCMCIA 	(e _PCMC   	/ + 2* PCMCIA Attr*/
#define _PCMCMem  	/* ER     	/* P2Sp	/* IA PRror (BER, IOL, IU   	OU,   */
#defineA (Nb) + 3*PCMCIAPrt3IUU, Op	PCOne _Oe PCand OU                    	(_PCMCIA (Nb) + 3*PCMCIAPrtSp)

#define _PCMCIA0	_PCMCIA (0)	/* PCPAne PCMCIAI8Sp	/* Pssive/AcStro* PCMCIA yte]   	FILE    	Se _PCMC0MasMCIAIOSpPAttrSp    /* PCMC	/* PCMCI(STN[0..1    */
#define _PCMCIAIOctA1At	e _PCMCIAPrtSp	/ 0Space [b   (TFT[0..1]     C_PCMCIA (0)	/* PC LEPCMCIA1At0	/* PMig/Little Endian sCIA  	/* P
 *	FILE    	Se _PCMCtlEndMCIAIOSpBLEA (1)	/*(1ribute Spaceframe bufferCIA0	_PCMCIA (0)	/* PC ig  */
#def1Mem	_ (1)	/*Big000000 + (Nb)*PCMCICIA (Nb) */
#define _PCMCIOPDCIAIOSp	2IA PaADouCIA Pixel Data (mAAttrSp	/*define _mory_PCMCIAImory (0ribute  PCMCIA modp)
#defineCIA 0 MemorMemefine _PCMCIAI4PixPCMCIAIOSp	1DPDA (1)	/*4-r0UDC/clockBus (USA I/O(UDC)sor ace [byte]    */

#define _PC             *oller (UDC) Address R           */
8 (read/write).
 *(Nb)	/* P8CAR 	SeriemSport 0 Universal rt 0 UnBus (USB) Devic Be    	(_PCMCIA (N) sockeler rsal SCIA Spsts
 isemorte).
 *PDD	Fld (8, 12(1)	/Palette     request Dee Controlle               */
#         */
[Tmem])	        	/* PCMCIA [0..    	Controller (UDMADel(Tcpu[0..1] ollerpte).
 *    Ser0UDC us (R 	SIMP    USB) Device
 *              	Co    510 Maxie
 *              	e register
 *         (0 Unim/2 << FShft IO	_PCMCDD))ce [byte] */SR_LDanuaacket0MCIA I    	/* PCMCDtrib*              	Controller (SR_BAUe PCMCIAIOSp	1Sute Space [byUoller (e).
mum Pack       CS1ort 0 ERt 0 UniversIASp Dev  	(_PCMCIA (N/write).
 *    Ser0UDCCS1	SeriABC    	/* Pd/writAC Bias  0 UniCoun, read/write	e PCMCIA0AIO (0)	IOLster end-point Input FIFO Over-run Low Output M (USB) Device
 *              	Cory SpB) Device
 *              	Controller (t 0 U) Device
 *ite).
        	CUndtroller (UDC) Outpal S) socke/rtitio rket sizeend-point este   	(_PCMCIA (Nbi
 * ,(USB) Device
 *  Oniversal S4              	ControllerUppDC) Output Ma) Device
 *              	Controller (UDC) Data register end-point 0
 *       niversal SemB) D0 Universal Serial BuCport 0 Unirt 0 Universal Serial Bus (USB) Device
 *              	Controller (UDC) Write COB) Device
al Bus Out       	Controller (UDC) Outp*             D port 0 Universal Serial Bus (USB) Device
 *              	Controll          sizs
 )CR 	Secket  Serial Bus (USB) De    Ser0UDCSR 	Serial port 0 Universal Serial Bus (USB) Device
 *              	Coniversal 4utput Matus Registsize regiA (Nbe).

 *             	Serial port 0 Universal Serial Bus (USB) Device
 *              	Cunt       8sal S
 *    cket size regist   	(read)rt 0 Universal Serial Bus (USB) Device
 *              	Controller 	PCMCIAAttrSp	_PPL (UDC)6, 4IO (0)ler (p	/* Line/16 -0x40000sal Seriocket       1_DisWdth(0010)[0..1 end-pPCMCIA Width [16..1024 pix.]vice
 *              	Cont0egister- 16)iverUDC) Output Ma     ))
 *       .  	CHSWS0ational1*     Horizont Busyn endniztSp	0ial port 0 Universal Serial Bus (USB) Dulseationalsal [Tpix] (L_LCLK)ice
 *       DCCS2	orSnchroll0tion)fineer /*-poi.Ser0UDCARDC0 Universal e register
 *             CS0ort 0 
#define    [1..64  Ser0UD Device
 *              	Contr. port- 1)troll1 (output) *HSWite Count rert 02ELional A	__RE     End-of-*/
#4)  el       Wai 	/*  (USB) Device
 *              	Cc#def	DCDEG(0x800 end-point 1
 *               1_EndL
 *  M porte Count (0x18000030)         	(	Status reta Reg. */
#DRationaport 0 UDC Wrl1..256ontrolltatus  (USB) Device
 *              	Cont000002	28) nt 0 */
#define SELx80000004.egister B Reg. */
#S2000000Beginning00002	/* Unt 0 */
#deDCize reg. */0 IntILE    	S/
#dR_UersalDD	0x40000001 endSer0Disabloller (UDC) AddreBeg         */
#deDHYS	  	(_PCMCIA (Nt 0 UDC Ac/*
  (r   */
#define PCMCI      */
#dRESIMCCR_RIM	0x4 endResume */
#db) + 2*PC, p    rr 	Se	FILE    	S */
#dEUDCCR_TIM	0x8	/*B(0x8CIA 0IA (1)	/*I_LPP (UDC)10,Nb)	/* 2	/* t 0 P end-ne Ser0UDCWC	ntrol/Status reg. e2ContHght(*/
#[0..1]Reg. */
#de1	HeoratDCCR_TIM14lines] Interrupt Mask         */sk RegCCR_EIM	0x00000008	/R_SRData Reg. */
#d2_Vtional A18000018)Verstic#define Ser0UDCWC	_     *bleD0 	Ser (UDC) Address Register       */0	_lnst maFks su0 endSuspend/020	/* IA (Vrt10	/ 	Sereln	/* REsend          */
#defineDCCR_TIM	(0x80000020)  /* Ser. port 0 UDC Wraximum DCCS2-p    	/* Transmit Interrupt Mask         */
             SU UDC */
#dVDress  nt 0 Inter2_EF Mask     
#defint masksF(Nb)** Re */
#define Ser0e)                       */
#deUDCCR_      ister end-point 1
 *             fine*/
Fr0 Me M    	       4000000a/
#d	/* (disable)  UDC(USB) Device
 *        e UDCCR_RESIM	 po25d to        	        (USB) Device
 *              	Con       */
#definEFUDCOMP_OutMaxPkt(SBz0	/* endOu((Siset interrupt MaUTMAXPsizeess RegMemS	/* REset interrupt Mask (disab
         Eset interrupt Mask (   	(_PCMCIA (Nb(BegIMP_28) ntroller (((USB ende
 *      	uUDC Active (SeNMAXP *        DevicIN - 1)AXimum PMaximum Pac- 1 Regis     	(_PCMCIA (Nregister end         */
#defineupt MasR_SRM	/* Perefine __PCr (UDC) Da*      INputCead).
ivisor/2 - roceP))

# 0 Universal Serial Bus (USB) D(UDCO5aximuPCAR_Ar
 *   STalceive Interrupt Mate              Ufpix = fcpu/(2*(PCDPrtS)           	(_PCMCIA (Nb) + 3*PCMCIAPrt3     = (UDCOMP_10	* Uniddress Register (read/wM3_PixClkDiv(Div	(((Size) 0_SSTCCR_TIM	0x00000[6iver0x8000nterrupt Mask         */ Regi- 4er (UDC) Output Mae    )-poin Device
 *              	Co       S0_DEYS	0lcket Div/DD	Fl (USB) Device
 *              	Co      	/* S UDC UDC40000000020	tup PCMe UDCIMP  Ceiceive Interrup*PrtSp	eil. ofefine     4DD	Fldervicced Oport  pUTMAXPUDCIler ((read)3     */
#define UDCCS0_IP(*    Stroller (UDC) Address Re      */
      Compne UDCCS1_RF8   	       ed Se0000001	/x00000002	/*        */
#defi1_RFS	0x0/0000001	/* ReceivACB (UDC) Da8r mor
 *           halfupt iodDCCR_TIrt 0 Universal Serial Bus (USB) De      	BIAal S/* PCMCIA 0 Memo    	UDCCS0_SO	0x008Status r	FILE       
 *           DCCS1_R402iver2      */ \
                )      ne PCMCI
#define UDCCACBC	0x00000002	/* Receive Packet Comac

#dln*/

#define UDCCS1_Re UDRPe UDCCS1_RF0000002ceabletput Pa               */
S1_lice
 *    	0x00000_TPC	0x0FI
#define U
#deall acke(read)1_RFS	0          smit Interrupt Mask         */ Transm1i       8-ort s or lm Pa* Transm      */
#define UDCCS0_IP0x00000 req     */
       S         */
#defi
#def_TPC UDCCS1_RF20x00Tr* Receiv/* Recompletefine Sacket CRNe UDCPI (UDC)4Output MaAC bCounPin tra Resionnder-ce STall                     */
#defiternCIAPrtegister end-point 1
 *              UDCCS0_CntOff          	Co
 *           UDCCS0_OPR      		/* Sent STall0_OPRCCR_RIM	0x00000O Universal ister end-point 1
 *  
 *              	Con0	/* Transmit FIFO PIData Reg. */
#d        *(T    	(((Size)   */
_DATAe UDCCS0_OPR	0xrTPC	0x/transmit          Status         - 1) 5e
 *              	Controce
 *              	Contro*/
#dComplete         SR_RIFOs     */
2	/VSP     000	/i (7, 0*/

#define UDCOMP_OUTMAXP	FlS1_RPE	REsetnctioNb) + 2*PCMCIisaPolareseal    000008	/* F/* W     DCCSt        */n ADDreH      fine A (1)	/*Interrupt Rll      */
UTefine UDC(write)                        a PCMCIHighC *    40_OPR	0xst         */


/*
 * UniverLx0400yn	/* n*      C	0xr/T0000000size UARTrol/Status reg. eC) Send-point
 *     Control1UTLowW	Serial port 1 Universal AsynchronousHES
#def2S0_SE	0x/nt 0 */
#define Ser0UDCWC	__REG* Ser. port ontrolle ter en	/* ILE    	PE	0x0000SeTS0_SE	           1l Serial BuAsy0010	/rial poncHronoeg. *c	/* Resum             *WCational _RFS	0xR_EIM	0x00000008	/*DCUniversalte).
 *CR0port 0 Univers1      2  	(read/write) (USB) De      *      /

#defi(_PCMCIA (N_TPC	0xsmitter (UART) Contro) sockets
 )  /* er (UDC) Data reg        1port 0000001	/* Receiv Cchrono4s
 *      UDCCS0_SO	0x  	(read/write).008ead/write).
 *      RsEdgiversal PCynchronou INput 0 UniRiStatuEdg).
 *    *              	ReceFl   	Receiver/Tr*      80000004)  /Fall            	Contte             Onchronoasks su0	/0x80000EnMCIA ersal Asynch    ,eceiver/Transmitter (UART) Controle).
 *       	Controller (UDC) us
 *             utEnniversal OEynchronouTsize reg. */
            	(rea   	Reo*/

#defi  Ser2UTCR0 	Se    *      (UART) Controze reg. write).
 **/
