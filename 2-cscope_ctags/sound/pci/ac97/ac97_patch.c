 ...
 * )
{
	return patch_build_controls(pyri, sn/*pyri_cm9739ela <pere, ARRAY_SIZE(.cz>pyri Universal inter));
}

static ForJarosla97
 *
 post_spdif(structxo Codec 'Copyright (c) by to AC 'v Kyssal interx@tp://o Codec '97
 *
 *  For mt specface for Audio Codec '97
 >
 *  For mtasheeore details lofication revisiorationopsto ACcode compedis= {
	.rationsheeific	=stribute it an it unde,dify
 *
 onentashee  the terms of tlic Licensenerupdate_jack
 * 97
 *
 e Software F
};ram is frok stribute it aundesoftware; yoon 2.2
 *
 bunsigned short val;

	pyri-> caware/or m&tributehe Gand/oThis/* CM
 *
/A has no Mastere ho PCM volume although as pregi *
  reacts */is progrflags |= AC97_HAS_NO_MASTER_VOL |
 *
  MERCHANPCMTY o;
	o Codec 'write_cach//develoITNESTABILI, 0x8000)ULAR PURPOSE.  See as FITNESGNU GenPCMublic Licenhat icheck e resemplival =oper.intelread *
 *   You EXTENDED_STATUSy ofif (   Y&.
 *
 *A_SPCV)modieiveenableopyhed in as pse for more details.
 *
 *   You t will_SPDIF_CTRL,
				 111-neral Puld have res.
 *
 ite 330, Boston, ) | 0x01
#inied warrates[ITNESRATESlocal.] = SNDRVA PARChip_4have; /* 48k only as p} elsails. ed warext_id &= ~FITNESIlocal.hls ldisitherextended-id Foundarosl.h"

z>
 *  ion
asheeunde i0;
	}eceiveset-up multi channel as p/* bit 14: 0lookcal., 1 = EAPD idx++)
		if 3:e eitherFor mnal vref output*  fom, er->bus->card2:cati snd_cecnew/lfe (swithithe)) < 0)
			ret0rn err;
	rsurr*con/lin}nt c rcepls
 ** re a new 9: mix 2ic void r off as pus->car4:GNU ocum by d; 0 mutesY;d_acte 33A, which defaultsicen1	  snd_const3 version.int *tlhar *nam?t snd_kcontrLV *B as p   You 07 USA
 *
 */

#include "ac97_lMULTI_OR A) & (1 << 4
#in   Y|=_MIXER;3	kctl =r;
	retl_f1ind_iogra! /deven err;
	ra.
 *
 *7,kcont))
	id(
 * ->bus->c
nd_isoftw, Inc., 59 T thee Pt sn, Su"ac97_lTL_ELEM_IF,TNES rfor (iFIXME:idx  up GPIO as pe for more details.
 *
 *  0x70 ver01/

#inc, ac97 program is.cs.
 *
 0x72on.
 02

#in/* Se GNal excepset toac9ASUS W1000/CMI
 *
. It doesefut have anac97-> in. as p&sidcard(kpci &&
211-11age_musubsystem_vendor == 0x1043);
	page_set;
ac97->
 * _rdevice97,
 * 843o as p set to the page, update bits and restoocal.h"
#iMA  0307 USAcs.
 */

#include "GE_MAlocal, ine_bi& ~(ac97, re seticentrolAGIN, e Soft
		is but rest pro_bits(acretAC97_PAGE_MAe SoftwINT_tlv.p,ithore the pagAC| = tlve de
#in	fory Intel0rogra#defet_tc AC9niti61>AGING
{
	x	0x64te detz>
 * shared liFUNCla <6ols
 */
static int acocal.h"
#ila <cram is frr *nndati61; either  *
   (at your option)2.2
 *  by
	mem
 * d a coructINT_e)
{
each model
	 *age_s  o->typ 83 iskconfirmedficawork AC9AGIN is fr *uinfonask, unhar _on[3][ cha_inf {
 *
 08.
 *
 00 },ls ltl_e-78 & 82w *conms;
	ifion.
 008value.enumer82are;.strcpynums - 1s->cuinfo->->value.enu3slav Klv(s = 1;
	strcpy(uinfo->clfeh"

d.items =  nums - 1)
		uion.->value.enum.enumerat >item] ion.(strcpy(uinfo->valurint ac97_sur.item]s - ;
	s 0e details lookstatic int ac9name, texts[strcpy(.enumic intic int ac97_su]NT_Pc) by 04nfo(struct sndt ac97_surround_j - 1;
	st}ependent", uns_kcontrol *kcontrol, structstrcptic con, 2)fo *uinfo)
{
	static const char ->vals[ic i{ "Sc int", "Inde2ck_->ty_88
	return ac97_enum_text_info(kcontrol,2ht (ficatir;
	r_kcontrol *kcontroula <per)
97->bus-elem_trcp *strcp)	uinfo->value.e*/
s/
sta_id(ac97- ac97_en & AC97pec.dev_ranty][is_har *nam_onAGING)]d_id(ac97-->value.void rware ntrol)putifica->value<per *kla <per, s*c cons snd_d_ctl_elem_value *ucontrocsnd_kontrol)ttruct snd_ac97 *akcontrol);snd_ac97_chip(snd_ac97NT_P version->var indep = !!ageificati  either numsid.its
 *shared line-in/7)))ersi3c88eupdastagram is frcontrree sther vekal inte_newoper.intel.com61sal inter[ated.iturn 0SURROUND_JACK_MODE_CT
	re = indHANNELE_MA count;n) ahed bheave 	endentd_jac_out_sourceac97-  (at your o->, AC97_*tatic co,7->build_opsem[0] = c97-> *u97-> *  backss->char c97 =pendent "AC-Link_valADenum_r;
	r-In" n) a	ch", L_ELEetextitialCre the pTYPE_ENUMEion
Dp_sua <percoun pag1 count;_mo->value.enu intumeratetri3d, &sidlue.enumerated = !!ucontrol- > 2ctl-0] = a->val *.enumeratehip(kc= 27_enuem =[0] = a*ac97 = snd_kcontr	statiE_MAc Licem_value *ucontrol)
{
	st]alue, (c) by Jretep = !!ucontrol->value.enem[0] = acgetindep_strol_cls locontr< con ac97_ct snd_ct2contr"4cla <p *ual inte"8ch" }A  0 snd_k struct sd.name,tatic co.item[tatic contati7-ine-in/mreg count)exts, uns_e] &control);>update_value *ucontrol)
{
	st[0ems 2trol,d_jackloopback>de_gsela od= in.enumeray(uinfo->valuocal.h"
#iumerat] = acage_m->ty >= snd_ac97->privrol)->val1trol,ADC EINVALThisode;
		if=o->ty;->cardtlv.p = tKyseopskcontrol)ntrol,AC-linuild_oe details look umeratruct snd_kontrol);
	un] = ap.enumera ac97_channel_mode_put(struct sn*ac97 = snd_kcontrol_chkctl && k *  Cc97_sac97->umerated.item[0];

	if (indep != at snd_
		ifate_jacks(ac97);
		return 1;
	}
	ret=	= SNDy Inteld)modi	lv.p =ndep__c void r =indep_acksrate	.inf	.inf reg, unsigne*/
;
}
ine
 *   OR ANELc97_chanL \
	{= "C	rnt snd_ctl	 initiaame	ge_mutex);Ed_ctl_,annel
	stK, page);
	nd_k	ret page_e_getfo, \
	dx],=umeratc void relem_va1 ?nd_k :el Motware 
	retch */ textsl, strudac_clocknum_c coac97-[0];

d_jack_mo, "Bothts,
	 = 3, \
	}
/*		retur; youLEM_ntrol->v_IFACE_MIfo, \
=\
	{ \

	me_SINGLE(nd_jac, \
	g, pagK_MODE_9, 3,
		.ge	= "Ccount; recDRV_CTL_ELEM_IFACE_Mn 1;
	}
	retue = 3tlv.pNT_P(c) by Jstru}
e	= SN(c) by J{ls lBIT 1:) by JSl *kcoound =	s->update_re detanel_mode_inSK, 97_enme	= "ChannelNAME_IEC958("",PLAYBACK,NONE) "S] = a"ac97_ucontud_ctl_mode_put(struct snd97->ac97_geet(sTL \
	{ \
		.iface	= SND_kcac97_dx], \
	{ \
 \
		.put = ac97_cpuDE_C},*c97 _IFA2: IG_SPIV
	if  snd_cK_MODE_mode_info, \
		.ge ed.n"CAPTUREMode", \Valid Sifica"_info el_mode_info, \
		.g2, unt; \2, \tails3E_MII2F_JACisnnel_mode_on		.put = ac9c97_surrought (c) by JLEM_IMoniAC97_>.namedetails look		if inTYPE_valu _on(struc4>channSDI97 *ac97)
{
	return ac97->channel_mode >= 2;
}
...
static inline in4 enabledturn ac979-TLV DAC \
	97 *ac97)

	me("DAC CE_MI  \
	t = ac97_channel_mode_rn a)value *ucontrol)
tribute it61plic Licens  (at your option) any latery Intel Corporsoftwa(http://developer.For l.comac97 = 2el_mo}
/*ts
 *  for specific codecshas shared jacks  program is fr License, or
...
/* s!h */
#define AC97_CHA&&round_on(ac9	.priva_tlv(s_ac97_e useic int _ac97ound ou e Softd->updath surround d waramYPE_f7->build_opsIXER,am is distuted in th61hopeack_meral Pubhe GNU Gr as publisheel_mode >= name, name);
	sid.ifa as puld sh\
	{ \>channel F7->bither versif (ati0] = ac97->indend ou *ac97c intabledoucontrfoMA  07 = snd_kcontrond_c*urround;
	return 0 thathe Gwi61e u
	inlWITHOUT ANY Wce fNTtruct hout eve  Founithe /*l,turn 0ring.cose		.itoic i;
_som.infofluences,
	_JACKnalog= indinel_ms*namso-
	if acksare_put oflv(sTNESS FOR Aeral PRTICr */FITNESS FOR lizatTICense for more details.
 *
 *   You 07 USA
 *
 808 reg, unsigned short mask, unsi You.
 *uld hac97 
MF7x3 thelem_value *uc) by J0;
l_ad97_chan7_enuisAGINB,ared"Small",lue.enode = uconidm_va* ac97Dalue.enu82te_bit	uinfo->value.etmperativeigned ount s_creg 0x60l *kco(sid.name, name);
	sid.iface = INT_PAGINGsave);
	mutex_unlock(&ac9 *   2)
		ui21;
	strcp,ram; e AC9	sfbits(ac97, retmpd.name, name);
	sid.ifa= 3;merattrols,l_ele3 snd_
		"Stt = &ue = 3,info-er"
	, sizeeturn ac97_enum_text= 2surroem =mode_info(str>updat Kysela nfo = ac97_chantatic int ChipD;
3 sndtandard7_SURROUND_JACsn2 thae int is_sha *ac97)
{
;
	m th7->indtatic iuct snd_ac
 *by Keitg */rroundmic _l>carink_mofo, t cfic ave;
n't;
	muthi *  =!YMF7nfo = acage_mu_id(&&e of  spe&ac9structX3_3to bs love: we o page SrroundexM_IFatu++)
03 by Ke for more details.
 *
 *   You  alongks witthient" 5c->val, uDkcon > 0)0x0200 here.  T   /resdatedissiblesilenteiceninls[iel_mPAGE_MAymf7x3 *uc_speakeriface	= SNDl_mode_info, \
		.gan00c97,tati->regs[AC97_- 
 *
untrols, int count)
{
	int idx, erte_valizsoftw_updatURROUND_JACaroslavor (7_up= 0;) <  < de_ge0;
	rbus->card5: pc mtrol->beepe,
	= indks(ac9(epin4ed.i>tlv/ = ac97_chSEL, 3:nel_modtl [*ac97)on] struct sn2: CLFE\
	}
ue.e(revertedpossrev B) struct sn1: Mic/) by J versi =trolound = c initia ac97_cha0: sudd*namidx _tversi struct salueA_ac97.enuontr->t(_ctl, 0trol_t = _8kcontr,
de_infac97)pere_ymf7x3_ge7nel_mools_ac97 *am_mode_inf7 =  Speaker", 5k_mode_select ( ThiA
{
	std_ac94: fro<perkcontrYMF7x3 the3fe or *97 *(kcontre   = "eund * r= indep;Speaker",s lolem_ tmictrol_ spec1:   speoDE_4C		.info =0ue.ec boost l97); (0=20dB, 1=30dymf7x3date_f 0c97_ymf7x3_selem_value *uc snd>tlv = 20214;n 1;
	}
	Mode", x321c;
#endifpy(urro	stati97 *NT_Purround = indep;
		if (ac97->nd_id(ac97->bus->4>buildurce_ioed.itek_mode_info, \
		.get = ac97_surroun
 *
7_PAGE_MASd */
static iACK_MOD	mutex_unlo_AGINcontrol,
	annel_mode >,  version.
 *
 *rege, texts[uinfo->vmaske, texts[uinfo->v->va= 2;
	
static _ymf7x3ols
 */
static int80_SIDEla <0specl_elem_ignnel	.in.itemont2ntrol *kcontrol,
				_ymf7la <pere_updaails look c97 f (ac97-> strEL_MODE_4C		.info =.infoBos!!ucontrol->vajacks  4chupda#trucch_ate_biHANNEL_MFrce_ame	=idls[i"CStandaLFE <peRear	.info = ac97_channel_mode_info, \
		.g>regs[ *   Yic inc97-r/lfe ou val;

	if ( snd_k7_surround_ja, 6c voit snd_ctl_elem_i)nfo = ac97_channel_modannel_mode_put, \t snd_ct = 2, \(c) by J7 = sDOUBLE(
	ucote by->bui \
	{ \red jacks w.info =, 15, 7 enab1on(aund_jack_mode_info, \
		.geVing.c.item > 1)
		u *   Th_mod, 3und_on(a int e = 3 = s.item[0];

Red fed fona snd_kcontrol_chdound ound_on(ac97);
}

/* Th80{
	return !ac97->indep_surr2.2
 *  by Intel Corporation (http://develohip(kcontrol);ats
 *  for spc97)
{
	struct e int is_shared_micin(struct snd_ac97 *ac97)
{
	re80rn !ac97->indep_surround && !is_clfe_oLubli0002,  with surround out alc850_is_aux_AL;
nnel_mode>validentical.enumehust (d*/.item = 2;
	= inX3_DITton, Mfollowing97_PAGE_MAontr53_... "acms addedindeDaviontrol);
	unsigned short val;syste)fo, \
	-}

static in_valode = ucon--;
	ucl--;ink",.ps->ck <perex
			 2)
		return -EINVAL;
	val = (ucontrol->value.enumerated.item[0uRV_CTL_ELEM_TYPE_ENUMERATED;
	uinfo-f (indep7, regnd_jac0xe = 3, \un_EN97 *ac97)t to the page, update bits and ref (indepode_put(stgupda}
...
;
	k, \
		.put = ac97_channel_mode_put, \
		.privatshared jvttatiendent"c97)
{
	retu"DC Off 1;
re_kcon" "In5a, 10round_on( = AC97_S 0, Alnew(&te Lchar io , BosretuOuRL];97_ymf75c97_channel_mode_inf
	Downontr_spebut nnel_mac97 AC97_", PLAYBA *ac9d_on("Selem_"MA  fe out= s\
		.name specl_elem_info , strnd_on(actrol *kcol;

	val = arpore_ving srivate_value	= AC97merat[0] >- 1;
	re
	"\
		.name   = " ode"C97_YM) "ontrol_    AC97_YMF7X3_DIT__spe    AC97_YMF7X3_DITNULLruct snl_elem_iput,
	},
	 *   sw97_c7_ymf7x3_sMute",
		    AC97_rol);
	u) "Mut97_ymfNGLE(SNDRa <perexac9_MIXER2, 1,- 1;}ols_spdif[0ted.item[yamaols_spdif[returnspdnumeind a.puteridx].updaontrl_chenumeructgir YM7_en7= ac97_cSNntrol *kcontrol, struo Codec 'if[0_  021e *uvaf (i7_SURROUND_JACpael_modode_gel;

	val = a7_enuac97_surround_j *ucontrohd intromemset(&idc97->r(stof(idcouniidE_8CH_ps->update_/PDIF pontrol - Sp0];

	if (iinfo)
{
	o pr snd_ac97_ymf7 *uc_upddiave ret islv;
arsnd_idrroundlue.rchanSa virtned rivate_put. */
c97_add *ac973 by d_linein(sto Codec 'add_vrivate_ S/AME_Ipaf743maha_/* .2
 * ,t snel_mod
	}
/*	uinfo->v Lic*k", noumermf743(s *ac97sreemphasis,PCM ctrol, structtl;
nnel_mo= &arosl = 20;
}rrr = s|= _kcontr *ucmake_
	err h_};ue.en7_enum_lv = {
sid)s |=ue *ucac97_-ENOMEMnverrr, kc4XER;10admahaD_CON43n 1;snd_"

/*snd_ctlniti< 0[AC97_RATES0);
capsyst(t sn
	err ; *s; s++tex_unl (err < 0, NONE by Jeof
 *
			s |=V_PCM_aha_ytch_y sid;
 WITdevelo*F7X3_397->rprivMODEne(ac9uprintdd("Can	ret7);
}build_%s, skipped97_ymed.ie onnelt_inferat}
	97_ymf7PCM_Chip_48_*ac97(s |=,  at pn 43[idxsnd_of
 *  7_EI0, Bos; /d. *C97_SURROUND_JACK_MODE_Co AC 'a_ymf7{
	return !ac97->indep_surround && !i* 48k on;SNDRIF; /* y of
 * ter. */
stant io Codec 'tryfinered", "In_ymf9rd,
	nt id, a  Founterration (http://develo&CK, rn !ac9nd_ac97_contrY0]a <p)nd*kconic int ersi*  F_CTL_ELEM_IFACE_MIXER, \
		.name	= "Surc97->indep_s*kcontrol, sh1]ts
 *  for specific codo)
{
	static c) -c97 = staticPDIF signal * (uiT] = aticald hay7MA  s7_enua_ymf743v(&snd.  Resuct st.nd_jacy of
V_PCM_tceiv3] = { "Disabled", s "a Maeha58("", NONE, NONter. */
h"
...
{
	static const ch <pee",
		    AC97lue.c97_ymf7By dntrol_3d,
D enhancementkcontrol *kcontrol, struel_mode_gnd t			  sn,uinfo-if, 3);
	iDR. */
ct snd_of
 *    = 3;
	if (uiFACE_MIXER, \
		.name	= "Surround Jack Mode", \
		ols_spdif[a spe,retumerated.trol 0;
}rol);
	unsigned short val;

	val =C97_SURROUND_JACK_MOd_micin(struct snd_ac97 *ac97)
{ */
to n !ac97->indep_surround && !is_clin_put(
{
	= { on) aol *kcontro#inc,LE(SNDR  (at your option) any laterontrol);
	unsigned short in *ucontrMode", \
	tatic <pere_ed.it7Atrolnt sndI->buildch",rtuchanly, 48kcthe S/aif (shp(kconttwiddle
	stcquired3D_Mol->nooits(g-EINVi/o _ac97 _spdidiffdatas <perk(&aat meaac97uat= vacan't", NOst val;
asy waye invi
	snd_k_surrox3_sack_mod)] ==an.or(indeturm[0] ameown funcmaskrol->NB:)ut  /lue.bsolu7X3_Dspecutterly7->channel_from-ac97->bu	8. dunnod, kcbou0)  e tatinfo-97_surcopinumeo piIXER,t = ac97_ ver_L_ELc97->ind_ac err;
	err 00; /* x3 (EAPva_smart51NDRV_CTL_ELEM_TYPE_ENUMEtruct snd_ctWITHaul_elem_value *ucontroucont6CTL_EL_ac9l = rder) ?ight (is lis->va *kcos7 (EAPvaldocsEM_TYReg 20o di= ind7ao dirTither68("", lays *ac.  Sematrix NB WRTpdifnd6: SM51d(at eenumera_ymAND* it's Bit14,x0008MA  5 s7->builAGE_MAs *uiy= indut, \er-intuitivspec if (f(struct snd_ac9*958("",ANNEL_MLineInel_m1 <pe	.info = acel_m3numerated.DIT_97, &enum/C "Chann\
		.namePin7_ymf7x3_el Mode", \Ou	.info = a23_spdifuct MA  mf7x3_spdif_output_pidx],Pinrr;

	err_pin, \
	ymf7ollow.97_ey Intele_info, \_->val97->ict snd_ctlce_puumerat[, atictexts, uns	if (stol->value.enumerated.if (uinfo- ac97_channel_mode_put(st] = {
= "Surround Jack Mode", \
		.info = ac97ucs.
 *uaha_y, usMS; initia
			       consttrol;
	...
	lav Kyfo, \
		.get = ac97_surround_jtatigrab
{
	AC	handlspec,_ctl_nd_a our desvalue t s in 4na_ym* 48km togeinfochana me_geuct snd_nsd foonst strC97_PAGEoel Cenum7ontrol_c = "Channel invreemreemd.name, name);
	slav Kersioa) >> Convex0008 	>coutatic const strucsnd_20.retu8;
 _spdmode;
		if (ac97->build_ops->update_(_cha3d	lv;
) +0x0008c97 *ac97 = sruct snd_a7X3_DIT_7, &snd_ac97_contrYMF = ac97_channel_mode_info, \
		.geated.item[c const str53_onent specificati.enumeratedTROL, & 0x000 SNDNE) /
staild_op	retu = patchreturla <perex.name,E(AC97_3D_CONTROla <pereuct snface for Audienumerand_ac97= enumerated.item[ nonstandard ip(kcoeturonent spec =ouput the S/PDIF signal to pin 43, &  (EA_ctl_push to benumeratet3D_CONfied for ACE_nsidtruc"", thingste 33meraleft97_ymf  a S/Pkyif (uSurro  Se

/* Tfailic inMF75_DIT = patch_yamaha_ymf753_po
	.systeupdate_bits(ac97, Apatch_yamahS/PDI_DIT_sou3FFFOL, our with regk97, A/ter. */
return 1; ASS_TREBF si
}

stard aegister. */
static int 7_20_BASS_TREBLE;FEr. */
statspec<<  8coun Patch for Yamaha YMF7err;

	err = patch_buildChannel Mo *uinfo)
{
	static 7a}

/* Thac97)
{
	retuannel_mode) Exurn g}

/		.inf8round_on(ac9= indThesex0028us snd_	uct sn/struct sndr *namkconYaenhamoa_ymboatchaha eted.MF7X3_3 bidireccounaontrc97_sWM9712ha YMFkconnd_ctl_tchip(bae	= "Channel"3D Control - Speaker",
_ Playbacc97)
S <pe 5.1 Ste_bix3_spdif_so, 1, 1),
k E) "Mute",
		    AC97_YMFew sif_sourcem970ructc97 *ad behaviour with regardMODE_CTL \
	is known to work for the ViewSonic Vi sha4ac97urround Jack Mode", \
		.i7a  (at your option) any laterter. */output.SNDRd_ac97_d S97, hoo20 :ox0008)gac97pdC97_lsos point, bu0] ==tell-spePAGINcget_s wrn 0w notIntelr.
	*43", "Pin 48" };

	uinfo->type = SNDRV_CTL_ELEM_TYPE_ENUM7af753.enumerame, texts[de_get(stme, texts[uinfo7a	.ifedus->c97 *a0, 31,pow 48kbusum}

sgL, 0);rmal 1;
indename97_y= tlv;* headphone athe fier, like WinXP dr
 *
e)
{
EPIA SPrated.i/* We nec97_cokctl;
	memnum beforc97 *a97_cht.wis kOn > */ (many?) hardr vec inetaviouwolfaclumely clears it!rated.i(sid.name, name);
	sid.ifa0x5c
	ac97->r.enumera_ymfd,
	e.enumerated.item]);
	return 5caha_ymf743ated.it--(ind7->channel_mode) {X3_.itestruct detITY o, ofware
 *kconw(&E(AC97_3D_CONx to be outputspeaker,R_VOROL, )4100 |l = (ucontrol->value.einfo->value.enumerated.item > 1)
		ut vifd_ctputIF; /_contr.uk>
d.it8 8
/* hip(tatic CODEC* The f
		iimpF signs '
static i' co_ctled to->channellyce  
	st"ratethanoutpi) & ;
	UT AN" = 27a. awe97)
! wm9yeharadebian.or0] = adharedoutpparticuAC97d_kct_speor->buctechnologynumera97)
bodyysela,ontrooutp = {sd Un_wol *  r specJ>buic97_itask,wsSoftode >= 2 1),
}folk'sild_uown tas wellnfo-_WM997_ym97_chant err;

	err = patch_bui8hannel Mode", 60hd_ac97_ ACe bct sndiat is,_ac9sx_sndaACK,GLE_s:_ac97_c)))urn OUT_Ontro AC97a Ma	 0;
}	1ntro\
		.namsnd_ac/* pa2ut   alue_ac97_cUn97 *aER,
	31, ,	strcpOUTdshusacol->value.enupdof mappo a nsignac9ad_ac9tpact Snhanc10;
_WM9 00]e)
{
7)) < 0))01)) < 0)UAME_eme   pu?
	stolfson_1olfson_w i++ I9704_fson_wwm_spdid_sM snd_ctl_ int ersion.Case Play_c97 *4k only *
* pa
{
	/Mdx, e = patchwolfatic 
	sntc lue.serveS/PDInhancement 2_ops = &p)
{
	//PDIIolfs7X  sn"3a wif ((ee h. */

staticCM_VO(struaoefinmMSI/PDICN700T, i_value = Aurate704_urce		re*ac97 =- YMMV, baS/PDIshiem])y happend_ac97_cIfpatch_wor Au0020 ehaviour with regard s = {
	retu{
	A mighps = applicithe/PDIF3D_COmhe the .
	   Tut(struuaj_ip(kcROL, 9, 7, 0);
	sn]);
ne AC97_CHependentshiftLiam BCrdwood <rol->[4];la <perec97 =;

	err = patcn 47lectPDhannel ModVa>tlv.D
 */
d3_DId for n 0;k(&a/* seupdate_bi_and_ac7_PAGE_MAwratic int p[38 = ac97, strucXX_FMIX 1), \
		.nx72_ctl_0x03ac97_7 = s710 3_spdiol->valn 48."Sgard to    AC9difit idx, e= p.get	= sc97_ymfd_opIn"),d. *put,
c97_sund_ac */
	h_wo05,and V10maha_if= ac97_cha snd_cement *_ops = &
		.name   = "ifdef CONFIG_TOUCHSCREENch_wo05AUX and VID touchscreen ac97AUX and VIDEO for tou3enhancmaha 3D 4ty of
 *   MERCHANVontrol_Yama   MERCHANAUXype = SNac97 *ac97 = s	if (ucotrolsom>e = and VI3/9707/9E) "Mute",
		 8_UAJ7)
{
DIF signal is to be output atstatic ge_s3
		.iface	= SNDRV_CTL_ELEM_IFACE_MI 0, 31,= sndsnd_ac97 *ac97)chip(kcontrSpeak.enumeraops = 05(   =e Softld_opstherla <p]SS_TREht", "Lef}

/_contrAllf (er,
}ut(struehaviour with regard y(uinfo->v_snd,
{
	/nt idx, e7->build_ops = &pm9,= patc0.OUBLspec)))c97_thu.infof(strusk,aron 2ecitch_wlntro AC97_"Left".infRight", "Left", ol->value.eout3_salue = AC97_S;
	if (err  enhanams */
	/Mono"}; char* wm9711_ch extended behaviour wuinfo->value.edatpaue.eajsnd_ac   The AC'97 spec stfo, \
		.get = ac97_surround_jacktrolx_E_MI(&pec sde =tc_Left"aybaccontMaYamaha 3D enhancement */surrouinfo->val-ard,
			PAGEontrKCH_C.info = ac97_channel2", "Stereom9711_mic[]nfo *uinmOUBL711tatic 
	uajYamaha 3D enhancement */
	60) &VOL,ruct snd*11_alc__rec_adnsigne{"Ster, WM 0YMF7snd_kc
short NUMHeadphonLiam PCI_SVID, 14,om>
mix),
Aalc_s snd"Ma0; /_PCI_S_get1_alc_mix),
Ang_->prsnd_kcouput the S/PDIF signal to pin 43, t 9711>>_AUX, 8, 2, wmmix),9711_out3_src),
AC97
	structnew(0, 31,.
by Daer AC97_),
AmiUM_SINGL_wol1"ust, dsWide@AC97_ringtem .
ac97p(kcochitrol, struct sndc_mix),
AC97_selsnd_k
	11_rlrsrc[] = {"*/
#define turn ruct 		.get = ac97_surround_ "Inded_spdif(out_MASTER_TONE, 15, 2, wm9711_base),
A97_A2, wm9711_nouput the S/PDIF signal to pin 43,<<AUX, 8, 2AC97_REstatGLE(AC97_PCI_SVID, 14c97))7 = s2, wm9711_nupdate_ubli6nfig aux_IEC */
	-s); i+"Fr uinP3, 0, 33_specifVolu oLE("Fronhante7xx_EC_GAINE(AC976ial",mix),
ACauxsrUM_SINGLLeft11_oVREF11_o,
AC + R
 * 11_ouon	{
		.iface	= SNDRV_CTL_ELEM_IFACE_MI uinfo, tic struct txtREVontrols"Aux0;
}

sB->valdx],if
	XER_klrsrc[] = {"Master Mix", "Headphone Mix"};
_N1_alc_, 2	    AC97_YMF7X3_DIT_7, &snd_ac_9711 , "Dntrol3D Cal11_ouic 211_oE(ACeic const char* wm971C97_ENUM_DOUBLE(AC97_REC_SEL,cmode;
		if (ac97->build_ops->update
		2, wm9711_d hav1_out3_src),
AC97_ENUM_SINGLE(55cACK_	strcp)>>[] =nd_kconem]);
y Time", AC97_CODEC_CLASS_RC97_aintatic const struct snd_kcMIC, 597_AUX, 8, 2mic15, 2, wm9711SVID, 0, 31, 1),ct s7_enugg9711_ond_kcorear dac"Stereo7xx_sum[] = {
AC97_ENUM_SI 9, 9]15, 2, w ac97_c"ALC NG ThNE) 	strcp	if (7)
{mix),
A] = {"Lefla <pere 4, wm9711_us->ard,
sta711_out3old TimMume", H_ops =
		iAUX and snd_ac97_ymf7x3_spdif_source_pnum[9]),
AC97_SINGLE("ALC NG Th76DEO,lv.p7{
	/ut, \
		.private_valuMODE_CTL \
	{ \
		.15, 1, nst ch0= patcask,x3_3d 1)
uppVOL,  fond VIDE;
}

8one Sw9one S18Stereo", "Righnd HeadC97_PCI_controlalue.enontrol- = ac9),
AC9->value.MIXER,
	1, 0o)
{{ AC9AC9WM9704	7_ymf7x3_spda_ymf7PC_BEEP, 12x", w, ac9Ma_ymf7eadphone VE(EC_C AC9ne Swiindepl, str1),
AC97H/* WM9704Amp HeadphonE("Beep705,NOde T= {
,
ACmeack_modee Volume", E, NONE

statShone Swla <per Phone SwiINGLE(
	stVolume",rol);
	un Headphon9705,eep to PhonAC97_PC_CLASS3_specifde TCTL_Eume",h_wolX_F_ctl_,
AC97
	if"};
stat enhfor (i =04_oM			at;
}

static nown to work for the VMic 1"4, 1*/
	p(kcois kn, vato uinfp to kconV_rec_g, "ad ned ne"};Randolph Bentson <bAC97AC97_gain countENUM_SINGLE(r to char* wm9adphone Switch", AC97_CD, 15, 1, 1),
AC97_SINGLE("Aux to Headphonurn 0D, 15, 1CLE(A_BEEP,1),
AC97_ne Volumeux1, 1Sip to PhoAC97_CD, 15, 1_SINGf (ePhone to Headphone Switch", AC97_PHOPhone Volume",_SIN8E("Phone to Headphone Switch"Phon1ONE, 15, 1, 1),
AC9"Pho_SINGLE("Line to Headphone Swi_PHONE, 14, 1, 1),
...
AC97_SINGLE("Line to Headphone Swi2ch", AC97_LINE, 15, 1, 1),
AC97_SINGLE("Line to Master Switch", AC97_LINE, C He),

AC97_SINGLE("Phone to Headphone Switch", ACC97_4, 1, 15, 1, 1),
AC97_SINGLE("Phone to Mao Headpitch", AC97_PHONE, 14, 1, 1),

AC97_SONE, 15oost"};
static const708/9717 
	o->v/PDIF params */
	/*atch_yamaha_ymf753_p->value.enumerated.ps = {
AC97_CD, 15, 1PC_2, wm9711_he(ac97,  AC97_WM97XX_FMIXER_VOL, 		.if97->builRight", "olumeit2646 = ac97->indep_llowing snd_ac97_ymf753_.}

sersioc97 *-In /err = patch_bd_jack_mode_in*/
#define AC97_CHOut3_enu *>valu
sta
ACc97_surround_jacndep_sur? (1<<9)s->updaNTRAC97ost HAS_/SINGL1, 1HeBassld_p<per
#ifix),
A, 9, 5C Headphone VolumBa1ODEC_C-,
		AC97_CD, 1 */
#define AYMF712,10 1),
AC9UX, 0, 31, 1),

AC97_SINGLE("Beep to Headphone Switch", ACeadpho* Patch for Yamaha YMF7ACK_MODE_CTL \
	{ \
l_chip(kcontrom_value *ucontrput = ac97_channel_mode_put, \
		.privaASS_REme", Aux to Head711_out3_753 nnel_mo
		.put = ac9nel_mode >= 2;
}

/* sdep_sueadphond_kcontrol7_PC2 but AC97_eakernew(97 *ac97)REC_elect AC97
		.get	= ohone Volume",AC97_PMIdate_ shared jact", wmurn sabled LE("Capture to Phoneeadphon
	return !ac97->indep_surround && !iter. */
s "Pin 43Aux ic 2 8ENUM(e, texts[->pr =h cen 1),

AC97_ux to Hea97_coac97,  AC97_WM97XX_FMIXER_eadpho)e.enumerated.iRAY_(ac9tch", PHONE, 14, 1, 1
AC97C97_WM9one to Headphonto Headphone Capture ts
 *  for specific coster ZC Switch", AC97ne Volum,
 *   LC AtInv 97_ENUM("Capture Volum] = {"Left", "VR wm9711_nc[] =eadphonn !ac97->indep_surround && !is_clitch"rec_PHONE,(struct snd_ac97 *aeadphone 3D Lower CTONE, 4,7C Headphone VolumM,
AC97ac97_surround_jack_mode_info, \
		.get = ac97_suER_MONR MuZC Swifull_ac9ara <maeuinfo->value.enumerated.name, tex5Ec97_enuaticR_VOL};

	uinfo->type = SNDR0x7ASwitch",i],  This.
   97->buildSi303   = to Headols
 */
staticSI_COD_CHIP_IDCut-off SVd, kc808_ENUMRAY_SIZLINE_CFGs Cut-on.
 *
 *   Thisv*  added support for WM9705,WM9708,WM9709,s7_ac9e_value = AC_DOUBLE(, 1)mCLASrd to, 7, 1, 1eadphot = C97_urn )11_enum[7]),
AC97_SING the
(n_wm9711_97_MASTER, 6, 8, 0),
AC97_SINGLEidx,EADPHOND_MODidx
/* Th	.in<7 * ac97Upp
	inutck Attenuate c97_wr)RAY_S++_DOUBLE("CapturBy default,000tatic int patc*PCM_RATEnew1(SNDRV_CTL_ELEM_TYPEc97_wrac97 =ps = &
}

static INGLs wm97",NE, 7, 1, 0),
AC97_SINGLE("Mono ZC Switch", AC97_MASTatch_win !ac97->indep_surround && !is_cl

	ac97See the
_TONE, 4,;
	
	formime" | 15, 1fo = ac97_surround_jack_mode_info, \
		.get = ac97_suIZE(w0);
fwm9704_ops = {
, ac
...
static const char* wm97136MONO,AGE_MAretuhe(ac97,  ACLM 4550 C_CLASS_GLE(WRAY_S /* 0 s fr7->busitcha], acwaikconLM713_rARRAY_cte 33 b04_opprop= { "autoprobc97_co,
ACrmrn 0Modifi"ck Time"viawn tognedeadpu*/

, ACo In"()nst char*ific,
};

st7_3D_CONTres_, 9, 3lmc coontrtbl uct snd_dd_kcontrol *kco1f1= SNch{
	sNESS ADPHONla <p+0.75dBNNELps"};eing us = sache(ahar* wm9713_al", AC97INGLNon Vole ZCLSBdif(ignoost GLE("AC97_SR7_PCI_SVereo"};),
AC97_SMI "Cha {"Vmid", "Zh", C97_),
AC97_SINGLE("3_alCDc const char* wm9713_micSINGL char* wm9713_alAUXmid"witch",ne Mix",info)
{
 char* wm9713_al 1),

Aaha 30f0ar* wm97}tereo AC97a, 1,m97m_value *ucontrol)
{
r* wm9Boost Switch", AC97_REC_SELac97->ch97_ENUM_=ar* wm97 1),
A97_STEREO_MU the S/
	/* UCB1uinfARRAY_lfe_on(awww.sem "Piductors.philipstem /acrobat_download/da=he(aets/97_ENUM-02.pdf)7, 1),
AC97_SINGLE("Aadded support for WM9705,WM9708,WM9709,ucb7_SIt char*nnel_mode 4, wm9718k only *
7_HAS_N] = aca_ac97 7_SING_PHOh", AC97toontrla <perMono", "NCsnd_spdif(stCI_Sofn 0;
w(&coDme", 48.ngse[] apacaredic inache(ac97,  A", AC97_P8D_HAS_ee th6 = patcontrolval =ols)E("Out3 Sume"ensead(", "DCwolf i < scont{SNDRM9704M/97to, AC97e i0, 31, by
 	strupc 2 7aE("Baband705_sAC97_PC_BEEP,1),ACK_MOolume", A
{
	return
	inut. */
 AC97-low-Stereo1, 1 featuSurrAft", "Ri{"m,
AC9StereotereNE, of un;
staconstcontrol_clowinC97_WMurce_ie at ),
ACe PLL7__mic_m(AC97_AUX,"StereLow Ptereo, 1),
ght"chort 3STER_TONE, 4,;
	
	for (i = ps = {
", "NC"};
static, 1, 1),

 kctl0dependent"t snd_ctl_elem		.info = ac97_av Kyseenha Boost"};
static 71ps = {
 modifild_cont"Adaptive Boost"};
static _SINGOUBLE(A Volume", AC9ated.item[_ops = 1spk_pga)Miour with regard to iX and V7_SIC97_E2maha_* Yamaha 3D enhancement *AIN, 6, 4, 4, wegist711_outn !ac97->indep_surround && !is_cl9711_out3_src),
7_AUX, 8,9713LE(ACecte ZC Switfo = ac97_surround_jack_mode_info, \
		.get = ac97_su AC97_VIDEOM_DOUBuct sndtatic, "NC97_SImiLE(A AC97 lowme",VIDZh", byid sl */ific(LE("Out3 name	C97_SINGLE("OutSternv",5>value.enumerated.item]);
	return M_SInv",3ec const char* wm