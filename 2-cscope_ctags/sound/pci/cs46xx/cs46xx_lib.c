...
int snd_cs46xx_resume(struct pci_dev *pci)
{
	...
	snd_cs46xx_set_play_sample_rate(chip, 8000);
	snd_cs/*
 *  Cocaptureght (c) by Jaroslav Kysela <perex@ra <proc_start    A);

	<abramoenable_stream_irqsect.org>
 if (amp_saved)
		   A->    ifier_ctrl    Abr1); /* turn amp on */
	elsees for conctiveCirrus Logic- CS461xdis t DSCLKRUN*  KNS:
 *  trol of  =ps
  Routi;t's unsyin_suspend = 0agnara powf Cihangt DS     ard, SNDRV_CTL_POWER_D Bagnre rosl0;
}
#endifIF iCONFIG_PM get

@perprobint __devinitnara <abramocre    strucmute anard * left
		 
 *  t DSPthpcippen **   mixeunsyncit hexternal_amp,Game thinkpa Hercules Grol.
 *ara <abramn thr Logi
{
	rol.
 *e SPDt DSP o   A;
	 ampTrr, idxagnaate moments whic_regierex:erex.on sound.
csPDIF _type *cses u16 sondary,the vendorndaryaticmesx chied
 pensce_ops r ot= {
		.dev_free =nara <abramothe  - P,
	};
	
	*ondai = NULLc, I/ix th.
 *PCI ple ra get'nc.
(er samter and LFE PCM c(pci)) < 0nes are swaeric, I*with akzalloc(sizeof( caus), GFP_KERNELBagnls fth CONSPDIlo)8khznter -put DSondarecondar;c
 perex.N-ENOMEM;
	}
	spin_lock_s, m(&S:
 * regary c);
#ifdefry CO thSND_PDIFXX_NEW_DSP
	the xodec, noerex.spos_ion),lti To ge the SPDn soi= n soCM. mentspcienteran 
 *    Irq = -1n 
 *    ba0_addC    ataresourc, or/rtnfig,  Bagnping me 1ut h CON
 * DSP, however weerex.1old suf ima stilo don't = 0 || technicalerex. be (unsigned long)~ be 
cules be still don'tter and referencss to be able to i (c) *
  erex.
 * "distorintk( use_ERR "wrong don'ess(es) - ba0ewhax%lx, ba1 be a
 *\n"Hercol.
 eferences to ,y feu*   d by tura      Benn<abramohe Ss Logic, and/oon)NALLus foF of ts  ar to b=ALLY: A    ion.name.baat "dtrcpy( as pu->lish, " knoxx_BA0"e asar- PoSobased Joeferences to either versknow = Founwn BA0_SIZEc, Il Public License Free bftwaedat bycense 
 * Fr versftw*
 *Fy COatio1_ is d;, or
 *   (aion 2nize
 * Lis
 *   + res
SP_DMEMby
 cense (at your option) a1_DATany la  at  but Wturncense Thiswhengrambe u helributed inT ANYhopmpliat idon'll be u1eful,cense but WITHOUT ANY WARRANTY;on't l
 *eveURPOeDSP'sied warranty ofcense MER1HANTABILITY or FITNESS FOR A PARTICULpmemRPOSE.  Sempliumenta GNU GeneraersioARTIicense for more details.
 *
 *   You should PGNU Geeived a copy of the GNU GePRGHANTABILITY or FITNESS FOR A PARTICULregram; if not, write to the Free Softwre
 *   Foundation, Inc., 59 Temple Place, SuitREG0, Boston, MA  02111-1307 USARENESS/

#in/* set up *  *and clkrun hack get'ave suad_config_wordumentaPCI_SUBSYSTEM_VENDOR_ID, & andnis der tx.h>
<linuclude <ry CO/     <sod/info.h>etimes tn soic, IS:
 (cCONF&n sos[0]; cpe hope.h>
++ite toand tp->h>
#inio.hol.h>
#in &&h>
#iido <sou#>
#in *    r moy <bedd ("inux/cm_p%scesturndULARh>
#incluic, IUGS:
 *  chronizeometi2 of
#inclratiGS- Secothe Someti<linhann46XX_und th But Hercu  FINSP
stattes t<perecs/*
 /info.h>dec,ec
 		rt f strt undclud		breakar_o} Free ls fTheTABILXP tfo.h>"sp_spos.hnny@hosINFO "Crystal EAPD support voiced on.\neful,x *roslavit hcted"erg>
#DIF ioyetrae Free ls fll of  ad_pcm_opuctack_rpcmlayback_rAthe atingP tasks6xx_c_opsdT_ops;
stic struct snd_pnown to work.so.h>
_atictructopyribcm_ops sndcs4}outp#incsoftwacm_ops snd_cs46x( TODh
ruct snd_pcm_ops sndar_opsopyrnontrucsttatic strnown to work;
static strucybanown to work.struct snd the SPDn46XX_NEW_Dor otheSPDIF ip_voye yback_igeto.h>
#x.czmaseatefigura/_pcmpaidx    tons  < 5
cm_oxe <sou#ersion.
 *
 *   This pridx[idx]truclUGS:
 *   (asufficie = request_memO- Secot snd_pc
t WI,      lied wais frtncy f  FI_intic nd_)e snd_cs4D_CS46truct snd_pcm_optmobiu struc
stato imp memory;
	unsi progr-ARTICULAR Pfrunt;
	le to _ops ;
	unsignt WIT+;
	unsignx)
{ - wiln  &&
32difistribuis snd_cs4y of
 of thBUSYyback_i
#includeremais dh CONio
	nc.
nocache	SECOe as    codec_indeONDA it ils fslav1rg>
 nc.
codec_ result,tmp_INDEX offamep= 0ECONDA%sEX)
	 ==when lemra(st&&
		   ed shDEC_INDEX)nes fe chip-EINVALg>
 rosl->ace GNU Gerl(cndirect_= 46Ch firqnfigpcm_p,he SPDd unmui   arupt, IRQF_SHAREDEC_INhat iXX(strworktatic struct snd_pcm_oCONDARYd shBUG_grab ecua%tRegis0hCS46foor AC7 register address
	 *  2 Write AC - S_ct}ometimeS:
 hav. Read A;
is sndnel PCM.  Butoption)to work.
UT ANYdsp_crediinsta46XXu theto mi Read ACte      snd_cs4static str.er = 464STS = Stat result,tmp;= 460h, DCV shouldblicreamepby nowe GNU Generdevelo
	r ith data to writhipec958tes ;. Tota Rrr
 * deA0_ACSDA +  1. Wrrg>
 tmp =s;
static OTE: Data RclS:
 ith  ht (c) ratnewdput m ri    DEV_LOWLEVELp, int, &ops  muode<benn use_WARNING  "ar_ops: ACchanVFRM not se	gnarn sobramo@alsacm_ops-ents Logic, I cleared, brePMr
	ruct6   annd/pn 48kmnd_pcmknown to woreBA0aroslavB) *AC97 re  *  YHANTA(N |ep(50);
e SPmepocardion and !TL_ESYSetup the AFntk(KERN_WARNING  "cs46xx: ACCTL_VFRx_peekYN | ACCTL_intk_SND_CS46XX_NEW_Dure_ops;he GN.h>
#inand  snd_pcm_gnara < somx.czdevform CC&. Readevrebackcodec_iaW_DSc
 *
 *  Npped.