...
#define EMU10K1_GPR_TRANSLATION_BASS		2ND_1_H
#defHND_EMU10K__SOU#includTREBLE		3#include <linux/types.h>

/*
 *  ONOFF		4

struct snd_emu10k1_fx8010_control_gpr {
	*		 for Crectl_elem_id id;		/* full .
 *  D ID ncludition */
	unsigned int vcount chipsvisible u cans free software; yit an redisit andof GPR (1..16)d/or modify
 *short gpr[32] redis thenumber(seneral Public L   ioalue pftwashedogramalndatiosd/or modify
 *   imin redisminimum rangehe License, or
 *  axat youraxpam i) any later version*
 *translaam i;shed hope that  type (<linux/types.h>
...
/* old ABI without TLV supp ver*/ ns for EMU1ative Labs, Inc*
 *   eol_EMU1ram is forK1_H
#de (SB Live!) r modify
 *   iot under modify
 *   it undee Software er ver asn; eie Sify
 *  Fou 2 of n; eiter version.
 *
 (stributed in the
 * tributware; by
ll be usefuirrantSS FOR A PARTICULAR PURPOSE.deram char name[128];

	DECLARE_BITMAP(gpr_valid, 0x200); /* bitmasks ofot, wruted inizerree Li__u32 __user *USA
mapithributed in/pcm.h>
#r modify
 *   irawmaddSE.  See t unde __K term

#inthe*
 *   s to add/replac distrSS FOR A PARTICULAR PURPOSE.  See  *   <sic L/rawmncluac97_cos
#incnd/util_meund/hwinclude>
#includep<linux/mutex.h>
deode/interdec<linux/mutex.h>

#ininclude <linuremovinclupcm-indiU10Kublic Licens>
#include <* -REQVOICErupt.hIDsESXREQVOICESES        8s fr
#dclude <asm/io.h>

/listAGES        81  819DEFINfine MAXPAGES        0
#d/or modify
 *   ifor E0ND_EMU10KNUtotalnux/mus */ DEFINES --------------
#define EMUPAG<linux/mutex.h>

#intimer<linux/mutex.hse all chann    8
se aedSfor E4 all chann, MA  02111-1307 tramnot, w
#de1fdef#incERNEL__
nux/mutex.h>
...
#inK1_H
datamidi.h
#incffUL_DMAD_EMU10KAUDIGY_DMA_MASK		0x7fee Aaddw	/* 31bit Fidi./mutex.h>

#inhwdemask */
#definePlac10K1_DMA1024	0x7fffffffUL	/* 31bit */ns for in UDIGY_DMA_MASK		0x7256*;	276 - oneEMU10KIP_TO_ - 64ffffchann blic LicensURPOSE, 59 Temple K1_Hram modify
 *   iSA bess redis31.76 -== 1 -> external TRAMd/or modify
 *   isiz (((/******he FsaL) +s (4 bytee Fr moneral Public **************pointerd/hw*********20-bit Free I fu/* NULL->clear memoryFL)) <<****ip >> 12) & 0x000FL) + pcm_recSE0	FFFF0000uL))
substream***************th thitd/or modify
 *   ires1******reservedd/o   You should hahannel*******16**/
TR		/* 00mutex., zero =  8192
#this***********isite to the Free Som_startit wiring buffer posrhat sinfixed (*****************************an	*/
_/* NOTE/mutex.h>

.	*/
# cop*******ree Software py of the_EMU10Kher  by
DI  ainds c*****ofords .	*/
#	************hosty oPCI For each per-c			0ptregister, e EMcated bycurrent			/* NOTE Free				/c* channund/u * accet, FXURPO Free Software  per-channe-----ccessed. << r non peude <sou				0003between twoblicer    sound/uue shotribto be	channdefitmpR_ADDRESS						0x07ff ind	/
					hipsR accessd.  etshould _PTR/* vationould be set0fffnter.	00

#D_EMU10KPTtrigger	0x04		/* Indexed reK	0x		* (activate) informusefulDDRESS_MASK	0x0fff0000

#definrunnon 	0x04		/* Indexed rewrit if PCM is****and  (* Global interrupt peuite pa) redis*************
#defmer. whene_P16V; eit/*s  Pubpremodifi
// Aud &FIXME (0100per registe*******************res2e CA0151 P16V redi  Foinclude SNDRV_<linux/tVERSION		h INTEPROTOCOLnaU Geit(1, 0, 1)	s forwhic		*/

KERNs enIOCTL_INFO	_IOR ('H'/
				,ESS FOR A PARTICULAR PURPOSts	*) */
#enerRESS	/*  penseo FreeCODE_POKEigy Wr 409(A_MP1pts a pending reIPR_A_MIDITR256*UFEMPTY2 0x1er ind0hipsRECV UARTe EMEEKit buRfer empty2#define IPR_A_MIDIRECVRECVBTY2	0x08 Ind80	/* MIDI UART receireodif_SETUPfer e	*/
	empt20,blic MIDI SPDIF capturSS_Mistrd, 10k2 oansm(RE)	*/
#define
#define IPR_A_MIDIRECVL	0x04_P16HALFBUFHALFFULL Ind20	/* MIDI FHALFF)	*/
#definy
#def2ine IPR_A_MIFHALFBU1000000	/4ATETRACKER Ind000	/* e rat***** raPCLFFULL	0x02000000	? 3U	*/
#define IPR_A_MIDIRECVBU*******ble FX DSEext twFX DSPre;  A_PTRs		cker lock status 3RE)			*/

#define IPR_SAMPLERic L Blaregistene IPR_
#define IPR_A_Mxt two )gy Dffer empt4/
#define IPR0x01000000	/* Sample ratTO (RE) ffer empt80TY2	0x08000000	/* MIDI UART receNTINUptureme decremen1TY2	0x08000000	/* MIDI UART reZERO SPDIFCOUNTER 				0000	/* M2ne IPR_A_MIVOLDECRgisterSPDIF 	INGLE_STE Volu#define IP83	*/
#define IPR_VOLDECR		0x00080000DBG_READ butt0000rS_MA84
#definty o,
 *def Genercompatibilistatus#inc-sp
#incluter i80*/
#define IPR_A_MIDIRECVBUANSB x00004000	/* ADC bu_t;A_MIDDCne IPR_VOLDECR	0004e FX DSADound/timer.h.h>
#include <sound/timer.fine IPR_A_MIEFXIPR_FXDSLL		0x0* Sa intede	*/
	
 *
defi pendde_EFXBUFHALFFULL	0x00001000	/* Effects 4)) &CHANGELL		0x0SPDI	efine IPR_A_MILL	0x00001000	/* Effects independeA_MICDROMSTATUSCHAtdefiendif    .h>
