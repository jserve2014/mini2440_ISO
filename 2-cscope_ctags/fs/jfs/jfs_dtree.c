	 ...
	 struct component_name * key, ino_t * orig_ino, ino_t new_ino, int flag)
{
	...
	/* get the head/only segment */
	entry = (struct ldtight (*) & p->slot[nal B_si];

	/* substitute the inode number of  Thinal Buspyrnal B->iram is = cpu_to_le32(new_ino) 2000-2unpinee soleaf pagee;  yDT_PUTPAGE(mp
 *  return 0;
}
