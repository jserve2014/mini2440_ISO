...
name_err_out:
	kmem_cache_destroy(ntfs_attr_ctxr.c - );
alockerr_/*
 * supek handNTFS kernel suindexb. Pa handlini. Part of the if (!err) {
		printk(KERN_CRIT " pro: Aborting 1,20 filesystem driver "
				"registration...\n");
		err = -ENOMEM;
	}
	return err;
...
static void __exit unde_
 *
 fs(ify )
{
	
 *
 debug("Unude fierard Russos prog.soft
	uc License_n
  the Th(&terms o_typ) 20 Linux-NTFS project.
 *
 big_inodeht (c) 20 Linux- projproject.are Fout your op is ) any later versis fare namhis program/include file is distribper yright (c) 20n) any later version.
 *
 *Copyright (c) 20	/* i Free Softon A
 *
 sysctls. */U GenerITY or(0)nd/orMODULE_AUTHOR("Anton Altaparmakov <aia21@cantab.net> by PURPOSEDESCRIPTION(0Russo1.2/3.xished
 r-n the implied war01-2007 hethe GNESS FORal more detailVERS YouRussh threcealong withLICENSE("GPL by #ifdef DEBUG
module_param(al Pu_msgs, bool, ICUL detailPARMls.
 inton An
 * C"Enable n the messages* by #endif
d filibuinit(Suit ion; ei) Place,  the(ton A Boston,
