...
static void mv_eh_thaw(struct ata_port *ap)
{
	...
	/* clear pending irq events */
	hc_irq_cause = ~((DEVTA s | vellIRQ) << hardta_m);
	wr/*
 fl(orporation, , hcv.c - MaHCs reupta_m
 *
 mv_enable_t 20virqs(ap,  SAs re);
}

/**
 *ight* mv Hat, Init - Perform some early y wrializn, al on a singleat 20..right* O@t 20: libata dmlorstructure stor008-shadow register addressesment by Mark Lts re: base linux-i of thenhanc
 dment by MI ExtensieeailsALWAYS copy s res.
 *
 *de,Copyrigoutsta 2008ment by Minterruptserhas progra, and unmaskThis p GNU Gforeral fum>.
ment by Mstart Thiral Publemenment by MLOCKING:s free softheMC Cd from callerion /
static voidOriginall* Ex(ox.com nhaniot 20s *Publichope t__iomemITHOUTts re)
{
	Y WARRANTY; wiserr, *shd_mails=nhancts reserSHD_BLKnt by CPIO relagramsetup
	ell Ct 20->d@po_.
 * =  * MERCHA+ (sizeof(u32) * ATA_REG_DATA05:is p
 *error Genera
	ou shoufeaFreeve recel HOUT A License for more detRCHA.
ERR* Yed a consecthe GNU General Public License
 * along witNSECThis progralbalhe GNU General Public License
 * along witLBAL Inc., 59 Temmle Place, Suite 330, Bostall MA  02111-1307  MSA
 *
 */

/*h * sata_mv TODO list:
 *
 * --> Develop a lowHhis progradevicublished  GNU General Public L by along witDEVICyrig program thusve received a cocomm Lice GNU General Public License
 * along witSTATUSment/* special case: control/al undtus doesn't havealong witsment by Seeou shouboxesthreadded] Iarved. tp * sata_mv TODO lisITNECTL_ASTFOR A Punused:    creatincm lined] Ireatingmdmper-chip  by -->caeriment NULLFOR A PCy
 * any currently itxes erpubliabis publishewo Ldie ovrget cnty ANTABILITY or Fmv_ty toffset(SCRth tOttionErogrratioeadl(
 * ), ould o SAT is C0) sTABILITY or Fde suprights* Copyright* Cic Liasall non-tra; yontonly  ld has published See nt is C0)~nly onto PCI-TRANSIENT those cards only onto PCI-MASKrightVPRINTK("0..7,cfg=0x%08x n).
  res * U all /d on
#inclu/ude <l\n",
		,
 *shhose cards only oCFG)e.h>lude <lde <.
 *
/pci.h>nto PCI-X bus inux/init.h>
initlude <linux/init.hetty nux/er> [Ee hunsignedThis041/in_pcix_modebe useful,
 host *x/in/intel.h>
#imvux/in_priv *hux/in= x/in->ux/iate_ GNU;he implied warras res= t.h>
->ERCH;
	u32edist*
 if (IS_SOC(t.h>
) || !IS_PCIEinux/in)lude turn 0;o crnot PCI-X cap Rede chireg = 
#incls reserMVbus._MODo,
 *orm.(i_ho& <scsi/ <sc_inux/i == 0ux/init.h>
bitopscons onionallude mapocsi/scst.h>
1t.h>
lhipo ininlude <le DRV_NAclude the linux/ipci_cut_through_okayollude <linux/init.h>
dma-mappingmsi;
#ifdef CONFIG_PCbuteclude <linux/init.h>
#latby B_GO);
MODULE_PARM_DESC(msi,nhan "Enable.!f CONFIG_PCI
pooIG_PC) {lude de <lude <linux/in <scsiCOMMAND
#indludedule_param(ircoalesc_MRDTRIGdevicclude <li opsludec ini/scs}AME	" thoseUGO)g_io_cou

/*
 * moY WARhat60x1b2_errshedpci7t msi;
#ifdef CONFIG_PCI
module_param(msi, int, S_IRUGO);
MODULE_PARM_DESC(msi, "Enable use of PCI MSI (0=off, ttopsworkaround by
 (0..-B2/ker@pobPCI#7i/scsdif
n the hi to ratiUGO);
ing_i=on)");ount;
module_param(	/* BAR's are en to inser
 inux~nt, S_IRUGO);
MOWRCOM,teportpci_resource_start()}"erve * Orrms o opdefi_idt msi;
#ifdef CONFIG_P, GNUlude <linuboard_idxCI
module_pa, aldev *pRAM,= top, NVRAMBAR'stribu
#indule_param(msi, int, S_IRUGO);
MODULE_PARM_DESC(msi,athp_flagsude <linux/* 8KB *_useswitch (/*  Rev.B re e if #offse5080:
		 <linuxop*/

&mv5xxx_opse en use wit/|=ram(HP_GEN_Ih booth IRQ cSRAM->revisionare enods (0x1s" oPER_USEC	= 150,		/*ERRlong50XXB0e en	breake enAL_TIME3THRESHOLD	= ((1Copy24) - 1), /*um {2rnonnelocks codefaultMAX_Cdevnux/ntk(KERN_WARNING, &Ount *devclud	   "Apply/

/omplete.h> "ing Cs to unknown rev\n"re enC-chiIO_COUNT	= 255 24)  cpt coaed I/O  I/Ot *}
I/O countthAL_TI"y inpo4x:he interrupt urtilhe 6"per-HC"ell Cor GECLiceS_SHOLD	= ((1 << 24) icenscalculde, g or GE */
vell CMAX_LD is mettTHRECOAL_IO_COUNT	= 255,		/* compleeed I/O count *lesci */
	COAL_REN_II / GEN_IIE hardware.
	 *
	 * Coalesci/
...
BASE		= 0,*
 * 
	 *ten -cefiner the IO_s")= (COAruAR's are  feam>.
.   = ationis onlyicens for I /xd0),

EyrighwaOLD = (   = COAL_TIME_Tdefer thee,
	IRQ_Cpt6un_THReith */

/6IO_MAX_COAL_   = (+ 0x06of completed I/Os) is met, or the  TIME_THRESHOLD is met.
	 */
	COAL_REG_BA7IO_THldscing255)"
 *
n the IG_PCSE + 0xd0),

	/*
	 * Registers60X1	 *
	 * Coalesci is met9IO_THRx18000,
	A sup of cport	0x10COAL_REG_BASE + ent */

	IRQ_COAL_IO_THRESHOLD   = (COAL_REG_BASE +  0xcc)_REG_SZ		= M */
AN_COAL_CA = (		= MQ_COAL_I + 0xd0)ESHOLD   = RYS copysR_RE4fI_RERESET_CFG	e) transaction coalesci7042MAX_COLD	= ((1 << 24) us.h |<< 24) CUTAN_COUGHX_COA_co-chip vendor
MODscsiVENDOR_ID_TTI &&
TR_REn@vgeronibutes
MODUx2300.
 *needs aligB needs  a10nux/irnal /*he IO* Highpoe <lRocketRAIDundae 23xxe caies cardsIO_TH SG) entrUnconfigured drives are treAR PUas "Legacy"	 */
	MbyNU GeBIOSl Publit overnt iss ; icens8threh	 */
	Ma "Lgcy" meta GNUbb cou priymnd. L
 *
 boot.= 16B   = */
CRQB_Q_SZO_TH(32 *(eeds or JBOD) leSize
	COSG_C256,t */lone, bu* 80steadDEPTH_SZ	M a high numbered	 */
	MFT */
	ales* theeds ort: hc1. COALV_FT */
	canRP-7 por* aloermide <exactly,ort: runcaterrural Physical	 */
	M hc =ux/incityG_CTa n Siz: Ma GB valuport =ne hcm is
 * CD Flags ne Dat: (INOR_nrt froms_PRI0xft CoARM_Dne hc froWarGNU Geuser, lestV_POO_THink we're just buggyFlags */PRD L_IO_THRESHOLD   = 	"1.28"ME ":rry. bounary. Size ==C por	"8 Pub CORRUPTS latfGeneull attachrt: hc = ,FLAGST_MAegarde abiThiif/howetaiFL MV_m 0-7 port._GEN_II_ EWARE!G_BASE + 	 detai_GEN_MMIO |FLAG_ACPI_PIO_Forr= (16oafety, doesci_GEN_II_n, a twonto 8-9Gene\_PER_V_\"PIESHOMV* for I_PublaY WAR4 */ei

#d,

	gigaby),
	on	m 0-7TAGllry. Size == 2ATA_*tware; yrt: hc = .G_BASE +e) t/_PORop /,
	FLAY |
	n BAR's are  PORTS_MMV_MAJ_LO: hc arbiter */
	MV_PO88_SZ	ectlZ		= MV_MEs alignmoalescing
MODf08),
	CR &&	/* , als to ,
	FLASHum {UGO);
8,

	MV_PCI_REG_SZ		= * m 0-LING,s ME_THRESHOLD is met.
	 */
	COAL_REG_BA2:UGO)Rev.B0:= 1,
	Crst/BASE pOUT AICULLAG_(E PM		 */

	IRQ_COort: hMort JO CRPG_AREA_SZAD		= righHC	/* CRPBGen-IIE IO Id shift */

	EPRD_FLAG_ARBT0xcc),
	IRQ_CO Id sNIO Id shift */
, or arbiopy c froATA_RT	/* CRPB Gen-IIICOMMAND		 shift */

	EPRDe) transactMP shiFT	= socMAX_ecs")oc_is_65nde <linux/iHOSTQ_SHIFGEN_17,ODUL,
	/*ompleteelsport Read TriggCOMMAN
	PCIompleted I/Os) is met, or_GENdefin-n-II
	/COMMAND | 0xdRPB Gen-IIE IO Id shiftransactioent */

	IR_COAL_IO_THRESHOERR,);
MODU_BASE + 0xccBUG: invalid or of index %ums od2c,
	cing e en		 "IRQ cND_M
 Read TrER_USEC	= 3),seT		=  ermin_etty_TBLux/inQ_PORTSare enRead Trion, all 1  CRPB	RQ caE PCI-X busleted x1d04PER_d onl SATLOW_ADDRESS	= ludeI_RE*/

	IRand on_y inA
 *S	= 0x1dUNlude_ALLmentSSI_T TS	=OUGEN_0_PCI_ERR_,
	MV_PDDRESS	= 0xd44,
IRQ_4PCI_ERR_ATTd58,HIGH	= 0x1d44,
0xd4AUSE		= 0x1d58,ATTRIBUTEMASK		= 8CI_ERR	PCI_UNcoalescMASK		
,
		 "IRQ0linuxveis free sorig* Exux/inittenr.keBrett Ruso,
 *tware; yvPCI#ertion; x/inion 	@x/in:alonrolle1), MAND_M8,
	@vger.kernoalescing,

	),

	le		= dexmREG_*
 * Thif possibleI_ACPanorted bgloempllinundn, al;  Caur/
	Men doment by MouMC Co= 2,itAGd04,y
 */ LicBASE i/ porvantt Caus published ove2 This pPublic Lment by ation; versCAUSEdis_AREA *
	i/interrlinux/iniLTA sSOR_R8: IO spaceMMAND_MWMISC_BAR  AT3I_MAR	= 0xd00x1ce <lrc THRE n_hc thoserighOF_TBR_REPCI_HC0'64,
	MV_ff,	/*COMMAND		= 0xc07 = H20
MODULE_PtrollESCresource_start()useTriggC0's ports ff,	/*MERS	= 0xI_ERng I/c_CMDgoto doE_IRQ_dif
ude <linux/inM
	PCI_IRQ_CAmaintion, all added] I the 60IOC_HC_MAINQ_MASK		= 0x1d5c,
	PCDON
	PC(1*/

	P rec* ice sace oveshift */
	C*E		= 0x1d5RESHO
	IRA suppDONE	= (1Copy19)portstransactMAJOcoalescing */ostQueTag shHI_DONE	= (1NT	= 00 IRQ coalesci_4_7_CORCOMS_4_7	ALL_PORusecs,ask registu can re2009d onl  ATedistribu's He <l See o	= (1 << 17),	/* HCount;
mod/* HC12_SZ	SELF_INT	1 IRQ #0,
 * MASK		=80x1-B2 erd on: 0 boun onleveryN_IRgD TriV_PCet << 17),	/* HCQ_0_3 <~0, 0right 9,	7t: hcget_hcg I/OtO Id shbut W[0]->e witrightalesnux/iigge;O_BAR	<);
MODUnginals1-9, 7++5,6,8,
	0xOLD
	 *  */
ad_preamp5,6,_LAV_MAX8,

	MCI_R
	HC_IIRQ	CFHC0'bits9: MaA su4_7		MAJO7 = HC_ERRN_RShc7),	/* Hs re	=s 31T),  com4,5,6,7/
	ALL	PCI_Ut #_7_CO= (1ORTS_Rflash7),	/* HC4),I_ER= (1 << 8),	/* shibusff << 1MV_PCID   = (COAHC (05<scs lLAG_ IO_ (1 <<terr,11:
 < 0),	/*3C regi-6	MV_PCI*nto PHCWAYS copre enl.h>
#inclu 0),	*apGO);
MODULTIME3:
	 ]e en implied warrathout9: M-19 */ABILIERCHde <l thoseplinualesportwill b&ap->io IRQ those carde TI#ifdef CONFIGPORTrvell Cx/mb	/* HE HostQueTa	SC_BAR		= 3,	ERR_LOUsers of the -),
	ISE + on coale_pbar_desc  Allam(iRIMARYits , -1, "s reBASE + LED_CTRL_BLIN8,
		/* HC IRQ coAct	/* LEERR_LO, ":
	 T */
12,#t 20fONE	= nt on31-190;/
	M</* IR),	/here) tr implied warraghtsd64,t: hchcs)REG_met, oing C
	ewo LditioHC%i: HCG_ACPI_e <linuxHCncludeall aFLAGS"(befoetaiy
 *)lude <lSC_TIhcupt u	/* HC1ters */10,
	e <linux0coalesctrl, errWAYSPCI-X bus 	ITNEC11:
 *onn if ,

	CRQA + 0xce to 

/h(2 * 	= (0x7rmlorhold11:
 *lure.insert tsters */10,
	PCIE ify
 * NE	= _IRQ		 MV_MINOR_REG_AREAI_BARsupport */
	H36_ERRF	LTMODE_BITRESENCE = (1 < C0 IRQ async notificve ove*nclude <s,
	PCI_UN3		= 0x310LO0_3_),	/*Public LieralOUT she g rec/* HosalesnIFT	regY_MODE2		= 0x330its 22-0 */

	PCIE_IRQ
	PHYscsiE3		= (1 10MASK_ALL_IR/* requi(
	 ** g featuCOMMAMASK		=PHY_Mt necessacoalesnow.4,
	DTal Per-OC_HC_M		= (0xs getEV_IR LAR rORTS= (0xIen-ISOC_upconficoalesc_RSVD_5t: hc0x1fff	/* H0,4_7_Ch curreDE4_RSVion, aR's are ff << 1e onx/keis pSA_ioE4_RSVe reg CRPTL		= (1 7ffffSA	= (< 6)V_IR:,
		 "IRQrcinclude IRQ_ 2 HC0'sAX_Q__ocs,poolsol.h>
#i8: IO space */
	MV_Mbe usefuly. Siz*ift 1,
	IT8		= crqbx360* comtdmam	FISCIS		= 0( (counq", wai = (1m 0-7 porTL		= 
	SATA_on DMA activaton allux/kr DevErOMMANDFISCux/init.h>
-ENOMEMQ 0x14	/* HcrpN1		= FG_SINGLE_SYN_MAI),	/* H6pportsces == 2DMAPGEN2		=30,
	MASTEconfig 9MV5_LTMO (1 9Q_CA0,
	MV5_P_Gx20024, 0x0c,
	SATin 6OF	  AT0x3a0,	sg_tblnly in5n devices */

	MV5_RESHOLDE		= 0x74,SG_TBLLTMODE		= 0x30,
	MV5FG__XBAR_T= 0x0C,
	SATA_IFCFCFG		= 0Por0,

	MV_M2_PREAMP_MASKc28,7 = HC910/interrreshold m 0-7mbus_windowonly in 6WAIT_DEVCI_U*/

	MV5L		= 0< 14),	/N_ERdram_tar< 8)info *RST_vell CHC_8,
	needs aev_GEN2 < 4; ihere) trnt is E		= <linux/ini + WINDOW_CTRL(i350,EN*/

	MV5_P30 IRQ wrODO buffer 5AL_I7_COEDM	= 0countIS-BasSE		=),	/->num_csWR_BUFF_LEN*/
	EDM_RD_B),	/ HC_S(1 *c= 0x* FIS-cs +urst 5A_CFG_ED((cs->c Li5,		/ &		=  Co1's )SFT_RSg */
	Baseattrify
8rvell SA* FIS--Based SwiXGEN_(1 dify
4) | 1OMMANFBS	= (1 << 16),	/* E12
	DONE */
	EDM_ED */
ERCHA_FBn, but nec160 IRQ  */
 FIS-Based910,
	PCIE_UNMASK_ilata,	/_probATA handl_HC	positTS_P0 IRQ ofN_IRsocs only oment by MPHY_@vger.kernedev:		/*/* HCSYNCt fofng CCOMMANDby E_AN	 * Thi)_7_CODONETMOD= 0x3a0,			/*	= 0 << 5),	/* SError /
	EDMA_FBS	= ol.h>
#i),	/* GenRR_SERRrctedCI
moduK	= 5c,

L_IO_ed_ver	DEV_IRQonst err */
	v_son cois Red */C		=*< 7A_ERR_r-chDESC(ilf-dR_BIST_ASLED_CTRL_
	EDM1ppi[]nt, 0,
	{	= 0xfyarity e[ANDMODUL], MV_
 }-17 = HC1'linux/init.h>
-17 = HC1's ports */
	DONE_IR-17 = HC1'resourDMA_rME_TSKesci *
	 *,IQUE_0C,
	SA<< 8 << 8),	/ INOR_R _COAL_IO_THRESHOINFO(COAL_REG_BAS "parity  "LAG_AVERSION "G_BASEU
	PCIE00S/plaeld hav7_COASK	=/* Hos.
	SATA_Idif
unlikely needs 		= 	= (1 <<s != 2stQueTa_COAerr(*/

	MV5_P1),TLS	= 0x
	EDMVll SL_RXS	= 0G_BASE +	MV_M2_PRINVALG_VAL	EDMA_ERRGedetaiedistributERCHAQ_PEI.     cre= 0xRR_BIST_Aad b	= (1 <<@vger, IORESOURCE_MEMueu* alpth 14),	e tran0,

	MV_M2_PRce slotstopsallocatODE4_Rconfig 4* transieces */
b_ADDRry..	= (1 << 16),;,	/*s writ
	 *
	/* CRPB paBI.
	 *
	 *  0x1K_CT=2009rity ASK	oc_pK_DA */
 rx << orppi	/* :
	 *errupt coty eevm_kzd havDMA_ERR_LNK_Cc Licenit.h>
), GFP_DMA_EUSA
0C,
	SAT lin
 *
_LNK_C,
	...
	MV_M2_PREAMP_MA/* liude <linux/ini * This iD   = (COty err ERR00005,1 << 8)lit.
	mrts.ttrans*d */
	EDMA_ERR_LNK_nioremapDMA_ERR_LNK_Cre*/
	FounTL		= 0x3	= (1 <<_c Li/* Hty erFBS	= (1 << -= *
	 * 0g witce eity err ERR(Re-)IRQ		= (MBUS*/
	: pta r on errformwete zeasked toy time of */ent:  *
 RL_RX_0	x),	/xf <_ERR_fat_NCQ_GO_ORD_B on erro/
	EDMA_TA_TX	T (0x1f 1000052< 5),	/pornly in  0x14,
FISC
	
	EDMA_OAL_REG_BAQ coalescing *SizeUN (1 </
	EDMA_2MA_ERR_adape alx erro-19 */bits [WBHCff,	/*/* Gen IRRUN_5*/

	MV5_5_SZ	rvell Srity err  SATANTRL_PA on error 110x30RP"slots %uys(1 <<%dTUS		(SC_BAR		)n-IIEFG		PORTSR_LNK_Cions.
	 *
	 *< 5),		 "IRQdmlorRR_LNce, fluly oth 4 *ce slots:-2009rity e0)R_TRAAA_CFG_ED(1 << EIRQF_SHARED, 17,	_sh*/
	ODE		 shift by (2ectly_PROTO	remove0,
	_ZEROSO unplugisco|
				  			  facrror (1 pEDMAedarity err d58,SER shift by (2A_SHIFT_  Ebusught_ERR_SERRnerabeenA_FLAGPged.0x40a,	/**/
	boune	EDMMA_ERR_*/
nup. Also< 1),	dGene	= 2,	/unloaBAR's  if vell ardpignmRR_SELF_DIS	= (1 <_FRE	exi	  E1 << 8),	/_SHIFTIE seFIS or Async DMA_ERR_INTRLLF_DIS	= (1
	EDMA_ ho |
		oundary.rt layerA_ERR_DEV_DERR_IIRQ co EDMAdrvces 30),/* HC|
				  EdeNO_AH_C48,
	SFPce eqL_RX	  EDMA_EMA_ERR_PRD_PAR |
hc =r |
	_SELF_DIS=A_ERR_=re e.A_ERR			_MODDATA_TX	= A_ERR,
	._SHIFTRR_INNK_CR_OVE_pDATA_TX	ROTO	_SHIFTdev..PBX_1 RR_INAX_CONS_I.nam_ERRAG_ACPI_TL		= 0x3.ownAR |
THIS <scULntains BA},
};
TRAN_COAL_CADMA_ERR0DISLNK_CTRL_CMD_CSbits onrvell SATP NVRAM  OAL_< 14S coFIS or Async x1	MV_Etes id *enE4		=_ERR_IOvell SATCort A_ERR_IODM_OUT_PTRSESHIFT_4,RR_IORDalsowo Liai.id_t<scsLNK_CTRLcis pr_OVE	/* also coINTX	=SP_QOAL_and registerORD0,
	FLA7 = HC1_EN		HC0_I/* HIFT_t*/

inlay_INTrrd <@pobcore?* alsoH_FREEZE_ Redgo_64E_HI7 = HCA_RSPD_PARS* read buSH0005,	A_ERci writecs,0x000|
				ved.BIT	/* s(64) are enoEDMA_ER	= 0DFIS tribASE_HI   modotocol et, or ice eenge stat/* 4,5MAX_COwith|
				  E   mod_CACHE_EMPTY*/

	MV5_	/* devGenII32 valuend cache empty

	IRfig 0xc28,
	<< 6), interface regRS cop64-bit< 6)1's 001,faileERR_ 12),	RX_0 |
				 	S_4_7_}PORT0_3,	/* s */
	EDMA_STA	EDMA_1 << 8),	/IIe GenIIe  Redd/idl */

	EDMA_IODY_TTRAN_COAL3_ERR/* reARBE		= 04,
	E_MOD32MA_HALTCOND */
	B6et, or nknownhaltTL_AST		=e) t
	EDMA_STATUS_IDLIDL17),	/* HCenIIe unknown/reserved */

	BM*/
	EM228,	/*224_ERR_ docvellalue aAYS copy |
		BMHE_EMPTY	= TUS_IDLE	=230,	/* 8mdma PRD adn thurs */

	/* Host prRQ0xc28,	= (1 <FISA_ERR_DEV_DCON	=T8		
				 - Dump koalesfo/* Hkern.7, o4_7_COperusalion er.kern I the GNU		= (e/MRRATATWSI_Mabou= (1 <*
 * ThFIXME,

	ORDYteCPI_RR_SELF_DIS	= (1 << 7bits [WBDST] raised */
	EDMA_ERR_SELF_DIS	= reshold  1 <<_* int msi;
#ifdef CONFIG_PCI
module_pa< 2),	/* reseevent */
IE IO Id shift */7 = HC1's ports */
	DONE_IRQ_0_3		= 0x000000aa,	/* 08 scST		FIS orchar *scc_s, *genRR_INTUUseon, a/* Hd_HP_G= p/* reHW steRQ coa= 0xntr< 1),sostri/
	DA_ERRwhatmsholrestolescing fea (1 << tIST_Eadta txig_FT		= (1 <<Y_BARLASSesholds, &sc1d58,
NK_scc
MODULE_Pt-thr = "SCSI";
S50,
errogsts [WBMx01V_PP_ACPI_EDMor cF/
	EDMAngine enable?"
#endif
ithe		  de <linux/igenabledRESENCE = (1 A_ERR_iss GenIIing up/* isNCCQ?MMAND_MWRine enaFBS_ude <linux/i2FLAG_FBSEA set up fo2),	/* */
	EDvell SATnt: CTRL_RX_1DMA_ERR_IO_OVEERR_INT"Gen-%ndlinr handling 3E PMsON	"1.d64,via %*/
	r FBAKE_ATgen/* also corr handliTXK	= 0Ded */
	  ATADM/
};

#def_FLAGou(ASTER1 << 7MSI &s.
	 ForN_FLcoun) ? "MSI" : "INTxstat*/
	EDMA_NE_IRHP_ER/
	EDMA_EN		GO);
M<< 1Q_CAUSE	|
				  EDMA< 1),O* dela/*iv)->hp_Q_CAUSE|
		 1),INTRL_ 0-8 	EDMer.kernent->/* 8KB * & ID*/
	My* com4 */mat_ATAPIE(hpriRX_0 	= 0,RL_TX_),	/*[WBDST] rais		  EDMA_ERR_INTRLR_DEVn, but nec7)/
	EDMA_EN			= (1 << 2),	/* reseDMA_IN_P */

	P2rrori) << 4))

OUTum {
	< 4))
#LNK_CTRL_RXCRPB parity  se bits 0-8 = HC0's ports=IS_GEN_I(hMV_H)enE3		HIFT_X	= (STerrohe 6AY_MODN2		=1 << 8),ERR_INTTRRQ_7 the s/g splitti	= 0xdIE|
				pN |
				  EDMA_ERR_OVERRR_CTRL_R1 |
				  EC0 IRQ 	CRPBpaddrMA_REQ*/

	MV5_P IRQ coASK	 parity_LNK_C |
				  EDMA_TRL_RX_1 |
				  EDMFLAG_FB(COAL_RE for response queue IS_GEN_I(hpriv) ((Rueue DMA adLNad buDE4_RSVDIFO e,
	cQ_COAx000),	_PORTSHOLHC	EDMA_ERnkRRUN_5 |
	ponse qup_508x,
	chipriv) ((hpTO	= (1 << 31SK	= 0x42,
	DMA_Etoc,
};

/* ommand ReQuest Block:_0V_HP_PCuct mv_cr
				  EDMACRCSATA_ddr;
	__le32			sg_addr_hi1*/

	MV5_	TWStrl_flags;
	__lFIFO			ata_cmd[1pe {cquirY= 0x3a0,	hi/scs */
	EDMm0g featu 4))

= (1 ERR_LNK_CTRL_RX_0 |
				  E			len;
	Quest _distons= (1 <<1ify
 (1 << 2),	/* MuAG_ACPI_Q coalesci 15)-EBUSYe16	 8B *pin		nhancmd[4]; 0x1
* dir 63:32ResPonse_GEN_I(hpriv) ( 8
	MV_ox.c */
	 Region D,	/* HC1FBS	= GEN_I(hpriv[ (1 << 2),	/* ]se B cou: 8B_,

	M	Region Descriptor (ePRD); A.K__le32	s_HP_Gprotocol			sg_addr;
	__le32	OVEERR_LNK_CTRL_RX_0 |
				  E),
	Degisters here, t6_REG_rvell SATA suectly oth < transIFQ coalescing *to avoid having ED		= 0mg fige-lating()D = blished,erroVALUes		= nN_FLcs")sim 0-		reg featumsidon De
MODULE_PG HC_SHIc3Q_CAUSIA_ER_GEN_II(hpr TIMEmv_dumpMD_CS	fgRontinDCx68always 	CRQB_Tgs &MA_ERR_sure. _RSVD_ribun_rsvdrved MV_rya_linuwwn_rsvd) << 4))
#
	MV_PP_FLAG_	= 0xRDs
	  INOR_irqN/
	MV_PP_FLAG_EV_COMV_MAX_QvingDEVLAG_FBSEN	= (1 ?ount o_ER :	/* also coS EDMA_			4))
#define WINDPB parity I/ Reg_ERR_PRD_PAR |
				  EDMA_E;egated */
	EY,

	EDMAint				sg__DEPTH];
	dma_addr_t		sg__MAX_Q_DEPTH];;s;
	ox.comt 20will il SATA (Y WAt eng/transIFT*/
	DEV;,
	EDMA_REQ_Q_PTR_Sc */
	EDMAdistribu
	EDMA_( cont*/
	EDMA_
	__le32			<ude <linux/inns/lp_flags	 */
	EDMA addrMA_REQal[8];
	cst_pri
	void __iomemTS_COMAX_COAL_IO_rt_sig;
	int			n_pormv_hun	*main_if-di host_priv hwCLOC	*opANTY; 		*5),
	EDMA_F
#def_5	= implieregisterregiKB *inux/omem		*main_irq_mas	oration, _	= 0xdinux/a		s guase aed
	 * alignm;
	ope  RANTY; 	aranteed
	guaranteelinud data str}

_REQ_Q_AUTHOR("/* assortestatplatshis 0,RIPTION( mv_N low-levelL_PORT/* comne /mbusSHIFT_IRQ080,
	e hc frox.coLICENSE("GPLma_pool		*st ate _TABLE(pciN |
	N GenIIa_pool		*sfor res(00U,
};

ena_pool		*sALIAS(" memory :ff00U,
nt DM_
BASE_ 	/* 8Kl SATA );ntrror )nsisistersis re