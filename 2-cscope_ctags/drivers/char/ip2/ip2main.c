...
ip2trace (unsigned short pn, unsigned char cat, unsigned char label, unsigned long codes, ...)
{
	...
	bc.hdr.label = porat;
	tracebuf[*****stuff++] = bc.value;

	for (;;) {
		if ( ********** == TRACEMAX*********************0;
		}*   PACKAGE:     Li***KAGE:  rip
*   PACr Int++elliPort faGE:     L
*amily ofCelliPorollernux ttty De	iport
*wrap*
* y*********!c) 19-- )DESCbreak Device Driver for Int*********++pCode;
	}
}
#endif
...
static struct pci_device_id ip2main_
//
tbl[] ate  Fnitdata =****{ PCI_DEVICE(r thVENDOR_ID_COMPUTONE,eel se
*
*neturn logic i_IP2EX) }, ove}
};*********ipl_iocTABLE(pci,e immediate DSS_);
