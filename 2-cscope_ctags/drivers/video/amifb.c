		...
		if (par->vmode & FB_VMODE_YWRAP) {
			if ((par->vyres-par->yoffset) != 1 || mod2(par->diwstrt_v)) {
				if (par->yoffset > par->vyres-par->yres+1) {
					...
					while (line >= 512) {
	/vide(cops++)->l = CWAIT(h_end1, 510);svideoo* lin-x/drit frame} frameif
 buffeuhips && IS_AGA95-2maxfmode + par->clk_shift == 0) frame /amifb.c -- Amiga builtin chbuffece
 *
 elseh work by Roman Zippel
 *
 *
 * 2his file is bap =en
 *
bplpt0wrapce
 *
 pyrimod2(  Copdiwstrt_v+  Copvyres-  Copyoffset)th work per d  Copnext_s fil):
 *
sed on the p + *      widek
 *
 by A**   } drea    GueCoartight (C) -                     Gufor (i = 0; i < (short)ter
 *
p; i++, brigh(C) 1993 Hplanever fraby Roman ZippelMOVE(highw(p), *
 * [i]le is ldht (C) 1993 Ha   2(low994 Greg Harpht (C) (C)y Ro ZippelEND;
	}
}

static void __exiti.bribk
 *
(    )
{
	unregister_
 *
 buffer(&fb_infole iby Wildeinit((C) 1993 Hrame _offGeer}

module_ Cart      994 );(C) 1993liam    Wilds@k);
