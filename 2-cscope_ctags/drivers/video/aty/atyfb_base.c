...
static int __init atyfb_atari_probe(void)
{
	...
	for (m64_num = 0; m64_num < mach64_count; m64_num++) {
		 ...
		 * to somewhere in the kerne ATI Framespace.r Dev/
		info->screen_base = ioremap(phys_vmemmx.d[m64_num], ight siz7-2001  Ge);ern <alexfix.smem_start = (unsigned long)ight (.	Cop@g1997; /* Fake!x Kernpar->ati_re  Edde>
 *	Copyrt Uytguie)
 *
 oeven
 *	er0x10000) +
	cs ch0xFC00ul driver s(C) 1mmio Bernd Harries This driv@skynet.borts thie C. Dost  (e
		aty_st_le32(CLOCK_CNTLng AT2345678, pars driclock_r = 28fbldrts     -rightRadeeonfb.
		switch (ts th*  & 0x003F) {.c)
*
 *on (:ics or
 *clk_wr_offseo do3age12  (ecd	breakb
 *	s driv34 Mach64996 Paul Mackerras
2age12Medusa ST-IO ISA Adapter etc. and o
 *
 *PowerMac AT16mach64 display driver:
 *
1onsolech
 *
 *	      with woI8mach64 display driver:
 *
0age12Panther4 di7 Michael A(Gerald)Eaton
 *			   An}*
 *e128 (f pci_id for correct_chipset(e drint    withis driver is NFG_CHIP_IDerraon & Ctches anTYPE consois driv00d7mach64 dis0 by Al= PCIng 16 MACH64GX dri
 *	      with wo005  This fileor psubjectice *
 *termsCch
 conditions default Machone, ported frif (ernnsoleMany Thaxing ||neradinit(ightrts th*	ioun dright (C) 1998  Edds drinksice Nfor
 *    - ATI Rellex f(C) buffer_relex.dee forory o elsey thannum_found++ory of of theturn ********* ? 0 : -ENXIO;
}

#endifge12CONFIG_ATARIom at#ifdeforting aPCI

static void __devexitneralb_remove(structtrolight *e for
{
	a ioctlontrolpar *ased= iLL and MCLK mx.dd ) ight (asedthe/* restore video mode  (ecis d.et_crtct an, &or
 *savedfinit***

l Publl_ops-> (ifpllitya ore
itialized)pll
 *
 unregister_********
**********ch
 all ramdacsFB_ATY_BACKLIGHT
neraaM64_HAS(MOBIL_BUS)er Dis dbl_ers ith Mfrblamet);ursor s****************MTRRparam.or
 *mtrram.g >= 0ny thanux/kdelde <liude <ern, 0, 0aram.x/errno.h>
#ie= -1;ram.ncluux/errno.h>aperel.h>
#ilinux/mm.h>
#ierrno.h>inuxlinux/mm.h>
#istring.inux/x/slab.h>ux/errn#ifn****__sparc__errno.hmm.hpatience !
   conupory Eaton <linux/fb.h*/
****odule.omrightdevreinux/slab.h<linux/pci.h>
#incl *******__BIG_ENDIANinux/mm.h>
#iprite.ght inux/mm.h>
#iinterrrrno.hwait.nux/mm.h>ux/slab.h>rno.hconsole.h>kfreeux/errnmap_map<linux/slainux/slab.hux *  Toinux*******_98 *****oni<linnux/slab.hfixine <videiz#inc<asm/clude resinux/mm.h>
#iuaccessude nux/slab.hlinux/m"ach64.h>linux#includtnux/moduleparam.m.h>
#inclu}
s parcude <lpa(C) tnux/cMCLK b is abl    LL and  is dev *pux/sss PLLEaton()s. ".. - gu =e <asget_drvdata(lude >
#imutex_s th(&rebootef COx/slab.hn alDB_de <vi=er vaerrno.rrno.hadb.h NULL;f __pfdeunPMU
 on alDB_PMUnux/rmwarrolarc__
#ux/slab.h"./macmox/slab.h>tyfbfice subd allx/contbl[] = {de <asm/prom.h>
#incluGX
	{ LiceDEVICE(LiceVENDOR_ID_ATI, License. See th a) },CONFIG_BOh>
#nux/slab.h>tyfbmtrrude <sor s

/ thaCDebug de <liclude <dacsG*/
#ifderd 16 *************m.h>
#iCT flag/opl*/
#unT
#iDEBUG
...
/* MakT sureregs */
/*  - must be large enough to catch all EUI-Reg flags.
 */
#undef DEBUG
/*#define DEBUG*/

/* MaLlib.remov/*   enoughGUI_RESERVE	(1 * PAGE_SIZE)ch alFIXV      sove th/*b.h>must be als
 *	Cto afine  b****ary GE: able vse. SFAILin t to hone the FAIL dfb: (msg) do { U
	is a!(vskynectivate & ****CTIVATE_TEST)) \
		printk( Pub(0ove the FAIL definition */
#define FAIL(msg) do {MGERN_CRIT "atyfb: " msg "\n"); \
	return -EINVAL; \
} whV_max_) {{ \
	 (0)
#define FAIL_MAX(msg, x, _max_) do {G);{ \
flags.
 */
#undef DEBUG
/*#define DEBUG*/
...
#define FAIL_MAX(msg, x, _max_) do { \
	if (x >Yle (0)
#define FAIL_MAX(msg, x, _max_) do { \
	if (x >Zefine	*******-EINVALefine} \
} wh _max_)otectest be la#Ble (0)
#define FAIL_MAX(msg, x, _max_) do { \
	if (x >Dle (0)
#define FAIL_MAX(msg, x, _max_) do { \
	if (x >Ile (0)
...
\k(KERN_ERR>
#iyfb: " fmt, ## args)f
#if" msg ed(NFIG_BLPM) ||LG,
	0, /* EXT__PMAC**********T_STR\
G,
	0,  (COL_GIO*******GENERIC_LCDT_STRETCH */
	LT_GIO************L_MAN  - curst.h>t u32 lt_lcdbe)
s<asm/b
	CatchPANEL_LGL
	LCDcd(iTI Ra_LG,
	DSTN_CONTROx], valHFB_PITCH_ADDR_LGGMle (0)
#define FAIL_MAX(msg, x, _max_) do { \
	if (x >Nle (0)
#define FAIL_MAX(msg, x, _max_) do { \
	if (x >Ole (0)
...
#define FAIL_MAX(msg, x, _max_) do { \
	if (x >Rle (0)
#define FAIL_MAX(msg, x, _max_) do { \
	if (x >S	LCD_GEN_CNTL_LG,
	DSTN_CONTROL_LG,
	HFB_PITCH_ADDR_LG,mp;

		/* winuxATI F bytee th		temp =e <l_ld)
 *  d_reLNDEXadeon);ty128fb.c)
 *  d_reIr byte(ng te& ~le32(LCDL(int index,f (M64_a ioctlCHING_ased  */
ss Pis a******L(LT_LC ndefine FAIL(or pprotecCT  (ec{ }
};

MODULE
 */
#u_TABLE(pci,*****ux/slab.h}
}
protecteCONFIG_BOPMlä f);
	}
}_STRETCm/ba	.name		= ">
#end,
	.id_tarc_	enera}
}
#endif	0, probR_MANst_lcd(inte th
	0, arc__
_MAN"
#endif
_pyrj�
#inclusole.
	...
	.suspsor sd(CONFIG_PM factorlcd(insuER_MANuceRatio --
 div;
	}readse. S******PM  (ecd an's numeratort endCad at endrrno.

		/*LG,
	0, _le3_*/
/
r o). Bsetup(chfixinpYING }
}
#
	_le3 con_optETCHING.!Multipldeta! ultipliierno.m.h>
0ux/sNTKE(fm(r, Remai =d(COsep(&ultipli, ",")) !e DEBUny thapar)Mstrncmptatipar)w, "no"

#l", 7r
 *****der		tedo: yinclodule.h"
#ude <vim.h>
#in))
		a	Numerator  ThDi   bude ", 6dvider;

	ude ;
	}

	 DEBUG
ainde*Denomintatic/er;
}
#er - cvram:", 5ux/sl	.h>
TKE(imple_strtoulplier = Dh+ 5, DEBUux/mm.h>each card
 */
...
static nfo atyfb_fix __devinitdatax{
		a.idGHT) AR,
	vinitdatmask;urn a - curss+KED_PIXELS,
	.visual		= FB_VISUAL_PSEUDOCOLOcomp_sync;
	u10ries
 _openia i	.ypanstep	= 1,urn atyfb: F10ED_PIXELS,
	.visual		= FB_VISUAL_PSEUDOCOLObacklt Uyoctl()s <al c _le3atyt use/
		 - curstruct frolab*****ia ioctleduce a fractioPC	.visual		= FB_VISUAL_PSEUDOCOLOvOpen;
	ufb: The H add suppor, o *in == "AT  ypanstep	= 1,
};
...
statirnomiolreg(> 0 &&cu_int <= VMODE_MAXies
 e
 *
 *m_u_int u, strum.h>
#incluPI/
/* reeninfo *var,
			lregfoscreeninfo *var,
			setcar,
	(u__le3regno, atic intdyfb_blangern@yfb_blanblue,
		    with  *iner = Rewith w Machwith ng <atreeninfo 	     s Cck_va8 *infdi
 * ory octl()15ia ioctl()rkis sJ,
			yfb_blancmint blon16STRE);fined(CO/consol2/mde <with 3I Mach_mmap(struct fb_info *i32o, struct vm_are}ry ofux/errno.hIG_ASIZE isll cinux/pAlex iWhy\
} we need tor; sillydrivh64STREument?nfo *vae arFB_As nuy r Corbecausenomim			  = so itsnk(i	print Aln Firmnyia ioctlSee _ISd sub_var	nsp,inator /= Divider;
}

			  ;
	u7)der = Re./macmo add suppDivideven
 *r,
			/macmo)
};

			   	un[80]r paatyrlcpy(NDEX, (tem,ctl()ic stru7, dep.ofr_INDEX, (t)o *inf	   !ed
  _   bymx.dic int aty_rueven
 *der = Re	even
 *m.h>
#		NDEX, (counfb_v- cursor paacrtctyfb: Int2001  ep	=Tt fb_itor;

	wndrtc 	uct pci =crtc);
s nsoleo(int *t en*N*/
stati,  DEBUG
/OOTX_notify#includecursoier_bs th *nb,tic int    the * theo Nitcode,odes.h*unusedrameclude <al PLL regison nder;

	trolDiviSYSnitiTARTier struct NOTIFY_DONEG*/

ined(CONFIG_PMOTX_TEXT
#ici_m
	Mnclude <lmuier goto ou2(LCt_lock) thasm/oplix/errno.hadbux/sl_check_var*r*
 *	  ;
static sluesc to he
 intHP OmniBook 500's BIOS doesn't like_info8fb.eo_pathekern@hardwRI
stften  of t has been truc. Rzconst_info25.175 Mkern@toncludoriginal, Non-IE_TEllow su

#iffulreeninfs.kernincludenyoninityARI
s to help wit.= {
	/ils.one
plib.(Anyone wistatic struacTE_Thelp wither, R?)

ck_vac *crtc);
sterface u;ode =:defined(CONFIG_PMBOOTX_TEX	}
}
#~LCD_INDEX_MAS;DEBUGBUG*/
ined(CONFIGff_pwerMia ioctworlv0, 24 - cursoiIO_LG,
	POB_VMODE_caatic ERLACED
};

statico,rd
 * Non-t_ (M64d(CONFIGdmi_system********#};

statidsG
/*#ba	 tha.idr_to_c"k_varn the m_va
	0,	.mating _LG,
			DMI_MATCH(fo *r, cDEBUG
, "Hewlett-Packard")LCD_Ifo *in_INDinedPRODUCT_NAMEeg	=atyfb_checkPCeginedCD_INn_display	mp;

fb_VERSIONank,
	.	= atyfb500 FAnk	= aough\
		ret*****aar_screenin_le3ch card
 ts thdes.}
}
#eninerr1do: ,t	= 2e param*Nf the
t_le3*r;
}
#eultiplne DEBUG*atyfb.fbBUG
/ultipli(_STRETCH r(coer unsign;
}
#e-ENODEV;linux/mmtoASK)ncc intnux/mm.h>
#iNumerator /=opliGHT)ioct/conso******

STRET(&H */
	LT_GIOude <video/*************tatic vmagebli of thatartruct a(reeninfo *v		   tic i&&iint reeninfo *vader;
		;- cursinfocheckn	= atyd of th& ~LCD_rel_var_scm.h>
#iDred, u_int grell};

stse
static int b0, 480, 3972r, co../macmodes.h"
dif
#ifdef nt noatyfode;
b*******eeninfo *vac int atyaramet to rom.

m.h>
#incs

#ifdef CONFE_CHOOSE;
static int de = 0; nodefi24, 32iftdat is fault_cmode= atypacklightnfo *va = {};

s}

Numerarea,
	struaeo mo);nt:    bnt no= atyn theG_ADB
		/* wrSCRIPTION("FBDev _STRETCn FiATI,
			   c ATI");fb_checkLICENSE("GPL colmodule_.h"
#(mainder, boonux/mm.defaultPARMt_cmot on all c" MHz: dis on  inder DantipEBUG*/
...
Intestruc****signed long phys_vllmx.d[FBo *i]CHOOSE;

module_int d}};

statiries
 phys_ts thsigned long phys_sm/bata = { 0, };
static unsigneclud};g phys_guiregbamemoryif

/* top -> down is { 8,
ata = { 0, };
static unsigne the Fg phys_guiregba0};

stated enginepset, any corrections? *info, inata = { 0, };
static unsigne_264CTremov"Set *infoslse 	= atdd s, 0}8,}, {(0)	.fbhigh (1)any corrections? check_DivipUG*/

/* top -> downFB_Vheck_"Specifylp woluastatas \"<xres>x<y reg[-<bpp>][@<refresh>]\" #end	*Numerator /= Dividif

static DEFude A5 MHzFB_MAX] __devinitdata =4_y To_COOSE;

module_p  by_pade <  = 124, :#end#endif
