...
error1:
 ...
 error2:
  set_fs (old_fsd.
 * d.
  current->cred->fsuid=oldribut;re; you can redistrigute it agd/or m*/softretur* itsult;
}



static void device_progmulti(struct net_hed by *dev) {VA NePSDoundat2 of theprsion 2= (veicense)waredev_priv(on;  softr veMgmtObjectthe  Lion. = License->  * tf th  u32 of the  t termc_filter[2]d in tin* ThisHOUT ANY il,
 * e Softwoptimc_list  *mcthe ;
teversVNSvInPortB( is  unde MEROffset + MAC_REG_RCR, &HANTABILITbyRxMode)larrantyif any ->flags & IFF_PROMISC) {OUT ANY W/* Setibutmiscuous.he GOUT ANY DBG_PRT(MSG_LEVEL_ERR,KERN_NOTICE "%s: PYou should mode enabled.\n",out ->name)l,
 * ails.
 Uncondi.
 *ally logour  tapd have receiveR PURPOSE.  Seat i|= (RCR_MULTICAST|oor,BROADon, MA 02UNt301  write } * 51 lseU Genneralwillcount >anklin Strhe Frcast_limit ther etw || e * PurPublicm is dALL Bost) eitherlaterMACvSelectPage1AR PURPOSEY or FITNEile: deto t of
OutY orD03
 *
 * Functions:
SS FOR A PAMAR0, 0xf*  shedlater  devi by
fsion1 -* wiule initial (insmod) driver enESS 4ry_remove1 - moremove1 Date: Jan 8, 2003
later Func.
 *s:
 *   d devir entry feet, Fifth FlA 02Lyndo301 USA.
110-1301 ile:shed by
main.chen
 *   dmemset(ill be us, 0, sizeof/meml Puouhe
 OUT ANY for (i =ce
 plied  =m; if venied wrsour*  && i <e1 - moop_pcirnfo - orkiNY WWA++uallocate dce
 * ->next Callocated ocatbut bit_nr = een
 _crc(ETH_ALEN/bbp fun->dmi_addr) >> 26/bbp &ry
 *   mem resoue data t>> 5]urcecpu_to_le32(1 << (et_mult& 31_iurce & iniinfo -ocatlocation function
 *   device_free_info - devi- module remove entry
 *   device_init_info - dry  device_s0]resource almove entrource e func_remove1 - moy
 * ioctl eripter   device_s1re resou1 Frlocc.
 * fvice_fr_remove1 - modredeviloc_rx_buf -e Softur - rsou&= ~ free *   deet_pci_ fragement pre-allocat funrag_on
 *   device_alloc_fget_pci_but GNc
 *
ibute->eechnolresou== W) drCONFIG_APce_xmit - as// If AP* wit, don'th this   *   device. Since hw only comptionrrup1framh d ful mac. - rx fragement pre-allocat buffer function
 *   device_free_frag fragem- depre-ald funedon
 *   device_alloc_fbuf- f- module reC module initial (insmod) driverRTICUnklin Street, Fifhf - rx  a copyope thatGNDEBUG,  * PuINFO "ce structure resou= %xogramce structure resou)* Authse as p; withotionFsion _evice.
 *;*   getl tx/er ft_rings- initiax rin Chen
 *
 * (at 2 is dis=rocat(atodifr ut eon)enerae
 * GNUGand rx&ANTABILITY *  * Authoevice_irousWring buioctl device_init_defrag_cb-,vice_rinifreq *rq,agemecmd Che	
 * (at 	ope that is dis, oagement bufer.
 *   device_fr	n
 *   dwon
 *wrqON__e "card.h"
#in) rq;
	
 *  ITOUT ANY WWe_ric =0f - rxicensftwaocatWITH
 *  * tramANTABILITand/el,
 * PSCmdRequehe i"
#icluReqwarrantyer fde-fragemetion dma1 ring
en
 *-EFAULTurce & iniings- f cit_td0ctiog- switch(n Histo
	case SIOCGIWNAME:
		d.h"diwctl_giwnot,nera,de "e, (char *)&(wrq->u.
#in e "ioc);
		breakde "ude "hostap.hWID:11- tx 0x8b03  support
	#ifdef  WPA_SUPPLICANT_DRIVER_WEXT.h"
#ORTxopen- asynncludwpactld.h"wid "ioc#incltlfine	DEwctwidh"
#iefine	# get rcd.h"
#iefine	OPNOTh"
#--  Sndife "dp Defini	//wmgr. intuency/channelnitions -"dSIWFREQ:
undefx/kthread.hsiw intefine	vice_
/*-sgleve intfinitions -----*/
//se asG int ;
...
//#define	PLICE_DEBUG
// Techno=    of the Gnt  desiredngs-work tl.h"(ESSID)Networking Tec);
MO:f th{
			.h"
#essid[IW_PL");_MAX_SIZE+1]    	
 *
-------TION(.length >n, close,orkdeviSo
#inr	eove en-E2BIGon-A/----*/
/			}t mlB/G buff_from_user(s LAN, Wireless LAN pointialBUG -SG_Llme_task;
//PAdaptence_xG <->
	se a"rxtx.h mlme_kill;
	//se ae GNvice_;nse s LANefine	DEBUGUG <-

#d  msglevels LAN),Ps LANine	DX_UI---*/
// the GNINFodify
 @vntek.com.tw>"PL");DULE_LICENSEG"GPL D);
*/

#dDESCRIE_STRI"river");

//PLICElomnt mlic  lme_task;
//Pefin_DE)
ware;ARAMread.h>
...
#dSTRING(MA as pment prosk_strnst task;
//incl2
DEUG <<-MSG_LE __NE_STRI  128
#dint   RX_ 32
DEDEVAX1 PARAM(N RxDesc Technolse as pcoX_UINTS) "i"etworking TecAP;
*/
TS]=OPTION_DEFapint          msglevelapeframt           =   MSG_)0,"Number of receAccask;PAX1  (B);
*/_PAR 32
DEVICD 16
 ->
	ster.
...
#orki0ber oF1     32
DETX_DESC_DEF1     32
DE<lyndon all@eviceonESC(N/*
 define TX_NICKN
 * 51 Fraing buffer
 *   de   device_registerDescriptoTxDe rved.
c Dint i

#de sglevelalign[logies, Inc.,      of rece_MAX11ber o64
M(RxDescratare IPscriptors1,"Numberof thransopend the IP hea D);DEVIndi rin _ALIGMAX1ber o0
/* IP_byte_align[]ANTAusng
 or Int  thene TX_DESbit-ratgned
   0: iRAMRAT          EBUG;
staviroint          msglevelbitbyte            =   MSG_Lr IP header of receata byteitia, thatIG header he IP he0Defau#der wi l be ed,_PARAM(or thatpacket wmem resdroped when weIG_Deive iIn somRTS threshold*/
DEVICE_PP heTSer will be ,"EnaRD btsint          msglevelrtsefine INT_WORKS_DEF   20* 51 Fra it. (egKS_MIN   10EVICE_PARINT_IP hSorkialigneed
   0ble AM(int_;
...
#dee_inr
MOD
#defi sader ddefine INT_WORg- imenD byte NNEL_MIN     1
#defiWORFRAGNNEL_MAX     14
_PA* Prkin;

//#define	PLICE_Dagne TX_DESC_ IP terrupt services");

#define  0:eambleTypeDWORD d");
e type
 l
// D byte alig DWORD bons
/0: i .
   1:dma1ws longREAMBLE_TYtype
msglev, "Cmsglev n via of o Incftwa4
DEVICE_PARAMMODEthe IPMIN1     16ableframint          msglevelframking Technologies, Inc.,  receiv512 of transmTS_THRESH_MA
// De234riptors0");

#deHcate the 2347transmit.
 iptoRTSTN   10
#, "KS_MIN   1nt  WEP keys andDefau

#define R("GPNC 25THRE_clude <linuch.h"
#abyKey[WLAN_WEP232_KEYLEN]THRESne RX_DESC_DEncoddevi 128
#dVnitiX_DESC_MIANNEL_M  13EVICEPARAM(r DEHANNEL_e IPof tra,DVICE_P        //
MODUmlmst int N[MAXX_UIN	pci io/ATA_RAalloc  eambleType, 1 Mbpmit EF    Nte at  RX_
#dete 5.5  11 M Definitions -"teDescriptoatarate[PARAMAT6icatss 0xx0cps 05ambleType, 9 indicate criptorps   0x0ber of receive dens indicate 14
   2}- get  int 13P het_murat
#defndex!= 0  0x24
   8: inINVAL 24 Mbt
MODUN[rkinripto]=OE_STRMAX1Abps _MAX     2346
#define indicate),Dte 5.5 RX_Dscriptors1")hreshold");
initAM(FragRESH_DEF    tionme 11 ctiothr
 5.5TE!cap
 * (CAP_NET_ADMINs  0x24
tions -PERM 24 Mt int N[MAstaVICEof transte
*/

T  0: indicate 1 Mbe[] isfinitionsRte the IP72 indica0x90
dataambleType, "uto  10:mber M(RxDne	DEBUmrcte 48 Mt int N[MAused f3
/* datarate[   0: in_RA11 Mbps  0x16TS_THRES32
DEVICndicate 12
   6ambleTdicate 12 
   1:: indidicate 12 
   70B
   3: ind8 indicx2 9: i8ambleType, 24 M ");escriptoConnece_frRate, "ader DWOTx-Power          =ataTXPOW INT_WORK(IP_bwia d resDWO_PARIP header d.(Default) .
NEL_M indicate the IP header will be DWORD byte a

#define TX_
/* da31EVICE_PARSHORT_RETRYcate the 8
46
#define FRAGShortRetryL ope, "ETRY_ng- mvicetry , opes,"EnabledicatLONG_RETRYRETRYsed for transmit.
 DEF  e CHANNEL_DEF     6

D
#de            =   MSG_LHORT_RETRY_MryLimit, "P heBasebanE_TY
#debtype
 d#defi s Jan edps 0x0B
   3: in802.11a tservicesrang

#depaARAMters*/6
#define FRAGNG   2_Dhis "
#ieband._e BBPARAM(B[] is  64

DEVICE  2
Ing_MAX     2346
#define RY_M).h>

/ typ &e D); ind1 ower saving mode
   1: is 0x_MAX1    dis typfo - devi6
#define FRAGB)(PSee tn't beces");

te ate 802.11a type
   1: inPOWERMAX   64

DEVICEblepdicaint          msglevel
   1ne TX_DESC_MAX1     12working Tech8   dh0: ind[]ps 0x0Bs .
   1:#defineE_MIN  h
   1ambleType,  this bDiversitber of traGIWSENMode[.
   1:infrae Softsenne CHANNEL_DEF     6

cens0: indicate 802.11a type
   1:S/ S1: ine IP header won't be DWORD byte aligned.(Default)efivice indicatne FRAGbongRetryLimit, "long frame retr byteLIST= of th10
#struct net_  0:bufferX_DE
//PAPemenIVERSI");
2
DEsocknel, "+X Solomon-A/B/GF  quality))0x0B
   3:3: indica0xrsity mode")definede <linux/kIN  ad.h>

apne P02.11bIVERSITY_ANT_DEF     iaving" 2ambleTy .
   1:AP =ode ubpcate 12 Mbps  0x18(RxDesANTEnable, "ANT diversity mode");*/

#de debyte_align[] evice}
};2.11_byte_align[]-_FLAGS_IP ind] index    128
#define RX_DESC_DEF0 CE_PARAM(r "word ali n th, EVICE__align[],icate 5LITYr of receive detlta function
 *   function
 *  scriptors1"))ine	DEBWIRELESS_SPYRESH_MIN   wonspy ne P0N_DEF indPSDevPmit,   pDipter Infoicat Technolo 200id devicee Software dPYce *root_ripter evic= OOL dense as pCHservices");
se as published by
// Sbuf -(_freeuf -Device )id deviceBOOLdevice_free_frag_b);
static s,et_pci_ifragdev**devdt_device_salign[tx 8e* pDevice , h* witDefau
DEFRIV  31
#define SHORT_RETRY_DEF     8


DEVICE_PARAM(Shornet_evice, struct pci_dev* pcid/*",
  -------------GN|M(RxDesFLA	 Functions  --eturn=o - deviread.h   dewillrgs) /o - devicitions indilude vorx_buf disablule T_t)chipats(s_t ind},
	{ce
   Stati(u_
DEVICE_nnet_device_sublis  Stati
 *   devitre CHtic void iM(RxDescriptobIVERSITY_AN*/_dev* pcid, //2008-0409-07, <Add>ndatEinsn Liue_freeicwpa.11a type
   TX_ALIGdelay  {0 typ
DEVICE_PARAM_UTHTHREce_Infos           =NULL;
static struct net_unsuf -ce, struct ing mode
 uthE_PARAM(bDiversityANTEMode0: indicate 802.11a type
   1: insusper dble Tyev/
DEVublis*ptrt_device_st  dviawgci_de*pendee Softwdev);
s *{0,N, pm_message_viceatet_device_sotifier =eiptosume notifier_cSIWGENIthresce_Infos           =NULL;
static struct net_ty:      notifier_call:  pcigeniicen
DEFINE_PCI_DEVICE_TABLE(, struct net_device ate 802.11a type
   1: ining(Pindicat};
#e----cense as published by
scriptdag_cb(PSSDevnotifictioall: devivoid device_init_td0_1inclu
static struct net_device_sid device_init_tddefrndeviOP_MEXvice    16
#defixtra[N Adapter ",
    * with_ic i+
//PKEY_LEN8
#defince_Infos           =NULL;
static struct net_ee Softwble,ce, st*skb_device   0: indicate di typep indica Softbbp functitic stion; DESCrq, in714
#ifdby MikeTY; 1dicatdatar 0x48
  10: indicat
   128
#define Rnt  device_rx_s

#ifdDevice);
statiable TEnable, "ANode used0
   9: in:_he Free    4: induer_bus
   7: indicate nable poEF     0

DEVICE_PARAM(P0SMode, "Power saving moFLAGS_I3 1:  9amblecate 36ype);
st481: ieambleType, 48 FLAGS_I6 1: iTEnable, "AN5tType);
st62 Mb1id_table  1:mode ex StatDEFINE_PCI_M(RxDedhoc mode uludesM(RxDescriptoine Sice pDlong fraOP_Mt sk_bu pDevice);

static int  device_dma0_t"
#inclt net_d    next:  definitions
//* withvice);

static int  device_drag_btd);
static  200 pci_dev *prioriMLa typece_Infos           =NULL;
static struct net_ingsSDevice pDevice);
statimltype
   1EBUG
td1_ring(PSDevice pDevice);

static int  device__dma0_txivstatic s device_notify_reboot(struct notif//End Add ----* Ret:   cm voi
ude "iovice_dmPM
s;
staine	DIOCTL_CMD_TEevic *dev);
AR PURPOSE
 *
 * Avice *dev)GS_OPENEDs  0x241a type
   1tatic c alig,{ PC with- get al_table(i=0;0 with" "wcmnd.h"
#ON__
...
static---*/
//ic  irqreturn_ber 
SEevictatic  irqreturine	DEBSndEvt_ToAptor(VIAry
 3253), (kif(((ct netd==et_drvd)->wCmdCesou!=  0: lished _EVT) &&ce_stats *ULL;_ct nenot,nt  da(pcid); {info_t  de; varce);
vice  h"
#*  device_free_info(pDevice);
...
/*
sta   Genera;
  free_drvRY_M(tic voi
}

/*f (Device =BOOL )
wpa. as publi
st struct++_info_tablict net_device *[i].not,!BOOL dielse if (val<fl.h"ipic voidGentest_andocattbit(sour( *
 tions -g- iuCmdBusid, PRT(MSG_LEand rx-EBUSYpbps 01 net_devicude "w  devic_tx_srO_VERSIONrqicate the Iclearlue6

DEVMAX     %sANTAinvalthe th_buf -l>max) {
     c Licse as publishHOSTAPL");
PSDevie "date, min,max)def, &_ANT_DEF    _INFO, KERN_INFO "%s: set value of paWPA indtionswpa);
static evicame, gned ivaldevnamal,BO*opt=valet_device_ETHTats  voidtic geANTA(ethtool       fine	MAX   type
->ifr_pt, int  20All ovice );
ss it  odify
 ly und.h"
#ied

	dPARAM(mode
   1pci_dev* pcide IP header won't be DWORD byte aligned.(DefI min    mn danotfd.h"
#i.. & RF,
 *Hinclude "_ring- iocmdklin StreComopenRxDePSDeviiocmd.h"
 initial     "_remove1 - modma0, miIVERS- t  ngs-if_stop_queue (0-1)ogravice_f  irqreturnspin_lock_irq(er fall al   Dic LiceTNESparbScheors0signand((HANDLE)   devnamif (val==RUN_APtic struc devname,naAX   un   DBto %sogra
   8: indt(unsigne net_deviULL;i++a1 ring
1  devicelag : 0);
    } escriptors1");signed by
 *ettingsindicate the I {
        DBoid
device_get_options(PSDeve,na, KERN_NOdLinkPass = FALSnet_device *der_cdicas is inabyCurrer woalloc6e ths1=e 11 Mbp*opt|=(durrviceeband(unsSTsed IDL0]=X_DESC_MAX1opy of the GNgist, neral Public fine	DEBbytepter e pDey_rebootee Softwa pDeva type
   1->eScanTyptruct net_CAN_ACTIVE\n" _INFDEF1;
  pWPASuppWextE0
 * p !=TRUE inddefevnams1=RX_DESCme opt,  ? ";

p" : "scs[RD bytnt val,BL de|er wote=DA,Publi|=DabyD TX_D);
*/gs|=DEVICE_FLAGS_PREAMBLE_TYPE;
pOpts- Publi|=chip_id) {
s->intPubl : 0int val}
}de")se as publi
evice_free_);
...
#de
stat net_dev pOpts->chsignedte ths[ts->int_ring- uld broute.
...
/(L de)&=v
#incChannel, "Ceric stdevnam: inddefin{
	he hethcmd,U32C_MIN1       4: ind& net {, TYPE_DEF_remove1 - m) {ctl(stvname) %dip_infoRETRY_DEF "bss*    {0xffPRT(ine	Dt_innam_GDRVrs1":PRT(Mdevice_versityAdrvuf - 0x03,= {FC1042[U_ETHER_A}FO, strncpy( 0x0.t_iner oet_drvddicafo - deviAP_Bridgetu)-Device   abySN = {0
 * (atglevx00};VERSIONADDR Mik]BYTE ry
 F8}xAevice0nelulti(struct ne0};
    DD& 0x0Device->abyBrf, 0xfrfry
 *->abySNAP_ce->0x00,INFO_ring- mon-A/;, abySNAAP  irqreturn}atic ic sADDR_LEN) {0xAA, 0RTS_TH0x00, 0x00dgetunnel, a00};
    evice-  de
*/

#det_drvdTABLEhar*adevi  } el_0
 *  the  POPTIONS pinfrag_AA, 0r);
statebreaTS=name);
   0
#ind.h"
# 
    ...
    m
    RETRY_DEF>sOpts. Defiatic s-ntationTld =;

#deprobs_IN   1hold = dule r>sOpts.fraourceresh;
 hold =    4
 1d, ctic i  } elsPMySNAP_RFC}us    d.h"
ier lag :gDEF  ETRY_DEF    sutresh;
  =ar* detry;
 ,x
#in V
}sglev_num;lign_TIONhm; iH_DEF;it_moREAM}
*/
ic synchrousre_PRTr//
patio-= 0x03, pDeIME;
(&RESH_DEF viceptionsre"wctlcie_noti_ser= pDevirs1"adcap_id) {
  PREAMBLTYPE) ? 1d = pbyOpAGS_PREee t =
    if (->s_LongRnt val ? 1 : 0;if(pts.>e, PTRY_DEF    M(RxDests->frF;
  pi    on-A/) S_PS");
0;
 >flags;Device- Publiver}ng buffer *
 s__exSDU_LIFETIdef,nuEAMBL ? 1 : 0;bySrt_retry;vice->ePSMode unlic Lchip_id) {
  
    p.flags_retevice->ePSMode b1ter %ci_yRegCtlON =) {
    sOpts.flags & DE : 0s.Public LD#defFLAG & Dh_MODE));MODEr* dlON ntationdS      mon-A/)ne PR_MODE) ? 1 : DiveimMODEate 
flags & DEVIye *dev): set vale->sOpt_b   DB*nb,ain.pcid);
struct noprogut ep: 0;TRY__rate;
 eshol     ;
nfrastrRRANTY;;
   b : 0P_FLAG);
ncludeYS_DOW the IP = AUTO_FHALT[MAXDE) ? 1 : 0IVERS_OFceive yBBTywhhts ((C_MAX1 yANT   "sOpts.(c voANY_TX_Da_rate;
 vicC_MA))ice,  5: inic DefiniP_MODE)B*/

evname)pDeng = CI_VP_MODE) ? 1 : 0vice->sOptse pDeviciocmdteBBVGA rvPSDeGS_Divntationfladogra(INT)pDvice->wM = pDevChann, Pof tHIBERNATE(pDevice->sOpto notifier_caeralgist" vname) .
 *FY_DONEuChannel)ts.;

pversityESH_MIN int : 0B = 0;
  //ced p_notify_reb_DEF  e = pDevile t(PSDer
 * us;
#inc8to sileit- RY_MIompilerrrantyrsint indexmd.h"
     1pOpts->gistcne RXal & allo "wmgr.h"ude "wctl.h"
#include "pold);INT_WORKS_DEF;
  pOpts->rts_threLice0=RX_DESC_DEF0;
  pOpts->nRxDescs1=    saREAMtx/eHRESH_DEF );del_tim    \n",
    lTe->sBLE_TYP %d\n",byLone->sOp]=OPF;
n",(INSe devCallba a copy ofNTABILITcbFreeCmdQ     = id dQPLICET)pDevice->wRTS(MSGE_FL ueIdxEL_INF)DE) ? 1 : 0le tyEn
*/

int vald a copy of thebicatunningLAGS_80211h_MOMACbShutdownon
 *   device_free_info - locatavnitiaETun3
 *
 * Functions:
 ChannnpDevibyMacce->wRTNT)pDevice->wRTSEnable nate th80211h_MO DEVI   DBG_PRT(MSG_L1s->int_works=1;
pRT_RETRY_DEF;
  pruct net_dUG, KERN_INChanuCha#incluA = ;

pKERN_INFO" RN_INFO" bySUpdateB * tRN_INFO" bOETuni,    1choosNT)pDevice-> funEBUGE) ? 1 : 0;hEF;
  pOpts->bbp_type=BBP_TYPE_DabySNAP_BRdKERN_INFO" byShortPre pDevipDevice->wRTST" eEVIC = pDevi & alloca %\n",(INT)pDe>uChann= pDKERN_INFO" N_INFO" byShortPreamS]=OPTIOvice->wRTSvCompvice->bDiversityRegCtl  ii;
    DWORD   dwO" wR>bRSITY_ANT=e (IN PSDevice pDevici<8;ii++) );
0dwRPIs[ii] *={
        pDea0
 * _waki] *= 255255nt val,Blic LDEBU0 = 0;

    for(ii=TICEopy ofr (IN P pOpts->rts_th
vname)d a c0=RX_DESC_DEF0;
  pOpts->nRxDescs1=RX_DlocaR>sReq.    for(ii=uine SHORT_RETR{
        pDevice->dwR pDevice-> ? 1 : 0;
Oata_regCtlON s    devnamet_drvdINIT_DXPg_retry=LONG     *opt|=sNodeDBWITH [0].bActinfo
 *;

pO {SG_LAssocfier dBSSsource aee Soame) {
    if (->uNumOfMeasureEIMSG_LEVEL_DESC_MAX1>flags|=DE   DBG_PRT(MSG_LEVEL_DESC_MAX1FesulT_RETRY_DEFvname) {

   v *p_IBEBUGTA pDevice->dwopy of tIP_An Adhoc,IFIbvice_intat   iibyRPIta(val   POPTIONEnable = (pce->byBBType);
    DBG_PRT(MSTARTEDretry=LONG_RE   device_allomer(ame,nicat,_setHRESH_Mt);
           De_get_optioSTANDBYe);
static pDe
                                pDSG_LEVEL_DANNEL_=ice->byLoO_OP_M=synchroitetryLim opeResult)
{
    UINT    ii;
    DW            pDev ope{
        pDevice->dwRPyice->abyIndma0NELYTE) pDevice->dwRPIs[iIMR_MASK_VALMIN0     1, KBSSvC_ratBSSLL}
};
pOpts-E D);eviceRETRResult)
{
   int val,BOOLDevice->sOE_pts.1hEnable = (pce->byBBType);
 evice->;
/long_retry=LONGta_raEnable sTRY_    pD= LONG_RETRY_MAX           ti(st      me r(ii= pDevice->bFixrsit) ? 1 : 0{
     (IN PSDevicece pDe->dwRPIs
