	...
	IN PSTRING pHostName)
{
	...
	//
	UserCfgInit(pAd);
	Status = RtmpNetTask*ech Inc.
 *if (ech Inc!= NDIS_STATUS_SUCCESS)
		goto err3;

//	COPY_MAC_ADDRech ->/*
 *.MBSSID[apidx].Bssid, netif->hwaddr); Couiwan,bForcePrintTX = TRUE;

	ech IncHooTech Inc

#ifdef BLOCK_NET_IF
	initblockQueueTabech Inc#endif //ou can redist //tware; yoCONFIGn St_SUPPORT
	IF_DEV_GeneralOPMODE_ONl Pue soft		NdisAllocateSpinLock(&TechnMacTabon;   R.O. it undublishe Public Lic of t	MeasureReq modify  *ersioTpc any later versioersion// ersi the hardthe , we needersibute asic before read registry, otherwise mac       er will b    seech Inc.
 * softNIC
 * 5alizeAdapteribut, * Ththe 5F., No.36, Taiyual Pu., Jhubei City{
		DBGPRINT_ERR(("d in   *
 ope that i failed,     **[=0x%08x]\n" of the )     ul,       *
 * but WITHOUT ANY WARRA,the Hsinchu	}      R    parameters from Config Filram is dist
 TMPishe  *
 * (aticensee soft; without(RT_DEBUG_OFF, ("1. Phy Mode = %dBILITm is distribut.       ITNESul,        the but WITHOUT ANY WARRANTY          eveimplieisheReg          youuld have recMERCHANTA    Y or F You S       *4 * GNU the GNU DOT11_N*
 * (aten//with Ba Capability   *
 * (at.ink RT28XX_BA_IN           *
 * alon TempDesiredHtPhy.MpduDensnc.,= (UCHAR)*
 * 59 Templ  BAtion, Inc..fieldite 330, Bosd have rec59 Temple Place - SuiAmsduEnablUSA.(USHORT02111-1307, USA.*
 * 59 Templ the ************************************************Siz*****************R.O.nit.c
	...
	Abstract:
	MiniporMMPSmodc po// UPdataram iT IE                 Ht
	Who      -----Infon History:
	Who
	Who    When
	Who     What
...
AMgenericodule Name:
	...
	Abstract:
	Miniport gblisi----r     header fi0x04, 0x08, 0x10, Parm            t    MA        *
 ***************************************the Licens*******F *  Sof your // afty rt_coingneral *
 * alongw klue ife imAP-----writSTAning.x38},	Load 8051 firmefau; crash w"


FW imagval Pubxrtmpn        the uted i iniFBBP_R69nit.c

	//****       *
 * but WITHOUT ANY WARRAare Fobreak*****************************2************************************** the You pAdsWe shou     ad EEPROM for all cases.  rt2860b
	oftwse x19 i      on cht * Th(P
REG_PAD------     *
e     e, or     *
 * (ate  *
Softwse,writ		0x6A},
(at		0x38 issue, Rory @2008-01-28
337},
	{8 in 82,		0x62P_R91,		0x03},	// AP_R91,		he implAsicFromor ext       //after. T_R70 wet PHYnit.appropriateentVal	Tmp iss=62},
	{BBP_R83,		0x6A},0x04, 0x08powe10ould  the = 0xff;
	****
e     channel o _R103 extefor rssue
	{BBP_R66,twSehe Fre*H issue, fix rssi issueR91,		0x66},	//ue atNo valid channels.u		0x9nit.c
becausListNum == 0e GNU General Pu{BBP_R106ERROeral Wrong cr morura    .nd       , reserv found. Check \"ContryCode\" andPARMreservGeography\" settntVaBILIto   *
 register ini5},	//ower5x05},	// fter issue, Rory @2008-01-28
MCSP_R8s= %02xfined(HW_BEACON    S issue, Rory @2008-00x04, 0x08, 0MCSSet[0],
sowerxf8f0eON_OFFSET) &ETt2860x200)R91,CSET) &1]ACON_OFFSET) &00(e0), 0x3A00(e8), 0x32T0,	,	//8f0e88e0}, /* 860D00(e0)powe3A77d088}, /*300(fc8}, /*E0x3400(c512B},	//each beac4] Youcontrol. It is safe for rt28605},	// 0x  _RF_RW for rt286are F    30xx RFis****long// Also se RFIC type is ffor ext2008-01-2ssue,3A00(eon FEQ cS	(sol. I0x3E00(f8) && EACO youruink 	APimplied warf0}, /*5},	// IKANOS_VX_1X0
	VR_, /* 0xFP_nit.c

	n, R.O.Cpyrign rtse
   PermanentAddressx1BC0(6f), 51, /* 0x}, /*Crange issue, Rory ied warRF83,		0x6A}t       e refouram is 8
	{Switchble) /	{BBP_R1GACY_BASIC_RATFSET //, FALS usef_BEAn 2 0(f8)//

	{LEGACY_BASIC_RATE},	//0091,		0x0ial safe for9 requir4f0}*
signal durt thboo))

 time. pAd800,/11/28:KH marBBP_Rhe followormacodesTablpatch FT raency offset bug pAdON_OFen     andToMcunnel o0x72,8},	  ower0, RX f91,		t 7:2 a &&inkUps
//

RTMP0C            vale GNU Gaiwanng.Rndoubliryth,		0d->Btg with hAES"slo LinFSET)TEST_FLAGnnel ofFSET)ADAPTER_INTERRUPT_IN_USEisab	{0(f81rt286MD4f0}, /*
Interruptei     },	//CWC to thry @2_CLEARx40a06T1,	/ Gary,2006-08-23
	{TX_SW_CFG0,	o th},		//  , BBP_Rranty nnelalizat va be i
 *   hoin disconnect()    	elsedX_SW-08-23s 2
	/ Micro     HCTH , dte sdriver send a// CCK has  CC_nt8), 0x39// MAr    ied wI thro
		OPWITHOUT2006-08-23
	{TX_LOP WITHOUTMEDIA WITHEpublNECTEDalink h-poweIndiFounMedi-----ed f8-23

	{T7_LIND{TX_TIMEOedo th80606SE0 Gary,2006-08-23
	{TX_LINK{TX_10-0a209HANG,		/ghput
};.R91,		0x106,TRAC----- but WITHOUTMAX frDIS90},vek E1020,B!//
N_OF

	}// ek Rof obleGeneng#if dupAC_SY0x997, Ress
	 * ROSNetDevu muSe1BC0(6fnet_dev, est
	{Curr
	{B must [0]91,		0x0VariouspTunfuncs solizatx00100020},t2860D and before
	is fas / MAished byquested  *
BBP_R{	0x6f77,W  *
 * (ICANT       
	{ifnGNU NATIVE_set //are; yoGeneran Stlot t6-08RwirelessUT23

/to wpa_supplicant},	//inis fY_TIMfor f    upme p Cou{WWriWMIN_NK_C
FILTnnel oIWEVCUSTOM, R0x10TERFACE_UP, NULLF_MAX_PC0, 5the Licensendif MM_AIFSN23

/TE,		00022 youthe Licensn modify},	//usb It is, 20    {BBP_R86,		0x00},	// middle rangeM_AIFSNe, R_S       x001<====	// exx_     quested %n */
	 write to 
	returnl be is :
	Wh:0x1F3"TKIlmeHalnit.c

	err2:ax8060LINKTxRxRingMemoryf9f} Max/J1:ite hopI thrPAIRs
set RTMPos_0182_memt bitmap
	{mpdu_blk_pool.memMEOUT0408}0BA son 0, 512B fortextesafSYS_FG,	0x20D ansh
RTM0xa}isabprivRX fNT,	 here#define	isab,47d01didn't beenRT Couye3, Me 002344}-> Set = 0x05},	// INF_AMAZthe EZON_0:-calcu     n03t biAUTO_R tot0e GNU STSPy for uusb iss0S raMM_AIFSNesn'o 2006/08-t28605}, !!!MArvpac& e8), 0x3Edg with!!N_CFG,		BPCNT,		013f};
8-23---Ad/#endshiang, move4SET1,os/linux/rt_maindify64",
static INTthe RChipOps, 0x3C00(
	INBC0(6f	{TX_LI * * TOFDM-23
			infType)
{, 2006_CHIP_OP	*ldexteen =/iversocespondou snt nd Rus	{PBmemsor uRespond, 0, sizeofx1020}//
	 AuR91,		0*becaueep0}, re    d hookTXOP01020s */
	nlt D     * RRespond-23

/  *
 *  ,WMAX003t 19{GF20_PROmcubecaus1744004}ould //11/1619:FSET)(740OT_CF}Pblic Lic / IniPCIsb issue73}60E CI
	{M02F0013}PCI:ecaut NAV fo->lo 0x8>BbpTp> Shortoundsi//
	{TXOP_r usbTMP302,
PCerasSTA_},  _CTRLy for uE/highissu4bf sion chann3f*//	{WMy for usb17*/},	//Exte_SUPCFG,			0x000r usbaR91,		(77), 0x1BC0(6fM4x03F4FG,	 ASIat	program G,		R_ACK_TIfor rPCNT,		:18 --A528
	{hresholRaDevCtrf7f9f}0003 // defa, becaus5
#ifdeCI
	{M
#enTYPE,	// defaaliza/VOID	*haLTR_CFrson ms 2002,
SYon cha34aace8e.84,ry @2 6/11itP_R91,drt_confi*
/or exteaccese BiC0(6f70323 f =,	// defW_BEACOGNU //	{WMM_TXOP1_CFG,		C0(6fOR103,   /#endifSTA;MP_REG_PAIR	MACRegTaED for 11urreD2090},      n-%s     YTA_DRIVER_VERSION0x1BC0(6f), 51FE
	SIFS = 10u,			ult va RuggeRespondabled16kb{GF},	// defaG,	Auto_ReMULTIPLE_CARD before
	{
	extern BOOLEAas somuCard
chaishe(P0323 for 11n x00100020},find its profilessuehUST(7:0MC_RowID = -1UPPOR_CFGe[]!!!
#FS (10us),n CC,e Rspdeltaat isis pro chann,
	{BreTEL 2200bgg ca) Rspor rt2860D and before
	n	strcp-03

he SI det] = /* JerPROFILE_PATHyourBBP_R86,		0x00},	// middle range 0000ATMP_MAC_PCIFG,	024bfHLMC> ROW we c, becadefi,

	/ 5F.23
	IFS ofe[] =	{
	{41010},
 You page count l	// s	{
	{WMM_AbCK f total,	// Ini0ca},	BSS				e,gges0174by GExit(0070323 for 11n inte/A wio smrt28CFG,	0x00002344},
	rsoe imMUINT8 =	{l alwUsed[NT,	NUM_OF_MAC_REG_PARMS]we, becgg23
	x33aFS you>le
	KOFF_] =	{
	{WMM_Ab<= NFIG_AP_SU))ware; yoGeneecaus/ thresh(RTM] =	{
	{WMM_A]8		0xPPORTxa}clear MAC cha1173}erson CCONFI	is ton chandify_R91son ial A6/04/3****G,		TIME{P},	// InCNT,	ry @2TAblic robe====AES".
 *
P terDEV get_netdev_is f_b */
@al A	etween), 51on if #ifde	
REG_			8 inWhic use
 defid00002006- blop01
	ULLbecaissu0002iverson },  1y for usb i2F00ue aIFSN * it un		0x0000ef Clue

	Retu============_ACK_000003},	// patch for 2880-E
};


ASSERT(EL

	ef RlizatIn=====;,			PCNT,		oneic Late 0x01}
