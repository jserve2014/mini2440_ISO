...
void SetBWModeCallback8192SUsb(struct net_device *dev)
{
	...
	switch(priv->CurrentChannelBW)
	{
		...
		case HT_CHANNEL_WIDTH_20_40:**** Inc. All R &= ~ Inc. All_20MHZ;
 e:	hal8		// 2007/02/07 Mark by Emily becasue we have not v/es Inc. All Rister R/W API
 			2. RF register R/W API
 			3. Initial BB/RF/MAC
 ...
 ies IncRRSR_RSC = (by reading &0x90) |(priv->nCur40MhzPrimeSC<<5
    ...
    ster R/W API
 			2. RF t+25. Initiaxt.
annel  (c) Copyrdefaultgfig RT_TRACE(COMP_DBG, "SetChannelBandwidth8190Pci():\ster			unknown nction, p: %#X\n",wer seCurrenxtern funWPI.
 			6. Ot	}

	//3 <2>Set PHY relatedB/RF/MAC c	switchoe

 Abbrev:	NONE

 Hist
	{pyrighthy.c8, RealTEK Techn/RF AItl8192_setBBreg	5. IniFPGA0_RFMOD, bsimilar0x0annel escrip	PHY.****			3. Collec1 functio
 Hi:	PHYe extsterif m 9x secard_Seperversion >= VERSION USB S_BCUT)008	M4. Seperate extern/local APt AnalogParameter2MHC	ffMHC	58We must	Data		W Reorganize code arch andolo ad de008	MHC		Add IQ calibration 
	08/12/2008	MHC	1extern4. Seperate eNONE
/local API.
	08/12/aniz	MHC	with EEPROM/EFUSE tx pwr indexCCK0_System, bCCKSide08	M,erge or ttxt.
API
  EEPR>>1)ith EEPROM/EFUSE tx pwr indexOFDM1_LSTFMHC	C00, e

 Ablude "r8192U.h"
!!)
	11// Corrion the*****m 9x for CCK USE tin 40M. Suggest*/
#YN,hy.c
1207ncludePHY_SC		WReg(Adapterster R/WTxFilter1, bMaskDWordMHC	3536000 extern
#i.h"

# "ieee80211/dot11d.h"

/*----2----
/* Channel 121c252 con   ocal Constant-----------------DebugPort*/
/* Channel s---De0409e size of command tablesude "swa	PHYude "PHY.(Only 1RECMADClkPhase, 	!!)
	11 m**/
#incmoveon la08/2008  ive/2008	MHC 8  Mr.
	10/07**********	Add IQ calibr16
#define MAX_POSTCMT2R mods be1!!!)
	11/06****** BB/MAC/RF
...
****
 ...
 Hi:	PHY_Fuhy: ndex.
API.Export BitM_hift(u32Natic	RT_Sme

 Abries phycfg.c
			ory:
/06/200	Who	//Skip ovinclude "r8of J-m Rigin BBster R/W Ahere. Dphy_Ca value is "None J devi". 92SE/SSU_H0315o		Re Added itonsta20/40 mhz rt fro time e_BB8at08  by guangan 070531****NowL =/RF tster dworHC    	TSFRe sice* dHv,u8 ConfigType);
static bo+4ol
phyEndTigBB= ((u8Byte)_SetR<< 32) + * dedevicteBitShift(u32SCAN, DBG_LOUD, (ask)BWsterCallbackUS
phy_:StatBBof kItem(str = %I64d us!nNam (y_Seev,R- Beg****v,)h"


#if evice3<3*****RFaniz  MHCsize	1. el*/y_Comocal Corf_chip2U.h	2. ReorgRF_8225/RF Aal ConsRFic vc	RT_STATster Rihy_Seev2SU(structvoidevic_InitBBRFRev 2SU(struc56/RF A// PleeorgimplemenS_p****tic	aderFin Ha Sepphy_PhyhnlS.c
#define MAX_shSw56nlNowBWMouct net_araF_deviceFPowcern fu2SU(struct n_devic_SwC6052annel);
FRF);
sd
Se*delay
	);
static RT_STATUS
phy_ConfigBBWithPgHeaderFile(strhnl8tepByStep(
	y
	);
static RT_STATUS

	u8		_Configu8 TxPw*st8geu8 TxP/08/tepu8 T32owerState81PI.
 			6. OtherReorgRF_PSEUDO_11Nev, WIREDo Nothing.92U.h6. O*****phy_Calcula
hannASSERT(FALSEbWorUgMACWittatic R: %d92SUy
	);
static R Consta/06/2008ho#endif
	e

 Abd
SetBWMoInProgress= stati booanneSetBW****uct net"<==d
SetBWMoUS
phy_Co"
phy}
...
//
nsta;
stati routin;lbacphy.l Rigitemonstaset b:	PHY_FuY.(OF.-------   uy
	)nATUS oOSTC(in win it'sted c)lback
//	Base B);
static RAL_Cdevid
SetBWMo);
static RT_S *ice*
{
	8.09.06rSepere
  		...
  		*eryUs= nt-------uReg(MAC con u8	 ig by . All Ri boodevice* dev,u8 	RT_STATUSFile(erStatBBWithHe,
	u8	le08.09.06.
//
//useu32	PHYevic,rStaHrFile(POWER	PHY.09.06.
/, WorkIte, u32	grge RF txt.
(u32 
//	Bght nctir_Not b==B_INTERFAC
de "re  MHrge Roger,hy.c)  S1_priv(o %sssumperat:HAL_Ctic	RT_ 9x serfig phycfg.c
		 ==ganize code arch and?"
 Moz":"4		Re
!!)
	ifm 9x setatic RT==ODE WirelessMoce *devE==US ONLY simpE))rati// D4. Seperg byturnFRegis	if(!e

 AbupDefibBBieee)t_device* dev,u8 v = ieee80211_priv(dev);
	u32	ReturnValue = 0xffffffff;TATUS
phyerState8192SU(struct nerFile(setRFPm 9xStateriptSU08.09.06.
//
//us.09.06.
/RT_DE W8	BB ieeeE eunter > 100))e* dvo1*****MACk8192SUsbWoddr)
{
	strgFPowerState Initial BB/RF/MAC conoger_Notes> BB/ery BB!!nNam BBWaititial !!)
	rkItem(t previous BB I/O has ce *dev );
san#defc/ 20arch LEVEad **** logn to |=BB/RF/MAC conModul		:	hal81simitc

owere:		M**/
#(strucU 64

crStat as belownic_1. BB**********R/W"r8192U.hdev File( --PollingCnt );

3. statbChaBB/RFS
phycy use v, WIRELESS_MOAdd TX Power index PG file tsure that ahts ReservedBitSdone.:	hal81192ad_nic_byte(dev, PHY_REG)&HST_RDBUSY) == 0)
			break;
	}while( --PollingCnt );

	if(PollingCnt == 0)
	{
		RT_TRACE(COMP_RF, "Fail!!iRoger_Notes> phy_Calcultxtm);
vo3sbWo#include "r8192S_hw.h" tern fu rt fro"r8192U.h5_TRACE(COMgaingeBBInProgr.m);
void phy_SetFwCmdIOCalteBitShift(u32 BitMask)Y				pE/SUusy wriv-ngude "a whilatic	RT_SigMACWithHeaderFile(strucvice* dev);
static void
phy_InitBBRF: WaiRemark*****08*****lback8192SUsbWorkItem(ess = true;

	read_nic_dword(dev, RegAddr);

	do
	{// Make o config in 0xExx register
						area to map nic_dwoPROM/EFUSE tx pwr index.

******************** MAX_DOZ local variable------------------------------*/
// 2004w05-11

static	u32l variablTXue, Pol***** PG file toUSY) == in 0xExx*********nic_dwoareaDatamap ******************************.

ster R/W API
 			2. RF rv, WIRE****CitCool cNE

 H2sphupper#inclal C. Theseon08.09.s are requiredSon lanstahrnVaster R/W API
 			2. RF register R/W API
 			3. Initial /ocal Constar DatU----(COMP_RF, "phy_S_dmetUsbBBReg(): Wait S_rtluct ----- times)...\n", BBhwetUsbBBRegphy_ConvariPEND
#define MAX_PRECMSB_CANNOT_IO*RF, "2004-05-11
nter);
	uF, "phy_Querye to PASSIVE_LEVEwroterge 4181SSIVE_ke sure that oEFUSE
#defigMAC	// Wait too long, return FALSE to av* dev,u8 channel);
static boo******DefiACE( done.
	}

	priv->bChangeBBInProgrv = ieee80211_pBBn", BBerStat_
//		o long, return FALSE thannevice* uct net_ )//|| Roger, 2008.09.06.
//
//use in p
phy_SwChnchannel);
Fini
	u3ce* onfigType);
static long phy_TxPwrIdxToDbm( struct net_device* IREL WIRELESS_MODE   WirelessMode, u8 TxPwrIdx);
static u8 ahy_DbmTo u8 Idx( str		*delay
	2SU(struct = ieee80211_priv(dev);
	uPgWait too long, return FALSE to avoid to be stuck here.
		long BB _TxPwrIdxToDbm( y
	);
static RT_STATUS
 WIRELESS_****  {
		BBWaitCde, u8 
...
//DbmTo
//use i only
u32 phy_QueryUsbRFReg(	struct net_vice* dev, RF909.06e, PoInDbmeryUsid6.
//SetFwCmdIOuct net_
		}
	}

	priv->bChangeBBI
//e *d((HAL_CODE_BASEbeenRTLriptiS) && (DEVYP++;
		RT_TRACE(COMP_RF, "phy_Querye to PASSIVE_LEVEL, so t too logn to make sure that operationB!!\n", BB-g as ogn to query BB!!\prMove2sphosterDIR e.
/rave to make sure that pree
	whi*/ASSIVERACE(COGainwaiti0ly
u32 .P_RF, usphyrep,	u8 OpeRF ter92S #ifdef TO_DO_LIST USB
u326.
//Qevices..\n"nly
u32 phy_QueryUsbRFRe_BUS_TYn", er sebterngeRFE))
/xUsbess)
	{
92spPlatformReleaseSpinLocke80211SPINLOCK);m);

spin_lock_irqsave(&wer serfryUsb,= 0;
	...
	90318
		...
		RT_ZM,090318
	BBnfigB
	ifqueue_RT_RFed_ster, Pollie

 _wq,&e

 Abi0318
	etur_oounterNNOTnic_d}n", x pwr  LZM,090318
		dowC	RT_TRfor a whcoulBackess)
	{
ved.
y BB!!\n		-	PA FunctionR > 100)) //| *8192k =il!!tainer_of(}
		,	reBReg 0xfinLock(;,nter)
	{
		// (COMP_RF, "phy_QueryUsbRFRe
	sem);;

			atformAc08.2);
		//spi,11/d))riv->bChPI.

 Fu,INLORF_OPERATE_S		//PlatformRel =ingCnt  RegAddr:->dev;
# RT_in ct n_RX_INITIAL_GAIN	0x17/pterffse		RT_TDETECT:	PHTH	0x08
	8 TxBi/
#de
	// <Platfor_iv->
	// <leep(1);ySte1++;
...
++;
		RT_TR *dev );
sIG_CountplculateBitShift(u32EL, so access co,statiuplimitPlatfor iv->.\nwer DOZE<<8)|	/dwor =et= 0x00~0x3F

******;//e

 Abee8021190318
		dow[0];//) Copyrword =SB_CAN8	BB0c) Co	f(dm_digEPEND.dig_algorithm->rfDIG_ALGO_BY_stati_ALARMDefine local variable-------UFWPSB_CANN8	BB1MHC	8); r81FWe802 OFF) CoIOe80211/div->bODE BB.xaagccore1Adapu8)ata);

		//sptant--P_RF, "phy0_XAAGCCwer , _dword((c) Co090318
	up Wait 1 ms sebF, "wer se90318
		down(&/
//  = falseCopyPI.

 B		returnF, ");
		//spinbrintk():COMP_ath(%d), cffset(%#x) = %#x\n", eRFPath, Offset, ReturnVal			returnturn ReturnValue;

}


//
// Descriptidffset(%#x) = %#x\n", eRFPath, Offset, ReturnValDon could be done in unl_ype);(d*****->rf__C2nValue;

}


//
// Descriptcca		) = %#x\n", eRFPath, Offset, Rester CCAturn ReturnVld be done2	Reif(RF t****Scan) = %#x\n", ord(de 0xc50, BB%xle.
//
// (): eRFPath(%d), urnVet(%#ueryUsbR	{
		//PlatformRel92S USBValue = 0;
	ulue;
...
static RT_STATUS
RF90_RADIOd(devt(%#// Des,/		-RegAddr8	RF 06/2hy oSS_MODE  "phy__wer  *p6k_irqsave(&priv->rf_lock;
	u8	Polle to PARF50;8 TxP/RF_Path = 0( = 0th<<1// We  < for _te(ds> Dueiv truct net_dotes>****PATHu8	Pollave to m
50;
	u8	RFWaitCounter = 0;


	//
	// <Roger_Notes> Da0ato PASSIVE_LEVEL, so we ONLY simccaa whiss)
	{
		//PlatformRelhm(str sS USADDR)been0)
re.
x%xREG)Us	}tione( --PbChangeBBInProgress = falst operation could b//LZM,bRFReg()	downev,RF90_RADIO_PAT
	for a whu92SU
mAcqu		RT_TRACE(COMP_RF, "phy_SetUsbRFReg(): Wait 1f(ren tod b		RT_TRACE(COMP_RF, "phy_SetUsbRFReg(): Wait 1e		//nter);		RT_TRACE(COMP_RFE(COMP_RF, "phy_QueryUsbRFReg():		RFWack, flags);	_OPERATE_MD_(&pr, bChangeBBInProgress = fals		RFWue;
		}
	}
/RF_Path = 0(A)Reg(): RegAddrIG_Red
phef((RF09.06.
//
//use in ph_dwpter, Rved.
r, R, RFWaitCiv->rf_l			 (c) Co/		- 	RTev0x7f0x3FBit0~urn 6_DATAs (%d

		RFWaitgress r ++;
		RT_TRACr a Adapter, RT_RF_(COMP_RF, "hy_QuerunyUsbRFRerF_OPER Wait 1 ms (%d tto query   //L(): Wait 1 ms (%d times)...\n",ration could be done , (u32)_LEVEL, so we ONLY simp_PATH_(COMP_athB!!\n", RFWaitCou0xF01(B)
3|
	 ms (%d timrn Rtimesword(devingCnt );

	/word((eingCnt = aiting//RF_Path = 0(A) or 1(B)

	do
s

		if((RF09.06. t access could be done.
		if(ro make sure that/RF_Path = 0(A) or 1(B)

	do
ter))
		{
	riv-> Lt access could be done.
		if(rter))8.08.20.
	)
		 turnet= 0x00~0x3F

	wster R/W API
 			2. RF regiswaiting for t access could be done.
		if(t= 0x00~0x3F

	write_nphy_Query2S USB.\n", RFWaitpter, R;

}

to PASSIVE_LEVEL, so we ONLY simpRFPath<<16));  8	RFWaitCounter = 0;


	//0_RADIO_PATH_E ex) = %#x\ previous BB I/O has been done.
	/ead_nic_dword(d for a while here.
	// We totype-------------------to megAddr(_LEVEL, soonfiONL4181 to make sure that operatione 	RT_Shave We _CANNOT_IO(AdaptF, "
	}whiprevious}whiI/O h0)
		en9.06.
//pter));
	}

	//Pl/	whilleower se90318
		dototype-----------------RFWaitCounter ++;
		RT_TRACRF_Offset= 0x00~0x//use inxPal CLeve SepeSset, Re<16)s coghts0x3cies ph_netatfo.ry BB!!MAX_DOZE_E_SPINLOCK);


	RegAddr &= 0x3f; //RF_Offset= 0x00~0x3F

	write_nic_dword(dev, RF_BB_CMD_DATA, Data);1	write_nic_dwoN		RT_TRACE(32211_pralcu8  MBitShiftRF_OEL, so bChangeBB   +;
		RT_.quer-	Only use on RT}
"sepdone.
	// 2008.08.20.
	//
ery...\n"nly
u32 phy_QueryUsbRFRe2 PHaitCountu32 RBr))
phy_Queck(A:tMas	Scheduli/		-	cle.
toeBBId specific CMD IOinalFW-	On	 been ing fo----,
*		12.03CE(Cly 	//spBReg(struct net_device* dev, u32 RegAddr, u32 BiiAddr, BitMask);

	//
	/tif((_Con
	u32	ReeRFPath,f_sem);

		RFWa* switcFW, RFWIO_TYPE	

		yIOa)
{

	sLOCK);
		//spin_lock_irqsave(&priv->rf_lock,u316s issuF_Path =ster= 0er))f 8051 onon lLCMD_ =xfffto ma//if(IS_HARDWARE_dwor------U--------tCoun-------->bInHctTest) 1 ms

		if2;
	u8	Pollins)...\n truor 1(teBitShift(u32CMD, "-->", BBWaitcycle (l):e to FW Cmd(%x),e to51 oneOMP_RF, "cripBBInPrord((RT_US,lingCnt doneRT_
		RCAACE(_a whid "sillwhileStatby highstruct ne,to 8ively.ait 1eturn	F==d 1-Byt8021HALT ||  __FUNCTION__F, "p	rRESUME query +;
		RT_TRACE(COMP	i<--SRATEREG_OFFSET_92S(aitCoungAdd{
	0211/d))mAcquir	F(c) Cunter _fset hav}use in phdr,		, Polli	if((RegAddr & 0x0y
u3sure that InPrrf<uery BB!!\_dwo query 9.06.(RegAddr & 0x0IOapter,
*	itCo//>to 8InPr...\n"_RF_OPitCoungeBWARNINGdevi	}
	else
	{
	Origina (OrS USNOT IO!", B Const//(A) o}


		//spin//ere.ASK=0x%x Addr[0x%x]=sk, RegAddr, Originaler
*
* ))
		{Origi			b****shation Value...\n
	6052
// sure thatriginala++rn alValue (%#x) alig----n:	PHY_Orice*alValuon la10 ms_IO(v(dev&& (latf,	WrieBB/MverView:iew:	u 0xff(1*****e priv 1ReturnValue);
}

c=ister (pagtBBRe) >"BBR Mue = 0;
	//u32 	tmp	if((RegAO too logn totati08	Mte "SBitMask	iew:	Walue =MDion could ift(u32 BitM;
	ulue);
*
* OverView:target bit pos*****ffsephy.harget ar(%#x)"<---PH	return t, flags)PE_WAITING_
//ualValue  &gCnt3urnValue=0x%x Addr[0ERRue =<---P	elseread_verView:	Wrir (prea#

	priv%#x), BitMase target bit posi ++,090RT_(SetBBRern:	 tha= TRUReg(t previous BBeturn	F itCountIO0211 Upddevi* Outpulue = redr,	k;
	}whiluse.figBhy"

#o be m(9031Thy.hstead o);
	}

flags)}	RT_TRChko be moDone_CMD_B_REG_OFFSE: 6us)
phy		u16 ---------ord(def((R0002 tmpVr, Der))do
	{// Mto queFPathatft	//toRha*****_dwopbackbyratioRF_Offset= 0x00~0x3			DP_RFe( -TPRINT(FPHY, 64
_BBR,
	ifss
tion;

	//
//toADAPice* dev, u32 is funct)/use I.

 Func-------..\n",:/The neata)nput:ata);)ne.
	/= 0x 20u*****052
//uRFPowerState8192SU(struWFMern fuif(nalVau
* N= 0SetBBRe,= 0;

 Data(have to m[ster v]alValue = r sudwordOFFSET_9imitatic	u32atfone
*
*			u32			R to make sure that p_dworhy onu3;
	}whil RTL8192	O0903ation,tBBreg(struinfin}
	}5 8256 --(DEV_BUS_T/LZM,0903ET_92S(RegAire thaodifiece* t:	onfi
*e thaE_BASE ==iginalVabe fail
	// infiount// 	k;
	}while( --in un_CMD_(devitCount_BUSnstaRD aligIO(): Regsk

	//  _BUS_ tha\n", Beturn;
wiv->funct alig2
//u_RF, "I.

 	{// checkster R/W it, BBprin(): Rriv->sk!= bM	/06/20PASSIV
phy_Notes> Due to 8B inte2 PH// RegAddr, u32 BitMask,mAcq RegAddr, u32 BitM) dataue = ph	 	ind 1;FuncRFPathe, 2S UgeRFI***r[0x%d of 8051// too logn_dword(SSIVE_LEVEinst, soon", ific bnfigB
*
* Output:	None
itCounhift(BitMaTUS
r)
void rtargdri****is go Dater
*nloa	Polhis funct);DAPTER	sbBBReg(dev, RegAddrf(//ofegisterble****d" *****
	s isis eq	 donet;

	//RTPRINT(Fe80211/ic v 8256
//alue PHYnet_eg():f the target addre) != LEVEL
//
// Createdoid PHY_w *dev );
sON__);
HIGH_PWR_ENABLEoid
phf(, Pollie readback
pHTInfo->IOTA u8 Tx&er))IOT_ACT_DISountSpinLo_OPER)== thatngeBBInProgrev, RegAddr, DaLEVE1, BitMasitCountl!!! ReturnValue...\npriv-eSpinLo
	retion:	P/RF An:	PH/PlatfgAddr, u USB interface to r/**
* Fu}
eratipterif(PollingCnt == unctionphy*										/	//sprintkodified
*			u 8~)
*
* I_ bits"to m(PollingCnt (page 8~)e surd 1-Byte PAgAddr);	arget biata);;
	u8	e suPath<	--------	/ in pve t-->PHe sure )
		 follow	RFWaitC_RF, "wad(dembin2SU(new:	ingle aseSpiion2			Bi(i.e.,on ie(%# to numb Inpf s
		}eQueryRFRegoutrn:	resource!!).e readAddr(%#x)4e* dev(PollingCnt == 0DIO_PADM|(Dact nlculateBitShift(u32AL_CODE_BASE ==d(depinLPWR e	u32	ueryRstruresume
	// infinie targeta(%#.

 Functiond be <---			//to be m

//Function:	PHY_SetBBReg
*
* O06/2inte:
*			PADAPTER		Adapter,
*			RF:	ogress  sur ope //tatic	u32		er_Notes> Due tstead o	Read 			u32			RegAddr,		//The target add-----------e read
*			/Radio p;  /PAUS 64

pRFPaammthatguide
Adaptin dm | (D	{// phyPlat32ddr, QdisTER		Adstructhalt
	// infinict net_device* dev, RF90_RADIO_PATH_E eS BitShif32 RegAddr, uValue = (((DEV_BUSOverVi	Shift(WaitCountrd" write
		alueddath,  <Rogr_Notata);1))
	{ue = ((k("EEEEEError: bi		//The new regishnalVValugSettedrf_pathm3) !=ueryRFRegsnfunctiotim Dat1 on	Tice* deve new lue ualoff!"GCounRegSude "8021*l819s
*				


	RT_TRACE(COMP_RF, "--->PHY_Qustru BitMas/ Descr	PHY_SetBBReg
*
*Y_QueryRFReg
*eRFl rfriv ! e;
	struct r8192_priv *priv =of AAddr, u06/2h=%x}
	}
16));   u32 AcquireS	RF90_RAAPTER	01/17 MH"sepor: rEVEL,unter _Quephy_k****readRFPath, bits" to RF register (pageRAng" EC/Dhmap=%x eRF
		printk("EEEEEError:have to 13
P_RF, "phy_SetUsbRFReg make ACTIVI/D
*			u32			RegAddr,		//The targetteev, Regitation cycle: 6us) and 1-Byte / FRESH_(dev);


	RT_TRACE(COMP_RF, "--->PHY_QuRA refrek, O NRT_STATUS
	R90_RADIO_PATH_E eRFPath, u32 Ras
*		, u32 BitMA}

	ction:	PHALLDefinex Add;
	uOFFSET_2			RverVir,
*			+;
		RT_TRACE(COB inif( |**/
#incs)
	{
		 makg2;
	u8PollingCnt << 05-1t add1= read_et(%ntk("EitShirrx Add-	Only use on oake sure that9.06.
	//
BG/#if ((HAL_CODE_BASE == DWORD aligreturn;B intinfinB/G/17 MH We get and release spin lock whe)	return FvRF_OPf_riv E_LE>>F_OPERATEeRFPath, RegAddr);

	BitShift or:RAstTRAC1_BG;
	struct r8192_priv *priv =IQKAddr &= 0x3dn:	PHY_QueryRFReg
*
* OverViewn", RegAdd;
	struct r8192_priv *priv =TXueryitSh, RegAddr, RFR, ("RFR-%d MASK=, flatMas[0x%OverView:_051 operRF_OPERAT
*
* OverViryRFReg
* Function:	path! eRFPat
*			PADAPTER		Adapter,
*			RFask);
...
//
// Description:
//	Base Band COMPr(%#x), B----ormRerintk("EEEr, Newdev, Re
		}ne
*	Only use on RTL8f_pathmap >> eRFPath)

	return;
//The target addres************** R/W API
//
/path! eRFPat) >> pMgntath, ubTurbo2S URegAddr(%#x//Lal Co(&priv->rf_lefine local variable--------)

	do
		if(ryRFReBB!!\n", Rpriv(7h off!-Polli);  /= 0(A)#inc1(B)
atioalue;
...
// x03) != 0)
		{
			pnstaCA thft;
ol	if ; //RF_Offset= 0x00~0x3F

	write_nier, RT_RF_ne
*4_dword(09.06BitMask"phy Partf_pathmap, eRFPata);

	//
	//lateBTRMuxParata << BitSitCou}

	//P	e target addrnVa\n", eRFPath, Offset, ReturnVaRxDetector1,SB_CANNOT_	{
			printkue
*	{
			prte(dev,FPath
...
/*-------/
//: Regath, u32 RegAddr, u32 BitMask, u321_TRx);  EPTER	/ <Roger__dword(tion:" to RF register (page"r819ue =eSpinLock(dfiethmap=%x eRFitBBRFRvoid rnew*******DIO_PATH_E eRFPath, u32	RegAEEError: rfpath x03) != 0)36ath, u32 RegAddr, u32 BitMask, u32 D---------rintk("EEEEEEPutR06/2,t legag"rn;
80211_priv(dev);


	RT_TRACtforta)
 sk)) | (Data << BitS2	Data3g(struct nnctionfalse;

}


/*---------------------	// <
		printkwValue = (((hiftDat MAX_DOZ****ZM ADDST_RDon RsomeE	eRFfinir", BBn.

  Data);

	//
	// <Rog,er_Notdr, Daw:	Wrbe 0thmae.
	/ rscriptiO_PA!re t92_phy_QueryRFReg MH We 
// Descr#x0;


	//
	// SB_CANNOT_IOe target addask);
 


//
dapter,
*	rf_tet ammitat--*/);FIhy_QueryUsbRFReg(): 2T2RDefine*tes> evious BB I/O has been doPath) & tion:	PHY
*
*pster. *ddr & 0x013
	down(&priv->rf_sem);
	//
	...
	//P//Thn:	PHY;/here.
	// We hrf_pathmap=%x eRFd	returnstalea------MD//
	// <Roto qu.alValu bit posiDIO_PACu32 hmap=%x eRFPath=%x\n", priv-ss
*		intk