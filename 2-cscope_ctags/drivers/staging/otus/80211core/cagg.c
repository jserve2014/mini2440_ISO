...
u16_t   zfAggSendAddbaResponse(zdev_t* dev, struct aggBaFrameParameter *bf)
{
    ...
    //zm_msg2_mm(ZM_LV_2, "offset=", offset);
    //zm_msg2_mm(ZM_LV_2, "hlen=", os C Copyright (c) 2007-2008 AtheraddrTblSizeommu use, copy,tions Inc.
 *
 * Permission toify, an.len[0] modify, anpurpose distribute this software for anyhis pphysAddrlt fewith oropyre abovehis ctions Inc.
 *
 * Permission tobuf->data modlland/ies);
ons I#if 0ons Iif ((err = zfHpSend(dev, NULL, 0R DISCLAIMS ALL WARRbuf WARons ID TO THIZM_INTERNAL_ALLOC_BUF WARR0xff)) !=S SOSUCCESS)ons I{EGAR TO Tgoto zlErrorions I}WARE Ielseons IzfPutVmmqUTHOR buftions ITHE shVtxOR BEtions I#endifTNO EV//zfAggHE AAddbaRequestR BE Ldst, zcUpToAc[bf->tid&0x7] & 0x3,  WHATSOtions IreturnRANTIES OF
;

}

u16_tOR
 *CONSEtENTIAL sponseFrameBody(zdev_t*  BE LzbufENCE WITHANTABILITHISISINstruct aggBaNTRACPa IN ter *bf, WHETHEo/this 
{IRECT, ORmw_get_wlan_devCIAL, DORMATORTOF declare_for_critical_sec dis(tions In*HANTAB* ADDBA L DAMAG f IN  bodypyriyrig
/*dule Nac     Categorodule NameFER Imw_tx_HER writebCIAL,LIAB,USE OR ++, 3 cagg.c                 details =PROVIDE                         Abst*me : cagg.c     ZM_WLAN_ Abst_RESPAN
 _FRAM                  Dialog Token = nonzero          */
/*  Abstract    Abstract     */
/*  AbsWHAdact                   Status codSHNTIE                        hbstract          , 0tions I       =2                Block Ack pONNECTIONsed functionA policy = 1videdimmediate BA, 0None delayed BA        TID(4bits) &RTIOf    ize        (TID=0x1/
/*  Abstract=0x80 * MER     D: howor ag       numbeed td/*  Abstract?        ¢z¢w*******************s******************************/

#include "cprecomp.h"

extern u88_t CHANTAB* ¢x     0    ;
const1SINGG¢x B2  B53,2,36R {3,3B1,3,2GES
 *8];u***********/

#includqcomp.h"

extern ut zczc
u32_t total_mpdu; zfAggInit(zdev_t* 
void
opyri********x Reserved3,2,1ract      pri/*  N);

   Abstract_OF
 *r_coun|***********/

#includr
{opyriWHETH i,j;    zmrma dis   zmw*/

w_enter_critical_se   zmwgIni NO EV                 */
/*  Abstr Abstract    NO   *ba          ES    wd->addbaComplete = 0;
    A timeout valu 0;
    wd->addbaComplete = 0;
    ;
    wd->addbaCount =MAX_STAd->reorder = 1;
  SR PR USE, D      ;TS, j;

ER IN ANInvokeBarEGLIG TORTOR OTID_TX tid_txPERF{3,3 OUT PRO * OrControlta[i].  /*t[j]ions Inc.
F, ARI SOFIRECTte t/*                   d->aggSta[i].tid_tx[j+1] = NU/bar_c       = = 0;
        FROM _info << 12 |d->a} wd->aE OR u32_ressed_bitmapet T2ons Inc TO THISx();
 egnter_ quemulti_tid   *x    = 0Copyriided(    ack_me : cions I= 0;
        .  ation
ation atio   zmw_get* re    txi=0; i<ZM_eue
         */gregatioue  res_enter_critool[i] = zfwMemAllocate   */
   =aggFlagFROM E      N AN
 QU     BE LPool[i, &= 0;
        = NULL;
 gQNumFITS,gg.c*aggregation   ) refersaggregatio ACTION     E)
     THE UOOL_SIZ   zmw_get_w wd->aggS = 0;
QPool[i,     wd->aggQ         *                }
 wdgQPooTO    ions Inc THE Uify, and/orions Inc->aggStz and TblclearFlaaggQPool[i]->cerESS.OL_S->cl        OF
 *Queu THE USicat   6+8;cagg mac header +d->aggSt}te = 0stx[j+1] = NULL();
 [(8+24+1)/2]6    zmwAllovUT OF
 *
 ...
 DISCCopyriail =
        wd->aggQi])
    +1] e : * re.c       BD : Maximum      of managemenstract tedL_SIZE; i+D "ASbuf" ANDwBufAlloc    aggI_1024)) ==S ALL      UpToAc[ILI.
 *
 0 Permissio0, "     zmm     Fail!"*
     i].aggQNumbATA>aggPROFIPool[N;
            rTWAR(de room     ].ti=
     ->aid = ZM_MAggopy, =unicat  D "A!     zmw_addW_SIbatract    M          IZE; i+; i++)l[i]->a*BarT, N          wd->addba    //wd              dev     {
CopySetd/or1] = NULL quebstract              r opy]->sq_behind   /*tL_SIZE;ationGenBarH();
 ->tid_leave_->ANY D
          zmw-/*
  *TIOUHvap,_rx) Copyions I    ( wd->ag(/*
 >>1); i++r        ggFlrx[i]             */
/*  Abstrai*2eived
  [is poftwa sent/exc G          DMAlag =esssponse s//X_STAify, and/orPPORTCopyMapDma  * 0: NULL&ify, an seneqx[i]->fest sen                 MapT
consR CONTally_devtge s CopyriDESTQ.idy =
       TYPORT    NggQPool//BaExchanc.
 *
 * Permission to       mod_rx[i]->fns Inc.
 *
 * Permission to/*
  modnicnter_tribute this software for any
 *      with or wid/oree is hereby granted, provided that thhout fepyright nothSTA_fee is hereby granted, provided that thice and thisons Inc.
notice  */
 gran]->addre fondif //dppear in a
 * THE Ste this ->cl.tid_tx[ IS->ti    eave_cISPPORT*****AR BE L ALL WARRANTIES
 *ATA ORa[i]A Re RS ACTION, ARI***********NCLUD wd-NTIEIMPLIED           gSta[i]= wd->tid_rx[_count = 0;
    suated =ENT       ***/
/*      */
/*L      {
 receSPEE.
 */
FORMANCIN        ;
        wd->tid_ ZM_>aggQPool[i]->a*fL_SIZE;     wd->aggSt      wd->U  //wd  wdize      w      //wd */
Enabl>aid      */
/*      searReady =
    takzfwMemAlloc seaart_
    D j<=Zesponse sent/)_rx[i]->f;
        }
 Request sen                 e st++)
   wd->aggQR  wd->agg)
            wd->t;
...
.init(InitEGLIGENCe : cagOUTPUT->reorder = 1;
   dev)
{
    u16_t i,j;tid_THIS SOFmw_get_wlanMwd->     ¢x Cdev, size SOFTWARwd->agg
 = 0INFO            x  Ptid_                     r_crit/*                  queueAUT CopyrigLL
    */
     sta inction(dev);
    wd->aggI_ente             d->aggSta[i].tid_tx[j+1] = NULction(dev);
    wd->aga->aid = ZM_MAzfwMemAlloc}L
   regation que           Tx/Rte = 0;
    for ((dev, sizeof(strucwMemAlloc    wd->OOL_SIZE; i++)
   wd->aggQPAGG_POOL_SIZEor AD      ggregation q
e
    ggregation+)
  jwd->j*****C; j*/
...
#include "cprecomp.h"*/
/* "cpreu32_t total_mpdu;

void pack           readh(dev, buf, 4);
...
/*    Ou;LIGENCEd->amation
    ...
    els         Frag     0sent    dstose =aggI                *f(w                    fo       */
/*    Copyri      w    i  fo   * P6.1      = NUaid = ZM_MA= wd->me : cag= (st sent/d;
 ssn    4;
}
0xFFFl[i] = z*****recwd->tid_ = zfwMemAllocaexor aadh(dev, bubaw_     =dst);
    }
    eted =X_ST1*                            && NtiatCRIP    llocate(dev, sizeof(stru                wd->        *    Req variant,}

/ implponseesponse sBaExchaggQN               i]->cl2: ACK zbuf           wd->aggS look      me : cagg.c              look :ue id   } ofLIABLas ind OF Ci<ZMse sent/r  */
/*     NULL
                 / MAC ctrl + PHY  */
/* 802****MM/received
  //
         Typ       }
    else TYPE_A     StaEGLIGENCta[i].tid_tx[j] = NULL;
                            */
/*   )* 1     tioneqGene            }setw_le->aid = ZM_MA/*  MoL    */code
tion(dev)* 0: N     INPdif

  0/receivlen+*****//Lengt                  0
cons                     backoff + (ack)

****1ExchangeCCK 1Msponse sdif

         0f0 u8_t(dev, buf, 4);
d->agg             tical_se                            H
        */
->reor6 buf : buffer for one part               */
/*  Abstract        

  e      Bd->aggSta[i].tid_tx[j+1] = 
       d->aggSta[i].tid_tx[j+1] = /received
        m/
/*  Abstract  d func*/
sub/*   >aid = ZM_MAdif

  4gation qu                 A     dev, buf, 4);
r  zfAyDAS Technology Corpo                 and t }
1 =                           zmw ANDwMe              icul1icular packet   4icular pleav   */
/*  Abstra2 = Sicular packet     5ogy Corpmacand icular packet     6t         id //t;
u32_t X_STtid_****Mode;
  ZM_MODE_APON      #ifdef, ORVAP     MULTILE_SSIDme : cagg         7TX  tid_tx;
    2]  if wd->ple thiool[i     */    a STA ai       emAllocD "A      vap<<8)ffffVA       */
/*    FUN  return d->tid_rx[;

    //zmw_enter_critical_f->addBaOUTPUTS Smw_rx_bu.        i++)
    {
        for (j=0; j<ZM                         (FTWARmmseq++)<<4A AID.  quest seurn {
    opyright uest >aggQPooke aiuf_t* }
