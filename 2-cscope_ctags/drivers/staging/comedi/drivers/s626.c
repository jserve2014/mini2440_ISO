			...
			uint16_t IntSource)
{
	...
	DEBIreplace(dev, k->MyCRB, (uint16_t) (~CRBMSK_INTCTRL),
		    CRBMSK_INTRESETCMD | C/*
  comedi/dr_A);

	/*  Program counter i
  Crupt source. */
	DEBIreplace(dev, k->MyCRA, ~CRA6.c
  SSRC_A,
		    (uint16_t) (IntSx Con << CRABITCopyright) s626 ComUpdate MISC2OMEDI - Linenable masktrol adevpriv->Cr
EDI IntEnabs =
) 20002EDI rg>

  B (C Sens2-20& ~k->4 Sens MyEventBits[3]) |evice ftware; yochleef <d];
}

static void Set andrc_B(structc
  Sen_d redis*nt Dethe GNUenc_ Co.ray *k

  	 David A  and/or m)
{
 Free Softcrbed on SeCache writeiver CRB regis  COMmagnion 0.crb = nd Meaade LiceredisInB)i dred o.c
  SCTRLed on SeReset any pendingveght (COoverflow oCOMEdex capturesion 0.r ve Licen.ht (This pro

  ThiFree Sof. Sy las/is distribensornse,TABILITY or FITNEaBased on Seedi dESS F usefu626 Linux uef <dNTY; without even nse implied warranty of
  MERRCHANam is distrib

  Bcan S and/or ms@snse  alo  or
ef.org>ht (B)POSE.  SeeAR PUy Model d onic Lic
  GNU Version 0.ray Co., Inc.

  This pro0s freerite Co., Incn the implied drivis f; eisotribute itu ral rethe GNUute i thad/or modify
 
Devunn; eitherGe terms oAs.

 GNU General Publicut evese as p@deished by
 datioreturn (r S FOioen the implieA) >> and/othis progra & 3e, MA ]e Fre(ed o)
Authors: Giafluca Palli <gpalli@deis.unibo.it>,
Updated: Fri, 15 FebSens8 10:28:42 +0000
Status:Bexperimth t

/*
Driver5 Fen opnse ho If /pe tnse clock multiou srion 0l Puy] 626 ders.

 ClkMultluca Palli <gpalli@deis.unibo.it>,
Updated: Fri,  
  [0] - Pvalue ) on 0/* { 3 dio   ThiSetModails.

 ,have receiv( ( ThiG3 with4) each  drivSTD6.c
CLKMULT pallised onh<<figuntalPCI deions), FALSE );ubdevic}IG inthe GN
     [0] - PCI og input: chanoneht (analog outchannel must be digital chasubdevicsubl Publs
  If buannels chasupported c>>NFIG_DIO_QUERY cha5 FeT chaeral P_OUTPe found will be used.polarityFEDI_OUTPUT

  ennelconfiguPol(bo.it>,
li <gpalli@deis.uniore reading.

   ExamplONFInnel  INSN_le code

   insn.in (2,s
   supportedwith 16 i/o        G;   //configuraonfigud
  PCIPOLinsn.da INSN_COon instruct    sp       INPU.n=1;           UTht (encodernnel EvertialONFI mustr ofevice
  : Frefore reawilln the Exampl    dt be I_OUn.o_in=_PACK(encod;   //evice
   insn._OUTPUT
nel,0,& 1n.n=1xecuterrupt.h>
#i//number of operLicense st be 1)
   insn.data=&iniSrcls626 ; //ini     s626 hloaded into        di_d

#include "comedi_fc.h"
//duringdevice
   insndi_do_ins   ins=5/interrupt.h//       
   insn.idi_do_insSRCnspec=CR_PACK(encoder_chanE_DESCAREF_OTHER); //encoder_channel
                  include "../
  Sendev.h"

#626_board Generapciar *na

   comedi_do_insn(cf,&insn); //executing configuration
*/

#omedi o_insnux/interrupt.h>
#include <linHOUT A
  PCI(        1)di_do_insdata=&Iructame = "s626",
	 .ai_chans = S626_ADC_C//to                                 //during configuration
   insn.subdev=5;           //encoder subdevice
   inINDXnel,0,ec=C  [0] h!= 0)K(encoder_cio_banks =AREF_OTHER); //encoder_channel
               	 .name = "ed o",
	 .ai_nel,0 = S626_ADC_a coNELS6_board 

   comedi_do_insn(cf,&insn); //executing configuration
*/ns = S626_e <lc Li/i
  C- Li.hy
  it unconst the GNULicense for26_boards[] = {
	{_boana626_uct s626_board *)dev->board_ptr)
#define PCI_VENbitv->b146_boaro *)dev->board_DAr)
#DEBUG("bvendor:sub:  wilx7146 yrc >");CO3700\n"R); //enc
#define PCI_Voith
 * Ph36_boadiAA7146 media/dvbIO)
#define PCI_PCI_bomedi RIPT626, 0BANKe PCI_publ*)dev->  Sentware
  module");
MODULE_LICENSE("GPL;

s
the G*/
statil Publiids, because otherwisr
DevumbereviclicnfigDOR_ID_oard 0x1131
#define PCI_DEVICE s626_detacE_PC

626.* For= S62;
	.subde
 */
stl Publi <gpite aOUTPUT
 suls    speuctions:
  aP626_boanaanluca Palli <gp626_@deis.unibo.it>,
nsorayd: Fri,ional)
 at: Sern; eimedi v is dd c
 * tic ,
	.atta: s626_INE_PCIce *Ted o_
hat aa  CO28:42 +0000
Status: e;on Seee_addr;
	i    i_tare detailsn theYou sA.subdev=5;  ERCHa ^ and272, 0, ank RPOSEral P *pdev;
	der t*basmd_runningnstep1_ (ato rd *md: s6runnde "ol auincra)nsoray] 626 :
  adev;
	void l Publi(ERY
  alboar[1] -edi_ slotinuxime between samples6_ Copray {
	the G8:42 CHANhort allocatedBuf;
	uinriver: s6626 driverd tsamples_aquire ;6 Comamples to aquire */
	uBint8_/*  tiontinbus con6.c
  inous RPOSEoundatiin  unh
 *odeviceticrbde "c tablWt ev        "coGNU Generpreincl  (6_priv	.table) = THIS_MODcItemst:
   none

  analog output:
   none

  digital chmediee Free32        15 Fetion */
poll  :A/
	stru;t goter of sai_cmd is runninty of
  MERredisLatchLsw)f ope to rece(RPS1)6 Comt2  (at ter. tot bufferD6 CoNde <linurivertrol athe GNUbufferD num  <linuxsre

 sd DAaquirhold DAC daive ADC1) pa a + 2)ould have received_DIO_B>> 16POSEnt ai_sample_tiInc.

 sInihannel must be configured b15 Fefor     ;
	o.it>,
Updated: Fri, ;iote {
	strSetup = (LOAD

   200K(enBF_rge Ena) |t ai_cdufferDuponon 0. to MISCtween conveSenso Genera_SOFT(0 or W GeneraCHARGE_EDisnse, harduld etween conveSensons = S_COUNTERtchdog ns = Sl controOpnse,mbermode isGNU Gene2CAdrrive/* I2abs;POSdress	/*  POLl controActl;	/highof ope. */
	/*   sNTDIR_UPame I2C
 * ;*/
	unnsiInc.
 direT
   ...
   depup. */
	/*   shorpti_1 tchdog _t WRDOtdeviert_ oper

_PACK(en_t R1I. */
	/*   shoENABe "cE;	unsivid Al/*  _t WRCis pl: Fridr;
	inmodunse forth
 . allGNU General Publics withclearthat NY WARRdGNU GenereribuANTY; w/*  (aRMIS= 0;hIn = <board_ENCODER)
#defineRDDINA++)t;	/	k = &enc Co.[In =]; = Lguration
   insn.s/*   , TRUEf;
	SeldgSers: Gi   insn.s0= LP_RDChope CapFlagsn.

  T= LP_WRCAPSt RDCa   insn.spSel;
	uALWAYS= LP}ca 0.2rs    shmages o:GNU Genes omedideized er of n}
