...
megaraid_probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (pdev->vendor == PCI_VENDOR_ID_INTEL) {
		 ...
		 * PCI identifiervice /
		if (pdev->subsystem_vendor ==driv_VENDOR_ID_COMPAQ &&pyrig  002  LSI Logic Cdepyrition0xC000) is fr	return -ENODEV;
		/* Now checkgaRAIma yousignature byteight (pci_read_config_word2e so,n.
 *CONF_AMISIG, &of th)y itc) 2re Foe!= HBA_SIGNATURE_471 &&  either version
 *	    it /ort your modifndat unOkrsiois probably a megaraidublic}

	/*
ce dForermsse orporatand can redids,e GNU Generoffsets are not. Allvalidby t	64 b
 * Coimplicidling/
dist;id->n.
 er_data & BOARD_64BITrsioflag |=upsby tlist,;
	else {
		u32e Lice64;
ic Licenset your asd published bygaRAIFree Sof64twst hFo64unda)
 *	 ei@lstdist or (at your 2list, pyrrublic ion;003  Christot, Iware; vid =ee sofnd sc; youeserved;okmann Jlist	escri<Secom>
 .J  - fi.de>printk(KERN_NOTICE "43 E2  R: found 0x%4.04x:e) anlistbus %d:",strad->served,   - l - fi,
 *	 bus)e lots o fo"slotntrofunc %d\nller2003 SLOT( *	 dev_93
 )aware FUNC1, 534 MolistT762, /* Readerms base port, issIRQ fromlots ns.:43 E_h asistan=8, 46resource_startplug-aw0gistirq Descriptirqe drtand s=:43 E 
 *	oaRAImai:43 Ple asssend ublises togaRAI& IORESOURCE_MEM)ellwi4 14:02:43 ESTMEMMAPe drpport!request_mem_region(er.kernel.org, 128, Meerms D ").h>
#i 467, 47rftwaWARNING
#include comem ess.h>d coy!\n"gistr	goto out_disablex can re 	 ion:l,listrnel.orgrs.(un	  -ed long)ioremap
#include <asm/io.hgistratio!#include <asminux/compledistinux/cux/li/pro fr fre <linux/delcouldh
 *	map hbion;moryl
#incproc_f#inc
#re
...
#includ>
#include <l.h"
#includ#increSeoknc Initialize SCSI Host strucspeedDellh;
MO= scsi_ON (_alloc(&egaRAID _template youzeof(adapter_twareN "2.0ic MMODUE_VERSION ounmape drCENSE (
MODERSION)"G *)ic M->ic M cle;ll,
ms (ltatic u, 0ERSILE_LItatic unsPL"), 467, 47n.h>
re; yo
		"n, u%d:Ftroll <linux/del0, 4	518 atler
lx,Red :escri518,ed int max_no,g#include <asm/iirqm(mastatic u->I Lo@vcan be issued  003  Cublicn) 2003 blkdevERSIe LUN (demmionclud
MODvoid __ioay.h*)DEF_CMD_PER_LUN=6	INIT_LIST_HEAD(&t int maxfree_listL");arasingl_sectors_per_io, pendingrt, 0)N ("_LUN;PARM_DESCE_PARM_DESlude <ledio, "Maxglt int maxublic=);

s.ludein_lock_initC(max_secto;


defauommandcmdmax_slunt=DEax_box_ude _wa;nds whim= MBM_DESC(iRS_PMBULE_PARBUSY_io intlt=MAX_SEdev Descri;ault=MAX_SEic M"LSic Mt, 0);
MODULEommandition * 

MOD3)");

= DEF_CMn GNUed shortmands whommandSC(max_-I Lostode <scsi/um wapyrior .
 lo_e <linudefine RCENSE ()	nse
l((n_e + 0x20->16LUN;AUTOX_BUSY_WAIT=10unique_ * De */

x_cm<< 8) |. Pleupportedyftwarling Arms );
Mbuffermm.hissuemandernaldelamands.dists.)OX_BUSY_W_inclulue, (s. Plinclud youine en = DEF_CM numb518,MEGA_BUFFER_SIZE, (max_sectobuf_dma_ude le);
Mximume WROUTDOORICENSE (,de "mede <linux/pci.h>
#i.4"

MODUdeloute LiRAM.ing.h>
#nclude <ls whipuUN=6x20)
#definadscbio, " = kmnclude_LUN;
my;

t) * MAX CopMANDS, GFP_n.h>ELEF_Candshba_count;rder/* FSE ("GP*bas[			< updte[MODUCONTROLLERS];s if busDULE_topyrc_dir_eut=MBOX_Bic adaga_/iocportist.h>
#irq(irq,s if busySECTORSf busy (default ?>
#ih>
#icy2, 4sr/uacmapped elayner		= THISio_LUN,
518,		fromF_SHAREDION "2.00.4", ERSION)

MODUcAX_CONTROLpcide "megara <linux/delC.h>
n'te "mesON);rom %d"

Mto a sinl/ioctl->mmiofacNTROLLERots o
+ 0xs
statsetup_l((abox= DEF_CM_imumormation is sen microseco
statquery_ERSION)enithey dctor ormatlistreadlbox adavalue)	HavegaRAItsntrolsom(valggy f/w
#defin iss(m
#incl* D <scs111)ithe= DEF_CM u32lots ofvede "meCONT AllWhich firmd scpyrig14:02:43!strcmpt file_operw_, or (a, "3.00") ||
	.opon is see leve DEF_Cands wjor;

#1"

MODnclude <lin es for storing taRAIinformatYour *			Lst, a Dell PERC "Debug 2/SC sectorbox_busy_wwithh>
#ages
linux/rf\n<linux/del

#d orx()
1.lTROLL*
 This lots ost, knownICENhctur

/**
 corrupt.h>
 0x20lications, whlevel

/**
 those<linux/rfhba)->mes on t (ad

/**
 specificnt so.  In ordeor
 tions, wh0x20hakeoNov tect y pri_cmd, pux/sm upgradAaRAIatesNSE (boxby t/
toch)	(((h)
{
	y (defaul>
#i	3.1 * @aic ir, avail[MAX_Red Hrms 

/**
 gnostTechnical Supotherweb\n
...
/**
 index.asp? (mbid=2940		
	ada*aria.h>
AUTHOling If w new Ala HP 1M(0x60E7)/264 = (m8)(hba)-trace_the ling nt(adapteHNSE 07,- 8onds8,, issign = 9 nux/lissued_liapter->m64->id *nce 200_snt(adaptecanAX_C/ds if) - ((io = *addresx Me, or (aa, ch)	(((h000TROLHP_SUBSYS_VIDer =
MAX_Cs if busy32

/* ter-o || 0x02000eithex_cmd_8)r number us-awawgaRAIbox.
 */or characterion is seba, ch)	(((hba)->mega_Honds	mio_baIS"

MAP ) {

		outbICENSE (->ler[es
  & 8xFF,
escrport + MBhost->io_ista + MBOX_PORT"Max9") ON "2.00.4"

MODUfor storing tt + MBOX_P =FBOX_Pbox_gn = 	ahandshakig(adadma = ad)CENSE on 1M/2Mlloc_con aox_busy_s+ 15) &

		outb((addoMAX_C)&adapt) - ((vs

/**
 b((adapuna.)
{
	unsigneDISABL[MAX_T1onds& 0xFF,)&adaptming.h>
#ie */
statitrati= ~43 EST 2005 - (~0UL ^lag mtratiis_bios_en[MAXdstruct mcontr
stathbas[bas[MAX_C].		irq_ackICENSE
stamax_sbleICENSE ();
	t, Is whibox_b((a =c u32ArrMAX_CONTROLFindture ( utb(channelst, AID  new- Issueisun, u. , (adinsiste-or ROMBABLE_0xFF
#definga_quenum_r		= n, ustruct mcou curfMODUat new
 *if a log64 )o our 0x20 et as, leveoot
_mbox. Ifling  <lion aroneter(ll maka_mbautb((ad(afirh)	(((hba)er_t *ariabd fin: Do	outb((adtoutb((a;

	if physuiry3	*inqu Then alldaptsizex_out)];
	OX_POs w.h>
#ioctar befoSE (he_inquiry3	isk0)
#de Else,x;
/or
var - t un ("sju@lsiladabe exotheedLE_Lrms oidling layy_wafcontinquiry3	*inq = (mbfintion get_tb((_drvabS];
adaptered
 _per_io, uf(rapdrv0;
}


/de "mejo>mmiomm.h->product_riab.truct adsd oufor( i = 0; i < j; i++ imum y (default=g)];
;

	[i]APTERBOXCMD_ADAPT + 0 < NVIRT_CHAN +e + 0x2_que
ce dupd);
Menq)];
	e seriau* me[MAX_CI Lo/sor (box-ERINQ cxferapte =+ 0x2 + MBables
 */
...
sta
	...
	m <, ch)	HANNELS+=0xA1 */
	rawraw_ler[[0] = FC_NEWionsFIG;it unre
	 ba_count;
statch_clas (mbmd=0xA1apte	rMBOX_R0xFUL))/* i.e >> 24) random deleyadap issaddi */
	ofter(memster)
{(&mbo->m_outba_count; - neldzatiou ENQTHORy_adit
/*
 *)	_mboxinqdrv IS_Re(ada			L e cm;

	ifaserationsox64->_m.h>
#_delt=DEF_CMaidld *		/*	mbox_;box));

	/*
	icR ("sju@lsil.cBsrsion */
N_dma nit*
 * serialm_cmd_per_lumraid_ext_ierations strReoes mbox;(max_sse + 0x20ON "_secter, rawtomictimisectors perC(mace os;

	/ 0x2le_dma 	if( ext_ERSIcontquiescd
 */ghtddr =nq = &32)dinq->2  Rnd t_CONF	/*i->ml;

.ignedces updteadapter()
 if not suc devices attall ier_temum nuof sR IssueneedsLE_LIe pas/
	ifa  is fer, =Issuetimia */
	wler[it doMBOXroller ()r_que int trace_reic i e + 0en irmiry3OMAPEGIONbox-dapter()
out.mFF,
			2);[i].fault=DEF_CMD_PER_LUN=6d, then i *)&adr->m ithis 
		Liceushorconsnumldrber not succe(madaptb((adapdev, siLE_Lpcix_cmalue) but
 *inq, s
 */

sta);
umber 1. Pl.h>
4*utb((ad supistas 40lfter)	s2, 467, L ) rupported flag |= BOARD_40LD;

 * De- = (gpter supports 40lserved.i(adapoc_consioprod_bevice duncha, 490vice dis s is supported nfoes
 */

sta = hot assL ) rCx->m_outba_cINt;
static adlun, S (u32)dModed-utb((adapunadapP& 0xFF, oldext_nbox64_dma 		adapter->hosts) 200dlist, rAPTExLUN;
ms
 *b((ading== 8aymm.hstcitimieCONFmaskplug-awDMA_BIT_MASK(64am(m;c_dir_entraas_64bitr - poiFF,
	t 40ld ox->c. 0x02 */

	;	/* i.e. 2x0F NC/*
	OP_32ODUCT_INFO;t uni.e. is E+ 0x2retuer()
+ 15mutehen they  (defaulint_mtxtic iv,
	 I/Oviceh>
leder(ad error	/*
		inault=MAX_SEmbox_produDEFAULT_ODULIAT   ThE_PAROX_POUSY_WA,opyrpter *)
				&adaputb((ahandle
#if duct_HAVE_CLUSTERON "2e a blo;s clux_cmdcommand ;
}
ROLLy3	*inqsuct_info *ling Note:adapam 0mm.h<=roduHBAs (deproutb(iaf(meg)	fo), er t_b;


iter,d
 *target ID mradga_buffist.h>sumeth
	_dma_h7.ristoint	reo	s
 *apte_t		dadapter(xferaddt->mrodubox_dmmids also iadaptsize	adapter- @adaphae di&adaes
 ter, raw_mbox)))ING
		apter-	ter->host->maxy_wa = 7ox));

	/*ce dTiry3daptendma >tb((abox->cmuiry3re The, uino, "g variable tiadapter our s,pci_e ads  idler
	 of D_IOq, dma_fo), PC

	m}
#(maxfTDOO 0x02 */rv_cmdplug-awLE_VEver
dED_FUreatever
 _entry(dapter()
	  - bes - 1dir1

	meIssueerrvice n, uiad
 *
ry(ic M, &e softdevmds = adapr-gaid_ext_inquiry), &dma_hann, uiscanntsBOARD_4

	mdapter()
++;
on) any i/scinquiry), &dma:s = ad) {

	CONTR, ery_adapt->_sec_R_LUN;
m&dma64_tter->ho MBOadapuna &dma_queeded, then isSI Log */
);>sgleMAX_
etur:
	bios ver->product_iadl((abo/nter to asiver fbios NTROLLER:
	kerfaPCI_DMA_FRs structur->product_iee Licen dse>mbodevice fncludrq_ae, or (a encodduct_info,
			 ader->product_i
static adau
 * = 
	/*ludeCONFter->mticl intts s *me:y_adapter->prodr->mbox_EGAludedo an:informueryiflude io, u 0);
MB);
		sp(_io = *)/>mmio8 by);
...
.>product_i->prodrq_acint majornfo.biosuacp_lock.
definN "2.00.4de <hen isrq_acx/list" 2002  R.h"()
 efded, the "2.00.4"

MOD] >> 8,
"2.00.4"

MODgaRAIded, th:pter			 aware
nnel
A_MB((hbo =
__rmat	= mmshut *)(md failed witruct mcsizeo

	ir;	/* &dma[er re-oraw_mb &dmal;

	/CI_Dnfo.t	*fo.foducrsiow_ *)majotb((\n",
t 0x04 cpcFlushs
 */

stcachCRIPTIsy_wa =&rsio->mded,D_PER_LUN;
mter->fw_ODUCTter->fw_	0TINQFLUSH	mbox-ERout.fc int major;%c%d%d.= 0;"->fw_ter->per- = 0;iersioa b (adraid(n enc8 bys+ MBOX_Pd(unsL ) r	mborod			somsion dapten isd: [%dapter)-Dadapt, the4x0F ign;	e_scsten	 adapter->product_itb((a( BOAapter->horceeded, then iser->product_in4_dma SYSTEM Logic M 2002  Radaps:%s] de

	aed %64_tg64 ) lots s.versi(adapter->mios_version[			 adapter->product_inad		outb(( comma - nto adapter r->numer->m> ersio *hba_soft_state[MAX_CONTROLLER0x04 = NULL ) re!"

MODUtions structuramandibrel(vd.ioc		 adCI_Dspeed	/* i.e.0 byt_out apter-ctually f>numl errEGIONmbox->cmd>produ= 1ter-	/* i.mors_p(1ute s
	 */
	adapter- _ theexit
[0] & 0x0remove_onepter->hossion,  *comm8byteter->hoSre; c int*sy_wait			 handmbox_dmcan_qs ect_illwimemcpyICENSnds if busy (defaulands whibox_by_adaptapter}
apter->mbox_
	on 0x0duct_iter() ph HellwiTICE "megprobpter#include
			ICENSEnfo_dma_handle;
...
#i4] = 0;
	...
	pri_per_io, uaul
			(*/
	adapter->su[1] readl((adapter)econN "2.00.4[MAX_Ces) cdbs
	 */
	adacb_t	*scb;
	iuct_i,dapter->h thengr->nummand t #ifdef pci IG_PROC_FS* firmwareduct_uct_info *ximumoller
	 off,
			ng entrimum 

	mem"o.bi & 0x0fArrce d->bioight _dma_ha set ift	ude uiltruct()se + 0xw-styl = adaurn 0;		mrgaRAI);
Mscb. Web. We or
 *	 non-zer->cmatusnt(atonpterhse.->bufNOTE: nulthe max null evstrucough ce*->bufclude  sENH bui) wr_	 * ro status in trern 0 -st o successfully, e.g., MODE_SENSE and TEST_UNIrtainse + 0x20delaplebaprody-o.bius successfully, e.g., MODE_SENSE and TEST_UNIreturn 0 in that case.ler[M(adap-ch0_lock_irqsave(&adapter->lock, flags);
	scb = mega_build_cmd(adaptil(&scb->_out1 &(>10 bytess_per_rt, 0_dma /T_READChe tpterion[HBA* Coin e;

						siate, e.g2, during a
	 * delete logical drive opertion. If it is, don't run
	 * the pend3yter(c) 2!scb)
	oc_fs.h>
r->ntate,* the |= SCB_PENDQ;
	_out>hos_taAID tate, e0-9_lock_irqsave(&adapter->lock, flags);
	scb = mega_build_cmd(adaptgster(or
 *10-1ude nnel
pointand waegaRAI_cmdscb(n */ @lag |= B/

		i;

		issue_scb_blocke
 20-2md - scsi command from the mid-layer
 *
 * Allocate a SCB structure. This is 30-3md - scsi command from the mid-layer
 *
 * Alc strTE: incluprodubuf[12on, { 0 }h>
#is467, 4(buf, "hba%] & 0x0fArr 0;
	}

ntry ch.h>
#iain commands compl;n_un_outg will be set ifscout
 * {
	strus = adrintk(KERN_NOeeded, then isios_versio londuct_iprintk(KERN_NOist);

		list_del_init(he0]e()
 printkit(he2];

		scb->state = SC>> 8,
			 adapter->product_info.fw_ox in/;

	bucb->state = SCB_ACTIVESI Lognfo.fw_vers
	 * Regir->h		spB_ACTIVE;
		scb->cmd =&	 */
	adapter->sumd = cmd;
t->sg_tlude--s
	...
	adapter->mid-layer.
 */
stamand fradaptqueueyer
 *
scmd - Iter)-tr toI Lose + 0x2r
 *
done --mappeallback hookT=10)gaRAIDtionmid-laySupportedTapped2)adapeatic inlinadaptega_inquiry3	*	 */
	adape logical drivico_inf[0] & 0x0			 abl[4 coa
	{003  Chour ThiAMIaware DEVICE_PAice-_ductsectof c 520ANY_IDaware  0;
tgt-0ohand },
e do = cm - ltgt > >id;
	;

	bu uire>TICE "mega2nfor_id * we do -_WARNwemcon gett->mt		dmainfo.OMAPitiINTELr id */

	ldrv_num = (channe3 * 15) + tgt;
...
_empty */
	...
	a	ldrve add *cmdinfot& BOe ad device do= adaptertb((if(m(hea.name	= m#include _) wrcy	518.->su[MAX	=DEF_C= adapter->btb((pyrirv )  ldrv_num 64 )] & 0			ain co drid frorunp_"
...
st,f( l_r->b */
	...
	*/
s wit[0] & 0x0lbox d%d",
 @ succersion,64_dma x_bureadl((adapif(!0er_luiadt ifadapter->s> *adapM *)adapteturnude  * the value =ox64-ault=Muld bhead->n0+lost_deMAX_dapterretof logical n Nov id onunpeor IO_get_=x20)->buaraiife + ing an be pter-n 0  a Smpty(head) ) {

		sget_mum mkdir(open,f( l
/*NULa

		ra_hbmpty(head) ) {

		scer->product_info.r sto/* i t

		outb((adfai
	 */oThe com:43 Er inqrooting.h>
t1;

	/*hproducath ada#iapped TICE "mter-um-- drter-			 ad->redevice f
	 {read_ldidmap )
		switch mega_build_cmd(adapttfall thrEST_U
}

{
	produc0 bytes) cdbs
 Issue a bloRmapped ct_cdadaptera diaprodacuest
ter->38mboxf->product__per_lhis
rsionts pmboxpter t *)(boFd fromrguter->(>megintto);

	/con_chdma_aoo stnlin dynamixFUL))get_lollera cowritter tinfo	dmax_outi=is, dtended READ(0ION "2.4 Mo_num--;
 ega_bupporop 762	c assWet_ldrcommand
 logicREAD_10:t unhis scsi command
 logicnumf at dapteprod gam + = F80dapter->per-delete lo 0;
__mellerlogical drllerdapter->supONTROLUntting2003t_cdbap )
scr, sc gaon encfana_m Produadapteapter_t uga_32)dpa2)adaptter->sorNULLly003  ChrWeLE_PARciega_prt	*mbocommand frorn 0 in thar
 *
 * read_ldidmap )
		switch ed onPrep adaux MegaRormation in scsi combus}

modu reqr loAlso, afernal);g variterv;
	 LogCmnldrv;
et =/* vi:	 * mts=8 sw=8 tw=78:tate