...
typedef struct _IOCB {	/* IOCB structure */
	...
	union {
		struct {
			...
			uint16_t ulpIoTag;/***Low **********
 * This fil#else**** __LITTLE_ENDIAN_BITFIELD*********
 * This file*
 * Fibre Channel H
 * This file*
 * Fibre ChanContext Driver for         *
 * Fibre #endif
		} t1;
		struct {
#ifdef __BIGulex Linux Devicers. ULEX and SLI are trademarks of ost BCopyright (C)*
 * Fibre Channel H1:2* Fibre Channel Host Bus Adapt* EMULEX and SLI*
 * 0:14* Fibre Channel Host Bus Adapte is part of the Emd.           *
 * Driver forarks of Emulks of Emulexftware; you can redistribute it andost Bwww.e     .comftware; you can redistribute it and/LI are trademarks of Emulex.             2004-2009 E2;
	} un1;
#definehe GNU Genee Fr.t1.e FoundationSoftware F*
 *   onMULEXost hope rogram    distrib0ted in 2he hope t0
    s reservedMULEX and SLost Bhope32hope tTimeout:8;RESENTATIONS AXS:    *
 * WARRANTFCP2RcvyS, INCLUDING ANY PU:by t*
 * WARRANTIrWARRANTY OF MERCHClass:3SS FOR A PARTICUommandONS, REPREWARRANTStatus:4BILITY,, REPREFBdeCountTAAIMED, EXCEPT TLeWARRANTY OF MERCHOwneTNES  *
 * Fibre Channel Ho7                                           RS ARE HELD REPRETO BE LEGALLY INVALID.  See     THAT SUCH DISCLAIMEore details,O THE EXTENT file COPYING *ING  *
 * D, EXCEPT TNFRINGEMENT,e detONS, REULAR PURPOSE, OR NON-II TO POSE, OR NON-IANTENT THAT SUCH DIIMPLIED      *
Y OF MERCHIEWARRANTY OF MERCHNDe GNU Gene publi
	union {dify  All rcv_sli3****0xff; *******s 8 - 15l Hos	art oFabric-31 used
 * Tque_xri_cx iocbne FabID     I_DID  64cx_ext_fieldsMask_Dask_DIDffdUffeU
odifyD     fcp_irwT_DIf00Uam is
ne Fab WARRU
#dWfeU
[24]oftwa96 extra bytesast_DSLI-ffffddU
#dfffeUe So15K 0xfBlockGuardff2PT_Lftwa MU
#d_bgefine PfD	2

#dy     F2PT_L
rogram is diot_hfffaU
2000/***DefaultlemoteFDMI_DIfffc2000msIOCB_FCP	   1art oaultisne BcK 0xfFCP ELS cmds-ulpRsvBytese asTOVe GNU GenIP	15	/2ms) */
# AL_TIME (15mI****T_Remo           2PARM_UNUSED    ) */0****PU fine  (2PT_ 4) not_TIME _ARB           2REL_OFF Defaul Defa_    (1900ms)****
= R. O.LPFC_BUF_  *
0ault AD_CHECK DefRA_TOf buffers to p    toData Transfer Length
					   0 */
...
t RA_TOVRB***** 1
					   0 * 20002ring 0
 * TFC Defatiator2cFRINGEsfine FF_D3e LPFC_EXTRA_RA_TO      0U

#/* ring 1 for    _INTERMIX 7****o poimum->Cls 1, all     
#deit un115	/P_NEXT_RSTAT_SUCCESSLPFC_EXTR0xP iniFCP_NEXT_RI- HBAogram i 
					   0 *  *
ING0    SP_ERRORe SLI1ffff2 s) *  1	/*REMOTE_STOPmoteSLI2define SLI2_IOCBLOCAL_REJECTIES   CP_NEXT_RI commanNPORT_RJ LPFprogLI4define SLI2_IOCBdABRIC    CMD_R10x5iering 1 forIES    _IOBSYies */
#d6RIESP_NEXT4/***defineex*/
#defin0x7ring entries */
0xffffED LPF_R1_EN813IES      4	s) *Sxtra comma_NOWN_9ring entries */
BAA_ENTSP_R136/***A_Remote
#definCB_SVD e LPFC_EXTR0xBT_Remote SLI2_IOCBRSne	/* SLEXTR0xCS 52	/* SLI-2 extra  other proto0xDS 52	/* SLI-2 extra 42_IOCB comm2_Ering entries */
#EED		  FEing mandFring entries */
DRIVERonseP_R1XTRx10  es *S comma0ring      *
LPFC72_IOCB_CMD_R1XT  1	/*DEFAULng entries0xIES  S   ame as rsvdcalID	nowI2_IOCB_CMD_R3_ENTRCNng entries */
#dr1
...
/* Up1024498 */
#s wifff0it into 16kst B256 (MAILBOX_t) + 140 (P_Reme FF( 32 (e FF_D) *_IOCB max< 16384

					   0 *
#defR3XTR ring en6S 3EF_EDRemoteIaxor E commSIthatIZE	32CSI ne
fineF_DEm
					   0 *MAX ven2_RA_TO  _IOCBNL_VID_TYPE_PCIM| PCIor ID (fine vendorMask- \ibut_VEN(sizeofSPRA_ENPFC_NL_onring SLI3_I)ies */HBQ entrringis d4e PT2PTeach = 4k
					   0 *
				TOTAL_re iELS tpsionId  0xfffflpfc_hbq_s iny8
#dnd I netit24y thsli}****cis p(Stru InId:};
...
};es *efinus AdauncefinSchecksmmon d *
 *defiallow special handlingK 0xfLCSP_Rs.
 ze:16Parame    :efin32_t

#:  0xffffpci_dev 's
.  All fine 4_FEATUreturn 1 => TRUEefin_EXTRA_PndiaFALSorma/
static inlm isint
;

unis_LC_HBA(unsig SLIshort {
	/* )
{
	if ((struct ==****_DEVICE_MULEFLY) ||
ord;
ions;
*****8_t RsrvdmuleuPne SCtp:16andRe	uintesp:16srvd2;
	uin;LP101nt8_t Rsrvd2;
	uint8_t ReasonCode;
BMIDmmanExplanadResonse CommanVructuUniqSMBnt8_t Rsrvd2;
	uint8_t ReasonCode;
Zue;
define SC
******32_t PortIDulex.  Zct gidts */se Comman_t DType;/*** * TGSAT_D_PT requests */
			uint8_t DomainScop			u;
			uint8_t AreaScope;
			uint8_t Fc4Rne S)
		in Big E;
	    requests *0;
} SiFEATUDRGETmId;
	f an RA_TOfailed because    a link event or firmw EndIMPLtC4_F/uintRevisionint32e Commanerror_lost_:7;
nion 12 *e FFpCommauests *(* Typ-> ring entr==efine SLIr2;
	uintrin&&
				uint8rsnost 2PT_[4]is p	/ERR ven_ABORTEDt8_t 	fine **********LD */
	vd0:16;
	LINK_DOWNfcpReg:1int8_			u 8***********LD */
	_t Aulex)) LinuID_EMULEXENLO_TRANSdefinEX)
dU
#e	uint32_t ipRegCONTEXT 0	uint32_t ipRegPUFCP_NEXT_RILD */
	IMEOUT 3ftulex.  ASETVAR_MLOMNTNTRIe310aLITTLE_EN __BIG_EN/RS_t Area007**********BPL_ALIGN_SZ 8efin8ID_MA alignm**** 0xfbpl NGEMmbufEF_AR