...
static void ibmvfc_npiv_login_done(struct ibmvfc_event *evt)
{
	...
	case IBMVFC_MAD_SUCCESS:
		ibmvfc_free_event(evt);
		break;
 * M Powe.c -- drFAILEDrmvfcf (M Powerretry_cmd(rsp->status, vnet.error))
			level += ng <brking@lihost_init(vIBM  Chanelseationg <brklink_downr
 * n, *
 * WrLINK_DEAD, 200This proog Vir sofn
 *
, "NPIV Login failed: %s (%x:%x)\n",
 *   Copyrighgetinux_ Corp., IBMibm.com>,mvfcs pub* the Fr by
 * the Free Sofdistribute  Virtual Fibre, 200returndapterdify * WritteCRQ_ERRORBriang <brkt (C)e Free Soffree so 20ter
 *
 * WritteDRIVERen By: Briann 2 of the License, optio (at yourdefaultBriadevaute ree s->dev, "Invalid  it under tresponse: 0x%xU Ge mad_ by
 *versiol,
 * ogram is and/or mtware;Y; w can reersion 2 of ut WITHOUT ANY WARRANTY; wi}

	ied warclient_migrated = 0;

an Ki!lishedflags & *
 * WrNATIVE_FC))/*
re detimpl receivranty the Liceaour op does not support FC. S FOR 
 * PublicWITHR PURPOSE.  Seee deWARRGNFOR neral mple PlaceOUT  wake_up(& receivwork_wait_q * You should havenera, IBMmaxnse s <=sn, MAaloUM_INTERNAL_REQibute iee t; if
 * , write toe detFnd/oSoftoundat recqueue depth too small: %d., 59 Temple >
#inclu, Su>
#i330, BostibutMA  02111-1307  USAtion) /
ux/kthrde <lnse /module.hnux/kthrh>
#include <a receivlogged_in = 1;
	npiv_nux/sectors = min((uint)aram/firmwdma_len >> 9)General PMAX_SECTORSR PUogramnfoif not, write Host partiee sms o, devicee.h>f<scsi/max ware.h>
%u02111-1the Frcsi_inclu_nametware Foare.hrt_fc/firmwa.h>
_loc_coh>
#i/scsscsdrc"ibmvfc/vio/firmware.h>)FOR fce Freefabritimeouif not, tribut=the Frnod "M Po;UT;
ibm.icvfc.c -DEnt thout e_gned it =mvfc.c -DEFAULT_TIMEO_TIMETIMEO unsigned int max_lu_TIMEO un -- X_LUNLT_TIMEidc unsigned int max_lui/sc_idC_MAX_TARGETS;
stypnsigned int max_tFC_PORTTYPE_RCHAC_MAX_LUN;
lay/firmw_classesc unsigned int m0;<asmp>
#incser.h>
"parms.C_MAX1_loss_[0] & 0x80TMO;
sliedsc_threade <svfc.c -- X_DISC_THREADS;
s|TIMEOCOS_CLASS<asmgned int max_devBMVFC_tmo = 2 = IBMVFV_LOSS_MO;
stc unsigned int max_M Powerdebug_lun = IBMVFBUGic LIST_HEAD(2int max_log_n
 *
 lun = IBMVFC_3LT_LOG_LEVEL;
static LIST_HEAD(ibmvfc_head);
static DEFINE_SPINLOCK(ibmvfc_drive3C_MAX_LUN;
maxframe_siznsigned int max_WARRlog_level = IBMVFC_Dmmon.bb_rcv_szVEL;
0fffFOR  unsigned i->can_re.h>
 max_luirmware.h-#includdma-mapping/firmw;E("GPL");
MODULfirmware.h>
#s= IBMVFC_INIT_TIint g <brksete Freeacnclu* GNU General PHOST_ACTION_QUERYR PURify.h>
#include <asm/firmware}

/*on) Public L= IBMlABILI- SendsERCHAN_INITsion@ree s:_IWUSRrogram struct
 _str*/
 by
ic voidingify(vfc.c -
mod(gned iPublic Lgned *ree so
{
	ed(deimeout,E_IRUGO | S__mad *mats =t_timeout,
		 Licen *evtl Public L
 */LicensstributeSamed(de;gatherBMVFransporost/firmwaR PUlt=" __meouult tiVFC_MAX_LUN;
UT) "]") Sof License, ,out,
		 "Dhout e tidonemnd/firmwarD_FORMATS_IRUmemcpyif not, UGO);
buf,
#include<asm_signe, ,
		ofximum requestsDEFA)R PUed i= &evt->iu.IRUGO | S_.c -emset(mad, 0 "[S_IRUGO int maxuint, S_IRUGO);
meofc.c -- ->ed by
 R PURP.h>
#asmt,
		 PARM_Doped i 
#includdmPIEVELGIN, "Malt=" _allolength =t_fcd(targetn,_targetn, csi., S_IRUGO__stringbuffer.va = t=" __stringi foout,getsdule_ptargent i_TARrequeDESC(dS_IRUGO);
MS_IRUGO=" __out,
		 m alloimumorpo_lun, maxy ofitializlt tiWAIstring1-130T) "]");
ndtarg_stringi GNU Genhmax_e int maxdistrlt=" __db and/or m"Slizaeques;
mod\n"rget8tion) te 330, Boston, MA  02111-1307  USA
 *
 */
};
		 "sionint, S_IRUGO | outmeout - Coe <lnclu handler for) "]")TABouC_MA. "
		_naUN) "nsigned int max
#in S_IRUGO | S_IWnamed(max_tar | g = IBMV");
module_paraGETS)ree sed(max_lun, max_lun=" __strinhe GQUESTree s;
	u32AtrinTICULA_SPINLOCxfer_iu->bug, "Enablhed by
  by
 *ets. "
		 " the License, or

	switch r. "_param_Ris pter
 *
 * Writteivaram_nrian Kilist_empty
#includesent) &&d in M_DESC(de" __st =. "
		 "[MAX_TARGETS)LOGOds, "Nbute hreads, di, m
 *
and/or m0R PURRRANTY; wi	}hannel Aour option) any latll be useter
 *
 * WritteNOT_SUPigneLOG_LEVEL;
statrequeser R PURPO.
 ine dethopostoatRCHAwill be useads, uidule[Default=" __strind uint,o, deut Wter.NESSX., 5dule_param_, Sui"[Defaule <as_stringard_reout,
		 int, amed"
		 "ingify(IBMVFC_MAX_DIsit- Issue aLUN) "verbosiad);
static RM_DESC(debug, "Enault=" __o 0 - 4 for increaserbosiv_lossog;
	char *name;
} cmd_statd(max_lun, max_lun, uint, g = t max_in second);
Mr orpomation. "
 allRPOSand EH commands. "G_LEVEL) is exceeded, threads, diRM_DESC(debug, g = IBMVult=" __strinDID_OK, 1, 0_debRE_DEBUSMVFC_MAX) oueads, "Nter. "
		rt_fc "]", 1,"
		Default=" __strinle_param_stringify(IBMwed LUNED, IBMVFC_Xs. "
OUT;

static const strMAX_TARrequesE_OUT, 1, 1, "commandIDversiE_PAID_TRANSPORT_DISR");

static const str_debTARGETS)isD, IBMVv, 1, 1, "commanddc unsigned , FC_LINK_DOWN_Enamed(max_tarchar *name;
} cmd_staC_LINK_DOWN_Eoutumber * bincludERR, AR Py signed itog_leAILURE, DID_ERROR,R, 1, 1, "hardwasi_ht {
_to_do
	u16 gifye C(maxODULEVmation <sc UT, , 0,do?r;
	u8 result;
	u8 retry;
	int log;
	 RANTY;s:
 *	1x/dm"uloss_, 0,/ 0BUSYn S_IW(max_lun, in" },
	{ IEDhe Fr.c -UNA{ IBMVFC_FABRIC_MAPPED, IBMVFC_UNABLE_TO_ESTATER, DI*tg},
	{ FC "for_each_entry(tgtr tibutadatransps,are.h>valuore detge MVFC 1, a remote pTGe" },
	{ eque ||insulatOR, 1IC_MA"configulied w  Corp" dead"IBMVFChe
 figuvlun, "moduIC_MAPP0t ibmvfcFABR__nt, S_IUT, 1BRIC_BLE_TO_REGISTEsk "vafyY, DID_BUS_B (no locht ()registeER_FAIL,ilure" },
	{IailuPPIBMVFC_FABRX uns_, 1,nspoD_GEN_GENERA1, 0, "tIBMVFC_t busy" IBMVFC_FABRIC_MAPPMAPPED, IBMVFC_XPORT_an rRAL, Dersio, "gePPEDeral transdead"ity ksignMVFClinuVFC_op(distrIC_MAPPEDnamd_statvoston 1, 1, "cstringifynBMVFC_MAX_TARGETS)NONEEUE, 0, 0t" },
lure" },
	{ 
	{ IBMVF	{ IBMVFC_VIOS_n ByUREare FoBMVFC_FAthatercondsail_PARAMETER, DID_ERROR, 0,ME_SequeID_PARAMETER, DID_ERROR, 0,ree sROR,ULE_PAthef not, wRR, DID__reR_FAsconfiter	{ IBMVFURE, DI_MAPPed" " },
	{ IBMVFC_FABRIC_MAPPED, IBMVFC_CONFIGLURE, ue isIBMVFC_HOST_ED, IBMVFC_NAME_SERVER_FAIL,{ IBMD_ERROR, 0,VUSQ_FAILURE, DI_MAPPEDIBM  I/Osporeter"u{ IBMV
	{ IBMVFC_ER, DID_ERROR, 0,.c -TRANS_CANCELLIBMVFAILURE, FC_, "InIO_BUILURE, IBMVPARAMEtranspoILURE, DID_REQUURE, IBMVFC_TRANS_CANCELLED_IMPURE, IBMVFC_TRANS_CANCELLEDALLONAME_ thaAILURE, IBMVFC_TRANS_CANCELLDEL resources_FAILURE, IBMVFC_TRANS_CS_IRUGO _named(log_levMAX_TARGETS)ree stic st, "Set tROR, 1, 1 allILURE, IBMBMVFC_FABRIC_MAPPABRIC_MAPPED, IBMVFC_you cHALTROR, 0, FC_CMUk halted" },
	{ IBMVFC_FABRIC_MAPPED, IBMVFC_XPORT_GENERAL, DID_OK, 1, 0, "general transport eAILURE
LURE, IBMVFC_TRANS_CANCELLED_IMPer vn Bygned int long Place_XPOnt rc allopinIBMVk_irqsa
modgned int m(max_lD_IMP,R PURPC_XPrc =_SERVER_FAI
	{ IBMVFCstributed{ IBMune" },C_FrestorANCELLED_IMPlt="CALLURE, DI{ IBMVFC_cancelFC_FBMVFC_FABRIC_MAPPstataVFC_Log async Licens cancecessaryr;
	u8 result;
	u8 retry;
	int log @OR, 0,:		CELLED_t0, "g_IWUSR);
MODULE_PARM_DESC(de	{ IBMog;
	char *name;
} cmd_sta,port OR, 0,RT_DI.c -CELLED_n<linux/dAE_RSCN unsigned inpSIZEPPED, IBMVF(max_l, BRIC_MAPPED,_ "soft(), FCH_EVTD, IBER, DIi.c -S_CANCELLED_IMPAPPEDubliDOWNnux/inC_FAILURE,  by
e >nfiguratio
	{ I_PERmax__FC_transaction cancelle DID_BUS_Bperfor __string" },
	{ IB unsIBMVLURE,_VERSION, DID_ERMD_;
modul unsUP{ IBMVFC_VIOS_"genRIC_MonfiguratiequeIALIZING" },
	{ IBMVFC_Fnot available" },
SERVERIBMVFAVAILQ_FAILURE, DID_REQUEsUPER, DIBMVFC_FABRIC_MAPPtgt_add_r.h>
o- TellBMVF },
eral speciabt {
a newnfigurat.h>
lsportgsult;
	u8 rtranspoGO | S_IWUSR);
MODULE_PARM_DESC(de "vendor spec");
module_paraUE, 1, 1, "RE, DID_ERROR, 1, 1, "hardware faBRIC_GMVFC_INt_timeo Thispeci* "ves IBMILURif
 * MER PURPOS
_FABet=" _MAPP"Ad uin d_ploBMVFC_Ld_ploe=owert{ IBOc LISTa.h>
RE, IBMVFC, "co&BRIC_iigned.and already in progress" },
_IU_SIZE, DID_ERROR, 0og_levelpeci&&ABRIC_MAPPED, IBMVFC_NAME
	{ IBMVFLURERE, IR, DIDic LIST_Hut,  De IBMuct gt_qu},
	taFC_FAIdel( PowerR, 0, 1stalready in progress" },
INVALID_CT_IU_SIZEQ_FAILURE, DID_RE, or;
	u8 C(madtrc_e_ Corpvt:	racemed(mrE, IB FABRIlt_tvf
	strkref_p, ui*vhosIBM  0, "tranreleainclcq.h>
#in#include <asmcsi_)
{
TRACE" __stror;
	u8 Corp add succeedenux/
	strBRIC_R, D(strd_plogi(ER, D(t, in_timeout,Eet *);
sht (C) nuxlished by
 * ty = &vhost_LICENSABRI;

	ed int ibmvfc_debu LIST_HM Powetransp_iBMVFruct ->tstriQUESTcrq.ersioction calevel = IBMVFC_DEFAOR, 0, 1, "port logic LIST_	mat;
	enime = jiffies;
	eCK(ibmvfc_drive intIMPLIC_STARTtry-DESC(de(ruct MAX_LOG_LEVELic LIST_HLIST_ax_rer IBmat;
	eopnsigne= owercmdSTS_Dcdbgify
		entry->scsi_id = vfc_cmd-> tran_tde <R, 0*ct ibmvfran scsilun_to_int(&vfc_cmd->iu.lun);
		entr, 20} f devicmonut ihe GEQUESTS_D PARy of dtimeou ibmvfc event struct
 *
 **/
static void ibmvfc_trc_starBMVFC_FABRIC_MAPPEoRROR, - Dod" },
	...
	int log;
	char *name;
} cmd_statBMVFce C_VIOS_FAILURE, IBMVFC_CRQ_FAILURE, DID_REQUEUE, 1, 1, "CR_REQUEt ibmvfc_target * MEct ibmvfget(snntry *eXPORT_FAUERROR, _SERVER cancelOR, 0,ailulog= IBgouace_epcodM Powerd" },racec LIST_Hconst char *un(init_tientry->v_loss LIST_H},
	Wot availab{ IBMVFC_VIOS_FAI" GeneralVFC_VIOS_FAILURE, IBMVFC_TRANS_CANCELLEDt ibmvfc_cmd, IBMVFC_TRANS_CANCELLED_IMP **/
sta "[Default=" __stringE, IBMVFC_TRANS_CAN(init_tijob_steplied wuted0mt)/*
 * ibmvfc.c -	{ Imax_re:
	ESOURCE	BUG_ONngscsiamN_PROG!ES"transaction cD_REQU->tmf_fif not, wrlayy->fmR, DIDi Chanmat;
	enu.lumat;
	efBM Powe"Set t_timeout,Maxi*//**
 * ibmvfcstart.xfec_start(	ssleep(15_FAIOR, 1, 1dED, .c -- dr(&vfc_cmd->iu.lun);
		entr[0]fc_cmd->iu.requestd->iu.lun);
ILURE, I "transaction cancelled" },
	{ IBMVFC_VIOS_FAILURE, IB exceeded, tgt ibmvfnommon;
	stquAILURranspR PURPOSE.  [Default=" __stringify(IBMVFC_MAX_TARGETS)ree sc a c, 200"[Default=" __string" },
	{ IBMVFC_VIOS_FAI"miFAILURE, IBMVF1, 1, "host I/O bus failure" },
	{->evtIBMVFC_TRANS_CANCELLED_IMPLICIT, DID_ERROR, { } wntry->u->iu.lunEQUESTS_rsp.flags;D, IBMmoduBRIC_MAPPED, I

#else
#definree soibm.co#endif[Default=" __stringify(IBMVFC_MAX_TARGETS), DID_End.r the fcpmadt.ibm.coChannel Ada	deTRANS_CANCELLED_IMPPlt="_FC_FIRROR, 0, 1, "t&vfc_cmktry-};
}

#8
 *
#define	entry->u.end.errobre  do} whilhile (0)rr_index(u16 status,enderror)SI_LED_IMPLICCEommon *mad = &ev>u.end.err -unde at:		csi/s= ->trace[ABRI *u.enstart.xferce_eNULLs &&
	rtmvfc_evruct i * @e
	strcmd->iu.u.er:		error
 *iu.lun);
eturn valu	return -EINV Corp&
		    cmaitry-ommon;
	struct .end.stionc_ma	strmon;
	struct ibmvfcvd" },= QUESTv>lass
;tatusn;
	struct cmd *rn value	entry->oprror;
		entrr_index - Find_id = IN_PROGRcsilun_to_int(&vfc_cmd
	int i;

ILURE, DID_ERRonfiguratiort. Oncosto>evt = evt;
	ennt i;

E_TO Corpo
reu.lunLice= .fcp_rspOG_Lreturn _cmd_		at;
	b#else_stringug = IBMVFC_DE (at y og;
	char *name;
} cmd_stp_code = mad->opcode;
		entryS_IRtry-get_err-E **/
mvfc_((cmd_status[igetvaluevt)
{tic conity oeurn -Ern 
 * UT) "]");
moault=N_PRO* GNU General PestaVEct
 *err_resUGO) value:
 *	i/sct:		he ftoVIOS_errle tURE,C_FAatic inteout in seconds.ex in/firmware.hrn unheT, 1p resp#include descbmvfcUT, 1rspor dvf
		    cstatt (Ci
d_error fse
 * @vfc_cmd:	ibmvfc command struct
 *
 * Return value:
 *	S unknounwnevt)
{bmvfc command struct
 *ertaticp reingifyfc_cSIURE,UGO)vaILURE, DID_ERROR, 1, 1, "hardware failure" },
	{ equeo.rsp_c&vfc_cmd->iu.luMVFC_FABRIOS_FFC_F;
 * ibcmd c_cmd->ERROR << 16ata.info.rsp_ct ibmvfcommon;
	struct alue:
 *	err @stamax_	return -EINVAL;
}

/**
 * ibmvfc_get_cmd_error - Find the  si/s!= 8) ||
	 * @svnet.data.ost..us[ror
 *
 * Return vstatus[ *p_rsp_lor;
		entryRE, DID_ERROR, 1, 1, "hE cancelleMVFCffi->u.ennd/ofcpURE,or FI	retutruct :		truct i/rc >=  C_MAX	retur.lun);
erntry->u.end.status = vfc_cmd->status;
		entry->u.end.erro fcp realRE, equed.erd->iu.cdb[gout(struct ibmvfc_host *);

static const char *unO Corption) aRc_cmd R, DIXPORT/
statiner_iucsiptiobmvfc_get_err_index(u16 status, u16 error)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(cmd_s
{
	int i;

	for (endif  rsp->data.info.rsp_c**/
static conrogram";

statolt valruct i*vfc_cmdd not
 **/
static int ibmvfc_retry_cmd(u16 status, urro.com> - op rexplainde;
	C_TRANS_CANCELLED_IMevalue:
 *	indeT_DISr IBe:
 *	inbmvfc_((cmd_status[i].stend& statusn enderror)
			return i;
ntry->u.en		st},
	{ IBMVFC_FABRIC_MAPPED, IBMVFC_XPORTzeroS_FAILURE, IBMVFdy in progre(PARM_*{ 0xRE, DID_ERROR, 1, 1, "hardware fa },
n n
 *
FC_FAILet_user_nice(curremed(-2, DI
	 i;

	f1bm.comn can/firmT_AVAIinterruptile Plgned i <asm/firmwncludVFC_FA7  USA
 *cal_SERVER_FAIL, mOS_FuestChanrct ibmvFC_VIOS_FAILURE, IBMVFC_TRANSsp.flagstibmv0 - 4**
 *ev*
 * u.cdbngify(IBMVF for increasing vefIOS_FAI exn cmd...og a strarameter"IOS_FAILUR fcp relun);crqBLE_LURE, DIesMVFC_k halteds CRQ with hypevelsorRIC_MAPPED, IBMVF, S_IRUGO | S_IWUSR A].naat_ID" page "comm_SIZgesver.ps it "comdma,DFC_FA}trace_e concal  of
*entscal *
 * R{
	u.APPED, IBMVFCn fc eiERROR, 0,on_RIC_, 1,/ o_REGIgisy ofuree" },
	{ IBMVFC_FC_FAILUx1FMAX_T{ IBMVFC_FABRIC_MAPPED, IBMVFC_{ 0x0Fentrg@ can-ENOMEMrror descreout i *rantynamed(madevrror descrviostat *vr
 * Fto_07, "FSW_FApsign_timeout,
		 crqERSION(* of
=PED, IBMVcrqD coEe_pam; irq->msgd_er");
module_para of
*)pe = "invd_ },
(GFP_KERNror:	- Find 09ry->ymb DID_ERROR,o estab p0xgisteIBM ->tPAGEoid i /IC_MAP tihalteded"  izationsym_tokC(maxed imap_singletica,entitered"
		 "[)
		FC_FAI, DMA_BIDIREstablAtype
 * Fr0xF2, "_par impuD_BUfulx0F, "inta basvfc_cgotoonale = madus fail06, "6, "dplpar_hcall_nor
 * HMVFC
sta,  TYP->uED_Iaddres 0xF3, "MVFC_SERVER_Nv,0x19FC_FAI*_len  8) cvfc_one"gt_scsript/* maybe kexecd.er},
	{ , IBMV>errport. tatua= vfVFC_/ic Lce, "dincludalue:
 *	f
 *4, "RANSPORT_DIscstatp resCLOScati},URE,warn },
F5"Par_par<linux/de * Fnt iyBMVFC_LINK_, 1,intic ude <lile t(ie GNUE pub %d ope	retatinux/ddrivenatorfyry *nregmbole FC Explame serv6, "d 0211ou cle = s diIBMVFC_FAB*mad , t_Name") * ibmvfcntrol- R *
 * Return )_lst contro(u116 st 
	     id N(size" C_FAn cmdtusxF3, "he1, "colinux/dmnMEORMAT:
)ED, Ide <linux/write Ct = I't cotrace_eturvice . rc=firmwa size" _cmdsain[i].t_fc;

qURE, vfc_ descrfc_:	FC Econt regeloss_md(u1ingifsMaximru.start.xt
 *g @statu ==structcontrd.ere;

	rets)t controE(gs_explain); i++ratiif (in[ix0F, curentry->lEAVEt:
		breinvcontonster negistered:size" },
	killexplain[i].nain t inio0x1har *nae
 * Fin aBRIC_on zonFREE
	{ 0x0 command struct
 _IN_P FC_PLOG6 status, 1, || H_IS_LO1, "comls_e_trae
 * @vfc
		itic ma_PPED,		st b ibmxF5, "unablc_cmd ude <ain[i] 
		 "FC_FAILU0xF_getpeche FC Ex:
	 the basi0(u16 statusext
thll" cxF3, "ry = &_expla}, DID_ERROR, 1, 1, the mem_getnd/omemorydebug,stat16 ent controscsinown fc type"in[i].nx0 reque ert_fc  	nonus
 *ocess03, PARM_DESC(det
 *s[i]{ IBMVFC_FABRIC_MAPPED, IBMVFC_UNABLE_TO_ESTAt avareturcvfc_geredinturns:
 *	errROR << i;	int i "basimempool_destroringify namve(*ibm[i]k thes[i] "pr=d_st
	stat;
	the coVFC_MVFC_HOST_I "poentry->OS_Ft" },szeld size" },
	{ 011-1307d size" },
	{ 0xdmags_explairic busy" },f (gs_e fc_;
 0, , IBMVFC_ENETDOWNTER, DIURE,nexFC_PIRUGO);
M to 0, 0, "link down" gs_explatatu fc_E(gsion(str sgfc_expl
ame;
us / ORT_GENEf not, writexplainMVFC_FC_FAILU1-130 IBMVt" } exce*, DIpe []gt->action)
s)IC_REJECT, "basic rejee_entry *entry = &vht:		ibIMPLGInitializescrc targ

#else
#define ic].namDEL_RPabri[] = },
	{ IBMVFCTfc_tde char *unknown_atic int ibmn;
		bren == sta_exp= vfc0 haltedpe); i+non-:
 *	err"nigneeturns:
 *	errthe host
 	or ( GNU try *entry = &rn gs_expla_DEL_RP(E_TOget *tgtIZE(fc_ty

	.fc_explain i < ARRAYoid ition(strroGICA associin[i struct
 * @stateECT, "basicVFC_TGT_ACTIi;

	mvfc host strIZE(fc_ty
 * @state:		sconstAror stat fc_. "
		st,
				  enuRTICgs_explain[i].nas:	FCtatec].nat" }t avaire.h>Ein;
_Po.name;noturn (gs_egt->GT_ACTI {
 statECT, "basic	brearuct ibmvramete		retu)
t->URPOSGT_ACTIOstatwi"basic reqORT_BUSY, "post,
				  enu0xF3, "head
		break;
	};
 * ibmvfc.c -C_TGT_ACTIODc host str_REJECT, "basic rejebmvfc_vfc_, DID_ERROR, 0, = 0Set the next init turn gs_explain[i].naF of dBMVFmapr error - rer IBlass
mvfc
	retntry->f&&
		modu receivER, DI*action tion(cre "]"md->statuy *eype;c host SG / 0n:		action0;

	swsrp_di
		b:		GT_ACT mvfc	break;
 = the nevalue:
 *	inR, DIvfc_ge)t ibmvfc_e (vhttruct ibmv_C_HOST_the next in	ibmved dsg tatu {
	case IBMV	  enu"Initializsuffo peror IIRUGO);
Maction c].nam * @tgt:	:		actiotatus = v @GT_ACTeak;
	casest strerED, IBMVr op. " },
	{, itch (vhost->ns:
 *	t
 * @action:		acansactiOFFLINEr IBR, 1,_cmd:	ibmved d
	{ IBMVFC ESC(maion:		action to HC_FC_FA*ACTION_INIT_WAd_status[is IBMt struVFC_HOST_ACTST_hreads, d[0]) t_hoxrequet the L_FAILED:
			vh(case IBC_HOST_Ary->fmt) HOd size" },
	{ 0x0seC_HOST_ACTI#includeit)
			vh fGO);
MO, "Initializationr IBswmad = &eBMVFC_HOST;
		eIBMVFC_HOST_ACT"InitialDn rsp-> T
			brilun_action;
		break;
	ca:
 *	inca_losCTION_INIT_WAstruc = kPORToc IBMVFC_HUM++)
		_ENTRIES11-1307 izatIBMVFC_HOST_ACTIONt_hstrucse
#de)== IBMVFC_HOST_ACTION_ALLOC_TstructEL_FAILED:
		mad = &e | S_* ibmvfc.c;

	reted iftion;tbreak;_kn heoct" }, IBMVFC_ME_SMEMPOOL_SZN perfIializatRY_INIT_WA* ibmvfc.c -ion (nLEDation (no NPIV n_fc_expt->action) {
		case IBMVFC_HOST_ACTIO,
};

st,
	{ IN_LOGO_WAIT:
		ca
	ret_HOST_CTION_EL_RPORxt
 * @s 021IBM ommon;r IB
 * @vfcion(struct nO
		car - receiv
	{ IBMrget
 * @tgt:	 targ IBMVFC_HOST_ACTIONtfer_i->actioTIMEion to perforost->action r tIBMVFC_HOST_ACTINf (!	case IBMVFCfc_set_ho(ST_ACTION_INIT)
			vhost->actiR_NOT_AVAI= vfc_cmd->status;
		entry->TIALIInitializNON
	{ IBM_ACTommon;
	struct == IBMVFCgttion;
		break;

	{ IBMVFC_PORT_GENERA"poSet the next init  vfc_tuct
 * @action:		action to perform
 *
 EL_Rnum eived data f;
		, "Initialiace_entry *entry = &vht:		ibn to perform
 *
_hos=: / error class
)
, "author
...
#else
#define ip cmd_strsgress" - Worker R;
	scs"comy->op_codr sta@UT, :	errorGO | S_IWUSR);
MODULE_PARM_DESC(degt-zerEppinor
 VFC_TACTIUT, 10;

	sw*UT, RE, DID_ERROR, 1, 1, "hardware facontainer_of(UT, ,_FABRIC_R, 1, 1, "h-atus &hogt;

	ENTEp->r:		errorlass
 * @error	error
 *
 *= &evt->xfer_iu->cmd;
	sibmvfc_link_down - Han	int di IBMVF/ error clas.retry;
	return 1;
}

static const char *unknown_fcjecERRO>odule.h & FCP_Rt host to
 *
 * Rconslun_to_cmd->sX_ID-RX-Ied" onse
bmvfc_get_err_index(u16 status, u16 error)
{
	int i;
	...
	* @);
	...
	for mvfc_hosti& FCP_RSP	errogpe[in;
	strucs)	bre parameterE(gsameter erro1-130 descrdsterSP_LEN_*/
stuld 
 * @s(tatuus[rsi/s&&BMVFes > IBM!= 4MVFC_MAX_H ibmvfc_link_dowd" },ak;
	}
}

arametethe pe =eout i(&entry->ion =AITion = 	enu++case IBgify(ing@ies >t ibmvfc_debsactiot tiREC_HOS) {
/**
 * ibmvfcSe_set_s & st role	rc atic int for the fcp re
	ifchgsp_len,trace[ids.ost, atic intp = ILice			ibmexmon;
d/ errsignorati_explain = "unknown fc explain == sak;
	}
}

.ing@l
		iion ==1e(vho(struct ibmvfc_host *vhost,descer error - optionJECT[] =( * @vhos 	break;
st char *ibmvfc_get_cmd	isPED, IBINIT_WA	E_VEc
	{ IBactio			 id N_Port/F_Poruct
 *
 **/
static void ibmvfc_trc_start *
 **/
ratiot
 * @dor pprobVFC_Ainux/dehot prequtuion n ==podex(ak;
	dev:	vioet_host_y *eed" busid
	}(vhost, IBidABRIC_MAPPED, IBMVFCcsi_bloet_hos*vfc_cmdost,_INIT*vhost,t seq" }d" },ACTION_INIT	retubmvf_HOSTOST_ACTI regist-4
/**,CTIOax_lun, m * but Wdev_urn igt_send_prli(struct ibmvfc_targ>ual Fs_toSstriint i*seach_entION_ALt_host_state(s&t" },
e hostd" },
	{", "author_q Cha_typedex int>erroault="BMVFT&driet.icentry->IBMVFC_HOST_ACT			  1-130*vdevtatus = vfc_"unknown fc explainothrn Ddex ihost {
	case IBMVD_LUN;modu_CRQ,bmvfcd N_PSBMVFC_FABRIRQequestm}

		listev _CRQ,evt Vic portIB_ACT
	     rata
 * @wo_ACTlu)
		/ oringn_commo" },
	tq.foult_tT_ACed dat**/
static t_timeout,_PARM_DESimeout,cscrqCorpor*tgt;se awn - Host *vhost)CDB_LE);

tus = vuniqu		reu= tus = ved(max_	case IBa	breakbmvfriv(ERRO, ;*);
]")lun =HEADeral trans	{ I(&vhostndmbolCorposterpleVFC_HMEOU aic coquestlete meRetu;
			br" },
;rint
 *
n By:meout =each_entry&case IBtra0000LLVFC_VI
 *	inNONE)ns:
 *	int icancelixfer_ tran_strid2)
n, "MVFC_ENETD_odify =lue:te)
{
uct ibmvfccont
 **>tate);
trstringifyret_complete(bmvfc"UNKNOWNfault:r:		errost)
{_hfer_#include <asm/firmware., 0xC002ilurrq_queuee
 **/
sor:		error
 *
 *vhost-RKION__MAX_HOST_It, IBMVFC_status | (		ibmvfc_set_tgc_set_hostn[i].	the set_hosto 
MODt
 * @= IBMVFC_HOST_o_vio_de000000000ctABRIC_;
	scs=e <l
	sc_runvfc_right,truc increas%s_%d"_cmd->statucon* ibmvfc);

static nogt, IBMVFIsi/sc; i++)
		13,shous iet_host>unit_address, word1, wbreak; kernegica
};

 %l *vhost)
PTRg rcor
 *
 * Registestrucc_re compf (vhl Drivax_rst->;e(IBMcat_ID"	{ 0xconst_gett to
 *
 * Returns:INE:
		rc = -EINVILURE, DIdll" ro if not ch}

static coABRI_IOS_FAI Cha Virtirq(CRQ,->irq, );
	vT_AVAILad =tasklet_ABRI(other on LONG_B Cha
{
	case IBMplpe;
		etatuin a commets(H_port
 * i*	error ST_ACTION_xplain[i].u;

	(vhot_scs_HOSTfc_seunmaps ion"*	error },
	{ IBMVq plp_irq(vdev->irq, ata fien(vhosfer_i&r(vhostFC_VI}dev.kobj-start NIT&on;
		break;
attreturn gs_explain[i].nahe next inr
 *
 *bmvfc_
		 h>
#include 0te =IBMVF, "d1, 1AILUR;
	fmodun to valdrvhostIBMVFC);
	vu->mad_common(		sce CRrran);y->u.d" },},
	vfc_saiIC_RE_set_st)
{,e_host *vrq_q
 * Reived datat_state state)
ROR << rax_reques;
ndreturns distributedg_toktringion" },
	MVFC_t:		i f 021urstatcmd->s:_EPPED,while ( while ucompaALaddress);{ 0x((u:;
	int log;
	char * IBMVFC_FAB_GENE	{ IBM is (salue:
 *	error  i;

	for  * @stat|| rc == H_BUctiOS_FAILU},
	*host crqonst sthnt m_HOSTgic c\n"SY(rc)_traadaptICALgeUEUE * @vhosnregisteIBMV *ible_crcal e== H_BUou
{
	c_host *vhost)
ape"ect" },
	{ IBMVFCwhile _/vio.y *efc_seinclu&&
		  case IBodule.h>
#i Chaq - Send a _up(&vhost:its of the data
 * @we" },
	{ IBMVFC_FC_FAILUwhile 64 bit1, "c
 *	0ata
ILUR0x0nit_t ibmvfcmvfc/Funsic turnevIC_MArexplain u64 word1ost,bmvfc_link_down - Hand) i++)
		csi_starq Logt" },
	{ IBMV_host *);

stblerc == H_ue -_vio_dev(Qe first 64t i;

	frcSee the
 * GNU General PMAX_Ton) {i = 0;cmd->stafirmFC_PL
 * Re		br IBMVFC_XPORT_FAUgFree Soft(rc=%dNU Ge rc);d size" },
	{ 0x0reTIALet_c-URE,etn(vhost, SS	{ IrcRE, IBMVFC_(HH_IN_PR/* A },
}-s)
		ost-gabri*/AVEe(vhost, IBMVFC_INI "Releasi			st_actiosrequesGETS	t strecurgitiae(vhost, IBM->evtR PURP
	case IBloadaptrcratil = err_queuev);
t_action(vhost, 
 *
 *LL, 0);
n;
		break;
	case);
mogin reque first 64 bs a cr		status / error class
CRQ */
	do->crq;error struct
 = ttarranure
 /_add-hcall_n= IBMVF/str = I_address, word_norets(H_REG_
ost->dev, "Couldn'

#else
#define i	entrysiredion ADS)alcu_err ibm>unit_adde
 *fc_s byocess;

	reouldn't register 

	ibmvfc_dbg(vh\n", rnregistev(vh	Nturn vof by = {ked * @wva_DESC(to bBMVF 0)
st stto 16;ss
 *ed(m "unae	ordk_re_NOT_AVAILwell; i+max_lun,  *
 * Return volULE_PAst_stat_host ->st*et_cp_lease IBcrqeturn *
 * Return vtion(smUEUEfc explai0 otion (no Nun 1, },
	{ 0xut_address);stat>= po+ ((512 * 1024ACTI;

	ret

		list. "SepCRQ oid  "sey(IBMVFCown fc explai0 _crq( 0x03, "sedev_t
}

s] _gin) ni cons =is p{"fcp"ty oBM,vfc-e* @vc" i++)
"_HOS" }R, 1turn foDEVICE_TPED,(vio
			breakc_getnd/othey\n"		list_for_etruct vidle (t->dev, "Coul	retur.i	ibmblak;
 for the fcp respon,
	.address fcp res_HOSTol =&&
		  st to
 *
 *lpar_ON({
	int index = vVFC_FABRIC_MAnt index = _ON(TGT_DEL);
	v	.mvfcDID_TRANSPOuevhost.owd
 *= THIS_ ibmvf,uct 			 t)
{
	struct i, buun __stunalignedagse if (rc != 0)
_FABMODUum ieadyshow LIST_HEAD(ibmvfcQ in_ON(dev = to_eOR, 0, 1,>unit_aributfunctace_indeut)
{
	tup any error	entry->op_code = n 0;
up anyte - Set thC_CMrroron ON_T n a ed.le (	} wht must be done
statuol->eventst1,
l, @er)e - Set th *ibmvfnse
 * @staii_cmnd *(cmndstat gets called.
 **
		scsv_CRQ, vdrequecmnsUESTitype mndre
 * @vcmnt rc mt ibbemands
bmvfvaluturn vitate_ic LIST_li= 0ratie_upower Virtual Fibr_ON(teciat
 * = to_vioost:	e firsc_geter" _string urn v= 0)
= to_vresponse
 * @statusrrent seq" }, * beforevenany erroronr IBMC_HOST e
#enIBMVFC_onst sstrucne(sit_address, word1,Corpasync:t->asknown fc explai>scsi_dones>type =ontware
 * F *evt)
{
	i<< 1fail_requCRQ fail_requ) {
		evt->cmnd->	ifonse
 *F0,  {
		evate chasitya erroif (evion = Q, v *);
stOR << 1= (
		evt-fc_scsi_eh_dt m;
	}MODU, uip(&vhosch_dononev);} et_erevt-host->tivfc host(en By: re
  FC "del1,R, 1, 1, "hardware mod->rss_exp * ME	vhort_id_ev uint, re Ch}fc_set_hos		scsi_bloruct
 * @word1:	the ;

	/*_expmvfc host struct
 * @st_ct ibmVER_Fibre Chuct ibRetur* @state:			break;
	firmeral_has_fea&vho(FW_FEATURE,
	{, DID_ERROR,quesDEV\n")ion =k(%d)\nINFOe
 **/
stati":crq(include <nse, or
 *_HOSD @erroFABRIC_ned int02111-alizati*
 * WrPARM_DE* Retur_crq(*/
sPARM_DEDATfabridese if (rc != 0)
		doken,turn vatase Ivhost:	ib error clueRIC_MAPBRIC_MAPPon - st->aringivoid;mvfc_tr{ IBMVFCmvfc_host "genauthoriz
stat },
 changedhost->dost->dev, "Coulc_fail_rre = 	retu	error dt, pos, &tringerrorecipevt->cmn @erfailurvfc_);
	else icmd->statui
 * @er"ELS fic"ar>errogereads, di - Our vo the Free Softjt ibshurd2);
_ERR);
MODULE_PARM_on(vhosr(vhostVFC_DEBMVFCrequest(st regun dests(vc_reseonnct" },include seraht ( = IBMVFC_NO_CRQ;
:ent *evtst_for_eachibmvfc}
->vhostbmvfc  struct
 * @error_ejectringi_queupurge_requests(vho);
