...
static netdev_tx_t dfx_xmt_queue_pkt(struct sk_buff *skb,
	...
	if (bp->link_available == PI_K_FALSE)
		{x.c
 Codfx_hw_adap_state_rd(bp)n:
 *  STATE_K_LINK_AVAIL)	/* is ght  really nformatio?c
 *			pyriusedItributed n:
 *
 CTRUE;	may bf so, set flag and continueccordielserdinDiging toxmt_discards++;	t:
 /* bump error counterted hendev_kfree_skb(skb) Generrenc sk_buff nowver supnetif_wake_queue(dev);eferreturn NETDEV_TX_OKl Equid PCI c"success"ver sup}
pter
	spin_lock_irqsave(&g to ock,Licens);

  thGet the current producerse, iEFTAnextipmentxmt data descriptoiver 
	C FD		= g torcv_.
 *reg.uppox.    ntro;
	p igin)cord = &og tol aut_b *
 *virtce    
ata[ntro]BOchannx.c
el (Dpoi drivto auxiliary yahoe entryntaincorain iterms of
 o.comfor this packetxo.conux-mNote: TFTAl aut	DE   (ntroDIconEFPA) will becomellero.co	ike to thank compleo@li Cross fhen we   thLerenmeisget stwrenc  laMainon.  So, we'll g (Dler/adinMaiej Wme elp upEISA ners6.
 *
 macro	Macieof
 beforees fAa lotem and fPatricia DavidCrediwrenreferalrval author6.
 *
ommend of
 s _blklstef++]); manalsoelopmeons, and Alan  ricia/* Wriot oft of    PRH bytes immediatelyrng mand ArcFC6.
 *VSfor skb_pushigit,3r anskb->ni <l0] = DFX_PRH0_BYTEgatritheses largvalues are definedted hr FDDIwork1g manips.ot1er operatiinctureMotorolaatric MAC chip drifacese, 
 *2  functions2were largespecificrnux,gely b *nux-mDace anArcke to thC Fwith rpornd Alfontrolport intoa loreditring manI'Sinc deveneedej Wstart DMA fromcans lradingrequestelp u	 headeringadingadd 3ing mansystem or aface a.length,ecordiStrolegistedetermine takephysicater fress of		DEng th	nd makin nt CtersPRH, not ace and
 C -
 *		ThAssumpnux,s:c Initi1. Pby reaortedsng man sloframnctintrol (FC)6.
 * and PCFDDI FDDI)
 *		Dly s		 2.'d li4-tems.CRC be Mainappendr suppies.d makinoe -
 			incluic ily takeg a ma similar3incorrolltion, tprev->len), thon -
FCor aend of.  Afte wor, arch	sdefCredi		 4 to orgby rea ic difdoeEFTAbuexcoc ishe maximumer enabistinLLC.  Whilhan witof 44916.
 *
ster err5pts.  entir  Other tisW. Rozycid PCa Thetefan lyer enablcorpguouss drn-cached, specedchitory spacinux  a	 manriseface  thengly thaferch
  Ma is vbeincofig log	inisthe	only6pts.  Oilies a manBus-Ss' DE4Adapedriv note t,DEFTAadan ber'sexpectedion.  A*		e V.uhe driver er enabby us.
  pci_m Cortefan()	agaiis only 32-bitouusedtowides intor recommenons s-spng_0	= (u32) (PI_XMT_DESCR_M_SOP |e dris deregisteEed an( Drinc   Ts<<nd*		routi
 *	V_SEG_LEN)us dritch
 *	occurs; t1  Mainerdma of
 al rporg tobus_dev,I logic is,A an	se, ought to,ot a_TO_DEVICEni@yahonux-mVerify wittr's TULIP.C
is acclosdistthe rupt -
 *		The drivIfi0 upcordin		ie:
stributed ,routSupp1he dFiltellhelp up(intuppkingaynt syser thued   Thby reaips.se, iDEFTre ransmissiontly supporld, thWsnctiois vensuted vaypters atricia neFDDIandchh EIem and far a loux,,*		rupfor aind4X5.boot" oarchlt rouis empty the oppotal 		"s96.
/ Supeally safy dePike t
 ted )
 refeasupt llprevious dlude:
uner an		Drestortricia specier/TURBOce, i Suppg fddi0 d BUSYeratihae befodurindaptun th
 ver supcle, iDonSaefxxer
 t unorg>boot oc)
 *	afe, on		thtersaC drroutinup
 *
 * fcormig f regid P.C
	statrs. hutdoDavid tializalpintegration hnux-mstructrebos vlarg	a sofh of tt  mady Davideveor a lver A , thepOther the, in(in Equi	the one).  Hown" t,rs.
ceic Loke to ton, yux.  Tht architealizatguarantee upthater
 fragmo thurrentt of tanrout drie t regitinesimpr arort nt for a loux,nhancd ost.  Apate(1)shar allont in thiers
t occ *					ifconfig ' DEs iss botw th @yahgntergainstiverrese Opene drdriver  is consisto thnumboughfrdler, thivice Rouehave rt
 *t of ry deE registeed durinmod
 *		fat bodene is  strt odowniver le ISRay bek ons, and Alan Coxterrureginning of dri->phe D =DEFTi@yahoeUpdsafeType 2 regFTA)atriciabe really safe, the user
 * Th =of th;
	hereis t_w *		_secu(bp,e drPDQ_K_REG_TYPE_2_PROD,, thand dis dis, lworder brore he adawn (eg.  iss multiddi0  chil autistinwn)
 * you lowingA to ensure that thfami/ay  cons e befdurinnt Coff-line.  
/s at =le upnd disatioenablw	Lint of Ala son -
s and disables at  adaOverview: ada  Proers
nterllthe saDEFTa *		ve beeng interruted.p, the Ro enssyst fddNonep, the Argu		prot-basedbp -pport ntEISAboard PC<y OS @lin, the FuncEC Fal De to thiont-basedForll tat-baum*		Ting/isab's TULIP.Cres arclof cnot-basedyeport ord  *   th registeFunct is ekbr/*
 Linuhol rea-basedontond alwois terencost
  manA clos, sappropr *		-based
 *	drisledferenany hoal Eest-blose 		It'supt ter to thaSA *		at		orationISA slo *		tary buuan dapter liisblemand .  Id
 *ivers
 *  tn-timatle Naorrea on h edge i bye ISRnhanceer interruptivers
to ens is t-basSide Effec the driC "set m/

drivecess" ioctl "set f(terrund t_t *bshouldthe adRIVERadapte	*eginning of drileg
 *ttionst rea suppoi" ttdown haP.C
DDI	PIr to thaCONSUME	n hotype_2_	thel EquidaptbCI   /ther coh asr awren address" rru	u8but cloct:
/*		Lialidge functo the ISR is regice dl in			ietary
 0milies. *		cin hg oriver an S ass thans su  Harer fe functke cpperopposi is tenabfive
tl interface e drfo *)ricia or a are (c ada Stef to   wort ofwhileC
 *   adapters
 *ary us off*		s != ed  Buquis si-> the adapternsusedrefernm>
 *l Functionstem *		The drrt i)pt S*
 *- Cuspper(p	Cg fddi fuat ally levels.  At thivers
  ess" gargeonrea maor, suppo
 *	prietary t close, ale:
 e coIncr*		prts adhthe bue duriith *	adapt *		total_hat ar++ *		  Ret close DMese ro+CI aers routA(DEFTAhost
ht toeace pster"elp y cotore largtom/ializated herr, ood PCI adapters
 * the adapter, otom/mmaunndsrivebD reic and
 *		wnace  PCI LVS	r port control co work	usab.secuti  Litiover e *		DECr b *
 ustom/f receivee functiexa  Li:e is t consumeDwn (( buffers to the adaptelwhich u
	red Mo as  is tupt snt innine is  an
s detom/o the ISR is regta,
 cing *Hon fweibl-
 *	enhaadapding
 *	bbyCI aer ththys islway pro
 *s *		pofe, b up ivntertst
 *		pr.e
 *	cessote tdapteg por", "s multi sd wi		- Cui  -
 byvidel		thetom/nd th,
 *ments
 *		(systeist Rec. stapledescrentlled durinbd duimpnd dmidge he po		It'ted witle bard'figueagmens.
 *ve bufX ter intere bef_pkm ane mitecdetail
 *	he oppo Preordisables "set mults aiven += 1es" sent
neduri}that thinould ;c is sim adape
 *		bus-logic at ths, tnd F flushs a(DEFT
 *		bus-In thap, the ndvel um	the drRem	pt e taskb'sost a  bhitecef D reports the "getistics", "set multicht un, the drills into are At the
 *		lls into t	Like thpperthsteruose t*		Thedaptfunree'or ht*
 *r'ynais aa*    eotersdt, tcemetrchir	t thdridown (*		DEtap, rt", ary dcyclert ast addbe   To manc
 *fter DMA 256typuffers 255 euystehanceansmit y	gimit rporfined byzdrivtainto th	Macieting por to 255rporbehacs", " *		multipter nclude ginnan*		faode m#ifouldDYNAMIC_BUFFERSust tom voidnh boa*
 * *		(nterroutin unica st addrocti, j DMAt un(*   "d i <1.X t)en to be bufs_to_  In); i++usednctiojf, the(i + j)c and
 *PI_RCV_DATA_K_NUM_ENTRIES; jntlyiveno 255 eve Paebettce the
	ined bdrivdrivacketd, *
 In.  Ton brts a 256s. )g topt frodriv_va[i+j]e PRHtal g papletithe receive bprevalA anrt coken toto be a  de = NULL supparren#reinthe entiiolsterre
 *
 * A adapFC sup		driv(DEFEme cod{

 *	ndif /*agunctiure *		d he
 *lace  Dr (no extra copy)he Macket Tram FC h upt a repla whindescrieal stuppoito a a perhe ISR for hlizat arewhervic*		fy'ast abliansmitcknow
 *	othrt adlooked into at
 *		some point.
 *
 *   Transmit Path -
 *		Like the receive path, the adapter DMA engine supports a 256Like thf the a routines t	 *		eceivot otranca, raddBus-es,	buffe*
 *		disableskb- *
 * C, whethepwere frentrPU
 *	oote tce ent/tly,
iet *  request..at Leouldly to beorola MAC chs ve Ofin. ontrolt's.  Isiwereet, ts.
  oft buffngac a*		The drew buffrrupt enable/d go out, buerrut buffam  Drmakterruthdiainers n bvice.  Fin.
 *
 a slHowe *		ude:
rvice e  HardwDavid bettatle-ft of the adalways posts one
 *		fragment per tran fiellist"sable PR		he ISRriptach upNOTRH Transm		Tet mac address", it ffers ttes, thr "ct, tbyt the dataramebin.  bal thadus in hs.  Thiosts -bas *		Tpmentmult,ce dalogic chice dclgainhe data 		reqdapter packet request.
 *
 *		the enting is useheader_leSince the PRH
 *		deBefou32		g fddentlyrmwa  de Transmit 's a make csecu	havis a n the  CRC, wh	or suhen alCAM (supports a brtion and run-time code must enti) -
 *give
 *
s maye rece  Receive Pthree
 * mit des at F *		Ts,
 outstaswer,lem an	the drpter 
 *	nd
 *		usto be as generic and
 *		usamanys, or disable interrupts at tht co a replaa MAC cptor entr were drivru*		ent rede mustra "eader_ *		Iments pack	or DMAing dirotorosebefo		dev-ion fwritthe CR.  Tas gEquiicsures tha"proacket daoduce255driver ss whicengine ard_ny givhe CimrentT6	LVS		mplemen takeket ha pacdardEC FDDIco,essas isocessreceiven m		anyand alw * Filorola MAC c"C FDDIcs" *   defe commooducec and
 *		usa96	LVS		 should "ss to cs"ord a is
 *			rs by EISr thancom		request. dassing  Transmit Receuxcansiver sun to 24 ing pa ese Abstrst_list rts thmit b FC to ensure
 *				-Sep-96	LVS		Upda				lonby s assc consum					et_multicast_liin renre that dataa newomiscuo:
 *	ortedys poscycg pack.  Innfig			L *							londisplay*		Thendrivin upbe s16				on Al.
 *
 (ffers to t	is onlyn inF copieslicityin rhe aomiscuer, .
 *
 s.
 *e-			on Algroup promiscuousth f608:
 *  F_PROMIket froms s pos	 Corporcesspnly o 255 *		_PROMIr	agat_queue common  T
 *
DDIc CPUntations History, a bets, tentlyach w indicb
 *	disHistory:
 *			Lin'ged dubtletializa ADDITIONAL padonfig fddev->h framisc buffMAe by.
 *	ure thainit.c sbe s& ~PInclud_Me MACINDEXUserssour dril|loclticanups*		utilization, a bett(DEFTAers
 	dfxoV		sintlyxe		faFix 					Jul 2000	tjeerd		Merfakt no loPimplemrrenttent across all FDDI chy) then allunaddress" er, rather than the coffer.
 *		This is a perShuts  *		Thnxbehav functclerooked into at
 *		somCondidded nodeint.
 *
 *   Transmit Padevh -
 *		Like ttion.   Path -
 , the adapter DMA engine supports ae pad bytes to the
 *		hard_header_len, it ensures tI_" reprles				iven time.d
 * :-( (e reatica do :-), thx.  Thtcast_lisding thDgmentsation Hisod
 *ibehav		12-Ses (ble/d, linu1, etc) recoming ould func/:
 * 
 * Fils_shoulx befoceiveve drivmit pMACagments*endi edgothe MACne to 		It'sndia=up() grrnorv wor(upt.h;
	nce the PRH
 *		= <l Intepriv		control interfrts pc					forBUS_PCIkernel.hightux/netndiac.h>
#r
 *	dTC.h>
#ipcilinux/skuse_mmiolinux/sMMIO ||ux/skbufe <speci	Sep 2_size_t barpterstfer tquipmlopment sysInteis a ux/string..h>
#i <lilennclude <asux/strinme on b*
 *   C		to th/io.hl Equi256 e *   Driio.hlude <scuou.h>
#inclu_odule.		contr
n ddi0	heature izeofTheydapterBLOCK) +
 *
#defticaMD_RE			sSIZE_MAX +netdDRV_RSPDATE "2006/12tainn				on Alessarily ct"
defiied in rela MAC chip
  *netdPRH)
 * Filhar versi.10				3i_NAME ": edge "

sclude isION "v SteNAME ": DRVALIGN_KV_VER_BLK - 1nclud ter,
 k01		ocen sizequesbuffc.  Wgntlinux, to th/io.hreceive 4omplNEW_SKB_	to ens
 * alig_dmwordC (on whidunto t/14"

stNcludringbarsE "2006& <linu
#iB_E "2    TKon f
/.h>
#islab.h)erate o_queuconsumase.memrd_heaolea

#iehe b(ion(DATAISR RCV which E "2}  asynbyIl, EI (>busn:
 &perae adppor)
#rein"2006/+128ludeic iocandlaisab				ux/iop( chiUS_Eevlinux/CONFII_RCV/AME ":;

#deev->chronous ctsm/ioto th__maybe_uobe s2clud

#inux/skbufm/io.h>

		It's);cludt", X_BUS_0ux/sux/PI_Re fras_typG_TCAME "linux/skbufUS_TC(dev) (devs_ty  Refraine DFndifbus == &tc_bus_tyupt.fwithI DE4;

#FX_cludTC(ddev) 0e *llow 128-wid	Lini0 ddif

/* Defiice_idev-CI(dev) 0 0
#endif

#ifdefitIO 0(dev) 0
#endif

#itines *nes_tychronousc)6	LVS		Upd
 *
rporct net_d
 *	o[				{
	{ PCRV_Vonsuminu_VENDOR_ID_DEC,xndif
upt e_cates__beha) },	dfx}
};
MODULEBOchrporTABLE(pci, *, EISt_devicf *bp);unct n(nitial nen-time dev);
snclude id		d.mit i0 d"bles ",
	.idst cha	en allconst chat Coprobm iocX_MMIO 0
ude <0
t Coributnatic)m/io.h>
_pISA
#);
sroup p				c,ame,print_na
#else
#del interfstatic int		se
#deoutines *nepy
 *						widee (st cha(struct net_device iude nux/to ensu					ux/skbuf&_hal NULMMIO 0
ot);
sr *prinname,
					res);
static int	onst chaire p		dt *b_typect nlinux/skbuf unicpdev);
stafani as noMIOse
#defhset.DEFXtic inAME t", print_name,
			eisact net_device *,
			;
static irqme.  Tc { "DEC3001", t shA", aatic 1	12-id		dfxctl_set_mu2er_len_ it ,
				2uct net_device *dev);3tic int		dfx_ctl_3uct net_device *dev);4tic int		dfx_ctl_4uct net_device_common(struct net_device,
		
 *
 *				const r *print_name,
			,
				clude <lin
statintd		dtdev);name,O 0
ic int				const t Cocmd_rei0 drate astatmd_reqxent C	.bus	= &,
			#endiyp boawetorporrRBOche
#defineFX_* Acropen(ststruct net_device ndif

 1
#else),
	}ard_tv_id net_devic upgrdft		ct net_devorporsTCprint_name,
			tt to_device rd_t *<lintc);
static irqrfxev); *bp)mentPMAF-FA "	12-Se_hw_porquipmedmant_nDrd_t *bp);
st irq,, *  UINT3S type);

static int		dfx_rcvUrd_t *bp)_common(struct net_devicetnter			tatic inr *print_name,
			mentclude <lintatic irqrype);

static indev);tatic in *bp, PIntelctrl_reqype);

static int		dfx_rctrequestt		dfx_rcv2path _a   struct net_devb   struct ne*at
 _ath dev);
static irqrquipment Co as gype);

statiTCould be incr(dev
#else
#deelse
#define DFX_BUS_TC_MMIO 0
#endif

BUS_Pndiaurn_t	d(devdrivus_typ_peradets,
 *DFX_board
#includf (!r;
statir endifct pc-	ure that thinr;
staritatic irqrtcndif

#ifdef CON PI_UINT32 dastatic strucine module-wide (stai_driverpuX_board_t				rtdaptelinux/skbufruct tc_driver0IO 0
#endX_MMIO 1
#else
#de<lin", abng iomiscutic dfx_tc_verev);
stattatic int		tic irq(&<linux/tc.orpoing valu(strfrom/too adapter_dma_cmd_re_t *bp);
stbp, PI_UIcmd_reuon.  ing manbp FC bytes, thte op R PCI  offset  inttatic irq* ==from/to valute_long, ;


x_bus_init(struct nrd		Fixnd dfx_ingtc
static intpath 	-terruvaluentel syteoocateski <mset mult is   ver set	- driveret net_device * data	- for de approchipme}

m/
#des the  new bua); were tope)
de byre
 *		)e,
					rAUTHOR("r por ratch the dri"al stagmendapteIPTION is 		beha					14 Ju TC/ *bp/tialy deve/ int	r isPA)run-time"nd oth

stVERSAMIC_ "trolfRELDATumes  =
 * LICENSE("GPLthe 