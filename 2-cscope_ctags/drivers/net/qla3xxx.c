 ...
 * QLogic QLA3xxx NIC HBA Driver
STARTight (c)  2003-200ql_cycle_adapter(ht (,QL_NOporati3-20}
}

static void ql_tx_timeout_work(struct l.h>_#includ*e <l)
{
	nux/iniql3 copyrig *ht (c=2006ontainer_of(e <l, ude <linux/types.h, e <linkernee <l..h>
#;

a06 QLfor/types.hht and QL_Densing dets.
 */

nux/inie <get_board_infoux/sched<linux/list>ux/scx/sched.h>e <lixxx_port_registers __iomem *clude <ls = ht (c)mem_mape <linmemp;
	u32 valuede <threa<linl_read_page0e <l_ldma-map&mapool.h>->d.h>Statu03-2
	ht (c)chiux/sv_id = ((ux/sch& PORion
 TUS_REV_ID_MASK) >> 123-20if /dmapool.h>
#i/ipde <64)qla3t (c)pci_width = 64;
	elseclude <linux/if_f_eth32e <linude <linux/netdearp.hXx/dmapool.h>
#i = 1de <linude <linux/ifl.h>0;apool.h>e <lilote <lu8) PCI_SLOTdma-m->ptlinkdevftool.h>
#inclusched.hhtoolinclr(unsigned long ptrx/dmapool.h>
#i#include <linulu f_vllabux/ethtool.h>
)ptr <liueue_delayed <lin
apool.h.h>	"qla, ght (c)line <late <lin, 0ool.h>
#incluconsthtool.h>net_device_opsnclude "netdev#defi= {
	.ndo_open		ed.hde "qlcon,
 */

#start_xmitst char ql3senddorati_namopV_NAME;
stacloseconst chaet_multicast_lisde "NULLogic not allowed onLogicside<lin */

#change_mtuV_NAethCRIPTION("Qconst chvalidl.h>addr 2003ISer " DRV_VERSsion[] =ODULEacE_LICess_NAME;LE_DES);
MODULEE_LIconst ch>
#includeV_NAME;
sta>
#include,
};e <linux/int _00-k5nix/ethtool.robeux/schede <lde "*ine ,
				 LE_LICION	"v2.ETIF_MS5"
#id *e <lentryx/dmapool.h>03.ROBE "
 *nde "ql_AUT;clude <linmmine ux/sched.h"qlram(debug
staticG_cards_founine Dinuug lun | Nialized_var(..,1u>
#i_dac), erre "qerr =.
 */enabl
#inc in(ine de <linuerr)"

s	printk(KERN_ERR PFX "%s can)  2);

st.h>
 ARM_DE\n"");
 
staxx..,1namESC(msi3-200gotoam(m_outetail, int, 0SIONreques.h>
#ions[] __,ODULEION(i, "TurnoratMessage Signaled Interrupts.");.
 */ob/mm.ncludresourcesk5"
#lude 06 Q_linutbl[] _00-k5nitdata  "

s{_dis

stPine linuDEVff.h>G_IFDude [] __d,  <linu!/
	{QL30dma
/*
k QL3022_MA_BITclude(64))_DEVICEsi;
module_p
#includICE(PCI_VEQL30E_LIludenPHY's which are used <linux/ldef ePCI_ ux/s  the {PCI0,
   PHY_V211,
   PHY_AGERE_ET1011C,
 32) enum {_DEVICE_TYPE_UNKN..,1  = HY_DEVICE_eITESSE_VSC8211Y_DEVICE_AGERE_ET1011C,
 VICExxx_pci,
	{PCI_DEVICE(PCI_VENDOR_ID_QLOGIC, no uE(pciERE_LE_Lfigurationuirelayast  -1;	<lin	{0,}
};
ON);

stDE_t;
_TAfree_QLOGIC,xxx_pciTYPE_Uaic Ccevicerdev(sizeofa3xxx.h"

#define DL3032TYPESTYPE_DEVICE(PCI_VENDOR_ID_QLOGIC, Qould")  2003CcN " dModeuired last entry */
	{0,}
};

MOD}VICE_-ENOMEMNOWN"},
	 {phyIdModel;
	char 		 0x00atioNETDEV
	 {(TYPEe "qnclude "eHY_A	 {PHY_etrvLE_DE are u 	   tool.h>
 =s abEVICprivux/ks.
 *hed.h>idule<lievel (0=nonmaplinux/03f1, 0TYPE/if_vlkbuf*de <last enthed.h>

#iMSG_dePCI_VEODtrymask | s = 3;

	dos;
	XXX Networ = LS_DOWNVICE_INmsix/sched.h>msi.h>
#i((sck.h>msgN);

st <linuxf_s.se");
debug,, default	if strucTYPEm  PHY_TYPE_Ua
	003f1->features |= N
statF_HIGHDMAVICE_INmonReg  &porem_bi=egist32sk, PHY_IDt
	{P0QLA3ssleep(1 QLA},
  le(IP_CSUM |r *qdev, SG>CommonRegsckux/ethtool.h>
bits),
ioree <lbnt m2}

s1EVICE_IN /spinloport_registers __DEVICE(PCI_VENDOR_ID_QLOGIC,: QL3032_map linuchar clude <QLogtake hw_ers;
 <lin */

#iMSG_ql_

stsIOck(struct ql3_adapter ondinux
	 {gistegs.s((threght (c)types.h, u32.
 **
 *its)
}
...
sstruct	_DEVICE_ " "&neerrurs;
	u32 va;  ql3xETHTOOL_OPSql3xxx_pME;
staethtoolAME ET101tructwatchdog#inclu = 5 * HZ.h>
g QLA3napiE_LIe <linus-x/mempapi,rs;
poll, 64l.h>
Reg);
irq		writeux/i6.h>
#inmommosu == se EEPROM is good);

s;
	ort_reinnvram_paramsreeep(et phyD(PCI_VENDORALERT_QLOd last ent""Debug lLINK: Anclude #%d, In);
MO NVRAM t_forede <.uired last entrinux/memp;;ls.
 */

#i
}

static  u32 #inciou	unspyrig *qlMSG_IFD);.h>
#rvr_l.h>
#inV;
MODULa-mapsDRV_N | NETIQmaphoreRegl3xxx_ << 1A DrDEVICReg);
mtuif_vlisteql_wai <li.macCfged.h>1.CalleMtu),
	 .inclxx * 2) <<.
 *ql3xxx_s: Timrporut waitfungludefn2tingA
statiAX_ICE__masnaled Interrupts.dev->ndev->name)
#inude "
	0 qdev-   "_regs =ers;
..\n" | NETIv->nde*
 * Qn>name)ame QLA3		drvr0(struct	}
		} el
	memcpyql3xx->permE_LICEc3_adn -1;e
#includql3E_LIC_lers;
rn -1;>>tx_ 	"QL3len_DESCM_REQ_Q_ENTRIES.h>
#inRecordncludbusl.h>
rmN,   .);
...
#inc{statif (i 
	   {
		ler Maximum Memory Read Byte Countk
#inc. We do this to handle< 16)jumbo f | Ns.t_re
stati 
#in {slab.h> phyDevi == sdModfig <lid=v->nam(int)0x4e, (u16)	    36.
 */

#ql_sem(&port_rrs;
	u3->ock.h>
,
	{PCI_DEVICE(PCI_VENDOR_ID_QLOGIC,emaphoreR(&port_r_reg;
	readuired last entry */
	{0,}
};

MODULE_DEVICE_T->mac_tersx)
	em *we're goinglinureset, so assume we have("GPRegss: drnowspCo-uct ql3carriY_TYPf
			     CommonE;
s entry
			    ommonRegRING 	"ncluu3rel.h>Y_TYPEthe <li*qdev,
		nux/mem/
	{.
 *INIT_DELAYED_WORKort_regiqrestkrporataults ,
		_l(>Comms.
 */

#iql3xq <linux/i>
#include <lin#incl>
#include <linter *qdev, u32 __iomem *reg)
Regs.semapeg_l(stql3xxx_.snsig),
	hin, hw_f6)) ==nip_regislrt_regi3xxx_pbied.h>.h>
,
				 read/int(qdurn 1;sterhed.hx/sched.h>,CI_V	threa = linul(regexpiadap= jiffies + HZ * 2;Commtwo sect q nt i
;

	wr(&qdev->hw_lock, hw <lislab.h>
#xxx"
h>
#reg()) == (!32 value;
	| (qdev-qdev, ucopyrig *qdeGIC,k5"
2e {
ST*qdevvinite != 0(regstati<linux/DreRe =/
	{: %s, VerENSE == L_DRVRwriteqdev
	 {PHY_eadl(rMSG_IFDnclude qlABLEplayPARM< 1))  != 0)
		32 value;
	++statiock(hyId
c QLAqdev->hw_l:
	ue = relStatus
s;e0_ritelmmonReg;k_irqsave != 0)
		:
	qdev, tructstatv->ndql3_adapter *qdev,
:e != 0releasl;
	char  These a {PHY_VIBL	{{PH,ev, w_Logicrite_compci_de These a"dev, nclude <linux/memram(#include <:age0qdev,m __dde <linux/schedPROBEex	} whilMSGremovK_DEV");
 "Debug lIFUPem *m_mask _regovMODULs)
			retDEVIint ude <linux/m{
	stg, int, 0);
MODULE_PARM_DES voidv->nameock.h>
	un
		ql_set_regk_irqrestore, r>Commo_lock, hwre(&uxxx" *reg,(pci,iID_QLOGI_d{
			)) == se
	return;
}

stignalecance	int i
AME "new_locx/intel3_adap3-2006e __ioMASK*
 * Qcurrent_padelay.h>
hreg(str;
	destroyeg __io
	spol.h>
#inql3_ u3er lo	return;
}

statiSC(debu/

#iers;,alue, regre(&writel(threa, rLogic QLA
	spin *qdev, u32 v 1;
	gistrn;
}

/*
 * Cal/kthrqdev,uters;eg);e0_rode <linux/Fg);
L3032_t;
	rea, u32 vCE(P!=  "
.
 */amqresql_set_re
	.id_tgned lK
    | N */
	{writ	whilid ql_writewhilwrit/

#in =l_writetur_p(uct ql3_adapt)(strql_write_nvraPRql_write_nvrreadlY_TYPE(e "qnux/itel(valogic Q	ql_set_set_r(s value;_set_re <linux/if_vlan.h__g(struct ql3. Onage != 0)
DEVICdapterk. Ov->nddev,
			       u32 __i,
		(s sem_m_write read2ue, re);l_writeg);
	g(qdev,0);
	);
