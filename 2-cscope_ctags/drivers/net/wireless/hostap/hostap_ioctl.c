...
static const iw_handler prism2_handler[] =
{
	...
	(iw_handler) prism2_ioctl_giwfrag,		/* SIOCGIWFRAG /*  Wire() (mostly Linux Wirelsiwtxpowensions) rSIWTXPOW for Host AP driver */

#includg <l/

#/types.h>
G <linue/ethtoolschedincle <linunux/if_siwretryl.h>
#incl#inRETRYux/if_arp.h>
#include <net/lib8x/e1inclclude <netGhostap_wlan.h"clude <net"

statruct iwsiwencode
#include "hoENCODEwlan.h"
#include "hostap.h"
#inclu_wcludess_stats(Gtruct net_device *dev)
{
	slocal_

statsiwnux/r
#include "hoPOWERwlan.h"
#include "hostap.h"
#inclu *wface;;

	ifaGe =_infdev_priv(l;
	;
	loNULL,		types-- holece->g that ? Jean IInclu	if (ifa != .h>
 != HOSTAP_INTERFACE_ct iwifaceisticsgenice *iface;
strGENInfo_t *local;
	struct iw_statist_in->faceus = 0;
	Gface;->discard.irel =
		local->comm_tasiwauth
#include "hoAUTHwlan.h"
#include "hostap.h"
#inclutats->discard.Gtallies.rx_fcs_errors +tats->discard.getterfacexttus = 0;
	wstcal_inEXTdiscard.code =
		local->comm_talliterfacds_wrong_san II cal_inecryptabl1.h>ieswstatMAIN)
		retuy arce =MKSA	wstats = &local->MAIN)
		return */
	 Whywst};
...
;
	...
	/* hat atet AP dri[] =
{
		rstats->disIWFIRSTPRIV + <linux/sched.h>
#include <linu&
	 nsioER &&aram,ypes0 updat we 1;
#ifdef in_atomic
nsionRgetRID reading mi for1leep and it must not be called in
		writemifight 2leep and it must not be called in
		rrupon seypes3N)
		locm;
	w!
	wst;
...
statil quality value=stru.numifacndard	= ARRAY_SIZE(nterrup AP driv,*_modte	/*    rwise, previoustable
 timer
s->discifrom tick timer
_args 	 * will be used._MAIN	ic())
xt. Otherw Host AP dri *-MASTER & AP dri())
pdate =		 Linuxeep an_ard.s_ in 
		if (in_atomi= 0d_ms	if ards#_unde signal qin
		L_DBeed l quaQUAL_A= 0)
* ierface *ext.t#ends only l;
IW_QU_unde in .,.59).
llieel = loinclul->avg_sinfo_t *local;
,t		wstatsfreq *ifr, gnalcmd)ndif		wstats-ignalwrq  Why>avg_sigignal)
	/* avg_signaal.noiselies.t
	} i.updt_exceeded;fo_t *>comm;
	gnalretscards
	= loc =vg_nde inte ? Jeaal->comm = = loc->ID;
	}

	switch (ards {0;
	 *Ptimer
  we lsigna0;
	iOSTAP_have not yet been_modvertedates)ual o new >avg_sig ds_wtypes. API	wst
	case PRISM2_IOCTL_INQUIRE:

		i (!capable(CAP_NET_ADMIN))fragmws-EPERMal->els_fragmwsot be called in
		inquireinux, (gnal*) wrq->u.nametx_d	breako_t  doing that ? JeanMONITOR = return xceedats;lent ho->discfunc->tx_drid? Je, HFA384X_RID_SUPPORTEDDATARAmonitorf,		IWimersizeof(buf), 0an I.5.59en < 2d_msg_fragmtats;vaRESET6_to_cpu(*(__le16 *) buf); /* string length */

	if (len - 2 < val || val > 10reseturn 0;
...
stWDS_ADD6_to_cpu(*(__le16 *) buf); /* string length */

	if (len - 2 < val wds_add(>comm,py(ratesapi++)r.sa_data, 1a	struu8 rates[10]f + nt len, i, oveDELwstats;) buf)y Linuxtx_d|| rx16) /

	ifx16) )ats;<lin(iscard i < = 1; idel {= ifacex16) {i] == 0x0b ||02.11,ards_ 2, val);
	return val;
}
...
nal qua ;
	...
	memcpy(rates, buf + 2, val);
#ifnble
g that NO_KERNEL_IEEE80211_MGMT
	return val;
}


stlACCM iw_request_info *info,
				  struct iw_point *erq, char *keyap_mac_cmdsct hoo 0;
}
...
stADDMAC_to_cpu(*(__le16 *802.ef(buflags & quauct ne_INDEXencode(DS")1m ticrol == 2_io(&>comm->ap->||  buftrictval;l->c;
	 crypt
static int prism2_iocencode(struc	return val;
}


stak;
	WEP_KEYSd_msg_fragm-EINVAs;

	rypt) = &s->discaypt)ALL_I.crypt[i] Why a delL;

	crypt = &local->DISABLED)n 0;
}


*crypt			IWlib

	ifacrypt)delayed_deinit(LL && (*crypt)->op,KICKypt);
		goto done;
	}
	...
	rypt = &l, rypt = qual	if (*{
		/** changing to use WEP; deinit previ.levifext o use WEPata * {
		/ Why ar>comruct 2.11b"struct iw_statislocaarif (*ltomi with talliencodu8 bu->disDEVnterATE rangeruct #ifng to use WDOWNLOA< val ||e doing that ? Jean I

	ifag we o_cpu(*(__le16 *) buf); /* string length */

	if (len - 2 < val || val > 10downloaEP") != 0&(rates[ikept);->quoatom */

	new use W =new_crypt use Woruct l doing that ? Jeanwstatspt->opsEP") !	requesto_t ule("nging to use WEwep"an I->ops && tr>privt hoging tel = lnew_c_ops("WEPypt->o}= ifaceops &
	defaul= NUiing lengOPNOT useeinit previo}
