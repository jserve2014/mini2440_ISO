...
static long cpia_do_ioctl(struct file *file, unsigned int cmd, void *arg)
{
	...
	case VIDIOCMCAPTURE:
	{
		...
		cpia _PiA d= match_
 *
  Sup(vm->width, ia Cheight);
		if (
 *
 * Supp< 0) {
			retval = -EINVAL) Co	breakpyri} Copyr * ( 1999-20!= cam->ght 1999-2Peter P (CpyriScott J. = 
 *
 999-2;
cott/*lt <joC Suppis changing, re 199the subcapture areadriver	memset(&00 Johc, 0,
 * (of(m is fr))s@erdfset_vwhannes yo Joha000 Jocmd_queue |= COMMAND_SETFORMAT modidisps>
 *commandsnd/or Genify
/* according to v4l-spec we must staratioreamhe Fher 
 * Th00 Jommap_kludgdfel1pyriregler
 *ectroni_fraicen/oa's. redis J. Bert}

	case VIDIOCSYNC:
	99-2int *99-2v = argredis//DBG("istributed  %d\n",pe that *
 * ScotY WARR<0 ||NY WARRA>= FRAME_NUM1999-20ur option00 Johannes Erd Bertin0
		switch nd/ou  that[Y WARR].staSS FOR Ayou darrantUNUSED:eral Public LiREADYfs pubre detailGRABBING
 * 	sefulsyncFreeunus WITWARRAHOUT ANY WARRANT MERCHANTABILITY or FITNESS FORor more detailDONEeivedURPOSE.  SeY WA00 STGNU   wlic License 00 ST u a co,ABILbut WITHO py oere detail MassABILESS FOR A P0 Scotur optioBILITYTR1999-20/* FIXME - xawtv does not handlY WAis nic 2 of tour option0pe desheT 1 *progra you d,
 * bGCAPTUR Fr999-2struclt.com>_) any lc*vc it will bembridge, MA>
#inclu\n" *
 * nux/vm you cat 199
#include <linux/init.huinux/scde <linclufs.hlinux/sc#include <vmallocnux/ctype.h>
#c_linux/hednux/ctScottc->decim eitnnABILPet   ed bHow should_  1 *beion, ?ctype.h>
#includLITY or FITNESS FOR A Pr verbtc->flagssm/ionux/ctyx/scEven/odd grab_CPIAsupportede.h>
"
 *
.h"

 GNUice hopcpia _nr
 * 1shed bClipf thMicrresolue <aswe cant 199for MERCROI
		   (every 8 columns and 4 rows)
 * Th
c->x_parafelt;
MODULE_D& ~(__u32)7 _CPCRIPyION("VESision>
 * (4L-2 of th3or VisioCamerA based "GPL");");
MODULE
 * ENSE(t 199( based );
#end")RIPTION("LI/delafdef E_SUPP== nFoundeosignc unimoduleJohannisioTI+PTION("SUPPO>includew.E_SUPP_conv, ushory, 0444)m(colorE_PARMA ba);
#enCABILMERCHANTABILITY or FITNESS FOR A PART a co%d,%d/%dx Publicshort,pace
defin"GPL"a's Colorspa *
 * mutex_lock <linuxULE_mA bas Visionyou ca.TION("Vbased T _CPIA_PTIONcamerasbaNU Gc)
# ABOUe CLE_SUPPOIPTION("SUPEM			(1<<5)
m(colorsf. Bertin utlt <* but /vmaversi publfyABILit underTMicrterms ofTMicron, GsiocameraunULE_SYST "V4Ls"
M			(1<<5)
//t 19format ignornet>yinclera durversition ver,onv,* so stop/r more d/ion; e * Thr more detail Mass as odul/ctype.h>
#includinit.h>UNIT#include <linux/ctypunit *vude <linux/pagemap.h>
#lfine<linux/devuJohannePIA			00 Johdev.minor("vidine bi_CPIA			VIDEO_NO_fineA_ termsradi 0);
 |er	(OUTPUT | 0);
ine auECPIA |  4EM			(1<<5)
ICPIA_MOteletex)
#d4)
#define CPIArms oGetPnPID	
efinpointlessf thimplement overlay withmutex.	(1<<5) * Tinux/init.h/ctype.h>
(INPUT | CPIFBUFh
#define CPIASfine CP_GotoLoPo8)KEt 19/* tunthe hoerface -r_Prhave non 2 of (INPUT | CPITUNERine CPIA_COMMAr	(OUTPUT | CPIA_MONGFREQne CPIA_COMMAND_T | CP)

#D_GotStatus
#define CPIA_COMMAne CPIA_COM10)

AUDIfine CPIA_COMMA
IA_MODYSERCHANTABILITY or FITESS FOR defaultegs		(INPUTD_ReNOIOCTLCM Ave,
#include <l1<<5INine (DATA_IN busyM			(1<<de <urn ur opt;
}

<5)
ic longlt <j_ioctl(e <linufile *efinPIAVe	(OUunsignedPIA |cmd,e CPITEM |PIA_Marg)
{ne CPITEMM			(1<sercopy(CPIA_M			(1arg,rms ofdoSetBau)			(1
cott )odif/TPUT | C 5)
#s of MasdRate
#define CPIA_NPUTlinuxm_s
 *_TPUT | *vmaetECPe <linux/ctypdev


#idev =defin->private_data;
	VCRegs		(INPUT_MODUL= vma->vm__MODU terms oGenericCalErdfefine CPIA_end -_WriteVCReg		(OUTPe CPIA_C 9)
#page, pos;#define Ccam | CP *camDESC <johget_drv| CP(dev(OUTUT |oPower	(
elay.!YSTE|| 			(->ops)| CPIAPIA_MOD3DEV_MOD a coPIA_COMMA: %lPublict J. _MOD11)e ABOU>c., 675y o*IA_COMAX_lic LiSIZESTEMI2CWrite	 Johannine CP_ReadVCRegs		(INPUTLE_SYSTEM |
#define /* makGmutex._really_ smp-saf 2 of ScotIA_MODULE__PIA_Cruptibledefine CPIA_COMMAPUT | CPIA_MOD1TRe CPIA_COMMAtware
 _buf) {_COMwe do lazy <linu#incas * ThScotbos shobune CPIA_la thatSetCe as )	 "\n  TEM MCPortMMAND_WriteVCReg		(OUTLE_SYSTEMoPower	(ule.hnym.pos = (PUT | CPIA_MO)PSoftware
 MMANo;
	whfine| CPIA_Mo.h>er iminefine  CPI_to_pfn((void *)pos JohannesreMasspfn_rangVideaABOUart,| CPIA_PAGA_COMMerms ofeHAREDExposurSYSTEM | 7)
#deeg		(OUTPVP_CTRLoPower	(OUT-EAGAINr = -1;
_MODUL+=efaultstVPCPIADULE_VP_CTRL | 8)
#dCOMMAND_GetVPVeMODULEND_	 ABOU- | 9)
#define CPlserCtrl
#dex/paceND_Sart		(OUTPUT | CPIA_MOD12ne CPIPIA_MODULE_VP_CTRL | 8)
#define T | CPIA_0	(OUTPUT | Ccond eitUT | C4l2_efin_operPIA_MOCPIA_Cfopiver{
	.owner		= THISg		(OUT,6LE_Sen<<5)T | CiverMMANrelenux/		(OUTrBalanceclos_CTR.
 *
uVP_CTRL (1<<e CP Mas CPIA_COM Mase CPne CP		(OUTPtoLoPoSetFliBau,
};UTPUT | CND_WriteVCReg		(OUTP| 18)templn, Inc16)
name<<5)"
 * (C(1<<5"e CPfine  terms oCo= &)
#defineine CPIA_COM	_I2CStopMatrix	(OP_CTR_emptyine OUT* T | ialisERCH | CPIAurPara <liI(OUT		(INP
#def-1;

_	(1<<5ND_STEMULE_SYSTCRegs		(INPUTA		(O/ab.he followvers<< 8)ne CPvaluescs
  MERC 25)
#du caom
Vers"Softw1<<5DeveIA_MO's GuidCTRLrD_Se1ULE_defs".  AnyhannesesFram
 * MERCD_GetVPVe1<<5no_AUTin tailents.PIA_MODIN << 8)s.colourP | CPIbr
#ennfine= 5pacePIA_COMMAND_GetVPVeSetGracontrasdefi48eVCReg		(OUTPU#inclEe		(#defsatuCTRL |_WriteVCReg		(OUTPUAeUT | CP.gainModCPIA4amCap	(OUwer	(OUTPUT | CexptVPVeFi2;definAECet	(OUTTPUT | CPICOMMAND_Getm		(OUTPUat yine CPIA_COPDefaulttarentreW5<<5)
#dMMAND_WriteVCReg		(OUTPUPIA_MPIA_M#dTPUT | CPIA_MODULE_CAine Expine CPIA_COMMAND_EndStreaPTUcoars| 7)Loefin85A_MODULE_CAPTURE | 3)FATA_ONPUT |Hi
#define CPIA_COMMAND_SetForredComUTPUCOMP_Rsne CreamCap	(OUTPUT | CPI_ULE_1_InitStreamCGREENA_COMMAND_GetVPVeEndSther  | 32Compresdefi
#defi2MODULE_efine CPIA_COPDefblue_InitStreamCBL 8)
#PIA_COMMAND_GetVPVP_CTRL .bUT | CnitStreamCp	(OCB)
#define CPIA_COPDefauA_COMMAND_dGCa)
#d3RE | 3)#define CStreamCap	(OU1CPIA_Compres6ompressionParine CPIA_COMMAND_lueCompres9CPIA_COMMAND_SetapcoramCap1PIA_x1GetVPVernitTURE amSYSTEM |AND_CPICOMMAND_EndStrea | 14)
#de4ine C2niTURE |defineUT | 14)
#de8ine C3PIA_MODULE_CAPTUflickerOMMANolutputRS2ODULE_Cefine CPIA_COMMAUT | CP32
#defidisabledefine _DEBUGnParetCompressionTargetdefineJu	(OU
		UT | CP_jumps[hefineainUT |q]P_CTULE_CSetDrm>
  CPIsensorFps.ba7)
#deD_WriteVCReg		(OUTPs	(OUTPUT | dedivisor]amUpload	(T | CPIA_MODULE_DEBU | 3w_AboOverETPUT | ine C-find__Got_ine CPIAPURPOSDULE_CAPTURE | 3)
#dbModSYSTEMdefifine CPIA_COvlOftribdefine CP2ND_WriteVCReg		(Oe CPIA_COMMAine P_CTRL | 8)
etFore CPIA_COMMANine 2COMMAND_EndStreae CPIA_COMMANComp	(OUTPUTOUTPUfinerAPTURe CPI| 3)
#dhysteresifine_LICAND_WriteVCReg		(OUTPs	(OUTPUTthreshMax| CP
#define CPIA_COReady to grab intosE | SteUTPUve rec,		/* Idule_cludcesUG			beinlargebbed inrrants.
 *	FRAMEReadyFreeint,e hootype.h>
#aH)

enumeterFULE_CAPTURE | 3)Coady to grab intofrDiffg, bTriveFine ot been synced yet */
	FRAME_UNUSq0x0000tCompressiOCPIAbeen py oed yetdrivearrantcense 	FRAMEUnefine MortPrerCtm(vindare;define 1))
#defPIA_ 
#define CPIA_COMMAND_SetFo*/esOMMANDtransfer_IA_COy to grab iine CPIACTRLtufineEXPOSURE_N<<5))
#de/* Set Sd	(OU FPSdFra15fps. Ttex.seemx.h>t| 31than 30fpUploadap	(indoor l
#enhe F)
#define CPIA_COad
#define CPIA_C_UNUS2
#define CDONad	(OUTPUTine CPIAtP/* Fin
#define CPIyuv_MODULEld.yms of tSEe CPIx/scF		0xwPRESws 2 of tUTPUT | CPIA_MODULE_CAPPS		0xNerms of tSE08MMAND_SETCOMPerms of tAPCOR		0ss	(OUTPUT | CPOUTPO.subSaIA_CO= SUBSAMPLE_42CPIA_COMMAND_SetOLOURBAyuvOrdene CYUVORDER_YUYfine eadI
};)
#define Oe COM.mDULE_CdE_VP(OUTRESSIONODULOot been synced yet */
	FRATbbint.frEXP_10_COMMne tMODUL*/tCompressTARGET_QUAle, EXP
#in   302tCompressNPUT 255
#2t255
#FRe ho5omatic frame raMAND_SETVLOFFx1OMMAND_SETC 140
#CEXP   7GH_COM94
#Qrame x2000
#define COMMAND_VLOFFSET	EXP4OMMANDqx3ME  _detecODULy to grab intoUTPUE   topNPARAe LIGHT_TI       70

/*bottomximum numberG			10ms loops touttD_RESmber of 10ms loops tcradbS		0x00x0400
#dght 1999-20or4)
#dCOMM_CIFel CPIDULEpOMMAND_1<<5)768;ame ra/* 50%)
#define vp.huETLI5)
#defineraerate][G | 5)
#de | CP_COMMAND_APUT5)
#defier_jumps[2][2][4] =fine CPIA_Cstatic u8 er_jumps[2][2][4] =whiteCOMMAND0c u8 fli/* 0);
5)

#->e hoyscale onlyps[2][2][4] =depnv;
ownlOMMAND_ColifyCdule_e, MAyn; ene	EXP_ACC_vp.pal80
#LoPoOUTPUPALETTE_RGBet_cuct(ux/fstA_INCPIA_OMMANm); Bertne CTCOLOURBREALE_DEs);
statiras"
rspac;
params, volat(5<<5)
#dvdexed by [wness);
statiparwms *ras"
LE_CAPTURE			(6<<5)********chroD_RE********s, volw.		(OUT******ght 199Mclipcoun the stream tht 19**y maNUL)
#defdefine CPIA_MOULE_DEBUG	NONM	(OUTPURfir GNU 31)NCE	0x04_SYSTETCOLO_SYSTEM |adzPRegsE_VP_CULE_VP_CTRL | 8)
#define C  50 CPIA_COMMAND_GetVPVeSTEM Void NPUTIA_Mine CP 
};

#define C 90
#deRL *

	mA		(OoveriH	EXP00Derms ofs ErdthE_VP_ee0D_RESnclude rdefins(OUTare;
	mem = vmallocEM |di| 6)
#def= opUTPUcamera  50ATA_IN << 8)
#define Peter SetPageReseefine CP_CAPTUREPUT | CPIUT | CPIA	(6<<5tsCE("varoc_Cap	************memcpy <linuxfdev, CPIrs	e CPIA_C*/
	adr =  tati, _SYSTE rptionABIL
#define CPid rvfree(vo PAGE_A_MO;
	curd *re <lpaceap	((6)
#defi <MAND_I2CRe; i++EM |  mortware
 *i]tTPUTu32 *comaroid *)adrved(gs,er P hopoSYSTPS	(OUTPU*
 *oCPIA_M., 675 MasRE | ,ge((void *)adr CPIAmem#defile.hge((vatic frameedid *rvmvmalloc_to_picen_************
 *
 * olor		adr +=**** *
 *********long adA_MO-= PAGE_SIZE;
	}
	vfe
 *
 ****************** *
 ********* 90
#mem = vmalloc_32DULE_Cgied (rn NULL= (_SYSTEMtSTEMir_e;

is pro,return*lowIA_MODNPUT | CPI|riteVPReg		(OUratCompres= PA
		s= kE | 3)_DEBUcurery *cpiadA_COM), GFP_KERNEL)) for *
 VPVersion	(;
}
...
sta, 0
	...
	adALIGN(PiA );
		size,iA dce
 *
 *era->ntMAND_S| CPIA= [29];

	ize); /s);
sproms	( UTPUT |  CPIA_Mlinux/ut MUST c		(OUTmem)
mpstrreturnVFL_TYPd | CPIER,
#definnr)-1;
ine CPkfreON("V <<  _CPprintk()
{
 grab i"IZE
	e, Cn");
	out or failP   
d-onle;
adr +lena's.fin/* g * cero gr T  90
#dd.%dine D, "rea:river/

	ad/| 17)	ze); /*ER, CND_Sp9-20
	adre need* Mas(outensy\n--f(o[29]t=NUAME_M8)
#define Cchar **staPUT |0 ST PreUT | CPIA_Mine CPGE_SIZE;
	tA_MAJ_dE
module		retusion:uon,
_VER, rameion:\n");
	out + %, "V4L Driv%d)\n",
Revision_VTCH_Rroc out ****
.vreRevi.firmadr RevNSE(n;
		n");
	oca
#ispaceCONFIG_PROC_FS
	cre<5)
gramrR);
...
V    .%39, .%02d (***
d)COMM
L Drf (!m->paramtCompres cam->sion., 38,* (C) 
	out += sprim->*****
.pnpID.devicevisionvisi     ca+=  Version:rsionVP-areRevisiovcon      ca%04x\n",
		       cam->params.vp      d.ntf(ntf(out, "VP-VersiovendorPnP-I_SYS04x:s.verscamuctut += sprintf(out, "VP-Ver   cam->parD)t=NUn.vpVersion,
		      systproductte:             %#04x\n",
		   		(OUTareRevi.vpvision);
	out += sprimVP-on);
	o*****
dreRevadID);
	...
	out += sprintf(out, pon);
	oabPIA_e);
rintf(out, "VP- GNUus.int,ut += sp	DEBUrsion.%dersioUT At += sprintf(ou         , "VP-HeadIDT | CPIA_MOD%d)\n",
gramCOMM%04x:un*        _dir_TUREy *cpivmalloc_32(s6, 38| CPIAus.fatalEvers
 *
 	      tCompres-\n");
	out += sweodulo geCOMMA04x\n" "E_DEB *
 *;
		sizustruc       c 25)
#LE_VP_C_t ofCH_VE--;
stTCOLOA_COtover v	       VP_CTPUT(siD_GetCon);
		      URE		ams.vpVe | CPIdestroyvers/gram/R);
/
 *
  PublicUTPUT ion:nu6<<5)ags           sion		         cam->pCPIA_MODULEt += sprintf(out a co		  vers.vpVer	       
		          }efine CPIA_over_e((vo		ad8)
(voi->pa(mem, tf(out, "VP-Versi%s v     . PublicABOUT  cam->partmentNPJ_Vated.qx3.qIN 3
#deted) PATCH 3
#	adr ode);
	/* QXWARNING "Sinc	(OU-kernelI2CSr fo->paconx\n",.%d\  0);
"	out += s"DULE_ed, itutto  50A
/* byne COMMANnow. Usersclude <mfix MERC	           ppliP_CTRLsM	(OUnux/they don't work(OUT
ouon; ms.qx3.buparams.qx3. | 3dsion,
		rsiondeb Pre'printf(out_:   ' Genulout, "VP-    efine CP31  50<linux/  cam->params.status.de\n",
		    , "ro(    cam->par1000
#define COMMANG->par__ex %#04x\n	out_staerrorrsion.vpVersion,
		      roiut, "   %#04x\n",
art*4}

r	(OSItream_, "stream);+= sprintf(ousprintf(oy\n-
#dexne CAUTsymbolsETCOM      |0x010
STEM T_SYMBOLart*4,           %#04);      actual_fpssion,
		          #04);
