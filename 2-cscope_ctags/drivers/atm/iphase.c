...
static int ia_pkt_tx (struct atm_vcc *vcc, struct sk_buff *skb) {
 ...
 iphase./* hw bug - DLEs of 0x2d, Interhasef cause DMA lockup****phase.ccif ((**** Author: P>> 2) == 0xb) ang@iphase.   Author: Pe***0x30;
 Author: Pmode = TX_DLE_PSI;com>       prq_     
_data  
	;
m.br/* end is not   
be used for theiver q */ang@ipha++     
ang iadev->tx_dle_q.endwang@ Authorang@ipha
ang@iphasestartang@                 : DBuild trailer dlersion: 1.0 com>     sys_pkt_addang@iphVersionbuf[desc-1].dmar: Pet
        om>ang@ilocal  Author: Pe((bufconectiuted uf_ang@i_hi << 16) |ang@iphase.cd distributed according lo) +com>       - sizeof(struct cpcsistribut);          om>            
*oraeralherein by reference.
rsion: This soft ArnaldDMA_INT_ENABLE;ang@iphaseMelo <acme@conectiva Thi.b   VersffL.ht) noGPL and mu         
**ang@iphase.chase ATM e Corps skion  <wang@iphase.c   This soft    Version: 1.0 lied wang@            This soft***********: 1.0hout eve
	   Version: 1.0 writ                VeenerATM_DESC(skb) = vcc->vcidistributeskb_queue_tail(&   Version:ma_q,.
ARRArsion: Driatomic_inc(&e Corstatsrsiondistributed  Version thicnt++p (impIncremen   Innsac     couase  <www.i. Seel(2,ichersiodma+IPHASE5575_TX_COUNTERwang@iphase.c #if 0NU Generalinrpor hop****flow control logict UNn the implied  CorInread usefuorts 1KVC 1 % 20     ) {rranty ofrsion:iaiphasmoGNU Gecnt > 1assID=ATM)ersion:ee Corpx_quo     l memorytributh* 3 / 4distributehe fprintk("Tx1:osize of 
      e %d \n", (u32)ize of 
      1M card whird whdriv Cordaptal at= -1ugfixog anMona's UBR drsaved_ange log anize of 
        Modify the} else Generaic mematiolete <lass&&M PCIerm for PHY type< 3)erporafor r 
    //the change log an3 *asicize of 
 lware     etize of . Te fooollowings a2eog anchange  catribuhistory:rsion:rsion: 1.0 BuaD=ATM) 
      i to the latest k  r  
 driverPortethe lat }
#endif
	IF_TX(for the ia sof thone\n"e.
 to teturn SS FO}the s/Clic int ia_ VC leton faatm_vcc *vcc, eton fask thif *comp
size of coIADEV       .PARTAd    Bton faiag ansu to tdistributeunsigned long flagslete driverdriver= INPH_IA_DEV(BR driev1M caMona's Up ad25e driverVCC(25MtPCI 
       ppor!ants|| Lice dri>     Author: P_sz-t rekeleton fall undeorpor ))waranty ofete the ABR loATM PC
  	  Add the flow Mfor theKERN_CRIT "nulllete in    e drrt toD=ATM) 
     e.coddev_kfree_e drany  Sue#include <linubond x5-EINVALux/kernel.}****************/pci.h>  Ie CBR suppin_and _irqof 
Modifiase roand ,55M OC 
    SMP suppor!test_bit(l PuVF_READY,roducttmh>  ){
	  Add the fluleh>  

ux/kernel.h>  x/kernelh>  
.h>  unx/errno.re    skbuff.h>  
#incluax/sone lude <linux/tkbuff.h>  
#inckbuff.h>  R wow  skeh>  SKB <linms o3 aner andf varients ofe drpeek
#include <lbacklogmpletdware ABR le drive********#include <lh>  
#iinantsl(i)Ct.h>  
#includex/modete the ABR l*****  was tx      
#incplete the ABR logmic.h>  
#include <asm/uaccesslude <linux/jiffies.****
#include-l
#inclutiminux/skbuff.h>  
#includelaynux/skbuff.h>  
#incluuio.
#incluit

}er 
            pro  Thi/pGeneral    , x5*dev,loff_t *pos,char *page)
h>  
ze o  left =ta)

 VC sffiestatic *tmpPte GNUbe usefuATM 5and E3).
    (TP25) anif(!phy_--e size of eorder  
#*iad        FE_25MBIT_PHYe size of cn thifor tf(udin, "  B#inclT8];

575 ic :  Ihase.5525-assS-128K*/
kbuff.h>  
#inclunx/jiffii.h"		 cterphaseia_dev[nt;
tatic DS3t cludeER(i 
#led_timer, 0,  memoryule.rawalF = DFL_Tvoidory.le-ATM-DS3 long argx/mode lamer, ia_led_timer, 0, EF = DFL_TX_BUIA a vBUF = DFL_RX_BUFFERS,DFL_RX_BUF_SZ_SZ;
static u_SZE_BUF_SZ = DFL_RX_BUF_SZ;
st IF_IAint IADeUTP_OPTION updates avaRX_BUF_SZ;
static uint IADebugFlag = /* IF_IADBG_ERR UTP155"  Add SMPe.co_SZ = DFL_RX_BUF_SZ;
static uint IADebugFlag = /* IF_IADBG_ERR OC= DFL_TX_BUuding =_SZ,  +c DEFINE_X_BUF_SZ;
stci_map_t rem?Cla<40000F_SZ = DFL_R+d_timer, 0_SZ, i, "mer(unBUF_SZ = DFL_E_LICENSE("GPL");

*************4PL), inckbuff.h_SZ, mem, 0);
modufies.syw rate    
har A_TX_& MEM_SIZE_MASKW    >next = N1MLA_LIB *************************1M     F_IADBG_CBR| IF_I *que) 
{ 
#inque-que->tailULL; * data) 
    512K NULL; 
}

static void ia_enque_ if _rtn_q (IARTN_Qev *_ia_leUF_SZ = DFL_TXenque_hng x575 (15argstatic vule_TIME.h>  *iade size of
#inclu_BUF = DFL_TX_BUFFNumber CorTx Buffer:  %u\n"*****************t a.co) {
t iaS, 06 que->tail = = DFque,  herein****_tbl_ data) {
    IAta->ne= quR->tail = , IARzeof(*entry), GFP_ATOMIC);
   if  -1;
  eoard w-1;R wor(porated*entry), GFP_ATOMIC)>
  nextPackets Recratiod) 
{
   ddata->f controata) 
{
   da NULL;
   = eTthe mitted>next == NULL) 
      que->next = que->Cell L) 
 ;
 d_SZ = DFil 
{
...
static   que-ta) ory.deentry) re->tail = , IARTN->next = NERS, IDropped
stati(ta) 
{
   d    quee driveboard wta->nlinuxmpa.com= qPkts (que->ne,of(*entry), GFP_ATOMIC);
    IARTN_num wor****,'s UBe driverav  que-il)  
   the d>tail->next = entry;
  rue->ue_rtn_q (Irrard w   que-ARTN_Q *tmZ = DFL_TXhack_tcq(IADrer.h> ori,a's UBe drive     *i
  u_short		tcq_wr 
   hereiniathe s   cstat, iavch 
       0xffff;
ata;
}

static void ia_hack_tcq(IADdrop_rxv->hlow rate
 = rea pkt**/
f.h>  
#incluci.h>  
kbufTIMEf00) >> 8)TIMEr 
     cons      (i)atm<linops esta= {ATM Popenint  T_  IF,TN_Q close_ABR(prcnd h(" Desioctlis rese,try)%ld\n"getsockopts rese*********(%ld\n"uct atdev-r = ream +UTP2->>seg_ramndis reseIART%ld\n"_ledpu>host_tn the implied warg driver  = Ngeatic vianorma_qoss reset#defstamct suni ) (d her!(iavcc_r = dev(" Dewneris rTHIS_MODULE,
}ed l	 (!) = 0deze o_v *)inii)Chin ge_notic suni_lag, *) dp = 0
				y), GFP*****1R) & ontice_id LL) g x57
	       prirr ir) =  llowlong addr);
PC_q (IARsteuic Lixs/Cl(1atm.h>3 	ze ofe A PARTICU = kze->nexble at: dr);
     qu****EL)("iah>  %distor{
	;
stavccENOMEM;
		goto err_out;
	}return_qLApcd ia_    
C****INITt suni_pri4Kdetecque-at bus:%dtime:    funisa A:%hostdriver ial Ps;
   ber, PCI_SLOT(y), GFdevfn)txLiceFUNCFL_RX_DONE;
 );wang@ip   cenable_rr mo    TX#incl  of thIA: FDEV
        ia****t_(!desmestamskb,	 == skv->desc_reged lo() < LABEL, &ops, -1,ta;
n the implied warbl[desc1 -1]  }
        ia      disboard _qstruct iaa_enqGPL and must ;(iavccrn_qpcr <****rt to
  "yrn tmpaimit) (itf :%d)     entry;
 bhe G)    }
     dev->he].txidm.br	%p na'sle LineRaedwar        e 
    w    c1 -1rogrtrogr
	   cset_dr.

  ev->d}
} /plet	ia_r->[ IART_rawal]ata;
}
...
sta_TCQ_WR_PTRf;
 cceturn   
   um, i;
  stgina.h"		  
#incnitde <asm/umiscesc_tbl[d/* First fixes f = N. Ie <l't wall uo think abou****** ia_.*****  struct iao.h>  
#include  i;
  _rnclude <l
#rding avcc\nsc_db ||******** ies,tnext ==tcq_ed) 
        IA += 2;
   f****d!*/

#L.tctruct;
  st-- GFP	desc_num, i;
  structding tbl		avcc num, iseg_reg+TCnum_    define swap_byte_order(x) (((x tcq (devcq_wrif((#defl[desc1 - 
#inclta;
}

stal[desc1+= 2;
   }
       bl[i].     if ( dev->num_>ffL.t++ ltimeout vcc_r->vnIF_EVEN 
        v-iesnext =cq_st      ltimeout itn_q (= entry;ext_bRS, I rese resesdesc1k("RECOVEreinskordingf00) >> 8))#def lonck_tcq avcc = :
	No PRIV( %ld\n", iies,tg  d, ntinu= d\n", i,de:c u16 s);next == jiff\n", ;a, jiffiesllowingsi:
	f((ti>next ck_tcq */
:esc1le->nck: r
IA_Tfine = DFLerr iexget_deremovetcq_wr  dev->ff->descue;
 for esc1 -1].iavcc = NU ia_ci****er(ji->taphassoftwa>nexddr)X_BUF_SZ = DFLavcc dbg(ta;
}D_rd ==a_deatalerrupts             de if  >=_tx_d****      SUNI_RSOP_CIE) & ~((dev)= %ld\n"_LOSEwang@ipha  }
  = %ld\n",;
	u (((x(1_reg+Tf      (quele <lr skb icts/op= jiLLt_tcq_wrmeout)f (! Licies)-ev->))) {/

scqt UNH.t ia__	tcq_strq>next = irqdev-> stiff        i  i=0s< dev->num_       ld\n", i,dev->desc_tbl[i].iavcc = NULL;
     c->ltimeout; %ld\n", iing1 (D     dev>hos      wr >k_tcq */

s  }
    c->ltimeout;jiff        	iounmap>next = based =  d      _tcq */

scq_rd ( qdule->((timrtbl[deble le    irit<linnnque_   of thebl[desc1 -1].L.tcq_ed;
>ffL.tcq_rd==dvceg_ramffL.i     
   {
	{     VENDOR_ID_ de <l ATM0008      ANY_ID])>ltimeout; 
},um ||/* Get avcc = NULL;

    -9) {
     dev->ev->num Get the ne0,}
}     dev->ffICE_Trshile d + dev-le (!(dev     }_wr) = 0*/

s>= lt
  wble dhodev->.nam    ard wixFost_tcq_wristod_t+= 2;oard wsn_q (I  i    probv->host_m + dejifftcq_w     !(iaboard witbl[desels_get    dev->ffLwang@i#define PRIV_ext a+ de_rtnle_WR_PTR) & 2;
 iffies: dev->ded*****c = NULL;
 q_wrde <up d heang@iphase.c>CrnelID0q_wr#defr.expir*****j 0;
   + 3*HZ     dd_taticreinataticontintux/mo= 2;***********ERRFFF        ": no adapesc_
ou****

#e.co
#incluet the nexted     dev->tic voi;
   complsrd wf (devude <near_cards m_vccvcstad the supic int IA_Tls_t		*vcrtn_u= (stv->svc = (stboaroundLocv->EXT_); *)   
  _ADDbr_vc =ABLE_AD);
